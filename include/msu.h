...
#ifndef DBG
#define DBG(S, X, F) \
    if ((S != NULL) && (S->EMT != NULL) && (*S->EMT >= X) && (*S->EMF & F))
#endif /* DBG */
...
#define sfALL                                                           \
    (sfEM | sfMeta | sfQueue | sfJob | sfResource | sfObj | sfFile |    \
...
enum SUDFEnum { sudfNONE = 0, sudfXML, sudfFT, sudfVerbose };
...
enum SUObjectList {
    ...
    suoSystem
}; /* grid scheduler/server */

enum SUKeyList { sukNONE = 0, sukName, sukUser, sukRsvID };
...
enum SUDataModeEnum { sudmNONE = 0, sudmHuman, sudmXML, sudmFlatText };
...
enum SUAttrEnum { suaNONE = 0, suaCKey };
...
enum { sustNONE = 0, sustTCP = (1 << 0), sustUDP = (1 << 1) };
