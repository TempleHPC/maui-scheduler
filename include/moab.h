...
enum MValModEnum { mvmByte = 0, mvmWord, mvmKilo, mvmMega, mvmGiga, mvmTera };
...
#define MNAT_GETLOADCMD \
    "/usr/bin/vmstat | /usr/bin/tail -1 | /bin/awk '{ print \\$16 }'"
...
enum MResType { mrtNONE = 0, mrtJob, mrtUser, mrtMeta };
...
enum MJobSearchEnum { mjsmBasic = 0, mjsmExtended, mjsmInternal };
...
enum MLimitTypes { mlNONE = 0, mlActive, mlIdle, mlSystem };

enum MJobNonEType { mjneNONE = 0, mjneIdlePolicy };

enum MEventFlagsEnum { mefNONE = 0, mefExternal };
...
enum { bnfNone = 0, bnfQuery, bnfStatus, bnfMessage, bnfSet };
...
enum MGroupAttrType { mgaNONE = 0, mgaClassWeight };

/* user object */

enum MUserAttrType { muaNONE = 0, muaMaxWCLimit };
...
#define QFUSER                                            \
    ((1 << mqfignJobPerUser) | (1 << mqfignNodePerUser) | \
     (1 << mqfignPSPerUser) | (1 << mqfignJobQueuedPerUser))
#define QFSYSTEM \
    ((1 << mqfignMaxProc) | (1 << mqfignMaxTime) | (1 << mqfignMaxPS))
...
enum { mjifNONE = 0, mjifIsExiting };

enum MHoldTypeEnum { mhNONE = 0, mhUser, mhSystem, mhBatch, mhDefer, mhAll };
...
typedef struct {
    ...
    int (*JobGetProximateTasks)(mjob_t *, mrm_t *, mnodelist_t, mnodelist_t,
                                long, int, char *, int *);
...
enum { mamtNONE, mamtQBANK, mamtRESD, mamtFILE, mamtGOLD };
...
enum MBalEnum { mccmNONE = 0, mccmExecute, mccmQuery };
...
enum MClientAttrEnum { mcltaNONE = 0, mcltaCSAlgo, mcltaTimeout, mcltaFlags };

enum MWCVioActEnum { mwcvaNONE = 0, mwcvaCancel, mwcvaPreempt };
...
enum MJobPrioEnum { mjpNONE = 0, mjpRelative, mjpAbsolute };

/* checkpoint modes */

enum MCKPtModeEnum { mckptResOnly = 0, mckptNonRes };
...
enum MFrameAttrType { mfraNONE = 0, mfraIndex, mfraName };
...
#define MNODEISUP(N)                                        \
    (((N->State == mnsIdle) || (N->State == mnsActive) ||   \
      (N->State == mnsBusy) || (N->State == mnsDraining) || \
      (N->State == mnsReserved))                            \
         ? TRUE                                             \
         : FALSE)
...
#define MJOBISSUSPEND(J) (((J)->State == mjsSuspended) ? TRUE : FALSE)
...
#define MJOBISSCOMPLETE(J) (((J->State == mjsCompleted)) ? TRUE : FALSE)
...
#define MSET(B, F) (B) |= (1 << (F))

#define MUNSET(B, F) (B) &= ~(1 << (F))
