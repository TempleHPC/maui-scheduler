        ...
        case 'j':
            ...
            switch (*sindex) {
                ...
                case svcResCreate:
                    ...
                    MUStrCpy(JobFeatureString, OptArg,
                             sizeof(JobFeatureString));
                ...
                case svcShowBackfillWindow:
                    ...
                    if (isdigit(OptArg[0])) {
                        ...
                        DPrint("INFO:     arg: '%s' comparison '%s' (%d)\n",
                               OptArg, tmpLine, index);

                        for (MIndex = 0; MComp[MIndex] != NULL; MIndex++) {
                            if (!strcmp(tmpLine, MComp[MIndex])) break;
                        }

                        if (MComp[MIndex] == NULL) MIndex = 0;

                        sscanf((OptArg + index), "%d", &Memory);
                    }
            ...
            switch (*sindex) {
                ...
                case svcJobShow:
                    ...
                    if (strlen(OptArg) >= sizeof(NodeRegEx)) {
                        fprintf(stderr,
                                "ERROR:    expression too long. (%d > %d)\n",
                                (int)strlen(OptArg), (int)sizeof(NodeRegEx));
        ...
        case 'p':
            ...
            switch (*sindex) {
                ...
                case svcRunJob:
                    ...
                    DBG(4, fCONFIG)
                    DPrint("INFO:     partition set to %s\n", ParName);
            ...
            switch (*sindex) {
                ...
                case svcResCreate:
                    ...
                    if (MUStringToE(OptArg, &BeginTime) != SUCCESS) {
                        fprintf(stderr,
                                "ERROR:    invalid StartTime specified, '%s'\n",
                                OptArg);
...

{
    ...
    switch (*sindex) {
        case svcShowStats:
            ...
            if (ObjectType == -1) ObjectType = mxoSched;

            if (ObjectID[0] == '\0') MUStrCpy(ObjectID, NONE, sizeof(ObjectID));

            if (ObjectType == mxoNode) {
                /* set default EvaluationDepth to MAX_MJOB */

                if (Flags < (1 << 1)) Flags |= (MAX_MJOB << 1);
            }

            sprintf(MsgBuffer, "%d %s %s %d", ObjectType, ObjectID, ParName,
                    Flags);
        ...
        case svcShowTasks:
            ...
            if (Args[1] != NULL)
                sprintf(MsgBuffer, "%s",
                        Args[1]); /* add argument to the buffer*/

            break;

        case svcShowQ:
            if (QueueMode == -1) QueueMode = 0;

            sprintf(MsgBuffer, "%d %s %d %s", QueueMode, ParName, Flags,
                    ShowUserName);
        ...
        case svcSetJobUserPrio:
            ...
            */
            ...
            DBG(2, fCONFIG)
            DPrint("INFO:     setting priority on job %s to %s (%d)\n",
                   Args[ArgIndex + 1], Args[ArgIndex], PrioMode);
        ...
        case svcSetJobQOS:
            ...
            */
            ...
            DBG(2, fCONFIG)
            DPrint("INFO:     setting QOS on job %s to %s\n",
                   Args[ArgIndex + 1], Args[ArgIndex]);
        ...
        case svcReleaseJobHold:
            ...
            if (HType == -1) HType = mhAll;

            DBG(2, fCONFIG)
            DPrint("INFO:     hold type %s selected\n", MHoldType[HType]);

            /* route through mjobctl */

            sprintf(MsgBuffer,
                    "<schedrequest action=\"modify\" attr=\"Hold\" "
        ...
        case svcSetJobDeadline:
            ...
            if (ArgCount == 3) {
                if (MUStringToE(Args[ArgIndex + 1], &BeginTime) != SUCCESS) {
                    fprintf(stderr,
                            "ERROR:    Invalid Job Deadline Specified\n");
            ...
            sprintf(MsgBuffer, "%s %ld\n", Args[ArgIndex], BeginTime);

            DBG(2, fCONFIG)
            DPrint("INFO:     deadline %ld set on job %s\n", BeginTime,
                   Args[ArgIndex + 1]);
        ...
        case svcSched:
            ...
            /* Format:  <CMD>  -k | -r <RESUMETIME> | -R | -s [<STOPITERATION>]
             * | -S [<STEPITERATION>] ] */
            ...
            sprintf(MsgBuffer, "%d %s", SchedMode, SchedArg);

            DBG(2, fCONFIG)
            DPrint("INFO:     sending schedctl command: '%s'\n", MsgBuffer);
        ...
        case svcDiagnose:
            ...
            if (DiagMode == mxoQueue) {
                if (PType == -1) PType = ptSOFT;

                sprintf(MsgBuffer, "%d %d %s", DiagMode, PType, ParName);
            } else {
                sprintf(MsgBuffer, "%d %d %s %s", DiagMode, Flags, ParName,
        ...
        case svcResCreate:
            ...
            if (BeginTime == 0) BeginTime = ClientTime;
            ...
            if (strlen(RegEx) >= sizeof(NodeRegEx)) {
                fprintf(stderr,
                        "ERROR:    regular expression too long. (%d > %d)\n",
                        (int)strlen(Args[ArgIndex]), (int)sizeof(NodeRegEx));

                exit(1);
            }

            sprintf(MsgBuffer,
                    "%ld %ld %ld %s %s %s %s %s %s %s %s %s %s %s %s %s %d %s",
                    ClientTime, BeginTime, EndTime, ParName, UserList,
                    GroupList, AccountList, ClassList, QOSList, ObjectID,
                    ResourceList, ChargeAccount, RegEx, FeatureString,
                    NodeSetString, ResFlags, TaskCount, JobFeatureString);
        ...
        case svcResShow:
            ...
            if (C.Format == mwpXML) Flags |= (1 << mcmXML);

            sprintf(MsgBuffer, "%d %s %d %s", ObjectType, ParName, Flags,
        ...
        case svcNodeShow:
            ...
            if (ArgCount == 2) {
                sprintf(MsgBuffer, "%s %d", Args[ArgIndex], Flags);
            } else {
                MCShowUsage(*sindex);
        ...
        case svcJobShow:
            ...
            if (ArgCount == 2) {
                sprintf(MsgBuffer, "%d %s %d %s %s", PType, Args[ArgIndex],
                        Flags, ResList,
        ...
        case svcShowEarliestDeadline:
            ...
            if (ArgCount == 2) {
                sprintf(MsgBuffer, "%s %s", Args[ArgIndex], ParName);
            } else {
                MCShowUsage(*sindex);
        ...
        case svcRunJob:
            ...
            if (ArgCount == 2) {
                sprintf(MsgBuffer, "%s %s %s %s", Args[ArgIndex],
                        MClientMode[Flags], ParName,
                        (NodeRegEx[0] != '\0') ? NodeRegEx : NONE);
            } else {
                MCShowUsage(*sindex);
        ...
        case svcMigrateJob:
            ...
            sprintf(MsgBuffer, "%s %s", Args[ArgIndex], Args[ArgIndex + 1]);
        ...
        case svcShowBackfillWindow:
            ...
            sprintf(MsgBuffer, "%s %s %s %s %ld %d %d %d %d %s %d %d %s %s %s",
                    UserName, Group, Account, ParName, Duration, NodeCount,
                    ProcCount, DMemory, Memory, MComp[MIndex], BFMode, Flags,
        ...
        case svcClusterShow:
            ...
            sprintf(MsgBuffer, "%s", (UseXML == TRUE) ? "XML" : "DEFAULT");
        ...
        case svcMJobCtl:
            ...
            switch (JobCtlMode) {
                case mjcmCancel:
                    ...
                    sprintf(MsgBuffer, "%s %s", MJobCtlCmds[JobCtlMode],
                            Args[ArgIndex]);
                ...
                case mjcmSubmit:
                    ...
                    sprintf(MsgBuffer, "%s [NONE] [NONE] %s",
                            MJobCtlCmds[JobCtlMode], Args[ArgIndex]);
        ...
        case svcMNodeCtl:
            ...
            sprintf(MsgBuffer, "%s %s %s", Args[ArgIndex], Args[ArgIndex + 1],
                    Args[ArgIndex + 2]);
        ...
        case svcMGridCtl:
            ...
            switch (MGridMode) {
                case mcCommit:
                    ...
                    sprintf(MsgBuffer, "%s %s", MGridCtlCmds[MGridMode],
                            Args[ArgIndex]);
                ...
                case mcList:
                    ...
                    sprintf(MsgBuffer, "%s %s %s %s", MGridCtlCmds[MGridMode],
                            Args[ArgIndex], Args[ArgIndex + 1],
                            (Args[ArgIndex + 2] == NULL) ? "ALL"
                                                         : Args[ArgIndex + 2]);
                ...
                case mcModify:
                    ...
                    sprintf(MsgBuffer, "%s %s %s", MGridCtlCmds[MGridMode],
                            Args[ArgIndex], Args[ArgIndex + 1]);
                ...
                case mcSet:
                    ...
                    sprintf(MsgBuffer, "%s %s %s %s", MGridCtlCmds[MGridMode],
                            Type, Args[ArgIndex], Args[ArgIndex + 1]);
                ...
                case mcQuery:
                    ...
                    sprintf(
                        MsgBuffer, "%s %s %s WIKI %s", MGridCtlCmds[MGridMode],
                        Type, RangeList,
...

{
    ...
    if (!(Mode & (1 << mcmRelative))) {
        fprintf(stdout, "%20s  %9s %18s %10s %4s %11s %11s %s%20s\n\n",
                ...
                "Start", "Duration", (Flags & (1 << mcmVerbose)) ? "     " : "",
                "StartTime");
    } else {
        ...
        while (MXMLGetChild(NE, "nres", &RTok, &RE) == SUCCESS) {
            ...
            if (Mode & (1 << mcmRelative)) {
                ...
                fprintf(stdout, "%11s %11s  %20s", StartLine, DurationLine,
                        (Flags & (1 << mcmVerbose)) ? ctime(&tmpTime)
...

{
    ...
    DBG(3, fUI) DPrint("Buffer: '%s'", Buffer);
...

{
    ...
    fprintf(stdout,
            "%18s%c %1s%s %3s %6s %8s %2s %9s %8s %8s %5s %s%11s  %19s\n\n",
            "JobName", ' ', "S", VLine, "Par", "Effic", "XFactor", "Q", "User",
            "Group", "MHost",
            (C.DisplayFlags & (1 << dfNodeCentric)) ? "Nodes" : "Procs",
    ...
    while ((ptr = MUStrTok(NULL, "\n", &TokPtr)) != NULL) {
        ...
        rc = sscanf(
            ptr, "%s %s %s %s %ld %ld %d %ld %s %lf %lf %s %d %s %ld %ld %ld",
            name, JState, UserName, GroupName, &stime, &qtime, &procs, &WCLimit,
            tmpQOS, &psdedicated, &psutilized, partition, &DMemory, MHostName,
            &JFlags, &AWallTime, &Priority);
        ...
        fprintf(
            stdout,
            "%18s%c %c%s %3.3s %6s %8.1f %2.2s %9s %8s %8s %5d %s%11s  %19s",
            name, JobMarker, JState[0], VLine, partition, effic, xfactor,
            tmpQOS, UserName, GroupName, MHostName, procs, SMPLine,
            MULToTString(WCLimit - AWallTime), MULToDString(&stime));
    } /* END while (ptr) */
...

{
    ...
    while ((ptr = strtok(NULL, "\n")) != NULL) {
        ...
        for (index = 0; MHoldType[index] != 0; index++) {
            if (Holds & (1 << index))
                fprintf(stdout, "%6s ", MHoldType[index]);
            else
                fprintf(stdout, "%6s ", "      ");
        }
...

{
    ...
    DBG(2, fUI)
    DPrint("INFO:     idle backlog: %8lu seconds (%8lu hours)\n", QueuePS,
           QueuePS / 3600);

    DBG(3, fUI)
    DPrint("INFO:     X: %lf  WCA: %lf  CA: %lf\n", AvgXFactor,
           WeightedCpuAccuracy, CpuAccuracy);
...

{
    ...
    fprintf(stdout, "%16s  %10s  (%12s)  %s\n", "Job Name", "CpuLimit",
            "StartSeconds", "DeadLine");

    while ((ptr = strtok(NULL, "\n")) != NULL) {
        sscanf(ptr, "%s %ld %ld", Name, &Deadline, &CpuLimit);

        if ((Deadline - Now) > 9999999) {
            fprintf(stdout, "%16s  %10ld  (NO DEADLINE)\n", Name, CpuLimit);
        } else {
            fprintf(stdout, "%16s  %10ld  (%12ld)  %s", Name, CpuLimit,
                    (Deadline - Now - CpuLimit), MULToDString(&Deadline));
        }
    }
...

{
    ...
    switch (QueueMode) {
        ...
        default:
            ...
            DBG(1, fUI)
            DPrint("ALERT:    unexpected showq type, %d\n", QueueMode);
...

{
    ...
    while ((ptr = strtok(NULL, "\n")) != NULL) {
        ...
        rc =
            sscanf(ptr, "%s %s %ld %ld %d %ld %s %d %d", name, UserName, &stime,
                   &qtime, &procs, &WCLimit, tmpQOS, &state, &priority);
        ...
        fprintf(stdout, "%-18s %8s %10s %5d %11s  %19s", name, UserName,
                MJobState[state], procs, MULToTString(WCLimit - (Now - stime)),
                MULToDString(&stime));
    } /* END while (ptr) */
    ...
    while ((ptr = strtok(NULL, "\n")) != NULL) {
        ...
        fprintf(stdout, "%-18s %8s %10s %5d %11s  %19s", name, UserName,
                MJobState[state], procs, MULToTString(WCLimit),
                MULToDString(&qtime));
    }
    ...
    while ((ptr = strtok(NULL, "\n")) != NULL) {
        ...
        fprintf(stdout, "%-18s %8s %10s %5d %11s  %19s", name, UserName,
                (state > 0) ? MJobState[state] : "-", procs,
                MULToTString(WCLimit), MULToDString(&qtime));
    } /* END while (ptr) */
