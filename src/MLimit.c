...

{
    ...
    DBG(4, fSCHED) DPrint("%s(%s)\n", FName, MAList[ePartition][P->Index]);
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        ...
        if ((JobWCX >= 0) && (J->WCLimit > 0) && (MSched.Time > J->StartTime) &&
            ((MSched.Time - J->StartTime) >
             (J->WCLimit + J->SWallTime + JobWCX))) {
            DBG(2, fCORE)
            DPrint(
                "ALERT:    job '%s' in state '%s' has exceeded its wallclock "
                "limit (%ld+S:%ld) by %s (job will be cancelled)\n",
                J->Name, MJobState[J->State], J->WCLimit, J->SWallTime,
                MULToTString((MSched.Time - J->StartTime) -
                             (J->WCLimit + J->SWallTime)));

            sprintf(tmpMsg,
                    "JOBWCVIOLATION:  job '%s' in state '%s' has exceeded its "
                    "wallclock limit (%ld) by %s (job will be cancelled)  job "
                    "start time: %s",
                    J->Name, MJobState[J->State], J->WCLimit,
            ...
            if (MSched.WCViolAction == mwcvaPreempt) {
                if (MJobPreempt(J, NULL, -1, NULL, NULL) == FAILURE) {
                    DBG(2, fCORE)
                    DPrint(
                        "ALERT:    cannot preempt job '%s' (job exceeded "
                        "wallclock limit)\n",
                        J->Name);
                }
            } else {
                ...
                /* detect if any of the nodes are down. we cannot be certain,
                 * which one
                 * is the mother superior, so we assume if one is down it is the
                 * one */
                ...
                if (isDown)
                    MRMJobCancel(J,
                                 "delpurge=1\nMAUI_INFO:  job exceeded "
                                 "wallclock limit\n",
                                 NULL);
                else
                    MRMJobCancel(
                        J, "MAUI_INFO:  job exceeded wallclock limit\n", NULL);
            }
        } /* END if ((JobWCX >= 0) && ...) */
        ...
        if ((P->ResourceLimitPolicy[mrProc] != mrlpNONE) &&
            ...
            DBG(3, fSCHED)
            DPrint(
                "INFO:     job %s exceeds requested proc limit (%.2lf > "
                "%.2lf)\n",
                J->Name, (double)RQ->URes.Procs / 100.0,
            ...
            JobExceedsLimits = TRUE;
                   ...
                   (RQ->DRes.Mem > 0) && (RQ->URes.Mem > RQ->DRes.Mem)) {
            DBG(3, fSCHED)
            DPrint(
                "INFO:     job %s exceeds requested memory limit (%d > %d)\n",
            ...
            JobExceedsLimits = TRUE;
                   ...
                   (RQ->DRes.Swap > 0) && (RQ->URes.Swap > RQ->DRes.Swap)) {
            DBG(3, fSCHED)
            DPrint("INFO:     job %s exceeds requested swap limit (%d > %d)\n",
                   J->Name, RQ->URes.Swap, RQ->DRes.Swap);
            ...
            JobExceedsLimits = TRUE;
                   ...
                   (RQ->DRes.Disk > 0) && (RQ->URes.Disk > RQ->DRes.Disk)) {
            DBG(3, fSCHED)
            DPrint("INFO:     job %s exceeds requested disk limit (%d > %d)\n",
                   J->Name, RQ->URes.Disk, RQ->DRes.Disk);
        ...
        switch (P->ResourceLimitPolicy[VRes]) {
            ...
            case mrlpBlockedWorkloadOnly:
                ...
                /* does job reservation exist at Now + MSched.Iteration for node
                 * utilized by job? */
            ...
            default:
                ...
                DBG(1, fSCHED)
                DPrint("ALERT:    unexpected limit violation policy %d\n",
        ...
        sprintf(tmpMsg,
                "JOBRESVIOLATION:  job '%s' in state '%s' has exceeded %s "
                "resource limit (%d > %d) (action %s will be taken)  job start "
                "time: %s",
                J->Name, MJobState[J->State], MResourceType[VRes], VVal, VLimit,
        ...
        switch (P->ResourceLimitViolationAction[VRes]) {
            ...
            case mrlaCancel:
                ...
                rc = MRMJobCancel(
                    J, "job violates resource utilization policies", NULL);
            ...
            default:
                ...
                DBG(1, fSCHED)
                DPrint("ALERT:    unexpected limit violation action %d\n",
        ...
        DBG(1, fSCHED)
        DPrint("ALERT:    limit violation action %s %s\n",
               MPolicyAction[P->ResourceLimitViolationAction[VRes]],
