...
int MBFValue(
    ...
    mjob_t **JobList, int PolicyLevel, nodelist_t NodeList, time_t MaxDuration,
    int NodesAvailable, int ProcsAvailable, int PIndex)

{
    ...
    /* if resources remain, backfill jobs which cannot run due to walltime
     * constraints */
...
int MBFPreempt(
    ...
    mjob_t *JobList[], int PolicyLevel, nodelist_t NodeList, time_t MaxDuration,
    int NodesAvailable, int ProcsAvailable, mpar_t *P)

{
    ...
    DBG(2, fSCHED)
    DPrint("%s(%s,%s,%ld,%d,%d,%s)\n", FName,
           (JobList != NULL) ? "JobList" : "NULL",
           (NodeList != NULL) ? "NodeList" : "NULL", MaxDuration,
           NodesAvailable, ProcsAvailable, P->Name);
    ...
    while (1) {
        ...
        for (jindex = 0; JobList[jindex] != NULL; jindex++) {
            ...
            MJobGetBackfillPriority(J, MaxDuration, 0, &JobBFPriority, NULL);
        ...
        if (MJobCheckLimits(J, PolicyLevel, P, (1 << mlActive), NULL) ==
            FAILURE) {
            DBG(4, fSCHED)
            DPrint("INFO:     job '%s' rejected by active policy\n", J->Name);
        ...
        if (MJobSelectMNL(J, P, NodeList, MNodeList, NodeMap, MBNOTSET) ==
            FAILURE) {
            DBG(4, fSCHED)
            DPrint("INFO:     cannot select procs for job '%s'\n", J->Name);
        ...
        if (MJobAllocMNL(J, MNodeList, NodeMap, 0, P->NAllocPolicy,
                         MSched.Time) == FAILURE) {
            DBG(4, fSCHED)
            DPrint("INFO:     cannot allocate resources for job '%s'\n",
                   J->Name);
...
int MBFFirstFit(
    ...
    mjob_t *BFQueue[], int PolicyLevel, nodelist_t BFNodeList, time_t Duration,
    int BFNodeCount, int BFProcCount, mpar_t *P)

{
    ...
    DBG(2, fSCHED)
    DPrint("%s(BFQueue,%d,BFNodeList,%ld,%d,%d,%s)\n", FName, PolicyLevel,
           Duration, BFNodeCount, BFProcCount, P->Name);
    ...
    for (jindex = 0; BFQueue[jindex] != NULL; jindex++) {
        if ((ChunkingEnabled == TRUE) && (ChunkingActive == FALSE)) {
            ...
            if ((J != NULL) && (MJOBISACTIVE(J) == FALSE) &&
        ...
        for (sindex = 0; RQ->TaskRequestList[sindex] > 0; sindex++) {
            ...
            SpecWCLimit = (J->SpecWCLimit[sindex] > 0) ? J->SpecWCLimit[sindex]
                                                       : J->SpecWCLimit[0];

            if (sindex != 0) MJobUpdateResourceCache(J, sindex);
            ...
            if ((ChunkingActive == TRUE) && (PC < MPar[0].BFChunkSize)) {
                ...
                DBG(5, fSCHED)
                DPrint(
                    "INFO:     %s:  job %s.%d blocked by chunking (P: %d T: "
                    "%ld)\n",
                    FName, J->Name, sindex, PC, SpecWCLimit);

                continue;
            }

            DBG(5, fSCHED)
            DPrint("INFO:     %s:  evaluating job %s.%d (P: %3d T: %ld)\n",
                   FName, J->Name, sindex, PC, SpecWCLimit);
            ...
            if (MJobCheckLimits(J, PolicyLevel, P, (1 << mlActive), NULL) ==
                ...
                DBG(8, fSCHED)
                DPrint(
                    "INFO:     job %s.%d does not meet active fairness "
                    "policies\n",
                    J->Name, sindex);
            ...
            if (MJobSelectMNL(J, P, BFNodeList, MNodeList, NodeMap, MBNOTSET) ==
                FAILURE) {
                DBG(8, fSCHED)
                DPrint(
                    "INFO:     cannot locate available resources for job "
                    "%s.%d\n",
                    J->Name, sindex);
        ...
        if (BestSIndex >= 0) {
            ...
            if (sindex > 1) {
                if (MJobAllocMNL(J, BestMNodeList, BestNodeMap, 0,
                                 P->NAllocPolicy, MSched.Time) == FAILURE) {
                    DBG(0, fSCHED)
                    DPrint(
                        "ERROR:    %s:  cannot allocate nodes for job %s.%d\n",
                        FName, J->Name, BestSIndex);
            ...
            if (MJobStart(J) == FAILURE) {
                DBG(0, fSCHED)
                DPrint("ERROR:    %s:  cannot start job %s.%d\n", FName,
                       J->Name, BestSIndex);
    ...
    DBG(2, fSCHED)
    DPrint(
        "INFO:     partition %s nodes/procs available after %s: %d/%d (%d jobs "
        "examined)\n",
        P->Name, FName, P->IdleNodes, P->ARes.Procs, jindex);
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (N != NULL) N->State = NSList[nindex];
    } /* END for (nindex) */
...

{
    ...
    for (nindex = 0; NL[nindex].N != NULL; nindex++) {
        ...
        if (N != NULL) N->State = NSList[NL[nindex].N->Index];
    } /* END for (nindex) */
...
int MBFGetWindow(
    ...
    char *ClassString, char *FeatureString, /* I */
    char *QOSName, char *Msg) /* O:  descriptive message            */

{
    ...
    DBG(3, fSCHED)
    DPrint(
        "%s(BFNodeCount,BFTaskCount,BFNodeList,BFTime,%ld,%s,%s,%s,%s,'%s "
        "%d',%lu,DRes,%s,%s,%s,%s)\n",
        FName, SMinTime, P->Name, UName, GName,
        (AName != NULL) ? AName : "NULL", MComp[MemCmp], Memory, WCLimit,
        (ClassString != NULL) ? ClassString : "NULL",
        (FeatureString != NULL) ? FeatureString : "NULL",
        (QOSName != NULL) ? QOSName : "NULL", (Msg == NULL) ? "NULL" : "Msg");
    ...
    if (MJobSetCreds(J, UName, GName, AName) == FAILURE) {
        DBG(1, fCORE)
        DPrint(
            "ALERT:    cannot create authentication for backfill pseudo-job "
            "(U: %s  G: %s  A: %s)\n",
            UName, GName, (AName == NULL) ? "NULL" : AName);
    ...
    MClassFind((ClassString != NULL) ? ClassString : (char *)ALL, &J->Cred.C);

    MJobGetAccount(J, &tmpADef);

    if ((AName == NULL) || !strncmp(AName, "ALL", 3) ||
    ...
    for (; nindex < MAX_MNODE; nindex++) {
        ...
        if (((N->State != mnsIdle) && (N->State != mnsActive)) ||
            ...
            DBG(6, fSCHED)
            DPrint(
                "INFO:     node %s not considered for backfill (State: "
                "%s/EState: %s)\n",
                N->Name, MNodeState[N->State], MNodeState[N->EState]);

            if (Msg != NULL) {
                if ((N->State != mnsIdle) && (N->State != mnsActive)) {
                    sprintf(&Msg[strlen(Msg)],
                            "node %s is unavailable (state '%s')\n", N->Name,
                            MNodeState[N->State]);
                } else {
                    sprintf(&Msg[strlen(Msg)],
                            "node %s is unavailable (expected state '%s')\n",
                            N->Name, MNodeState[N->EState]);
                }
            }
        ...
        if ((N->PtIndex != P->Index) && (P->Index != 0)) {
            DBG(6, fSCHED)
            DPrint(
                "INFO:     node %s not considered for backfill (invalid "
                "partition)\n",
                N->Name);

            if (Msg != NULL)
                sprintf(&Msg[strlen(Msg)],
                        "node %s is unavailable (partition '%s')\n", N->Name,
                        MPar[N->PtIndex].Name);
        ...
        if (MJobCheckNRes(J, N, RQ, MSched.Time, &TC, 1.0, &RIndex, &Affinity,
                          NULL, TRUE) == FAILURE) {
            /* node does not meet time/resource requirements */

            if (Msg != NULL) {
                sprintf(&Msg[strlen(Msg)],
                        "node %s does not meet requirements (%s)\n", N->Name,
                        MAllocRejType[RIndex]);
            }
        ...
        if (MJobGetNRange(J, RQ, N, MSched.Time, &TC, &AvailableTime, &Affinity,
                          NULL, BRes) == FAILURE) {
...
#endif /* __MLONGESTBFWINDOWFIRST */
        {
            if (Msg != NULL) {
                if (AvailableTime < (MAX_MTIME >> 1)) {
                    sprintf(&Msg[strlen(Msg)],
                            "node %s is blocked by reservation %s in %s\n",
                            N->Name, BRes, MULToTString(AvailableTime));
                } else {
                    sprintf(&Msg[strlen(Msg)],
                            "node %sx%d is available with no timelimit\n",
                            N->Name, TC);
                }
            }
        ...
        {
            DBG(5, fSCHED)
            DPrint("INFO:     node %s found with best time %ld\n", N->Name,
                   AvailableTime);

            ETime = AvailableTime;
        } else {
            DBG(5, fSCHED)
            DPrint("INFO:     node %s found with available time %ld\n", N->Name,
                   AvailableTime);
        }
...
#ifdef __MLONGESTBFWINDOWFIRST
            {
                if (tmpL >= SMinTime) continue;

                if (tmpL > ETime) {
                    DBG(5, fSCHED)
                    DPrint(
                        "INFO:     reservation %s found with best time %ld\n",
                        R->Name, tmpL);
    ...
    if (BFTime != NULL) *BFTime = ETime;

    if (BFNodeCount != NULL) *BFNodeCount = NodeCount;

    if (BFTaskCount != NULL) *BFTaskCount = TaskCount;
    ...
    DBG(3, fSCHED)
    DPrint(
        "INFO:     backfill window:  time: %s  nodes: %3d  tasks: %3d  "
        "mintime: %5ld (idle nodes: %d)\n",
        MULToTString(*BFTime), *BFNodeCount, *BFTaskCount, SMinTime,
        MPar[0].IdleNodes);
...

{
    ...
    DBG(2, fSCHED)
    DPrint("%s(BFQueue,BFNodeList,%ld,%d,%d,%s)\n", FName, Duration,
           BFNodeCount, BFProcCount, P->Name);
    ...
    while (1) {
        ...
        for (jindex = 0; LBFQueue[jindex] != NULL; jindex++) {
            ...
            DBG(3, fSCHED)
            DPrint(
                "INFO:     attempting BF backfill with job '%15s' (p: %3d t: "
                "%ld)\n",
                J->Name, PC, J->WCLimit);
            ...
            if (MJobCheckLimits(J, PolicyLevel, P, (1 << mlActive), NULL) ==
                ...
                DBG(8, fSCHED)
                DPrint(
                    "INFO:     job %s does not meet active fairness policies\n",
                    J->Name);
            ...
            if (MJobSelectMNL(J, P, BFNodeList, MNodeList, NodeMap, MBNOTSET) !=
                ...
                DBG(4, fSCHED)
                DPrint("INFO:     cannot select tasks for job '%s'\n", J->Name);
            ...
            DBG(2, fSCHED)
            DPrint(
                "INFO:     located job '%s' in %s (size: %d duration: %ld)\n",
                J->Name, FName, PC, J->WCLimit);
        } /* END for (jindex) */
        ...
        if (MJobSelectMNL(J, P, BFNodeList, MNodeList, NodeMap, MBNOTSET) ==
            FAILURE) {
            DBG(0, fSCHED)
            DPrint("ERROR:    cannot get tasks selected in %s\n", FName);
        ...
        if (MJobAllocMNL(J, MNodeList, NodeMap, 0, P->NAllocPolicy,
                         MSched.Time) == FAILURE) {
            DBG(1, fSCHED)
            DPrint("ERROR:    cannot allocate nodes for job '%s' in %s\n",
                   J->Name, FName);
        ...
        if (MJobStart(J) == FAILURE) {
            DBG(1, fSCHED)
            DPrint("ERROR:    cannot start job '%s' in %s\n", J->Name, FName);
    ...
    DBG(2, fSCHED)
    DPrint(
        "INFO:     partition %s nodes/procs available after backfill: %d/%d\n",
        P->Name, P->IdleNodes, P->ARes.Procs);
...

{
    ...
    DBG(4, fSCHED)
    DPrint("INFO:     greedy backfill procs : %03d  time: %06ld\n", BFProcCount,
           Duration);
    ...
    DBG(4, fSCHED) {
        ...
        for (index = 0; LBFQueue[index] != NULL; index++) {
            fprintf(mlog.logfp, "[%s]", LBFQueue[index]->Name);
        }
    ...
    while (scount++ < GP->BFMaxSchedules) {
        ...
        if ((BestValue > 0) && (BestList[0] == NULL)) {
            DBG(0, fSCHED)
            DPrint(
                "ERROR:    BestVal %d achieved but schedule is empty (%p, %p, "
                "...)\n",
                BestValue, BestList[0], BestList[1]);
        ...
        for (jindex = StartIndex; LBFQueue[jindex] != NULL; jindex++) {
            ...
            if (scount >= GP->BFMaxSchedules) {
                DBG(1, fSCHED)
                DPrint(
                    "ALERT:    max backfill schedules reached in %s (%d) on "
                    "iteration %d\n",
                    FName, GP->BFMaxSchedules, MSched.Iteration);
            ...
            DBG(5, fSCHED) {
                if (sindex > 0) {
                    ...
                    for (index = 0; index < sindex; index++) {
                        fprintf(mlog.logfp, "[%s]", BFList[index]->Name);
                    } /* END for (index) */
                ...
                DBG(4, fSCHED)
                DPrint("INFO:     checking S[%02d] '%s'  (p: %03d t: %06ld)\n",
                       jindex, J->Name, PC, J->WCLimit);
            }

            if ((J->State != mjsIdle) || (J->EState != mjsIdle)) {
                DBG(6, fSCHED)
                DPrint("INFO:     job %s is not idle\n", J->Name);
            ...
            DBG(4, fSCHED)
            DPrint("INFO:     feasible backfill MJob[%03d] (P: %3d T: %ld)\n",
                   J->Index, PC, J->WCLimit);
            ...
            if (MJobCheckLimits(J, PolicyLevel, P, (1 << mlActive), NULL) ==
                ...
                if (sindex == 0) LBFQueue[jindex] = (mjob_t *)1;
            ...
            DBG(6, fSCHED)
            DPrint("INFO:     reservation added for Job[%03d] '%s'\n", sindex,
                   J->Name);
            ...
            DBG(4, fSCHED)
            DPrint(
                "INFO:     located job '%s' for greedy backfill (size: %d "
                "length: %ld)\n",
                J->Name, PC, J->WCLimit);
        } /* END for (jindex) */
        ...
        switch (GP->BFMetric) {
            case mbfmPS:
                ...
                for (index = 0; index < sindex; index++) {
                    BFValue += MJobGetProcCount(BFList[index]) *
                               BFList[index]->WCLimit;
                }
        ...
        if (BFValue > BestValue) {
            ...
            DBG(1, fSCHED) {
                DBG(1, fSCHED)
                DPrint(
                    "INFO:     improved list found by greedy in %d searches "
                    "(utility: %d  procs available: %d)\n",
                    scount, BestValue, BFProcCount - SPC);
                ...
                for (index = 0; BestList[index] != NULL; index++) {
                    ...
                    DBG(1, fSCHED)
                    DPrint(
                        "INFO:     %02d:  job '%s' procs: %03d time: %06ld\n",
                        index, J->Name, MJobGetProcCount(J), J->WCLimit);
                }
            }
        ...
        if (sindex == 0) {
            ...
            DBG(6, fSCHED)
            DPrint("INFO:     releasing reservation for Job %d '%s'\n", sindex,
                   J->Name);
    ...
    DBG(3, fSCHED) {
        DBG(1, fSCHED)
        DPrint(
            "INFO:     final list found by greedy in %d searches (%d of %d "
            "procs/utility: %d)\n",
            scount, SPC, BFProcCount, BestValue);

        for (index = 0; BestList[index] != NULL; index++) {
            J = BestList[index];

            DBG(1, fSCHED)
            DPrint("INFO:     S[%02d]  job %16s   procs: %03d time: %06ld\n",
                   index, J->Name, MJobGetProcCount(J), J->WCLimit);
        }
    }
    ...
    for (index = 0; BestList[index] != NULL; index++) {
        ...
        if (MJobCheckLimits(J, PolicyLevel, P, (1 << mlActive), NULL) ==
            ...
            DBG(1, fSCHED) {
                DBG(1, fSCHED)
                DPrint(
                    "ERROR:    scheduling failure %3d in %s (policy "
                    "violation/no reservation)  iteration: %4d\n",
                    ++ReservationMisses, FName, MSched.Iteration);
        ...
        if (MJobSelectMNL(J, P, BFNodeList, MNodeList, NodeMap, MBNOTSET) ==
            ...
            DBG(1, fSCHED) {
                DBG(1, fSCHED)
                DPrint(
                    "ERROR:    cannot get tasks in %s on (ERR: %d/no "
                    "reservation/iteration %d)\n",
                    FName, ++ReservationMisses, MSched.Iteration);
        ...
        if (MJobAllocMNL(J, MNodeList, NodeMap, 0, P->NAllocPolicy,
                         MSched.Time) == FAILURE) {
            DBG(1, fSCHED)
            DPrint("ERROR:    cannot allocate resources to job %s\n", J->Name);
    ...
    if (Failure == TRUE) {
        ...
        if (MBFGetWindow(&tmpNodes, &tmpProcs, tmpNodeList, &tmpTime, 0, P, ALL,
                         ALL, ALL, 0, 0, 1, &DRes, NULL, NULL, NULL,
                         NULL) == SUCCESS) {
            MBFBestFit(BFQueue, PolicyLevel, tmpNodeList, tmpTime, tmpNodes,
                       tmpProcs, P);
        }
    } /* END if (Failure == TRUE) */

    DBG(2, fSCHED)
    DPrint("INFO:     partition %s nodes/procs available after %s: %d/%d\n",
           P->Name, FName, P->IdleNodes, P->ARes.Procs);
