...

{
    ...
    sscanf(Buf, "%s %s %ld", tmpHeader, GName, &CkTime);
    ...
    if (GS == NULL) {
        if (MGroupAdd(GName, &G) != SUCCESS) {
            DBG(5, fCKPT)
            DPrint("ALERT:    cannot load CP group '%s'\n", GName);
...
void MGroupGetFSGroups() {
    ...
    for (gindex = 0; gindex < MAX_MGROUP + MAX_MHBUF; gindex++) {
        ...
        if (G->F.FSTarget > 0) {
            DBG(5, fFS)
            DPrint("Found FSGroup %s(%f) with key %d \n", G->Name,
                   G->F.FSTarget, G->Key);
...
/* HvB */
int MGroupSecondary(char *UName, /* I */
                    mjob_t *J)   /* O */

{
    ...
    */
    ...
    while (FSGroupKeys[i] != -1) {
        DBG(5, fFS)
        DPrint("Checking if user(%s)  is member of group(%s)\n", UName,
               MGroup[FSGroupKeys[i]].Name);
        if ((sec_grp = getgrnam(MGroup[FSGroupKeys[i]].Name)) != NULL) {
            gindex = 0;
            while (sec_grp->gr_mem[gindex]) {
                if (!strcmp(sec_grp->gr_mem[gindex], UName)) {
                    DBG(5, fFS)
                    DPrint("Setting Group credentials for user %s to %s\n",
                           UName, sec_grp->gr_name);
                    if (MGroupAdd(sec_grp->gr_name, &J->Cred.G) == FAILURE) {
                        DBG(1, fPBS)
                        DPrint(
                            "ERROR:    cannot add secondary group for job %s "
                            "(Name: %s)\n",
                            J->Name, sec_grp->gr_name);
...

{
    ...
    DBG(1, fSTRUCT)
    DPrint("ALERT:    group table overflow.  cannot add %s\n", GName);
...
int MGroupInitialize(
    ...
    mgcred_t *G, char *GName)

{
    if ((G == NULL) || (GName == NULL) || (GName[0] == '\0')) {
        return (FAILURE);
    }
...
int MGroupToXML(
    ...
    mgcred_t *G, mxml_t **EP, int *SAList)

{
    ...
    for (aindex = 0; AList[aindex] != -1; aindex++) {
        if ((MCredAToString((void *)G, mxoGroup, AList[aindex], tmpString, 0) ==
             FAILURE) ||
            (tmpString[0] == '\0')) {
            continue;
        }
...

{
    ...
    if (G == NULL) {
        ...
        MUSNPrintF(&BPtr, &BSpace,
                   "%-12s %8s %12s %12s %12s%s %20s %6s %7s\n\n", "Name",
        ...
        MUStrCpy(GLLine, MCredShowAttrs(&G->L.AP, G->L.IP, NULL, NULL, NULL,
                                        &G->F, 0, (1 << mcsLimits)),
                 sizeof(GLLine));
        ...
        MUSNPrintF(
            &BPtr, &BSpace, "%-12s %8ld %12s %12s %12s%c %20s %6.2lf %7s\n",
            G->Name, G->F.Priority, FlagLine,
            ((mqos_t *)G->F.QDef) != NULL ? ((mqos_t *)G->F.QDef)->Name : NONE,
            (QALLine[0] != '\0') ? QALLine : NONE, QALChar,
            (G->F.PAL[0] == 0) ? NONE : MUListAttrs(ePartition, G->F.PAL[0]),
            G->F.FSTarget, (GLLine[0] != '\0') ? GLLine : NONE);
        ...
        if (Mode & (1 << mcmVerbose)) {
            ...
            MCredConfigLShow((void *)G, mxoGroup, TRUE, -1, tmpLine);
...

{
    ...
    if ((G == NULL) || (Value == NULL) || (Value[0] == '\0')) {
        return (FAILURE);
    }
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MGroupAttr, &aindex, NULL, TRUE, NULL,
