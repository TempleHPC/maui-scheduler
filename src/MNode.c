...
int __MNLATimeCompLToH(
    ...
    _mpnodealloc_t *a, _mpnodealloc_t *b)
...

{
    ...
    N->R =
        (mres_t **)((nr != NULL) ? nr : calloc(1, sizeof(mres_t *) *
                                                      (MSched.ResDepth + 1)));

    N->RE =
        (mre_t *)((nre != NULL) ? nre
                                : calloc(1, sizeof(mre_t) *
                                                ((MSched.ResDepth << 1) + 1)));
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if ((N == NULL) || (N->Name[0] == '\0') || (N->Name[0] == '\1')) {
    ...
    if (nindex >= MAX_MNODE) {
        ...
        DBG(1, fSTRUCT)
        DPrint("WARNING:  node buffer overflow in %s()\n", FName);

        DBG(1, fSTRUCT)
        DPrint("INFO:     node table is full (cannot add node '%s')\n", NName);
...

{
    ...
    while (MCfgGetSVal(head, &ptr, MPARM_NODECFG, tmpSName, NULL, Value,
                       sizeof(Value), 0, NULL) != FAILURE) {
    ...
    if (strcmp(tmpSName, tmpLName)) {
        ...
        while (MCfgGetSVal(head, &ptr, MPARM_NODECFG, tmpLName, NULL, Value,
                           sizeof(Value), 0, NULL) != FAILURE) {
            /* node attribute located */
...

{
    ...
    if ((N == NULL) || (Value == NULL) || (Value[0] == '\0')) {
        return (FAILURE);
    }
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MNodeAttr, &aindex, AttrArray, TRUE,
                      NULL, ValLine, MAX_MNAME) == FAILURE) {
...

                {
                    ...
                    case mnaMaxProc:
                        ...
                        if (AttrArray[0] == '\0') {
                            ...
                            if (((otype = MUStrTok(AttrArray, ":", &TokPtr)) ==
                                 NULL) ||
                                ((oid = MUStrTok(NULL, ":", &TokPtr)) ==
                                 NULL)) {
                                break;
                            }
                            ...
                            if (!strcmp(otype, "CLASS")) {
                                ...
                                if (N->MaxProcPerClass == NULL)
                                    N->MaxProcPerClass = (int *)calloc(
                                        1, sizeof(int) * MAX_MCLASS);
...

{
    ...
    if (N->FrameIndex > 0) MFrame[N->FrameIndex].NodeCount--;
...

{
    ...
    if (N->R != NULL) free(N->R);

    if (N->RE != NULL) free(N->RE);

    if (N->RC != NULL) free(N->RC);
...

{
    ...
    DBG(4, fSCHED)
    DPrint("%s(%s,%s,%s,%s,%s,%s)\n", FName, (J != NULL) ? J->Name : "NULL",
           (FNL != NULL) ? "FNL" : "NULL", (INL != NULL) ? "INL" : "NULL",
           (P != NULL) ? P->Name : "NULL",
    ...
    if (FNL == NULL) {
        ...
        if ((MSched.Time > IdleListMTime[P->Index]) ||
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     refreshing IdleList for partition %s\n", P->Name);
            ...
            for (index = 0; index < MAX_MNODE; index++) {
                ...
                if (((N->State != mnsIdle) && (N->State != mnsActive)) ||
                    ...
                    DBG(6, fSCHED)
                    DPrint(
                        "INFO:     node '%s' is not idle.  (state: %s  exp: "
                        "%s)\n",
                ...
                IdleNodeCache[P->Index][nindex].TC =
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     nodes placed in INL[%s] list: %3d (procs: %d)\n",
                   P->Name, nindex, tmpTC);
        ...
        for (index = 0; index < IdleNodeCount[P->Index]; index++) {
            ...
            memcpy(&INL[nindex], &IdleNodeCache[P->Index][index],
                   sizeof(INL[nindex]));
        ...
        for (index = 0; FNL[index].N != NULL; index++) {
            ...
            if (((N->State != mnsIdle) && (N->State != mnsActive)) ||
                ...
                DBG(5, fSCHED)
                DPrint(
                    "INFO:     node %s is not available for idle list (state: "
                    "%s  dprocs: %d)\n",
                    N->Name, MNodeState[N->State], N->DRes.Procs);
            ...
            if (MNodeCheckPolicies(J, N, MSched.Time, &tmpI) == FAILURE) {
                DBG(5, fSCHED)
                DPrint("INFO:     job %s violates policies on idle node %s\n",
                       J->Name, N->Name);
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     idle node %sx%d located (D: %d)\n", N->Name, tmpI,
                   N->DRes.Procs);
    ...
    if (TaskCount != NULL) *TaskCount = tmpTC;

    if (NodeCount != NULL) *NodeCount = tmpNC;

    DBG(4, fSCHED)
    DPrint(
        "INFO:     idle resources (%d tasks/%d nodes) found with %sfeasible "
        "list specified\n",
        tmpTC, tmpNC, (FNL == NULL) ? "no " : "");
...
int MNodeGetPreemptList(
    ...
    long Priority, int PIndex, int IsConditional, int *NodeCount,
    int *TaskCount)
...
#define MAX_MPJOB 16
    ...
    DBG(4, fSCHED)
    DPrint("%s(J,%s,PNode,PJob,%ld,%s,NC,TC)\n", FName,
           (FNL != NULL) ? "FNL" : "NULL", Priority,
    ...
    if (PreemptJob != NULL) PreemptJob[0] = NULL;
    ...
    if (IsConditional == TRUE) {
        ...
        for (nindex = 0; nindex < MMAX_NODE; nindex++) {
            ...
            if (NodeIsPreemptee == FALSE) continue;
            ...
            for (jindex = 0; jindex < N->MaxJCount; jindex++) {
                ...
                for (jindex2 = 0; jindex2 < PJCount; jindex2++) {
                    if (PreemptJob[jindex2] == tJ) break;
                } /* END for (jindex2) */
        ...
        DBG(4, fSCHED)
        DPrint("INFO:     preemptible resources found.  (%d nodes/%d tasks)\n",
               PNCount, PTCount);

        if (TaskCount != NULL) *TaskCount = PTCount;

        if (NodeCount != NULL) *NodeCount = PNCount;
    ...
    if (FNL == NULL) {
        ...
        if ((MSched.Time > PreemptListMTime[PIndex]) ||
            ...
            DBG(5, fSCHED)
            DPrint(
                "INFO:     refreshing PreemptList for partition %s (prio: "
                "%ld)\n",
                MAList[ePartition][PIndex], Priority);
            ...
            for (index = 0; index < MAX_MNODE; index++) {
                ...
                for (jindex = 0; jindex < N->MaxJCount; jindex++) {
                    if (N->JList[jindex] == NULL) break;

                    if (N->JList[jindex] == (mjob_t *)1) continue;
                    ...
                    if ((JobIsPreemptible == TRUE) &&
                        ...
                        MCResAdd(&PreemptRes, &N->CRes, &J->Req[0]->DRes, TC,
                                 FALSE); /* FIXME */

                        for (jindex2 = 0;
                             PreemptJobCache[PIndex][jindex2] != NULL;
                             jindex2++) {
                            if (PreemptJobCache[PIndex][jindex2] == J) break;
                        } /* END for (jindex2) */
                        ...
                        DBG(6, fSCHED)
                        DPrint(
                            "INFO:     job %s on node %s added to preempt list "
                            "(Proc: %d)\n",
                            J->Name, N->Name, PreemptRes.Procs);
                    } else {
                        DBG(7, fSCHED)
                        DPrint(
                            "INFO:     cannot preempt job %s on node %s (%s) "
                            "P: %ld < %ld\n",
                            J->Name, N->Name, (J->Flags & (1 << mjfPreemptee))
                                                  ? "priority too high"
                                                  : "not preemptible",
                ...
                if (PreemptRes.Procs <= 0) continue;
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     nodes placed in PNL[%d] list: %3d (procs: %d)\n",
                   PIndex, nindex, tmpTC);
        ...
        for (nindex = 0; nindex < PreemptNodeCount[PIndex]; nindex++) {
            memcpy(&PNL[nindex], &PreemptNodeCache[PIndex][nindex],
                   sizeof(PNL[nindex]));
        }

        if (PreemptJob != NULL) {
            for (jindex = 0; PreemptJobCache[PIndex][jindex] != NULL;
                 jindex++) {
        ...
        for (index = 0; FNL[index].N != NULL; index++) {
            ...
            for (jindex = 0; jindex < N->MaxJCount; jindex++) {
                ...
                if ((J->Flags & (1 << mjfPreemptee)) &&
                    ...
                    if (PreemptJob != NULL) {
                        for (jindex2 = 0; PreemptJob[jindex2] != NULL;
                             jindex2++) {
                            if (PreemptJob[jindex2] == J) break;
                        } /* END for (jindex2) */

                        if (PreemptJob[jindex2] == NULL) {
                            DBG(5, fSCHED)
                            DPrint(
                                "INFO:     new preemptible job %s located on "
                                "node %s (%ld < %ld)\n",
                                J->Name, N->Name, J->StartPriority, Priority);
                            ...
                            DBG(5, fSCHED)
                            DPrint(
                                "INFO:     preemptible job %s located on node "
                                "%s\n",
                                J->Name, N->Name);
                        }
                    }
            ...
            for (jindex = 0; jindex < index2; jindex++) {
                if (jindex >= MAX(MMAX_JOBRA, MAX_MPJOB)) break;
                ...
                MUStrCpy(PreemptorJ->RAList[jindex],
                         tmpJ[jindex - RATail]->Name,
                         sizeof(PreemptorJ->RAList[jindex]));
            } /* END for (jindex) */

            rc = MJobGetNRange(PreemptorJ, PreemptorJ->Req[0], N,
                               MSched.Time + 1, &TA, &TD, NULL, NULL, NULL);

            /* erase temporary access list data */

            for (jindex = 0; jindex < index2; jindex++) {
                if (jindex >= MAX_MPJOB) break;
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     node %s contains preemptible resources (P: %d)\n",
                   N->Name, PreemptRes.Procs);
        } /* END for (nindex) */
...

{
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
            ...
            for (jindex = 0; jindex < N->MaxJCount; jindex++) {
                if ((N->JList == NULL) || (N->JList[jindex] == NULL)) break;

                if ((FIndex == -1) && (N->JList[jindex] == (mjob_t *)1))
                    FIndex = jindex;

                if (N->JList[jindex] == J) {
                    if (rqindex == 0) N->JTC[jindex] = 0;
            ...
            if (jindex >= N->MaxJCount) {
                ...
                DBG(4, fSCHED)
                DPrint(
                    "INFO:     job table extended to size %d on node %s for "
                    "job %s)\n",
                    N->MaxJCount + MAX_MJOB_PER_NODE, N->Name, J->Name);
                ...
                if ((N->JList == NULL) || (N->JTC == NULL)) {
                    DBG(4, fSCHED)
                    DPrint("ERROR:    cannot extend node job tables\n");
...

            {
                ...
                for (jindex = 0; jindex < N->MaxJCount; jindex++) {
                    ...
                    for (tindex = 0; tJ->TaskMap[tindex] != -1; tindex++) {
                        if (tJ->TaskMap[tindex] != N->Index) continue;
            ...
            if ((N->RM == NULL) ||
                ...
                if (J->Cred.C != NULL) {
                    N->ARes.PSlot[J->Cred.C->Index].count =
                        MAX(0, N->ARes.PSlot[J->Cred.C->Index].count - PC);
                }

                DBG(4, fSCHED)
                DPrint("INFO:     node %s added to job %s.  PSlot: %s\n",
                       N->Name, J->Name,
            ...
            if (N == MSched.GN) {
                ...
                MCResRemove(&N->ARes, &N->CRes, &RQ->DRes,
                            RQ->NodeList[nindex].TC, TRUE);
            }
        } /* END for (nindex) */
    }     /* END for (rqindex) */
...
int MNodeSelectIdleTasks(
    ...
    int *TaskCount,   /* O */
    int *NodeCount,   /* O */
    char *NodeMap, int RejReason[MAX_MREQ_PER_JOB][MAX_MREJREASON])

{
    ...
    DBG(4, fSCHED)
    DPrint("%s(%s,%d,SrcNL,IdleMNL,TC,NC,NMap,RCount,RejReason)\n", FName,
           (J != NULL) ? J->Name : "NULL", (SRQ != NULL) ? SRQ->Index : -1);
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        for (index = 0; index < MAX_MNODE; index++) {
            if (FNL[index].N == NULL) break;

            N = FNL[index].N;

            if (MJobCheckNRes(J, N, RQ, MSched.Time, &tmpTC, 1.0, &RIndex,
                              &NodeAffinity, &ATime, FALSE) == FAILURE) {
                /* record why job failed requirements check */

                if (RejReason != NULL) RejReason[rqindex][RIndex]++;
            ...
            DBG(6, fSCHED)
            DPrint(
                "INFO:     node[%d] %s added to task list (%d tasks : %d tasks "
                "total)\n",
                nindex, N->Name, NodeList[nindex].TC, ATC[rqindex]);
        ...
        if (MPar[0].NAllocPolicy == mnalLastAvailable) {
            ...
            qsort((void *)MPN, nindex, sizeof(_mpnodealloc_t),
        ...
        if (ATC[rqindex] < RQ->TaskCount) {
            DBG(3, fSCHED)
            DPrint(
                "INFO:     inadequate feasible tasks found for job %s:%d (%d < "
                "%d)\n",
                J->Name, rqindex, ATC[rqindex], RQ->TaskCount);

            InadequateTasks = TRUE;
        }

        if ((RQ->NodeCount > 0) && (ANC[rqindex] < RQ->NodeCount)) {
            DBG(3, fSCHED)
            DPrint("INFO:     inadequate nodes found for job %s:%d (%d < %d)\n",
                   J->Name, rqindex, ANC[rqindex], RQ->NodeCount);
    ...
    if (TaskCount != NULL) *TaskCount = TotalTaskCount;

    if (NodeCount != NULL) *NodeCount = TotalNodeCount;
    ...
    if (J->Flags & (1 << mjfAllocLocal)) {
        ...
        if (MJobProximateMNL(J, IMNL, IMNL, MSched.Time, FALSE) == FAILURE) {
            DBG(4, fSCHED)
...
int MNodeSelectPreemptTasks(
    ...
    mjob_t *J, mnalloc_t *FNL, /* I:  nodes w/preempt tasks    */
    ...
    char *NodeMap, int RejReason[MAX_MREQ_PER_JOB][MAX_MREJREASON], long Mode)

{
    ...
    DBG(4, fSCHED)
    DPrint("%s(%s,%s,PreemptMNL,TC,NC,NMap,RCount,RejReason,%ld)\n", FName,
           (J != NULL) ? J->Name : "NULL", (FNL != NULL) ? "FNL" : "NULL",
           Mode);

    if ((J == NULL) || (FNL == NULL) || (PMNL == NULL)) {
        return (FAILURE);
    }
    ...
    for (rqindex = 0; J->Req[rqindex] != 0; rqindex++) {
        ...
        for (index = 0; index < MAX_MNODE; index++) {
            ...
            if (MJobCheckNRes(J, N, RQ, MAX_MTIME, &tmpTC, 1.0, &RIndex,
                              &NodeAffinity, NULL, FALSE) == FAILURE) {
                /* record why job failed requirements check */
    ...
    if ((TotalTaskCount < TasksNeeded) || (TotalNodeCount < NodesNeeded)) {
        DBG(6, fSCHED)
        DPrint(
            "INFO:     inadequate tasks found for job %s (T: %d < %d  N: %d < "
            "%d)\n",
            J->Name, TotalTaskCount, TasksNeeded, TotalNodeCount, NodesNeeded);
...

{
    ...
    if ((NName == NULL) || (NName[0] == '\0')) return (FAILURE);

    if (!strcmp(NName, "DEFAULT")) {
        if (NP != NULL) *NP = &MSched.DefaultN;
    ...
    if ((ptr = strchr(NName, '.')) != NULL) {
        MUStrCpy(QHostName, NName,
                 MIN((long)sizeof(QHostName), ptr - NName + 1));
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if ((ptr = strchr(N->Name, '.')) != NULL) {
            MUStrCpy(LHostName, N->Name,
                     MIN((long)sizeof(LHostName), ptr - N->Name + 1));
...

{
    ...
    DBG(6, fSTRUCT)
    DPrint("%s(%s,%s)\n", FName, (SrcN != NULL) ? SrcN->Name : "NULL",
    ...
    if (SrcN->R == NULL) SrcN->R = DstN->R;

    if (SrcN->RE == NULL) SrcN->RE = DstN->RE;

    if (SrcN->RC == NULL) SrcN->RC = DstN->RC;
...

{
    ...
    DBG(9, fSTRUCT)
    DPrint("%s(%s)\n", FName, (N->Name[0] == '\0') ? "NONE" : N->Name);

    fprintf(mlog.logfp,
            "[%03d] %s: (P:%d,S:%d,M:%d,D:%d) [%s][%5s][%5s]<%lf> C:%s",
            N->Index, N->Name, N->ARes.Procs, N->ARes.Swap, N->ARes.Mem,
            N->ARes.Disk, MAList[eNodeState][N->State],
            MAList[eOpsys][N->ActiveOS], MAList[eArch][N->Arch], N->Load,
            MUCAListToString(N->ARes.PSlot, N->CRes.PSlot, NULL));

    fprintf(mlog.logfp, "%s ", MUListAttrs(eNetwork, N->Network));

    fprintf(mlog.logfp, "%s ", MUMAList(eFeature, N->FBM, sizeof(N->FBM)));
...

{
    ...
    if (Flags & (1 << mcmParse)) {
        ...
        MUSNPrintF(&BPtr, &BSpace,
                   "%s=%s;%s=%s;%s=%s;%s=%s;%s=%d;%s=%d;%s=%.2lf;%s=%.2lf;",
                   MWikiNodeAttr[mwnaName], N->Name, MWikiNodeAttr[mwnaState],
                   MNodeState[N->State], MWikiNodeAttr[mwnaOS],
                   MAList[eOpsys][N->ActiveOS], MWikiNodeAttr[mwnaArch],
                   MAList[eArch][N->Arch], MWikiNodeAttr[mwnaCProc],
                   N->CRes.Procs, MWikiNodeAttr[mwnaAProc], N->ARes.Procs,
                   MWikiNodeAttr[mwnaSpeed], N->Speed,
                   MWikiNodeAttr[mwnaCPULoad], N->Load);
    ...
    MUSNPrintF(&BPtr, &BSpace, "Opsys:      %8s  Arch:      %6s\n",
               MAList[eOpsys][N->ActiveOS], MAList[eArch][N->Arch]);

    sprintf(tmpLine, "Speed:    %6.2lf  Load:      %6.3lf", N->Speed,
            (N->STTime > 0) ? N->Load : 0.0);

    if (N->MaxLoad > 0.01) {
        sprintf(temp_str, " (MaxLoad: %.2lf)", N->MaxLoad);
        strcat(tmpLine, temp_str);
    }

    if (N->ExtLoad > 0.01) {
        sprintf(temp_str, " (ExtLoad: %.2lf)", N->ExtLoad);
        strcat(tmpLine, temp_str);
    }

    if (N->ProcSpeed > 0) {
        sprintf(temp_str, " (ProcSpeed: %d)", N->ProcSpeed);
        strcat(tmpLine, temp_str);
    }
    ...
    if ((N->FrameIndex > 0) &&
        (MSys[N->FrameIndex][N->SlotIndex].Attributes > 0)) {
        MUSNPrintF(&BPtr, &BSpace, "Attributes: %s\n",
                   MUListAttrs(eSysAttr,
                               MSys[N->FrameIndex][N->SlotIndex].Attributes));
    }
    ...
    if (N->Pool[0] > 0) {
        ...
        for (index = 1; index < MAX_MPOOL; index++) {
            ...
            MUSNPrintF(&BPtr, &BSpace, " %d", (int)(N->Pool[index] - 1));
        }
    ...
    if ((N->AP.HLimit[mptMaxJob][0] > 0) || (N->MaxJobPerUser > 0) ||
        (N->MaxPEPerJob > 0.01)) {
        ...
        if (N->MaxPEPerJob > 1.0) {
            MUSNPrintF(&BPtr, &BSpace, "%s=%d  ", MNodeAttr[mnaMaxPEPerJob],
                       (int)N->MaxPEPerJob);
        ...
        if (N->MaxJobPerUser > 0) {
            MUSNPrintF(&BPtr, &BSpace, "%s=%d  ", MNodeAttr[mnaMaxJobPerUser],
                       N->MaxJobPerUser);
        }
    ...
    if (N->STTime > 0) {
        ...
        MUSNPrintF(&BPtr, &BSpace,
                   ...
                   STotalTime, SUpTime, (double)N->SUTime / N->STTime * 100.0,
                   '%', SBusyTime, (double)N->SATime / N->STTime * 100.0, '%');
    } /* END if (N->STTime > 0) */
...

{
    ...
    if (MNODEISACTIVE(N) == TRUE) {
        ...
        if (N->State == mnsBusy) {
            if ((N->Load - N->ExtLoad) / N->CRes.Procs < 0.2) {
                MUSNPrintF(
                    &BPtr, &BSpace,
                    "ALERT:  node is in state %s but load is low (%0.3f)\n",
                    MNodeState[N->State], N->Load);
            }
        } else if (N->DRes.Procs > 0) {
            if ((N->Load - N->ExtLoad) / N->DRes.Procs < 0.2) {
                MUSNPrintF(&BPtr, &BSpace,
                           "ALERT:  node has %d procs dedicated but load is "
                           "low (%0.3f)\n",
                           N->DRes.Procs, N->Load);
            }
        }
        ...
        if ((N->Load / N->CRes.Procs) > 0.5) {
            MUSNPrintF(&BPtr, &BSpace,
                       "ALERT:  node is in state %s but load is high (%0.3f)\n",
                       MNodeState[N->State], N->Load);
        }
    }
    ...
    if ((N->FrameIndex != -1) && (MFrame[N->FrameIndex].Memory > 0) &&
        ...
        MUSNPrintF(
            &BPtr, &BSpace,
            "ALERT:  node memory does not match expected memory (%d != %d)\n",
            N->CRes.Mem, MFrame[N->FrameIndex].Memory);
    }
...
int MNodeShowReservations(
    ...
    mnode_t *N, int Flags, char *Buffer, /* O */
    int BufSize, int Mode)               /* I */

{
    ...
    if ((N == NULL) || (Buffer == NULL)) return (FAILURE);
    ...
    for (rindex = 0; rindex < MSched.ResDepth; rindex++) {
        ...
        for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
            ...
            if (N->RE[reindex].Index != rindex) continue;
            ...
            if (Header == FALSE) {
                ...
                MUSNPrintF(&BPtr, &BSpace, "  %s '%s'(x%d)  %s -> %s (%s)\n",
                           MResType[R->Type], R->Name, RC, StartTime, EndTime,
                           Duration);
...
int MNodeDiagnoseReservations(
    ...
    mnode_t *N, int Flags, char *Buffer, int BufSize, int Mode) /* I */
    ...
    if (ErrorDetected == TRUE) {
        ...
        for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
            ...
            MUSNPrintF(&BPtr, &BSpace, "  RE[%d] %c %d\n", reindex,
                       (N->RE[reindex].Type == mreStart) ? 'S' : 'E', RIndex);
            ...
            if ((RCount[RIndex] < 0) || (RCount[RIndex] > 1)) {
                MUSNPrintF(&BPtr, &BSpace, "ALERT:  RE[%d] %s\n", reindex,
                           (RCount[RIndex] < 0)
                               ? "reservation end event located without "
                                 "accompanying start event"
                               : "reservation start event located without "
                                 "accompanying end event");
        ...
        for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
            ...
            if (N->RE[reindex].Time > ETime) {
                if (MUCResIsNeg(&NDRes) == SUCCESS) {
                    MUSNPrintF(
                        &BPtr, &BSpace,
                        "ALERT:  node is overcommitted at time %s (P: %d)\n",
                        MULToTString(ETime - MSched.Time), NDRes.Procs);
                }
            ...
            if (N->RE[reindex].Type == mreStart) {
                if (N->R[N->RE[reindex].Index]->Type == mrtJob) {
                    ...
                    MCResRemove(&NDRes, &N->CRes, &N->RE[reindex].DRes, 1,
                                FALSE);
                }
            } else if (N->RE[reindex].Type == mreEnd) {
                if (N->R[N->RE[reindex].Index]->Type == mrtJob) {
...

{
    ...
    DBG(6, fSTRUCT)
    DPrint("%s(%s,%s,%d)\n", FName, (J != NULL) ? J->Name : "NULL",
           (N != NULL) ? N->Name : "NULL", (TC != NULL) ? *TC : -1);
    ...
    if (((MJC > 0) || (MUJC > 0) || (MPC > 0) || (MUPC > 0)) &&
        (StartTime <= MSched.Time + 3600)) {
        TotalJobCount = JPC;
        TotalProcCount = 1;
        ...
        for (jindex = 0; jindex < N->MaxJCount; jindex++) {
            ...
            if ((tmpJ->State != mjsStarting) && (tmpJ->State != mjsRunning)) {
                continue;
            }

            TotalJobCount++;

            if (tmpJ->Cred.U == J->Cred.U) UserJobCount++;

            for (tindex = 0; tmpJ->TaskMap[tindex] != -1; tindex++) {
                if (tmpJ->TaskMap[tindex] != N->Index) continue;
        ...
        if ((MJC > 0) && (TotalJobCount > MJC)) {
            DBG(6, fSTRUCT)
            DPrint("INFO:     job violates MaxJobPerNode policy on node %s\n",
                   N->Name);
        ...
        if ((MUJC > 0) && (UserJobCount >= MUJC)) {
            DBG(6, fSTRUCT)
            DPrint("INFO:     job violates MaxJobPerUser policy on node %s\n",
                   N->Name);
        ...
        if (MUPC > 0) {
            if (UserProcCount > MUPC) {
                DBG(6, fSTRUCT)
                DPrint(
                    "INFO:     job violates MaxProcPerUser policy on node %s\n",
                    N->Name);
        ...
        if (MPC > 0) {
            if (TotalProcCount > MPC) {
                DBG(6, fSTRUCT)
                DPrint("INFO:     job violates MaxProc policy on node %s\n",
                       N->Name);
    ...
    if (TC != NULL) {
        ...
        if (*TC <= 0) {
            DBG(6, fSTRUCT)
            DPrint("INFO:     inadequate tasks on node %s in %s\n", N->Name,
                   FName);
            ...
            case mSet:
                ...
                if (oindex > 0) {
                    N->CRes.PSlot[0].count =
                        MAX(0, N->CRes.PSlot[0].count - oindex);
                ...
                if (oindex > 0) {
                    N->ARes.PSlot[0].count =
                        MAX(0, N->ARes.PSlot[0].count - oindex);
        ...
        default:
            ...
            if ((ptr = strchr(SpecName, '.')) != NULL) {
                ...
                if (MSched.DefaultDomain[0] == '\0')
                    MUStrCpy(MSched.DefaultDomain, ptr,
                             sizeof(MSched.DefaultDomain));
            ...
            if ((ptr = strchr(SpecName, '.')) != NULL) {
                ...
                if (MSched.DefaultDomain[0] != '\0') {
                    if (MSched.DefaultDomain[0] == '.') {
                        sprintf(Line, "%s%s", SpecName, MSched.DefaultDomain);
                    } else {
                        sprintf(Line, "%s.%s", SpecName, MSched.DefaultDomain);
                    }
...

{
    ...
    switch (AIndex) {
        case mnaCfgDisk:
            ...
            sprintf(Buf, "%d", N->CRes.Disk);

            break;

        case mnaCfgMem:

            sprintf(Buf, "%d", N->CRes.Mem);
        ...
        case mnaRADisk:
            ...
            sprintf(Buf, "%d", N->ARes.Disk);

            break;

        case mnaRAMem:

            sprintf(Buf, "%d", N->ARes.Mem);

            break;

        case mnaRAProc:

            sprintf(Buf, "%d", N->ARes.Procs);

            break;

        case mnaRASwap:

            sprintf(Buf, "%d", N->ARes.Swap);
        ...
        case mnaRCProc:
            ...
            sprintf(Buf, "%d", N->CRes.Procs);

            break;

        case mnaRCSwap:

            sprintf(Buf, "%d", N->CRes.Swap);

            break;

        case mnaStatATime:

            sprintf(Buf, "%ld", N->SATime);

            break;

        case mnaStatTTime:

            sprintf(Buf, "%ld", N->STTime);

            break;

        case mnaStatUTime:

            sprintf(Buf, "%ld", N->SUTime);
...

{
    ...
    DBG(8, fSCHED)
    DPrint("%s(%s,%d,%d,%d,%d,%ld)\n", FName, (N != NULL) ? N->Name : "NULL",
           Avl->Procs, Cfg->Procs, Ded->Procs, Req->Procs, Time - MSched.Time);
    ...
    for (rindex = mrProc; rindex <= mrDisk; rindex++) {
        if (RRes[rindex] == 0) continue;

        if (CRes[rindex] < RRes[rindex]) {
            DBG(8, fSCHED)
            DPrint("INFO:     inadequate %s (C:%d < R:%d)\n",
                   MResourceType[rindex], CRes[rindex], RRes[rindex]);
...

        {
            ...
            if ((N != NULL) && (N->NAvailPolicy != NULL)) {
                pval = (N->NAvailPolicy[rindex] != mrapNONE)
                           ? N->NAvailPolicy[rindex]
                           : N->NAvailPolicy[0];
            }

            if (pval == mrapNONE) {
                pval = (MPar[0].NAvailPolicy[rindex] != mrapNONE)
                           ? MPar[0].NAvailPolicy[rindex]
            ...
            if ((pval == mrapUtilized) || (pval == mrapCombined)) {
                if (Time <= MSched.Time) UseUtil = TRUE;
            }

            if ((pval == mrapDedicated) || (pval == mrapCombined)) {
                if (Time <= MSched.Time) UseDed = TRUE;
            }
        } /* END BLOCK */
        ...
        if (RRes[rindex] == -1) {
            if (CRes[rindex] == 0) {
                DBG(8, fSCHED)
                DPrint("INFO:     cannot dedicate %s (C:%d == 0)\n",
                       MResourceType[rindex], CRes[rindex]);
            ...
            if ((UseUtil == TRUE) && (ARes[rindex] != CRes[rindex])) {
                DBG(8, fSCHED)
                DPrint("INFO:     cannot dedicate %s (A:%d != C:%d)\n",
                       MResourceType[rindex], ARes[rindex], CRes[rindex]);
            ...
            if ((UseDed == TRUE) && (DRes[rindex] > 0)) {
                DBG(8, fSCHED)
                DPrint("INFO:     cannot dedicate %s (D:%d != 0)\n",
                       MResourceType[rindex], DRes[rindex]);
        ...
        if (TC <= 0) {
            DBG(9, fSCHED)
            DPrint("INFO:     TC from %s: %d\n", MResourceType[rindex], TC);
...

{
    ...
    if (((N->RM != NULL) && (N->RM->Type == mrmtPBS)) ||
        ...
        if ((N->State != mnsIdle) && (N->State != mnsActive) &&
            ...
            if (UseUtil == TRUE) {
                if (UseDed == TRUE) {
                    N->ARes.Procs =
                        MAX(0, N->CRes.Procs - MAX(UProcs, DProcs) -
                                   (short)(MPar[0].UntrackedProcFactor *
                                           MAX(0.0, (N->Load - UProcs))));
                } else {
                    N->ARes.Procs =
                        MAX(0, N->CRes.Procs - UProcs -
                                   (short)(MPar[0].UntrackedProcFactor *
                                           MAX(0.0, (N->Load - UProcs))));
                }
            } else if (UseDed == TRUE) {
                ...
                if (N->ARes.Procs < 0) N->ARes.Procs = N->CRes.Procs;
            }
        }
    } /* END if (N->RM->Type ...) */
    ...
    if ((UseDed == TRUE) &&
        ...
        DBG(2, fRM)
        DPrint(
            "ALERT:    node %s in RM state %s has %d:%d active tasks:procs\n",
            N->Name, MNodeState[N->State], DTasks, DProcs);
    ...
    if ((N->RM != NULL) && (N->RM->Type == mrmtLL)) {
        if ((N->State == mnsIdle) || (N->State == mnsActive)) {
            N->ARes.Procs =
                (int)MIN(N->CRes.Procs - N->Load + MIN_OS_CPU_OVERHEAD,
                         N->CRes.Procs - N->DRes.Procs);
        }
    }
    ...
    if (MaxLoad > 0.01) {
        ...
        AvailLoad =
            MAX(0.0, MaxLoad + MIN_OS_CPU_OVERHEAD - N->Load - N->ExtLoad);
    ...
    if ((N->State == mnsIdle) && (DProcs > 0)) N->State = mnsActive;
    ...
    for (rindex = mrMem; rindex <= mrDisk; rindex++) {
        if ((ARes[rindex] == NULL) || (*ARes[rindex] != -1)) continue;
        ...
        if (UseUtil == TRUE) {
            if (UseDed == TRUE)
                *ARes[rindex] =
                    MAX(0, CRes[rindex] - MAX(URes[rindex], DRes[rindex]));
            else
    ...
    for (rindex = 0; rindex < MAX_MGRES; rindex++) {
        N->ARes.GRes[rindex].count =
            MAX(0, (N->CRes.GRes[rindex].count - N->DRes.GRes[rindex].count));
    }
...

{
    ...
    R->Procs +=
        (Req->Procs == -1) ? Cfg->Procs : MIN(Cfg->Procs, Cnt * Req->Procs);
    ...
    if (Req->PSlot[0].count != MAX_INT) {
        for (index = 0; index < MAX_MCLASS; index++) {
            if ((index > 0) && (MAList[eClass][index][0] == '\0')) break;

            R->PSlot[index].count +=
                MIN(Cfg->PSlot[index].count, Req->PSlot[index].count * Cnt);

            if (EnforceConstraints == TRUE) {
                R->PSlot[index].count =
                    MIN(Cfg->PSlot[index].count, R->PSlot[index].count);
            }
        } /* END for (index) */
    }     /* END if (Req->PSlot[0].count != MAX_INT) */

    for (index = 1; index < MAX_MCLASS; index++) {
        if (MAList[eGRes][index][0] == '\0') break;

        R->GRes[index].count +=
            MIN(Cfg->GRes[index].count, Req->GRes[index].count * Cnt);
        R->GRes[0].count +=
            MIN(Cfg->GRes[0].count, Req->GRes[index].count * Cnt);
    } /* END for (index) */
...

{
    ...
    if (Req->PSlot[0].count != MAX_INT) {
        ...
        for (index = 0; index < MAX_MCLASS; index++) {
            if (MAList[eClass][index][0] == '\0') break;
            ...
            if (ccount > 0) {
                if ((EnforceConstraints == FALSE) ||
                    (R->PSlot[index].count >
                     MIN(Cfg->PSlot[index].count, ccount * Cnt))) {
                    R->PSlot[index].count -=
                        MIN(Cfg->PSlot[index].count, ccount * Cnt);
                } else {
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (Delta > MSched.NodePurgeTime) {
            DBG(2, fSTRUCT)
            DPrint("WARNING:  node '%s' not detected in %s\n", N->Name,
                   MULToTString((long)MSched.Time - N->ATime));

            DBG(2, fSTRUCT)
            DPrint("INFO:     removing 'stale' node '%s'\n", N->Name);
        ...
        if (N->EState == mnsNONE) N->EState = N->State;

        if ((N->State != N->EState) && (MSched.Time > N->SyncDeadLine)) {
            DBG(2, fSTRUCT)
            DPrint(
                "ALERT:    node '%s' sync from expected state '%s' to state "
                "'%s' at %s",
                N->Name, MNodeState[N->EState], MNodeState[N->State],
...

{
    ...
    switch (NAttr) {
...

        {
            ...
            if ((MUNLGetMinAVal(NL, mnaSpeed, &N1, (void **)&MinSpeed) ==
                 FAILURE) &&
                (MUNLGetMinAVal(NL, mnaProcSpeed, &N2, (void **)&MinPSpeed) ==
                 FAILURE)) {
            ...
            if (Val != NULL) {
                ...
                if (Val[0] == '\0') strcpy((char *)Val, MDEF_NODETYPE);
            } /* END if (Val != NULL) */
        }     /* END BLOCK */
...

{
    ...
    switch (NAttr) {
...

        {
            ...
            if ((MUNLGetMaxAVal(NL, mnaSpeed, &N, (void **)&MaxSpeed) ==
                 FAILURE) &&
                (MUNLGetMaxAVal(NL, mnaProcSpeed, &N, (void **)&MaxPSpeed) ==
                 FAILURE)) {
                strcpy((char *)Val, MDEF_NODETYPE);
...

{
    ...
    if (C[0] != '\0') {
        if ((C[strlen(C) - 1] == '$') && !strncmp(FString, C, strlen(C) - 1) &&
    ...
    if ((C[0] != '\0') && !strncmp(FString, C, strlen(C))) {
        ...
        if (MParAdd(ptr, &P) == SUCCESS) {
            ...
            DBG(6, fSTRUCT)
            DPrint(
                "ALERT:    cannot add partition '%s', ignoring partition "
                "header\n",
                ptr);
        }
    }
...
{
    int DAList[] = {mnaStatATime, mnaStatTTime, mnaStatUTime, -1};
...

{
    ...
    sscanf(Buf, "%s %s %ld", tmpName, NodeName, &CkTime);
    ...
    if (NP == NULL) {
        if (MNodeFind(NodeName, &N) != SUCCESS) {
            DBG(5, fCKPT)
            DPrint("INFO:     node '%s' no longer detected\n", NodeName);
...
{
    const int CPAList[] = {mnaStatATime, mnaStatTTime, mnaStatUTime, -1};
...

{
    ...
    if (MSched.Mode != msmSim) {
        if (MOSGetHostName(N->Name, NULL, &Address) == FAILURE) {
            DBG(2, fCONFIG)
            DPrint("ALERT:    cannot determine address for host '%s'\n",
                   N->Name);
        ...
        for (sindex = 1; sindex <= MAX_MSLOTPERFRAME; sindex++) {
            ...
            if ((ptr = strchr(tmpName, '.')) != NULL) {
                ...
                if (strchr(S->NetName[mnetEN0], '.')) {
                    ...
                    if (MSched.DefaultDomain[0] == '.') {
                        sprintf(tmpName, "%s%s", N->Name, MSched.DefaultDomain);
                    } else {
                        sprintf(tmpName, "%s.%s", N->Name,
            ...
            if (S->RMName[0] == '\0') {
                DBG(6, fCONFIG)
                DPrint("INFO:     RMName '%s' set for node[%02d][%02d] '%s'\n",
                       tmpName, findex, sindex, N->Name);
    ...
    if (MNodeLocationFromName(N, &findex, &sindex) == FAILURE) {
        DBG(2, fSIM)
        DPrint("INFO:     cannot determine node/frame of host '%s'\n", N->Name);
...

{
    ...
    if ((N->FrameIndex == F->Index) && (N->SlotIndex > 0) &&
    ...
    if ((SIndex == -1) && (N->SlotIndex == -1)) {
        ...
        DBG(1, fSCHED)
        DPrint("INFO:     node slot not set on node '%s'\n", N->Name);

        return (SUCCESS);
    }

    if (SIndex != -1) N->SlotIndex = SIndex;
    ...
    DBG(5, fRM)
    DPrint("INFO:     node '%s' assigned to location F:%d/S:%d\n", N->Name,
           N->FrameIndex, N->SlotIndex);
...

{
    ...
    if ((N->P != NULL) && (N->P->CWIsSet == TRUE)) {
        ...
        *Val = (double)N->Priority + 100.0 * JPref + 100.0 * RAff;
    ...
    if (StartTime == MSched.Time) {
        if ((N->P != NULL) && (N->P->DPIsSet == TRUE) &&
            ...
            tmpDP += CW[mnpcUsage] *
                     ((N->SUTime > 0) ? ((double)N->SATime / N->SUTime) : 0.0);
...
{
    int AList[] = {mnaMaxLoad, mnaPriority, mnaPrioF, -1};
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (aindex = 0; AList[aindex] != -1; aindex++) {
            if ((MNodeAToString(N, AList[aindex], tmpLine,
            ...
            sprintf(temp_str, "%s%s=%s", (NLine[0] != '\0') ? " " : "",
                    MNodeAttr[AList[aindex]], tmpLine);
...
int MNodeLocationFromName(
    ...
    mnode_t *N, int *FIndex, int *SIndex)

{
    char *FName = "MNodeLocationFromName";

    DBG(7, fSTRUCT)
    DPrint("%s(%s,FIndex,SIndex)\n", FName, (N != NULL) ? N->Name : "NULL");
...

{
    ...
    if ((FIndex == NULL) || (*FIndex < 0)) {
        ...
        for (findex = 0; findex < MAX_MFRAME; findex++) {
            ...
            if ((FName != NULL) && !strcmp(FName, F->Name)) {
                ...
                if (FP != NULL) *FP = &MFrame[findex];

                if (FIndex != NULL) *FIndex = findex;
    ...
    if (FName != NULL) {
        ...
        sprintf(F->Name, "FRAME%02d", F->Index);
    }
...

{
    ...
    DBG(3, fSTRUCT)
    DPrint("%s(%s,SP,Buffer,%d,%d)\n", FName,
           (SFName != NULL) ? SFName : "NULL", BufSize, Mode);
    ...
    MUSNPrintF(&BPtr, &BSpace, "--------- %2s %4s %8s %10s %10s\n\n", "NC",
               "Mem", "Disk(MB)", "Partition", "UpdateTime");
    ...
    for (findex = 0; findex < MAX_MFRAME; findex++) {
        ...
        if ((SFName != NULL) && strcmp(SFName, NONE)) {
            if (strcmp(SFName, F->Name)) continue;
        }
        ...
        MUSNPrintF(&BPtr, &BSpace, "Frame[%02d] %02d %04d %08d %10s %11s\n",
                   F->Index, F->NodeCount, F->Memory, F->Disk,
                   (P != NULL) ? P->Name : NONE,
...

{
    ...
    DBG(3, fSTRUCT)
    DPrint("%s(NS,RID,P,DFlags,DisplayMode,SBuf,SBufSize)\n", FName);
        ...
        if (ResFound == TRUE) {
            switch (DisplayMode) {
                case mwpXML:
                    ...
                    MUIXMLSetStatus(RE, FAILURE, "reservation not specified",
                                    -1);
            ...
            DBG(2, fSTRUCT)
            DPrint("INFO:     reservation not specified for summary\n");
    ...
    /*        verbose behavior, display all reservations on nodes with specified
     * reservation */
    ...
    for (nindex = 0; MNode[nindex] != NULL; nindex++) {
        ...
        if ((NS != NULL) && (MNode[nindex] != NS)) continue;

        if ((P != NULL) && (P->Index > 0) && (N->PtIndex != P->Index)) continue;

        DBG(5, fSTRUCT)
        DPrint("INFO:     checking reservations for node '%s'\n", N->Name);
        ...
        for (rindex = 0; rindex < MSched.ResDepth; rindex++) {
            ...
            if ((RID != NULL) && strcmp(RID, NONE)) {
                if (strcmp(RID, R->Name) && !(DFlags & (1 << mcmVerbose)) &&
                    !(DFlags & (1 << mcmSummary))) {
                    continue;
                }
            ...
            DBG(7, fSTRUCT)
            DPrint("INFO:     displaying res '%s' at slot %d on node %s\n",
                   R->Name, rindex, N->Name);
            ...
            if (DFlags & (1 << mcmSummary)) {
                ...
                if (strcmp(RID, R->Name) && (R->Type == mrtJob)) {
                    for (srindex = 0; ARes[srindex] != NULL; srindex++) {
                        if (!strcmp(ARes[srindex]->Name, R->Name)) break;
                    }

                    if (ARes[srindex] == NULL) ARes[srindex] = R;
            ...
            switch (DisplayMode) {
...

                {
                    ...
                    MNResToString(N, (!(DFlags & (1 << mcmVerbose))) ? R : NULL,
                                  &tE, NULL, 0);
        ...
        for (srindex = 0; ARes[srindex] != NULL; srindex++) {
            ...
            switch (DisplayMode) {
...

                {
                    ...
                    const int RAList[] = {mraName,      mraCreds,   mraJState,
                                          mraStartTime, mraEndTime, -1};
                    ...
                    MXMLSetAttr(tmpE, (char *)MResAttr[mraTaskCount],
                                (void **)&AResCount[srindex], mdfInt);
