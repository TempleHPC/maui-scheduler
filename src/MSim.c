    ...
    if (MSim.ScaleJobRunTime == TRUE) {
        ...
        if ((MinProcSpeed > 0) && (MinProcSpeed < MAX_INT)) {
            if (MSched.ReferenceProcSpeed > 0) {
                J->SimWCTime =
                    (long)(J->SimWCTime * (double)MSched.ReferenceProcSpeed /
                           MinProcSpeed);
            }
        }
...
int MSimJobModify(
    ...
    mjob_t *J, char *Attr, char *SubAttr, char *Value, int *SC)
...

{
    ...
    if (MaxLoad > 0.01) {
        ...
        N->ARes.Procs = MIN(N->CRes.Procs - N->DRes.Procs,
...
{
    if (J->ASFunc != NULL) (*J->ASFunc)(J, mascUpdate, J->ASData, NULL);

    J->ATime = MSched.Time;

    if (J->Ckpt != NULL) {
        if ((J->State == mjsStarting) || (J->State == mjsRunning)) {
            if (MSched.Time - MAX(J->StartTime, J->Ckpt->LastCPTime) >
                J->Ckpt->CPInterval) {
                MJobCheckpoint(J);
            }
        }
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        DBG(7, fSIM)
        DPrint("INFO:     updating node %d '%s'\n", nindex, N->Name);

        OldNodeState = N->State;

        MRMNodePreUpdate(N, (N->NewState != mnsNONE) ? N->NewState : N->State,
                         N->RM);

        __MSimNodeUpdate(N);

        MRMNodePostUpdate(N, OldNodeState);
    } /* END for (nindex) */
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (R->Func != NULL) (*R->Func)(&R->Data, mascUpdate, NULL, NULL);
    } /* END for (nindex) */
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = JNext) {
        ...
        JNext =
            J->Next; /* store next job pointer in case current job is removed */
        ...
        if (J->State == mjsCompleted) {
            ...
            MRMJobPostUpdate(J, J->TaskMap, J->State, J->RM);
    ...
    for (jindex = 0; MAQ[jindex] != -1; jindex++) {
        if (MAQ[jindex] == 0) {
            DBG(0, fSIM)
            DPrint("ALERT:    corrupt MAQ table entry at index %d\n", jindex);
        ...
        DBG(5, fSIM)
        DPrint(
            "INFO:     check completion of AQ[%02d] job '%18s':  now: %ld  "
            "start: %ld  completion: %ld (deadLine: %ld res: %ld)\n",
            jindex, J->Name, MSched.Time, J->StartTime,
            J->StartTime + J->SimWCTime, J->StartTime + J->WCLimit,
...

{
    ...
    if (MSched.Iteration == -1) {
        if ((MJobTraceBuffer = (mjob_t *)calloc(
                 1, sizeof(mjob_t) * MAX_MJOB_TRACE)) == NULL) {
            /* cannot allocate workload trace buffer */

            fprintf(
                stderr,
                "ERROR:     cannot allocate workload trace buffer, %d (%s)\n",
    ...
    switch (MSim.JobSubmissionPolicy) {
        case msjsTraceSubmit:
            ...
            if (MSched.Iteration == -1) {
                ...
                while (index < (int)MSim.InitialQueueDepth) {
                    if (MSimJobSubmit(-1, &JTrace[jindex], NULL, jtNONE) ==
                        FAILURE) {
                    ...
                    if (jindex >= MAX_MJOB_TRACE) break;
                ...
                DBG(5, fSIM)
                DPrint("INFO:     new trace time detected (%s)\n",
                       MULToDString(&TraceTime));

                DBG(2, fSIM)
                DPrint("INFO:     initial queue depth %d set (%d requested)\n",
                       index, MSim.InitialQueueDepth);
            } /* END if (MSched.Iteration == -1) */
            else {
                while (MSimJobSubmit(NewTime, &JTrace[jindex], NULL, jtNONE) ==
                       SUCCESS) {
                    DBG(3, fSIM)
                    DPrint("INFO:     job '%s' submitted\n",
                           JTrace[jindex]->Name);
        ...
        case msjsConstantJob:
            ...
            for (jindex = 1; jindex < MAX_MJOB; jindex++) {
                ...
                if (J->State != mjsIdle) continue;

                MPolicyAdjustUsage(NULL, J, NULL, mlIdle, MPar[0].L.IP, -1, 1,
                                   NULL);
            } /* END for (jindex) */
            ...
            while (MPar[0].L.IP->Usage[mptMaxJob][0] < MSim.InitialQueueDepth) {
                if (MSimJobSubmit((MSched.Iteration == -1) ? MSched.Time : -1,
                                  &JTrace[jindex], NULL, jtNONE) == FAILURE) {
                    DBG(3, fSIM) DPrint("INFO:     job traces exhausted\n");

                    break;
                }

                MPolicyAdjustUsage(NULL, JTrace[jindex], NULL, mlIdle,
                                   MPar[0].L.IP, -1, 1, NULL);
        ...
        default:
            ...
            DBG(0, fSIM)
            DPrint("ERROR:    unexpected JobSubmissionPolicy detected (%d)\n",
    ...
    if (NewTime != MSched.Time) {
        DBG(1, fSCHED)
        DPrint(
            "NOTE:     adjusting scheduler time to job submission time (%s)\n",
...

{
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        ...
        if (!(J->Flags & (1 << mjfPreload))) continue;

        /* attempt to start jobs */

        if (MJobSelectMNL(J, &MPar[0], NULL, MNodeList, NodeMap, FALSE) ==
            FAILURE) {
            DBG(6, fSCHED)
            DPrint("INFO:     cannot locate %d tasks for preload job '%s'\n",
                   J->Request.TC, J->Name);
        ...
        if (MJobAllocMNL(J, MNodeList, NodeMap, NULL,
                         ...
                         MSched.Time) == FAILURE) {
            DBG(1, fSCHED)
            DPrint("ERROR:    cannot allocate nodes to preload job '%s'\n",
                   J->Name);
        ...
        if (MJobStart(J) == FAILURE) {
            DBG(1, fSCHED)
            DPrint("ERROR:    cannot run preload job '%s'\n", J->Name);

            continue;
        }

        DBG(1, fSCHED)
        DPrint("INFO:     preload job '%s' successfully started\n", J->Name);
    } /* END for (J) */
...

{
    ...
    DBG(2, fSIM)
    DPrint("%s(%s,%s,%s)\n", FName, Directory, TraceFile,
           (Buffer != NULL) ? "Buffer" : "NULL");

    /* setup default RM */

    if (MRM[0].Type == mrmtNONE) MRMAdd(DEFAULT, NULL);

    if (MRM[0].Type == 0) MRM[0].Type = DEFAULT_MRMTYPE;
    ...
    if ((Buffer == NULL) || (Buffer[0] == '\0')) {
        ...
        if ((TraceFile[0] == '/') || (TraceFile[0] == '~')) {
            ...
            if (Directory[strlen(Directory) - 1] == '/') {
                sprintf(Name, "%s%s", Directory, TraceFile);
            } else {
                sprintf(Name, "%s/%s", Directory, TraceFile);
            }
        }
        ...
        if ((buf = MFULoad(Name, 1, macmWrite, &count, NULL)) == NULL) {
            DBG(0, fSIM)
            DPrint("ERROR:    cannot open resource tracefile '%s'\n", Name);
    ...
    while (ptr < tail) {
        if ((MSim.NodeCount != 0) && (NodeCount >= MSim.NodeCount)) {
            DBG(1, fSIM)
            DPrint(
                "INFO:     simulation node count reached.  (node count: %d)\n",
                NodeCount);
        ...
        if (MTraceLoadResource(ptr, &Offset, &tmpN, &Version) == SUCCESS) {
            if (MNodeFind(tmpN.Name, &N) == SUCCESS) {
                DBG(1, fSIM)
                DPrint("ALERT:    node '%s' previously detected in tracefile\n",
                       tmpN.Name);
            } else {
                for (cindex = 0; cindex < MAX_MCLASS; cindex++) {
                    /* located configured classes */

                    if (tmpN.CRes.PSlot[cindex].count > 0) break;
                }

                if (cindex == MAX_MCLASS) {
                    DBG(2, fSIM)
                    DPrint(
                        "INFO:     non-batch node '%s' trace loaded.  "
                        "ignored\n",
                        tmpN.Name);
                } else {
                    if (MNodeAdd(tmpN.Name, &N) == SUCCESS) {
                        ...
                        DBG(5, fSIM)
                        DPrint("INFO:     customizing MNode[%03d]: '%s'\n",
                               N->Index, N->Name);
                        ...
                        if (MSim.NCPolicy == msncPartitioned) {
                            for (index = 6; index < MAX_MPAR; index++) {
                                if (N->CRes.Mem <= (1 << index)) break;
                            }

                            N->PtIndex = index - 5;
                            N->CRes.Mem = 1 << index;
                        } else {
                            if (N->PtIndex <= 0) N->PtIndex = 1;
                        }
                        ...
                        DBG(5, (fSIM | fSTRUCT))
                        DPrint(
                            "INFO:     host '%s' assigned to frame: %d node: "
                            "%d partition: %s\n",
                            N->Name, N->FrameIndex, N->SlotIndex,
                        ...
                        if ((tok = strstr(Name, PARTITIONMARKER)) != NULL) {
                            ...
                            DBG(4, fSIM)
                            DPrint(
                                "INFO:     node '%s' added to partition %d\n",
                                N->Name, N->PtIndex);
                        }
                        ...
                        DBG(2, fSIM)
                        DPrint(
                            "ALERT:    node buffer overflow.  %d nodes read.  "
                            "cannot alloc memory for node '%s'\n",
                            NodeCount, tmpN.Name);
    ...
    DBG(3, fSIM)
    DPrint("INFO:     node traces loaded: %d (%d procs)\n", NodeCount,
           ProcCount);
    ...
    DBG(2, fSIM) {
        ...
        for (index = 0; index < MAX_MFRAME; index++) {
            if (MFrame[index].NodeCount > 0) {
                fprintf(mlog.logfp, "[%02d: %2d]", index,
...

{
    ...
    if ((J->State != mjsRunning) && (J->State != mjsStarting)) return (FAILURE);
...

{
    ...
    if (J->Ckpt->LastCPTime == 0) J->Ckpt->LastCPTime = J->StartTime;
...

{
    ...
    if ((J->State != mjsStarting) && (J->State != mjsRunning)) {
        DBG(2, fSCHED)
        DPrint("WARNING:  attemting to requeue non-active job %s\n", J->Name);
    ...
    if (!(J->Flags & (1 << mjfRestartable))) {
        ...
        DBG(2, fSCHED)
        DPrint(
            "WARNING:  attemting to requeue non-restartable job %s (job will "
            "be cancelled)\n",
            J->Name);
    ...
    if (!(J->Flags & (1 << mjfPreemptee))) {
        ...
        DBG(2, fSCHED)
        DPrint("WARNING:  attemting to requeue non-preemptible job %s\n",
               J->Name);
    ...
    if (J->Ckpt != NULL) {
        if ((J->Ckpt->StoredCPDuration == 0) ||
            (J->Ckpt->InitialStartTime == 0))
...
int MJobLocateResources(
    ...
    mjob_t *J, int TaskCount, int JobList[], mnalloc_t NodeList[])

{
    ...
    /* NOTE:  only preempt if started job is of greater value than sum of
     * preempted jobs */
...
int MJobSelectPreemptableJobs(
    ...
    mjob_t *PreemptorJ, mjob_t *PreempteeJ[])

{
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        ...
        if (!(J->Flags & (1 << mjfPreemptee))) continue;

        if (J->RunPriority >= PreemptorJ->StartPriority) continue;
...
int MJobGetAdjRunPriority(
    ...
    mjob_t *PreemptorJ, mjob_t *PreempteeJ, mnalloc_t FeasNodeList[],
    int *TaskCount)

{
    ...
    /* note:  value is proportional to amount of resources freed by preempting
     * job */
...

{
    ...
    if (J->StartTime < J->SubmitTime) {
        DBG(1, fSIM)
        DPrint(
            "ALERT:    simulation statistics corrupted (negative queue time: "
            "%ld)\n",
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
            if (nindex == MAX_MNODE) {
                DBG(0, fSCHED)
                DPrint(
                    "ERROR:    job node buffer overflow in job %s (iteration: "
                    "%d)\n",
                    J->Name, MSched.Iteration);
    ...
    DBG(3, fSIM)
    DPrint("INFO:     job %s terminated in state %s.  %3d nodes freed\n",
           J->Name, MJobState[J->State], J->NodeCount);
    ...
    if (J->ASFunc != NULL) (*J->ASFunc)(J, mascDestroy, NULL, NULL);
...

{
    ...
    switch (JobType) {
        case jtTrace:
            ...
            if (MTraceLoadWorkload((char *)JobSource, NULL, &tjob, MSched.Mode,
                                   &Version) == FAILURE) {
            ...
            if (MJobFind(tjob.Name, &tmpJ, 0) == SUCCESS) {
                DBG(4, fSIM)
                DPrint("INFO:     existing job %s resubmitted\n", tjob.Name);
        ...
        default:
            ...
            if (JobTraceIndex >= JobTraceCount) {
                if (MSimLoadWorkloadCache(MSched.HomeDir,
                ...
                DBG(4, fSIM)
                DPrint("INFO:     refreshed job cache with %d job traces\n",
                       JobTraceCount);
            }
    ...
    MJobGetName(tmpJ, NULL, &MRM[0], tmpJ->Name, sizeof(tmpJ->Name),
                mjnShortName);
    ...
    */
    ...
    if ((Now <= 0) || ((MSched.TimePolicy == mtpReal) &&
...
{
    if ((JName == NULL) || (R == NULL)) return (FAILURE);
    ...
    switch (R->Type) {
        case mrmtLL:
            ...
            sprintf(JName, "%s.%d.0", R->Name, R->JobCounter);
        ...
        default:
            ...
            sprintf(JName, "%d.%s", R->JobCounter, R->Name);
...
int MSimLoadWorkloadCache(
    ...
    char *Directory, char *TraceFile, int *TraceCount)

{
    ...
    DBG(3, fSIM)
    DPrint("INFO:     reading workload tracefile at %d byte offset\n",
           MSim.TraceOffset);
    ...
    for (jindex = 0; jindex < MAX_MJOB_TRACE; jindex++) {
        ...
        DBG(3, fSIM)
        DPrint("INFO:    freeing memory for trace record %d '%s'\n", jindex,
    ...
    while (ptr < tail) {
        if (MTraceLoadWorkload(ptr, &Offset, &tmpJ, MSched.Mode, &Version) ==
            SUCCESS) {
            Found = FALSE;
            ...
            if ((Found == TRUE) || (MJobFind(tmpJ.Name, &J, 0) == SUCCESS)) {
                if (Found == TRUE) {
                    DBG(1, fSIM)
                    DPrint(
                        "WARNING:  job '%s' previously detected in tracefile "
                        "(MJobTraceBuffer[%d]/JC: %d  IT: %d)\n",
                        tmpJ.Name, index, JobCount, MSched.Iteration);
                } else {
                    DBG(1, fSIM)
                    DPrint(
                        "WARNING:  job '%s' previously detected in tracefile "
                        "(Job[%d]/JC: %d  IT: %d)\n",
                        tmpJ.Name, J->Index, JobCount, MSched.Iteration);
                }
            } else {
    ...
    DBG(3, fSIM)
    DPrint("INFO:     jobs loaded: %d  earliest start: %ld  %s", JobCount,
           EarliestQueueTime, MULToDString(&EarliestQueueTime));

    if (TraceCount != NULL) *TraceCount = JobCount;
...

{
    ...
    if (MSim.RandomizeJobSize == TRUE) {
        ...
        DBG(6, fSIM)
        DPrint("INFO:     job '%s' tasks adjusted to %d\n", J->Name,
               J->Request.TC);
    }
    ...
    if (MSim.NCPolicy == msncPartitioned) {
        ...
        for (index = 6; index < MAX_MPAR; index++) {
            if (RQ->RequiredMemory <= (1 << index)) break;
        }
    ...
    if (J->SMinTime > 0) J->SMinTime = J->SubmitTime;
    ...
    /*
      if (J->SimWCTime > J->SpecWCLimit[0])
        {
        DBG(4,fSIM) DPrint("INFO:     scaling WC time for job '%s' to fit within
      WCLimit (%6ld > %6ld)\n",
          J->Name,
          J->WallClockTime,
          J->SpecWCLimit[0]);
    ...
    */
    ...
    if (MSim.WCAccuracy != 0.0) {
        ...
        DBG(3, fSIM)
        DPrint(
            "INFO:     WC Limit accuracy adjusted to %.2lf percent on job %s\n",
            MSim.WCAccuracy, J->Name);
        ...
        DBG(8, fSIM)
        DPrint(
            "INFO:     WC Limit scaled by %.2lf percent to %6ld on job '%s' "
            "(acc: %6.2f)\n",
            MSim.WCAccuracyChange * 100.0, J->SpecWCLimit[0], J->Name,
    ...
    if ((MSim.WCScalingPercent != 0) && (MSim.WCScalingPercent != 100)) {
        ...
        DBG(6, fSIM)
        DPrint("INFO:     job '%s' runtime scaled by %.2lf percent\n", J->Name,
...

{
    ...
    DBG(1, fSIM)
    DPrint(
        "INFO:     efficiency:  %3d / %3d  (%.2f percent) [total: %.2f]  jobs: "
        "%3d of %3d [total: %d]\n",
        (GP->UpNodes - GP->IdleNodes), GP->UpNodes,
        (double)(GP->UpNodes - GP->IdleNodes) / GP->UpNodes * 100.0,
        MStat.TotalPHBusy / MStat.TotalPHAvailable * 100.0, MStat.ActiveJobs,
        MStat.EligibleJobs, GP->S.Count);
    ...
    DBG(0, fSIM)
    DPrint("SUM:  Statistics for iteration %d: (Poll Interval: %ld) %s\n",
           Iteration, RMPollInterval, MULToDString(&MSched.Time));

    DBG(0, fSIM)
    DPrint(
        "SUM:  scheduler running for %7ld seconds (%6.2lf hours) initialized "
        "on %s\n",
        SchedRunTime / 100, (double)SchedRunTime / 360000,
        MULToDString(&MStat.InitTime));

    DBG(0, fSIM)
    DPrint(
        "SUM:  active jobs:          %8d  eligible jobs:       %8d  idle jobs: "
        "     %5d\n",
        RunJobs, EligibleJobs, IdleQueueSize);

    if (TotalJobsCompleted != 0) {
        DBG(0, fSIM)
        DPrint(
            "SUM:  completed jobs:       %8d  successful jobs:     %8d  avg "
            "XFactor:    %7.2f\n",
            TotalJobsCompleted, SuccessfulJobsCompleted,
    ...
    if (TotalProcHours != 0.0) {
        ...
        DBG(0, fSIM)
        DPrint(
            "SUM:  available proc hours:%9.2f  running proc hours: %9.2f  "
            "efficiency:     %7.3f\n",
            TotalProcHours, RunningProcHours, SchedEfficiency * 100);

        DBG(0, fSIM)
        DPrint(
            "SUM:  preempt proc hours:  %9.2f  preempt jobs:      %d         "
            "preempt loss:   %7.3f\n",
            (double)MStat.TotalPHPreempted / 3600.0, MStat.TotalPreemptJobs,
            (double)MStat.TotalPHPreempted / 3600.0 / TotalProcHours * 100);

        DBG(0, fSIM)
        DPrint("SUM:        min efficiency: %8.2f  iteration:           %8d\n",
               MinEff, MinEffIteration);
    } else {
        SchedEfficiency = 1.0;
    ...
    DBG(0, fSIM)
    DPrint(
        "SUM:  available nodes:      %8d  busy nodes:          %8d  "
        "efficiency:     %7.3f\n",
        UpNodes, BusyNodes, (double)BusyNodes / UpNodes * 100.0);

    if (SuccessfulProcHours != 0.0) {
        DBG(0, fSIM)
        DPrint(
            "SUM:  wallclock accuracy:    %7.3f  weighted wc accuracy: %7.3f\n",
    ...
    if (TotalProcHours != 0.0) {
        DBG(0, fSIM)
        DPrint(
            "SUM:  est backlog hours:     %7.3lf  avg backlog:         "
            "%8.3lf\n",
    ...
    DBG(0, fSIM)
    DPrint("SUM:  backfill policy:      %8s (%d)  BFDepth:            %3d\n",
           MBFPolicy[GP->BFPolicy], MStat.JobsEvaluated, GP->BFDepth);

    DBG(0, fSIM)
    DPrint(
        "SUM:  allocation:    %15s  wc accuracy:            %5.2lf  wc "
        "accuracy change:      %5.2lf\n",
        MNAllocPolicy[GP->NAllocPolicy], MSim.WCAccuracy,
        MSim.WCAccuracyChange);

    DBG(0, fSIM)
    DPrint(
        "SUM:  nodes:                %8d  nodeconfig:          %8s  "
        "reservation policy:  %14s\n",
        GP->UpNodes, MSimNCPolicy[MSim.NCPolicy], MResPolicy[GP->ResPolicy]);

    DBG(0, fSIM)
    DPrint("SUM:  queuedepth:          %9d  submitpolicy: %14s\n",
           MSim.InitialQueueDepth, MSimQPolicy[MSim.JobSubmissionPolicy]);

    if ((MSched.Mode == msmSim) && (MSim.ComRate != 0.0)) {
        ProcEfficiency = RunningProcHours * 36.0 /
                         (RunningProcHours * 36.0 + MStat.TotalSimComCost);

        DBG(0, fSIM)
        DPrint(
            "SUM:  processing hours:    %9.2f  comm hours:        %10.2f  proc "
            "eff: %7.2f  total efficiency:  %7.2f\n",
            RunningProcHours, MStat.TotalSimComCost / 3600,
            ProcEfficiency * 100.0, SchedEfficiency * ProcEfficiency * 100.0);
    }
...

{
    ...
    DBG(1, fSIM)
    DPrint("INFO:     Queue Depth:  %8d  Submit Policy:   %15s\n",
           MSim.InitialQueueDepth, MSimQPolicy[MSim.JobSubmissionPolicy]);

    DBG(1, fSIM)
    DPrint(
        "INFO:     BF Policy: %5s  Allocation:  %20s  Reservation Policy:  "
        "%15s\n",
        MBFPolicy[GP->BFPolicy], MNAllocPolicy[GP->NAllocPolicy],
        MResPolicy[GP->ResPolicy]);

    if (GP->BFPolicy != 0) {
        DBG(1, fSIM)
        DPrint("INFO:     BFDepth:                 %8d  BFMetric: %15s\n",
               GP->BFDepth, MBFMPolicy[GP->BFMetric]);
    }

    DBG(1, fSIM)
    DPrint("INFO:     Nodes (UP):   %8d  Nodes (Idle):            %8d\n",
           GP->UpNodes, GP->IdleNodes);

    DBG(1, fSIM)
    DPrint("INFO:     Procs (UP):   %8d  Procs (Idle):            %8d\n",
           GP->URes.Procs, GP->ARes.Procs);

    DBG(1, fSIM)
    DPrint(
        "INFO:     ScaleFactor:  %8d  WC Accuracy:             %8.2lf  WC "
        "Accuracy Change:  %15.2lf\n",
        MSim.WCScalingPercent, MSim.WCAccuracy, MSim.WCAccuracyChange);
...
int MSimShow(
    ...
    msim_t *S, char *Buffer, int DMode)

{
    ...
    sprintf(temp_str, "%-30s  %s\n", MParam[pWorkloadTraceFile],
            S->WorkloadTraceFile);
    strcat(Buffer, temp_str);
    sprintf(temp_str, "%-30s  %s\n", MParam[pResourceTraceFile],
            S->ResourceTraceFile);
    strcat(Buffer, temp_str);
    sprintf(temp_str, "%-30s  %s\n", MParam[pSimAutoShutdown],
            MPolicyMode[S->AutoShutdown]);
    strcat(Buffer, temp_str);
    sprintf(temp_str, "%-30s  %ld\n", MParam[pSimStartTime], S->StartTime);
    strcat(Buffer, temp_str);
    sprintf(temp_str, "%-30s  %s\n", MParam[pSimScaleJobRunTime],
            (S->ScaleJobRunTime == TRUE) ? "TRUE" : "FALSE");
    strcat(Buffer, temp_str);
    ...
    if (S->ComRate != 0.0) {
        ...
        strcat(Buffer, temp_str);
        sprintf(temp_str, "%-30s  %s\n", MParam[pCommunicationType],
                MComType[S->CommunicationType]);
        strcat(Buffer, temp_str);
        sprintf(temp_str, "%-30s  %4.2f\n", MParam[pIntraFrameCost],
                S->IntraFrameCost);
        strcat(Buffer, temp_str);
        sprintf(temp_str, "%-30s  %4.2f\n", MParam[pInterFrameCost],
                S->InterFrameCost);
        strcat(Buffer, temp_str);
    }
...

{
    ...
    if (MSched.Iteration == MSim.ExitIteration) {
        DBG(0, fSCHED)
        DPrint("INFO:     scheduling complete (%s reached) on iteration %d\n",
               MParam[pExitIteration], MSched.Iteration);
    ...
    if ((MSim.AutoShutdown == TRUE) && (IQ[0] == -1) && (MAQ[0] == -1)) {
        DBG(0, fSCHED)
        DPrint(
            "INFO:     scheduling complete (queues are empty) on iteration "
            "%d\n",
            MSched.Iteration);
    ...
    if (!(MSched.Iteration % 1000)) {
        DBG(0, fSCHED)
        DPrint("INFO:     iteration %d completed\n", MSched.Iteration);
    }
...
int MSimProcessOConfig(
    ...
    msim_t *S, int PIndex, int IVal, double DVal, char *SVal, char **SArray)
    ...
    switch (PIndex) {
        ...
        case pSimNodeCount:
            ...
            if (IVal > MAX_MNODE) {
                DBG(1, fCONFIG)
                DPrint(
                    "WARNING:  specified nodecount value (%d) is too high  "
                    "(adjusting to %d)\n",
                    IVal, MAX_MNODE);
            }
...

        {
            ...
            for (index = 0; SArray[index] != NULL; index++) {
                if ((findex = MUGetIndex(SArray[index], MSimFlagType, FALSE,
                                         0)) == 0)
                    continue;
        ...
        case pSimJobSubmissionPolicy:
            ...
            if ((S->JobSubmissionPolicy = MUGetIndex(
                     SVal, MSimQPolicy, 0, S->JobSubmissionPolicy)) == 0) {
                DBG(1, fCONFIG)
                DPrint("ALERT:    invalid %s parameter specified '%s'\n",
                       MParam[pSimJobSubmissionPolicy], SVal);
            }
