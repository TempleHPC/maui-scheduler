...
int MWikiJobRequeue(
    ...
    mjob_t *J, mrm_t *R, mjob_t **JPeer, char *Msg, int *SC)
...
int MWikiClusterLoadInfo(
    ...
    mrm_t *R, int *RCount, char *EMsg, int *SC)

{
    ...
    if ((S == NULL) || (S->ClusterInfoBuffer == NULL)) {
        sprintf(ArgLine, "%ld:ALL", MTime);

        sprintf(Command, "%s%s %s%s", MCKeyword[mckCommand],
                WMCommandList[cmdWMGetNodes], MCKeyword[mckArgs], ArgLine);

        if (MWikiDoCommand(R->P[0].HostName, R->P[0].Port,
                           R->P[0].Timeout * 1000000, R->AuthType, Command,
                           &WikiBuf, &DataSize, &tmpSC) == FAILURE) {
            DBG(2, fWIKI)
            DPrint("ALERT:    cannot get node list from %s RM\n",
                   MRMType[R->Type]);
        ...
        DBG(2, fWIKI)
        DPrint("INFO:     received node list through %s RM\n",
               MRMType[R->Type]);
    ...
    if ((ptr = strstr(*Data, MCKeyword[mckArgs])) == NULL) {
        DBG(1, fWIKI)
        DPrint("ALERT:    cannot locate ARG marker in %s()\n", FName);
    ...
    /* FORMAT:
     * <NODECOUNT>#<NODEID>:<FIELD>=<VALUE>;[<FIELD>=<VALUE>;]...#[<NODEID>:...]
     */
    ...
    if (ncount <= 0) {
        DBG(1, fWIKI)
        DPrint("INFO:     no node data sent by %s RM\n", MRMType[R->Type]);
    ...
    if ((ptr = MUStrChr(ptr, '#')) == NULL) {
        DBG(1, fWIKI)
        DPrint("ALERT:    cannot locate start marker for first node in %s()\n",
               FName);
    ...
    for (nindex = 0; nindex < ncount; nindex++) {
        if (MWikiGetAttr(mxoNode, Name, &Status, AttrLine, &ptr) == FAILURE) {
            DBG(2, fWIKI)
            DPrint("ALERT:    cannot get wiki state information\n");
...

{
    ...
    if ((S == NULL) || (S->WorkloadInfoBuffer == NULL)) {
        ...
        sprintf(Command, "%s%s %s%s", MCKeyword[mckCommand],
                WMCommandList[cmdWMGetJobs], MCKeyword[mckArgs], ArgLine);

        if (MWikiDoCommand(R->P[0].HostName, R->P[0].Port,
                           R->P[0].Timeout * 1000000, R->AuthType, Command,
                           &WikiBuf, &DataSize, &tmpSC) == FAILURE) {
            DBG(2, fWIKI)
            DPrint("ALERT:    cannot get job list from %s RM\n",
                   MRMType[R->Type]);
        ...
        DBG(2, fWIKI)
        DPrint("INFO:     received job list through %s RM\n", MRMType[R->Type]);
    ...
    if (jcount <= 0) {
        DBG(1, fWIKI)
        DPrint("INFO:     no job data sent by %s RM\n", MRMType[R->Type]);
    ...
    if ((ptr = MUStrChr(ptr, '#')) == NULL) {
        DBG(1, fWIKI)
        DPrint("ALERT:    cannot locate start marker for first job in %s()\n",
               FName);
    ...
    for (jindex = 0; jindex < jcount; jindex++) {
        ...
        switch (Status) {
            ...
            case mjsSuspended:
                ...
                if (MJobFind(Name, &J, 0) == SUCCESS) {
                ...
                } else if (MJobCreate(MJobGetName(NULL, Name, R, NULL, 0,
                                                  mjnShortName),
                                      TRUE, &J) == SUCCESS) {
                    ...
                    if (MWikiJobLoad(Name, AttrLine, J, TaskList, R) ==
                        FAILURE) {
                        DBG(1, fWIKI)
                        DPrint("ALERT:    cannot load wiki job '%s'\n", Name);
                    ...
                    DBG(1, fWIKI)
                    DPrint(
                        "ERROR:    job buffer is full  (ignoring job '%s')\n",
                        Name);
                }
            ...
            case mjsVacated:
                ...
                if (MJobFind(Name, &J, 0) == SUCCESS) {
                    ...
                    if ((J->State != mjsRunning) && (J->State != mjsStarting)) {
                        DBG(1, fWIKI)
                        DPrint("INFO:     job '%s' was cancelled externally\n",
                               J->Name);
                    ...
                    switch (Status) {
                        ...
                        case mjsVacated:
                            ...
                            if (MSched.Time < (J->StartTime + J->WCLimit)) {
                                ...
                                sprintf(Message,
                                        "JOBWCVIOLATION:  job '%s' exceeded WC "
                                        "limit %s\n",
                        ...
                        default:
                            ...
                            DBG(1, fWIKI)
                            DPrint(
                                "WARNING:  unexpected job state (%d) detected "
                                "for job '%s'\n",
                                Status, J->Name);
            ...
            default:
                ...
                DBG(1, fWIKI)
                DPrint(
                    "WARNING:  job '%s' detected with unexpected state '%d'\n",
                    Name, Status);
...
int MWikiJobStart(
    ...
    mjob_t *J, mrm_t *R, char *Msg, int *SC)

{
    ...
    for (tindex = 0; tindex < MAX_MTASK_PER_JOB; tindex++) {
        if (J->TaskMap[tindex] == -1) break;

        N = MNode[J->TaskMap[tindex]];

        if (N->RM == R) {
            if (tcount > 0) MUStrCat(TaskLine, ":", sizeof(TaskLine));
    ...
    if (tcount == 0) {
        DBG(3, fWIKI)
        DPrint("ALERT:    no tasks found for job '%s' on RM %d\n", J->Name,
               R->Index);

        return (FAILURE);
    }

    sprintf(ArgLine, "%s TASKLIST=%s", J->Name, TaskLine);

    sprintf(Command, "%s%s %s%s", MCKeyword[mckCommand],
            WMCommandList[cmdWMStartJob], MCKeyword[mckArgs], ArgLine);

    if (MWikiDoCommand(R->P[0].HostName, R->P[0].Port,
                       R->P[0].Timeout * 1000000, R->AuthType, Command,
                       &Response, NULL, &tmpSC) == FAILURE) {
        DBG(2, fWIKI)
        DPrint(
            "ALERT:    cannot start job '%s' on %s RM on %d procs (command "
            "failure)\n",
            J->Name, MRMType[R->Type], tcount);
    ...
    if (tmpSC < 0) {
        DBG(2, fWIKI)
        DPrint(
            "ALERT:    cannot start job '%s' on %s RM on %d procs (rm "
            "failure)\n",
            J->Name, MRMType[R->Type], tcount);
    ...
    DBG(2, fWIKI)
    DPrint("INFO:     job '%s' started through %s RM on %d procs\n", J->Name,
           MRMType[R->Type], tcount);
...

{
    ...
    sprintf(Command, "%s%s %s%s TYPE=%s", MCKeyword[mckCommand],
            WMCommandList[cmdWMCancelJob], MCKeyword[mckArgs], J->Name,
            CancelType);
    ...
    if (MWikiDoCommand(R->P[0].HostName, R->P[0].Port,
                       ...
                       &Response, NULL, &tmpSC) == FAILURE) {
        DBG(2, fWIKI)
        DPrint("ALERT:    cannot cancel job '%s' through %s RM\n", J->Name,
               MRMType[R->Type]);
    ...
    DBG(2, fWIKI)
    DPrint("INFO:     job '%s' cancelled through %s RM\n", J->Name,
           MRMType[R->Type]);
...

{
    ...
    sprintf(Command, "%s%s %s%s", MCKeyword[mckCommand],
            WMCommandList[cmdWMSuspendJob], MCKeyword[mckArgs], J->Name);

    if (MWikiDoCommand(R->P[0].HostName, R->P[0].Port,
                       R->P[0].Timeout * 1000000, R->AuthType, Command,
                       &Response, NULL, &tmpSC) == FAILURE) {
        DBG(2, fWIKI)
        DPrint("ALERT:    cannot suspend job '%s' through %s RM\n", J->Name,
               MRMType[R->Type]);
    ...
    DBG(2, fWIKI)
    DPrint("INFO:     job '%s' suspended through %s RM\n", J->Name,
           MRMType[R->Type]);
...

{
    ...
    sprintf(Command, "%s%s %s%s", MCKeyword[mckCommand],
            WMCommandList[cmdWMResumeJob], MCKeyword[mckArgs], J->Name);

    if (MWikiDoCommand(R->P[0].HostName, R->P[0].Port,
                       R->P[0].Timeout * 1000000, R->AuthType, Command,
                       &Response, NULL, &tmpSC) == FAILURE) {
        DBG(2, fWIKI)
        DPrint("ALERT:    cannot resume job '%s' through %s RM\n", J->Name,
               MRMType[R->Type]);
    ...
    DBG(2, fWIKI)
    DPrint("INFO:     job '%s' resumed through %s RM\n", J->Name,
           MRMType[R->Type]);
...

{
    ...
    if (MGroupAdd("NOGROUP", &J->Cred.G) == FAILURE) {
        DBG(1, fWIKI)
        DPrint("ERROR:    cannot add default group for job %s\n", J->Name);
    }
    ...
    while (JobAttr != NULL) {
        while (isspace(*JobAttr) && (*JobAttr != '\0')) JobAttr++;
    ...
    if ((J->Cred.U == NULL) || (J->Cred.G == NULL)) {
        DBG(1, fWIKI)
        DPrint("ERROR:    cannot authenticate job '%s' (no user/no group)\n",
               J->Name);
    ...
    if (MJobSetCreds(J, J->Cred.U->Name, J->Cred.G->Name, AccountName) ==
        FAILURE) {
        DBG(1, fWIKI)
        DPrint(
            "ERROR:    cannot authenticate job '%s' (U: %s  G: %s  A: '%s')\n",
            J->Name, J->Cred.U->Name, J->Cred.G->Name, AccountName);
    ...
    if (J->Request.TC == 0) {
        ...
        if (J->Request.TC == 0) {
            DBG(1, fWIKI)
            DPrint("ERROR:    no taskcount specified for job '%s'\n", J->Name);
    ...
    if ((J->Cred.G == NULL) || !strcmp(J->Cred.G->Name, "NOGROUP")) {
        ...
        if ((GID = MUGIDFromUID(J->Cred.U->OID)) > 0) {
            ...
            if (strncmp(ptr, "GID", strlen("GID"))) {
                ...
                DBG(1, fWIKI)
                DPrint("INFO:     job '%s' assigned default group '%s'\n",
                       J->Name, MUGIDToName(GID));
            }
        }
...

{
    ...
    while (ptr != NULL) {
        if (MNodeFind(ptr, &N) == FAILURE) {
            DBG(1, fWIKI)
            DPrint("ALERT:    cannot locate host '%s' for hostlist\n", ptr);
...
int MWikiUpdateJob(
    ...
    char AList[], mjob_t *J, int RMIndex)

{
    ...
    while ((tail = MUStrChr(ptr, ';')) != NULL) {
        ...
        if (!strncmp(JobAttr, "TASKLIST=", strlen("TASKLIST="))) {
            MWikiAttrToTaskList(TaskList, &JobAttr[strlen("TASKLIST=")],
                                &J->TaskCount);
        } else {
            MWikiUpdateJobAttr(JobAttr, J);
        }
...
int MWikiNodeLoad(
    ...
    char *AString, mnode_t *N, mrm_t *R)

{
    ...
    DBG(6, fWIKI)
    DPrint(
        "INFO:     MNode[%03d] '%18s' %9s VM: %8d Mem: %5d Dk: %5d Cl: %s %s\n",
        N->Index, N->Name, MAList[eNodeState][N->State], N->CRes.Swap,
        N->CRes.Mem, N->CRes.Disk, MUCAListToString(N->CRes.PSlot, NULL, NULL),
...

{
    ...
    if ((Tok[0] == 'A') && (isdigit(Tok[1]))) {
        ...
        if (aindex > MAX_WIKINODEATTR) aindex = MAX_WIKINODEATTR;
    } else {
        for (aindex = 0; MWikiNodeAttr[aindex] != NULL; aindex++) {
            if (!strncmp(MWikiNodeAttr[aindex], Tok,
                         strlen(MWikiNodeAttr[aindex])))
                break;
        } /* END for (aindex) */
    }
    ...
    switch (aindex) {
        ...
        case mwnaCClass:
            ...
            if (strcmp(Value, NONE)) {
                ...
                for (cindex = 1; cindex < MAX_MCLASS; cindex++) {
                    ...
                    DBG(6, fWIKI)
                    DPrint("INFO:     configured class '%s'(x%d) added (%s)\n",
                           MAList[eClass][cindex], N->CRes.PSlot[cindex].count,
        ...
        case mwnaAClass:
            ...
            if (strcmp(Value, NONE)) {
                ...
                for (cindex = 1; cindex < MAX_MCLASS; cindex++) {
                    if (N->ARes.PSlot[cindex].count <= 0) continue;

                    DBG(6, fWIKI)
                    DPrint("INFO:     available class '%s'(x%d) added (%s)\n",
                           MAList[eClass][cindex], N->ARes.PSlot[cindex].count,
...

{
    ...
    if ((Tok[0] == 'A') && (isdigit(Tok[1]))) {
        ...
        if (aindex > MAX_WIKIJOBATTR) aindex = MAX_WIKIJOBATTR;
    } else {
    ...
    if (aindex == 0) {
        ...
        DBG(1, fWIKI)
        DPrint("INFO:     unexpected WIKI job attr '%s' detected\n", Tok);
    ...
    switch (aindex) {
        ...
        case mwjaState:
            ...
            if (strcmp(MJobState[J->State], Value)) {
                ...
                DPrint("INFO:     job '%s' changed states from %s to %s\n",
                       J->Name, MAList[eJobState][J->State], Value);
        ...
        case mwjaQueueTime:
            ...
            if (J->SubmitTime > MSched.Time) {
                DBG(2, fWIKI)
                DPrint(
                    "WARNING:  clock skew detected (queue time for job %s in "
                    "%s)\n",
                    J->Name, MULToTString(J->SubmitTime - MSched.Time));
        ...
        case mwjaStartTime:
            ...
            if (J->StartTime > MSched.Time) {
                DBG(2, fWIKI)
                DPrint(
                    "WARNING:  clock skew detected (start time for job %s in "
                    "%s)\n",
                    J->Name, MULToTString(J->StartTime - MSched.Time));
        ...
        case mwjaCompletionTime:
            ...
            if (J->CompletionTime > MSched.Time) {
                DBG(2, fWIKI)
                DPrint(
                    "WARNING:  clock skew detected (completion time for job %s "
                    "in %s)\n",
                    J->Name, MULToTString(J->CompletionTime - MSched.Time));
        ...
        case mwjaUName:
            ...
            if (MUserAdd(Value, &J->Cred.U) == FAILURE) {
                DBG(1, fWIKI)
                DPrint("ERROR:    cannot add user for job %s (UserName: %s)\n",
                       J->Name, Value);
            }
        ...
        case mwjaGName:
            ...
            if (MGroupAdd(Value, &J->Cred.G) == FAILURE) {
                DBG(1, fWIKI)
                DPrint(
                    "ERROR:    cannot add group for job %s (GroupName: %s)\n",
                    J->Name, Value);
            }
        ...
        case mwjaAccount:
            ...
            if (MAcctAdd(Value, &J->Cred.A) == FAILURE) {
                DBG(1, fWIKI)
                DPrint(
                    "ERROR:    cannot add account for job %s (Account: %s)\n",
                    J->Name, Value);
            }
        ...
        case mwjaRFeatures:
            ...
            MUMAMAttrFromLine(eFeature, Value, mAdd, RQ->ReqFBM,
                              sizeof(RQ->ReqFBM));
        ...
        case mwjaRClass:
            ...
            if (strstr(Value, NONE) == NULL) {
                ...
                do {
                    ...
                    if ((CIndex = MUMAGetIndex(eClass, tok, mAdd)) == FAILURE) {
                        DBG(2, fWIKI)
                        DPrint("WARNING:  cannot add class '%s' for job %s\n",
                               tok, J->Name);
            ...
            DBG(1, fWIKI)
            DPrint("INFO:    classes loaded for job %s : %s\n", J->Name,
        ...
        case mwjaHostList:
            ...
            while (ptr != NULL) {
                if (MNodeFind(ptr, &N) == FAILURE) {
                    ...
                    sprintf(HostName, "%s.%s", ptr, MSched.DefaultDomain);
        ...
        case mwjaResAccess:
            ...
            while (ptr != NULL) {
                if (strcmp(ptr, "FREE")) {
                    ...
                    DBG(4, fWIKI)
                    DPrint(
                        "INFO:     job %s granted access to reservation %s\n",
                        J->Name, J->RAList[rindex]);
        ...
        default:
            ...
            DBG(2, fWIKI)
            DPrint("INFO:     WIKI keyword '%s'(%d) not handled\n",
                   MWikiJobAttr[aindex], aindex);
...
int MWikiGetAttr(
    ...
    int OIndex, char *Name, int *Status, char *Attr, char **Start)

{
    ...
    if ((ptr = MUStrChr(attr, '#')) == NULL) {
        ...
        if (Start != NULL) *Start = *Start + strlen(*Start);
    } else {
        MUStrCpy(Attr, attr, MIN(ptr - attr + 1, MAX_MBUFFER));

        if (Start != NULL) *Start = ptr + 1;
    }
...
int MWikiDoCommand(
    ...
    char *HostName, int Port, long TO, int Auth, char *Command, char **Data,
    long *DataSize, int *SC)

{
    ...
    DBG(2, fSCHED)
    DPrint("%s(%s,%d,%ld,%s,%s,Data,DataSize,SC)\n", FName, HostName, Port, TO,
           MRMAuthType[Auth], Command);
    ...
    if (HostName == NULL) {
        ...
        if (MSUConnect(&S, FALSE, NULL) == FAILURE) {
            DBG(0, fSOCK)
            DPrint("ERROR:    cannot connect to server %s:%d\n", S.RemoteHost,
                   S.RemotePort);
    ...
    if (MSUSendData(&S, TO, UseCheckSum, FALSE) == FAILURE) {
        DBG(0, fSOCK)
        DPrint("ERROR:    cannot send data to server %s:%d\n", HostName, Port);
    ...
    if (MSURecvData(&S, TO, UseCheckSum, NULL, NULL) == FAILURE) {
        DBG(0, fSOCK)
        DPrint("ERROR:    cannot receive data from server %s:%d\n", HostName,
               Port);
    ...
    DBG(4, fUI)
    DPrint("INFO:     received message '%s' from wiki server\n", S.RBuffer);
    ...
    if ((ptr = strstr(S.RBuffer, MCKeyword[mckStatusCode])) != NULL) {
        ...
        if (*SC < 0) {
            DBG(0, fSOCK)
            DPrint("ERROR:    command '%s'  SC: %d  response: '%s'\n", Command,
                   *SC, (Response == NULL) ? "NONE" : Response);
        ...
        DBG(0, fSOCK)
        DPrint(
            "ERROR:    cannot determine statuscode for command '%s'  SC: %d  "
            "response: '%s'\n",
            Command, *SC, (Response == NULL) ? "NONE" : Response);
    ...
    *Data = S.RBuffer;
    ...
    if (DataSize != NULL) *DataSize = S.RBufSize;
