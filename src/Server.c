...
int ServerDemonize() {
    ...
    if (MSched.Mode != msmSim) {
        if (setpgid(0, 0) == -1) {
            ...
            DBG(0, fALL)
            DPrint("ERROR:    cannot setpgrp, errno: %d (%s)\n", errno,
                   strerror(errno));
        }
        ...
        if (MSched.DebugMode == FALSE) {
            ...
            if ((pid = fork()) == -1) {
                ...
                DBG(0, fALL)
                DPrint(
                    "ALERT:    cannot fork into background, errno: %d (%s)\n",
            ...
            if (setsid() == -1) {
                MDB(3, fALL)
                MLog(
                    "INFO:     could not disconnect from controlling-terminal, "
                    "errno=%d - %s\n",
                    errno, strerror(errno));
            }
...

{
    ...
    for (aindex = 1; aindex < ArgC; aindex++) {
        if ((ArgV[aindex] == NULL) || (aindex >= 1024)) break;
    ...
    if (MSysLoadConfig(MSched.HomeDir, MSched.ConfigFile, (1 << mcmForce)) ==
        FAILURE) {
        if (MSysLoadConfig(".", MSched.ConfigFile, (1 << mcmForce)) ==
            FAILURE) {
            DBG(3, fALL)
            DPrint(
                "WARNING:  cannot locate configuration file '%s' in '%s' or in "
                "current directory\n",
                MSched.ConfigFile, MSched.HomeDir);
        }
    }
    ...
    /* walk to end of ArgV[] because ServerProcessArgs() may free portions of
       tmpArgV[]
       as side-affect */

    for (aindex = 0; aindex < ArgC; aindex++) {
        if (ArgV[aindex] == NULL) break;
    ...
    while (1) {
        ...
        DBG(2, fALL)
        DPrint("INFO:     starting iteration %d\n", MSched.Iteration);
        ...
        if (MSched.Mode != msmSim) {
            DBG(1, fSCHED)
            DPrint("INFO:     scheduling complete.  sleeping %ld seconds\n",
        ...
        if ((MSched.Mode == msmSingleStep) && (MSched.StepCount <= 1)) {
            DBG(4, fSCHED)
            DPrint(
                "INFO:     scheduler has completed single step scheduling\n");

            break;
        } else if ((MSched.Mode == msmSingleStep) &&
                   (MSched.Iteration >= MSched.StepCount)) {
            DBG(4, fSCHED)
            DPrint(
                "INFO:     scheduler has completed multi-step scheduling (%d "
                "steps)\n",
                MSched.StepCount);
    ...
    DBG(1, fSCHED)
    DPrint("INFO:     scheduling completed on %s\n",
...

{
    ...
    if (ArgC > 1) {
        ...
        if ((ArgV[aindex][0] == '-') && (ArgV[aindex][1] == 'L')) {
            if (ArgV[aindex][2] != '\0') {
            ...
            DBG(3, fCONFIG)
            DPrint("INFO:     early LOGLEVEL set to %d\n", mlog.Threshold);
        }

        if ((ArgC > aindex) && (ArgV[aindex][0] == '-') &&
            (ArgV[aindex][1] == 'l')) {
            if (ArgV[aindex][2] != '\0') {
            ...
            DBG(3, fCONFIG)
            DPrint("INFO:     early LOGFILE set to %s\n", LogFile);
        }
    }
    ...
    if ((LogFile[0] != '/') && (LogFile[0] != '~')) {
        ...
        if ((ptr = getenv(MSCHED_ENVHOMEVAR)) != NULL) {
            ...
            sprintf(LogFile, "%s/%s", tmpLine, DEFAULT_MLOGDIR);
        } else {
#ifdef MBUILD_HOMEDIR
            sprintf(LogFile, "%s/%s", MBUILD_HOMEDIR, DEFAULT_MLOGDIR);
#else  /* MBUILD_HOMEDIR */
...

{
    ...
    if (MSched.LogFile[0] == '\0') {
        ...
        sprintf(temp_str, "%-30s  %s\n", MParam[pSchedLogFile], MSched.LogFile);
        strcat(Buffer, temp_str);
        sprintf(temp_str, "%-30s  %d\n", MParam[pLogFileMaxSize],
                MSched.LogFileMaxSize);
        strcat(Buffer, temp_str);
        sprintf(temp_str, "%-30s  %d\n", MParam[pLogFileRollDepth],
    ...
    if ((mlog.FacilityList != fALL) ||
        (VFlag || (PIndex == -1) || (PIndex == pLogFacility))) {
        Line[0] = '\0';
        ...
        if (mlog.FacilityList == fALL) strcpy(Line, MLogFacilityType[dfALL]);
    ...
    if ((MSched.Action[mactAdminEvent][0] != '\0') || VFlag) {
        sprintf(temp_str, "%-30s  %s\n", MParam[pAdminEAction],
                MSched.Action[mactAdminEvent]);
        strcat(Buffer, temp_str);
    }

    if ((MSched.Action[mactJobFB][0] != '\0') || VFlag) {
        sprintf(temp_str, "%-30s  %s\n", MParam[mcoJobFBAction],
                MSched.Action[mactJobFB]);
        strcat(Buffer, temp_str);
    }

    if ((MSched.Action[mactMail][0] != '\0') || VFlag) {
        sprintf(temp_str, "%-30s  %s\n", MParam[mcoMailAction],
    ...
    strcat(Buffer, temp_str);
    sprintf(temp_str, "%-30s  %s\n", MParam[pCheckPointInterval],
            MULToTString(MCP.CPInterval));
    strcat(Buffer, temp_str);
    sprintf(temp_str, "%-30s  %s\n", MParam[pCheckPointExpirationTime],
            MULToTString(MCP.CPExpirationTime));
    strcat(Buffer, temp_str);

    if ((MSched.MonitoredJob[0] != '\0') ||
        (VFlag || (PIndex == -1) || (PIndex == pMonitoredJob))) {
        sprintf(temp_str, "%-30s  %s\n", MParam[pMonitoredJob],
                MSched.MonitoredJob);
        strcat(Buffer, temp_str);
    }

    if ((MSched.MonitoredNode[0] != '\0') ||
        (VFlag || (PIndex == -1) || (PIndex == pMonitoredNode))) {
        sprintf(temp_str, "%-30s  %s\n", MParam[pMonitoredNode],
                MSched.MonitoredNode);
        strcat(Buffer, temp_str);
    }

    if ((MSched.MonitoredFunction[0] != '\0') ||
        (VFlag || (PIndex == -1) || (PIndex == pMonitoredFunction))) {
        sprintf(temp_str, "%-30s  %s\n", MParam[pMonitoredFunction],
                MSched.MonitoredFunction);
        strcat(Buffer, temp_str);
    }

    if ((MSched.ResDepth != DEFAULT_RES_DEPTH) ||
        (VFlag || (PIndex == -1) || (PIndex == pResDepth))) {
...
int ServerProcessArgs(
    ...
    int ArgC, char **ArgV, /* I (potentially modified) */
...

{
    ...
    while ((Flag = MUGetOpt(&tmpArgC, ArgV, AString, &OptArg, &OptTok)) !=
           (char)-1) {
        switch (Flag) {
            case '-':

            {
                ...
                if (!strcmp(OptArg, "about")) {
                    ...
                    MUStrCpy(MSched.ConfigFile, Value,
                             sizeof(MSched.ConfigFile));

                    DBG(3, fCONFIG)
                    DPrint("INFO:     server configfile set to %s\n",
                           MSched.ConfigFile);
                    ...
                    DBG(3, fCONFIG)
                    DPrint("INFO:     log level set to %d\n", mlog.Threshold);

                    break;
                } else if ((!strcmp(OptArg, "host")) && (Value != NULL)) {
                    MUStrCpy(MSched.ServerHost, Value,
                             sizeof(MSched.ServerHost));
                    ...
                    DBG(3, fCONFIG)
                    DPrint("INFO:     server port set to %d\n",
                           MSched.ServerPort);
            ...
            case 'a':
                ...
                MPar[0].NAllocPolicy = MUGetIndex(OptArg, MNAllocPolicy, FALSE,
                                                  MPar[0].NAllocPolicy);

                MPar[1].NAllocPolicy = MPar[0].NAllocPolicy;

                DBG(3, fCONFIG)
                DPrint("INFO:     node allocation policy set to %s\n",
            ...
            case 'A':
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     scheduler fallback mode enabled\n");
            ...
            case 'b':
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     backfill policy type set to %s\n",
            ...
            case 'B':
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     backfill depth set to %d\n", MPar[0].BFDepth);
            ...
            case 'c':
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     backfill metric set to %s\n",
                ...
                if (mlog.FacilityList == 0) {
                    for (index = 0; MLogFacilityType[index] != NULL; index++) {
                        if (strstr(OptArg, MLogFacilityType[index]) != NULL) {
                            ...
                            if (index == dfALL) mlog.FacilityList |= fALL;
                        }
                    } /* END for (index) */
                }

                DBG(3, fCONFIG)
                DPrint("INFO:     LOGFACILITY set to %ld\n", mlog.FacilityList);
            ...
            case 'i':
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     RMPollInterval set to %ld seconds\n",
                       MSched.RMPollInterval);
            ...
            case 'j':
                ...
                MUStrCpy(MSim.WorkloadTraceFile, OptArg,
                         sizeof(MSim.WorkloadTraceFile));

                DBG(3, fCONFIG)
                DPrint(
                    "INFO:     simulation mode set.  using workload tracefile "
                    "'%s'\n",
            ...
            case 'm':
                ...
                MSched.Mode =
                    MUGetIndex(OptArg, MSchedMode, FALSE, MSched.Mode);
            ...
            case 'n':
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     nodecount set to %d\n", MSim.NodeCount);
            ...
            case 'N':
                ...
                MSim.NCPolicy =
                    MUGetIndex(OptArg, MSimNCPolicy, FALSE, MSim.NCPolicy);

                DBG(3, fCONFIG)
                DPrint("INFO:     nodeconfiguration set to %s\n",
            ...
            case 'r':
                ...
                MUStrCpy(MSim.ResourceTraceFile, OptArg,
                         sizeof(MSim.ResourceTraceFile));

                DBG(3, fCONFIG)
                DPrint(
                    "INFO:     simulation mode set.  using resource tracefile "
                    "'%s'\n",
            ...
            case 's':
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     simulation statfile set to '%s'\n",
                       MSim.StatFileName);
            ...
            default:
                ...
                DBG(1, fCONFIG)
                DPrint("WARNING:  unexpected flag '%c'\n", Flag);
...

{
    ...
    fprintf(stderr,
            "This software includes modifications by the Temple HPC Team, "
            "Copyright 2016\n");
    fprintf(stderr,
            "  for the latest release, see "
            "https://github.com/TempleHPC/maui-scheduler\n");

    fprintf(stderr,
            "This software includes the Maui Server Module, Copyright 1996 "
            "MHPCC, All Rights Reserved\n");
...

{
    ...
    if ((MSched.Reload == TRUE) &&
        ...
        if (MSysLoadConfig(MSched.HomeDir, MSched.ConfigFile,
                           (1 << mcmForce)) == FAILURE) {
            if (MSysLoadConfig(".", MSched.ConfigFile, (1 << mcmForce)) ==
                FAILURE) {
                DBG(3, fALL)
                DPrint(
                    "WARNING:  cannot locate configuration file '%s' in '%s' "
    ...
    MLogRoll(NULL, 0, MSched.Iteration, MSched.LogFileRollDepth);
    ...
    if (FC->FSPolicy != fspNONE) {
        ...
        if (NewFSInterval != MSched.CurrentFSInterval) {
            if (MSched.CurrentFSInterval != 0)
                MFSUpdateData(FC, MSched.CurrentFSInterval,
                              (1 << mfsactRotate));

            MSched.CurrentFSInterval = NewFSInterval;

            DBG(1, fFS)
            DPrint("INFO:     FS rolled to interval %ld\n",
...

{
    ...
    if (MOSSetGID(GID) == -1) {
        fprintf(stderr,
                "ERROR:    cannot change GID to primary admin group '%s'  "
                "(GID: %d) %s\n",
                MUGIDToName(GID), GID, strerror(errno));
    }

    if (MOSSetUID(UID) == -1) {
        fprintf(stderr,
                "ERROR:    cannot change UID to primary admin user '%s'  (UID "
                ": %d) %s\n",
                MUUIDToName(UID), UID, strerror(errno));
...

{
    ...
    if (ValidAdmin == FALSE) {
        fprintf(stderr,
                "ERROR:    user %s (UID: %d) is not authorized to run this "
                "program\n",
                MUUIDToName(uid), uid);
    ...
    if (MSched.SecureMode == TRUE) {
        ...
        DBG(3, fCORE)
        DPrint("INFO:     executing scheduler from '%s' under UID %d GID %d\n",
               MSched.HomeDir, uid, gid);
    }
    ...
    if (MSched.Mode != msmTest) {
        if ((MSched.LockFile[0] != '~') && (MSched.LockFile[0] != '/')) {
            sprintf(FileName, "%s%s", MSched.HomeDir, MSched.LockFile);
        } else {
            strcpy(FileName, MSched.LockFile);
        }

        if ((fp = fopen(FileName, "w+")) != NULL) {
            fprintf(fp, "%d", MOSGetPID());

            fclose(fp);
        } else {
            DBG(0, fALL)
            DPrint("ERROR:    cannot update lockfile '%s', errno: %d (%s)\n",
