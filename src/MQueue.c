...

{
    ...
    if ((*Q = (mjob_t *)calloc(1, sizeof(mjob_t))) == NULL) {
        DBG(1, fSTRUCT)
        DPrint(
            "ALERT:    cannot allocate memory for queue %s, errno: %d (%s)\n",
            (QName != NULL) ? QName : "NULL", errno, strerror(errno));
...

{
    ...
    if (JobIndex != NULL) jindex = 0;

    if ((Q == NULL) || (Q[0] == NULL) || (Q[0]->Next == NULL) ||
        (Q[0]->Next == Q[0])) {
        ...
        for (jindex = 0; JobIndex[jindex] != -1; jindex++) {
            ...
            if ((J->State == mjsStarting) || (J->State == mjsRunning)) {
                ...
                MaxIdleStartPriority =
                    MAX(MaxIdleStartPriority, J->StartPriority);
            }
        }
    } else {
        jindex = 0;
        ...
        for (J = Q[0]->Next; J != Q[0]; J = J->Next) {
            ...
            if (JobIndex != NULL) JobIndex[jindex++] = J->Index;
        }
    } /* END for (J) */

    if (JobIndex != NULL) {
        if (jindex > 1) {
            qsort((void *)&JobIndex[0], jindex, sizeof(int),
        ...
        for (jindex = 0; JobIndex[jindex] != -1; jindex++) {
            ...
            if (((J->State == mjsStarting) || (J->State == mjsRunning)) &&
                ...
                DBG(2, fCORE)
                DPrint(
                    "INFO:     backfill job '%s' no longer preemptible (%ld > "
                    "%ld)\n",
                    J->Name, J->StartPriority, MaxIdleStartPriority);
            }
        }
...
int MQueueBackFill(
    ...
    int *BFQueue, int PLevel, mpar_t *P)
...
/* backfill partition */
    ...
    while (MBFGetWindow(&BFNodeCount, &BFProcCount, BFNodeList, &BFTime,
                        OBFTime, P, ALL, ALL, ALL, 0, 0, 1, &DRes, NULL, NULL,
                        NULL, NULL) == SUCCESS) {
        DBG(3, fSCHED)
        DPrint("INFO:     backfill window obtained [%d nodes/%d procs : %s]\n",
               BFNodeCount, BFProcCount, MULToTString(BFTime));
        ...
        if (MQueueSelectJobs(BFQueue, tmpQ, PLevel, BFNodeCount, BFProcCount,
                             AdjBFTime, P->Index, NULL, FALSE,
                             FALSE) == FAILURE) {
            DBG(5, fSCHED)
            DPrint(
                "INFO:     no jobs meet BF window criteria in partition %s\n",
                P->Name);
        ...
        for (jindex = 0; tmpQ[jindex] != -1; jindex++) {
            ...
            if (index != jindex) tmpQ[index] = tmpQ[jindex];

            JList[index] = J;

            index++;

            if ((MPar[0].BFDepth > 0) && (index >= MPar[0].BFDepth)) {
                break;
            }
        } /* END for (jindex) */
        ...
        switch (MPar[0].BFPolicy) {
            case bfPREEMPT:
                ...
                MBFPreempt(JList, PLevel, BFNodeList, BFTime, BFNodeCount,
                           BFProcCount, P);
            ...
            case bfFIRSTFIT:
                ...
                MBFFirstFit(JList, PLevel, BFNodeList, BFTime, BFNodeCount,
                            BFProcCount, P);
            ...
            case bfBESTFIT:
                ...
                MBFBestFit(JList, PLevel, BFNodeList, BFTime, BFNodeCount,
                           BFProcCount, P);
            ...
            case bfGREEDY:
                ...
                MBFGreedy(JList, PLevel, BFNodeList, BFTime, BFNodeCount,
                          BFProcCount, P);
            ...
            default:
                ...
                DBG(0, fSCHED)
                DPrint("ERROR:    unexpected backfill policy %d (using %s)\n",
                       MPar[0].BFPolicy, MBFPolicy[DEFAULT_BACKFILLPOLICY]);
...

{
    ...
    for (jindex = 0; Q[jindex] != -1; jindex++) {
        ...
        if ((J->EState == mjsStarting) || (J->State == mjsStarting) ||
            (J->EState == mjsRunning) || (J->State == mjsRunning) ||
            (J->State == mjsSuspended)) {
            /* continue if job started in MQueueScheduleRJobs() */

            DBG(7, fSCHED)
            DPrint(
                "INFO:     job '%s' not considered for prio scheduling "
                "(State/EState)\n",
                J->Name);
        ...
        if ((J->R != NULL) &&
            ...
            for (rindex = 0; rindex < MAX_MQOS; rindex++) {
                if (MPar[0].ResDepth[rindex] == 0) continue;

                for (qindex = 0; qindex < MAX_MQOS; qindex++) {
                    if (MPar[0].ResQOSList[rindex][qindex] == NULL) break;

                    if ((MPar[0].ResQOSList[rindex][qindex] == J->Cred.Q) ||
                        (MPar[0].ResQOSList[rindex][qindex] ==
                         (mqos_t *)MAX_MQOS)) {
                        break;
                    }
                } /* END for (qindex) */
        ...
        if (IdleJobFound == TRUE) {
            DBG(7, fSCHED)
            DPrint(
                "INFO:     job '%s' not considered for prio scheduling (Idle "
                "Job Reached)\n",
                J->Name);

            /* reserve job if eligible */

            if (MJobCheckLimits(J, ptSOFT, P, (1 << mlActive), NULL) ==
                FAILURE) {
                ...
                DBG(5, fSCHED)
                DPrint(
                    "INFO:     skipping job '%s'  (job now violates active "
                    "policy)\n",
                    J->Name);
            ...
            if ((MJobPReserve(J, P->Index, PResCount, NULL) != SUCCESS) ||
                (J->R == NULL) || (J->R->StartTime > MSched.Time)) {
        ...
        for (sindex = 1; J->Req[0]->TaskRequestList[sindex] > 0; sindex++) {
            ...
            if (MJobCheckLimits(J, ptSOFT, P, (1 << mlActive), NULL) ==
                ...
                DBG(5, fSCHED)
                DPrint(
                    "INFO:     skipping job '%s'  (job violates active "
                    "policy')\n",
                    J->Name);
            ...
            DBG(4, fSCHED)
            DPrint("INFO:     checking job %s(%d)  state: %s (ex: %s)\n",
                   J->Name, sindex, MJobState[J->State], MJobState[J->EState]);
            ...
            if (MJobSelectMNL(J, P, NULL, MNodeList, NodeMap,
                              AllowPreemption) == FAILURE) {
                DBG(6, fSCHED)
                DPrint(
                    "INFO:     cannot locate %d tasks for job '%s' in "
                    "partition %s\n",
                    J->Request.TC, J->Name, P->Name);
        ...
        if (BestSIndex > 1) MJobUpdateResourceCache(J, BestSIndex);

        if (MPar[0].NodeSetDelay > 0)
            memcpy(tmpMNodeList, MNodeList, sizeof(tmpMNodeList));

        if (MJobAllocMNL(J, MNodeList, NodeMap, NULL, NAPolicy, MSched.Time) ==
            FAILURE) {
            DBG(1, fSCHED)
            DPrint(
                "ERROR:    cannot allocate nodes to job '%s' in partition %s\n",
                J->Name, P->Name);
            ...
            if ((MJobPReserve(J, P->Index, PResCount, &ResCountRej) ==
                 SUCCESS) ||
                (ResCountRej == FALSE)) {
                continue;
            } else {
        ...
        if (MJobStart(J) == FAILURE) {
            DBG(1, fSCHED)
            DPrint("ERROR:    cannot start job '%s' in partition %s\n", J->Name,
                   P->Name);
    ...
    if (SchedCount != 0) {
        DBG(2, fSCHED)
        DPrint("INFO:     %d jobs started on iteration %d\n", SchedCount,
               MSched.Iteration);
    }
    ...
    DBG(2, fSCHED)
    DPrint("INFO:     resources available after scheduling: N: %d  P: %d\n",
           MPar[0].IdleNodes, MPar[0].ARes.Procs);
...
int MQueueDiagnose(
    ...
    mjob_t **FullQ, /* I */
    int *NBJobList, int PLevel, mpar_t *PS, char *Buffer, int BufSize)

{
    ...
    if ((FullQ == NULL) || (Buffer == NULL) || (PS == NULL)) {
        return (FAILURE);
    }
    ...
    for (J = FullQ[0]->Next; J != FullQ[0]; J = J->Next) {
        if (((int)strlen(Buffer) + 256) >= BufSize) break;
        ...
        DBG(4, fUI)
        DPrint("INFO:     checking job '%s' QOS: %s  (QFlags: %lu)\n", J->Name,
        ...
        for (jindex = 0; jindex < MAX_MJOB + MAX_MHBUF; jindex++) {
            if (NBJobList[jindex] == -1) break;

            if (J->Index == NBJobList[jindex]) break;
        } /* END for (jindex) */
        ...
        if (J->SMinTime > MSched.Time) {
            sprintf(
                ...
                J->Name, MULToTString(J->SMinTime - MSched.Time));
            strcat(Buffer, temp_str);
        ...
        if (MLocalCheckFairnessPolicy(J, MSched.Time, NULL) == FAILURE) {
            DBG(6, fSCHED)
            DPrint(
                "INFO:     job %s rejected, partition %s (violates local "
                "fairness policy)\n",
                J->Name, PS->Name);
        ...
        for (pindex = 1; pindex < MAX_MPAR; pindex++) {
            ...
            if (MUNumListGetCount(J->StartPriority, RQ->DRes.PSlot,
                ...
                sprintf(temp_str,
                        "job %-20s requires classes not configured in "
                        "partition %s (%s)\n",
                        J->Name, P->Name,
            ...
            if (MUBMCheck(P->Index, J->PAL) == FAILURE) {
                sprintf(temp_str,
                        "job %-20s not allowed to run in partition %s  "
                        "(partitions allowed: %s)\n",
            ...
            if (MJobCheckPolicies(J, PLevel, 0, P, &RIndex, Message,
                                  MSched.Time) == FAILURE) {
...

{
    ...
    for (jindex = 0; Q[jindex] != -1; jindex++) {
        ...
        if (J->RMinTime > MSched.Time) continue;

        if (MJobSelectMNL(J, &MPar[J->Req[0]->PtIndex], J->NodeList, MNodeList,
                          NodeMap, AllowPreemption) == FAILURE) {
...

{
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = JNext) {
        ...
        DBG(7, fSTRUCT)
        DPrint("INFO:     checking purge criteria for job '%s'\n", J->Name);

        if ((J->RM != NULL) &&
            (J->RM->WorkloadUpdateIteration != MSched.Iteration)) {
            continue;
        }
        ...
        if (Delta > MAX(MAX(15, MSched.RMPollInterval), MSched.JobPurgeTime)) {
            DBG(2, fSTRUCT)
            DPrint("WARNING:  job '%s' no longer detected (%ld > %ld)\n",
                   J->Name, Delta,
            ...
            if (J->Cred.A != NULL) {
                if (MAMAllocResCancel(J->Cred.A->Name, J->Name, "job purged",
                                      NULL, &Reason) == FAILURE) {
                    DBG(1, fSCHED)
                    DPrint(
        ...
        if ((J->State == mjsIdle) || (J->State == mjsDeferred) ||
            (J->State == mjsHold)) {
            if (J->Flags & (1 << mjfNoQueue)) {
                DBG(2, fSTRUCT)
                DPrint("INFO:     cancelling Non-Queue job '%s'\n", J->Name);

                if (J->Cred.A != NULL) {
                    if (MAMAllocResCancel(J->Cred.A->Name, J->Name,
                                          "interactive job purged", NULL,
                                          &Reason) == FAILURE) {
                        DBG(1, fSCHED)
                        DPrint(
                            "ERROR:    cannot cancel allocation reservation "
                            "for job '%s'\n",
                            J->Name);
                    }
                }
                ...
                if (J->EState == mjsDeferred) {
                    sprintf(DeferMessage,
                            "SCHED_INFO:  job cannot run.  Reason: %s\n",
                    ...
                    if (MQueueSelectJobs(SrcQ, tmpQ, ptHARD, MAX_MNODE_PER_JOB,
                                         MAX_MTASK, MAX_MTIME, -1, ReasonList,
                                         FALSE, FALSE) == FAILURE) {
                        strcpy(DeferMessage,
                               "SCHED_INFO:  job cannot run.  Reason: cannot "
                               "select job\n");
                    } else if (tmpQ[0] == -1) {
                        for (index = 0; index < MAX_MREJREASON; index++) {
                            if (ReasonList[index] != 0) break;
                        }
                        ...
                        if (index != MAX_MREJREASON) {
                            ...
                            strcpy(DeferMessage,
                                   "SCHED_INFO:  job cannot run.  Reason: "
                                   "policy violation\n");
                        }
                    } else {
                        strcpy(
                            DeferMessage,
                            "SCHED_INFO:  insufficient resources to run job\n");
                    }
...

{
    ...
    qsort((void *)&MAQ[0], (jindex + 1), sizeof(int),
    ...
    if ((J->EState == mjsIdle) || (J->CTime == MSched.Time)) {
        ...
        DBG(6, fSCHED)
        DPrint(
            "INFO:     previously detected active Job[%03d] '%s' added to "
            "MAQ\n",
            J->Index, J->Name);
    }
    ...
    DBG(5, fSCHED) {
        DBG(5, fSCHED)
        DPrint("INFO:     job '%s' added to MAQ at slot %d\n", J->Name, jindex);

        DBG(5, fSCHED) DPrint("INFO:     MAQ: ");

        for (jindex = 0; MAQ[jindex] != -1; jindex++) {
            fprintf(mlog.logfp, "[%d : %s : %ld]", MAQ[jindex],
                    MJob[MAQ[jindex]]->Name,
                    MJob[MAQ[jindex]]->StartTime + MJob[MAQ[jindex]]->WCLimit -
                        MSched.Time);
        } /* END for (jindex) */
...

{
    ...
    GResPolicy = (MPar[0].ResPolicy == resDefault) ? DEFAULT_RESERVATIONMODE
                                                   : MPar[0].ResPolicy;
    ...
    for (jindex = 0; Q[jindex] != -1; jindex++) {
        ...
        if ((J == NULL) || (J->State != mjsIdle)) continue;

        MTRAPJOB(J, FName);

        DBG(5, fSCHED)
        DPrint("INFO:     checking job %s in %s()\n", J->Name, FName);
        ...
        if (J->Cred.Q->Flags & (1 << mqfreserved)) {
            ...
            if (MJobReserve(J, mjrQOSReserved) == FAILURE) {
                DBG(1, fSCHED)
                DPrint("ALERT:    cannot create %s reservation for job %s\n",
                       MJRType[mjrQOSReserved], J->Name);
            ...
            if (rindex == MAX_MQOS) {
                ...
                DBG(2, fSCHED)
                DPrint(
                    "ALERT:    cannot locate QOS group for reserved job %s "
                    "(QOS: %s) in %s()\n",
                    J->Name, J->Cred.Q->Name, FName);
            ...
            if (J->R != NULL) {
                switch (ResPolicy) {
                    case resCurrentHighest:
                        ...
                        DBG(6, fSCHED)
                        DPrint(
                            "INFO:     releasing reservation '%s' for res "
                            "policy CurrentHighest\n",
                            J->R->Name);

                        MResDestroy(&J->R);

                        /* recreate top 'ResDepth' reservations */
                        /* appropriate reservations will be re created in
                         * MQueueScheduleIJobs() */
                    ...
                    case resHighest:
                        ...
                        /* all 'Highest' reservations should be slid forward
                         * regardless */
                        /* of ResDepth */
        ...
        if ((MSched.Time >= J->R->StartTime) && (J->State == mjsIdle)) {
            DBG(2, fSCHED)
            DPrint("INFO:     located job '%s' reserved to start %30s", J->Name,
            ...
            if (MJobCheckLimits(J, ptSOFT, &MPar[PIndex], (1 << mlActive),
                                NULL) == FAILURE) {
                if (strcmp(J->ResName, J->R->Name) != 0) MResDestroy(&J->R);

                if (MSched.Mode != msmTest) {
                    if (MSched.Mode == msmNormal) {
                        MOSSyslog(LOG_NOTICE,
                                  "cannot run reserved job '%s'.  job violates "
                                  "active policy",
                                  J->Name);
                    }
                    ...
                    DBG(1, fSCHED)
                    DPrint(
                        "ALERT:    cannot run reserved job '%s'.  (job "
            ...
            if (MJobSelectMNL(J, &MPar[PIndex], NULL, MNodeList, NodeMap,
                              TRUE) == FAILURE) {
                /* not enough nodes to run job */

                DBG(2, fSCHED)
                DPrint(
                    "ALERT:    insufficient nodes to run reserved job '%s' on "
                    "iteration %d\n",
                ...
                for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                    ...
                    if (MRM[RQ->RMIndex].Type == mrmtLL) {
                        if (RQ->Network ==
                            MUMAGetIndex(eNetwork, "hps_user", mVerify)) {
                ...
                for (index = 0; index < MAX_MNODE; index++) {
                    ...
                    if ((N->EState == mnsIdle) || (N->EState == mnsActive)) {
                        if (((N->RM->Type == mrmtLL) ||
                             (N->RM->Type == mrmtPBS)) &&
                            (RMNodeStateCheck == TRUE)) {
                            ...
                            if (N->State == mnsBusy) {
                                DBG(6, fSCHED)
                                DPrint(
                                    "INFO:     node '%s' is not available for "
                                    "scheduling.  (state: %s  exp: %s)\n",
                    ...
                    if ((N->RM->Type == mrmtLL) && (HPSAdapterCheck == TRUE)) {
                        if (!(N->Network &
                              MUMAGetBM(eNetwork, "hps_user", mVerify))) {
                            DBG(6, fSCHED)
                            DPrint(
                                "INFO:     LL node '%s' is not available for "
                                "scheduling.  (missing 'hps_user' adapter)\n",
                                N->Name);
                ...
                if (((MSched.Time - J->R->StartTime) < MPar[0].ResRetryTime) &&
                    ((HPSAdapterDelayCount > 0) ||
                     (RMNodeStateDelayCount > 0))) {
                    ...
                    if (MSched.Mode == msmNormal) {
                        MOSSyslog(
                            LOG_NOTICE,
                            "extending reservation for job %s, A: %d PS: %d",
                            J->Name, HPSAdapterDelayCount,
                            RMNodeStateDelayCount);
                    }
                    ...
                    DBG(4, fSCHED)
                    DPrint(
                        "INFO:     reservation for job '%s' blocked by RM race "
                        "condition.  (retrying reservation)\n",
                        J->Name);

                    if (MJobReserve(J, J->RType) == SUCCESS) {
                        DBG(4, fSCHED)
                        DPrint(
                            "INFO:     reservation for job '%s' recreated for "
                            "time %s\n",
                            J->Name,
                        ...
                        DBG(4, fSCHED)
                        DPrint(
                            "ALERT:    cannot create slide-back reservation "
                            "for job '%s'\n",
                            J->Name);
                    }
                } /* END if (MSched.Time...) */
                    ...
                    DBG(2, fSCHED)
                    DPrint(
                        "ALERT:    insufficient nodes to run reserved job '%s' "
                    ...
                    if (MSched.Mode != msmTest) {
                        if (MSched.Mode == msmNormal) {
                            MOSSyslog(LOG_NOTICE,
                                      "reserved job %s cannot run.  deferring",
                                      J->Name);
                        }

                        MJobSetHold(
                            J, (1 << mhDefer), MSched.DeferTime, mhrNoResources,
            ...
            if (MJobAllocMNL(J, MNodeList, NodeMap, NULL, NAPolicy,
                             MSched.Time) == SUCCESS) {
                if (MJobStart(J) == SUCCESS) {
                    SchedCount++;

                    DBG(2, fSCHED)
                    DPrint("INFO:     reserved job '%s' started\n", J->Name);
                } else {
                    DBG(1, fSCHED)
                    DPrint("ALERT:    cannot run reserved job '%s'\n", J->Name);
                }
            } else {
                DBG(1, fSCHED)
                DPrint("ERROR:    cannot allocate nodes for job '%s'\n",
                       J->Name);
            }
    ...
    if (SchedCount != 0) {
        DBG(2, fSCHED)
        DPrint("INFO:     reserved jobs scheduled: %d\n", SchedCount);
    }
