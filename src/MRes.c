...
int MResCreate(
    ...
    int Type,                                               /* I */
    macl_t *ACL,                                            /* I */
    char *CAName, unsigned long Flags, mnalloc_t *NodeList, /* I */
    long StartTime, long EndTime, int NodeCount,            /* I */
    int ProcCount,                                          /* I */
    char *RBaseName, mres_t **ResP,                         /* O */
    char *RegEx,                                            /* I */
    mcres_t *DRes)                                          /* I */

{
    ...
    DBG(3, fSTRUCT)
    DPrint("%s(%s,ACL,%s,%ld,NodeList,%ld,%ld,%d,%d,%s,ResP,'%s',DRes)\n",
           FName, MResType[Type], (CAName != NULL) ? CAName : "NULL", Flags,
           StartTime, EndTime, NodeCount, ProcCount, RBaseName,
    ...
    if (StartTime >= EndTime) {
        ...
        DBG(0, fSTRUCT)
        DPrint(
            "ERROR:    invalid timeframe for reservation ID '%s' (starttime "
            "%ld >= endtime %ld)\n",
            ((ACL != NULL) && (ACL[0].Name[0] != '\0')) ? ACL[0].Name
                                                        : "SYSTEM",
            StartTime, EndTime);
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if ((R == NULL) || (R->Name[0] == '\0') || (R->Name[0] == '\1')) break;
    } /* END for (rindex) */

    if (rindex == MAX_MRES) {
        DBG(1, fSTRUCT)
        DPrint(
            "ERROR:    reservation overflow on reservation type %s for ID "
            "'%s'\n",
            MResType[Type],
            ((ACL != NULL) && (ACL[0].Name[0] != '\0')) ? ACL[0].Name
                                                        : "SYSTEM");
    ...
    if ((CAName != NULL) && (CAName[0] != '\0') && strcmp(CAName, NONE)) {
        MAcctAdd(CAName, &R->A);
        ...
        if (MSched.Iteration >= 0) {
            ...
            if (MAMAllocRReserve(&MAM[0], R->Name, StartTime, CAName, ProcCount,
                                 NodeCount, WallTime, 0, MDEF_NODETYPE,
                                 &Reason) == FAILURE) {
                DBG(1, fSCHED)
                DPrint(
                    "ALERT:    cannot reserve allocation for %d procs for "
                    "reservation %s\n",
                    ProcCount, R->Name);
    ...
    if ((Type != mrtJob) && !(R->Flags & (1 << mrfStandingRes))) {
        sprintf(Message, "RESERVATIONCREATED:  %s %s %s %ld %ld %ld %d\n",
                R->Name, MResType[Type], RBaseName, MSched.Time, StartTime,
                EndTime, R->NodeCount);
    ...
    if (R->Flags & (1 << mrfMeta)) MResAdjustGResUsage(R, 1);
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        if ((NodeList == NULL) || (NodeList[nindex].N == NULL)) break;
        ...
        R->TaskCount++;
        R->AllocPC +=
            (R->DRes.Procs == -1) ? N->CRes.Procs : (TC * R->DRes.Procs);

        if (MResAddNode(R, N, TC, 0) == FAILURE) {
            return (FAILURE);
        }

        if (N->PtIndex != R->PtIndex) R->PtIndex = 0;
    } /* END for (nindex) */
...

{
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if (TaskCount > 0)
            MReqSetAttr(J, J->Req[0], mrqaTCReqMin, (void **)&TaskCount, mdfInt,
                        mSet);

        if (MJobGetRange(J, J->Req[0], &MPar[0], MSched.Time, GRange, NULL,
                         NULL, NodeMap, (1 << rtcStartEarliest),
                         NULL) == FAILURE) {
        ...
        if (StartTime > 0) {
            if (GRange[0].StartTime != StartTime) {
                DBG(2, fSCHED)
                DPrint(
                    "INFO:     cannot obtain desired starttime (%ld != %ld)\n",
                    GRange[0].StartTime, StartTime);
        ...
        if (MJobGetRange(J, J->Req[0], &MPar[0], BestStartTime, GRange,
                         MNodeList, NULL, NodeMap, 0, NULL) == FAILURE) {
        ...
        MREInsert(MRE, R->StartTime, R->EndTime, R->Index, &R->DRes,
                  (MAX_MRES << 2));

        if (MResAllocate(R, (mnalloc_t *)MNodeList[0]) == FAILURE) {
            DBG(1, fCORE)
            DPrint(
                "ALERT:    cannot adjust reservation %s (reservation empty)\n",
                R->Name);
...

{
    ...
    switch (AIndex) {
            ...
            if (Format == mdfString) {
                if (strcmp((char *)AVal, NONE)) {
                    if (MAcctAdd((char *)AVal, &R->A) == FAILURE) {
                        DBG(1, fCKPT)
                        DPrint(
                            "ALERT:    cannot add account %s for reservation "
                            "%s\n",
                            (char *)AVal, R->Name);
            ...
            if (Format == mdfString) {
                if (strcmp((char *)AVal, NONE)) {
                    if (MGroupAdd((char *)AVal, &R->G) == FAILURE) {
                        DBG(1, fCKPT)
                        DPrint(
                            "ALERT:    cannot add group %s for reservation "
                            "%s\n",
                            (char *)AVal, R->Name);
            ...
            if (Format == mdfString) {
                if (strcmp((char *)AVal, NONE)) {
                    if (MUserAdd((char *)AVal, &R->U) == FAILURE) {
                        DBG(1, fCKPT)
                        DPrint(
                            "ALERT:    cannot add user %s for reservation %s\n",
                            (char *)AVal, R->Name);
        ...
        case mraHostExp:
            ...
            if ((R->RegEx == NULL) &&
                ((R->RegEx = (char *)calloc(1, strlen((char *)AVal) + 1)) ==
                 NULL)) {
                break;
            }
            ...
            for (nindex = 0; nindex < MAX_MNODE; nindex++) {
                ...
                switch (Mode) {
                    case -1:
                        ...
                        if (NPtr == N) MResAddNode(R, N, NCount, Mode);
...

        {
            ...
            if (tmpDuration > 0) R->EndTime = tmpStartTime + tmpDuration;
            ...
            for (nindex = 0; nindex < MAX_MNODE; nindex++) {
                ...
                for (sindex = 0; sindex < MSched.ResDepth << 1; sindex++) {
                    ...
                    if (RE == (mre_t *)1) continue;

                    if (RE == NULL) break;

                    if (N->R[RE->Index] != R) continue;

                    /* adjust times of RE */

                    /* NYI */

                    /* if node contains multiple container reservations, node RE
...

{
    ...
    switch (AIndex) {
        case mraAAccount: /* accountable account */
            ...
            if (R->A != NULL) strcpy(Buf, R->A->Name);
        ...
        case mraAGroup: /* accountable group */
            ...
            if (R->G != NULL) strcpy(Buf, R->G->Name);

            break;

        case mraAUser: /* accountable user */

            if (R->U != NULL) strcpy(Buf, R->U->Name);
        ...
        case mraDuration:
            ...
            sprintf(Buf, "%ld", R->EndTime - R->StartTime);

            break;

        case mraEndTime:

            sprintf(Buf, "%ld", R->EndTime);

            break;

        case mraFlags:

            sprintf(Buf, "%ld", R->Flags);
        ...
        {
            if ((R->Type != mrtJob) || (R->J == NULL)) break;
        ...
        case mraMaxTasks:
            ...
            sprintf(Buf, "%d", R->MaxTasks);

            break;

        case mraName:

            if (R->Name[0] != '\0') strcpy(Buf, R->Name);

            break;

        case mraNodeCount:

            sprintf(Buf, "%d", R->NodeCount);
        ...
        case mraStartTime:
            ...
            sprintf(Buf, "%ld", R->StartTime);

            break;

        case mraStatCAPS:

            sprintf(Buf, "%.2lf", R->CAPS);

            break;

        case mraStatCIPS:

            sprintf(Buf, "%.2lf", R->CIPS);

            break;

        case mraStatTAPS:

            sprintf(Buf, "%.2lf", R->TAPS);

            break;

        case mraStatTIPS:

            sprintf(Buf, "%.2lf", R->TIPS);
...
{
    const int DAList[] = {
        mraName,     mraType,     mraStartTime, mraEndTime,  mraNodeCount,
        mraACL,      mraFlags,    mraAAccount,  mraAGroup,   mraAUser,
        mraStatCAPS, mraStatCIPS, mraStatTAPS,  mraStatTIPS, mraResources,
        mraHostExp,  mraMaxTasks, mraXAttr,     -1};
    ...
    for (aindex = 0; AList[aindex] != -1; aindex++) {
        if ((MResAToString(R, AList[aindex], tmpString, sizeof(tmpString), 0) ==
             FAILURE) ||
            (tmpString[0] == '\0')) {
            continue;
        }
...

{
    ...
    sscanf(Buf, "%s %s %ld", tmpName, ResID, &CkTime);
    ...
    if (R == &tmpR) {
        if (MResCreate(R->Type, R->ACL, (R->A != NULL) ? R->A->Name : "",
                       R->Flags,
                       /* R->NodeList, */ NULL, R->StartTime, R->EndTime, 0, 0,
                       R->Name, &ResP, R->RegEx, &R->DRes) == FAILURE) {
        ...
        if (R->A != NULL) MResSetAttr(ResP, mraAAccount, (void *)R->A, 0, 0);

        if (R->G != NULL) MResSetAttr(ResP, mraAGroup, (void *)R->G, 0, 0);

        if (R->U != NULL) MResSetAttr(ResP, mraAUser, (void *)R->U, 0, 0);
...

{
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if ((R == NULL) || (R->Name[0] == '\0')) {
            DBG(8, fUI)
            DPrint("INFO:     cannot locate reservation '%s'\n", ResID);
        ...
        if (!strcmp(ResID, R->Name)) break;
    } /* END for (rindex) */

    if (rindex == MAX_MRES) {
        DBG(8, fUI)
        DPrint("INFO:     cannot locate reservation '%s' (end of list)\n",
               ResID);
...
int MResGetNRange(
    ...
    mcres_t *DRes, char *BRName) /* O: blocking reservation name     */

{
    ...
    DBG(7, fSCHED)
    DPrint("%s(%s,%s,%ld,RRange,ARange,BRName)\n", FName,
           (RPtr != NULL) ? RPtr->Name : "NULL", (N != NULL) ? N->Name : "NULL",
           Duration);
    ...
    for (eindex = 0; eindex < (MSched.ResDepth << 1); eindex++) {
        ...
        DBG(6, fSCHED)
        DPrint(
            "INFO:     ignoring early RE[%d] event for '%s' (E: %ld <= S: %ld) "
            ": P: %ld\n",
    ...
    if (N->RE[eindex].Type == mreNONE) {
        ...
        if (ARange[0].TaskCount < RRange[0].TaskCount) {
            ...
            tmpTC = MNodeGetTC(N, &N->ARes, &N->CRes, &N->DRes, &RPtr->DRes,
                               MAX_MTIME);
        ...
        DBG(5, fSCHED)
        DPrint(
            "INFO:     node %s supports %d task%s of res %s for %s of %s (no "
            "RE)\n",
            N->Name, ARange[0].TaskCount, (ARange[0].TaskCount == 1) ? "" : "s",
            R->Name, MULToTString(ARange[0].EndTime - ARange[0].StartTime),
            WCString);
        ...
        if (RPtr->Flags & (1 << mrfAdvRes)) {
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     no reservation found for AdvRes res %s\n",
                   RPtr->Name);
    ...
    DBG(8, fSCHED) {
        ...
        DPrint("INFO:     requested resources: %s (x%d)\n",
               MUCResToString(&RPtr->DRes, 0, 0, NULL), RRange[0].TaskCount);
    }
    ...
    for (; eindex < (MSched.ResDepth << 1); eindex++) {
        ...
        if ((N->RE[eindex].Time > RRange[0].StartTime) && (PreRes == TRUE)) {
            ...
            if ((MUCResIsNeg(&CurRes) == SUCCESS) ||
                ...
                TC = MNodeGetTC(N, &CurRes, &N->CRes, &DedRes, &RPtr->DRes,
                                MSched.Time);
                ...
                if (DRes != NULL) MUCResGetMin(DRes, DRes, &CurRes);
            }
        ...
        if (ActiveRange == TRUE) {
            /* be generous (ie, Duration - (R->StartTime -
             * ARange[rindex].StartTime) */

            Overlap = MIN(R->EndTime, ARange[rindex].StartTime + Duration) -
                      MAX(R->StartTime, ARange[rindex].StartTime);

            if (Overlap < 0) Overlap = MIN(R->EndTime - R->StartTime, Duration);
        } else {
            Overlap = MIN(R->EndTime - MAX(R->StartTime, RRange[0].StartTime),
                          Duration);
        }
        ...
        if (N->RE[eindex].Type == mreStart) {
            ...
            if (MResCheckRAccess(R, RPtr, Overlap, &Same,
                                 &ARange[0].Affinity) == TRUE) {
                DBG(8, fSCHED) DPrint("INFO:     reservations are inclusive\n");

                InclusiveReservationCount++;

                if (RPtr->Flags & (1 << mrfAdvRes)) ResourcesAdded = TRUE;
            } else {
        ...
        if ((N->RE[eindex + 1].Type == mreNONE) ||
            ...
            if ((ResourcesRemoved == TRUE) && (ActiveRange == TRUE)) {
                ...
                if ((MUCResIsNeg(&CurRes) == FAILURE) &&
                    ...
                    ARange[rindex].StartTime =
                        MAX(RRange[0].StartTime, N->RE[eindex].Time) +
                        ResBuffer;
    ...
    for (index = 0; ARange[index].EndTime != 0; index++) {
        ...
        if (ARange[index].EndTime < RRange[0].StartTime) {
            ...
            DPrint(
                ...
                "P: %ld): removing range\n",
                index, RPtr->Name, ARange[index].EndTime, RRange[0].StartTime,
                MSched.Time);
            ...
            DPrint(
                ...
                "truncating range\n",
                index, RPtr->Name, ARange[index].StartTime,
                RRange[0].StartTime);
        ...
        if (ARange[index].TaskCount < RRange[0].TaskCount) {
            ...
            DPrint(
                ...
                "removing range\n",
                index, RPtr->Name, ARange[index].TaskCount,
                RRange[0].TaskCount);
        } else {
    ...
    for (index = 0; ARange[index].EndTime != 0; index++) {
        ...
        if (ARange[index].StartTime > RRange[0].EndTime) {
            ...
            DPrint(
                ...
                ARange[index].TaskCount, RPtr->Name,
            ...
            DPrint(
                ...
                "%ld): truncating range\n",
                index, RPtr->Name, ARange[index].EndTime, RRange[0].EndTime,
                MSched.Time);
        ...
        if (RangeTime < MAX(1, Duration)) {
            ...
            DPrint(
                ...
                " removing range\n",
                index, RPtr->Name, RangeTime, Duration);
    ...
    if ((ARange[0].EndTime == 0) || (ARange[0].TaskCount == 0)) {
        ...
        DPrint("INFO:     node %s unavailable for job %s at %s\n", N->Name,
               RPtr->Name, MULToTString(RRange[0].StartTime - MSched.Time));
    ...
    DBG(6, fSCHED) {
        for (rindex = 0; ARange[rindex].EndTime != 0; rindex++) {
            ...
            DPrint(
                "INFO:     node %s supports %d task%c of job %s for %s at %s\n",
                N->Name, ARange[rindex].TaskCount,
                (ARange[rindex].TaskCount == 1) ? ' ' : 's', RPtr->Name,
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
            if (N->RE[reindex].Type == mreNONE) break;
...

{
    ...
    DBG(5, fSTRUCT)
    DPrint("%s(%s,%s,%d,%d)\n", FName, (R != NULL) ? R->Name : "NULL",
           (N != NULL) ? N->Name : "NULL", TaskCount, Mode);
    ...
    if (FreeSlot != -1) {
        ...
        DBG(2, fSTRUCT)
        DPrint(
            "WARNING:  reservation overflow on node %s (%d >= %d) - increase "
            "%s\n",
            N->Name, sindex, MSched.ResDepth, MParam[pResDepth]);

        return (FAILURE);
    }

    DBG(6, fSCHED) { MRECheck(N, "MResAddNode-Start", TRUE); }
    ...
    switch (Mode) {
        ...
        case 0:
            ...
            if (N->R[sindex] == R) {
                ...
                MREInsert(N->RE, R->StartTime, R->EndTime, sindex, &DRes,
                          MSched.ResDepth << 1);
            }
    ...
    DBG(6, fSCHED) { MRECheck(N, "MResAddNode-End", TRUE); }
...

{
    ...
    for (nreindex = 0; N->RE[nreindex].Type != mreNONE; nreindex++) {
        if (nreindex >= (MSched.ResDepth << 1)) break;
            ...
            for (crindex = 0; crindex < MSched.ResDepth; crindex++) {
                if (N->RE[nreindex].Index == CRI[crindex]) {
                    ...
                    if (JRETime != N->RE[nreindex].Time) {
                        ...
                        for (treindex = nreindex + 1;
                             N->RE[treindex].Type != mreNONE; treindex++) {
                    ...
                    if (JRETime != N->RE[nreindex].Time) {
                        ...
                        if (N->RE[nreindex].Type == mreStart) {
                            ...
                            if (OPC > NPC) {
                                /* if previous event tuple is larger, locate
                                 * outstanding end event */

                                for (treindex = nreindex + 1;
                                     N->RE[treindex].Type != mreNONE;
                                     treindex++) {
                                    if ((N->RE[treindex].Index !=
                                         N->RE[nreindex].Index) ||
                                ...
                                memcpy(&N->RE[nreindex].DRes,
                                       &mCRE[crindex].DRes,
                                       sizeof(N->RE[0].DRes));
                                memcpy(&N->RE[treindex].DRes,
                                       &mCRE[crindex].DRes,
                                       sizeof(N->RE[0].DRes));
                                ...
                                if ((N->RE[CRESI[crindex]].DRes.Procs == -1) ||
                                    ...
                                    fprintf(stderr,
                                            "OUCH:  negative blocked resources "
                                            "detected\n");
                                }
                            } else if (OPC < NPC) {
                                memcpy(&mCRE[crindex].DRes,
                                       &N->RE[nreindex].DRes,
                                       sizeof(mCRE[crindex].DRes));

                                memcpy(&N->RE[CRESI[crindex]].DRes,
                                       &mCRE[crindex].DRes,
                                       sizeof(N->RE[0].DRes));
                                memcpy(&N->RE[CREEI[crindex]].DRes,
                                       &mCRE[crindex].DRes,
                                       sizeof(N->RE[0].DRes));
                ...
                if (CRI[crindex] == -1) {
                    ...
                    memcpy(&mCRE[crindex], &N->RE[nreindex],
                           sizeof(mCRE[crindex]));
    ...
    InitialJStartTime =
        (jreindex > 0) ? N->R[JRE[0].Index]->StartTime : MAX_MTIME;

    if (((RPtr != NULL) && (ResFound == FALSE)) || (jreindex == 0)) {
    ...
    DBG(6, fSCHED) { MRECheck(N, "MNodeBuildRE-Start", TRUE); }

    memcpy(ORE, JRE, sizeof(ORE[0]) * (jreindex + 1));

    DBG(6, fSTRUCT)
    DPrint("INFO:     updating cr reservations for res '%s'\n",
           (RPtr != NULL) ? RPtr->Name : "NULL");
    ...
    for (crindex = 0; CRI[crindex] != -1; crindex++) {
        ...
        for (jrindex1 = 0; JRE[jrindex1].Type != mreNONE; jrindex1++) {
            ...
            Overlap =
                MIN(CREndTime, R->EndTime) - MAX(CRStartTime, R->StartTime);
            ...
            for (jrindex2 = 0; JRE[jrindex2].Type != mreNONE; jrindex2++) {
                if (JRE[jrindex2].Type != mreStart) continue;

                JR = N->R[JRE[jrindex2].Index];

                if (JRE[jrindex2].Time >= CREndTime) break;

                Overlap = MIN(CREndTime, JR->EndTime) -
                ...
                switch (R->Type) {
                    case mrtUser:
                        ...
                        CROverlap = MIN(R->EndTime, JR->EndTime) -
                                    MAX(R->StartTime, JR->StartTime);

                        if (MResCheckJAccess(R, J, CROverlap, NULL, NULL) ==
                            TRUE) {
                            if (JRE[jrindex2].Type == mreStart) {
                                /* remove dedicated job resources from container
                                 * blocked resources */

                                MCResRemove(&BR, &N->CRes, &JR->DRes, JRC,
                                            FALSE);
            ...
            if (CRStartTime == R->EndTime) break;
        ...
        while ((CRE[creindex].Type != mreNONE) ||
            ...
            if ((creindex >= (MSched.ResDepth << 1)) ||
                ...
                DBG(1, fSTRUCT)
                DPrint(
                    "ALERT:    node reservation event overflow (N: %d  C: %d  "
                    "O: %d) - increase %s\n",
                    nreindex, creindex, oreindex, MParam[pResDepth]);
        ...
        } /* END while ((CRE[creindex].Type != mreNONE) || (ORE[oreindex].Type
             != mreNONE)) */
    ...
    for (nreindex = 0; N->RE[nreindex].Type != mreNONE; nreindex++) {
        ...
        if (CRStartTime > N->RE[nreindex].Time) {
            DBG(2, fSTRUCT)
            DPrint(
                "ALERT:    node %s RE table is corrupt.  RE[%d] '%s' at %s is "
                "out of time order\n",
                N->Name, nreindex, N->R[N->RE[nreindex].Index]->Name,
                MULToTString(N->RE[nreindex].Time - MSched.Time));
        }
    ...
    DBG(6, fSCHED) { MRECheck(N, "MNodeBuildRE-End", TRUE); }
...

{
    ...
    DBG(8, fSTRUCT)
    DPrint("%s(%s,%s,%ld,Same,Affinity)\n", FName,
           (R != NULL) ? R->Name : "NULL", (ReqR != NULL) ? ReqR->Name : "NULL",
           Overlap);
...

{
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if (MSched.Time < R->StartTime) continue;

        if (MSched.Time > R->EndTime) continue;
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
            ...
            if (N->RE[reindex].Time > MSched.Time) break;

            if (N->RE[reindex].Type != mreStart) continue;
            ...
            DBG(9, fSTRUCT)
            DPrint(
                "INFO:     checking utilization for MNode[%03d] '%s', slot "
                "%d\n",
                nindex, N->Name, N->RE[reindex].Index);

            DBG(5, fSTRUCT)
            DPrint(
                "INFO:     updating usage stats for reservation %s on node "
                "'%s'\n",
                R->Name, N->Name);
            ...
            if (R->A != NULL) R->A->F.FSUsage[0] += fsusage;

            if (R->G != NULL) R->G->F.FSUsage[0] += fsusage;

            if (R->U != NULL) R->U->F.FSUsage[0] += fsusage;
        } /* END for reindex */
    }     /* END for nindex  */
...

{
    ...
    DBG(3, fSTAT)
    DPrint("%s(%s,%d)\n", FName, (RS != NULL) ? RS->Name : "NULL", Mode);
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if (R->AllocResPending == TRUE) {
            ...
            if (MAMAllocRReserve(&MAM[0], R->Name, MSched.Time, R->A->Name,
                                 R->AllocPC, R->NodeCount, WallTime, 0,
                                 MDEF_NODETYPE, &Reason) == FAILURE) {
                DBG(1, fSCHED)
                DPrint(
                    "ALERT:    cannot reserve allocation for %d procs for "
                    "reservation %s\n",
                ...
                if (Reason == mhrNoFunds) MResDestroy(&R);
        ...
        if (R->CIPS <= 0.0) {
            if (Mode == 2) {
                if (MAMAllocResCancel((R->A != NULL) ? R->A->Name : NONE,
                                      R->Name, "reservation released", NULL,
                                      &Reason) == FAILURE) {
                    DBG(1, fSTAT)
                    DPrint(
                        "ALERT:    cannot cancel allocation reservation for "
                        "reservation %s\n",
                        R->Name);
                }
            } else if (MAMAllocRReserve(
                           &MAM[0], R->Name, MSched.Time,
                           (R->A != NULL) ? R->A->Name : NONE, R->AllocPC,
                           R->NodeCount,
                           MAX(1, MIN(R->EndTime, MAM[0].FlushTime) -
                                      MAX(MSched.Time, R->StartTime)),
                           0, MDEF_NODETYPE, &Reason) == FAILURE) {
                DBG(3, fUI)
                DPrint(
                    "WARNING:  cannot reserve allocation for %s on %d proc%s\n",
                    R->Name, R->AllocPC, (R->AllocPC == 1) ? "" : "s");
        ...
        if (MAMAllocRDebit(&MAM[0], R, &Reason, NULL) == SUCCESS) {
            ...
            if (Mode == 0) {
                if (MAMAllocRReserve(&MAM[0], R->Name, MSched.Time,
                                     (R->A != NULL) ? R->A->Name : NONE,
                                     R->AllocPC, R->NodeCount,
                                     MAX(1, MIN(R->EndTime, MAM[0].FlushTime) -
                                                MAX(MSched.Time, R->StartTime)),
                                     0, MDEF_NODETYPE, &Reason) == FAILURE) {
                    DBG(3, fUI)
                    DPrint(
                        "WARNING:  cannot reserve allocation for %s on %d "
                        "proc%s\n",
                        R->Name, R->AllocPC, (R->AllocPC == 1) ? "" : "s");
            ...
            DBG(1, fSTAT)
            DPrint(
                "ALERT:    cannot charge %6.2f PS to account %s for "
                "reservation %s\n",
                R->CIPS, (R->A != NULL) ? R->A->Name : NONE, R->Name);
        }
    } /* END for (rindex) */
...
int MRLAND(
    ...
    mrange_t *RDst, mrange_t *R1, mrange_t *R2)

{
            ...
            if (ETime <= R1[index1].EndTime) {
                ...
                if (R2[index2].EndTime != 0) {
                    ...
                    if (R2[index2].EndTime < R1[index1].StartTime) {
                        ...
                        while (
                            (R2[index2].EndTime == R2[index2 + 1].StartTime) &&
                            (R2[index2 + 1].EndTime != 0)) {
                            index2++;
                        }
    ...
    if (RDst != NULL) memcpy(RDst, C, MAX_MRANGE);
...

{
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if (R->StartTime == 0) continue;

        if ((R->Type == mrtJob) || (R->RegEx == NULL)) continue;

        DBG(5, fSTRUCT)
        DPrint("INFO:     checking R[%03d]: '%s'  end: %s\n", rindex, R->Name,
        ...
        if (MUREToList(R->RegEx, mxoNode, -1, NodeList, &NodeCount, Buffer) ==
            FAILURE) {
            DBG(2, fSTRUCT)
            DPrint("ERROR:    cannot expand regex '%s' to check node '%s'\n",
                   R->RegEx, N->Name);
        ...
        TC = MNodeGetTC(N, &N->CRes, &N->CRes, &N->DRes, &R->DRes, MAX_MTIME);

        TC = MAX(TC, 1);

        if (R->MaxTasks > 0) TC = MIN(TC, R->MaxTasks - R->TaskCount);

        /* find first available reservation slot on node */

        if (MResAddNode(R, N, TC, 0) == FAILURE) {
            DBG(1, fSTRUCT)
            DPrint("ALERT:    cannot add node %s to reservation %s\n", N->Name,
                   R->Name);
    ...
    for (srindex = 0; srindex < MAX_MSRES; srindex++) {
        ...
        if (SR->Name[0] == '\0') break;

        if (SR->R[0] != NULL) continue;
...

{
    ...
    DBG(8, fSTRUCT)
    DPrint("%s(%s,%s,%ld,Same,Affinity)\n", FName,
           (R != NULL) ? R->Name : "NULL", (J != NULL) ? J->Name : "NULL",
           Overlap);
    ...
    if (R->Type == mrtJob) {
        ...
        if (!(J->Flags & (1 << mjfResMap))) {
            if (!strcmp(R->Name, J->Name)) {
                ...
                DBG(8, fSTRUCT)
                DPrint("INFO:     exclusive (two job reservations)\n");
    ...
    if ((R->Type == mrtUser) && (J->Flags & (1 << mjfResMap))) {
        ...
        if ((R->Flags & (1 << mrfDedicatedResource)) ||
            ...
            DBG(8, fSTRUCT)
            DPrint("INFO:     exclusive (not shared resource)\n");
    ...
    if ((J->ResName[0] != '\0') && strcmp(R->Name, J->ResName)) {
        DBG(8, fSTRUCT)
        DPrint("INFO:     exclusive (not 'ResName' reservation)\n");
    ...
    if (J->Cred.CL == NULL) {
        DBG(0, fSTRUCT)
        DPrint("WARNING:  job '%s' has NULL cred list\n", J->Name);
...

{
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if ((MSched.Time > R->EndTime) ||
            ...
            if ((R->Type == mrtJob) && (J != NULL) &&
                ...
                DBG(1, fSTRUCT)
                DPrint(
                    "INFO:     extending reservation for overrun job %s by %ld "
                    "seconds\n",
                    J->Name, MSched.Time + MSched.RMPollInterval - R->EndTime);

                R->EndTime = MSched.Time + MSched.RMPollInterval;
            } else {
                DBG(1, fSTRUCT)
                DPrint(
                    "INFO:     clearing expired%s reservation[%03d] '%s' on "
                    "iteration %d  (start: %ld end: %ld)\n",
                    ((R->ExpireTime > 0) && (R->ExpireTime < MAX_MTIME))
                        ? " courtesy"
                        : "",
                    rindex, R->Name, MSched.Iteration,
                    R->StartTime - MSched.Time, R->EndTime - MSched.Time);
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
            ...
            if (N->RE[reindex].Type != mreEnd) continue;
            ...
            if (N->RE[reindex].Time <= MSched.Time) {
                ...
                DBG(6, fSCHED) { MRECheck(N, "MResCheckStatus-Start", TRUE); }

                for (reindex2 = reindex; reindex2 >= 0; reindex2--) {
                    if (N->RE[reindex2].Index != RIndex) continue;

                    if (N->RE[reindex2].Type == mreStart) break;
                } /* END for (reindex2) */
                ...
                if ((N->RE[reindex2].Type != mreStart) ||
                    ...
                    DBG(1, fSTRUCT)
                    DPrint(
                        "ERROR:    RE table on node '%s' is corrupt.  (cannot "
                        "locate start of res '%s'\n",
                        N->Name, N->R[RIndex]->Name);
                ...
                for (index = reindex2; index < (MSched.ResDepth << 1);
                     index++) {
                    if (N->RE[index].Type == mreNONE) break;
                    ...
                    if (Offset > 0)
                        memcpy(&N->RE[index - Offset], &N->RE[index],
                               sizeof(mre_t));
                } /* END for (index) */
                ...
                for (index = 0; index < (MSched.ResDepth << 1); index++) {
                    ...
                    if (N->RE[index].Index == RIndex) break;
                }
                ...
                DBG(6, fSCHED) { MRECheck(N, "MResCheckStatus-End", TRUE); }
...

{
    ...
    DBG(4, (fSCHED | fSTRUCT))
    DPrint("%s(%s,%s,%s)\n", FName, (SN != NULL) ? SN->Name : "", Location,
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
            ...
            if ((R == NULL) || (R == (mres_t *)1)) {
                IsCorrupt = TRUE;
            } else if ((R->Type == mrtUser) && (R->StartTime <= MSched.Time) &&
                       (N->RE[reindex].DRes.Procs * RC <
                        R->DRes.Procs * RC - N->DRes.Procs)) {
                IsCorrupt = TRUE;
            }
            /*
        ...
        if (IsCorrupt == FALSE) {
            for (cindex = 0; cindex < MSched.ResDepth; cindex++) {
                if (N->R[cindex] == NULL) break;

                if (N->R[cindex] == (mres_t *)1) continue;
        ...
        if ((IsCorrupt == TRUE) || (Force == TRUE)) {
            if (IsCorrupt == TRUE) {
                DBG(0, (fSCHED | fSTRUCT))
                DPrint(
                    "ALERT:    corruption found on iteration %d in location %s "
                    "on node %s\n",
                    MSched.Iteration, Location, N->Name);
            }
            ...
            for (rindex = 0; rindex < MSched.ResDepth; rindex++) {
                ...
                if (StartEventFound == FALSE) {
                    DBG(1, fSTRUCT)
                    DPrint("ALERT:    R[%03d] '%s' has no start event\n",
                           rindex, N->R[rindex]->Name);
                }
            } /* END for (rindex)  */
            ...
            for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
                ...
                if ((R == NULL) || (R == (mres_t *)1)) {
                    DBG(1, (fSTRUCT | fSCHED))
                    DPrint("ALERT:    bad pointer in RE[%3d] %c %2d R: %p\n",
                           reindex,
                           (N->RE[reindex].Type == mreStart) ? 'S' : 'E',
                           RIndex, R);
                } else {
                    DBG(7, (fSTRUCT | fSCHED))
                    DPrint(
                        "INFO:     N[%s]->RE[%03d] %c %s(%d)  %s R: '%s'x%d\n",
                        N->Name, reindex,
                        (N->RE[reindex].Type == mreStart) ? 'S' : 'E', R->Name,
                        RIndex, MULToTString(N->RE[reindex].Time - MSched.Time),
                        MUCResToString(&N->RE[reindex].DRes, 0, 0, NULL), RC);

                    if ((R->Type == mrtUser) && (R->StartTime <= MSched.Time) &&
                        (N->RE[reindex].DRes.Procs * RC <
                         R->DRes.Procs * RC - N->DRes.Procs)) {
                        DBG(1, (fSTRUCT | fSCHED))
                        DPrint(
                            "ALERT:    corrupt resource structure in RE[%d]  "
                            "(RED*RC < RD*RC - ND) RC: %d  RED: %d  RD: %d  "
                            "ND: %d\n",
                            reindex, RC, N->RE[reindex].DRes.Procs,
                            R->DRes.Procs, N->DRes.Procs);
                    }
                }
                ...
                if ((Count[RIndex] < 0) || (Count[RIndex] > 1)) {
                    DBG(1, (fSTRUCT | fSCHED))
                    DPrint("ALERT:    RE[%03d] %c %2d  Count: %d\n", reindex,
                           (N->RE[reindex].Type == mreStart) ? 'S' : 'E',
                           RIndex, Count[RIndex]);

                    Count[RIndex] = (Count[RIndex] < 0) ? 0 : 1;
                }
            } /* END for (reindex)            */
            ...
            for (cindex = 0; cindex < MSched.ResDepth; cindex++) {
                ...
                if (Count[cindex] != 0) {
                    DBG(1, fSTRUCT)
                    DPrint("ALERT:    R[%03d] %s started but not ended\n",
                           cindex, N->R[cindex]->Name);
                }

                if (TCount[cindex] < 2) {
                    DBG(1, (fSTRUCT | fSCHED))
                    DPrint("ALERT:    R[%03d] %s has no associated events\n",
                           cindex, N->R[cindex]->Name);
                }
            } /* END for (cindex)           */
...

{
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if ((PR == NULL) || (PR->Name[0] == '\0')) break;

        if (PR->Name[0] == '\1') continue;

        if (!(PR->Flags & (1 << mrfPreemptible))) continue;

        if (PR->Priority >= MIN(R->Priority, MinPriority)) continue;
...

{
    ...
    DBG(8, fSTRUCT) {
        ...
        DPrint("INFO:     releasing reservation %s (%s -> %s : %d)\n", R->Name,
               WCString, MULToTString(R->EndTime - MSched.Time), R->TaskCount);
    }
    ...
    if (X.XResDestroy != (int (*)())0) (*X.XResDestroy)(X.xd, R);
    ...
    if (R->Type == mrtJob) {
        ...
        if (MJobFind(R->Name, &J, 0) == SUCCESS) {
            ...
            DBG(5, fSTRUCT)
            DPrint(
                "INFO:     job '%s' reservation released (tasks requested: "
                "%d)\n",
                J->Name, J->Request.TC);
        } else {
            DBG(1, fSTRUCT)
            DPrint(
                "WARNING:  cannot locate job associated with reservation '%s' "
                "(nodes released)\n",
                R->Name);
        }
    } else {
        ...
        sprintf(Message, "RESERVATIONDESTROYED:  %s %s %ld %ld %ld %d\n",
                R->Name, MResType[R->Type], MSched.Time, R->StartTime,
                R->EndTime, R->NodeCount);
    ...
    if ((J != NULL) && (MSched.InitialLoad != TRUE)) MResAdjustDRes(J->Name, 1);

    if (R->Flags & (1 << mrfMeta)) MResAdjustGResUsage(R, -1);

    if (R->J != NULL) ((mjob_t *)(R->J))->RType = mjrNone;
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (sindex = 0; sindex < MSched.ResDepth; sindex++) {
            DBG(6, fSTRUCT)
            DPrint("INFO:     checking status of node '%s', slot %d\n", N->Name,
                   sindex);

            if (N->R[sindex] == NULL) break;

            if (N->R[sindex] == R) {
                fprintf(mlog.logfp, "[%s]", N->Name);
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (sindex = 0; sindex < MSched.ResDepth; sindex++) {
            DBG(9, fSTRUCT)
            DPrint("INFO:     checking release of node '%s', slot %d\n",
                   N->Name, sindex);
            ...
            if (N->R[sindex] == R) {
                ...
                for (sindex2 = 0; sindex2 < (MSched.ResDepth << 1); sindex2++) {
                    if (N->RE[sindex2].Type == mreNONE) break;
                    ...
                    if (Offset > 0) {
                        memcpy(&N->RE[sindex2 - Offset], &N->RE[sindex2],
                               sizeof(mre_t));
                    }
                } /* END for (sindex2) */
                ...
                DBG(5, fSTRUCT)
                DPrint("INFO:     node '%s' released from reservation\n",
                       N->Name);
            } /* END if (N->R[sindex] == R) */
        }     /* END for (sindex)           */
    }         /* END for (nindex)           */
    ...
    DBG(5, fSTRUCT)
    DPrint("INFO:     %d nodes/%d tasks released from reservation\n", NC, TC);
...

{
    ...
    /* return success if reservation adequately 'covers' up-to-date job
     * requirements */
    ...
    if (J->DispatchTime != R->StartTime) {
        ...
        DBG(3, fSTRUCT)
        DPrint("ALERT:    start time has changed for job %s (%ld != %ld)\n",
               J->Name, J->DispatchTime, R->StartTime);
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        if (RQ->NodeList != NULL) {
            ...
            MUNLGetMinAVal(RQ->NodeList, mnaSpeed, NULL, (void **)&tmpD);
    ...
    tmpEndTime =
        MAX(J->DispatchTime + (long)((double)J->SpecWCLimit[0] / NPFactor),
            MSched.Time + 1);
    ...
    if (tmpEndTime != R->EndTime) {
        ...
        DBG(3, fSTRUCT)
        DPrint(
            "ALERT:    completion time has changed for job %s (%ld != %ld)\n",
            J->Name,
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (sindex = 0; sindex < MSched.ResDepth; sindex++) {
            ...
            if (N->R[sindex] != R) continue;

            /* matching reservation found, verify job requests node */

            for (nlindex = 0; J->Req[0]->NodeList[nlindex].N != NULL;
                 nlindex++) {
                if (nindex == J->Req[0]->NodeList[nlindex].N->Index) break;
            }
            ...
            if (J->Req[0]->NodeList[nlindex].N == NULL) {
                ...
                DBG(3, fSTRUCT)
                DPrint(
                    "ALERT:    reserved node '%s' no longer in job %s "
                    "nodelist\n",
                    N->Name, J->Name);
    ...
    if (NLTaskCount != 0) {
        ...
        DBG(3, fSTRUCT)
        DPrint(
            "ALERT:    node list taskcount mismatch for job %s (%d %s tasks)\n",
            J->Name, (NLTaskCount > 0) ? NLTaskCount : -NLTaskCount,
...

{
    ...
    if (StartTime != NULL) *StartTime = MAX_MTIME;
    ...
    while ((R1[index1].EndTime != 0) || (R2[index2].EndTime != 0)) {
        ...
        if (R1[index1].EndTime == 0) {
            ...
            R1Time =
                (R1State != rEnd) ? R1[index1].StartTime : R1[index1].EndTime;
        }
        ...
        if (R2[index2].EndTime == 0) {
            ...
            R2Time =
                (R2State != rEnd) ? R2[index2].StartTime : R2[index2].EndTime;
        }
        ...
        if (((R1State == rInstant) && (R1Time == ETime)) ||
            ...
            if ((R1State == rInstant) && (R1Time == ETime)) {
                ...
                if ((R2State == rEnd) && (R2Time == ETime)) {
                    ...
                    if (R2Time == ETime) {
                        C[cindex].TaskCount =
                            R1[index1].TaskCount + R2[index2].TaskCount;
                        C[cindex].NodeCount =
                            R1[index1].NodeCount + R2[index2].NodeCount;
                        ...
                        C[cindex].TaskCount =
                            R1[index1].TaskCount + OldTaskCount;
                        C[cindex].NodeCount =
                            R1[index1].NodeCount + OldNodeCount;
                    }
                ...
                if ((R1State == rEnd) && (R1Time == ETime)) {
                    ...
                    if (R1Time == ETime) {
                        ...
                        C[cindex].TaskCount =
                            R2[index2].TaskCount + OldTaskCount;
                        C[cindex].NodeCount =
                            R2[index2].NodeCount + OldNodeCount;
                    }
                    ...
                    if ((R1State == rStart) && (R1Time == ETime)) {
                        ...
                        C[cindex].TaskCount =
                            R2[index2].TaskCount + R1[index1].TaskCount;
                        C[cindex].NodeCount =
                            R2[index2].NodeCount + R1[index1].NodeCount;
                    } else {
                        /* R2 instant only */
    ...
    DBG(3, fALL) {
        ...
        for (index1 = 0; C[index1].EndTime != 0; index1++) {
            DBG(5, fSCHED)
            DPrint("INFO:     C[%02d]  S: %ld  E: %ld  T: %3d  N: %d\n", index1,
                   C[index1].StartTime, C[index1].EndTime, C[index1].TaskCount,
                   C[index1].NodeCount);
            ...
            if ((C[index1].NodeCount <= 0) ||
                (C[index1].NodeCount > C[index1].TaskCount)) {
                IsCorrupt = TRUE;
            ...
            if (C[index1].EndTime < C[index1].StartTime) {
                ...
                DPrint("ALERT:    corrupt time detected:  E(%ld) < S(%ld)\n",
                       C[index1].EndTime, C[index1].StartTime);
            }
            ...
            if ((C[index1 + 1].EndTime != 0) &&
                ...
                DPrint("ALERT:    corrupt time detected:  S2(%ld) < E1(%ld)\n",
                       C[index1 + 1].StartTime, C[index1].EndTime);
            }
        }
    }
        ...
        DBG(3, fALL) {
            for (index1 = 0; R1[index1].EndTime != 0; index1++) {
                DPrint("INFO:     R1[%d]  S: %ld  E: %ld  T: %d  N: %d\n",
                       index1, R1[index1].StartTime, R1[index1].EndTime,
                       R1[index1].TaskCount, R1[index1].NodeCount);
            }

            for (index1 = 0; R2[index1].EndTime != 0; index1++) {
                DPrint("INFO:     R2[%d]  S: %ld  E: %ld  T: %d  N: %d\n",
                       index1, R2[index1].StartTime, R2[index1].EndTime,
                       R2[index1].TaskCount, R2[index1].NodeCount);
            } /* END for (index1) */
        }
    } /* END if (IsCorrupt == TRUE) */
        ...
        switch (R->Type) {
            case mrmtLL:
                ...
                if (Range[rindex].TaskCount < MinTPN) {
                    ...
                    DBG(2, fSCHED)
                    DPrint(
                        "INFO:     range[%d] taskcount for node %s violates "
                        "task distribution policies (%d < %d)\n",
                        rindex, N->Name, Range[rindex].TaskCount, MinTPN);
                }
...
int MResGetRID(
    ...
    mres_t *R, char *RBase, char *RUName)

{
    ...
    sprintf(tmpName, "%s.", RBase);

    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        tmpR = MRes[rindex];

        if ((tmpR == NULL) || (tmpR->Name[0] == '\0')) break;

        if (tmpR->Name[0] == '\1') continue;

        if (strncmp(tmpR->Name, tmpName, strlen(tmpName))) continue;
    ...
    sprintf(RUName, "%s.%d", RBase, index);

    DBG(6, fSTRUCT)
    DPrint("INFO:     unique reservation ID '%s' selected\n", RUName);
...

{
    ...
    DBG(3, fSTRUCT)
    DPrint("%s(%s,MNodeList,%s,%s,Res)\n", FName,
           (J != NULL) ? J->Name : "NULL",
           MULToTString(StartTime - MSched.Time), MJRType[ResType]);
    ...
    if ((MNodeList == NULL) && ((RQ == NULL) || (RQ->NodeList == NULL))) {
        DBG(1, fSTRUCT)
        DPrint("ERROR:    invalid nodelist passed to %s()\n", FName);
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if ((R == NULL) || (R->Name[0] <= '\1')) break;
    } /* END for (rindex) */

    if (rindex >= MAX_MRES) {
        DBG(1, fSTRUCT)
        DPrint("ERROR:    reservation overflow on reservation for job '%s'\n",
               J->Name);
    ...
    if (MResInitialize(&MRes[rindex], NULL) == FAILURE) {
        DBG(1, fSTRUCT)
        DPrint("ERROR:    cannot initialize reservation for job '%s'\n",
               J->Name);
    ...
    if (J->R != NULL) {
        DBG(0, fSTRUCT)
        DPrint(
            "ERROR:    reservation created for reserved job '%s' (existing "
    ...
    if (MNodeList != NULL) {
        ...
        for (rqindex = 0; MNodeList[rqindex][0].N != NULL; rqindex++) {
            MUNLGetMinAVal((mnalloc_t *)&MNodeList[rqindex][0], mnaSpeed, NULL,
                           (void **)&tmpD);
        ...
        for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
            ...
            if (RQ->NodeList != NULL) {
                if (MUNLGetMinAVal(RQ->NodeList, mnaSpeed, NULL,
                                   (void *)&tmpD) == FAILURE) {
                    continue;
                }
    ...
    R->EndTime = MAX(StartTime + (int)((double)J->SpecWCLimit[0] / NPFactor),
                     MSched.Time + 1);
    ...
    if (StartTime == 0) {
        DBG(0, fSTRUCT)
        DPrint(
            "ERROR:    invalid StartTime specified for reservation on job res "
            "'%s'\n",
            R->Name);
    ...
    DBG(6, fSTRUCT)
    DPrint("INFO:     linking nodes to reservation '%s'\n", R->Name);
    ...
    while (1) {
        ...
        if (MNodeList == NULL) {
            ...
            if (rqindex >= MAX_MREQ_PER_JOB) break;
        ...
        if ((N->Name[0] == '\0') || (N->Name[0] == '\1')) {
            DBG(0, fSTRUCT)
            DPrint(
                "ERROR:    job '%s' reservation nodelist contains invalid node "
                "at index %d\n",
                J->Name, nindex - 1);
        ...
        if ((N->PtIndex != R->PtIndex) && (N->PtIndex != 0) &&
            !(J->Flags & (1 << mjfSpan))) {
            DBG(0, fSTRUCT)
            DPrint(
                "ERROR:    job '%s' reservation spans partitions (node %s: %d  "
                "node %s: %d)\n",
                J->Name, (MNodeList != NULL) ? MNodeList[rqindex][0].N->Name
                                             : RQ->NodeList[0].N->Name,
                R->PtIndex, N->Name, N->PtIndex);
...

{
    ...
    if (RE[0].Type == mreNONE) {
        ...
        if (index == TableSize) {
            DBG(3, fSCHED)
            DPrint("ALERT:    RE table overflow detected - increase %s\n",
                   MParam[pResDepth]);
...

{
    ...
    for (index = 0; index < TableSize; index++) {
        if (RE[index].Type == mreNONE) break;
...
int MResShowState(
    ...
    mres_t *R, int Flags, char *Buffer, int BufSize, int Mode) /* I */

{
    ...
    DBG(5, fSTRUCT)
    DPrint("%s(%s,%d,Buffer,BufSize,%d)\n", FName,
           (R != NULL) ? "NONE" : R->Name, Flags, Mode);
    ...
    if (R == NULL) {
        ...
        MUSNPrintF(&BPtr, &BSpace,
                   "%-20s %10s %3.3s %11s %11s  %11s %4s %4s %4s\n", "ResID",
                   "Type", "Par", "StartTime", "EndTime", "Duration", "Node",
                   "Task", "Proc");
...

    {
        ...
        MUSNPrintF(&BPtr, &BSpace,
                   "%-20s %10s %3.3s %11s %11s  %11s %4d %4d %4d\n", R->Name,
                   MResType[R->Type],
                   (R->PtIndex >= 0) ? MAList[ePartition][R->PtIndex] : "ALL",
                   StartTime, EndTime, Duration, R->NodeCount, R->TaskCount,
                   R->AllocPC);
    } /* END BLOCK */
    ...
    if (R->RegEx != NULL) {
        snprintf(tmpLine, MAX_MLINE, "%s   HostList='%s'", tmpLine, R->RegEx);
    }

    if (R->Priority > 0) {
        sprintf(temp_str, "   Priority=%ld", R->Priority);
        strcat(tmpLine, temp_str);
    }

    if (R->SystemID != NULL) {
        snprintf(tmpLine, MAX_MLINE, "%s   SystemID=%s", tmpLine, R->SystemID);
    }

    if (R->MaxTasks > 0) {
        sprintf(temp_str, "   MaxTasks=%d", R->MaxTasks);
        strcat(tmpLine, temp_str);
    }
    ...
    if (R->CBHost[0] != '\0') {
        ...
        for (index = 0; MCBType[index] != NULL; index++) {
            if (R->CBType & (1 << index)) {
                if (tmpCBLine[0] != '\0') strcat(tmpCBLine, ",");

                strcat(tmpCBLine, MCBType[index]);
            }
        } /* END for (index) */

        sprintf(temp_str, "   CBServer=%s:%d/%s", R->CBHost, R->CBPort,
                tmpCBLine);
        strcat(tmpLine, temp_str);
    } /* END if (R->CBHost[0] != '\0') */
    ...
    if ((R->Type != mrtJob) &&
        ...
        if ((R->TAPS + R->CAPS) > 0.0) {
            E = (1.0 -
                 (double)(R->TIPS + R->CIPS) /
                     (R->TAPS + R->CAPS + R->TIPS + R->CIPS)) *
                100.0;
        } else {
            E = 0.0;
        }
        ...
        MUSNPrintF(&BPtr, &BSpace, "    Active PH: %.2f/%-.2f (%.2f%c)\n",
                   ...
                   (R->TAPS + R->CAPS + R->TIPS + R->CIPS) / 3600.0, E, '%');
    } /* END if (R->Type != mrtJob) && ...) */
    ...
    if (R->Flags & (1 << mrfStandingRes)) {
        ...
        for (srindex = 0; srindex < MAX_MSRES; srindex++) {
            ...
            if (strncmp(S->Name, R->Name, strlen(S->Name))) continue;
            ...
            for (dindex = 0; dindex < MAX_SRES_DEPTH; dindex++) {
                if (S->R[dindex] == R) {
                    ...
                    MUSNPrintF(
                        &BPtr, &BSpace,
                        "    SRAttributes (TaskCount: %d  StartTime: %s  "
                        "EndTime: %s  Days: %s)\n",
                        tmpSR.TaskCount, tmpTString,
                        (tmpSR.EndTime == -1)
                            ? MULToTString(86400)
                            : MULToTString((tmpSR.EndTime > 0) ? tmpSR.EndTime
                                                               : 86400),
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (N->ResMTime < MSched.Time) continue;
        ...
        for (nreindex = 0; N->RE[nreindex].Type != mreNONE; nreindex++) {
            ...
            if (jreindex >= (MSched.ResDepth << 1) - 1) break;

            if ((N->R[N->RE[nreindex].Index] == NULL) ||
                (N->R[N->RE[nreindex].Index] == (mrsv_t *)1))
                continue;
                ...
                for (crindex = 0; crindex < MSched.ResDepth; crindex++) {
                    if (N->RE[nreindex].Index == CRI[crindex]) {
                        ...
                        if (JRETime != N->RE[nreindex].Time) {
                            ...
                            for (treindex = nreindex + 1;
                                 N->RE[treindex].Type != mreNONE; treindex++) {
                                if (N->RE[treindex].Time !=
                                    N->RE[nreindex].Time)
                                    break;

                                if (N->R[N->RE[nreindex].Index]->Type ==
                                    mrtJob) {
                                    JRETime = N->RE[nreindex].Time;
                        ...
                        if (JRETime != N->RE[nreindex].Time) {
                            ...
                            if (N->RE[nreindex].Type == mreStart) {
                                ...
                                if (OPC > NPC) {
                                    ...
                                    if ((N->RE[CRESI[crindex]].DRes.Procs ==
                                         -1) ||
                                        (N->RE[CREEI[crindex]].DRes.Procs ==
                                         -1)) {
                                        fprintf(stderr,
                                                "ALERT:    negative blocked "
                                                "resources detected\n");
                                    }
                                } else if (OPC < NPC) {
                                    memcpy(&mCRE[crindex].DRes,
        ...
        if (((JName != NULL) && (NameFound == FALSE)) || (jreindex == 0)) {
        ...
        DBG(6, fSCHED) { MRECheck(N, "MResAdjustDRes-Start", TRUE); }

        memcpy(ORE, JRE, sizeof(ORE[0]) * (jreindex + 1));

        DBG(6, fSTRUCT)
        DPrint("INFO:     updating cr reservations for job '%s'\n",
        ...
        for (crindex = 0; CRI[crindex] != -1; crindex++) {
            ...
            for (jrindex1 = 0; JRE[jrindex1].Type != mreNONE; jrindex1++) {
                ...
                for (jrindex2 = 0; JRE[jrindex2].Type != mreNONE; jrindex2++) {
                    ...
                    /* if job reservation overlaps container reservation
                     * component */
                    ...
                    switch (R->Type) {
                        case mrtUser:
                            ...
                            if (MResCheckJAccess(R, J, CROverlap, NULL, NULL) ==
                                TRUE) {
                                if (JRE[jrindex2].Type == mreStart) {
                                    /* remove dedicated job resources from
                                     * container blocked resources */
                                    ...
                                    MCResAdd(&BR, &N->CRes, &JR->DRes, JRC,
                                             FALSE);
                                }
                            }
            ...
            while ((CRE[creindex].Type != mreNONE) ||
                ...
                if ((creindex >= (MSched.ResDepth << 1)) ||
                    ...
                    DBG(1, fSTRUCT)
                    DPrint(
                        "ALERT:    node reservation event overflow (N: %d  C: "
                        "%d  O: %d) - increase %s\n",
                        nreindex, creindex, oreindex, MParam[pResDepth]);
        ...
        for (nreindex = 0; N->RE[nreindex].Type != mreNONE; nreindex++) {
            ...
            if (CRStartTime > N->RE[nreindex].Time) {
                DBG(2, fSTRUCT)
                DPrint(
                    "ALERT:    node %s RE table is corrupt.  RE[%d] '%s' at %s "
        ...
        DBG(6, fSCHED) { MRECheck(N, "MResAdjustDRes-End", TRUE); }
    } /* END for (nindex)  */
...

{
    ...
    DBG(5, fSTRUCT)
    DPrint("%s(%s,%d,SBuffer,SBufSize,%d)\n", FName,
           (R != NULL) ? "NONE" : R->Name, Flags, Mode);
    ...
    if ((R->Type == mrtUser) ||
        ...
        for (rindex = 0; rindex < R->Index; rindex++) {
            ...
            if (!strcmp(R->Name, tmpR->Name)) {
                MUSNPrintF(
                    &BPtr, &BSpace,
                    "WARNING:  res '%s' (%d) is duplicated at index %d\n",
                    R->Name, R->Index, rindex);
        ...
        if (MJobFind(R->Name, &J, 0) == FAILURE) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  cannot find job associated with res '%s'\n",
                       R->Name);
        } else if (J->R != R) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  job '%s' does not link to reservation\n",
                       J->Name);
        }
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (nrindex = 0; N->R[nrindex] != NULL; nrindex++) {
            if (N->R[nrindex] != (mres_t *)1) RCount++;
        }
        ...
        if (IsCorrupt == FALSE) {
            if ((RECount < (RCount << 1)) || ((RECount % 2) == 1)) {
                IsCorrupt = TRUE;
            } else {
                for (cindex = 0; cindex < MSched.ResDepth; cindex++) {
                    if (N->R[cindex] == NULL) break;
        ...
        if (IsCorrupt == TRUE) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  RE table on node %s is corrupt\n", N->Name);

            memset(Count, 0, sizeof(Count));

            for (nrindex = 0; nrindex < MSched.ResDepth; nrindex++) {
                if (N->R[nrindex] == NULL) break;

                MUSNPrintF(&BPtr, &BSpace, "  R[%02d] '%s'\n", nrindex,
                           (N->R[nrindex] != (mres_t *)1) ? N->R[nrindex]->Name
                                                          : "EMPTY");
            } /* END for (nrindex) */
            ...
            for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
                ...
                MUSNPrintF(&BPtr, &BSpace, "  RE[%02d] %10s %5s %12s %s\n",
                           ...
                           (N->RE[reindex].Type == mreStart) ? "Start" : "End",
                ...
                if ((N->R[N->RE[reindex].Index]->Type == mrtUser) &&
                    (!memcmp(&N->RE[reindex].DRes, &ZRes,
                             sizeof(N->RE[reindex].DRes)) ||
                     (N->RE[reindex].DRes.Procs == 0))) {
                    /* empty dedicated resource structure */

                    MUSNPrintF(&BPtr, &BSpace,
                               "WARNING:  RE[%02d] user reservation has empty "
                               "resource structure\n",
                               reindex);
                ...
                if ((Count[N->RE[reindex].Index] < 0) ||
                    ...
                    MUSNPrintF(&BPtr, &BSpace, "WARNING:  RE[%02d] %s\n",
                               reindex, (Count[N->RE[reindex].Index] < 0)
                                            ? "reservation end event located "
                                              "without accompanying start event"
                                            : "reservation start event located "
                                              "without accompanying end event");

                    Count[N->RE[reindex].Index] =
                        (Count[N->RE[reindex].Index] < 0) ? 0 : 1;
                }
            } /* END for (reindex) */
            ...
            for (cindex = 0; cindex < MSched.ResDepth; cindex++) {
                ...
                if (Count[cindex] != 0) {
                    MUSNPrintF(&BPtr, &BSpace, "WARNING:  R[%02d] %s %s\n",
                               cindex, N->R[cindex]->Name,
                               "reservation start event located but no "
                               "accompanying end event found");
                }
        ...
        for (rindex = 0; rindex < MSched.ResDepth; rindex++) {
            ...
            if (N->R[rindex] != R) continue;

            ResNC++;

            for (reindex = 0; reindex < (MSched.ResDepth << 1); reindex++) {
                if (N->RE[reindex].Type == 0) break;

                if (N->R[N->RE[reindex].Index] != R) continue;
            ...
            ResPC +=
                (D->Procs == -1) ? N->CRes.Procs : (D->Procs * N->RC[rindex]);

            ResTC += N->RC[rindex];

            if (NodeRC++ > 0) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' has multiple links to "
                           "reservation '%s' (clearing extra links)\n",
                           N->Name, R->Name);
    ...
    if (ResPC != R->AllocPC) {
        MUSNPrintF(&BPtr, &BSpace,
                   "WARNING:  reservation '%s' has %d proc(s) allocated but %d "
                   "detected\n",
                   R->Name, R->AllocPC, ResPC);
    }

    if (ResTC != R->TaskCount) {
        MUSNPrintF(&BPtr, &BSpace,
                   "WARNING:  reservation '%s' has %d task(s) allocated but %d "
                   "detected\n",
                   R->Name, R->TaskCount, ResTC);
    }

    if (ResNC != R->NodeCount) {
        MUSNPrintF(&BPtr, &BSpace,
                   "WARNING:  reservation '%s' has %d node(s) allocated but %d "
                   "detected\n",
                   R->Name, R->NodeCount, ResNC);
    }
...

{
    ...
    IsInstantA = ((ARL[index1].StartTime == ARL[index1].EndTime) &&
                  (ARL[index1].EndTime > 0))
                     ? TRUE
                     : FALSE;

    IsInstantU = ((URL[index2].StartTime == URL[index2].EndTime) &&
                  (URL[index2].EndTime > 0))
                     ? TRUE
                     : FALSE;
    ...
    while (TRUE) {
        ...
        if (ATime == ETime) {
            if (AType == mreStart) {
                ...
                AType = mreStart;
                ATime = (ARL[index1].EndTime == 0) ? MAX_MTIME + 1
                                                   : ARL[index1].StartTime;
            }
        }
        ...
        if (UTime == ETime) {
            if (UType == mreStart) {
                ...
                UType = mreStart;
                UTime = (URL[index2].EndTime == 0) ? MAX_MTIME + 1
                                                   : URL[index2].StartTime;
            }
        }
        ...
        if (TC != OldTC) {
            if (OldTC > 0) {
                if ((ETime > C[cindex].StartTime) || (IsInstantA == TRUE) ||
                    (IsInstantU == TRUE)) {
            ...
            if (((ATime == ETime) && (IsInstantA == TRUE)) ||
                ...
                if ((ATime == ETime) && (IsInstantA == TRUE)) {
                    ...
                    IsInstantA =
                        ((ARL[index1].StartTime == ARL[index1].EndTime) &&
                         (ARL[index1].EndTime > 0))
                            ? TRUE
                            : FALSE;
                ...
                if ((UTime == ETime) && (IsInstantU == TRUE)) {
                    ...
                    IsInstantU =
                        ((URL[index2].StartTime == URL[index2].EndTime) &&
                         (URL[index2].EndTime > 0))
                            ? TRUE
                            : FALSE;

                    UType = mreStart;
                    UTime = (URL[index2].EndTime == 0) ? MAX_MTIME + 2
                                                       : URL[index2].StartTime;
                }
...

{
    ...
    while ((R1[index1].EndTime != 0) || (R2[index2].EndTime != 0)) {
        if (R1[index1].EndTime == 0) {
            ...
            if (R1[index1].StartTime == R1[index1].EndTime) R1State = rInstant;
        }
        ...
        if (R2[index2].EndTime == 0) {
            ...
            if (R2[index2].StartTime == R2[index2].EndTime) R1State = rInstant;
        }
...
int MResDiagGrid(
    ...
    char *SBuffer, int SBufSize, int Mode)
    ...
    if (MPar[0].MaxMetaTasks > 0) {
        ...
        if (MRange[0].EndTime == 0) {
            ...
            for (rindex = 0; rindex < (MAX_MRES_DEPTH << 1); rindex++) {
                if (MRange[rindex].EndTime == 0) break;

                strcpy(TimeLine,
                       MULToTString(MRange[rindex].StartTime - MSched.Time));

                MUSNPrintF(&BPtr, &BSpace, "%5d  %10s -> %10s\n",
                           MRange[rindex].TaskCount, TimeLine,
...
int MResToString(
    ...
    int *RC,                   /* I (optional) */
    char *SBuf,                /* O */
    int SBufSize, int *RAList) /* I (optional) */
...

{
    ...
    static int DNRAList[] = {mnraDRes,  mnraEnd, mnraName, mnraState,
                             mnraStart, mnraTC,  mnraType, -1};

    mxml_t *NE = NULL;

    if ((N == NULL) || ((SBuf == NULL) && (EP == NULL))) {
        return (FAILURE);
    }
    ...
    for (rindex = 0; rindex < MSched.ResDepth; rindex++) {
        ...
        if ((R != NULL) && (N->R[rindex] != R)) continue;
...

{
    ...
    sprintf(tmpLine, "%ld", R->StartTime);

    MXMLSetAttr(E, (char *)MNResAttr[mnraStart], tmpLine, mdfString);

    sprintf(tmpLine, "%ld", R->EndTime);
    ...
    switch (R->Type) {
...

        {
            ...
            if ((J = (mjob_t *)R->J) == NULL) {
                DBG(2, fUI)
                DPrint("ERROR:    cannot locate host job '%s'\n", R->Name);
...

{
    ...
    for (aindex = 0; ARL[aindex].EndTime > 0; aindex++) {
        ...
        for (rindex = aindex + 1; ARL[rindex].EndTime != 0; rindex++) {
            if (ARL[rindex - 1].EndTime != ARL[rindex].StartTime) break;

            RangeTime += ARL[rindex].EndTime - ARL[rindex].StartTime;
            TaskCount = MIN(TaskCount, ARL[rindex].TaskCount);

            if (RangeTime >
                (ARL[aindex].EndTime - ARL[aindex].StartTime + MinDuration))
                break;
        } /* END for (rindex) */
        ...
        if (RangeTime >= MinDuration) {
            ...
            SRL[sindex].EndTime =
                MIN(ARL[aindex].EndTime,
                    ARL[aindex].StartTime + RangeTime - MinDuration);
    ...
    if ((SRL[0].EndTime == 0) || (SRL[0].TaskCount == 0)) {
        if ((SRL[0].TaskCount == 0) && (SRL[0].EndTime != 0)) {
            DBG(0, fSCHED)
            DPrint(
                "ALERT:    corrupt range found in %s (initial range empty)\n",
                FName);

            for (aindex = 0; ARL[aindex].EndTime > 0; aindex++) {
                DBG(2, fSCHED)
                DPrint("INFO:     ARL[%d]  %ld - %ld  %d  %d\n", aindex,
                       ARL[aindex].StartTime, ARL[aindex].EndTime,
                       ARL[aindex].TaskCount, ARL[aindex].NodeCount);
            } /* END for (aindex) */
        }
