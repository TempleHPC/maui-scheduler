...
char *MACLListShow(
    ...
    macl_t *ACL, int OType, int Mode, char *Buffer) /* O: (required) */

{
    ...
    for (oindex = 0; oindex < MAX_MATTRTYPE; oindex++) {
        if ((OType != -1) && (OType != oindex)) continue;

        tmpLine[0] = '\0';

        for (aindex = 0; ACL[aindex].Type != maNONE; aindex++) {
            if (ACL[aindex].Type != oindex) continue;
    ...
    if (ACLFound == FALSE) {
        if (Mode & (1 << mfmAVP)) strcat(Buffer, NONE);
...
char *MACLShow(
    ...
    macl_t ACL, int Mode, int ShowObject)

{
    ...
    if (Mode & (1 << mfmVerbose)) {
        ...
        if (ACL.Flags & (1 << maclRequired)) ModString[cindex++] = '*';

        if (ACL.Flags & (1 << maclDeny)) ModString[cindex++] = '!';

        if (ACL.Flags & (1 << maclXOR)) ModString[cindex++] = '^';
    ...
    switch (ACL.Cmp) {
        ...
        default:
            ...
            sprintf(&ACLString[strlen(ACLString)], "%s%s%s", MComp[cindex],
                    ACL.Name, ModString);
        ...
        case mcmpGT:
            ...
            if ((ACL.Type == maDuration) && (Mode & (1 << mfmHuman))) {
                ...
                sprintf(&ACLString[strlen(ACLString)], "%s%s%s", MComp[cindex],
                        MULToTString(ACL.Value), ModString);
            } else {
                sprintf(&ACLString[strlen(ACLString)], "%s%ld%s", MComp[cindex],
                        ACL.Value, ModString);
            }
    ...
    if ((Mode & (1 << mfmAVP)) && (Line[0] == '\0')) strcpy(Line, NONE);
...

{
    ...
    DBG(7, fCONFIG)
    DPrint("%s(ACL,%s,%d,%s)\n", FName, (ACLList != NULL) ? ACLList[0] : "NULL",
           ACLCount, MAttrO[ObjType]);

    /* FORMAT:
     * [<CMP>]{[<MODIFIER>]<OBJECTVAL>[<AFFINITY>]}[{:,}{[<MODIFIER>]<OBJECTVAL>[<AFFINITY>]}]...
     */
    ...
    for (aindex = 0; aindex < ACLCount; aindex++) {
        ...
        while (ptr != NULL) {
            if ((ACLStart + aindex) >= MAX_MACL) break;
            ...
            switch (ACL[ACLStart + AIndex].Cmp) {
                ...
                case mcmpSSUB:
                    ...
                    MUStrCpy(ACL[ACLStart + AIndex].Name, ptr2,
                             sizeof(ACL[0].Name));
            ...
            DBG(7, fCONFIG)
            DPrint("INFO:     ACL[%d] loaded with %s %s (Affinity: %d)\n",
                   ACLStart + AIndex, MAttrO[ObjType], ptr2,
...

{
    ...
    for (aindex = 0; ACL[aindex].Type != maNONE; aindex++) {
        ...
        for (cindex = 0; CL[cindex].Type != maNONE; cindex++) {
            ...
            if (CL[cindex].Type != ACL[aindex].Type) continue;

            switch (ACL[aindex].Cmp) {
                case mcmpEQ:

                    if (CL[cindex].Value != ACL[aindex].Value) ValMatch = FALSE;
            ...
            if (Affinity != NULL) *Affinity = ACL[aindex].Affinity;
        ...
        if ((ACL[aindex].Flags & (1 << maclRequired)) && (ACLMatch != TRUE)) {
...
int MACLSet(
    ...
    macl_t *ACL, /* I */
    int Type, void *Val, int Cmp, int Affinity, long Flags,
...

{
    ...
    for (aindex = 0; aindex < MAX_MACL; aindex++) {
        if (ACL[aindex].Type != maNONE) {
            if ((Mode != 0) || (ACL[aindex].Type != Type)) continue;
        }

        if (ACL[aindex].Type == maNONE) ACL[aindex + 1].Type = maNONE;
...
int MACLGet(
    ...
    macl_t *ACL, int OType, void **Value, int *Start)

{
    ...
    for (aindex = sindex; ACL[aindex].Type != maNONE; aindex++) {
        if (ACL[aindex].Type != OType) continue;
...

{
    ...
    for (tindex = 0; tindex < MAX_MACL; tindex++) {
        if (ACL[tindex].Type == maNONE) break;
    }
