...

{
            ...
            for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                ...
                for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
                    ...
                    TotalProcDed[N->Index] +=
                        RQ->NodeList[nindex].TC * ((RQ->DRes.Procs >= 0)
                                                       ? RQ->DRes.Procs
                                                       : N->CRes.Procs);
                    TotalProcUtl[N->Index] +=
                        RQ->NodeList[nindex].TC * RQ->URes.Procs / 100.0;
                } /* END for (nindex)   */
            }     /* END for (rqindex)  */
        }         /* END if (J->State)  */
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        MNodeAdjustAvailResources(N, TotalProcUtl[N->Index],
                                  TotalProcDed[N->Index],
                                  TotalTaskDed[N->Index]);

        DBG(6, fPBS)
        DPrint("INFO:     node '%s' C/A/D procs:  %d/%d/%d\n", N->Name,
               N->CRes.Procs, N->ARes.Procs, N->DRes.Procs);
    } /* END for (nindex) */
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(%s,%d,%d,Buf,BufSize)\n", FName,
           (ResDesc != NULL) ? ResDesc : "NULL", DFlags, DisplayMode);
    ...
    if (MUSScanF(
            ...
            sizeof(UName), UName, sizeof(GName), GName, sizeof(AName), AName,
            sizeof(PName), PName, &MinTime, &MinNodes, &MinProcs, &ReqMem,
            &ReqMem, sizeof(ReqMemCmp), ReqMemCmp, &ShowSMP, &Flags,
            sizeof(CName), CName, sizeof(FeatureString), FeatureString,
            sizeof(QName), QName) == FAILURE) {
