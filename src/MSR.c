...

{
    ...
    if ((SRName == NULL) || (SRName[0] == '\0')) {
        ...
        while (MCfgGetSVal(MSched.ConfigBuffer, &ptr, MSRCfgParm, IndexName,
        ...
        while (MCfgGetSVal(MSched.ConfigBuffer, &ptr, MSRCfgParm, SRName, NULL,
                           Value, sizeof(Value), 0, NULL) == SUCCESS) {
            if ((SR == NULL) && (MSRFind(SRName, &SR) == FAILURE) &&
...

{
    ...
    for (srindex = 0; srindex < MAX_MSRES; srindex++) {
        ...
        if (strcmp(SR->Name, SRName) != 0) continue;

        /* reservation found */

        if (SRPtr != NULL) *SRPtr = SR;
...

{
    ...
    for (srindex = 0; srindex < MAX_MSRES; srindex++) {
        ...
        if (SR == NULL) {
            if (MSRCreate(SRName, &SRes[srindex]) == FAILURE) {
                DBG(1, fALL)
                DPrint("ERROR:    cannot alloc memory for SR '%s'\n", SRName);
        ...
        if (MSched.Mode != msmSim) MCPRestore(mcpSRes, SRName, (void *)SR);
    ...
    DBG(1, fSTRUCT)
    DPrint("ALERT:    SR table overflow.  cannot add %s\n", SRName);
...

{
    ...
    DBG(3, fSCHED)
    DPrint("%s(%s,%s,DstNL,NodeCount,%s,ReqNL,%ld)\n", FName,
           (J != NULL) ? J->Name : "NULL", SR->Name,
           MULToTString(StartTime - MSched.Time), ResMode);
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        for (pindex = 0; pindex < MAX_MPAR; pindex++) {
            ...
            if (SR->PName[0] != '\0') {
                if (!strcmp(SR->PName, "ALL")) {
                    ...
                    if (strcmp(P->Name, SR->PName)) continue;
                }
            } else {
            ...
            if ((strcmp(SR->PName, "ALL") != 0) && (P->Index == 0)) {
                continue;
            }

            DBG(6, fSCHED)
            DPrint(
                "INFO:     checking partition %s for resources for reservation "
                "%s\n",
                P->Name, SR->Name);
            ...
            if (MReqGetFNL(J, RQ, P, NULL, *FNL, NodeCount, &TC, MAX_MTIME,
                           ResMode) == FAILURE) {
                DBG(6, fSCHED)
                DPrint(
                    "INFO:     cannot get feasible tasks for job %s:%d in "
                    "partition %s\n",
                    J->Name, rqindex, P->Name);
        ...
        if (BestP == NULL) {
            ...
            DBG(6, fSCHED)
            DPrint(
                "INFO:     no feasible tasks found for job %s:%d in partition "
                "%s\n",
                J->Name, rqindex, SR->PName);
    ...
    if (!(SR->Flags & (1 << mrfSpaceFlex)) &&
        ((ResMode & (1 << stForce)) || (SR->TaskCount == 0) ||
         (SR->Flags & (1 << mrfForce)))) {
    ...
    if (IsSpaceFlex == FALSE) {
        ...
        if (MJobGetAMNodeList(J, BestFMNL, AvailMNL, NodeMap, NodeCount, &TC,
                              StartTime) == FAILURE) {
            DBG(4, fSCHED)
            DPrint(
                "INFO:     cannot locate available tasks in partition %s for "
                "job %s at time %ld\n",
                SR->PName, J->Name, StartTime);
    ...
    for (nindex = 0; ReqNL[nindex].N != NULL; nindex++) {
        for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
            for (nindex2 = 0; AvailMNL[rqindex][nindex2].N != NULL; nindex2++) {
                if (AvailMNL[rqindex][nindex2].N == ReqNL[nindex].N) {
    ...
    if (MJobAllocMNL(J, AvailMNL, NodeMap, tmpMNL,
                     MPar[J->Req[0]->PtIndex].NAllocPolicy,
                     StartTime) == FAILURE) {
        DBG(0, fSCHED)
        DPrint("ERROR:    cannot allocate best nodes for job '%s'\n", J->Name);
...

{
    ...
    if ((SR == NULL) || (R == NULL)) return (FAILURE);
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (nrindex = 0; nrindex < MSched.ResDepth; nrindex++) {
            ...
            if (N->R[nrindex] != R) continue;

            /* reservation located on node */

            if ((SR->TaskCount > 0) && (R->StartTime <= MSched.Time) &&
    ...
    if (TC < R->TaskCount) {
        sprintf(tmpLine,
                "RESERVATIONCORRUPTION:  reservation corruption detected in "
                "reservation '%s'  Req/Detected TC %d/%d\n",
                R->Name, R->TaskCount, TC);
    ...
    for (SRIndex = 0; SRIndex < MAX_MSRES; SRIndex++) {
        ...
        if (SR->TaskCount == 0) {
            if (SR->HostExpression[0] == '\0') {
                ...
                DBG(7, fCORE)
                DPrint("INFO:     SR[%d] is empty (TC: %d  HL: '%s')\n",
                       SRIndex, SR->TaskCount, SR->HostExpression);
            } else {
                /* continue if exact hosts specified */

                /* all hosts located and incorporated in reservation at node
                 * load time */

                DBG(5, fCORE)
                DPrint(
                    "INFO:     SR[%d] exactly specifies nodes (TC: %d  HL: "
                    "'%s')\n",
                    SRIndex, SR->TaskCount, SR->HostExpression);
            }
            ...
            if (MSRSetRes(SR, SRIndex, DIndex) == SUCCESS) {
                if (SR->R[DIndex] != NULL) {
                    while (SR->R[DIndex]->TaskCount < SR->TaskCount) {
                        if (MResPreempt(SR->R[DIndex]) == FAILURE) break;
                ...
                DBG(2, fSCHED)
                DPrint("ALERT:    cannot create standing reservation '%s'\n",
                       SR->Name);
            }
        } /* END for (DIndex)  */
    }     /* END for (SRIndex) */
...

{
    ...
    if ((SR == NULL) || (Value == NULL) || (Value[0] == '\0')) {
        return (FAILURE);
    }
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MSResAttr, &aindex, NULL, FALSE,
                      &ValCmp, ValLine, MAX_MLINE) == FAILURE) {
            /* cannot parse value pair */

            DBG(2, fSCHED)
            DPrint("ALERT:    cannot parse SR[%s] config attribute '%s'\n",
                   SR->Name, ptr);
        ...
        sprintf(tmpLine, "%s%s", MComp[ValCmp], ValLine);
        ...
        switch (aindex) {
...

            {
                ...
                MUStrCpy(SR->HostExpression, ValLine,
                         sizeof(SR->HostExpression));
...

            {
                ...
                if (strchr(ValLine, '*') != NULL) Required = TRUE;

                MACLSet(SR->ACL, maDuration, &MaxTime, mcmpLE,
                        nmPositiveAffinity,
                        (Required == TRUE) ? (1 << maclRequired) : 0, 0);
            } /* END BLOCK */
            ...
            case msraNodeFeatures:
                ...
                if (!strcmp(ValLine, NONE)) break;

                ptr2 = MUStrTok(ValLine, "[] \t", &TokPtr2);

                while (ptr2 != NULL) {
                    MUGetMAttr(eFeature, ptr2, mAdd, SR->FeatureMap,
                               sizeof(SR->FeatureMap));
            ...
            default:
                ...
                DBG(4, fFS)
                DPrint("WARNING:  SR attribute '%s' not handled\n",
                       MSResAttr[aindex]);
...

{
    ...
    struct {
        int PIndex;
        int OIndex;
    } AList[] = {{msraAccountList, maAcct}, {msraClassList, maClass},
                 {msraUserList, maUser},    {msraGroupList, maGroup},
                 {msraQOSList, maQOS},      {-1, -1}};

    if ((SR == NULL) || (Buffer == NULL)) return (FAILURE);

    /* display SR if enabled */

    if ((SR->Name[0] == '\0') || (PIndex != -1)) {
        return (SUCCESS);
    }
    ...
    for (index = 0; AList[index].PIndex != -1; index++) {
        if (MACLListShow(SR->ACL, AList[index].OIndex, (1 << mfmHuman),
                         tmpLine) == NULL) {
            continue;
        }
        ...
        sprintf(temp_str, "%s=%s  ", MSResAttr[AList[index].PIndex], tmpLine);
    ...
    if (SR->TaskCount != 0) {
        sprintf(temp_str, "%s=%d  ", MSResAttr[msraTaskCount], SR->TaskCount);
        strcat(SRLine, temp_str);
    ...
    if (SR->HostExpression[0] != '\0') {
        sprintf(temp_str, "%s=%s  ", MSResAttr[msraHostList],
                SR->HostExpression);
        strcat(SRLine, temp_str);
    ...
    if (SR->Flags != 0) {
        ...
        sprintf(temp_str, "%s=%s  ", MSResAttr[msraFlags],
    ...
    if (strcmp(tmpLine, NONE)) {
        sprintf(temp_str, "%s=%s  ", MSResAttr[msraNodeFeatures], tmpLine);
        strcat(SRLine, temp_str);
    }
...
int MSRDiag(
    ...
    sres_t *SRS, char *SBuffer, int SBufSize, int Flags)

{
    ...
    if (SBuffer == NULL) return (FAILURE);
    ...
    for (srindex = 0; srindex < MAX_MSRES; srindex++) {
        ...
        if (SR->Name[0] == '\0') continue;

        if ((SRS != NULL) && (SRS != SR)) continue;
        ...
        if ((SR->TaskCount <= 0) && (SR->HostExpression[0] == '\0')) {
            ...
            MUSNPrintF(&BPtr, &BSpace,
                       "SR %s does not specify requested resources "
                       "(taskcount/hostexpression not set\n",
                       SR->Name);
        ...
        for (rindex = 0; rindex < MAX_SRES_DEPTH; rindex++) {
            ...
            if (SR->TaskCount > 0) {
                if (R->TaskCount < SR->TaskCount) IsPartial = TRUE;

                MUSNPrintF(
                    &BPtr, &BSpace,
                    "SR reservation %s is %s (TC: %d requested/%d reserved)\n",
                    R->Name, (IsPartial == TRUE) ? "partial" : "complete",
                    SR->TaskCount, R->TaskCount);
            }
...

{
    ...
    if ((SR == NULL) || (Name == NULL)) return (FAILURE);
    ...
    if (SR->HostList != NULL) MUFree((char **)&SR->HostList);
...

{
    ...
    for (srindex = 0; srindex < MAX_MSRES; srindex++) {
        ...
        if ((SR->TaskCount == 0) && (SR->HostExpression[0] == '\0') && !VFlag) {
            continue;
        }

        if ((PIndex == -1) || (PIndex == pSRTaskCount))
            strcat(Buffer,
                   MUShowIArray(MParam[pSRTaskCount], srindex, SR->TaskCount));

        if ((SR->TPN > 0) || (VFlag || (PIndex == -1) || (PIndex == pSRTPN)))
            strcat(Buffer, MUShowIArray(MParam[pSRTPN], srindex, SR->TPN));

        if ((SR->DRes.Procs != -1) ||
            (VFlag || (PIndex == -1) || (PIndex == pSRResources))) {
            sprintf(tmpLine, "PROCS=%d;MEM=%d;DISK=%d;SWAP=%d", SR->DRes.Procs,
                    SR->DRes.Mem, SR->DRes.Disk, SR->DRes.Swap);

            strcat(Buffer,
                   MUShowSArray(MParam[pSRResources], srindex, tmpLine));
        }

        if ((SR->Depth != DEFAULT_SRDEPTH) ||
            (VFlag || (PIndex == -1) || (PIndex == pSRDepth)))
            strcat(Buffer, MUShowIArray(MParam[pSRDepth], srindex, SR->Depth));

        if ((SR->StartTime != -1) ||
            (VFlag || (PIndex == -1) || (PIndex == pSRStartTime)))
            strcat(Buffer, MUShowSArray(MParam[pSRStartTime], srindex,
                                        MULToTString(SR->StartTime)));

        if ((SR->EndTime != -1) ||
            (VFlag || (PIndex == -1) || (PIndex == pSREndTime)))
            strcat(Buffer, MUShowSArray(MParam[pSREndTime], srindex,
                                        MULToTString(SR->EndTime)));

        if ((SR->WStartTime != -1) ||
            (VFlag || (PIndex == -1) || (PIndex == pSRWStartTime)))
            strcat(Buffer, MUShowSArray(MParam[pSRWStartTime], srindex,
                                        MULToTString(SR->WStartTime)));

        if ((SR->WEndTime != -1) ||
            (VFlag || (PIndex == -1) || (PIndex == pSRWEndTime)))
            strcat(Buffer, MUShowSArray(MParam[pSRWEndTime], srindex,
                                        MULToTString(SR->WEndTime)));

        if ((SR->Days != 255) ||
            (VFlag || (PIndex == -1) || (PIndex == pSRDays))) {
            if (SR->Days == 255) {
                strcpy(tmpLine, "ALL");
            } else {
                int index;
        ...
        if ((SR->HostExpression[0] != '\0') ||
            ...
            strcat(Buffer, MUShowSArray(MParam[pSRHostList], srindex,
                                        SR->HostExpression));
        }

        if ((SR->A != NULL) ||
            (VFlag || (PIndex == -1) || (PIndex == pSRChargeAccount)))
            strcat(Buffer, MUShowSArray(MParam[pSRChargeAccount], srindex,
                                        (SR->A != NULL) ? SR->A->Name : ""));
...

{
    ...
    DBG(3, fSTRUCT)
    DPrint("%s(%s,%d,Start,Duration)\n", FName, SR->Name, PeriodIndex);
    ...
    switch (SR->Period) {
        case mpDay:
            ...
            if ((SR->WEndTime > 0) && (WeekDay == (SR->WEndTime / DAY_LEN))) {
    ...
    switch (SR->Period) {
        case mpDay:
            ...
            if ((SR->Period == mpWeek) &&
                ((SR->WStartTime / DAY_LEN) > WeekDay)) {
                /* ignore, week reservation has not yet started */

                DBG(6, fSTRUCT)
                DPrint("INFO:     week reservation %s starts after day %d\n",
                       SR->Name, WeekDay);
            ...
            if ((SR->WEndTime > 0) && (SR->WEndTime < WeekTime)) {
                ...
                DBG(6, fSTRUCT)
                DPrint("INFO:     week reservation %s ends before %s\n",
                       SR->Name, MULToTString(WeekTime));
            ...
            if (DayTime > SREndOffset) {
                ...
                DBG(6, fSTRUCT)
                DPrint("INFO:     day reservation %s ends before %s\n",
                       SR->Name, MULToTString(DayTime));
            ...
            if (!(SR->Days & (1 << WeekDay))) {
                ...
                DBG(6, fSTRUCT)
                DPrint("INFO:     day reservation %s does not include day %d\n",
                       SR->Name, WeekDay);
        ...
        default:
            ...
            if (PeriodIndex != 0) {
                ...
                DBG(6, fSTRUCT)
                DPrint(
                    "INFO:     ignoring infinite reservation %s for period "
                    "%d\n",
                    SR->Name, PeriodIndex);
    ...
    DBG(5, fSTRUCT) DPrint("INFO:     res start: %s", MULToDString(&tmpL));

    DBG(5, fSTRUCT)
    DPrint(
        "INFO:     standing res attributes: Start: %ld  Duration: %ld (W: %d  "
        "D: %s)\n",
        PeriodStart + SRStartOffset, SREndOffset - SRStartOffset, WeekDay,
        MULToTString(DayTime));

    if (SRDuration != NULL) *SRDuration = SREndOffset - SRStartOffset;

    if (SRStartTime != NULL) *SRStartTime = PeriodStart + SRStartOffset;
...
int MSRGetCurrentValues(
    ...
    sres_t *Config, sres_t *Override, sres_t *SR)

{
    ...
    if (Override->TaskCount > 0) SR->TaskCount = Override->TaskCount;

    if (Override->A != NULL) SR->A = Override->A;

    if (Override->StartTime > 0) SR->StartTime = Override->StartTime;

    if (Override->EndTime > 0) SR->EndTime = Override->EndTime;
...

{
    ...
    DBG(3, fSCHED)
    DPrint("%s(%s,%d,%d)\n", FName, (SR != NULL) ? SR->Name : "NULL", SRIndex,
           DIndex);
    ...
    if (SR->Name[0] == '\0') {
        sprintf(J->Name, "SR.%d", SR->Index);
    } else {
        strcpy(J->Name, SR->Name);
    }

    sprintf(temp_str, ".%d", DIndex);
    ...
    if (SR->Flags & (1 << mrfByName)) J->SpecFlags |= (1 << mjfByName);

    if (!strcmp(SR->PName, "ALL")) J->SpecFlags |= (1 << mjfSpan);
                ...
                if (SR != &SRes[SRIndex]) {
                    if (SRes[SRIndex].HostList == NULL) {
                        ...
                        SRes[SRIndex].HostList = (void *)calloc(
                            1, sizeof(mnalloc_t) * (MAX_MNODE + 1));
                    }

                    memcpy(SRes[SRIndex].HostList, SR->HostList,
                           (sizeof(mnalloc_t) * (MAX_MNODE + 1)));
                }
            } /* END if (MSRBuildHostList(SR) == SUCCESS) */
            else {
                DBG(3, fSCHED)
                DPrint("ALERT:    cannot build hostlist for SR %s in %s\n",
                       SR->Name, FName);
    ...
    if ((HL != NULL) && (HL[0].N != NULL)) {
        ...
        if (SR->TaskCount == 0) J->SpecFlags |= (1 << mjfBestEffort);
        ...
        for (nindex = 0; HL[nindex].N != NULL; nindex++) {
            ...
            TC = MNodeGetTC(N, &N->CRes, &N->CRes, &N->DRes, &SR->DRes,
                            MAX_MTIME);
            ...
            DBG(5, fUI)
            DPrint(
                "INFO:     evaluating node %sx%d of hostlist for reservation "
                "%s (%d)\n",
                N->Name, TC, SR->Name, TaskCount);
        } /* END for (nindex) */
    }     /* END if ((HL != NULL) && (HL[0].N != NULL)) */

    if (SR->TaskCount > 0) {
        if (SR->TaskCount != TaskCount) Mode |= (1 << stBestEffort);
    ...
    if (MJobSetCreds(J, ALL, ALL, ALL) == FAILURE) {
        DBG(3, fUI)
        DPrint("INFO:     cannot setup standing reservation job creds\n");
    ...
    if (MSRGetAttributes(SR, DIndex, &SRStartTime, &SRDuration) == FAILURE) {
        DBG(3, fUI)
        DPrint("INFO:     reservation not required for specified period\n");
    ...
    if (RQ->TaskCount * RQ->DRes.Procs == 0) {
        DBG(2, fSCHED)
        DPrint("ALERT:    no procs requested by reservation (%d:%d)\n",
    ...
    DBG(3, fSCHED)
    DPrint(
        "INFO:     attempting standing reservation of %d procs in %s for %s\n",
        PC, MULToTString(SRStartTime - MSched.Time), TempString);
    ...
    if (MSRSelectNodeList(J, SR, NodeList, &NodeCount, BestTime, tmpANodeList,
                          Mode) == FAILURE) {
        DBG(1, fSCHED)
        DPrint(
            "ALERT:    cannot select %d procs in partition '%s' for SR '%s'\n",
            PC, (SR->PName[0] != '\0') ? SR->PName : ALL, SR->Name);
    ...
    if (MResCreate(mrtUser, SR->ACL, (SR->A != NULL) ? SR->A->Name : NULL,
                   Flags, (mnalloc_t *)NodeList, BestTime, (long)SREndTime,
                   NodeCount, ProcCount, J->Name, &SRes[SRIndex].R[DIndex],
                   SR->HostExpression, &RQ->DRes) == FAILURE) {
        DBG(1, fSCHED)
        DPrint(
            "ALERT:    cannot reserve %d procs in partition '%s' for standing "
            "reservation\n",
        ...
        if (IsSpaceFlex == TRUE) {
            if (tmpR.CIPS > 0.0) {
                if (MAMAllocRDebit(&MAM[0], &tmpR, &Reason, NULL) == FAILURE) {
                    DBG(1, fSTAT)
                    DPrint(
                        "ALERT:    cannot charge %6.2lf PS to account %s for "
                        "reservation %s\n",
                        tmpR.CIPS, (tmpR.A != NULL) ? tmpR.A->Name : NONE,
                        tmpR.Name);
                }
            } else {
                /* cancel account reservation */

                if (MAMAllocResCancel((tmpR.A != NULL) ? tmpR.A->Name : NULL,
                                      tmpR.Name, "standing reservation removed",
                                      NULL, &Reason) == FAILURE) {
                    DBG(1, fSCHED)
                    DPrint(
                        "ERROR:    cannot cancel allocation reservation for "
                        "reservation %s, reason: %s\n",
                        tmpR.Name, MDefReason[Reason]);
                }
            }
    ...
    if (PC == R->AllocPC) {
        DBG(2, fSCHED)
        DPrint(
            "INFO:     full SR reserved %d procs in partition '%s' to start in "
            "%s at (%ld) %s",
            PC, (SR->PName[0] != '\0') ? SR->PName : "[ALL]",
            MULToTString(BestTime - MSched.Time), BestTime,
            MULToDString(&BestTime));
    } else {
        DBG(2, fSCHED)
        DPrint(
            "WARNING:  partial standing reservation %s reserved %d of %d procs "
            "in partition '%s' to start in %s at (%ld) %s",
            SR->Name, R->AllocPC, PC,
...

{
    ...
    for (SRIndex = 0; SRIndex < MAX_MSRES; SRIndex++) {
        if (SRes[SRIndex].Name[0] == '\0') break;

        if ((SSR != NULL) && (SSR != &SRes[SRIndex])) continue;
        ...
        if (SR->R[0] != NULL) {
            if (SR->R[0]->EndTime > MSched.Time) continue;
        ...
        if (SR->HostExpression[0] != '\0') {
            if (MSRBuildHostList(SR) == SUCCESS) {
                ...
                DBG(4, fCORE)
                DPrint("ALERT:    cannot create hostlist for SR %s in %s\n",
                       SR->Name, FName);
            }
        } /* END if (SR->HostExpression[0] == '\0') */
        else {
            if (SR->TaskCount == 0) continue;
        }

        /* roll standing reservations forward */

        for (DIndex = 0; DIndex < (MAX_SRES_DEPTH - 1); DIndex++) {
            if (DIndex >= (SR->Depth - 1)) break;
            ...
            if (SR->R[DIndex] == NULL) {
                ...
                DBG(5, fSTRUCT)
                DPrint("INFO:     creating reservation '%s'\n", ResName);
        ...
        if (MSRSetRes(SR, SRIndex, DIndex) == FAILURE) {
            DBG(2, fCORE)
            DPrint("ALERT:    cannot create SR[%d] %s at depth %d\n", SRIndex,
                   SR->Name, DIndex);
        }
    } /* END for (SRIndex) */
...

{
    ...
    while (ptr != NULL) {
        ...
        if (MUREToList(ptr, mxoNode, P->Index, ObjList, &NodeCount, Buffer) ==
            FAILURE) {
            DBG(2, fCONFIG)
            DPrint("ALERT:    cannot expand hostlist '%s'\n", ptr);
...
int MSRProcessOConfig(
    ...
    sres_t *SR, int PIndex, int IVal, double DVal, char *SVal, char **SArray)

{
    if (SR == NULL) return (FAILURE);
    ...
    switch (PIndex) {
...

        {
            ...
            DBG(2, fALL)
            DPrint("INFO:     SRes[%s].Days set to %x\n", SR->Name, SR->Days);
        } /* END BLOCK */
...

        {
            ...
            for (index = 1; SArray[index] != NULL; index++) {
                ...
                MUStrCat(SR->HostExpression, SArray[index],
                         sizeof(SR->HostExpression));
            } /* END for (index) */
        }     /* END BLOCK */
...

        {
            ...
            MACLSet(SR->ACL, maDuration, &MaxTime, mcmpLE, nmPositiveAffinity,
                    (1 << maclRequired), 0);
        } /* END BLOCK */
...

        {
            ...
            DBG(2, fALL)
            DPrint(
                "INFO:     SRes[%s].DRes set to  P: %d  M: %d  D: %d  S: %d\n",
                SR->Name, SR->DRes.Procs, SR->DRes.Mem, SR->DRes.Disk,
                SR->DRes.Swap);
        }
    ...
    switch (AIndex) {
        ...
        case msraEndTime:
            ...
            if ((Mode & (1 << mcmVerbose)) || (OSRes[SR->Index].EndTime > 0)) {
                sprintf(Buf, "%ld", OSRes[SR->Index].EndTime);
            }
        ...
        case msraOwner:
            ...
            if (SR->OType != mxoNONE) {
                ...
                sprintf(Buf, "%s:%s", MXOC[SR->OType], ptr);
            }
        ...
        case msraStartTime:
            ...
            if ((Mode & (1 << mcmVerbose)) ||
                (OSRes[SR->Index].StartTime > 0)) {
                sprintf(Buf, "%ld", OSRes[SR->Index].StartTime);
            }
        ...
        case msraTaskCount:
            ...
            if ((Mode & (1 << mcmVerbose)) ||
                (OSRes[SR->Index].TaskCount > 0)) {
                sprintf(Buf, "%d", OSRes[SR->Index].TaskCount);
            }
...
{
    const int CPAList[] = {
        msraName,          msraStIdleTime, msraStTotalTime, msraTaskCount,
        msraChargeAccount, msraStartTime,  msraEndTime,     -1};

    mxml_t *E = NULL;

    if ((SR == NULL) || (Buf == NULL)) return (FAILURE);
...

{
    ...
    if ((SR == NULL) || (E == NULL)) return (FAILURE);
    ...
    switch (AIndex) {
...

        {
            if (Format == mdfString) {
                ...
                if ((ptr == NULL) ||
                    ((oindex = MUGetIndex(ptr, MXOC, FALSE, mxoNONE)) ==
                     mxoNONE)) {
                    /* invalid format */
