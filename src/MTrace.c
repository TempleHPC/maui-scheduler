...

{
    ...
    if ((tail = strchr(head, '\n')) == NULL) {
        ...
        if (tail - head < 20) {
            if (TraceLen != NULL) *TraceLen = 0;

            return (FAILURE);
        }
    }

    if (TraceLen != NULL) *TraceLen = (tail - head) + 1;
    ...
    if ((ptr = strchr(Line, '#')) != NULL) {
        DBG(5, fSIM)
        DPrint("INFO:     detected trace comment line, '%s'\n", Line);
    ...
    if (!strncmp(Line, TRACE_RESOURCE_VERSION_MARKER,
                 strlen(TRACE_RESOURCE_VERSION_MARKER))) {
        if (MTraceGetResourceVersion(Line, Version) == FAILURE) {
            DBG(0, fSIM)
            DPrint("ALERT:    cannot determine resource trace version\n");

            /* assume default version */

            if (Version != NULL) *Version = DEFAULT_RESOURCE_TRACE_VERSION;
        } else {
            DBG(3, fSIM)
            DPrint("INFO:     resource trace version %d detected\n",
                   (Version != NULL) ? *Version : -1);
        }
    ...
    DBG(5, fSIM)
    DPrint("INFO:     parsing line '%s' (len: %d)\n", Line, *TraceLen);
...

{
    ...
    switch (Version) {
        default:
            ...
            /* FORMAT:  <RESOURCETYPE> <EVENTTYPE> <EVENTTIME> <NAME> <RESTYPE>
             * <RESDATA> */

            rc = MUSScanF(NodeLine, "%x%s %x%s %ld %x%s %x%s %x%s", MAX_MNAME,
                          ResourceType, MAX_MNAME, EventType, &ETime, MAX_MNAME,
                          Name, MAX_MNAME, ResType, MAX_MNAME, ResData);
            ...
            for (rindex = 0; rindex < MAX_MNODE; rindex++) {
                ...
                sprintf(tmpLine, "NAME=%s;STATE=ACTIVE;%s", Name, ResData);
...

{
    ...
    DBG(3, fSIM)
    DPrint("%s(%s,%d,Buf,%d)\n", FName, (N != NULL) ? N->Name : "NULL", Version,
           BufSize);
    ...
    switch (Version) {
        case 230:
            ...
            /* FORMAT:  <RESOURCETYPE> <EVENTTYPE> <EVENTTIME> <NAME> <RMINDEX>
             * <SWAP> <MEMORY> <DISK> <MAXPROCS> <SLOTSUSED> <FRAMEINDEX>
             * <SLOTINDEX> <OPSYS> <ARCH> [<FEATURE>]* [<CLASS>]* [<NETWORK>]*
             * <RES1> <RES2> <RES3> <RES4> */

            MUSNPrintF(&BPtr, &BSpace,
                       "%s %s %d %s %s %dM %dM %dM %d %d %d %d %s %s %s %s %s "
                       "%s %s %s %s",
                       "COMPUTENODE", "AVAILABLE", 0, N->Name,
                       (N->RM != NULL) ? N->RM->Name : NONE, N->CRes.Swap,
                       N->CRes.Mem, N->CRes.Disk, N->CRes.Procs, N->FrameIndex,
                       N->SlotIndex,
                       (N->FrameIndex > 0)
                           ? MSys[N->FrameIndex][N->SlotIndex].SlotsUsed
                           : 1,
                       MAList[eOpsys][N->ActiveOS], MAList[eArch][N->Arch],
                       Features, Classes, Networks, NONE, NONE, NONE, NONE);
        ...
        default:
            ...
            DBG(4, fSIM)
            DPrint("ALERT:    cannot create version %d resource trace record\n",
                   Version);
...

{
    ...
    switch (Version) {
        case 230:
            ...
            if (strchr(NodeLine, ';') != NULL) {
                sprintf(tmpNLine, "IFS-;%.*s", MAX_MLINE, NodeLine);
            } else {
            ...
            rc = MUSScanF(tmpNLine,
                          "%x%s %x%s %ld %x%s %x%s %x%s %x%s %x%s %d %d %d %d "
                          "%x%s %x%s %x%s %x%s %x%s %x%s %x%s %x%s %x%s %x%s",
                          MAX_MNAME, ResourceType, MAX_MNAME, EventType, &ETime,
                          MAX_MNAME, Name, MAX_MNAME, RMName, MAX_MNAME,
                          tmpSwapBuf, MAX_MNAME, tmpMemBuf, MAX_MNAME,
                          tmpDiskBuf, &N->CRes.Procs, &FIndex, &SIndex,
                          &SlotsUsed, MAX_MNAME, OpsysString, MAX_MNAME,
                          ArchString, MAX_MLINE, FeatureString, MAX_MLINE,
                          ClassString, MAX_MLINE, NetworkString, MAX_MLINE,
                          GResString, MAX_MLINE, tmpLine, /* RES1 */
                          MAX_MLINE, tmpLine,             /* RES2 */
                          MAX_MLINE, tmpLine,             /* RES3 */
                          MAX_MLINE, tmpLine /* check for too many lines */
                          );

            if ((rc == EOF) || (rc != 21)) {
                DBG(3, fSIM)
                DPrint(
                    "ALERT:    resource tracefile corruption (%d of %d fields "
                    "detected) on line '%s'\n",
                    rc, 21, NodeLine);
        ...
        default:
            ...
            DBG(4, fSIM)
            DPrint("ALERT:    cannot load version %d resource trace record\n",
                   Version);
    ...
    if ((MSim.Flags & ((1 << msimfIgnFrame) | (1 << msimfIgnAll))) ||
        ...
        if ((N->FrameIndex > 0) && (N->FrameIndex < MAX_MFRAME) &&
            ...
            if (!strstr(ClassString, "NONE")) S->Attributes |= (1 << attrBatch);
    ...
    DBG(4, fSIM)
    DPrint(
        "INFO:     node '%s' F:%d/S:%d  %06d %04d %06d %02d %02d %10s %10s "
        "%20s %20s\n",
        N->Name, N->FrameIndex, N->SlotIndex, N->CRes.Swap, N->CRes.Mem,
        N->CRes.Disk, N->CRes.Procs, SlotsUsed, OpsysString, ArchString,
        FeatureString, MUCAListToString(N->CRes.PSlot, NULL, NULL));
...

{
    ...
    DBG(5, fSIM)
    DPrint("%s(Buffer,TraceLen,J,%d,%d)\n", FName, Mode,
           (Version != NULL) ? *Version : -1);

    if (J == NULL) {
        DBG(5, fSIM)
        DPrint("ALERT:    NULL job pointer passed in %s()\n", FName);
    ...
    if (!strncmp(Line, TRACE_WORKLOAD_VERSION_MARKER,
                 strlen(TRACE_WORKLOAD_VERSION_MARKER))) {
        if (MTraceGetWorkloadVersion(Line, Version) == FAILURE) {
            DBG(0, fSIM)
            DPrint("ALERT:    cannot determine workload trace version\n");

            /* assume default version */

            if (Version != NULL) *Version = DEFAULT_WORKLOAD_TRACE_VERSION;
        } else {
            DBG(3, fSIM)
            DPrint("INFO:     workload trace version %d detected\n",
                   (Version != NULL) ? *Version : -1);
        }
    ...
    switch (*Version) {
        case 230:
            ...
            /*                Name NR TReq User Grop WClk Stat Clss QUT DST STT
             * CMT netw Arch Opsy MemC RMem DskC RDsk Feat SQT TA TPN QO MO AC
             * CM CC BP PU Part DPrc DMem DDisk DSwap STARTDATE ENDDATE MNODE RM
             * HL RES RES1 RES2 RES3 OVERFLOW */

            rc = sscanf(
                Line,
                "%64s %d %64s %64s %64s %64s %64s %64s %lu %lu %lu %lu %64s "
                "%64s %64s %64s %64s %64s %64s %64s %lu %d %d %64s %1024s %64s "
                "%1024s %1024s %d %lf %64s %d %64s %64s %64s %lu %lu %1024s "
                "%64s %1024s %64s %64s %1024s %s %s",
                J->Name, &J->Request.NC, tmpTaskRequest, UName, GName,
                tmpSpecWCLimit, tmpState, tmpClass, &tmpQTime, &tmpDTime,
                &tmpSTime, &tmpCTime, tmpNetwork, tmpArch, tmpOpsys, tmpMemCmp,
                tmpRMemBuf, tmpDiskCmp, tmpRDiskBuf, tmpReqNFList,
                &tmpSQTime,      /* SystemQueueTime */
                &TasksAllocated, /* allocated task count */
                &TPN, tmpQReq, tmpJFlags, AName, tmpCmd,
                tmpRMXString,   /* RM extension string */
                &J->Bypass,     /* Bypass          */
                &J->PSUtilized, /* PSUtilized      */
                tmpParName,     /* partition used  */
                &RQ[0]->DRes.Procs, tmpDMemBuf, tmpDDiskBuf, tmpDSwapBuf,
                &tmpSDate, &tmpEDate, MHostName, tmpRMName, MReqHList,
                Reservation, SetString, /* resource sets required by job */
            ...
            switch (rc) {
                ...
                default:
                    ...
                    DBG(1, fSIM)
                    DPrint(
                        "ALERT:    version %d workload trace is corrupt '%s' "
                        "(%d of %d fields read) ignoring line\n",
                        *Version, Line, rc, 44);
            ...
            if (Mode != msmProfile) {
                ...
                if (MASGetDriver((void **)&J->ASFunc, ptr, msdApplication) ==
                    SUCCESS) {
...

            {
                ...
                while (ptr != NULL) {
                    if (!strncmp(ptr, "JATTR:", strlen("JATTR:"))) {
                        MJobSetAttr(J, mjaGAttr,
                                    (void **)(ptr + strlen("JATTR:")), 0, mSet);
                    }
            ...
            for (index = 0; index < M64.INTBITS; index++) {
                if (!(MSim.TraceIgnoreJobFlags & (1 << index))) continue;

                if (!(J->SpecFlags & (1 << index))) continue;
            ...
            if (Mode != msmProfile) {
                if ((strcmp(MReqHList, NONE) != 0) &&
                        ...
                        if (MNodeFind(ptr, &N) == FAILURE) {
                            if (MSched.DefaultDomain[0] != '\0') {
                                if ((tail = strchr(ptr, '.')) != NULL) {
                                    ...
                                    MUStrCpy(MReqHList, ptr,
                                             MIN((long)sizeof(MReqHList),
                                                 (tail - ptr + 1)));
                                } else {
                                    /* append default domain */

                                    if (MSched.DefaultDomain[0] == '.') {
                                        sprintf(MReqHList, "%s%s", ptr,
                                ...
                                if (MNodeFind(MReqHList, &N) != SUCCESS) {
                                    DBG(1, fUI)
                                    DPrint(
                                        "ALERT:    cannot locate node '%s' for "
                                        "job '%s' hostlist\n",
                                        ptr, J->Name);
                                ...
                                DBG(1, fUI)
                                DPrint(
                                    "ALERT:    cannot locate node '%s' for job "
                                    "'%s' hostlist\n",
                                    ptr, J->Name);
                    ...
                    DBG(1, fSIM)
                    DPrint(
                        "ALERT:    job %s requests hostlist but has no "
                        "hostlist specified\n",
                        J->Name);
        ...
        default:
            ...
            DBG(4, fSIM)
            DPrint("ALERT:    cannot load version %d workload trace record\n",
                   *Version);
    ...
    if (!strcmp(AName, NONE) || !strcmp(AName, "0")) {
        AName[0] = '\0';
    }
    ...
    if (RQ[0]->DRes.Procs == 0) RQ[0]->DRes.Procs = 1;

    if (J->SpecWCLimit[0] == -1) J->SpecWCLimit[0] = MAX_MTIME;
    ...
    if (Mode == msmSim) {
        ...
        J->PSDedicated =
            MJobGetProcCount(J) * (J->CompletionTime - J->StartTime);
    ...
    if (J->State == mjsNotRun) {
        DBG(3, fSIM)
        DPrint("ALERT:    ignoring job '%s' (job never ran, state '%s')\n",
    ...
    if (Mode == msmProfile) {
        ...
        if (MSched.TraceFlags & (1 << tfFixCorruption)) {
            if (J->StartTime < J->DispatchTime) {
                DBG(2, fSIM)
                DPrint(
                    "WARNING:  fixing job '%s' with corrupt dispatch/start "
                    "times (%ld < %ld)\n",
                    J->Name, J->DispatchTime, J->StartTime);
            ...
            if (J->SubmitTime > J->StartTime) {
                DBG(2, fSIM)
                DPrint(
                    "WARNING:  fixing job '%s' with corrupt queue/start times "
                    "(%ld > %ld)\n",
                    J->Name, J->SubmitTime, J->StartTime);
            ...
            if (J->SystemQueueTime > J->DispatchTime) {
                DBG(2, fSIM)
                DPrint(
                    "WARNING:  fixing job '%s' with corrupt system "
                    "queue/dispatch times (%ld > %ld)\n",
                    J->Name, J->SystemQueueTime, J->DispatchTime);
            ...
            if (J->SystemQueueTime < J->SubmitTime) {
                DBG(2, fSIM)
                DPrint(
                    "WARNING:  fixing job '%s' with corrupt system queue/queue "
                    "time (%ld < %ld)\n",
                    J->Name, J->SystemQueueTime, J->SubmitTime);
        ...
        if ((J->SubmitTime > J->DispatchTime) ||
            ...
            DBG(1, fSIM)
            DPrint(
                "ALERT:    ignoring job '%s' with corrupt "
                "queue/start/completion times (%ld > %ld > %ld)\n",
                J->Name, J->SubmitTime, J->DispatchTime, J->CompletionTime);
    ...
    if (MJobSetCreds(J, UName, GName, AName) == FAILURE) {
        DBG(1, fSTRUCT)
        DPrint(
            "ALERT:    ignoring job '%s' with invalid authentication info "
            "(%s:%s:%s)\n",
            J->Name, UName, GName, AName);
    ...
    if (strstr(tmpOpsys, MAList[eOpsys][0]) == NULL) {
        if ((RQ[0]->Opsys = MUMAGetIndex(eOpsys, tmpOpsys, mAdd)) == FAILURE) {
            DBG(0, fSIM)
            DPrint("WARNING:  cannot add opsys '%s' for job '%s'\n", tmpOpsys,
                   J->Name);
        }
    }
    ...
    if (strstr(tmpArch, MAList[eArch][0]) == NULL) {
        if ((RQ[0]->Arch = MUMAGetIndex(eArch, tmpArch, mAdd)) == FAILURE) {
            DBG(0, fSIM)
            DPrint("WARNING:  cannot add arch '%s' for job '%s'\n", tmpArch,
                   J->Name);
        }
    }
    ...
    if ((MSched.ProcSpeedFeatureHeader[0] != '\0') &&
        ...
        sprintf(tmpLine, "[%s", MSched.ProcSpeedFeatureHeader);

        if ((ptr = strstr(tmpReqNFList, tmpLine)) != NULL) {
            ReqProcSpeed = strtol(ptr + strlen(tmpLine), NULL, 0);

            J->SimWCTime *=
                (long)((double)ReqProcSpeed / MSched.ReferenceProcSpeed);
        }
    }
    ...
    if ((MSched.Mode != msmSim) || !(MSim.Flags & (1 << msimfIgnFeatures))) {
        if (!strstr(tmpReqNFList, MAList[eFeature][0])) {
            ...
            do {
                MUGetMAttr(eFeature, tok, mAdd, RQ[0]->ReqFBM,
                           sizeof(RQ[0]->ReqFBM));
            } while ((tok = MUStrTok(NULL, "[]", &TokPtr)) != NULL);
        }
    }
...

    {
        ...
        if ((ptr != NULL) && (ptr[0] != '\0') && strcmp(ptr, "-1") &&
            strcmp(ptr, "0") && strcmp(ptr, NONE) && strcmp(ptr, DEFAULT)) {
    ...
    if ((SetString[0] != '0') && (strcmp(SetString, NONE))) {
        ...
        if ((ptr = MUStrTok(SetString, ",: \t", &TokPtr)) != NULL) {
            ...
            RQ[0]->SetSelection = MUGetIndex(
                ptr, (const char **)MResSetSelectionType, 0, mrssNONE);

            if ((ptr = MUStrTok(NULL, ",: \t", &TokPtr)) != NULL) {
                /* determine set attribute */

                RQ[0]->SetType = MUGetIndex(ptr, (const char **)MResSetAttrType,
                                            0, mrstNONE);
    ...
    if (MJobEval(J) == FAILURE) {
        ...
        DBG(1, fSTRUCT)
        DPrint("ALERT:    ignoring job '%s' with corrupt configuration\n",
               J->Name);
    ...
    DBG(6, fSIM)
    DPrint("INFO:     job '%s' loaded.  class: %s  opsys: %s  arch: %s\n",
           J->Name, MUCAListToString(RQ[0]->DRes.PSlot, NULL, NULL),
           MAList[eOpsys][RQ[0]->Opsys], MAList[eArch][RQ[0]->Arch]);
