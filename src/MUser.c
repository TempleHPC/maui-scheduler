...

{
    ...
    sscanf(Buf, "%s %s %ld", tmpHeader, UName, &CkTime);
...

{
    ...
    for (uindex = hashkey; uindex < MAX_MUSER + MAX_MHBUF; uindex++) {
        ...
        if ((U != NULL) && (U->Key == hashkey) && !strcmp(U->Name, UName)) {
            if (UP != NULL) *UP = U;
        ...
        if ((U == NULL) || (U->Name[0] == '\0') || (U->Name[0] == '\1')) {
            ...
            if (U == NULL) {
                if (MUserCreate(UName, &MUser[uindex]) == FAILURE) {
                    DBG(1, fALL)
                    DPrint("ERROR:    cannot alloc memory for user '%s'\n",
                           UName);
    ...
    DBG(1, fSTRUCT)
    DPrint("ALERT:    user table overflow.  cannot add %s\n", UName);
...

{
    ...
    for (uindex = 0; uindex < MAX_MUSER + MAX_MHBUF; uindex++) {
        ...
        if (strcmp(U->Name, UName) != 0) continue;
...
int MUserInitialize(
    ...
    mgcred_t *U, char *UName)

{
    if ((U == NULL) || (UName == NULL) || (UName[0] == '\0')) {
        return (FAILURE);
    }
...

{
    ...
    DBG(3, fUI)
    DPrint("%s(%s,Buf,%ld,%ld)\n", FName, (U != NULL) ? U->Name : "NULL",
           (SBufSize != NULL) ? *SBufSize : -1, Mode);
    ...
    if (U == NULL) {
        ...
        MUStrCpy(ULLine, MCredShowAttrs(&U->L.AP, U->L.IP, NULL, NULL, NULL,
                                        &U->F, 0, (1 << mcsLimits)),
                 sizeof(ULLine));
        ...
        MUSNPrintF(
            ...
            U->Name, U->F.Priority, FlagLine,
            ((mqos_t *)U->F.QDef) != NULL ? ((mqos_t *)U->F.QDef)->Name : NONE,
            (QALLine[0] != '\0') ? QALLine : NONE, QALChar,
            (U->F.PAL[0] == 0) ? NONE : MUListAttrs(ePartition, U->F.PAL[0]),
            U->F.FSTarget, (ULLine[0] != '\0') ? ULLine : NONE);
...
{
    const int DAList[] = {mcaID, -1};

    const int DCList[] = {mxoStats, -1};
