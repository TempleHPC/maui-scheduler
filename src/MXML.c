...
extern int MSecDecompress(unsigned char *, unsigned int, unsigned char *,
...

{
    ...
    for (cindex = 0; cindex < Dst->CCount; cindex++) {
        ...
        for (aindex = 0; aindex < Src->ACount; aindex++) {
            MXMLAppendAttr(Dst, Src->AName[aindex], (char *)Src->AVal[aindex],
                           Delim);
        }
    } /* END for (cindex) */
...

{
    ...
    if (CName != NULL) (*CE)->Name = strdup(CName);
...

{
    ...
    if (MXMLGetAttr(E, AName, &ATok, VBuf, sizeof(VBuf)) == SUCCESS) {
        ...
        E->AVal[ATok] = (char *)realloc(E->AVal[ATok], len);
...

{
    ...
    while (MXMLToString(E, *Buf, NewSize, Tail, NoCompress) == FAILURE) {
...

{
    ...
    for (index = 0; index < E->CCount; index++) {
        ...
        if (MXMLToString(E->C[index], BPtr, BSpace, &tail, NoCompress) ==
            FAILURE) {
            return (FAILURE);
        }
    ...
    if ((NoCompress == FALSE) && (strlen(Buf) > (MMAX_BUFFER >> 1))) {
        if ((E->Name != NULL) && !strcmp(E->Name, "Data")) {
            ...
            if (MSecCompress((unsigned char *)Buf, strlen(Buf), NULL,
                             MCONST_CKEY) == FAILURE) {
                return (FAILURE);
            }
        }
...

{
    ...
    switch (DFormat) {
        case mdfString:
            ...
            rc = MXMLGetAttr(E, AName, ATok, (char *)AVal, VSize);
        ...
        case mdfDouble:
            ...
            if (MXMLGetAttr(E, AName, ATok, tmpLine, sizeof(tmpLine)) ==
                FAILURE) {
                /* clear response */
