...
int MJobCreate(
    ...
    mbool_t AddJob, mjob_t **JP) /* O: job pointer */

{
    ...
    for (index = 1; index < MSched.M[mxoJob]; index++) {
        ...
        if ((J == NULL) || (J == (mjob_t *)1)) {
            if ((J = (mjob_t *)calloc(1, sizeof(mjob_t))) == NULL) {
                DBG(1, fSTRUCT)
                DPrint(
                    "ALERT:    cannot allocate memory for job %s, errno: %d "
                    "(%s)\n",
                    (JName != NULL) ? JName : "NULL", errno, strerror(errno));
        ...
        if (J->Name[0] == '\0') {
            ...
            if ((J->NodeList = (mnalloc_t *)calloc(
                     1, sizeof(mnalloc_t) * (MAX_MNODE_PER_JOB + 1))) == NULL) {
                DBG(1, fSTRUCT)
                DPrint(
                    "ALERT:    cannot allocate memory for job %s nodelist, "
                    "errno: %d (%s)\n",
                    (JName != NULL) ? JName : "NULL", errno, strerror(errno));
            ...
            if (MJobAddHash(JName, index, &jindex) == SUCCESS) {
                *JP = J;
                ...
                DBG(6, fSTRUCT) {
                    DBG(6, fSTRUCT)
                    DPrint("INFO:     job slot %d allocated to job '%s'\n",
                           index, JName);
                    ...
                    if (MJobFind(JName, &Jtmp, 0) == SUCCESS) {
                        if (Jtmp->Index != index) {
                            DBG(1, fSTRUCT)
                            DPrint(
                                "ERROR:  job table corruption (job index %d != "
                                " hash index %d)\n",
                                Jtmp->Index, index);
                        }
                    }
                ...
                DBG(5, fSTRUCT)
                DPrint(
                    "WARNING:  unable to add hash table entry for job '%s'\n",
                    JName);
    ...
    DBG(1, fALL)
    DPrint("WARNING:  job buffer overflow (cannot add job '%s')\n",
...

{
    ...
    DBG(2, fSTRUCT)
    DPrint("%s(%s,SrcRQ,DstRQ,DoCreate)\n", FName,
           (J != NULL) ? J->Name : NULL);
    ...
    DBG(4, fSTRUCT)
    DPrint("INFO:     adding requirement at slot %d\n", rqindex);

    if ((RQ = (mreq_t *)calloc(1, sizeof(mreq_t))) == NULL) {
        DBG(1, fSIM)
        DPrint(
            "ALERT:    cannot allocate memory for requirement %s:%d, errno: %d "
            "(%s)\n",
            J->Name, rqindex, errno, strerror(errno));

        return (FAILURE);
    }

    if (SrcRQ != NULL) memcpy(RQ, SrcRQ, sizeof(mreq_t));
    ...
    if (RQ->NodeList == NULL) {
        if ((RQ->NodeList = (mnalloc_t *)calloc(
            ...
            DBG(1, fSIM)
            DPrint(
                "ALERT:    cannot allocate memory for hostlist %s:%d, errno: "
                "%d (%s)\n",
                J->Name, rqindex, errno, strerror(errno));
...

{
    ...
    DBG(6, fSTRUCT)
    DPrint("%s(%s,DstJ)\n", FName, (SrcJ != NULL) ? SrcJ->Name : "NULL");
...
int MJobSetCreds(
    ...
    mjob_t *J, char *UName, char *GName, char *AName)

{
    const char *FName = "MJobSetCreds";

    DBG(3, fSTRUCT)
    DPrint("%s(%s,%s,%s,%s)\n", FName, J->Name, UName, GName,
    ...
    if (MUserAdd(UName, &J->Cred.U) == FAILURE) {
        DBG(1, fSTRUCT)
        DPrint("ERROR:    user hash table overflow on job '%s' with user %s\n",
               J->Name, UName);
    ...
    if (MGroupAdd(GName, &J->Cred.G) == FAILURE) {
        DBG(1, fSTRUCT)
        DPrint(
            "ERROR:    group hash table overflow on job '%s' with group %s\n",
            J->Name, GName);
    ...
    if ((AName == NULL) || (AName[0] == '\0')) {
        if (MJobGetAccount(J, &J->Cred.A) == FAILURE) {
            ...
            if (MAM[0].Type != mamtNONE) {
                DBG(1, fSTRUCT)
                DPrint(
                    "ERROR:    cannot determine default account for job '%s', "
                    "user '%s'\n",
                    J->Name, J->Cred.U->Name);

                sprintf(Message,
                        "AMFAILURE:  cannot determine default account for job "
                        "'%s', user '%s'\n",
                        J->Name, J->Cred.U->Name);
                ...
                if (MAM[0].DeferJobOnFailure == TRUE) {
                    MJobSetHold(J, (1 << mhDefer), MSched.DeferTime, mhrNoFunds,
        ...
        DBG(1, fSTRUCT)
        DPrint("ERROR:    account table overflow on job '%s' with account %s\n",
               J->Name, AName);
...

{
    ...
    *A = NULL;
    ...
    if ((J->Cred.U != NULL) && (J->Cred.U->F.ADef != NULL)) {
        *A = J->Cred.U->F.ADef;

        return (SUCCESS);
    }

    if ((J->Cred.G != NULL) && (J->Cred.G->F.ADef != NULL)) {
        *A = J->Cred.G->F.ADef;

        return (SUCCESS);
    }

    if ((J->Cred.Q != NULL) && (J->Cred.Q->F.ADef != NULL)) {
        *A = J->Cred.Q->F.ADef;
    ...
    if (MAM[0].Type != mamtNONE) {
        ...
        if (MAMAccountGetDefault(J->Cred.U->Name, tmpAccount, &Reason) ==
            FAILURE) {
            return (FAILURE);
        }
    ...
    if ((MSched.DefaultU != NULL) && (MSched.DefaultU->F.ADef != NULL)) {
        *A = MSched.DefaultU->F.ADef;

        return (SUCCESS);
    }

    if ((MSched.DefaultG != NULL) && (MSched.DefaultG->F.ADef != NULL)) {
...

{
    ...
    if (SpecJobName != NULL) {
        ...
        if (Buffer != NULL) strcpy(Buffer, "NULL");
    ...
    if (R->DefaultQMDomain[0] == '\0') {
        ...
        switch (RMType) {
...

            {
                ...
                if ((ptr = strchr(tmpName, '.')) == NULL) break;

                Head = ptr + 1;

                /* remove job/step id */

                if ((ptr = strrchr(Head, '.')) == NULL) break;
        ...
        if (R->DefaultQMDomain[0] == '\0') {
            ...
            if (MSched.DefaultDomain[0] != '\0') {
                MUStrCpy(R->DefaultQMDomain, &MSched.DefaultDomain[1],
                         sizeof(R->DefaultQMDomain));
            }
        ...
        switch (RMType) {
            case mrmtPBS:
                ...
                if ((ptr = strchr(BaseJName, '.')) != NULL)
                    ...
                    MUStrCpy(R->DefaultQMHost, MSched.ServerHost,
                             sizeof(R->DefaultQMHost));
            ...
            switch (RMType) {
...

                {
                    ...
                    if ((Host == NULL) || (JobID == NULL) || (StepID == NULL)) {
                        ...
                        sprintf(Head, "%s.%s.%s", Host, JobID, StepID);
                    }
                } /* END BLOCK */
                ...
                case mrmtPBS:
                    ...
                    if ((ptr = strchr(BaseJName, '.')) != NULL)
                        MUStrCpy(Head, BaseJName,
                                 MIN(ptr - BaseJName + 1, Len));
                    else
                        MUStrCpy(Head, BaseJName, Len);
...

{
    ...
    if (J->SIData != NULL) MSDataDestroy(&J->SIData);

    if (J->SOData != NULL) MSDataDestroy(&J->SOData);

    if (J->Ckpt != NULL) MJobCPDestroy(J, &J->Ckpt);
...
int MJobGetBackfillPriority(
    ...
    mjob_t *J, time_t MaxDuration, int PIndex, double *BFPriority, char *Buffer)
...

{
    ...
    J->Flags = (J->SpecFlags | J->SysFlags |
    ...
    DBG(7, fSTRUCT)
    DPrint("INFO:     job flags for job %s: %x, req napolicy=%s\n", J->Name,
           (int)J->Flags,
    ...
    if (J->Flags & (1 << mjfPreemptee))
        MJobSetAttr(J, mjaGAttr, (void **)MJobFlags[mjfPreemptee], mdfString,
                    mSet);
    else
        MJobSetAttr(J, mjaGAttr, (void **)MJobFlags[mjfPreemptee], mdfString,
                    mUnset);
...

{
    ...
    DBG(5, fSCHED)
    DPrint("%s(%s,%d,%s,%s)\n", FName, (J != NULL) ? J->Name : "NULL",
           (RQ != NULL) ? RQ->Index : -1, (N != NULL) ? N->Name : "NULL",
    ...
    if (J != NULL) {
        ...
        if (J->ExcHList != NULL) {
            for (index = 0; J->ExcHList[index].N != NULL; index++) {
                if (N == J->ExcHList[index].N) {
                    DBG(5, fSCHED)
                    DPrint("INFO:     node in excluded hostlist\n");

                    if (RIndex != NULL) *RIndex = marHostList;
        ...
        /*        ie, node missing from hostlist does NOT imply node not
         * feasible */
        ...
        if ((J->Flags & (1 << mjfHostList)) && (J->ReqHList != NULL) &&
            ...
            for (index = 0; J->ReqHList[index].N != NULL; index++) {
                if (N == J->ReqHList[index].N) {
                    DBG(5, fSCHED)
                    DPrint("INFO:     node in requested hostlist\n");
            ...
            if ((Found == FALSE) && ((J->ReqHLMode != mhlmSubset) ||
                                     (MPar[0].EnableMultiNodeJobs == FALSE))) {
                DBG(5, fSCHED)
                DPrint("INFO:     node is not in specified hostlist\n");
    ...
    /* check OS, node features, configured procs, configured memory, configured
     * classes ... */
    ...
    if (RQ->DRes.Procs != 0) {
        ...
        if ((RQ->Opsys != 0) && (N->ActiveOS != RQ->Opsys)) {
            DBG(4, fSCHED)
            DPrint(
                "INFO:     opsys does not match ('%s' needed  '%s' "
                "available)\n",
                MAList[eOpsys][RQ->Opsys], MAList[eOpsys][N->ActiveOS]);

            if (RIndex != NULL) *RIndex = marOpsys;
        ...
        if ((RQ->Arch != 0) && (N->Arch != RQ->Arch)) {
            DBG(5, fSCHED)
            DPrint(
                "INFO:     architectures do not match (%s needed  %s found)\n",
                MAList[eArch][RQ->Arch], MAList[eArch][N->Arch]);

            if (RIndex != NULL) *RIndex = marArch;
    ...
    if (MAttrSubset(N->FBM, RQ->ReqFBM, sizeof(N->FBM), RQ->ReqFMode) !=
        SUCCESS) {
        MUStrCpy(tmpLine, MUMAList(eFeature, N->FBM, sizeof(N->FBM)),
                 sizeof(tmpLine));

        DBG(5, fSCHED)
        DPrint("INFO:     inadequate features (%s needed  %s found)\n",
               MUMAList(eFeature, RQ->ReqFBM, sizeof(RQ->ReqFBM)), tmpLine);

        if (RIndex != NULL) *RIndex = marFeatures;
    ...
    if (!MUCompare(N->CRes.Procs, RQ->ProcCmp, RQ->RequiredProcs)) {
        DBG(5, fSCHED)
        DPrint("INFO:     inadequate procs (%s %d needed  %d found)\n",
               MComp[RQ->ProcCmp], RQ->RequiredProcs, N->CRes.Procs);

        if (RIndex != NULL) *RIndex = marCPU;
    ...
    if (!MUCompare(N->CRes.Mem, RQ->MemCmp, RQ->RequiredMemory)) {
        DBG(5, fSCHED)
        DPrint("INFO:     inadequate memory (%s %d needed  %d found)\n",
               MComp[RQ->MemCmp], RQ->RequiredMemory, N->CRes.Mem);

        if (RIndex != NULL) *RIndex = marMemory;

        return (FAILURE);
    }

    if (MUNumListGetCount((J != NULL) ? J->StartPriority : 0, RQ->DRes.PSlot,
                          N->CRes.PSlot, 0, NULL) == FAILURE) {
        DBG(5, fSCHED)
        DPrint(
            "INFO:     node is missing classes [ALL %d:%d]%s "
            "required:configured\n",
            RQ->DRes.PSlot[0].count, N->CRes.PSlot[0].count,
            MUCAListToString(RQ->DRes.PSlot, N->CRes.PSlot, NULL));

        if (RIndex != NULL) *RIndex = marClass;
    ...
    if ((TC == 0) || (TC < RQ->TasksPerNode)) {
        DBG(5, fSCHED)
        DPrint("INFO:     node supports %d task%c (%d tasks/node required)\n",
               TC, (TC == 1) ? ' ' : 's', RQ->TasksPerNode);
    ...
    if (RQ->Pool > 0) {
        ...
        if (Found == FALSE) {
            DBG(3, fSCHED)
            DPrint("INFO:     pool not found ('%d' needed)\n", RQ->Pool - 1);

            if (RIndex != NULL) *RIndex = marPool;
    ...
    if ((N->RM != NULL) && (N->RM->Type == mrmtLL)) {
        ...
        if (RQ->NodeCount > 0) {
            ...
            if (RQ->TaskCount % RQ->NodeCount) MinTPN++;
        }
        ...
        if ((RQ->BlockingFactor != 1) &&
            ((TC < (MinTPN - 1)) || (TC == 0) ||
             ((TC < MinTPN) && (RQ->NodeCount > 0) &&
              (RQ->TaskCount % RQ->NodeCount == 0)))) {
            DBG(5, fSCHED)
            DPrint(
                "INFO:     node supports %d task%c (%d tasks/node "
                "required:LL)\n",
                TC, (TC == 1) ? ' ' : 's', MinTPN);
    ...
    DBG(5, fSCHED)
    DPrint("INFO:     node %s can provide resources for job %s:%d\n", N->Name,
           (J != NULL) ? J->Name : "NULL", RQ->Index);
...
{
    if (J->C.TotalProcCount == 0) MJobUpdateResourceCache(J, 0);
    ...
    if ((J->State == mjsStarting) || (J->State == mjsRunning)) {
        for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
            ...
            if ((RQ->NAccessPolicy == mnacSingleJob) ||
                ...
                if ((RQ->NodeList == NULL) || (RQ->NodeList[0].N == NULL)) {
                    ...
                    /* HACK:  assumes all nodes have similar processor
                     * configurations */
        ...
        for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
            ...
            if ((RQ->NAccessPolicy == mnacSingleJob) ||
                ...
                /* NOTE:  assume homogeneous resources with MNode[0] indicative
                 * of procs/node */
                ...
                if ((RQ->NAccessPolicy == mnacSingleTask) ||
                    ...
                    proccount +=
                        TC * ((MNode[0] != NULL) ? MNode[0]->CRes.Procs : 1);
                } else {
                    /* assume packing */
...

{
    ...
    if ((J == NULL) || (J == (mjob_t *)1)) {
        DBG(0, fSTRUCT)
        DPrint("ERROR:    invalid job pointer passed to %s()\n", FName);

        return (SUCCESS);
    }

    if ((J->Name[0] == '\0') || (J->Name[0] == '\1')) {
    ...
    if (J->R != NULL) {
        if (MResDestroy(&J->R) == FAILURE) {
            DBG(0, fSTRUCT)
            DPrint("ERROR:    unable to destroy job reservation\n");
        }
    }

    if ((J->Flags & (1 << mjfAdvReservation)) && (J->ResName[0] != '\0')) {
        mres_t *R;
...

{
    ...
    if ((Mode != stForce) && (J->State != mjsStarting) &&
        (J->State != mjsRunning)) {
        return (SUCCESS);
    }
    ...
    DBG(6, fSTAT) {
        ...
        for (jindex = 0; MAQ[jindex] != -1; jindex++) {
            fprintf(mlog.logfp, "[%d : %s]", MAQ[jindex],
...

{
    ...
    if ((J->State != mjsStarting) && (J->State != mjsRunning)) {
        DBG(4, fSCHED)
        DPrint(
            "WARNING:  cannot preempt non-active job '%s' (state: '%s' estate: "
            "'%s')\n",
            J->Name, MJobState[J->State], MJobState[J->EState]);

        if (Msg != NULL) sprintf(Msg, "cannot preempt non-active job");
    ...
    switch (PPolicy) {
        ...
        case mppCheckpoint:
            ...
            /* NOTE:  checkpoint can either ckpt and resume or ckpt and
             * terminate */
        ...
        case mppSuspend:
            ...
            /* NOTE:  to prevent thrashing, by default, suspended jobs should
             * not *
    ...
    for (nindex = 0; J->NodeList[nindex].N != NULL; nindex++) {
        ...
        if (MSched.Mode == msmSim) N->State = N->EState;
    } /* END for (nindex) */

    /* release allocation manager reservation */

    if (J->Cred.A != NULL) {
        if (MAMAllocResCancel(J->Cred.A->Name, J->Name, "job suspended", NULL,
                              (enum MHoldReasonEnum *)SC) == FAILURE) {
            DBG(1, fRM)
            DPrint("ERROR:    cannot cancel account reservation for job '%s'\n",
                   J->Name);
        }
    }
...

{
    ...
    DBG(5, fSTRUCT)
    DPrint("%s('%s',J,%d)\n", FName,
           (SpecJobName != NULL) ? SpecJobName : "NULL", Mode);
    ...
    MJobGetName(NULL, SpecJobName, &MRM[0], JobName, sizeof(JobName),
                mjnShortName);
    ...
    if (Mode == 1) {
        ...
        if ((ptr = strchr(JobName, '.')) != NULL) {
            MUStrCpy(QJobID, JobName,
                     MIN((long)sizeof(QJobID), ptr - JobName + 1));
    ...
    for (index = hashkey; index < MSched.M[mxoJob] + MAX_MHBUF; index++) {
        ...
        if (!strcmp(MJobName[index].Name, QJobID)) {
            ...
            if ((tmpJ == NULL) ||
                ((tmpJ->Name[0] != '\0') && (tmpJ->Name[0] != '\1') &&
                 strcmp(tmpJ->Name, QJobID))) {
                /* hash table is corrupt */

                DBG(7, fSTRUCT)
                DPrint(
                    "WARNING:  job '%s' corruption found at hash[%d] %d '%s' "
                    "(J->Name: %s)\n",
                    QJobID, index, MJobName[index].Index, MJobName[index].Name,
            ...
            DBG(6, fSTRUCT)
            DPrint(
                "INFO:     job '%s' found at hash[%d] %d '%s' (J->Name: %s)\n",
                QJobID, index, MJobName[index].Index, MJobName[index].Name,
                (tmpJ->Name[0] != '\0') ? tmpJ->Name : "[EMPTY]");
...

{
    ...
    if (J->ResName[0] != '\0')
        MACLSet(J->RCL, maRes, J->ResName, mcmpSEQ, nmPositiveAffinity,
                (1 << maclRequired), 0);
    ...
    if ((J->Cred.Q != NULL) && (J->Cred.Q->Name[0] != '\0') &&
        (strcmp(J->Cred.Q->Name, DEFAULT))) {
    ...
    if (J->Cred.C != NULL) {
        ...
        for (cindex = 1; cindex < MAX_MCLASS; cindex++) {
            if (J->Req[0]->DRes.PSlot[cindex].count > 0) {
                J->Cred.CL[aindex].Type = maClass;
                MUStrCpy(J->Cred.CL[aindex].Name, MAList[eClass][cindex],
                         MAX_MNAME);
                J->Cred.CL[aindex].Cmp = mcmpSEQ;
                J->Cred.CL[aindex].Affinity = nmPositiveAffinity;
    ...
    for (cindex = 0; cindex < (8 * (int)sizeof(cindex) - 1); cindex++) {
        if (J->AttrBM & (1 << cindex)) {
            ...
            MUStrCpy(J->Cred.CL[aindex].Name, (char *)MAList[eJFeature][cindex],
                     MAX_MNAME);
            J->Cred.CL[aindex].Cmp = mcmpSEQ;
            J->Cred.CL[aindex].Affinity = nmPositiveAffinity;
...
int MJobCheckLimits(
    ...
    mjob_t *J, /* I */
    int PLevel, mpar_t *P, int Mode, char *Message)

{
    int JUsage[MAX_MPOLICY];

    const char *MType[] = {"active", "idle", "system", NULL};

    int OList1[] = {mxoNONE, mxoUser, -1};
    int OList2[] = {mxoUser, mxoGroup, mxoAcct, mxoClass, mxoQOS, mxoPar, -1};
    int PList[] = {mptMaxJob, mptMaxProc, mptMaxNode, mptMaxPS,
                   mptMaxPE,  mptMaxWC,   -1};
    ...
    DBG(7, fSTRUCT)
    DPrint("%s(%s,%s,P,%d,Message)\n", FName, (J != NULL) ? J->Name : "NULL",
           MPolicyMode[PLevel], Mode);
    ...
    for (mindex = 0; MList[mindex] != -1; mindex++) {
        if (!(Mode & (1 << MList[mindex]))) continue;
        ...
        for (oindex1 = 0; OList1[oindex1] != -1; oindex1++) {
            for (oindex2 = 0; OList2[oindex2] != -1; oindex2++) {
                if (((OList2[oindex2] == mxoPar) &&
                     (MList[mindex] != mlSystem)) ||
                    ((OList2[oindex2] != mxoPar) &&
                     (MList[mindex] == mlSystem))) {
                    continue;
                }

                if ((OList1[oindex1] != mxoNONE) &&
                    (OList2[oindex2] != mxoClass)) {
                ...
                for (pindex = 0; PList[pindex] != -1; pindex++) {
                    if (MPolicyCheckLimit(JUsage[PList[pindex]], PList[pindex],
                                          PLevel, 0, /* NOTE:  change to
                                                        P->Index for per
                                                        partition limits */
                                          OP,
                                          DP, QP, &tmpI) == FAILURE) {
                        if (Message != NULL) {
                            sprintf(Message,
                                    "job %s violates %s %s %s limit of %d for "
                                    "%s %s %s (R: %d, U: %d)\n",
                                    J->Name, MType[mindex], MPolicyMode[PLevel],
                                    MPolicyType[PList[pindex]], tmpI,
                                    MXO[OList2[oindex2]],
                                    (OID != NULL) ? OID : NONE,
                        ...
                        DBG(3, fSCHED)
                        DPrint(
                            "job %s violates %s %s %s limit of %d for %s %s %s "
                            "(Req: %d, InUse: %d)\n",
                            J->Name, MType[mindex], MPolicyMode[PLevel],
                            MPolicyType[PList[pindex]], tmpI,
                            MXO[OList2[oindex2]], (OID != NULL) ? OID : NONE,
                            (oindex1 != mxoNONE) ? MXO[OList1[oindex1]] : "",
                            JUsage[PList[pindex]], OP->Usage[PList[pindex]][0]);

                        if (PLevel == ptHARD && JUsage[PList[pindex]] > tmpI) {
                            if (J->LastNotifyTime + MSched.ActionInterval <
                                MSched.Time) {
                                MSysRegEvent(Message, 0, 0, 1);
...

{
    ...
    if (MSched.Mode == msmSim) {
        DBG(0, fSCHED)
        DPrint("%s(%s,%d,%s,%s,%s)\n", FName, (J != NULL) ? J->Name : "NULL",
               HoldTypeBM, MULToTString(HoldDuration), MDefReason[HoldReason],
    ...
    if ((J->ResName[0] != '\0') && (HoldReason == mhrNoResources)) {
        DBG(2, fSCHED)
        DPrint("INFO:     job %s requests reservation %s.  (not deferring)\n",
               J->Name, J->ResName);
...

    {
        ...
        if ((RM->FailIteration == MSched.Iteration) &&
            ...
            DBG(2, fSCHED)
            DPrint(
                "INFO:     connection to RM %s failed.  Not deferring job %s "
                "for reason %s\n",
                RM->Name, J->Name, MDefReason[HoldReason]);
    ...
    /* workaround to avoid defer state for GRES "gpu": when they are all
     * utilized
    ...
    if (HoldReason == mhrNoResources) {
        ...
        if ((rIndex = MUMAGetIndex(eGRes, "gpu", mVerify)) != 0) {
            ...
            if (J->Req[0]->DRes.GRes[rIndex].count > 0) {
                DBG(2, fSCHED)
                DPrint("INFO:     defer disabled (hack for GPUs)\n");
    ...
    if (J->Hold & (1 << mhBatch)) {
        DBG(2, fSCHED)
        DPrint("INFO:     job '%s' already on batch hold\n", J->Name);

        return (SUCCESS);
    }

    DBG(2, fSCHED)
    DPrint("ALERT:    job '%s' cannot run (deferring job for %ld seconds)\n",
           J->Name, HoldDuration);
    ...
    if ((MSched.Mode == msmSim) && (HoldDuration != MSched.RMPollInterval) &&
        (getenv(MSCHED_ENVAMTESTVAR) == NULL)) {
        DBG(0, fSCHED)
        DPrint(
            "INFO:     job '%s' removed on iteration %d (cannot run in "
            "simulated environment), reason: %s\n",
            J->Name, MSched.Iteration, MDefReason[HoldReason]);
    ...
    if ((MSched.DeferCount > 0) && (J->DeferCount > MSched.DeferCount)) {
        DBG(1, fSCHED)
        DPrint("INFO:     batch hold placed on job '%s', reason: '%s'\n",
               J->Name, MDefReason[HoldReason]);

        J->Hold |= (1 << mhBatch);

        sprintf(Line,
                "JOBHOLD:  batch hold placed on job '%s'.  defercount: %d  "
                "reason: '%s'",
                J->Name, J->DeferCount, MDefReason[HoldReason]);

        MSysRegEvent(Line, 0, 0, 1);
    } else {
        if ((HoldReason == mhrRMReject) || (HoldReason == mhrAPIFailure)) {
            sprintf(Line,
                    "JOBDEFER:  defer hold placed on job '%s'.   reason: '%s'",
                    J->Name, MDefReason[HoldReason]);
    ...
    while (key != NULL) {
        ...
        switch (aindex) {
            ...
            case mxaDDisk:
                ...
                if (Value[0] != '0') RQ->DRes.Disk = MAX(RQ->DRes.Disk, 1);

                DBG(5, fCONFIG)
                DPrint("INFO:     per task dedicated disk set to %d MB\n",
                       RQ->DRes.Disk);
            ...
            case mxaDMem:
                ...
                DBG(5, fCONFIG)
                DPrint("INFO:     per task dedicated memory set to %d MB\n",
                       RQ->DRes.Mem);
            ...
            case mxaFlags:
                ...
                while (ptr != NULL) {
                    if ((findex = MUGetIndex(ptr, MJobFlags, 1, mjfNONE)) ==
                        mjfNONE) {
                    ...
                    DBG(5, fCONFIG)
                    DPrint("INFO:     flag %s set for job %s\n",
                           MJobFlags[findex], J->Name);
                    ...
                    switch (findex) {
                        case mjfAdvReservation:
                            ...
                            if (Value[0] == ':') {
                                sscanf((Value + 1), "%s", J->ResName);

                                DBG(5, fCONFIG)
                                DPrint("INFO:     reservation %s selected\n",
                                       J->ResName);
                            }
            ...
            case mxaHostList:
                ...
                if (MSched.Mode == msmProfile) break;
                ...
                if (J->ReqHList == NULL)
                    J->ReqHList = (mnalloc_t *)calloc(
                        1, sizeof(mnalloc_t) * (MAX_MNODE_PER_JOB + 1));
                ...
                while (ptr != NULL) {
                    if (MNodeFind(ptr, &N) == FAILURE) {
                        DBG(1, fUI)
                        DPrint(
                            "ALERT:    cannot locate node '%s' for job '%s' "
                            "hostlist\n",
                            ptr, J->Name);
                    ...
                    if (J->ReqHList[nindex].N == N) {
                        ...
                        if (J->ReqHList[nindex].N != NULL) nindex++;
            ...
            case mxaMasterFeature:
                ...
                if (J->MReq == NULL) {
                    ...
                    memcpy(RQ->DRes.PSlot, J->Req[0]->DRes.PSlot,
                           sizeof(RQ->DRes.PSlot));
                ...
                while (ptr != NULL) {
                    MUGetMAttr(eFeature, ptr, mAdd, RQ->ReqFBM,
                               sizeof(RQ->ReqFBM));
            ...
            case mxaNAccessPolicy:
                ...
                DBG(5, fCONFIG)
                DPrint("INFO:     job node access policy set to %s\n",
            ...
            case mxaNAllocPolicy:
                ...
                if ((RQ->NAllocPolicy == NULL) &&
                    ((RQ->NAllocPolicy = (mnallocp_t *)calloc(
                          1, sizeof(mnallocp_t))) == NULL)) {
            ...
            case mxaNodeSet:
                ...
                if ((ptr = MUStrTok(Value, ":", &TokPtr2)) != NULL) {
                    RQ->SetSelection =
                        MUGetIndex(ptr, MResSetSelectionType, 0, mrssNONE);

                    if ((ptr = MUStrTok(NULL, ":", &TokPtr2)) != NULL) {
                        RQ->SetType =
                            MUGetIndex(ptr, MResSetAttrType, 0, mrstNONE);
            ...
            case mxaPMask:
                ...
                if (strstr(tmpLine, MAList[ePartition][0]) == NULL) {
                    ...
                    if (J->SpecPAL[0] == 0) {
                        DBG(1, fUI)
                        DPrint(
                            "ALERT:    NULL pmask set for job '%s'.  (job "
                            "cannot run)\n",
                            J->Name);
                    }
                    ...
                    DBG(3, fCONFIG) {
                        MUStrCpy(tmpLine,
                                 MUListAttrs(ePartition,
                                             MUMAFromString(ePartition, tmpLine,
                                                            mVerify)),
                                 sizeof(tmpLine));

                        DPrint(
                            "INFO:     pmask '%s' specified.  resulting pmask: "
                            "'%s'\n",
                            Value, MUListAttrs(ePartition, J->SpecPAL[0]));
                    }
                } /* END if (strstr() == NULL) */
            ...
            case mxaQueueJob:
                ...
                if (MUBoolFromString(Value, TRUE) == FALSE) {
                    ...
                    DBG(4, fUI)
                    DPrint("INFO:     NoQueue flag set for job %s\n", J->Name);
                }
            ...
            case mxaQOS:
                ...
                if (MQOSFind(Value, &J->QReq) == SUCCESS) {
                    DBG(3, fCONFIG)
                    DPrint("INFO:     QOS '%s' requested by job %s\n",
                           J->QReq->Name, J->Name);
                } else {
                    DBG(3, fCONFIG)
                    DPrint("WARNING:  job %s cannot request QOS '%s'\n",
                           J->Name, Value);
            ...
            case mxaSGE:
                ...
                tmpRQPtr->DRes.GRes[index].count =
                    (unsigned short)strtol(Value, &tail, 10);
            ...
            case mxaSID:
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     system ID '%s' set for job %s\n", J->SystemID,
                       J->Name);
            ...
            case mxaSJID:
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     system JID '%s' set for job %s\n",
                       J->SystemJID, J->Name);
            ...
            case mxaTPN:
                ...
                DBG(5, fCONFIG)
                DPrint("INFO:     tasks per node set to %d\n",
                       RQ->TasksPerNode);
            ...
            case mxaTRL:
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     task request list loaded (%d,%d,...)\n",
                       RQ->TaskRequestList[1], RQ->TaskRequestList[2]);
            ...
            */
                ...
                while (ptr != NULL) {
                    ...
                    DBG(3, fCONFIG)
                    DPrint("INFO:     GRES requested = %s@%d\n", gresName,
                           gresCount);

                    if ((rIndex = MUMAGetIndex(eGRes, gresName, mVerify)) ==
                        0) {
                        DBG(1, fPBS)
                        DPrint("ALERT:  Unknown GRES '%s'\n", gresName);
                    } else if (gresCount <= 0) {
                        DBG(1, fPBS)
                        DPrint("ALERT:  Invalid GRES count %d\n", gresCount);
                    } else {
...

{
    ...
    for (index = hashkey; index < MSched.M[mxoJob] + MAX_MHBUF; index++) {
        ...
        if (MJobName[index].Name[0] == '\0') {
            ...
            if (MJobName[index].Index != 0) {
                ...
                MUStrCpy(MJobName[*KIndex].Name, JobID,
                         sizeof(MJobName[*KIndex]));
                MJobName[*KIndex].Index = JobIndex;

                DBG(8, fSTRUCT)
                DPrint("INFO:     job hash for job '%s' (index: %d) created\n",
                       JobID, JobIndex);
    ...
    DBG(1, fSTRUCT)
    DPrint("ERROR:  job hash table is FULL.  cannot add MJob[%03d] '%s'\n",
           JobIndex, JobID);
...

{
    ...
    for (index = hashkey; index < MSched.M[mxoJob] + MAX_MHBUF; index++) {
        ...
        DBG(6, fSTRUCT)
        DPrint(
            "INFO:     searching for hash entry at MJobName[%03d] '%s' "
            "(JobName: '%s')\n",
            index, MJobName[index].Name, JobName);
...

{
    ...
    for (sindex = 1; RQ->TaskRequestList[sindex] > 0; sindex++) {
        ...
        if (MJobGetEStartTime(J, &BestP, &NodeCount, NULL, MNodeList,
                              &BestTime) == FAILURE) {
            /* job cannot be run on any partition with shape sindex */

            sprintf(Message, "cannot create reservation for job '%s'", J->Name);

            if (InitialCompletionTime > 0) {
                DBG(1, fSCHED)
                DPrint(
                    "ALERT:    cannot create reservation (job '%s' previously "
                    "reserved to start in %s)\n",
                    J->Name, IStringTime);

                sprintf(Message,
                        "%s (job previously reserved to start in %s)\n",
                        Message, IStringTime);
            } else {
                DBG(1, fSCHED)
                DPrint(
                    "ALERT:    cannot create new reservation for job %s "
                    "(shape[%d] %d)\n",
                    J->Name, sindex, RQ->TaskRequestList[sindex]);
    ...
    if (BestSTime == MAX_MTIME) {
        ...
        if (InitialCompletionTime > 0) {
            ...
            DBG(1, fSCHED)
            DPrint("ALERT:    cannot create new reservation for job %s\n",
                   J->Name);
        ...
        if (MSched.Mode != msmTest) {
            MJobSetHold(J, (1 << mhDefer), MSched.DeferTime, mhrNoResources,
                        Message);
        }
    ...
    if (J->Req[1] == NULL) {
        ...
        MJobDistributeTasks(J, &MRM[RQ->RMIndex], tmpNodeList, tmpTaskList);
        ...
        if (rc == FAILURE) {
            DBG(1, fSCHED)
            DPrint("ALERT:    cannot create reservation in %s\n", FName);
    ...
    DBG(2, fSCHED)
    DPrint(
        "INFO:     job '%s' reserved %d tasks (partition %s) to start in %s on "
        "%s (WC: %ld)\n",
        J->Name, J->Request.TC, BestP->Name,
        MULToTString(BestTime - MSched.Time), MULToDString(&BestTime),
    ...
    if ((InitialCompletionTime > 0) &&
        ...
        DBG(1, fSCHED)
        DPrint(
            "ALERT:    reservation completion for job '%s' delayed from %s to "
            "%s\n",
            J->Name, IStringTime, MULToTString(J->R->EndTime - MSched.Time));

        sprintf(Message,
                "reservation completion for job '%s' delayed from %s to %s\n",
                J->Name, IStringTime,
...

{
    ...
    DBG(7, fSCHED)
    DPrint("%s(%s,%s,Value,%d,%d)\n", FName, (J != NULL) ? J->Name : "NULL",
           MJobAttr[AIndex], Format, Mode);
    ...
    switch (AIndex) {
        case mjaAccount:
            ...
            if (Value != NULL) {
                if (MAcctAdd((char *)Value, &J->Cred.A) == FAILURE) {
                    DBG(1, fSCHED)
                    DPrint(
                        "ERROR:    cannot add account for job %s (Name: %s)\n",
                        J->Name, (char *)Value);
        ...
        case mjaEEWDuration:
            ...
            if (Format == mdfString) {
                sscanf((char *)Value, "%ld", &J->EffQueueDuration);
            } else {
...

        {
            ...
            if ((index = MUMAGetBM(eJFeature, ptr, mAdd)) > 0) {
                if (Mode == mSet) {
                    ...
                    DBG(3, fSCHED)
                    DPrint("INFO:     attribute '%s' set for job %s\n", ptr,
                           J->Name);
                } else {
                    /* unset */

                    if (J->AttrBM & index) J->AttrBM ^= index;

                    DBG(8, fSCHED)
                    DPrint("INFO:     attribute '%s' cleared for job %s\n", ptr,
                           J->Name);
                }
            }
        ...
        case mjaGroup:
            ...
            if ((Value != NULL) && (((char *)Value)[0] != '\0')) {
                if (MGroupAdd((char *)Value, &J->Cred.G) == FAILURE) {
                    DBG(1, fSCHED)
                    DPrint(
                        "ERROR:    cannot add group for job %s (Name: '%s')\n",
                        J->Name, (char *)Value);
...

        {
            ...
            if (Format == mdfString) {
                sscanf((char *)Value, "%d", &tmpI);
            } else {
                tmpI = *(int *)Value;
            }

            if (tmpI == (1 << mhAll)) {
                tmpI = (1 << mhBatch) | (1 << mhUser) | (1 << mhSystem) |
                       (1 << mhDefer);
            }
        ...
        case mjaStartCount:
            ...
            if (Format == mdfString) {
                sscanf((char *)Value, "%d", &J->StartCount);
            } else {
...

        {
            ...
            if (cindex != 0) MJobSetState(J, cindex);
        } /* END BLOCK */
        ...
        case mjaStatMSUtl:
            ...
            if (Format == mdfString) {
                sscanf((char *)Value, "%lf", &J->MSUtilized);
            } else {
        ...
        case mjaStatPSDed:
            ...
            if (Format == mdfString) {
                sscanf((char *)Value, "%lf", &J->PSDedicated);
            } else {
        ...
        case mjaStatPSUtl:
            ...
            if (Format == mdfString) {
                sscanf((char *)Value, "%lf", &J->PSUtilized);
            } else {
        ...
        case mjaSysPrio:
            ...
            if (Format == mdfString) {
                sscanf((char *)Value, "%ld", &J->SystemPrio);
            } else {
...

        {
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     system min start time set on job %s for %s\n",
                   J->Name, MULToTString(J->SysSMinTime - MSched.Time));
        }
        ...
        case mjaUser:
            ...
            if (Value != NULL) {
                if (MUserAdd((char *)Value, &J->Cred.U) == FAILURE) {
                    DBG(1, fSCHED)
                    DPrint("ERROR:    cannot add user for job %s (Name: %s)\n",
                           J->Name, (char *)Value);
...

{
    ...
    switch (AIndex) {
        ...
        case mjaEEWDuration:
            ...
            sprintf(Buf, "%ld", J->EffQueueDuration);
        ...
        case mjaJobName:
            ...
            if (J->AName != NULL) strcpy(Buf, J->AName);
        ...
        case mjaQOS:
            ...
            if (J->Cred.Q != NULL) strcpy(Buf, J->Cred.Q->Name);

            break;

        case mjaQOSReq:

            if (J->QReq != NULL) strcpy(Buf, J->QReq->Name);
        ...
        case mjaStartCount:
            ...
            sprintf(Buf, "%d", J->StartCount);

            break;

        case mjaStatMSUtl:

            sprintf(Buf, "%.3lf", J->MSUtilized);

            break;

        case mjaStatPSDed:

            sprintf(Buf, "%.3lf", J->PSDedicated);

            break;

        case mjaStatPSUtl:

            sprintf(Buf, "%.3lf", J->PSUtilized);

            break;

        case mjaSuspendDuration:

            sprintf(Buf, "%ld", J->SWallTime);

            break;

        case mjaSysPrio:

            sprintf(Buf, "%ld", J->SystemPrio);
...

{
    ...
    DBG(7, fSCHED)
    DPrint("%s(%s,RQ,%s,Value,%d,%d)\n", FName, (J != NULL) ? J->Name : "NULL",
           MReqAttr[AIndex], Format, Mode);
    ...
    switch (AIndex) {
        case mrqaAllocNodeList:
            ...
            if (Format == mdfString) {
                ...
                for (nindex = 0; nindex < MAX_MNODE_PER_JOB; nindex++) {
                    if ((NL[nindex].N == NULL) || (NL[nindex].TC == 0)) break;

                    memcpy(&RQ->NodeList[nindex], &NL[nindex],
                           sizeof(mnalloc_t));
                } /* END for (nindex) */
        ...
        case mrqaReqArch:
            ...
            if ((RQ->Arch = MUMAGetIndex(eArch, (char *)Value, mAdd)) ==
                FAILURE) {
                return (FAILURE);
            }
        ...
        case mrqaReqNodeFeature:
            ...
            if (Value != NULL) {
                MUGetMAttr(eFeature, (char *)Value, Mode, RQ->ReqFBM,
                           sizeof(RQ->ReqFBM));
            }
        ...
        case mrqaReqOpsys:
            ...
            if ((RQ->Opsys = MUMAGetIndex(eOpsys, (char *)Value, mAdd)) ==
                FAILURE) {
                return (FAILURE);
            }
...

{
    ...
    if (D->TransferRate != 0)
        ...
        TransferRate =
            (long)(D->DstFileSize / MAX(1, MSched.Time - D->TStartTime));
    else
        TransferRate = MDEF_TRANSFERRATE;

    TimeRemaining = DataRemaining / MAX(1, TransferRate);

    if (ETime != NULL) *ETime = MSched.Time + TimeRemaining;
...

{
    ...
    if (MJobDistributeTasks(J, &MRM[RQ->RMIndex], J->NodeList, J->TaskMap) ==
        FAILURE) {
        DBG(3, fSCHED)
        DPrint("WARNING:  cannot distribute allocated tasks for job '%s'\n",
               J->Name);
    ...
    if (MAMAllocJReserve(&MAM[0], J, FALSE, &Reason, ErrMsg) == FAILURE) {
        DBG(3, fSCHED)
        DPrint("WARNING:  cannot reserve allocation for job '%s', reason: %s\n",
               J->Name, MDefReason[Reason]);

        if ((Reason == mhrNoFunds) && (MAM[0].FallbackAccount[0] != '\0') &&
            ...
            if ((MQOSGetAccess(J, J->QReq, NULL, &QDef) == FAILURE) ||
                ...
                if (J->QReq != NULL) {
                    /* defer job one iteration to allow it to be properly
                     * re-prioritized */

                    sprintf(ErrMsg, "job account adjusted to fallback account");

                    MJobSetHold(J, (1 << mhDefer), MSched.RMPollInterval,
                                Reason, ErrMsg);
                }
            }
            ...
            if ((Reason != mhrAMFailure) || (MAM[0].JFAction != mamjfaNONE)) {
                ...
                if (MAM[0].JFAction == mamjfaCancel) {
                    ...
                    MJobSetHold(J, (1 << mhDefer), MSched.DeferTime, Reason,
                                ErrMsg);
                }
    ...
    if (MRMJobStart(J, ErrMsg, &SC) == FAILURE) {
        DBG(3, fSCHED)
        DPrint("WARNING:  cannot start job '%s' through resource manager\n",
               J->Name);
        ...
        if ((SC == mscRemoteFailure) &&
            ...
            DBG(2, fSCHED)
            DPrint(
                "ALERT:    job '%s' deferred after %d failed start attempts "
                "(API failure on last attempt)\n",
                J->Name, J->StartCount);

            if (MSched.Mode != msmTest) {
                MJobSetHold(J, (1 << mhDefer), MSched.DeferTime, mhrAPIFailure,
                            ErrMsg);
            }
        }

        if (J->Cred.A != NULL) {
            if (MAMAllocResCancel(J->Cred.A->Name, J->Name, "job cannot start",
                                  NULL, &Reason) == FAILURE) {
                DBG(1, fSCHED)
                DPrint(
                    "ERROR:    cannot cancel allocation reservation for job "
                    "'%s', reason: %s\n",
                    J->Name, MDefReason[Reason]);
            }
        }
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        MUNLGetMinAVal(RQ->NodeList, mnaProcSpeed, NULL,
                       (void **)&MinProcSpeed);
        ...
        for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
            ...
            if (MinProcSpeed > 0) {
                ...
                if (MinProcSpeed != N->ProcSpeed) JobIsHetero = TRUE;

                NodeLoad *=
                    (double)MinProcSpeed / MAX(MinProcSpeed, N->ProcSpeed);
            }
            ...
            if ((N->State != mnsIdle) && (N->State != mnsActive)) {
                DBG(0, fSCHED)
                DPrint(
                    "ERROR:    job %s (TC: %d) started on node MNode[%03d] "
                    "'%s' which is in state '%s' on iteration %d\n",
                    J->Name, J->Request.TC, N->Index, N->Name,
                    MAList[eNodeState][N->State], MSched.Iteration);
            }
    ...
    if (JobIsHetero == TRUE) MStat.TotalHeterogeneousJobs++;
    ...
    if (MResJCreate(J, NULL, MSched.Time, mjrActiveJob, NULL) == FAILURE) {
        DBG(0, fSCHED)
        DPrint("ERROR:    cannot create reservation for job '%s'\n", J->Name);
    ...
    if (J->ASFunc != NULL) {
        ...
        sprintf(tmpLine, "STATE=%s", MJobState[J->State]);
...

{
    ...
    if (RQ == NULL) {
        DBG(2, fSTRUCT)
        DPrint("ALERT:    job %s contains no req info\n", J->Name);
    ...
    if (J->CompletionTime == 0) {
        ...
        sprintf(tmp, "%08ld", J->SimWCTime);
    }

    DPrint(
        "[%03d] %16.16s %3d:%3d:%3d(%d) %3.3s %10s(%8s) %8s %8s %10s %s "
        "%10.10s %10ld %8s %6s %6s %2s %6d %2s %6d ",
        J->Index, J->Name, J->Request.TC, MJobGetProcCount(J), J->Request.NC,
        RQ->TasksPerNode, MAList[ePartition][RQ->PtIndex],
        MULToTString(J->WCLimit), tmp,
        (J->Cred.U != NULL) ? J->Cred.U->Name : "NULL",
        (J->Cred.G != NULL) ? J->Cred.G->Name : "NULL", MJobState[J->State],
        (J->Cred.Q != NULL) ? J->Cred.Q->Name : "NULL",
        MUCAListToString(RQ->DRes.PSlot, NULL, NULL), J->SubmitTime,
        MAList[eNetwork][RQ->Network], MAList[eOpsys][RQ->Opsys],
        MAList[eArch][RQ->Arch], MComp[RQ->MemCmp], RQ->RequiredMemory,
...

{
    ...
    for (rqindex = 0; rqindex < MAX_MREQ_PER_JOB; rqindex++) {
        if (J->Req[rqindex] == NULL) continue;

        for (nindex = 0; J->Req[rqindex]->NodeList[nindex].N != NULL;
             nindex++) {
            if (J->Req[rqindex]->NodeList[nindex].TC == 0) break;
...
int MReqGetPref(
    ...
    mreq_t *RQ, mnode_t *N, char *IsPref)

{
    ...
    if (MAttrSubset(N->FBM, RQ->PrefFBM, sizeof(N->FBM), tlOR) == SUCCESS) {
        *IsPref = (char)TRUE;
    } else {
        *IsPref = (char)FALSE;
    }
...
{
    int DAList[] = {mjaAWDuration,     mjaEEWDuration, /* eligible job time */
                    mjaHold,           mjaQOSReq,
                    mjaReqReservation, mjaStartCount,
                    mjaStatMSUtl,      mjaStatPSDed,
                    mjaStatPSUtl,      mjaSysPrio,
                    mjaSysSMinTime,    -1};
...
{
    const int CPAList[] = {mjaAWDuration,
                           mjaEEWDuration,
                           mjaHold,
                           mjaMessages,
                           mjaPAL,
...

{
    ...
    sscanf(Buf, "%s %s %ld", tmpName, JobName, &CkTime);
    ...
    if (JS == NULL) {
        if (MJobFind(JobName, &J, 0) != SUCCESS) {
            DBG(5, fCKPT)
            DPrint("INFO:     job '%s' no longer detected\n", JobName);
    ...
    if (JS != NULL) {
        ...
        if ((JS->Cred.Q != JS->QReq) && (JS->QReq != NULL)) {
            ...
            if (MQOSGetAccess(JS, JS->QReq, NULL, &QDef) == FAILURE) {
                DBG(2, fSTRUCT)
                DPrint(
                    "INFO:     job %s does not have access to checkpointed "
                    "QOSRequest %s (setting to default %s)\n",
                    JS->Name, JS->QReq->Name, QDef->Name);
            } else {
...

{
    ...
    if ((RQ->NodeCount > 0) && (RQ->TaskCount > 0) && (RQ->TasksPerNode > 1)) {
        if (RQ->TasksPerNode * RQ->NodeCount != RQ->TaskCount) {
            DBG(2, fSTRUCT)
            DPrint(
                "INFO:     job '%s' has invalid task layout (TPN:%d * N:%d != "
                "T:%d)\n",
                J->Name, RQ->TasksPerNode, RQ->NodeCount, RQ->TaskCount);
...

{
    ...
    DBG(4, fSCHED)
    DPrint("%s(%s,%s,NodeCount,TaskCount,MNodeList,%ld)\n", FName,
           (J != NULL) ? J->Name : "NULL",
           ((PP != NULL) && (*PP != NULL)) ? (*PP)->Name : "NULL", *EStartTime);
    ...
    NAPolicy = (RQ->NAllocPolicy != NULL) ? RQ->NAllocPolicy->NAllocPolicy
    ...
    if (!(J->Flags & (1 << mjfSpan))) {
        for (pindex = 1; pindex < MAX_MPAR; pindex++) {
            ...
            if ((PS != NULL) && (PS != P)) continue;

            if (P->ConfigNodes == 0) continue;
            ...
            if (MUBMCheck(P->Index, J->PAL) == FAILURE) {
                DBG(7, fSCHED)
                DPrint(
                    "INFO:     job %s not allowed to run in partition %s "
                    "(allowed %s)\n",
                    J->Name, P->Name, MUListAttrs(ePartition, J->PAL[0]));
            ...
            for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                ...
                TC = MParGetTC(P, &P->CRes, &P->CRes, &DRes, &RQ->DRes,
                               MAX_MTIME);

                if (TC < J->Request.TC) {
                    DBG(7, fSCHED)
                    DPrint(
                        "INFO:     tasks not available for job %s in partition "
                        "%s (%d of %d tasks)\n",
                        J->Name, P->Name, TC, J->Request.TC);
                ...
                if (MUNumListGetCount(J->StartPriority, RQ->DRes.PSlot,
                                      P->CRes.PSlot, 0, NULL) == FAILURE) {
                    DBG(7, fSCHED)
                    DPrint(
                        "INFO:     pslots not available for job %s in "
                        "partition %s (%s)\n",
                        J->Name, P->Name,
                ...
                if (MJobGetRange(J, RQ, P, JobStartTime, GRange, NULL,
                                 &tmpNodeCount, NodeMap, Type,
                                 TRange) == FAILURE) {
            ...
            if (JobStartTime < BestStartTime) {
                ...
                if (TRange[0].TaskCount >= J->Request.TC)
                    memcpy(
                        BestRange, TRange, sizeof(BestRange)); /* FIXME:  only
                                                                  works for
                                                                  single req
                                                                  jobs */
                else
                ...
                DBG(5, fSCHED)
                DPrint(
                    "INFO:     start time %s found for job %s in partition %s "
                    "(%ld)\n",
                    MULToTString(BestStartTime - MSched.Time), J->Name,
                    EP->Name, *EStartTime);
            }
        } /* END for (pindex) */
        ...
        if (EP != NULL) {
            for (rindex = 0; BestRange[rindex].EndTime != 0; rindex++) {
                ...
                for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                    if (MJobGetRange(
                            J, J->Req[rqindex], EP, BestRange[rindex].StartTime,
                            GRange, tmpMNodeList, &tmpNodeCount, NodeMap,
                            (1 << rtcStartEarliest), NULL) == FAILURE) {
                        /* cannot obtain range nodelist */

                        DBG(2, fSCHED)
                        DPrint(
                            "ALERT:    cannot obtain nodelist for job %s in "
                            "range %d\n",
                            J->Name, rindex);
                    ...
                    for (nindex = 0; tmpMNodeList[0][nindex].N != NULL;
                         nindex++) {
                ...
                DBG(2, fSCHED)
                DPrint(
                    "INFO:     located resources for %d tasks (%d) in best "
                    "partition %s for job %s at time %s\n",
                    J->Request.TC, BestTaskCount, EP->Name, J->Name,
                ...
                if ((J->Flags & (1 << mjfAllocLocal)) ||
                    ...
                    if (MJobProximateMNL(J, tmpMNodeList, tmpMNodeList,
                                         MAX_MTIME, FALSE) == FAILURE) {
                        DBG(4, fSCHED)
                        DPrint(
                            "INFO:     inadequate proximity within available "
                            "tasks\n");
                ...
                if (MPar[0].NodeSetDelay > 0) {
                    ...
                    if (MJobSelectResourceSet(
                            J, RQ, (RQ->SetType != mrstNONE)
                                       ? RQ->SetType
                                       : MPar[0].NodeSetAttribute,
                            (tmpRSS != mrssOneOf) ? tmpRSS : mrssFirstOf,
                            (RQ->SetList[0] != NULL) ? RQ->SetList
                                                     : MPar[0].NodeSetList,
                            NodeList, BestNodeCount) == FAILURE) {
                        DBG(0, fSCHED)
                        DPrint(
                            "WARNING:  nodeset constraints prevent use of "
                            "tasks for job %s at %s\n",
                            J->Name, MULToTString(BestRange[rindex].StartTime -
                                                  MSched.Time));
                ...
                if (MJobAllocMNL(J, tmpMNodeList, NodeMap, MNodeList, NAPolicy,
                                 BestRange[rindex].StartTime) == FAILURE) {
                    DBG(0, fSCHED)
                    DPrint("WARNING:  cannot allocate tasks for job %s at %s\n",
                           J->Name, MULToTString(BestRange[rindex].StartTime -
                                                 MSched.Time));
            ...
            if (BestRange[rindex].EndTime == 0) {
                DBG(0, fSCHED)
                DPrint(
                    "ERROR:    cannot allocate tasks for job %s at any time\n",
                    J->Name);
            ...
            if (NodeCount != NULL) *NodeCount = nindex;

            if (TaskCount != NULL) *TaskCount = tmpTaskCount;
            ...
            *EStartTime = BestStartTime;
            ...
            DBG(2, fSCHED)
            DPrint("ALERT:    job %s cannot run in any partition\n", J->Name);
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        if (TC < J->Request.TC) {
            DBG(7, fSCHED)
            DPrint(
                "INFO:     resources not available for job %s in partition %s "
                "(%d of %d tasks)\n",
                J->Name, P->Name, TC, J->Request.TC);
        ...
        if (MUNumListGetCount(J->StartPriority, RQ->DRes.PSlot, P->CRes.PSlot,
                              0, NULL) == FAILURE) {
            DBG(7, fSCHED)
            DPrint(
                "INFO:     classes not available for job %s in partition %s "
                "(%s)\n",
                J->Name, P->Name,
                MUCAListToString(RQ->DRes.PSlot, P->CRes.PSlot, NULL));
        ...
        if (MJobGetRange(J, RQ, P, JobStartTime, GRange, NULL, &tmpNodeCount,
                         NodeMap, Type, NULL) == FAILURE) {
            /* acceptable range could not be located */

            DBG(2, fSCHED)
            DPrint("ALERT:    resources unavailable for spanning job %s:%d\n",
                   J->Name, rqindex);
    ...
    if (J->Req[rqindex] != NULL) {
        ...
        DBG(2, fSCHED)
        DPrint("ALERT:    resources unavailable for spanning job %s\n",
               J->Name);
    ...
    for (rindex = 0; BestRange[rindex].EndTime != 0; rindex++) {
        ...
        for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
            ...
            if (MJobGetRange(J, RQ, EP, BestRange[rindex].StartTime, GRange,
                             tmpMNodeList, &tmpNodeCount, NodeMap,
                             (1 << rtcStartEarliest), NULL) == FAILURE) {
                /* cannot obtain range nodelist */

                DBG(2, fSCHED)
                DPrint(
                    "ERROR:    cannot obtain nodelist for spanning job %s in "
                    "range %d\n",
                    J->Name, rindex);
        ...
        DBG(2, fSCHED)
        DPrint(
            "INFO:     located resources for %d tasks for spanning job %s at "
            "time %s in range %d\n",
            J->Request.TC, J->Name,
            MULToTString(BestRange[rindex].StartTime - MSched.Time), rindex);

        if (MJobAllocMNL(J, tmpMNodeList, NodeMap, MNodeList, NAPolicy,
                         BestRange[rindex].StartTime) == FAILURE) {
            DBG(0, fSCHED)
            DPrint("ERROR:    cannot allocate tasks for job %s in range %d\n",
                   J->Name, rindex);
    ...
    if (BestRange[rindex].EndTime == 0) {
        ...
        DBG(0, fSCHED)
        DPrint("ERROR:    cannot allocate tasks for job %s in any range\n",
               J->Name);

        return (FAILURE);
    }

    if ((BestStartTime >= MAX_MTIME) &&
        (BestRange[0].StartTime < BestStartTime)) {
...

{
    ...
    if (MSysLaunchAction(ArgV, mactJobFB) == SUCCESS) {
        DBG(2, fCORE)
        DPrint("INFO:     job usage sent for job '%s'\n", J->Name);
    }
...

{
    ...
    if (MJobToTString(J, DEFAULT_WORKLOAD_TRACE_VERSION, Buf, sizeof(Buf)) ==
        SUCCESS) {
...

{
    ...
    DBG(1, fSCHED)
    DPrint("%s(%s,%s,%s,ResCountRej)\n", FName, (J != NULL) ? J->Name : "NULL",
           MAList[ePartition][PIndex],
           (ResCount != NULL) ? "ResCount" : "NULL");

    if (ResCountRej != NULL) *ResCountRej = FALSE;
    ...
    if ((J->Cred.Q != NULL) && (J->Cred.Q->Flags & (1 << mqfnoreservation))) {
        DBG(1, fSCHED)
        DPrint("INFO:    reservation not allowed for job %s in %s (QOS)\n",
               J->Name, FName);
    ...
    if (GP->BFPolicy == bfPREEMPT) {
        DBG(1, fSCHED)
        DPrint(
            "INFO:     no priority reservations created (preempt based "
            "backfill enabled)\n");
    ...
    GResPolicy =
        (GP->ResPolicy == resDefault) ? DEFAULT_RESERVATIONMODE : GP->ResPolicy;

    ResPolicy = (P->ResPolicy != resDefault) ? P->ResPolicy : GResPolicy;

    if ((GP->BFPolicy == bfNONE) || (ResPolicy == resNever)) {
        ...
        if (ResCountRej != NULL) *ResCountRej = TRUE;

        DBG(1, fSCHED)
        DPrint("INFO:     no priority reservations created (bf/rsv policy)\n");
    ...
    if (J->R != NULL) {
        ...
        DBG(1, fSCHED)
        DPrint(
            "INFO:     no priority reservations created (existing "
            "reservation)\n");
            ...
            case resCurrentHighest:
                ...
                switch (GP->ResTType) {
                    ...
                    case rttBypass:
                        ...
                        if (J->Bypass > GP->ResTValue) DoReserve = TRUE;
                    ...
                    case rttXFactor:
                        ...
                        XFactor = ((MSched.Time - J->SystemQueueTime) +
                                   J->SpecWCLimit[0]) /
                                  J->SpecWCLimit[0];
    ...
    for (rindex = 0; rindex < MAX_MQOS; rindex++) {
        if (GP->ResDepth[rindex] == 0) continue;

        for (qindex = 0; qindex < MAX_MQOS; qindex++) {
            if (GP->ResQOSList[rindex][qindex] == NULL) break;
    ...
    if ((DoReserve == TRUE) && (ResCount[rindex] < GP->ResDepth[rindex])) {
        if (MJobReserve(J, mjrPriority) == FAILURE) {
            DBG(2, fSCHED)
            DPrint("WARNING:  cannot reserve priority job '%s'\n",
            ...
            if (J->R != NULL) {
                ...
                DBG(1, fCORE)
                DPrint(
                    "ALERT:    cannot create reservation for priority job %s\n",
                    J->Name);
            }
        }
    } else {
...
int MJobSetDependency(
    ...
    mjob_t *J, enum MJobDependEnum Type, char *Value)
...

{
    ...
    DBG(8, fSCHED)
    DPrint("%s(%s,Type,%s)\n", FName, (SJ != NULL) ? SJ->Name : "NULL",
           (Value != NULL) ? "Value" : "NULL");

    if (Value != NULL) Value[0] = '\0';

    if (Type != NULL) *Type = mjdNONE;
    ...
    if (RM->Type == mrmtLL) {
        ...
        for (sindex = 0; sindex < SJ->Proc; sindex++) {
            sprintf(Name, "%s.%d.%d", ptr, SJ->Cluster, sindex);

            if (MJobFind(Name, &J, 0) == SUCCESS) {
                DBG(6, fSCHED)
                DPrint(
                    "INFO:     job '%s' has job '%s' remaining as a prereq "
                    "step\n",
                    SJ->Name, Name);

                if (Value != NULL) strcpy(Value, J->Name);
    ...
    while (D != NULL) {
        ...
        switch (D->Type) {
            case mjdJobStart:
                ...
                if ((MJOBISACTIVE(J) == TRUE) || (MJOBISCOMPLETE(J) == TRUE)) {
        ...
        if (DependSatisfied == FALSE) {
            ...
            if (Value != NULL) strcpy(Value, D->Value);
...

{
    ...
    DBG(5, fSCHED)
    DPrint("%s(%s,%s,RQ[%d],%s,TCAvail,%.3f,RIndex,%s,FeasCheck)\n", FName,
           J->Name, N->Name, RQ->Index, MULToTString(StartTime - MSched.Time),
           MinSpeed, (Affinity == NULL) ? "NULL" : "Affinity");

    *RIndex = 0;

    if (TCAvail != NULL) *TCAvail = 0;

    if (Affinity != NULL) *Affinity = nmNone;
    ...
    if (StartTime == MSched.Time) {
        ...
        if (!(J->Flags & (1 << mjfPreemptor))) {
            if (((N->State != mnsIdle) && (N->State != mnsActive)) ||
                ...
                DBG(5, fSCHED)
                DPrint("INFO:     node is in %s state '%s'\n",
                       (N->State != mnsIdle) ? "" : "expected",
                       (N->State != mnsIdle) ? MNodeState[N->State]
                                             : MNodeState[N->EState]);
            ...
            if ((RQ->NAccessPolicy == mnacSingleJob) ||
                ...
                if ((N->State != mnsIdle) || (N->EState != mnsIdle)) {
                    DBG(5, fSCHED)
                    DPrint(
                        "INFO:     node is in %s state '%s' (Dedicated access "
                        "required)\n",
                        (N->State != mnsIdle) ? "" : "expected",
                        (N->State != mnsIdle) ? MNodeState[N->State]
                                              : MNodeState[N->EState]);
                ...
                if (N->ARes.Procs < N->CRes.Procs) {
                    DBG(5, fSCHED)
                    DPrint(
                        "INFO:     inadequate procs on node '%s' (%d "
                        "available)\n",
                        N->Name, N->ARes.Procs);
        ...
        if (TC < MAX(1, RQ->TasksPerNode)) {
            DBG(5, fSCHED)
            DPrint(
                "INFO:     node supports %d task%c (%d tasks/node required)\n",
                TC, (TC == 1) ? ' ' : 's', MAX(1, RQ->TasksPerNode));
        ...
        if (MUNumListGetCount(J->StartPriority, RQ->DRes.PSlot, NA->PSlot, 0,
                              NULL) == FAILURE) {
            DBG(5, fSCHED)
            DPrint(
                "INFO:     node is missing classes [ALL %d:%d]%s "
                "required:available\n",
                RQ->DRes.PSlot[0].count, NA->PSlot[0].count,
        ...
        if ((RQ->Network != 0) && !(N->Network & (1 << RQ->Network))) {
            DBG(5, fSCHED)
            DPrint("INFO:     inadequate adapters %d (%s needed  %s found)\n",
                   RQ->Network, MAList[eNetwork][RQ->Network],
        ...
        if (N->CRes.Disk > 0) {
            ...
            if (N->CRes.Disk <= (2 * J->ExecSize)) {
                DBG(5, fSCHED)
                DPrint(
                    "INFO:     inadequate free space in local filesystem for "
                    "executable on node '%s' (%d MB available)\n",
                    N->Name, N->CRes.Disk);
        ...
        if (RQ->DRes.GRes[0].count != 0) {
            if (MUNumListGetCount(J->StartPriority, RQ->DRes.GRes, NA->GRes, 0,
                                  NULL) == FAILURE) {
                DBG(3, fSCHED)
                DPrint("INFO:     generic resources not found (%d needed)\n",
    ...
    if (StartTime < MAX_MTIME) {
        ...
        if ((N->CRes.Swap > 0) && (N->ARes.Swap < MIN_SWAP)) {
            DBG(5, fSCHED)
            DPrint("INFO:     inadequate swap on node '%s' (%d MB available)\n",
                   N->Name, N->ARes.Swap);
        ...
        if (N->FrameIndex != -1) {
            if (MSys[N->FrameIndex][N->SlotIndex].Disk > 0) {
                if (RQ->RequiredDisk > MSys[N->FrameIndex][N->SlotIndex].Disk) {
                    DBG(5, fSCHED)
                    DPrint(
                        "INFO:     inadequate disk space on node '%s' (%d MB "
                        "available)\n",
                        N->Name, MFrame[N->FrameIndex].Disk);
        ...
        if (!MUCompare(N->CRes.Disk, RQ->DiskCmp, RQ->RequiredDisk)) {
            DBG(5, fSCHED)
            DPrint("INFO:     inadequate disk (%s %d requested  %d found)\n",
                   MComp[RQ->DiskCmp], RQ->RequiredDisk, N->CRes.Disk);
        ...
        if (MJobCheckNStartTime(J, RQ, N, StartTime, TCAvail, MinSpeed, RIndex,
                                Affinity, AvailTime) == FAILURE) {
            return (FAILURE);
        }
    } else {
        if (TCAvail != NULL) {
            *TCAvail = MNodeGetTC(N, &N->CRes, &N->CRes, &N->DRes, &RQ->DRes,
                                  MAX_MTIME);
        }
    }
...
int MJobCheckNStartTime(
    ...
    mjob_t *J,                    /* I:   job                          */
    mreq_t *RQ, mnode_t *N,       /* I:   node                         */
    long StartTime,               /* I:   job start time               */
    int *TCAvail,                 /* O:   tasks allowed at starttime   */
    double MinSpeed, int *RIndex, /* O:   selection failure index      */
...

{
    ...
    DBG(5, fSCHED)
    DPrint("%s(%s,RQ,%s,%s,TasksAllowed,%lf,RIndex,%s)\n", FName,
           (J != NULL) ? J->Name : "NULL", (N != NULL) ? N->Name : "NULL",
           MULToTString(StartTime - MSched.Time), MinSpeed,
           (Affinity != NULL) ? "Affinity" : "NULL");

    if ((J == NULL) || (RQ == NULL) || (N == NULL)) return (FAILURE);

    AdjustedWCLimit = (long)((double)J->SpecWCLimit[0] / MinSpeed);

    /* check reservation time */

    if (MJobGetNRange(J, RQ, N, StartTime, TCAvail, &AvailableTime, Affinity,
                      &Type, NULL) == FAILURE) {
    ...
    if (ATime != NULL) *ATime = AvailableTime;
...

{
    ...
    DBG(4, fSCHED)
    DPrint("%s(%s,%d,%s,%s,DstNL,NC,TC,%ld,%ld)\n", FName,
           (J != NULL) ? J->Name : "NULL", (RQ != NULL) ? RQ->Index : -1,
           (P != NULL) ? P->Name : "NULL", (SrcNL != NULL) ? "SrcNL" : "NULL",
           StartTime, ResMode);
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        if (SrcNL != NULL) {
            ...
            if (MReqCheckResourceMatch(J, RQ, N, NULL) == FAILURE) continue;
        } /* END (SrcNL != NULL) */

        if ((N->PtIndex != P->Index) && (P->Index != 0) && (N->PtIndex != 0) &&
            !(J->Flags & (1 << mjfSpan))) {
            continue;
        }
        ...
        if (SrcNL != NULL) {
            ...
            if (J->ExcHList != NULL) {
                ...
                for (hlindex = 0; J->ExcHList[hlindex].N != NULL; hlindex++) {
                    if (J->ExcHList[hlindex].N->Index == nindex) break;
                } /* END for (hlindex) */

                if (J->ExcHList[hlindex].N != NULL) continue;
            } /* END if (J->ExcHList != NULL) */
            ...
            if ((J->Flags & (1 << mjfHostList)) && (J->ReqHList != NULL) &&
                ...
                for (hlindex = 0; J->ReqHList[hlindex].N != NULL; hlindex++) {
                    if (J->ReqHList[hlindex].N == N) break;
                } /* END for (hlindex) */

                if (J->ReqHList[hlindex].N == NULL) continue;
            }
        } /* END if (SrcNL != NULL) */

        /* check node state */

        if ((N->State == mnsDown) || (N->State == mnsDrained) ||
            (N->State == mnsNone) || (N->State == mnsUnknown)) {
            DBG(7, fSCHED)
            DPrint("INFO:     node %s is unavailable (state %s)\n", N->Name,
                   MNodeState[N->State]);
        ...
        if (MNodeCheckPolicies(J, N, MAX_MTIME, &TasksAllowed) == FAILURE) {
            DBG(7, fSCHED)
            DPrint("INFO:     node %s is unavailable (policies)\n", N->Name);
        ...
        if (MPar[0].EnableMultiNodeJobs == FALSE) {
            if (TasksAllowed < J->Request.TC) {
                DBG(7, fSCHED)
                DPrint(
                    "INFO:     node %s is unavailable (policy/inadequate tasks "
                    "%d)\n",
                    N->Name, TasksAllowed);
        ...
        if (MJobCheckNRes(J, N, RQ, StartTime, &TasksAllowed, 1.0, &RIndex,
                          NULL, NULL, TRUE) == FAILURE) {
            DBG(7, fSCHED)
            DPrint("INFO:     node %s does not meet job requirements (%s)\n",
                   N->Name, MAllocRejType[RIndex]);

            continue;
        }

        DBG(6, fSCHED)
        DPrint("INFO:     node %s added to feasible list (%d tasks)\n", N->Name,
               TasksAllowed);
    ...
    if ((MPar[0].NodeSetDelay == 0) && (tmpRSS != mrssNONE)) {
        ...
        MJobSelectResourceSet(
            ...
            (tmpRSS != mrssFirstOf) ? tmpRSS : mrssOneOf, RQ->SetList,
            (mnalloc_t *)DstNL, rindex);
    ...
    DBG(2, fSCHED)
    DPrint(
        "INFO:     %d feasible tasks found for job %s:%d in partition %s (%d "
        "Needed)\n",
        tc, J->Name, RQ->Index, (P != NULL) ? P->Name : "NULL", RQ->TaskCount);

    if (NC != NULL) *NC = rindex;

    if (TC != NULL) *TC = tc;

    if (tc <= 0) {
        DBG(7, fSCHED)
        DPrint("INFO:     inadequate feasible tasks found for job %s:%d\n",
               J->Name, RQ->Index);
    ...
    if (!(J->Flags & (1 << mjfBestEffort))) {
        if (tc < RQ->TaskCount) {
            DBG(2, fSCHED)
            DPrint(
                "INFO:     inadequate feasible tasks found for job %s:%d in "
                "partition %s (%d < %d)\n",
                J->Name, RQ->Index, (P != NULL) ? P->Name : "NULL", tc,
                RQ->TaskCount);
        ...
        if (rindex < RQ->NodeCount) {
            DBG(2, fSCHED)
            DPrint(
                "INFO:     inadequate feasible nodes found for job %s:%d in "
                "partition %s (%d < %d)\n",
                J->Name, RQ->Index, (P != NULL) ? P->Name : "NULL", rindex,
                RQ->NodeCount);
...

{
    ...
    DBG(5, fSCHED)
    DPrint("%s(%s,SrcMNL,DstMNL,NodeMap,NodeCount,TaskCount,%s)\n", FName,
           (J != NULL) ? J->Name : "NULL",
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        for (index = 0; Feasible[index].N != NULL; index++) {
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     checking Feasible[%d][%03d]:  '%s'\n", rqindex,
                   index, N->Name);

            if (MJobCheckNStartTime(J, RQ, N, StartTime, &TasksAllowed,
                                    MinSpeed, &RIndex, &Affinity,
                                    NULL) == FAILURE) {
            ...
            if (J->Flags & (1 << mjfAdvReservation)) {
                if ((Affinity == nmPositiveAffinity) ||
                    ...
                    (Affinity == nmRequired)) {
                    DBG(7, fSCHED)
                    DPrint("INFO:     node '%s' added (reserved)\n", N->Name);
                } else {
                    DBG(7, fSCHED)
                    DPrint("INFO:     node '%s' rejected (not reserved)\n",
                           N->Name);
            ...
            DBG(5, fSCHED)
            DPrint("INFO:     MNode[%03d] '%s' added (%d of %d)\n", index,
                   N->Name, AvailableTaskCount[rqindex], RQ->TaskCount);
        } /* END for (index) */
        ...
        if (NC != NULL) (*NC) += nindex;

        if (TC != NULL) (*TC) += AvailableTaskCount[rqindex];

        if (AvailableTaskCount[rqindex] < RQ->TaskCount) {
            DBG(3, fSCHED)
            DPrint(
                "INFO:     inadequate tasks found for job %s:%d (%d < %d) at "
                "%s\n",
                J->Name, rqindex, AvailableTaskCount[rqindex], RQ->TaskCount,
                MULToTString(StartTime - MSched.Time));
        ...
        if ((RQ->NodeCount > 0) &&
            ...
            (BestEffort != TRUE)) {
            DBG(3, fSCHED)
            DPrint("INFO:     inadequate nodes found for job %s:%d (%d < %d)\n",
                   J->Name, rqindex, AvailableNodeCount[rqindex],
                   RQ->NodeCount);
    ...
    DBG(3, fSCHED)
    DPrint("INFO:     adequate tasks found for all reqs (time %s)\n",
           MULToTString(StartTime - MSched.Time));

    if (J->Flags & (1 << mjfAllocLocal)) {
        /* check nodelist locality */

        if (MJobProximateMNL(J, MtmpNodeList, MNewNodeList, StartTime, FALSE) ==
            FAILURE) {
            DBG(4, fSCHED)
            DPrint("INFO:     insufficient proximity in available tasks\n");
    ...
    if (MJobNLDistribute(J, MtmpNodeList, DstMNL) == FAILURE) {
        ...
        DBG(4, fSCHED) {
            for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                ...
                DBG(4, fSCHED)
                DPrint("INFO:     tasks found for job %s:%d  %d of %d\n",
                       J->Name, rqindex, AvailableTaskCount[rqindex],
                       RQ->TaskCount);
            }
        }
    ...
    DBG(4, fSCHED)
    DPrint("INFO:     tasks found for job %s (tasks requested: %d)\n", J->Name,
           J->Request.TC);
...

{
    ...
    if (((J->CompletionTime - J->StartTime) >= J->WCLimit) ||
        ...
        if (MAMAllocJDebit(&MAM[0], J, &Reason, Message) == FAILURE) {
            DBG(1, fSTAT)
            DPrint("ERROR:    cannot debit account for job '%s'\n", J->Name);

            sprintf(
                Line,
                "AMFAILURE:  cannot debit account for job %s, reason %s (%s)\n",
                J->Name, MDefReason[Reason], Message);

            MSysRegEvent(Line, 0, 0, 1);
        }
    } else {
        if (J->Cred.A != NULL) {
            if (MAMAllocResCancel(J->Cred.A->Name, J->Name, "job removed", NULL,
                                  &Reason) == FAILURE) {
                DBG(1, fSCHED)
                DPrint(
                    "ERROR:    cannot cancel allocation reservation for job "
    ...
    if (J->R != NULL) {
        ...
        if (J->R->Flags & (1 << mrfSingleUse)) MResDestroy(&J->R);
    } /* END if (J->R != NULL) */
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
            ...
            DBG(3, fSTAT)
            DPrint("INFO:     node '%s' release from job\n", N->Name);

            if (nindex == MAX_MNODE_PER_JOB) {
                DBG(0, fSTAT)
                DPrint("ERROR:    node overflow in %s() on job '%s'\n", FName,
                       J->Name);
    ...
    if (J->MasterJobName != NULL) {
        if (MJobFind(J->MasterJobName, &MJ, 0) == FAILURE) {
            DBG(2, fSTAT)
            DPrint(
                "WARNING:  cannot locate masterjob '%s' at completion of job "
                "'%s'\n",
                J->MasterJobName, J->Name);
        }
        ...
        for (rqindex = 0; MJ->Req[rqindex] != NULL; rqindex++) {
            ...
            if (MJ->Req[rqindex]->State == mjsCompleted) ccount++;
        }
...

{
    ...
    if (MAMAllocJDebit(&MAM[0], J, &Reason, Message) == FAILURE) {
        ...
        sprintf(
            Line,
            "AMFAILURE:  cannot debit allocations for job %s, reason %s (%s)\n",
            J->Name, MDefReason[Reason], Message);
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        for (nindex = 0; nindex < MAX_MNODE_PER_JOB; nindex++) {
            if (RQ->NodeList[nindex].N == NULL) break;

            N = RQ->NodeList[nindex].N;

            DBG(3, fSTAT)
            DPrint("INFO:     node '%s' returned to idle pool\n", N->Name);

            if (nindex == MAX_MNODE_PER_JOB) {
                DBG(0, fSTAT)
                DPrint(
                    "ERROR:    job node buffer overflow in %s() on job '%s:%d' "
                    "(iteration: %d)\n",
                    FName, J->Name, rqindex, MSched.Iteration);
...

{
    ...
    DBG(8, fSCHED)
    DPrint("%s(%s,RQ,%s,%ld,TA,WT,Aff,Type,BRes)\n", FName,
           (J != NULL) ? J->Name : "NULL", (N != NULL) ? N->Name : "NULL",
           StartTime);
    ...
    if (!(J->Flags & (1 << mjfResMap)) && !(J->Flags & (1 << mjfPreemptor))) {
    ...
    DBG(8, fSCHED)
    DPrint("INFO:     TC: %d  Maxtasks: %d  MinTasks: %d (TPNCap: %d)\n", TC,
           MaxTasks, MinTasks, TPNCap);
    ...
    while (MaxTasks > MAX(MinTasks, TPNCap)) {
        ...
        if (MJobGetSNRange(J, RQ, N, RRange, 1, Affinity, Type, ARange, NULL,
                           BRes) == FAILURE) {
            MaxTasks = TaskCount - 1;

            if (RQ->TasksPerNode > 0) MaxTasks -= (MaxTasks % RQ->TasksPerNode);
        } else {
            MinTasks = ARange[0].TaskCount;

            RangeTime = MIN(MAX_MTIME, ARange[0].EndTime - ARange[0].StartTime);

            if (Affinity != NULL) tmpAffinity = *Affinity;
        }
    } /* END while(MaxTasks > MinTasks) */

    if (WindowTime != NULL) *WindowTime = RangeTime;

    if (Affinity != NULL) *Affinity = tmpAffinity;

    if (TasksAvailable != NULL) {
        if (RQ->TasksPerNode > 0) MinTasks -= (MinTasks % RQ->TasksPerNode);
...

{
    ...
    DBG(7, fSCHED)
    DPrint("%s(%s,%d,%s,(%d@%s),%d,%s,Type,ARange,BRes)\n", FName,
           (J != NULL) ? J->Name : "NULL", (RQ != NULL) ? RQ->Index : -1,
           N->Name, RRange[0].TaskCount,
           MULToTString(RRange[0].StartTime - MSched.Time), RCount,
    ...
    DBG(8, fSCHED)
    DPrint(
        "INFO:     attempting to get resources for %s %d * (P: %d  M: %d  S: "
        "%d  D: %d)\n",
        J->Name, RRange[0].TaskCount, RQ->DRes.Procs, RQ->DRes.Mem,
        RQ->DRes.Swap, RQ->DRes.Disk);
    ...
    for (eindex = 0; eindex < (MSched.ResDepth << 1); eindex++) {
        if (N->RE[eindex].Type == mreNONE) break;

        R = N->R[N->RE[eindex].Index];

        if ((R->EndTime > RRange[0].StartTime) || (R->EndTime >= MAX_MTIME - 1))
            break;

        DBG(6, fSCHED)
        DPrint(
            "INFO:     skipping early MRE[%d] event for '%s' (E: %ld <= S: "
            "%ld) : P: %ld\n",
            eindex, R->Name, R->EndTime, RRange[0].StartTime, MSched.Time);
    } /* END for (eindex) */
    ...
    if (N->RE[eindex].Type == mreNONE) {
        ...
        DBG(8, fSCHED)
        DPrint("INFO:     ARange[0] adjusted (TC: %d  S: %ld)\n",
               ARange[0].TaskCount, ARange[0].StartTime);
        ...
        if (ARange[0].TaskCount < RRange[0].TaskCount) {
            ...
            DBG(6, fSCHED)
            DPrint(
                "INFO:     node %s contains inadequate resources (State: %s)\n",
                N->Name, MNodeState[N->State]);
            ...
            if (tmpTC >= RRange[0].TaskCount) {
                ...
                if (Affinity != NULL) *Affinity = nmUnavailable;
        ...
        DBG(8, fSCHED) {
            ...
            DPrint("INFO:     closing ARange[%d] (%s -> %s : %d) (0)\n", 0,
                   WCString, MULToTString(ARange[0].EndTime - MSched.Time),
                   ARange[0].TaskCount);
        }
        ...
        DBG(5, fSCHED)
        DPrint(
            "INFO:     node %s supports %d task%s of job %s:%d for %s of %s "
            "(no reservation)\n",
            N->Name, ARange[0].TaskCount, (ARange[0].TaskCount == 1) ? "" : "s",
            J->Name, RQ->Index,
            MULToTString(ARange[0].EndTime - ARange[0].StartTime), WCString);
        ...
        if (BRes != NULL) strcpy(BRes, "NONE");

        if (J->Flags & (1 << mjfAdvReservation)) {
            if (Affinity != NULL) *Affinity = nmUnavailable;

            DBG(5, fSCHED)
            DPrint("INFO:     no reservation found for AdvRes job %s\n",
                   J->Name);
    ...
    DBG(8, fSCHED) {
        ...
        DPrint("INFO:     requested resources: %s (x%d)\n",
               MUCResToString(&RQ->DRes, 0, 0, NULL), tmpTC);
    }
    ...
    if (UseDed == TRUE) MCResAdd(&DedRes, &N->CRes, &RQ->DRes, tmpTC, FALSE);
    ...
    for (; eindex < (MSched.ResDepth << 1); eindex++) {
        ...
        if (R->EndTime <= RRange[0].StartTime) continue;

        if ((N->RE[eindex].Time > RRange[0].StartTime) && (PreRes == TRUE)) {
            /* adjust/check requirements at job start */

            DBG(7, fSCHED)
            DPrint("INFO:     performing starttime check (%d)\n", eindex);

            PreRes = FALSE;

            if ((MUCResIsNeg(&AvlRes) == SUCCESS) || (PBlock == TRUE) ||
                ((J->Flags & (1 << mjfAdvReservation)) && (IResCount <= 0))) {
                DBG(6, fSCHED)
                DPrint("INFO:     resources unavailable at time %s\n",
                       MULToTString(RRange[0].StartTime - MSched.Time));

                /* no active range can exist if resource unavailable at
                 * starttime */
                ...
                if (N->RE[eindex].Time >= RRange[0].EndTime) {
                    /* range not active at time '0' and next event is too late
                     * to be considered */

                    DBG(8, fSCHED)
                    DPrint(
                        "INFO:     reservation extends beyond acceptable time "
                        "range\n");

                    break;
                }
            } else {
                TC = MNodeGetTC(N, &AvlRes, &N->CRes, &DedRes, &RQ->DRes,
                                MSched.Time);

                ARange[rindex].TaskCount = RRange[0].TaskCount + TC;
                ARange[rindex].NodeCount = 1;
                ARange[rindex].StartTime =
                    MAX(RRange[0].StartTime, MSched.Time + ResOffset);

                ActiveRange = TRUE;

                DBG(8, fSCHED)
                DPrint("INFO:     ARange[%d] started (TC: %d  S: %ld)\n",
                       rindex, ARange[rindex].TaskCount,
                       ARange[rindex].StartTime);

                if (DRes != NULL) MUCResGetMin(DRes, DRes, &AvlRes);
            }
        ...
        if (N->R[N->RE[eindex].Index]->Type != mrtUser) {
            ...
            DBG(8, fSCHED)
            DPrint("INFO:     non-user reservation[%d] '%s'x%d resources %s\n",
                   N->RE[eindex].Index, N->R[N->RE[eindex].Index]->Name, RC,
            ...
            DBG(8, fSCHED)
            DPrint("INFO:     user reservation[%d] '%s'x%d resources %s\n",
                   N->RE[eindex].Index, N->R[N->RE[eindex].Index]->Name, RC,
                   MUCResToString(BR, 0, 0, NULL));
        }

        if (ActiveRange == TRUE) {
            /* be generous (ie, JDuration - (R->StartTime -
             * ARange[rindex].StartTime) */

            Overlap = MIN(R->EndTime, ARange[rindex].StartTime + JDuration) -
                      MAX(R->StartTime, ARange[rindex].StartTime);

            if (Overlap < 0) Overlap = R->EndTime - R->StartTime;
        } else {
        ...
        DBG(7, fSCHED)
        DPrint("INFO:     checking reservation %s %s at time %s (O: %ld)\n",
               N->R[N->RE[eindex].Index]->Name,
               (N->RE[eindex].Type == mreStart) ? "start" : "end",
               MULToTString(N->RE[eindex].Time - MSched.Time), Overlap);
        ...
        if (N->RE[eindex].Type == mreStart) {
            ...
            if (MResCheckJAccess(R, J, Overlap, &Same, Affinity) == TRUE) {
                ...
                if (J->Flags & (1 << mjfAdvReservation)) ResourcesAdded = TRUE;
            } else {
                ...
                if ((J->Req[0]->NAccessPolicy == mnacSingleUser) &&
                    (tmpJ != NULL) && (tmpJ->Cred.U != NULL) &&
                    (J->Cred.U != NULL) && (tmpJ->Cred.U != J->Cred.U) &&
                ...
                if (BRes != NULL) strcpy(BRes, N->R[N->RE[eindex].Index]->Name);
            }
            ...
            if (MResCheckJAccess(R, J, Overlap, &Same, Affinity) == TRUE) {
                ...
                if ((J->Req[0]->NAccessPolicy == mnacSingleUser) &&
                    (tmpJ != NULL) && (tmpJ->Cred.U != J->Cred.U)) {
        ...
        if ((N->RE[eindex + 1].Type == mreNONE) ||
            ...
            DBG(8, fSCHED)
            DPrint("INFO:     verifying resource access at %s (%d)\n",
                   MULToTString(N->RE[eindex].Time - MSched.Time), eindex);

            if (((MJC > 0) && (JC >= MJC)) || ((MUJC > 0) && (UJC >= MUJC))) {
                PBlock = TRUE;
            } else {
                PBlock = FALSE;
            }
            ...
            if ((R->StartTime == RRange[0].StartTime) && (PreRes == TRUE)) {
                ...
                if (MUCResIsNeg(&AvlRes) == SUCCESS) {
                    ...
                    DBG(7, fSCHED)
                    DPrint("INFO:     policy blocks access at time %s\n",
                           MULToTString(RRange[0].StartTime - MSched.Time));
                    ...
                    if (N->RE[eindex].Time >= RRange[0].EndTime) {
                        ...
                        break;
                           ...
                           (IResCount > 0)) {
                    /* range is active unless advance reservation is required
                     * and not found */
            ...
            if ((ResourcesRemoved == TRUE) && (ActiveRange == TRUE)) {
                ...
                if ((MUCResIsNeg(&AvlRes) == SUCCESS) || (PBlock == TRUE) ||
                    ...
                    DBG(6, fSCHED)
                    DPrint(
                        "INFO:     resources unavailable at time %s during "
                        "reservation %s %s (%s)\n",
                        MULToTString(N->RE[eindex].Time - MSched.Time), R->Name,
                        (N->RE[eindex].Type == mreStart) ? "start" : "end",
                        (PBlock == TRUE) ? "policy" : "resources");

                    if (((N->RE[eindex].Time - ARange[rindex].StartTime) >=
                         JDuration) ||
                        ((rindex > 0) && (ARange[rindex].StartTime ==
                                          ARange[rindex - 1].EndTime))) {
                        ...
                        DBG(8, fSCHED) {
                            strcpy(WCString,
                                   MULToTString(ARange[rindex].StartTime -
                                                MSched.Time));

                            DPrint(
                                "INFO:     closing ARange[%d] (%s -> %s : %d) "
                                "(1)\n",
                                rindex, WCString,
                                MULToTString(ARange[rindex].EndTime -
                                             MSched.Time),
                        ...
                        if (rindex >= MAX_MRANGE) {
                            DBG(6, fSCHED)
                            DPrint("ALERT:    range overflow in %s(%s,%s)\n",
                                   FName, J->Name, N->Name);
                        ...
                        DBG(8, fSCHED)
                        DPrint("INFO:     range too short (ignoring)\n");
                    }
                    ...
                    if (N->RE[eindex].Time >= RRange[0].StartTime) {
                        ...
                        if (((TC + RRange[0].TaskCount) !=
                             ARange[rindex].TaskCount) &&
                            ((RQ->TaskCount == 0) ||
                             (TC + RRange[0].TaskCount < RQ->TaskCount) ||
                             (TC + RRange[0].TaskCount <
                              ARange[rindex].TaskCount) ||
                            ...
                            if (N->RE[eindex].Time > ARange[rindex].StartTime) {
                                ...
                                if (rindex >= MAX_MRANGE) {
                                    DBG(1, fSCHED)
                                    DPrint(
                                        "ALERT:    range overflow in "
                                        "%s(%s,%s)\n",
                                        FName, J->Name, N->Name);

                                    return (FAILURE);
                                }
                            } /* END if (N->RE[eindex].Time >
                                 ARange[rindex].StartTime) */

                            ARange[rindex].StartTime = MAX(
                                N->RE[eindex].Time, MSched.Time + ResOffset);
                            ARange[rindex].TaskCount = RRange[0].TaskCount + TC;
                            ARange[rindex].NodeCount = 1;
                    ...
                    else if ((RRange[0].TaskCount + TC) <
                             ARange[rindex].TaskCount) {
                        /* in 'pre-active' time */

                        DBG(6, fSCHED)
                        DPrint(
                            "INFO:     adjusting 'preactive' ARange[%d] "
                            "taskcount from %d to %d\n",
                            rindex, ARange[rindex].TaskCount,
                        ...
                        DBG(6, fSCHED)
                        DPrint(
                            "INFO:     ARange[%d] taskcount not affected by "
                            "reservation change\n",
                            rindex);
                    }
                ...
                if (NewRange == TRUE) {
                    ...
                    DBG(8, fSCHED) {
                        strcpy(WCString, MULToTString(ARange[rindex].StartTime -
                                                      MSched.Time));
                    ...
                    ARange[rindex].StartTime =
                        MAX(N->RE[eindex].Time, MSched.Time + ResOffset);
                ...
                if ((MUCResIsNeg(&AvlRes) == FAILURE) &&
                    ...
                    DBG(6, fSCHED)
                    DPrint(
                        "INFO:     resources available at time %s during %s "
                        "%s\n",
                        MULToTString(N->RE[eindex].Time - MSched.Time), R->Name,
                        (N->RE[eindex].Type == mreStart) ? "start" : "end");

                    ARange[rindex].StartTime =
                        MAX(MAX(RRange[0].StartTime, N->RE[eindex].Time),
                            MSched.Time + ResOffset);
                    ...
                    DBG(8, fSCHED)
                    DPrint("INFO:     ARange[%d] adjusted (TC: %d  S: %ld)\n",
                           rindex, ARange[rindex].TaskCount,
                           ARange[rindex].StartTime);
    ...
    if (ActiveRange == TRUE) {
        ...
        DBG(8, fSCHED) {
            strcpy(WCString,
                   MULToTString(ARange[rindex].StartTime - MSched.Time));

            DPrint("INFO:     closing ARange[%d] (%s -> %s : %d) (4)\n", rindex,
                   WCString, MULToTString(ARange[rindex].EndTime - MSched.Time),
                   ARange[rindex].TaskCount);
        }
    ...
    for (index = 0; ARange[index].EndTime != 0; index++) {
        ...
        if ((MPar[0].NodeDownStateDelayTime > 0) &&
            ...
            ARange[index].StartTime =
                MAX(ARange[index].StartTime,
                    MIN(ARange[index].EndTime,
                        MSched.Time + MPar[0].NodeDownStateDelayTime));
        }

        if (ARange[index].EndTime < RRange[0].StartTime) {
            DBG(6, fSCHED)
            DPrint(
                "INFO:     ARange[%d] too early for job %s (E: %ld < S: %ld  "
                "P: %ld): removing range\n",
                index, J->Name, ARange[index].EndTime, RRange[0].StartTime,
                MSched.Time);
            ...
            DBG(6, fSCHED)
            DPrint(
                "INFO:     ARange[%d] too early for job %s (S: %ld < S: %ld): "
                "truncating range\n",
                index, J->Name, ARange[index].StartTime, RRange[0].StartTime);
        ...
        if (ARange[index].TaskCount < RRange[0].TaskCount) {
            DBG(6, fSCHED)
            DPrint(
                "INFO:     ARange[%d] too small for job %s (%d < %d):  "
                "removing range\n",
                index, J->Name, ARange[index].TaskCount, RRange[0].TaskCount);
        } else {
    ...
    for (index = 0; ARange[index].EndTime != 0; index++) {
        ...
        if (RangeTime < JDuration) {
            ...
            for (index2 = index - 1; index2 >= 0; index2--) {
                ...
                RangeTime +=
                    (ARange[index2].EndTime - ARange[index2].StartTime);

                if (RangeTime >= JDuration) break;
            }

            if (RangeTime < JDuration) {
                /* append later ranges if immediate */

                for (index2 = index + 1; ARange[index2].EndTime != 0;
                     index2++) {
        ...
        if (ARange[index].StartTime > RRange[0].EndTime) {
            DBG(6, fSCHED)
            DPrint(
                "INFO:     ARange[%d] (%ld -> %ld)x%d too late for job %s by "
                "%ld\n",
                index, ARange[index].StartTime, ARange[index].EndTime,
                ARange[index].TaskCount, J->Name,
            ...
            if ((rindex > 0) &&
                ...
                ((ARange[rindex].TaskCount >= ARange[rindex - 1].TaskCount) ||
                 ((ARange[rindex - 1].EndTime - ARange[rindex - 1].StartTime) <
                  JDuration))
            ...
            DBG(6, fSCHED)
            DPrint(
                "INFO:     ARange[%d] too late for job %s (E: %ld > E: %ld  P: "
                "%ld): truncating range\n",
                index, J->Name, ARange[index].EndTime, RRange[0].EndTime,
                MSched.Time);
        ...
        if (RangeTime < MAX(1, JDuration)) {
            DBG(6, fSCHED)
            DPrint(
                "INFO:     ARange[%d] too short for job %s (MR: %ld < W: %ld): "
                " removing range\n",
                index, J->Name, RangeTime, JDuration);
    ...
    if ((ARange[0].EndTime == 0) || (ARange[0].TaskCount == 0)) {
        DBG(6, fSCHED)
        DPrint("INFO:     node %s unavailable for job %s at %s\n", N->Name,
               J->Name, MULToTString(RRange[0].StartTime - MSched.Time));
    ...
    if (J->ResName[0] != '\0') {
        if ((Affinity != NULL) && ((*Affinity != nmPositiveAffinity) &&
                                   (*Affinity != nmNeutralAffinity) &&
                                   (*Affinity != nmNegativeAffinity))) {
            if (Affinity != NULL) *Affinity = nmUnavailable;
    ...
    DBG(6, fSCHED) {
        for (rindex = 0; ARange[rindex].EndTime != 0; rindex++) {
            ...
            DPrint(
                "INFO:     node %s supports %d task%c of job %s:%d for %s at "
                "%s\n",
                N->Name, ARange[rindex].TaskCount,
                (ARange[rindex].TaskCount == 1) ? ' ' : 's', J->Name, RQ->Index,
...
int MJobProximateMNL(
    ...
    mjob_t *J, mnodelist_t SMNL, mnodelist_t DMNL, long StartTime,
    int DoExactMatch)

{
    ...
    DBG(2, fSCHED)
    DPrint("%s(%s,MSNL,MDNL,%ld,%d)\n", FName, (J != NULL) ? J->Name : "NULL",
           StartTime, DoExactMatch);
    ...
    switch (MSched.AllocLocalityPolicy) {
        case malpRMSpecific:
            ...
            rc = MRMJobGetProximateMNL(J, (J->RM != NULL) ? J->RM : &MRM[0],
                                       (SMNL != DMNL) ? SMNL : tmpMNL, DMNL,
                                       StartTime, DoExactMatch, NULL, NULL);
    ...
    for (index = 0; index < MAX_MATTR; index++) SetList[index].Index = index;
    ...
    DBG(2, fSCHED) {
        DBG(2, fSCHED)
        DPrint("INFO:     set summary:  (total nodes: %3d)\n", nindex);

        for (sindex = 0; sindex < MAX_MATTR; sindex++) {
            if (SetList[sindex].Size) {
                fprintf(mlog.logfp, "[%2d : %2d]", SetList[sindex].Index,
                        SetList[sindex].Size);
            }
        } /* END for (sindex) */
    ...
    qsort((void *)SetList, MaxSet + 1, sizeof(_MALPSet),
    ...
    if (TotalTasks < MJobGetProcCount(J)) {
        ...
        DBG(4, fSCHED)
        DPrint("INFO:     TC available: %d  procs required: %d\n", TotalTasks,
               J->Request.TC);
    ...
    DBG(4, fSCHED)
    DPrint("INFO:     job '%s' requires %d tasks in %d sets\n", J->Name,
           J->Request.TC, MinSets);

    DBG(4, fSCHED)
    DPrint("INFO:     adequate tasks found: %d tasks in %d sets\n", TotalTasks,
           MinSets);
...

    {
        ...
        */
        ...
        if (MinSets > (OptimalSetCount + MSched.AllocLocalityMargin))
          {
          DBG(4,fSCHED) DPrint("INFO:     inadequate locality found for job '%s'
        (max sets found %d (O: %d)\n",
            J->Name,
            MinSets,
            OptimalSetCount);
    ...
    DBG(4, fSCHED)
    DPrint("INFO:     job '%s' found %d tasks in %d sets (adequate locality)\n",
           J->Name, J->Request.TC, MinSets);
        ...
        switch (MSched.AllocLocalityGroup) {
            ...
            default:
                ...
                for (sindex = 0; sindex < MinSets; sindex++) {
                    if (N->FrameIndex != SetList[sindex].Index) continue;

                    DBG(4, fSCHED) {
                        fprintf(mlog.logfp, "[%2d : %8s]", nodeindex, N->Name);
                    }
    ...
    if (TaskCount < J->Request.TC) {
        DBG(0, fSCHED)
        DPrint("ERROR:    only obtained %d of %d tasks for job '%s' in %s()\n",
               TaskCount, J->Request.TC, J->Name, FName);
...
int MReqRResFromString(
    ...
    mjob_t *J, mreq_t *RQ, char *ResString, int RMType, int IsPref)
...
#define MAX_MRMRRESTYPE 4
    ...
    DBG(5, fSCHED)
    DPrint("%s(%s,RQ,%s,%d)\n", FName, (J != NULL) ? J->Name : "NULL",
           (ResString != NULL) ? ResString : "NULL", IsPref);
    ...
    switch (RMType) {
        case mrmtLSF:
            ...
            if (!strncmp(ResString, "expr", 4)) {
                for (index = 0; index < 4; index++) ResString[index] = ' ';
            }
    ...
    while (ptr != NULL) {
        if (MUParseComp(ptr, AName, &CIndex, ValLine) == FAILURE) {
            DBG(0, fSCHED)
            DPrint("ALERT:    cannot parse req line for job '%s'\n", J->Name);
        ...
        if (MRMRRes[index].RRIndex == -1) {
            if (MUGetMAttr(eFeature, AName, mVerify, RQ->ReqFBM,
                           sizeof(RQ->ReqFBM)) != SUCCESS) {
                DBG(0, fSCHED)
                DPrint(
                    "ALERT:    cannot identify resource type '%s' for job "
                    "'%s'\n",
                    AName, J->Name);
            } else {
                DBG(3, fSCHED)
                DPrint(
                    "INFO:     feature request %s added to job %s resource "
                    "requirements\n",
                    AName, J->Name);
            }
        ...
        if (IsPref == TRUE) {
            DBG(6, fSCHED)
            DPrint("INFO:     job %s requests resource preference %s %s %s\n",
                   J->Name, AName, MComp[CIndex], ValLine);

            switch (MRMRRes[index].RRIndex) {
                case mrrFeature:

                    if (CIndex == mcmpEQ)
                        MUGetMAttr(eFeature, ValLine, mAdd, RQ->PrefFBM,
                                   sizeof(RQ->PrefFBM));
        ...
        DBG(6, fSCHED)
        DPrint("INFO:     job %s requests resource constraints %s %s %s\n",
               J->Name, AName, MComp[CIndex], ValLine);

        switch (MRMRRes[index].RRIndex) {
            case mrrArch:

                if ((ValLine[0] != '\0') && strcmp(ValLine, "any") &&
                    (CIndex == mcmpEQ)) {
                    if ((RQ->Arch = MUMAGetIndex(eArch, ValLine, mAdd)) ==
                        FAILURE) {
                        DBG(1, fSCHED)
                        DPrint(
                            "WARNING:  job '%s' does not have valid Arch value "
                            "'%s'\n",
                            J->Name, ValLine);
                    }
                }
            ...
            case mrrFeature:
                ...
                if (CIndex == mcmpEQ)
                    MUGetMAttr(eFeature, ValLine, mAdd, RQ->ReqFBM,
                               sizeof(RQ->ReqFBM));
            ...
            case mrrHost:
                ...
                if (CIndex == mcmpEQ) {
                    if (MJobSetAttr(J, mjaHostList, (void **)ValLine, mdfString,
                                    mAdd) == FAILURE) {
                        char tmpLine[MAX_MLINE];
                        ...
                        MJobSetHold(J, (1 << mhDefer), MSched.DeferTime,
                                    mhrNoResources, tmpLine);
                    }

                    J->ReqHLMode = mhlmSubset;
                } else if (CIndex == mcmpNE) {
                    MJobSetAttr(J, mjaExcHList, (void **)ValLine, mdfString,
                                mAdd);
                } else {
                    ...
                    DBG(1, fSCHED)
                    DPrint(
                        "WARNING:  job '%s' detected with invalid host "
                        "requirment '%s'\n",
                        J->Name, ValLine);
                }
            ...
            case mrrOpSys:
                ...
                if ((ValLine[0] != '\0') && strcmp(ValLine, "any") &&
                    (CIndex == mcmpEQ)) {
                    if ((RQ->Opsys = MUMAGetIndex(eOpsys, ValLine, mAdd)) ==
                        FAILURE) {
                        DBG(1, fSCHED)
                        DPrint(
                            "WARNING:  job '%s' does not have valid Opsys "
                            "value '%s'\n",
                            J->Name, ValLine);
                    }
                }
            ...
            default:
                ...
                DBG(3, fSCHED)
                DPrint("WARNING:  resource req type %s not supported in %s()\n",
                       AName, FName);
...

{
    ...
    DBG(5, fSCHED)
    DPrint("%s(%s,RQ,%s,%s,GRange,%s,NodeMap,%d,%s)\n", FName,
           (J != NULL) ? J->Name : "NULL", (P != NULL) ? P->Name : "NULL",
           MULToTString(MinStartTime - MSched.Time),
           (MAvlNodeList == NULL) ? "NULL" : "MAvlNodeList", RangeType,
    ...
    if (MReqGetFNL(J, RQ, P, NULL, NodeList, NULL, &TaskCount, MAX_MTIME, 0) ==
        FAILURE) {
        DBG(6, fSCHED)
        DPrint("INFO:     cannot locate configured resources in %s()\n", FName);
    ...
    for (nindex = 0; NodeList[nindex].N != NULL; nindex++) {
        ...
        if (MJobGetSNRange(J, RQ, N, RRange, MAX_MRANGE, &Affinity, NULL,
                           ARange, NULL, NULL) == FAILURE) {
            DBG(7, fSCHED)
            DPrint(
                "INFO:     no reservation time found for job %s on node %s at "
                "%s\n",
                J->Name, N->Name, MULToTString(MinStartTime - MSched.Time));

            if (NodeMap != NULL) NodeMap[N->Index] = nmUnavailable;

            continue;
        }

        if (NodeMap != NULL) NodeMap[N->Index] = Affinity;
        ...
        if ((SRange[0].EndTime != 0) &&
            ...
            if (MAvlNodeList != NULL) {
                ...
                DBG(7, fSCHED)
                DPrint("INFO:     node %d '%sx%d' added to nodelist\n",
                       AvlNodeCount, NodeList[nindex].N->Name,
                       ARange[0].TaskCount);
            }
        } else {
            ...
            DBG(7, fSCHED)
            DPrint(
                "INFO:     reservation does not fit into requested time range "
                "on node %s at %s (A[0]: %ld:%ld  R: %ld:%ld)\n",
                N->Name, MULToTString(MinStartTime - MSched.Time),
                ARange[0].StartTime, ARange[0].EndTime, RRange[0].StartTime,
                RRange[0].StartTime + J->WCLimit);

            DBG(7, fSCHED)
            DPrint(
                "INFO:     reservation does not fit into requested time range "
                "on node %s at %ld (A[1]: %ld:%ld  A[2]: %ld:%ld)\n",
                N->Name, RRange[0].StartTime - MSched.Time, ARange[1].StartTime,
                ARange[1].EndTime, ARange[2].StartTime, ARange[2].EndTime);
        }
    ...
    if (J->GReq != NULL) {
        ...
        if (MJobGetSNRange(J, J->GReq, MSched.GN, RRange, MAX_MRANGE, &Affinity,
                           NULL, ARange, NULL, NULL) == FAILURE) {
            DBG(7, fSCHED)
            DPrint(
                "INFO:     no reservation time found for job %s on node %s at "
                "%s\n",
                J->Name, (N != NULL) ? N->Name : "NULL",
                MULToTString(MinStartTime - MSched.Time));

            if (NodeMap != NULL) NodeMap[N->Index] = nmUnavailable;
        } else if (MRangeApplyLocalDistributionConstraints(ARange, J, N) ==
                   FAILURE) {
            /* no valid ranges available */
    ...
    if (NodeCount != NULL) *NodeCount = AvlNodeCount;
    ...
    if (MAvlNodeList != NULL) {
        ...
        if (J->Request.TC > AvlTaskCount) {
            DBG(6, fSCHED)
            DPrint(
                "ALERT:    inadequate tasks located for job %s at %s (%d < "
                "%d)\n",
                J->Name, MULToTString(MinStartTime - MSched.Time), AvlTaskCount,
                J->Request.TC);
        ...
        if ((J->Request.NC > AvlNodeCount) || (AvlNodeCount == 0)) {
            DBG(6, fSCHED)
            DPrint(
                "ALERT:    inadequate nodes located for job %s at %s (%d < "
                "%d)\n",
                J->Name, MULToTString(MinStartTime - MSched.Time), AvlNodeCount,
                J->Request.NC);
    ...
    if (MPar[0].MaxMetaTasks > 0) {
        ...
        MRLLimitTC(GRange, MRange, NULL, MPar[0].MaxMetaTasks);
    }

    if (TRange != NULL) memcpy(TRange, GRange, sizeof(mrange_t) * MAX_MRANGE);
...

{
    ...
    if (!(RangeType & (1 << rtcFeasible))) {
        ...
        for (rindex = 0; G[rindex].EndTime != 0; rindex++) {
            if ((G[rindex].TaskCount >= J->Request.TC) &&
                ...
                DBG(5, fSCHED)
                DPrint(
                    "INFO:     G[%d] has inadequate resources (T: %d < %d) || "
                    "(N: %d < %d)\n",
                    rindex, G[rindex].TaskCount, J->Request.TC,
                    G[rindex].NodeCount, J->Request.NC);
            }
        } /* END for (rindex) */
    ...
    for (rindex = 0; G[rindex].EndTime != 0; rindex++) {
        if ((TC != 0) && ((G[rindex].TaskCount < J->Request.TC) ||
                          (G[rindex].NodeCount < J->Request.NC) ||
    ...
    DBG(7, fSCHED) {
        for (findex = 0; G[findex].EndTime != 0; findex++) {
            DPrint("INFO:     G[%02d]  S: %ld  E: %ld  T: %3d  N: %d\n", findex,
                   G[findex].StartTime, G[findex].EndTime, G[findex].TaskCount,
                   G[findex].NodeCount);
        }
    }
...

{
    ...
    DBG(4, fSIM)
    DPrint("%s(%s,%d,Buf,%d)\n", FName, (J != NULL) ? J->Name : "NULL", Version,
           BufSize);
    ...
    switch (Version) {
        case 230:
            ...
            if ((StatCount++ % 100) == 0) {
                /*             NAME NRQ TRQ UID GID CPU STA CLAS QUE DIS STA COM
                 * ADA ARC OPS MCMP MEM DCMP DSK FEAT SQU TAS TPN QOS FLG ACC
                 * CMD  CC  BYP NSU PAR DPROCS DMEM DDISK DSWAP STARTDATE
                 * ENDDATE MNODE RM HOSTLINE RESERVATION RES1 RES2 RES3 */

                MUSNPrintF(
                    &BPtr, &BSpace,
                    "%-20s %3s %3s %8s %9s %7s %9s %11s %9s %9s %9s %9s %9s "
                    "%6s %6s %-2s %4s %-2s %6s %10s %9s %3s %4s %3s %5s %9s "
                    "%15s %4s %3s %7s %9s %6s %6s %6s %6s %9s %9s %s %s %s %s "
                    "%s %s %s\n",
                    "# JobID", "NRQ", "TRQ", "UserName", "GroupName", "WCLimit",
                    "JobState", "Class", "QueueTime", "DispatchT", "StartTime",
                    "CompleteT", "Network", "Arch", "Opsys", "MC", "Mem", "DC",
                    "Disk", "Features", "SQTime", "Tas", "TPNd", "QOS", "Flags",
                    "Account", "Command", "Comm", "Byp", "PSUtil", "Partition",
                    "DProcs", "DMem", "DDisk", "DSwap", "StartDate", "EndDate",
                    "MNode", "RMName", "HostList", "Reservation", "AppType",
                    "RES1", "RES2");
        ...
        default:
            ...
            DBG(4, fSIM)
            DPrint("ALERT:    cannot create version %d job trace header\n",
                   Version);

            break;
    } /* END switch(Version) */

    sprintf(ResString, "%7.2f", J->PSUtilized);

    tmpStartTime = J->StartTime;

    if (J->Ckpt != NULL) tmpStartTime -= J->Ckpt->StoredCPDuration;
    ...
    switch (Version) {
        case 230:
            ...
            for (cindex = 1; cindex < MAX_MCLASS; cindex++) {
                if (RQ[0]->DRes.PSlot[cindex].count > 0) {
                    sprintf(temp_str, "[%s:%d]", MAList[eClass][cindex],
                            RQ[0]->DRes.PSlot[cindex].count);
                    strcat(Classes, temp_str);
                }
            } /* END for (cindex) */

            if (Classes[0] == '\0') strcpy(Classes, NONE);
            ...
            if ((J->Flags & (1 << mjfHostList)) && (J->ReqHList != NULL) &&
                ...
                for (index = 1; J->ReqHList[index].N != NULL; index++) {
                    if (J->ReqHList[index].TC <= 0) break;
            ...
            for (index = 0; J->NodeList[index].N != NULL; index++) {
                if (J->NodeList[index].TC <= 0) break;

                if (AllocHosts[0] != '\0') strcat(AllocHosts, ":");

                strcat(AllocHosts, J->NodeList[index].N->Name);
            } /* END for (index) */

            if (AllocHosts[0] == '\0') strcpy(AllocHosts, NONE);

            sprintf(QOSString, "%s:%s",
                    (J->QReq != NULL) ? J->QReq->Name : NONE,
                    (J->Cred.Q != NULL) ? J->Cred.Q->Name : NONE);

            /*            NAME NRQ TRQ UID GID CPU STA CLAS QUET DIST STAT COMT
             * Net ARC OPS MCMP MEM DCMP DSK FEAT SQUT TAS TPN QREQ FLG ACC CMD
             * CC  BYP NSUTL PAR DPROCS DMEM DDISK DSWAP STARTDATE ENDDATE MNODE
             * RMINDEX HOSTLIST RESID ASNAME RES1 RES2 */

            MUSNPrintF(
                &BPtr, &BSpace,
                "%-20s %3d %3d %8s %9s %7ld %9s %11s %9lu %9lu %9lu %9lu %9s "
                "%6s %6s %-2s %4dM %-2s %6dM %10s %9lu %3d %4d %s %5s %9s %15s "
                "%4s %3d %s %9s %6d %6dM %6dM %6dM %9lu %9lu %s %s %s %s %s %s "
                "%s\n",
                J->Name, J->Request.NC, J->Request.TC,
                (J->Cred.U != NULL) ? J->Cred.U->Name : NONE,
                (J->Cred.G != NULL) ? J->Cred.G->Name : NONE, J->SpecWCLimit[0],
                MJobState[J->State], Classes, (unsigned long)J->SubmitTime,
                (unsigned long)J->DispatchTime, (unsigned long)tmpStartTime,
                (unsigned long)J->CompletionTime,
                MAList[eNetwork][RQ[0]->Network], MAList[eArch][RQ[0]->Arch],
                MAList[eOpsys][RQ[0]->Opsys], MComp[RQ[0]->MemCmp],
                RQ[0]->RequiredMemory, MComp[RQ[0]->DiskCmp],
                RQ[0]->RequiredDisk, Features,
                (unsigned long)J->SystemQueueTime, J->TaskCount,
                RQ[0]->TasksPerNode, QOSString,
                MUBMToString(J->SpecFlags, MJobFlags, '[', tmpLine, NONE),
                (J->Cred.A != NULL) ? J->Cred.A->Name : NONE,
                (J->E.Cmd != NULL) ? J->E.Cmd : NONE,
                (J->RMXString != NULL) ? J->RMXString : NONE, J->Bypass,
                ResString, MAList[ePartition][RQ[0]->PtIndex],
                RQ[0]->DRes.Procs, RQ[0]->DRes.Mem, RQ[0]->DRes.Disk,
                RQ[0]->DRes.Swap, (unsigned long)J->SpecSMinTime,
                (unsigned long)J->CMaxTime, AllocHosts,
                (J->RM != NULL) ? J->RM->Name : MRM[0].Name, ReqHList,
                (J->ResName[0] != '\0') ? J->ResName : NONE, MASGetName(J),
                NONE, NONE);
        ...
        default:
            ...
            DBG(4, fSIM)
            DPrint("ALERT:    cannot create version %d job trace record\n",
                   Version);
...

{
    ...
    fprintf(stderr,
            "RM: '%s'  Orig: '%s'  S: '%s'  L: '%s'  QMHost: '%s'  DD: '%s'\n",
            MRMType[rmtindex], ptr, SJobName, LJobName, tmpR.DefaultQMHost,
...

{
    ...
    for (index = 0; index < 16; index++) {
        sprintf(tmpLine, "node%02d", index);
...

{
    ...
    DBG(7, fSCHED)
    DPrint("%s(%s,C,%d,Buffer,BufSize)\n", FName,
           (J != NULL) ? J->Name : "NULL", AdjustJob);
    ...
    if ((DefC != NULL) && (DefC->L.JMax != NULL)) {
        ...
        if (tmpWCLimit == 0) tmpWCLimit = JL->SpecWCLimit[0];

        if (tmpNode == 0) tmpNode = JL->Request.NC;

        if (tmpProc == 0) tmpProc = JL->Request.TC;
    }

    /* NOTE:  need complete method of obtaining 'most constrainted' cred limit
     * (NYI) */

    /* check user wc limits */

    if ((J->Cred.U != NULL) && (J->Cred.U->L.JMax != NULL)) {
        JL = (mjob_t *)J->Cred.U->L.JMax;

        if (JL->SpecWCLimit[0] > 0) {
            tmpWCLimit = (tmpWCLimit > 0) ? MIN(tmpWCLimit, JL->SpecWCLimit[0])
                                          : JL->SpecWCLimit[0];
        }
    } /* END if ((J->Cred.U != NULL) && ...) */
    ...
    if ((J->Cred.G != NULL) && (J->Cred.G->L.JMax != NULL)) {
    ...
    if (tmpWCLimit > 0) {
        ...
        if ((J->SpecWCLimit[1] > tmpWCLimit) || (J->SpecWCLimit[1] == -1)) {
            if (AdjustJob == TRUE) {
                MJobSetAttr(J, mjaReqAWDuration, (void **)&tmpWCLimit, mdfLong,
                            mSet);
            } else {
                if (Buffer != NULL) {
    ...
    if (tmpNode > 0) {
        ...
        if (J->Request.NC > tmpNode) {
            if (AdjustJob == TRUE) {
                ...
                if (Buffer != NULL) {
                    sprintf(Buffer, "nodes too high (%d > %d)", J->Request.NC,
                            tmpNode);
                }
    ...
    if (tmpProc > 0) {
        ...
        if (J->Request.TC > tmpProc) {
            if (AdjustJob == TRUE) {
                ...
                if (Buffer != NULL) {
                    sprintf(Buffer, "procs too high (%d > %d)", J->Request.TC,
                            tmpProc);
                }
...

{
    ...
    DBG(6, fSIM)
    DPrint("%s(%s,Buf,%d)\n", FName, (J != NULL) ? J->Name : "NULL", BufSize);
    ...
    if ((J->Cred.C != NULL) &&
        (MJobCheckClassJLimits(J, J->Cred.C, FALSE, tmpLine, sizeof(tmpLine)) ==
         FAILURE)) {
        if (Buf != NULL) {
            MUSNPrintF(&BPtr, &BSpace,
                       "job does not meet constraints of class %s '%s'",
                       J->Cred.C->Name, tmpLine);
        }
...

{
    ...
    if (J->Cred.C == NULL) {
        if ((MSched.DefaultC != NULL) && (MSched.DefaultC->Name[0] != '\0')) {
...

{
    ...
    DBG(7, fSIM)
    DPrint("%s(%s,TL,%d)\n", FName, (J != NULL) ? J->Name : "NULL", TLSize);
...
int MJobSelectAdjacentNodes(
    ...
    int PIndex, int MinTasks, mnalloc_t NodeList[],
...

{
    ...
    for (nindex = 0; NodeList[nindex].N != NULL; nindex++) {
        ...
        if (N->PtIndex != PIndex) continue;

        memcpy(&tmpNodeList[MAX(0, N->SlotIndex)], &NodeList[nindex],
               sizeof(tmpNodeList[0]));
    } /* END for (nindex) */
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        if (tmpNodeList[nindex].TC > 0) {
            ...
            for (index = StartIndex; index < nindex; index++) {
                memcpy(&AdjNodeList[rindex][index - StartIndex],
                       &tmpNodeList[index], sizeof(tmpNodeList[0]));
            }
...

{
    ...
    if (MJobSelectAdjacentNodes(NodeList[0].N->PtIndex, J->TasksRequested,
                                MyNodeList, tmpNodeList) == FAILURE) {
...

{
    ...
    if (MReqGetFNL(J, RQ, &MPar[0], NULL, /* I */
                   tmpNL, NULL, NULL, MAX_MTIME, 0) == FAILURE) {
        DBG(2, fRM)
        DPrint("ALERT:    cannot locate feasible pseudo resources for %s:%d\n",
               J->Name, RQ->Index);
    ...
    MReqSetAttr(J, RQ, mrqaAllocNodeList, (void **)tmpNL, mdfOther, mSet);

    J->Request.NC += 1;
    J->Request.TC += tmpNL[0].TC;

    DBG(2, fRM)
    DPrint("INFO:     %d of %d tasks allocated for %s:%d from node %s\n",
           RQ->NodeList[0].TC, RQ->TaskCount, J->Name, RQ->Index,
           tmpNL[0].N->Name);
