...
int (*Function[])(char *, char *, int, char *, long *) = {
    ...
    UIQueueShow,   NULL,
    NULL,          ShowJobHold,
    UIStatShow,    UIStatClear,
    UIResCreate,   UIResDestroy,
    UIResShow,     MUISchedCtl,
    UIDiagnose,    NULL,
    NULL,          NULL,
    UIJobGetStart, NULL,
    UIShowGrid,    ShowBackfillWindow,
    UIShowConfig,  UIJobShow,
    UINodeShow,    UIJobStart,
    UIJobCancel,   UIChangeParameter,
    NULL,          UIShowEstStartTime,
    NULL,          NULL};

int (*MCRequestF[])(msocket_t *, long, char *) = {
    NULL,      NULL, NULL,    MUIGridCtl, MUIJobCtl, MUINodeCtl,
    MUIResCtl, NULL, MUIShow, NULL,       NULL};
...

{
    ...
    if ((args = strstr(S->RBuffer, MCKeyword[mckArgs])) == NULL) {
        ...
        sprintf(S->SBuffer, "%s%d %s%s\n", MCKeyword[mckStatusCode], scFAILURE,
                MCKeyword[mckArgs], "ERROR:    cannot locate command args");
    ...
    *args = '\0';
    ...
    if ((ptr = strstr(S->RBuffer, MCKeyword[mckCommand])) == NULL) {
        ...
        sprintf(S->SBuffer, "%s%d %s%s\n", MCKeyword[mckStatusCode], scFAILURE,
                MCKeyword[mckArgs], "ERROR:    cannot locate command");
    ...
    if ((ptr = strstr(S->RBuffer, MCKeyword[mckAuth])) == NULL) {
        ...
        sprintf(S->SBuffer, "%s%d %s%s\n", MCKeyword[mckStatusCode], scFAILURE,
                MCKeyword[mckArgs], "ERROR:    cannot locate authentication");
    ...
    if ((sindex = MUGetIndex(ServiceType, MService, 0, 0)) == 0) {
        DBG(3, fUI)
        DPrint("INFO:     service '%s' not handled in %s\n", ServiceType,
               FName);

        sprintf(Message, "ERROR:    cannot support service '%s'", ServiceType);

        sprintf(S->SBuffer, "%s%d %s%s\n", MCKeyword[mckStatusCode], scFAILURE,
                MCKeyword[mckArgs], Message);
    ...
    DBG(3, fUI)
    DPrint(
        "INFO:     client '%s' read (%ld bytes) initiating service call for "
        "'%s' (Auth: %s)\n",
        S->Name, S->SBufSize, MService[sindex], Auth);
    ...
    if (Auth[0] == '\0') {
        ...
        sprintf(S->SBuffer, "%s%d %s%s\n", MCKeyword[mckStatusCode], scFAILURE,
                MCKeyword[mckArgs], "ERROR:    cannot authenticate client");
    ...
    switch (sindex) {
        ...
        case svcMNodeCtl:
            ...
            if (!(FLAGS & (1 << fAdmin1))) {
                sprintf(Message,
                        "ERROR:    user '%s' is not authorized to run command "
                        "'%s'\n",
                        Auth, MService[sindex]);

                sprintf(S->SBuffer, "%s%d %s%s\n", MCKeyword[mckStatusCode],
                        scFAILURE, MCKeyword[mckArgs], Message);
            ...
            for (index = 0; index < MAX_MADMINHOSTS; index++) {
                if (MSched.AdminHost[index][0] == '\0') break;
            ...
            if (hostcheck == FALSE) {
                sprintf(Message,
                        "ERROR:    command '%s' cannot be executed from host "
                        "'%s'\n",
                        MService[sindex], S->Host);
        ...
        case svcRunJob:
            ...
            if (!(FLAGS & ((1 << fAdmin1) | (1 << fAdmin2)))) {
                sprintf(Message,
                        "ERROR:    user '%s' is not authorized to execute "
                        "command '%s'\n",
                        Auth, MService[sindex]);
        ...
        case svcMBal:
            ...
            if (sindex == svcMGridCtl) strcpy(Auth, S->Name);

            S->SBufSize = (long)sizeof(SBuffer);

            sprintf(tmpLine, "%s%d ", MCKeyword[mckStatusCode], scFAILURE);
            Align = (int)strlen(tmpLine) + (int)strlen(MCKeyword[mckArgs]);

            sprintf(S->SBuffer, "%s%*s%s", tmpLine, 16 - (Align % 16), " ",
                    MCKeyword[mckArgs]);
            ...
            if (sindex == svcShowTasks)
                scode =
                    UIShowUserTasks(args, S->SBuffer + HeadSize, &S->SBufSize);
            else if (Function[sindex] != NULL)
                scode = (*Function[sindex])(args, S->SBuffer + HeadSize, FLAGS,
                                            Auth, &S->SBufSize);
            else
                scode = FAILURE;
        ...
        default:
            ...
            DBG(2, fUI)
            DPrint(
                "WARNING:  unexpected service (%d) requested  (ignoring "
                "request)\n",
                sindex);
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(RBuffer,Buffer,%d,%s,BufSize)\n", FName, FLAGS, Auth);
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        if (J->Hold != 0) {
            DBG(2, fUI)
            DPrint("INFO:     job '%s' added to hold list\n", J->Name);
    ...
    DBG(2, fUI)
    DPrint("INFO:     ShowJobHold() buffer size: %d bytes\n",
           (int)strlen(Buffer));
...
int SetJobUserPrio(
    ...
    char *RBuffer, char *Buffer, int FLAGS, char *Auth, long *BufSize)

{
    ...
    MUSScanF(RBuffer, "%x%s %d", sizeof(Name), Name, &Priority);
    ...
    if (!(FLAGS & ((1 << fAdmin1) | (1 << fAdmin2)))) {
        if (strcasecmp(J->Cred.U->Name, Auth) != 0) {
            DBG(2, fUI)
            DPrint(
                "INFO:     user %s is not authorized to set user priority on "
                "job %s\n",
                Auth, J->Name);

            sprintf(
                Buffer,
                "user %s is not authorized to set user priority on job %s\n",
                Auth, J->Name);
    ...
    DBG(2, fUI)
    DPrint("INFO:     user priority on job '%s' set to %d\n", Name, Priority);
...

{
    ...
    MUSScanF(RBuffer, "%x%s", sizeof(GStat), GStat);

    /* determine statistics type requested */

    for (sindex = 0; MStatType[sindex] != 0; sindex++) {
        if (!strcasecmp(MStatType[sindex], GStat)) break;
    } /* END for (sindex) */

    if (MStatType[sindex] == NULL) {
        DBG(2, fUI)
        DPrint("INFO:     invalid stat type '%s' requested\n", GStat);
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(%s,Buffer,%d,%s,BufSize)\n", FName, RBuffer, FLAGS, Auth);

    if (MUSScanF(
            RBuffer,
            "%x%s %x%s %x%s %x%s %ld %ld %ld %d %d %x%s %d %d %x%s %x%s %x%s",
            sizeof(UserName), UserName, sizeof(GroupName), GroupName,
            sizeof(AccountName), AccountName, sizeof(PName), PName,
            &RequiredTime, &RequiredNodes, &RequiredProcs, &DMemory, &Memory,
            sizeof(MemCmp), MemCmp, &ShowSMP, &Flags, sizeof(ClassString),
            ClassString, sizeof(FeatureString), FeatureString, sizeof(QOSName),
            QOSName) == FAILURE) {
    ...
    DBG(4, fUI)
    DPrint(
        "INFO:     locating backfill window for u: %s  g: %s  a: %s  p: %s  c: "
        "%s  f: %s (%ld:%ld:%d)\n",
        UserName, GroupName, AccountName,
        MAList[ePartition][(SP != NULL) ? SP->Index : 0], ClassString,
        FeatureString, RequiredTime, RequiredNodes, Memory);
    ...
    if (ShowSMP == FALSE) /* NON-SMP */
    {
        ...
        if (Memory > 0) {
            for (mindex = 0; MComp[mindex] != NULL; mindex++) {
                if (!strcasecmp(MComp[mindex], MemCmp)) break;
            }

            if (MComp[mindex] == NULL) mindex = 0;
        }
        ...
        sprintf(Buffer,
                "backfill window (user: '%s' group: '%s' partition: %s) %s\n",
                UserName, GroupName, PName, MULToDString(&MSched.Time));

        PCount = 0;

        for (pindex = 1; pindex < MAX_MPAR; pindex++) {
            if (MPar[pindex].ConfigNodes == 0) continue;
        ...
        for (pindex = 0; pindex < MAX_MPAR; pindex++) {
            ...
            if ((SP != NULL) && (SP != P)) continue;

            if ((PCount == 1) && (SP == NULL) && (P->Index != 0)) break;
            ...
            DBG(7, fUI)
            DPrint("INFO:     checking window in partition %s\n", P->Name);
            ...
            while (
                MBFGetWindow(&BFNodeCount, &BFProcCount, BFNodeList, &BFTime,
                             MinTime, P, UserName, GroupName, AccountName,
                             mindex, Memory, MAX(1, RequiredTime), &DRes,
                             ClassString, FeatureString, QOSName,
                             (Flags & (1 << mcmVerbose)) ? tmpBuffer : NULL) ==
                SUCCESS) {
                DBG(4, fUI)
                DPrint(
                    "INFO:     located backfill window [%03d nodes : %06ld "
                    "seconds]\n",
                    BFNodeCount, BFTime);
                ...
                if ((BFTime >= RequiredTime) &&
                    ...
                    if (strcasecmp(P->Name, CurrentPName) != 0) {
                        if ((strcasecmp(CurrentPName, GLOBAL_MPARNAME)) &&
                            (index == 0)) {
        ...
        sprintf(temp_str, "%20s %5s %6s %7s %7s   %14s\n", "HostName", "Procs",
                "Memory", "Disk", "Swap", "Time Available");
        strcat(Buffer, temp_str);
        ...
        for (nindex = 0; nindex < MAX_MNODE; nindex++) {
            ...
            if ((N->State != mnsActive) && (N->State != mnsIdle)) continue;
            ...
            for (RRange[0].TaskCount = 1; RRange[0].TaskCount < N->CRes.Procs;
                 RRange[0].TaskCount = ARange[0].TaskCount + 1) {
                if (MJobGetSNRange(J, J->Req[0], N, RRange, 1, &Affinity, &Type,
                                   ARange, &DRes, NULL) == SUCCESS) {
                    ...
                    sprintf(temp_str, "%20s %5d %6d %7d %7d   %14s\n", N->Name,
                            DRes.Procs, DRes.Mem, DRes.Disk, DRes.Swap,
...
int UINodeStatShow(
    ...
    int Flags, char *Buffer, long *BufSize)

{
    ...
    sprintf(temp_str, "%8s %5s %7s %9s %7s %10s %7s\n", "Memory", "Proc",
            "Percent", "InitialNH", "Percent", "ProcHours", "Percent");
    strcat(Buffer, temp_str);
    ...
    for (cindex = 0; cindex < MAX_MRCLASS; cindex++) {
        if (MRClass[cindex].Memory == 0) break;

        if ((MRClass[cindex].Nodes == 0) || (averageload == 0)) {
            Average = 0.0;
            IAverage = 0.0;
        } else {
            Average = (double)MRClass[cindex].Workload / MRClass[cindex].Nodes /
                      averageload * 100.0;
            IAverage = (double)MRClass[cindex].InitialWorkload /
                       MRClass[cindex].Nodes / averageload * 100.0;
        }

        sprintf(temp_str, "%8d %5d %7.2f %9ld %7.2f %10ld %7.2f\n",
                MRClass[cindex].Memory, MRClass[cindex].Nodes,
                (double)MRClass[cindex].Nodes / totalnodes * 100.0,
                MRClass[cindex].InitialWorkload / 3600, IAverage,
                MRClass[cindex].Workload / 3600, Average);
        strcat(Buffer, temp_str);
    } /* END for (cindex) */

    sprintf(temp_str, "%8s %5d %7.2f %9d %7.2f %10d %7.2f\n\n", "TOTAL",
            totalnodes, 100.0, (int)(totalworkload / 3600), 100.0,
            (int)(totalworkload / 3600), 100.0);
    strcat(Buffer, temp_str);
    ...
    for (cindex = 0; cindex < MAX_MRCLASS; cindex++) {
        ...
        for (nindex = 0; nindex < MAX_MNODE; nindex++) {
            ...
            if (N->CRes.Mem == mem) {
                ...
                if (N->State == mnsBusy) MemTotalBusyCount++;
        ...
        if (MemTotalNodeCount != 0) {
            sprintf(temp_str,
                    "Summary:  %3d %4dMB Nodes  %6.2f%s Avail  %6.2f%s Busy  "
                    "(Current: %6.2f%s Avail  %6.2f%s Busy)\n",
                    MemTotalNodeCount, mem,
    ...
    if (SystemTotalTotalTime != 0) {
        sprintf(temp_str,
                "System Summary:  %3d Nodes  %6.2f%s Avail  %6.2f%s Busy  "
                "(Current: %6.2f%s Avail  %6.2f%s Busy)\n",
...

{
    ...
    if (MCRequestF[CIndex] == NULL) {
        DBG(3, fUI)
        DPrint("INFO:   service '%10.10s' not supported in %s\n", ptr, FName);

        snprintf(ErrMsg, MAX_MLINE, "cannot support command '%10.10s'", ptr);
    ...
    MUSScanF(ptr, "%x%s", sizeof(tmpAuth), tmpAuth);
...

{
    ...
    DBG(8, fUI)
    DPrint("%s(%d,%ld)\n", FName, (SS != NULL) ? SS->sd : -1, TimeLimit);

    if (SS == NULL) return (FAILURE);
    ...
    while ((now < UIDeadLine) || (MSched.Schedule == FALSE) ||
           (RMDataStaged == FALSE) ||
        ...
        if (RMDataStageInitiated == FALSE) {
            ...
            if (X.XRMDataIsStaging != (int (*)())0) {
                RMDataStaged =
                    ((*X.XRMDataIsStaging)(X.xd, NULL) == TRUE) ? FALSE : TRUE;

                if ((RMDataStaged == FALSE) &&
                    ((now - RMDataStageTime) > 120)) {
                    /* data stage is taking too long */

                    DBG(2, fUI)
                    DPrint("ALERT:    data stage is hung, retrying\n");
        ...
        DBG(4, fUI) {
            if (!((UIDeadLine - now) % 60)) {
                DBG(4, fUI)
                DPrint("INFO:     selecting next client (%ld seconds left)\n",
                       (UIDeadLine - now));
            }
        }
        ...
        while (MSUAcceptClient(SS, &C, HostName, (1 << TCP)) != FAILURE) {
            ...
            for (index = 0; index < MAX_MCLIENT; index++) {
                ...
                if (S->sd <= 0) {
                    ...
                    DBG(5, fUI)
                    DPrint("INFO:     client connected at sd %d\n", S->sd);
            ...
            if (index >= MAX_MCLIENT) {
                DBG(2, fUI)
                DPrint(
                    "WARNING:  cannot accept client from '%s' (MaxClient "
                    "reached)\n",
                    HostName);
                ...
                DBG(3, fUI)
                DPrint("INFO:     client socket from '%s' accepted\n",
                       HostName);
            }
        } /* END while (MSUAcceptClient() != FAILURE) */

        DBG(9, fUI)
        DPrint("INFO:     all clients connected.  servicing requests\n");
        ...
        for (index = 0; index < MAX_MCLIENT; index++) {
            ...
            DBG(11, fUI)
            DPrint("INFO:     checking read status of client[%d]  (%d)\n",
                   index, S->sd);
            ...
            if (S->sd > 0) {
                ...
                if (UIProcessCommand(S) == FAILURE) {
                    DBG(6, fUI)
                    DPrint("INFO:     could not service client request %d\n",
                           index);
                }
                ...
                if (MSched.Shutdown == TRUE) {
                    DBG(1, fUI)
                    DPrint(
                        "INFO:     shutting scheduler down (user request)\n");
        ...
        if ((MSched.Mode == msmSim) && (MSched.TimePolicy != mtpReal) &&
            (X.XSleep == (int (*)())0) && (MSched.Schedule == TRUE)) {
...

{
    ...
    DBG(3, fUI)
    DPrint("%s(Buffer,BufSize,%s)\n", FName, (UName != NULL) ? UName : "NULL");

    MUStrCpy(Buffer, "Displaying user information...\n", MAX_MBUFFER);

    /* create header */

    MUStrCat(Buffer, MUserShow(NULL, NULL, NULL, IFlags), MAX_MBUFFER);
    ...
    for (uindex = 0; uindex < MAX_MUSER + MAX_MHBUF; uindex++) {
        ...
        if (!strcmp(U->Name, ALL)) continue;

        DBG(8, fUI)
        DPrint("INFO:     checking User[%04d]: %s\n", uindex, U->Name);

        if ((UName != NULL) && strcmp(UName, NONE) && strcmp(UName, U->Name))
            continue;

        MUStrCat(Buffer, MUserShow(U, NULL, NULL, IFlags), MAX_MBUFFER);
    } /* END for (uindex) */
...

{
    ...
    DBG(3, fUI)
    DPrint("%s(Buffer,BufSize,%s,%d)\n", FName,
           (GName != NULL) ? GName : "NULL", IFlags);

    MUStrCpy(Buffer, "Displaying group information...\n", MAX_MBUFFER);

    /* create header */

    MUStrCat(Buffer, MGroupShow(NULL, NULL, NULL, IFlags), MAX_MBUFFER);
    ...
    for (gindex = 0; gindex < MAX_MGROUP + MAX_MHBUF; gindex++) {
        ...
        if (!strcmp(G->Name, ALL) || !strcmp(G->Name, "NOGROUP")) continue;

        DBG(8, fUI)
        DPrint("INFO:     checking Group[%04d]: %s\n", gindex, G->Name);

        if ((GName != NULL) && strcmp(GName, NONE) && strcmp(GName, G->Name))
            continue;

        MUStrCat(Buffer, MGroupShow(G, NULL, NULL, IFlags), MAX_MBUFFER);
    } /* END for (gindex) */
...

{
    ...
    DBG(3, fUI)
    DPrint("%s(Buffer,BufSize,%s,%d)\n", FName,
           (AName != NULL) ? AName : "NULL", IFlags);

    MUStrCpy(Buffer, "Displaying account information...\n", MAX_MBUFFER);

    /* create header */

    MUStrCat(Buffer, MAcctShow(NULL, NULL, NULL, IFlags), MAX_MBUFFER);
    ...
    for (aindex = 0; aindex < MAX_MACCT + MAX_MHBUF; aindex++) {
        ...
        if (!strcmp(A->Name, ALL)) continue;

        DBG(8, fUI)
        DPrint("INFO:     checking Acct[%04d]: %s\n", aindex, A->Name);

        if ((AName != NULL) && strcmp(AName, NONE) && strcmp(AName, A->Name))
            continue;

        MUStrCat(Buffer, MAcctShow(A, NULL, NULL, IFlags), MAX_MBUFFER);
    } /* END for (aindex) */
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(RBuffer,SBuffer,%d,%s,BufSize)\n", FName, AFlags, Auth);

    MUSScanF(RBuffer, "%d %x%s %d %x%s %x%s", &PLevel, sizeof(JobName), JobName,
             &Flags, sizeof(ResList), ResList, sizeof(NodeName), NodeName);

    if (MJobFind(JobName, &J, 0) != SUCCESS) {
        sprintf(SBuffer, "ERROR:  cannot locate job '%s'\n", JobName);

        DBG(3, fUI)
        DPrint("INFO:     cannot locate job '%s' in UIJobShow()\n", JobName);
    ...
    if (!(AFlags & ((1 << fAdmin1) | (1 << fAdmin2) | (1 << fAdmin3)))) {
        if (strcmp(J->Cred.U->Name, Auth) != 0) {
            DBG(2, fUI)
            DPrint(
                "INFO:     user %s is not authorized to check status of job "
                "'%s'\n",
                Auth, J->Name);

            sprintf(SBuffer,
                    "user %s is not authorized to check status of job '%s'\n",
                    Auth, J->Name);
    ...
    if (Flags & (1 << mcmParse)) {
        /* FORMAT:      JNAME STATE UNAME GNAME ANAME CNAME  WCLMT  QTIME  STIME
         * TCNT  NCNT */

        MUSNPrintF(&BPtr, &BSpace,
                   "%s=%s;%s=%s;%s=%s;%s=%s;%s=%s;%s=%s;%s=%ld;%s=%ld;%s=%ld;%"
                   "s=%d;%s=%d;",
                   MWikiJobAttr[mwjaName], J->Name, MWikiJobAttr[mwjaState],
                   MJobState[J->State], MWikiJobAttr[mwjaUName],
                   ...
                   MWikiJobAttr[mwjaWCLimit], J->WCLimit,
                   MWikiJobAttr[mwjaQueueTime], J->SubmitTime,
                   MWikiJobAttr[mwjaStartTime], J->StartTime,
                   MWikiJobAttr[mwjaTasks], J->Request.TC,
                   MWikiJobAttr[mwjaNodes], J->Request.NC);
    ...
    DBG(1, fUI)
    DPrint("job '%s'  State: %11s  EState:  %9s   QueueTime: %26s", J->Name,
           MJobState[J->State], MJobState[J->EState],
    ...
    if (MSched.Mode == msmSim) {
        sprintf(Line, "  (Recorded WallTime: %s)", MULToTString(J->SimWCTime));
    } else {
        Line[0] = '\0';
    }

    MUSNPrintF(&BPtr, &BSpace, "WallTime: %s of %s%s\n", Duration,
               MULToTString(J->SpecWCLimit[0]), Line);
    ...
    if (J->SpecWCLimit[2] > 0) {
        ...
        for (tindex = 0; J->SpecWCLimit[tindex] > 0; tindex++) {
            if (tindex > 0) strcat(Line, ",");

            sprintf(temp_str, " %ld", J->SpecWCLimit[tindex]);
            strcat(Line, temp_str);
        } /* END for (tindex) */

        MUSNPrintF(&BPtr, &BSpace, "%s", Line);
    }

    {
        ...
        MUSNPrintF(&BPtr, &BSpace,
                   "SubmitTime: %s  (Time Queued  Total: %s  Eligible: %s)\n\n",
                   MULToDString(&J->SubmitTime), tmpLine,
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        MUSNPrintF(&BPtr, &BSpace, "\nReq[%d]  TaskCount: %d  Partition: %s\n",
                   rqindex, RQ->TaskCount, MAList[ePartition][RQ->PtIndex]);
        ...
        if (RQ->TaskRequestList[2] > 0) {
            ...
            for (tindex = 1; RQ->TaskRequestList[tindex] > 0; tindex++) {
                ...
                sprintf(temp_str, " %d", RQ->TaskRequestList[tindex]);
                strcat(Line, temp_str);
            } /* END for (tindex) */
        ...
        if (RQ->RequiredProcs > 0) {
            sprintf(ProcLine, "Procs %s%d  ", MComp[RQ->ProcCmp],
                    RQ->RequiredProcs);
        } else {
            ProcLine[0] = '\0';
        }

        {
            ...
            MUSNPrintF(&BPtr, &BSpace,
                       "Network: %s  %sMemory %s %s  Disk %s %s  Swap %s %s\n",
                       MAList[eNetwork][RQ->Network], ProcLine,
                       MComp[RQ->MemCmp],
        ...
        MUSNPrintF(&BPtr, &BSpace, "Opsys: %s  Arch: %s  Features: %s\n",
                   MAList[eOpsys][RQ->Opsys], MAList[eArch][RQ->Arch],
                   MUMAList(eFeature, RQ->ReqFBM, sizeof(RQ->ReqFBM)));
        ...
        if (Flags & (1 << mcmVerbose)) {
            MUSNPrintF(
                &BPtr, &BSpace, "Exec:  '%s'  ExecSize: %d  ImageSize: %d\n",
                ((J->E.Cmd != NULL) && strcmp(J->E.Cmd, NONE)) ? J->E.Cmd : "",
                J->ExecSize, J->ImageSize);
        }
        ...
        if ((Flags & (1 << mcmVerbose)) || (RQ->DRes.Procs != 1) ||
            (RQ->DRes.Mem > 0) || (RQ->DRes.Disk > 0)) {
        ...
        if ((J->AWallTime > 0) && (Flags & (1 << mcmVerbose))) {
            if (J->MSUtilized > 0.01) {
                MUSNPrintF(&BPtr, &BSpace,
                           "Average Utilized Memory: %.2lf MB\n",
        ...
        if (((J->State == mjsStarting) || (J->State == mjsRunning)) &&
            (Flags & (1 << mcmVerbose)) && (J->Ckpt != NULL)) {
            char tmpLine[MAX_MLINE];

            strcpy(tmpLine, MULToTString(J->Ckpt->ActiveCPDuration));

            MUSNPrintF(&BPtr, &BSpace,
                       "Ckpt Walltime:  current: %s   stored: %s\n", tmpLine,
        ...
        if ((RQ->SetSelection != mrssNONE) || (RQ->SetType != mrstNONE) ||
            (RQ->SetList[0] != NULL)) {
            ...
            for (sindex = 0; RQ->SetList[sindex] != NULL; sindex++) {
                if (sindex > 0) MUStrNCat(&BPtr, &BSpace, ":");

                MUStrNCat(&BPtr, &BSpace, RQ->SetList[sindex]);
            }

            if (sindex == 0) MUStrNCat(&BPtr, &BSpace, NONE);
        ...
        if (MJOBISALLOC(J) == TRUE) {
            ...
            for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
                ...
                MUSNPrintF(&BPtr, &BSpace, "[%.20s:%d]", N->Name,
                           RQ->NodeList[nindex].TC);

                if ((nindex % 4) == 3) MUStrNCat(&BPtr, &BSpace, "\n");
            } /* END for (nindex)               */
            ...
            for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
                ...
                if ((N->State == mnsDown) ||
                    ...
                    MUSNPrintF(&BPtr, &BSpace,
                               "WARNING:  allocated node %16s is in state %s\n",
            ...
            if ((long)TaskCount != RQ->TaskCount) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  allocated tasks do not match requested "
                           "tasks (%d != %d)\n",
    ...
    if (J->Flags != 0) {
        ...
        for (index = 0; MJobFlags[index] != NULL; index++) {
            if (J->Flags & (1 << index)) {
                MUSNPrintF(&BPtr, &BSpace, " %s", MJobFlags[index]);
        ...
        if (J->AttrBM != 0) {
            ...
            for (index = 0; index < 32; index++) {
                if (J->AttrBM & (1 << index)) {
                    MUSNPrintF(&BPtr, &BSpace, " %s", MAList[eJFeature][index]);
                }
            } /* END for (index) */
        ...
        if ((J->Flags & (1 << mjfHostList)) && (J->ReqHList != NULL)) {
            if (J->ReqHList[0].N == NULL) {
                MUStrNCat(&BPtr, &BSpace,
                          "WARNING:  empty hostlist specified\n");
            } else {
    ...
    if (J->EState == mjsDeferred) {
        MUSNPrintF(&BPtr, &BSpace, "job is deferred.  Reason:  %s  (%s)\n",
               ...
               ((J->State != mjsRunning) || (J->EState != mjsStarting))) {
        MUSNPrintF(&BPtr, &BSpace,
                   "EState '%s' does not match current state '%s'\n",
                   MJobState[J->EState], MJobState[J->State]);
    }
    ...
    if (J->R != NULL) {
        ...
        MUSNPrintF(&BPtr, &BSpace,
                   "Reservation '%s' (%s -> %s  Duration: %s)\n", J->R->Name,
                   StartTime, EndTime, Duration);

        if (((J->State == mjsStarting) || (J->State == mjsRunning)) &&
            (J->StartTime > 0) && (J->StartTime != J->R->StartTime)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  reservation does not match job start time "
                       "(%ld != %ld)\n",
                       J->R->StartTime, J->StartTime);
        }
    ...
    if ((MSched.Time - J->ATime) > MAX(300, 2 * MSched.RMPollInterval)) {
        MUSNPrintF(&BPtr, &BSpace,
                   "WARNING:  job has not been detected in %s\n",
    ...
    if ((!Flags) & (1 << mcmFuture)) {
        ...
        if ((J->State != mjsIdle) && (J->State != mjsHold)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "job cannot run  (state, '%s', is not idle)\n",
                       MJobState[J->State]);
            ...
            if ((J->EState != mjsIdle) && (J->EState != mjsHold) &&
                (J->EState != mjsDeferred)) {
                MUSNPrintF(
                    &BPtr, &BSpace,
                    "job cannot run  (expected state, '%s', is not idle)\n",
                    MJobState[J->EState]);
        ...
        if ((J->SMinTime != 0) && (J->SMinTime > MSched.Time)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "job cannot run  (start date not reached for %s)\n",
                       MULToTString(J->SMinTime - MSched.Time));
        ...
        if (MJobCheckDependency(J, &DType, DValue) == FAILURE) {
            MUSNPrintF(&BPtr, &BSpace,
                       "job cannot run  (dependency %s %s not met)\n", DValue,
        ...
        if ((RQ->DRes.Procs * J->Request.TC) > MPar[0].ARes.Procs) {
            MUSNPrintF(
                &BPtr, &BSpace,
                "job cannot run  (insufficient idle procs:  %d available)\n",
                MPar[0].ARes.Procs);
    ...
    if ((RQ->DRes.Procs * J->Request.TC) > MPar[0].CRes.Procs) {
        MUSNPrintF(&BPtr, &BSpace,
                   "job cannot run in any partition  (insufficient total "
                   "procs:  %d available)\n",
                   MPar[0].CRes.Procs);
    }
    ...
    for (pindex = 0; pindex < MAX_MPAR; pindex++) {
...

        {
            ...
            if ((MQueueSelectAllJobs(tmpQ, ptHARD, P, SrcQ, FALSE, FALSE, FALSE,
                                     tmpBuffer) == FAILURE) ||
                (SrcQ[0] == -1)) {
                MUSNPrintF(&BPtr, &BSpace,
                           "cannot select job %s for partition %s (%s)\n\n",
                           J->Name, P->Name, tmpBuffer);

                continue;
            }

            if ((MQueueSelectJobs(SrcQ, DstQ, ptHARD, MAX_MNODE_PER_JOB,
                                  MAX_MTASK, MAX_MTIME, P->Index, Reason, FALSE,
                                  FALSE) == FAILURE) ||
                (DstQ[0] == -1)) {
                for (index = 0; index < MAX_MREJREASON; index++) {
                    if (Reason[index] != 0) break;
                }

                MUSNPrintF(&BPtr, &BSpace,
                           "cannot select job %s for partition %s (%s)\n\n",
                           J->Name, P->Name,
                           (index != MAX_MREJREASON) ? MAllocRejType[index]
                                                     : "UNKNOWN");
        ...
        if (MUNumListGetCount(J->StartPriority, RQ->DRes.PSlot, P->CRes.PSlot,
                              0, NULL) == FAILURE) {
            MUSNPrintF(&BPtr, &BSpace,
                       "required classes not configured in partition %s (%s)\n",
                       P->Name,
        ...
        if (MJobCheckPolicies(J, PLevel, 0, P, &PReason, Line, MSched.Time) ==
            FAILURE) {
            MUSNPrintF(&BPtr, &BSpace,
                       "job cannot run in partition %s.  (%s)\n", P->Name,
                       Line);
        ...
        for (index = 0; index < MAX_MNODE; index++) {
            ...
            if ((NodeName[0] != '\0') && strcmp(NodeName, NONE) &&
            ...
            if (((!Flags) & (1 << mcmFuture)) &&
                ...
                if (MJobCheckNRes(
                        ...
                        &TasksAllowed, MinSpeed, &rindex, &Affinity, NULL,
                        TRUE) == SUCCESS) {
                    ncount++;
        ...
        if ((!(Flags & (1 << mcmFuture))) &&
            ((long)P->ARes.Procs < (RQ->DRes.Procs * J->Request.TC))) {
            MUSNPrintF(&BPtr, &BSpace,
                       "job cannot run in partition %s (insufficient idle "
                       "procs available: %d < %d)\n\n",
                       P->Name, P->ARes.Procs, J->Request.TC * RQ->DRes.Procs);
        } else if (pcount < RQ->DRes.Procs * J->Request.TC) {
            int rcount;

            MUSNPrintF(&BPtr, &BSpace,
                       "job cannot run in partition %s (idle procs do not meet "
                       "requirements : %d of %d procs found)\n",
                       P->Name, pcount, RQ->DRes.Procs * J->Request.TC);
            ...
            for (rindex = 0; rindex < MAX_MREJREASON; rindex++) {
                if (Reason[rindex] == 0) continue;

                if (!(rcount % 5))
                    MUStrNCat(&BPtr, &BSpace, "\nRejection Reasons: ");

                MUSNPrintF(&BPtr, &BSpace, "[%-13s: %4d]",
                           MAllocRejType[rindex], Reason[rindex]);
            ...
            MUSNPrintF(&BPtr, &BSpace,
                       "job cannot run in partition %s (insufficient idle "
                       "nodes available: %d < %d)\n\n",
                       P->Name, ncount, J->Request.NC);
        } else {
            MUSNPrintF(&BPtr, &BSpace,
                       "job can run in partition %s (%d procs available.  %d "
                       "procs required)\n",
                       P->Name, pcount, J->Request.TC * RQ->DRes.Procs);
        }
    } /* END for (pindex) */

    if (IsDeferred == TRUE) J->EState = mjsDeferred;

    return (SUCCESS);
} /* END:  UIJobShow() */

/* <jobctl
 * action={cancel,diagnose,hold,modify,query,resume,start,submit,suspend}
 * job={jobexp} flags={VERBOSE}></jobctl> */
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(S,%ld,%s)\n", FName, CFlags, (Auth != NULL) ? Auth : "NULL");
    ...
    switch (S->WireProtocol) {
...

        {
            ...
            if (((ptr = strchr(S->RPtr, '<')) == NULL) ||
                ...
                (MXMLGetAttr(E, "action", NULL, Command, sizeof(Command)) ==
                 FAILURE) ||
                (MXMLGetAttr(E, "job", NULL, JobExp, sizeof(JobExp)) ==
                 FAILURE)) {
                DBG(3, fUI)
                DPrint("WARNING:  corrupt command '%100.100s' received\n",
                       S->RBuffer);
        ...
        default:
            ...
            if ((rc = MUSScanF(S->RPtr, "%x%s %x%s %x%s %x%s", sizeof(Command),
                               Command, sizeof(JobExp), JobExp,
                               sizeof(FlagString), FlagString,
                               sizeof(ArgString), ArgString)) != 4) {
                DBG(3, fUI)
                DPrint("WARNING:  corrupt command '%100.100s' received\n",
                       S->RBuffer);
    ...
    if ((CIndex = MUGetIndex(Command, MJobCtlCmds, FALSE, mjcmNONE)) ==
        mjcmNONE) {
        DBG(3, fUI)
        DPrint("WARNING:  unexpected subcommand '%s' received\n", Command);
    ...
    if (MSched.UseJobRegEx == TRUE) {
        ...
        sprintf(tmpLine, "^(%s)$", JobExp);
    }

    S->SBuffer[0] = '\0';

    if ((MUREToList(tmpLine, mxoJob, 0, JobList, &JobCount, S->SBuffer) ==
         FAILURE) ||
        (JobCount == 0)) {
    ...
    for (jindex = 0; jindex < JobCount; jindex++) {
        ...
        if (!(CFlags & ((1 << fAdmin1) | (1 << fAdmin2)))) {
            if (strcmp(J->Cred.U->Name, Auth)) {
                DBG(2, fUI)
                DPrint(
                    "INFO:     user %s is not authorized to perform operation "
                    "on job %s\n",
                    Auth, J->Name);

                sprintf(tmpLine,
                        "NOTE:  user %s is not authorized to perform operation "
                        "on job %s\n",
                        Auth, J->Name);
        ...
        switch (CIndex) {
            case mjcmCancel:
                ...
                switch (MSched.Mode) {
                    case msmSim:
                        ...
                        DBG(3, fUI)
                        DPrint("INFO:     job '%s' cancelled by user %s\n",
                               J->Name, Auth);
                    ...
                    default:
                        ...
                        if (MRMJobCancel(J,
                                         "MAUI_INFO:  job cancelled by user\n",
                                         NULL) == SUCCESS) {
                            DBG(3, fUI)
                            DPrint("INFO:     job '%s' cancelled by user %s\n",
                                   J->Name, Auth);
                            ...
                            DBG(3, fUI)
                            DPrint("ALERT:    cannot cancel job '%s'\n",
                                   J->Name);
...

            {
                ...
                if ((MXMLGetAttr(E, "attr", NULL, tmpAttr, sizeof(tmpAttr)) ==
                     FAILURE) ||
                    (MXMLGetAttr(E, "value", NULL, tmpVal, sizeof(tmpVal)) ==
                     FAILURE) ||
                ...
                switch (aindex) {
                    ...
                    case mjaReqCMaxTime:
                        ...
                        MUIJobSetAttr(J, aindex, tmpVal, CFlags, FlagString,
                                      ArgString, Auth, Msg);
...

            {
                ...
                /* NOTE:  process job attributes (ie, uname, starttime, diag,
                 * etc) */
                ...
                if (MJobAToString(J, aindex, tmpVal, mdfString) == FAILURE) {
                    sprintf(
                        Msg,
                        "<job name=\"%s\"><attr name=\"%s\">%s</attr></job>",
                        J->Name, tmpAttr, NONE);
                } else {
                    sprintf(
                        Msg,
                        "<job name=\"%s\"><attr name=\"%s\">%s</attr></job>",
                        J->Name, tmpAttr, tmpVal);
                }
...

{
    ...
    switch (S->WireProtocol) {
...

        {
            ...
            if (((ptr = strchr(S->RPtr, '<')) == NULL) ||
                ...
                (MXMLGetAttr(E, "action", NULL, Command, sizeof(Command)) ==
                 FAILURE) ||
                (MXMLGetAttr(E, "res", NULL, ResExp, sizeof(ResExp)) ==
                 FAILURE)) {
                DBG(3, fUI)
                DPrint("WARNING:  corrupt command '%100.100s' received\n",
                       S->RBuffer);
    ...
    if ((MUREToList(tmpLine, mxoRsv, 0, ResList, &ResCount, S->SBuffer) ==
         FAILURE) ||
        (ResCount == 0)) {
        sprintf(tmpLine,
                "ERROR:    invalid reservation expression received (%s)\n",
                ResExp);
...
int MUIJobPreempt(
    ...
    mjob_t *J,                                      /* I */
    char *Args, char *Auth, int PPolicy, char *Msg) /* O */
...
int MUIJobResume(
    ...
    mjob_t *J,                         /* I */
    char *Args, char *Auth, char *Msg) /* O */
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(S,%ld,%s)\n", FName, Flags, (Auth != NULL) ? Auth : "NULL");
    ...
    MUSScanF(S->RPtr, "%x%s", sizeof(Command), Command);
    ...
    if (cindex == 0) {
        DBG(3, fUI)
        DPrint("WARNING:  unexpected %s command '%s' received\n", FName,
               Command);
    ...
    switch (cindex) {
        ...
        case mcModify:
            ...
            MUSScanF(ptr, "%x%s %x%s", sizeof(ResID), ResID, sizeof(ResDesc),
                     ResDesc);

            /* locate standing reservation */

            for (srindex = 0; srindex < MAX_MSRES; srindex++) {
                if (SRes[srindex].Name[0] == '\0') continue;

                if (!strcmp(SRes[srindex].Name, ResID)) break;
            } /* END for (srindex) */

            if (srindex == MAX_MSRES) {
                DBG(3, fUI)
                DPrint("WARNING:  cannot locate standing reservation %s\n",
                       ResID);
            ...
            while (ptr != NULL) {
                if (!strncmp(ptr, "MAXTIME=", strlen("MAXTIME="))) {
                    if (!strcmp(ptr + strlen("MAXTIME="), "DEFAULT")) {
                        ...
                        MACLSet(SR->ACL, maDuration, &MaxTime, mcmpGE,
                                nmPositiveAffinity, (1 << maclRequired), 0);
                    ...
                    if (!strcmp(ptr + strlen("DAYS="), "DEFAULT")) {
                        ...
                        for (vindex = 0; MWeekDay[vindex] != NULL; vindex++) {
                            if ((strstr(ptr + strlen("DAYS="),
                                        MWeekDay[vindex]) != NULL) ||
                                (strstr(ptr + strlen("DAYS="),
                                        MWEEKDAY[vindex]) != NULL)) {
                                SR->Days |= (1 << vindex);

                                if (strstr(ptr + strlen("DAYS="), "ALL") !=
                                    NULL)
                                    SR->Days = 255;
                            }
                        } /* END for (vindex) */
                    ...
                    if (!strcmp(ptr + strlen("STARTTIME="), "DEFAULT")) {
                        ...
                        SR->StartTime =
                            MUTimeFromString(ptr + strlen("STARTTIME="));
                    ...
                    if (!strcmp(ptr + strlen("ENDTIME="), "DEFAULT")) {
                        ...
                        SR->EndTime =
                            MUTimeFromString(ptr + strlen("ENDTIME="));
                ...
                } else if (!strncmp(ptr, "CHARGEACCOUNT=",
                                    strlen("CHARGEACCOUNT="))) {
                    /* flush existing charges */

                    if ((SR->A != NULL) && (SR->R[0] != NULL) &&
                        (SR->R[0]->CIPS > 0.0)) {
                        R = SR->R[0];

                        if (MAMAllocRDebit(&MAM[0], R, &Reason, NULL) ==
                            SUCCESS) {
                            R->TIPS += R->CIPS;
                            R->TAPS += R->CAPS;
                            ...
                            DBG(1, fUI)
                            DPrint(
                                "ALERT:    cannot charge %6.2f PS to account "
                                "%s for reservation %s\n",
                                R->CIPS, R->A->Name, R->Name);
                        }
                    }
            ...
            for (dindex = 0; dindex < (MAX_SRES_DEPTH - 1); dindex++) {
                ...
                if (SRes[srindex].R[dindex] != NULL) {
                    ...
                    MSRGetCurrentValues(&SRes[srindex], &OSRes[srindex],
                                        &tmpSR);

                    MSRSetRes(&tmpSR, srindex, dindex);

                    while (SRes[srindex].R[dindex]->TaskCount <
                           tmpSR.TaskCount) {
        ...
        default:
            ...
            DBG(3, fUI)
            DPrint("ALERT:   unexpected command '%s' specified\n", Command);

            sprintf(S->SBuffer,
                    "\nERROR:    unexpected command '%s' specified (%d : %d)\n",
                    Command, cindex, mcModify);
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (N == (mnode_t *)1) continue;

        if ((BestN != NULL) && (N->Load >= BestN->Load)) continue;

        if (N->CRes.Procs <= 0) continue;
...

{
    ...
    const char *MNCSCmds[] = {NONE, "create", "destroy", "modify", NULL};

    enum { mncsNONE, mncsCreate, mncsDestroy, mncsModify };

    /* node attributes */

    const char *MNCMArgs[] = {NONE, "resource", "state", "trace", NULL};

    enum { mncmaNONE, mncmaResource, mncmaState, mncmaTrace };
    ...
    if (MSched.Mode != msmSim) {
        ...
        sprintf(S->SBuffer,
                "ERROR:    nodectl only enabled for simulation mode\n");
    ...
    rc = MUSScanF(S->RPtr, "%x%s %x%s %x%s", sizeof(SubCommand), SubCommand,
                  sizeof(Arg), Arg, sizeof(NodeExp), NodeExp);

    if (rc != 3) {
        MUISMsgAdd(S, "ERROR:    corrupt command received\n");

        sprintf(tmpLine,
                "ERROR:    invalid number of arguments received by command "
                "'%s' (%d)\n",
                S->RBuffer, rc);
    ...
    if ((scindex = MUGetIndex(SubCommand, MNCSCmds, FALSE, mncsNONE)) ==
        mncsNONE) {
    ...
    switch (scindex) {
        ...
        default:
            ...
            if (MUREToList(NodeExp, mxoNode, 0, NodeList, &NodeCount,
                           S->SBuffer) == FAILURE) {
                sprintf(tmpLine,
                        "ERROR:    invalid node expression received (%s)\n",
                        NodeExp);
    ...
    switch (scindex) {
        case mncsCreate:
            ...
            if ((aindex = MUGetIndex(ptr, MNCMArgs, FALSE, mncmaNONE)) ==
                mncmaNONE) {
            ...
            if ((ptr == NULL) ||
                ...
                sprintf(tmpLine,
                        "ERROR:    cannot process node create request\n");
        ...
        case mncsModify:
            ...
            /* ARG FORMAT:
             * {nodetype|partition|priority|resource|state}={idle|down|draining}
             */
            ...
            while (ptr != NULL) {
                if ((aindex = MUGetIndex(ptr, MNCMArgs, FALSE, mncmaNONE)) ==
                    ...
                    if (MSched.Mode == msmSim) {
                        ...
                        if ((aindex = MUGetIndex(ptr, MNodeAttr, FALSE,
                                                 mnaNONE)) == mnaNONE) {
                            sprintf(
                                temp_str,
                        ...
                        for (nindex = 0; NodeList[nindex] != -1; nindex++) {
                            ...
                            if (MNodeSetAttr(N, aindex, (void *)ptr, mdfString,
                                             mSet) == FAILURE) {
                                sprintf(temp_str,
                                        "ERROR:    cannot set attribute '%s' "
                                        "to '%s' on node '%s'\n",
                                        MNodeAttr[aindex], ptr, N->Name);
                                strcat(tmpLine, temp_str);
                ...
                switch (aindex) {
...

                    {
                        ...
                        const char *NCModSubC[] = {NULL, "inc", "dec", "set",
                                                   NULL};
                        ...
                        if ((ResID = MUStrTok(ptr, ".", &TokPtr2)) == NULL) {
                            sprintf(temp_str,
                                    "ERROR:    no resource ID specified\n");
                            strcat(S->SBuffer, temp_str);
                        ...
                        if ((cindex = MUGetIndex(ptr2, NCModSubC, FALSE,
                                                 ncmodscNONE)) == ncmodscNONE) {
                            sprintf(temp_str,
                                    "ERROR:    invalid modify subcommand "
                                    "specified (%s)\n",
                                    ptr2);
                            strcat(S->SBuffer, temp_str);
                        ...
                        for (nindex = 0; NodeList[nindex] != -1; nindex++) {
                            ...
                            if ((gindex == 0) || (gindex >= MAX_MGRES)) {
                                ...
                                sprintf(
                                    temp_str,
                                    "ERROR:    generic resource overflow\n");
                                strcat(S->SBuffer, temp_str);
                            ...
                            switch (cindex) {
                                case ncmodscInc:
                                    ...
                                    N->ARes.GRes[gindex].count =
                                        MIN(N->ARes.GRes[gindex].count,
                                            N->CRes.GRes[gindex].count);
                                ...
                                case ncmodscDec:
                                    ...
                                    N->ARes.GRes[gindex].count -= MAX(1, aval);
                                    N->ARes.GRes[gindex].count =
                                        MAX(N->ARes.GRes[gindex].count, 0);
                    ...
                    case mncmaState:
                        ...
                        if ((vindex = MUGetIndex(ptr, MNodeState, FALSE,
                                                 mnsNONE)) == mnsNONE) {
                            sprintf(
                                temp_str,
...
int UIQueueDiagnose(
    ...
    char *Buffer, long *BufSize, int PLevel, int PIndex)

{
    /* NOTE:  must be synchronized with MQueueSelectJobs() and
     * MJobCheckPolicies() */

    const char *FName = "UIQueueDiagnose";

    DBG(2, fUI)
    DPrint("%s(Buffer,BufSize,%s,%s)\n", FName, MPolicyMode[PLevel],
           MAList[ePartition][PIndex]);

    if (MQueueDiagnose(MJob, MUIQ, PLevel, &MPar[PIndex], Buffer, *BufSize) ==
        FAILURE) {
...

{
    ...
    if (S->SBuffer != NULL) {
        ...
        sprintf(S->SBuffer, "%s%d ", MCKeyword[mckStatusCode], scFAILURE);

        Align = (int)strlen(S->SBuffer) + (int)strlen(MCKeyword[mckArgs]);

        sprintf(temp_str, "%*s%s", 16 - (Align % 16), " ", MCKeyword[mckArgs]);
        strcat(S->SBuffer, temp_str);
    ...
    switch (S->WireProtocol) {
        case mwpXML:
            ...
            if (S->SE != NULL) MXMLDestroyE((mxml_t **)&S->SE);
...

{
    ...
    switch (S->WireProtocol) {
        ...
        {
            if (S->SE == NULL) MXMLCreateE((mxml_t **)&S->SE, "SchedResponse");
...

{
    ...
    if (MCfgProcessBuffer(RBuffer) == FAILURE) {
        DBG(2, fUI)
        DPrint("WARNING:    config line '%s' cannot be processed\n", RBuffer);
    ...
    DBG(2, fUI)
    DPrint("INFO:     config line '%s' successfully processed\n", RBuffer);
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(RBuffer,SBuffer,%d,%s,SBufSize)\n", FName, FLAGS, Auth);
    ...
    if (strstr(RBuffer, "XML") != NULL) DisplayMode = mwpXML;
    ...
    for (findex = 1; findex < MAX_MFRAME; findex++) {
        for (sindex = 1; sindex <= MAX_MSLOTPERFRAME; sindex++) {
            ...
            if (S->RMName[0] != '\0') {
                MaxSlotPerFrame =
                    MAX(MaxSlotPerFrame, sindex + MAX(1, S->SlotsUsed) - 1);
            }
        }
    } /* END for (findex) */
    ...
    if (MaxSlotPerFrame == 1) {
        ...
        for (nindex = 0; nindex < MAX_MNODE; nindex++) {
            ...
            MaxSlotPerFrame =
                MAX(MaxSlotPerFrame, N->SlotIndex + MAX(1, N->SlotsUsed) - 1);
        } /* END for (nindex) */
    ...
    switch (DisplayMode) {
        case mwpXML:
            ...
            MUSNPrintF(&BPtr, &BSpace, "<%s %s=\"%ld\" %s=\"%d\"/>",
                       MXO[mxoCluster], MClusterAttr[mcluaPresentTime],
                       MSched.Time, MClusterAttr[mcluaMaxSlot],
                       MaxSlotPerFrame);
        ...
        case mwpAVP:
            ...
            MUSNPrintF(&BPtr, &BSpace, "%s TIME=%ld MAXSLOT=%d\n", MXO[mxoSys],
                       MSched.Time, MaxSlotPerFrame);

            break;

        default:

            MUSNPrintF(&BPtr, &BSpace, "%s %ld %d\n", MXO[mxoSys], MSched.Time,
                       MaxSlotPerFrame);
    ...
    for (findex = 0; findex < MAX_MFRAME; findex++) {
        ...
        DBG(5, fUI)
        DPrint("INFO:     collecting status for frame %s\n", F->Name);

        switch (DisplayMode) {
            case mwpXML:

                MUSNPrintF(&BPtr, &BSpace, "<%s %s=\"%d\" %s=\"%s\"/>",
                           MXO[mxoFrame], MFrameAttr[mfraIndex], F->Index,
                           MFrameAttr[mfraName], F->Name);
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (N->RM->Type == mrmtLL) {
            ...
            if (!(N->Network & MUMAGetBM(eNetwork, "hps_us", mVerify)) &&
                (N->State == mnsIdle))
        ...
        switch (DisplayMode) {
            case mwpXML:
                ...
                MUSNPrintF(
                    ...
                    MXO[mxoNode], MNodeAttr[mnaNodeID], N->Name,
                    MNodeAttr[mnaNodeState], MNodeState[N->State],
                    MNodeAttr[mnaFrame], N->FrameIndex, MNodeAttr[mnaSlot],
                    N->SlotIndex, MNodeAttr[mnaSize], MAX(1, N->SlotsUsed));
            ...
            default:
                ...
                MUSNPrintF(&BPtr, &BSpace, "%s %s %s %d %d %d %s\n",
                           MXO[mxoNode], N->Name, MNodeState[N->State],
                           N->FrameIndex, N->SlotIndex, MAX(1, N->SlotsUsed),
                           NONE);
        ...
        switch (DisplayMode) {
            ...
            default:
                ...
                MUSNPrintF(&BPtr, &BSpace, "%s %s %d %d %ld %s %s %ld %s ",
                           MXO[mxoJob], J->Name, MJobGetProcCount(J),
                           J->NodeCount, J->WCLimit, J->Cred.U->Name,
                           J->Cred.G->Name, J->StartTime, MJobState[J->State]);

                DBG(3, fUI)
                DPrint("INFO:     adding job '%s' info (%d tasks) to buffer\n",
                       J->Name, J->TaskCount);

                for (nindex = 0; J->NodeList[nindex].N != NULL; nindex++) {
                    N = J->NodeList[nindex].N;

                    MUSNPrintF(&BPtr, &BSpace, "%d:%d;", N->FrameIndex,
                               N->SlotIndex);

                    DBG(4, fUI)
                    DPrint("INFO:     adding node '%s' of job '%s' to buffer\n",
                           N->Name, J->Name);
                } /* END for (nindex) */
    ...
    DBG(5, fUI)
    DPrint("INFO:     job state data collected (%d bytes)\n",
           (int)strlen(SBuffer));
...

{
    ...
    MUSScanF(RBuffer, "%x%s %x%s", sizeof(ConfigMode), ConfigMode,
             sizeof(tmpParameter), tmpParameter);
...

{
    ...
    while ((O = MOGetNextObject(&OP, CIndex, OS, OE, &NameP)) != NULL) {
        if ((CName != NULL) && strcmp(CName, NONE)) {
            if (strcmp(CName, NameP)) continue;
        }
...

{
    ...
    MStatClearUsage(0, (1 << mlActive) | (1 << mlIdle) | (1 << mlSystem),
                    FALSE);
...
int UIQueueShow(
    ...
    char *RBuffer, /* I */
    char *Buffer,  /* O */
    int FLAGS, char *Auth, long *BufSize)

{
    ...
    DBG(2, fUI)
    DPrint("%s(RBuffer,Buffer,%x,%s,BufSize)\n", FName, FLAGS, Auth);

    MUSScanF(RBuffer, "%d %x%s %d %x%s", &QueueMode, sizeof(PName), PName,
             &Flags, sizeof(UName), UName);
...

{
    ...
    MUSScanF(RBuffer, "%x%s %x%s", sizeof(JName), JName, sizeof(PName), PName);

    if (MParFind(PName, &P) == FAILURE) P = &MPar[0];
    ...
    if (J->R != NULL) {
        ...
        if (MJobGetEStartTime(J, &tmpP, &NodeCount, &TaskCount, MNodeList,
                              &Start) == FAILURE) {
            DBG(2, fUI)
            DPrint(
                "WARNING:  cannot determine earliest start time for job '%s'\n",
                J->Name);

            sprintf(
                Buffer,
                "ERROR:    cannot determine earliest start time for job '%s'\n",
                J->Name);
    ...
    sprintf(Buffer, "%s %ld %ld %ld %d %s",
            (J->Name[0] != '\0') ? J->Name : NONE, MSched.Time, Deadline,
            J->SpecWCLimit[0], MJobGetProcCount(J), BP->Name);

    DBG(3, fUI)
    DPrint(
        "INFO:     earliest start located for job %s in %s on partition %s at "
        "%s",
        J->Name, MULToTString(Start - MSched.Time), BP->Name,
...

{
    ...
    MUSScanF(RBuffer, "%x%s %x%s %x%s %x%s", sizeof(JName), JName, sizeof(tmp),
             tmp, sizeof(PName), PName, sizeof(SpecNodeList), SpecNodeList);

    if (MJobFind(JName, &J, 0) != SUCCESS) {
        sprintf(Buffer, "ERROR:  cannot locate job '%s'\n", JName);

        DBG(3, fUI)
        DPrint("INFO:     cannot locate job '%s' in %s()\n", FName, JName);
    ...
    if (!strcmp(tmp, MClientMode[mcmClear])) {
        ...
        if (!strcmp(SpecNodeList, NONE)) {
            sprintf(SpecNodeList, "%d", J->Request.TC);
        }

        if (MRMJobModify(J, "Resource_List", "neednodes", SpecNodeList, NULL) ==
            FAILURE) {
            sprintf(Buffer,
                    "ERROR:  cannot set hostlist for job '%s' to '%s'\n",
                    J->Name, SpecNodeList);
            ...
            if (J->SpecFlags & (1 << mjfHostList)) {
                ...
                if (J->ReqHList != NULL) MUFree((char **)&J->ReqHList);
            }

            sprintf(Buffer,
                    "INFO:  successfully set hostlist for job '%s' to '%s'\n",
                    J->Name, SpecNodeList);
    ...
    if (J->EState != mjsIdle) {
        sprintf(temp_str,
                "job '%s' is in expected state '%s' (expected state must be "
                "idle)\n",
                J->Name, MJobState[J->EState]);
        strcat(Buffer, temp_str);
    ...
    if ((ForceLevel < 2) || !strcmp(SpecNodeList, NONE)) {
        if (RQ->DRes.Procs * J->Request.TC > MPar[0].ARes.Procs) {
            sprintf(temp_str,
                    "job cannot run  (insufficient idle procs:  %d needed  %d "
                    "available)\n",
                    J->Request.TC * RQ->DRes.Procs, MPar[0].ARes.Procs);
            strcat(Buffer, temp_str);
    ...
    if (ForceLevel == 0) {
        ...
        if (MJobCheckPolicies(J, ptHARD, 0, (PS != NULL) ? PS : &MPar[0],
                              &PReason, NULL, MSched.Time) == FAILURE) {
    ...
    if (!strcmp(SpecNodeList, NONE)) {
        ...
        for (pindex = 0; pindex < MAX_MPAR; pindex++) {
            ...
            if (ForceLevel < 2) {
                if (MJobSelectMNL(J, P, NULL, MNodeList, NodeMap, 0) ==
                    FAILURE) {
                    continue;
                }
            } else {
                ...
                if (MReqGetFNL(J, RQ, P, NULL, tmpNodeList, &NodeCount,
                               &TaskCount, MAX_MTIME, 0) == FAILURE) {
                    continue;
                }
                ...
                for (index = 0; index < NodeCount; index++) {
                    if (tmpNodeList[index].N == NULL) break;
                    ...
                    TC = MNodeGetTC(N, &N->ARes, &N->CRes, &N->DRes, &RQ->DRes,
                                    MSched.Time);
        ...
        if (pindex == MAX_MPAR) {
            sprintf(temp_str,
                    "job cannot run (available nodes do not meet requirements "
                    "in any partition)\n");
            strcat(Buffer, temp_str);
        ...
        while (ptr != NULL) {
            if (MNodeFind(ptr, &N) != SUCCESS) {
                ...
                sprintf(temp_str, "%s", MSched.DefaultDomain);
                strcat(tmpName, temp_str);

                if (MNodeFind(tmpName, &N) != SUCCESS) {
                    sprintf(temp_str,
                            "ERROR:  cannot locate node '%s' in specified "
                            "nodelist\n",
                            ptr);
                    strcat(Buffer, temp_str);
        ...
        if (nodeindex < (int)(RQ->DRes.Procs * J->Request.TC)) {
            sprintf(temp_str,
                    "ERROR:  incorrect number of procs in hostlist. (%d "
                    "requested  %d specified)\n",
                    J->Request.TC * RQ->DRes.Procs, nodeindex);
            strcat(Buffer, temp_str);
    ...
    if (MJobAllocMNL(J, MNodeList, NodeMap, NULL,
                     MPar[RQ->PtIndex].NAllocPolicy, MSched.Time) == FAILURE) {
        DBG(2, fUI)
        DPrint("INFO:     cannot allocate nodes for job '%s'\n", J->Name);
    ...
    if (MJobStart(J) == FAILURE) {
        DBG(2, fUI)
        DPrint("INFO:     job '%s' cannot be started by user %s\n", J->Name,
               Auth);
    ...
    DBG(2, fUI)
    DPrint("INFO:     job '%s' started by user %s\n", J->Name, Auth);
    ...
    if (MSched.Mode == msmNormal) {
        MOSSyslog(LOG_INFO, "job %s started manually", J->Name);
    }
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(Buffer,BufSize,%s)\n", FName, (P != NULL) ? P->Name : "NULL");

    MUSNInit(&BPtr, &BSpace, Buffer, (int)*BufSize);

    MUSNPrintF(&BPtr, &BSpace,
               "diagnosing job priority information (partition: %s)\n\n",
               P->Name);
    ...
    for (index = 0; MUIQ[index] != -1; index++) {
        ...
        DBG(5, fUI)
        DPrint("INFO:     diagnosing priority for job '%s'\n", J->Name);
...
int MParDiagnose(
    ...
    char *Buffer, long *BufSize, char *DiagOpt)
...
int MUIJobSetAttr(
    ...
    mjob_t *J,  /* I */
    int AIndex, /* I */
    char *Val, int CFlags, char *FlagString, char *ArgString, char *AName,
    char *Msg)

{
    ...
    switch (AIndex) {
...

        {
            ...
            MJobSetAttr(J, mjaHold, (void **)&hbm, mdfInt,
...

        {
            ...
            if (Mode == mSet) {
                ...
                if (ReqCTime < BestCTime) {
                    ...
                    if (MJobGetEStartTime(J, &P, &NC, &TC, MNodeList,
                                          &StartTime) == FAILURE) {
                        ...
                        if (MJobGetEStartTime(J, &P, &NC, &TC, MNodeList,
                            ...
                            DBG(2, fUI)
                            DPrint(
                                "WARNING:  cannot find earliest start time for "
                                "job '%s'\n",
                                J->Name);

                            sprintf(Msg,
                                    "ERROR:    cannot find earliest start time "
                                    "for job '%s'\n",
                                    J->Name);
                ...
                if (CTimeIsValid == FALSE) {
                    ...
                    DBG(2, fUI)
                    DPrint(
                        "INFO:     requested deadline %s cannot be set for job "
                        "%s",
                        MULToDString(&ReqCTime), J->Name);

                    sprintf(Msg,
                            "requested deadline %s cannot be set for job %s",
                            MULToDString(&ReqCTime), J->Name);
                ...
                if (MResJCreate(J, MNodeList, StartTime, mjrDeadline, NULL) ==
                    FAILURE) {
                    sprintf(Msg,
                            "ERROR:  cannot enforce request completion time "
                            "deadline %s for job %s",
                            MULToDString(&ReqCTime), J->Name);
            ...
            MJobSetAttr(J, AIndex, (void **)&ReqCTime, mdfLong, Mode);
        } /* END BLOCK */
...

        {
            ...
            if ((tmpPrio < 0) || (tmpPrio > 1000)) {
                strcpy(Msg,
                       "ERROR:  system priority must be in the range 0 - 1000");
            ...
            MJobSetAttr(J, mjaSysPrio, (void **)&tmpPrio, mdfLong,
                        (strstr(FlagString, "unset") != NULL) ? mUnset : mSet);

            sprintf(Msg, "job system priority adjusted");

            if (MSched.Mode == msmNormal) {
                MOSSyslog(LOG_INFO, "system prio set to %ld on job %s", tmpPrio,
                          J->Name);
            }
        } /* END BLOCK */
...

        {
            ...
            if (MQOSGetAccess(J, Q, QAL, NULL) == FAILURE) {
                DBG(2, fUI)
                DPrint(
                    "INFO:     job %s does not have access to QOS %s (QAL: "
                    "%s)\n",
                    J->Name, Q->Name, MQOSBMToString(QAL));

                sprintf(Msg,
                        "ERROR:    job %s does not have access to QOS %s "
                        "(QOSList: %s)\n",
                        J->Name, Q->Name, MQOSBMToString(QAL));
            ...
            MJobSetAttr(J, mjaQOS, (void **)Val, mdfString, mSet);

            if (MSched.Mode == msmNormal) {
                MOSSyslog(LOG_INFO, "QOS changed from %s to %s on job %s",
                          J->Cred.Q->Name, Q->Name, J->Name);
            }
    ...
    switch (SchedMode) {
        ...
        case msctlStop:
            ...
            if ((int)SchedIteration > MSched.Iteration) {
                DBG(2, fUI)
                DPrint("INFO:     scheduling will stop in %s at iteration %d\n",
                       MULToTString(MSched.RMPollInterval *
                                    (SchedIteration - MSched.Iteration)),
                       (int)SchedIteration);
                ...
                DBG(2, fUI)
                DPrint(
                    "INFO:     scheduling will stop immediately at iteration "
                    "%d\n",
                    MSched.Iteration);
        ...
        case msctlResume:
            ...
            DBG(2, fUI)
            DPrint("INFO:     scheduling will resume immediately\n");
            ...
            if (strchr(Arg, 'I') != NULL) IgnoreToIteration = 0;
        ...
        case msctlReconfig:
            ...
            sprintf(Buffer,
                    "ERROR:  dynamic reconfiguration disabled.  restart server "
            ...
            *BufSize = strlen(Buffer);
            ...
            DBG(2, fUI)
            DPrint(
                "INFO:     scheduler will be reconfigured before next "
            ...
            sprintf(Buffer,
                    "scheduler will be reconfigured before next scheduling "
                    "iteration\n");
        ...
        case msctlKill:
            ...
            DBG(2, fUI)
            DPrint("INFO:     scheduler will shutdown immediately\n");
        ...
        case msctlNodeTable:
            ...
            for (nindex = 0; nindex < MAX_MNODE; nindex++) {
                ...
                if (MTraceBuildResource(N, DEFAULT_RESOURCE_TRACE_VERSION,
                                        tmpLine, sizeof(tmpLine)) == SUCCESS) {
        ...
        case msctlModify:
            ...
            if (MCfgProcessBuffer(Arg) == FAILURE) {
                ...
                sprintf(Buffer,
                        "ERROR:  specified parameters cannot be modified\n");
        ...
        case msctlList:
            ...
            if (strstr(Arg, ALL)) VerboseFlag = TRUE;
            ...
            if ((ptr[0] != '\0') && strcmp(ptr, NONE)) {
                ...
                if ((ptr2 = MUStrTok(ptr, "@", &TokPtr)) != NULL) {
                    ...
                    if ((ptr2 = MUStrTok(NULL, "@", &TokPtr)) != NULL) {
                        MRMSetAttr(&MRM[0], mrmaPort, (void **)ptr2, mdfString,
                                   0);
                    }
                }
            } /* END BLOCK */
        ...
        default:
            ...
            DBG(2, fUI)
            DPrint("WARNING:  received unexpected sched command '%d'\n",
                   SchedMode);

            sprintf(Buffer, "ERROR:    unexpected simulation command: '%d'\n",
                    SchedMode);

            if (BufSize != NULL) *BufSize = strlen(Buffer);
...

{
    ...
    DBG(2, fUI)
    DPrint("%s(%s,SBuffer,%d,%s,SBufSize)\n", FName, RBuffer, FLAGS, Auth);

    MUSScanF(RBuffer, "%d %x%s %x%s %x%s", &OType, sizeof(FlagString),
             FlagString, sizeof(PName), PName, sizeof(DiagOpt), DiagOpt);
    ...
    switch (OType) {
        ...
        default:
            ...
            DBG(2, fUI)
            DPrint("ALERT:    unexpected diagnose type, '%s'\n", MXO[OType]);

            sprintf(SBuffer, "ERROR:  unexpected diagnose type, '%s'\n",
                    MXO[OType]);

            if (SBufSize != NULL) *SBufSize = strlen(SBuffer);
...

{
    ...
    if (Access == FALSE) {
        DBG(2, fUI)
        DPrint("INFO:     user %s is not authorized to create reservation\n",
               Auth);
    ...
    /* Format:  <CLIENT_PRESENTTIME> <START_TIME> <END_TIME> <PNAME> <ULIST>
     * <GLIST> <ALIST> <CLIST> <QLIST> <RESID> <RLIST> <CANAME> <NODEEXP>
     * <FLIST> <NODESET> <FLAGS> */

    MUSScanF(RBuffer,
             "%ld %ld %ld %x%s %x%s %x%s %x%s %x%s %x%s %x%s %x%s %x%s %x%s "
             "%x%s %x%s %x%s %d %x%s",
             &ClientTime, &StartTime, &EndTime, sizeof(PName), PName,
             sizeof(UserList), UserList, sizeof(GroupList), GroupList,
             sizeof(AccountList), AccountList, sizeof(ClassList), ClassList,
             sizeof(QOSList), QOSList, sizeof(ResID), ResID,
             sizeof(ResourceList), ResourceList, sizeof(ChargeSpec), ChargeSpec,
             sizeof(Pattern), Pattern, sizeof(FeatureList), FeatureList,
             sizeof(NodeSetString), NodeSetString, sizeof(FlagString),
             FlagString, &MaxTasks, sizeof(JobFeatureList), JobFeatureList);

    DBG(5, fUI)
    DPrint(
        "INFO:     processing %s: CT: %ld  ST: %ld  ET: %ld  PI: %s  UL: %s  "
        "GL: %s  AL: %s  QL: %s  RL: %s  AN: %s  P: '%s'  F: '%s'  NS: %s  FL: "
        "'%s'\n",
        FName, ClientTime, StartTime, EndTime, PName, UserList, GroupList,
        AccountList, QOSList, ResourceList, ChargeSpec, Pattern, FeatureList,
        NodeSetString, FlagString);
    ...
    if (MSched.Mode == msmSim) {
        ...
        if (EndTime < MAX_MTIME) EndTime += (MSched.Time - ClientTime);
    }
    ...
    if (StartTime >= EndTime) {
        DBG(3, fUI)
        DPrint("WARNING:  cannot create reservation (invalid timeframe)\n");
    ...
    if (strcmp(FeatureList, NONE) != 0) {
        if (strchr(FeatureList, '|')) tmpRQ.ReqFMode = tlOR;
    ...
    if (strcmp(NodeSetString, NONE) != 0) {
        ...
        if ((ptr = MUStrTok(NodeSetString, ":", &TokPtr)) != NULL) {
            ...
            if ((ptr = MUStrTok(NULL, ":", &TokPtr)) != NULL) {
                ...
                sprintf(SBuffer,
                        "ERROR:    invalid node set expression: '%s'\n",
                        NodeSetString);
    ...
    if (strstr(Pattern, "TASKS") != NULL) {
        ...
        if (MResAllocateRE(Pattern, sizeof(Pattern), P->Index, NodeList, /* O */
                           &TaskCount, StartTime, EndTime, SBuffer, *SBufSize,
                           &tmpRQ) == FAILURE) {
            sprintf(temp_str,
        ...
        if (MUREToList(Pattern, mxoNode, P->Index, ObjList, &NodeCount,
                       SBuffer) == FAILURE) {
    ...
    if (TaskCount > 0) {
        ...
        for (nindex = 0; nindex < NodeCount; nindex++) {
            ...
            if ((P->Index != 0) && (NodeList[nindex].N->PtIndex != P->Index)) {
                continue;
            }

            if (MReqCheckResourceMatch(NULL, &tmpRQ, NodeList[nindex].N,
                                       &RIndex) == FAILURE) {
                continue;
            }
        ...
        DBG(4, fUI)
        DPrint(
            "INFO:     reservation constraints reduced nodelist from %d to %d "
            "nodes\n",
            NodeCount, NIndex);
    ...
    if ((strcmp(FeatureList, NONE) != 0) &&
        ...
        for (nindex = 0; nindex < NodeCount; nindex++) {
            if (PatSize < 100) {
                DBG(0, fSCHED)
                DPrint(
                    "ERROR:    regex buffer overflow creating reservation "
                    "'%s'\n",
                    ResID);
            ...
            if (nindex != 0) {
                MUSNPrintF(&PatPtr, &PatSize, "|%s", NodeList[nindex].N->Name);
            } else {
                MUSNPrintF(&PatPtr, &PatSize, "%s", NodeList[nindex].N->Name);
            }
        }
    ...
    if (ResPLevel != ptOFF) {
        ...
        if (MPolicyGetEStartTime(J, P, ResPLevel, &tmpTime) == FAILURE) {
            sprintf(temp_str,
                    "ERROR:    limits prevent reservation creation\n");
            strcat(SBuffer, temp_str);
    ...
    if (MResCreate(mrtUser, ACL, (A != NULL) ? A->Name : NULL, Flags, NodeList,
                   StartTime, EndTime, NodeCount, PC, Name, &ResP, Pattern,
                   &tmpRQ.DRes) == FAILURE) {
        DBG(3, fUI)
        DPrint("WARNING:  cannot create reservation for %s on %d node%s\n",
               Name, NodeCount, (NodeCount == 1) ? "" : "s");
    ...
    DBG(3, fUI)
    DPrint("INFO:     reservation '%s' created on %d node%s (%d tasks)\n",
           ResP->Name, NodeCount, (NodeCount == 1) ? "" : "s", TaskCount);
    ...
    sprintf(Message, "RESERVATIONCREATION:  %s USER %s %ld %ld %d %d\n",
            ResP->Name, Name, StartTime, EndTime, TaskCount, NodeCount);

    MSysRegEvent(Message, 0, 0, 1);

    if (MSched.Mode == msmNormal) {
        MOSSyslog(LOG_INFO, "reservation %s created on %d node%s", ResP->Name,
...
int MUIQueueShow(
    ...
    msocket_t *S, mxml_t *ReqE, mxml_t **RspE)

{
    ...
    if ((S == NULL) || (ReqE == NULL) || (RspE == NULL)) return (FAILURE);
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        if (J->State == mjsSuspended) SuspQ[sindex++] = J->Index;
    } /* END for (J) */
    ...
    while ((MAQ[aindex] != -1) || (SuspQ[sindex] != -1)) {
        if (MAQ[aindex] != -1) {
            ...
            if (J->State == mjsSuspended) continue;
        } else {
            J = MJob[SuspQ[sindex++]];
        }
        ...
        if ((P->Index > 0) && (RQ->PtIndex != P->Index)) continue;

        if (__MUIJobToXML(J, &JE, FALSE) == FAILURE) continue;

        DBG(3, fUI)
        DPrint("INFO:     %s:  adding active job[%03d] '%s' to buffer\n", FName,
               J->Index, J->Name);
    ...
    for (jindex = 0; MUIQ[jindex] != -1; jindex++) {
        ...
        if ((J->State == mjsRemoved) || (J->State == mjsSuspended)) continue;

        if (__MUIJobToXML(J, &JE, FALSE) == FAILURE) continue;

        DBG(3, fUI)
        DPrint("INFO:     %s:  adding eligible job[%03d] '%s' to buffer\n",
               FName, J->Index, J->Name);
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        ...
        if ((J->State == mjsRunning) || (J->State == mjsStarting) ||
            (J->State == mjsSuspended) || (J->IState != mjsNONE)) {
            continue;
        }
        ...
        if (IdleJob == TRUE) continue;

        if (__MUIJobToXML(J, &JE, FALSE) == FAILURE) continue;

        DBG(3, fUI)
        DPrint("INFO:     %s:  adding blocked job[%03d] '%s' to buffer\n",
               FName, J->Index, J->Name);
...
int __MUIJobToXML(
    ...
    mjob_t *J, mxml_t **JEP, int DoShowTaskList)

{
    ...
    if ((J == NULL) || (JEP == NULL)) return (FAILURE);
    ...
    *JEP = NULL;
    ...
    MXMLSetAttr(JE, (char *)MJobAttr[mjaState], (void *)MJobState[J->State],
                mdfString);

    if (J->Cred.U != NULL)
        MXMLSetAttr(JE, (char *)MJobAttr[mjaUser], (void *)J->Cred.U->Name,
                    mdfString);

    if (J->Cred.G != NULL)
        MXMLSetAttr(JE, (char *)MJobAttr[mjaGroup], (void *)J->Cred.G->Name,
                    mdfString);

    MXMLSetAttr(JE, (char *)MJobAttr[mjaStartTime], (void *)&J->StartTime,
                mdfLong);
    MXMLSetAttr(JE, (char *)MJobAttr[mjaSubmitTime], (void *)&J->SubmitTime,
                mdfLong);

    MXMLSetAttr(JE, (char *)MJobAttr[mjaReqAWDuration], (void *)&Duration,
                mdfLong);
    MXMLSetAttr(JE, (char *)MJobAttr[mjaReqProcs], (void *)&Procs, mdfInt);

    if (J->Request.NC > 0)
        MXMLSetAttr(JE, (char *)MJobAttr[mjaReqNodes], (void *)&J->Request.NC,
                    mdfInt);

    if ((J->Cred.Q != NULL) && (J->Cred.Q->Index != 0))
        MXMLSetAttr(JE, (char *)MJobAttr[mjaQOSReq], (void *)J->Cred.Q->Name,
                    mdfString);
    ...
    if (DoShowTaskList == TRUE) {
        ...
        for (nindex = 0; J->NodeList[nindex].N != NULL; nindex++) {
            ...
            sprintf(temp_str, "%d:%d;", N->FrameIndex, N->SlotIndex);
            strcat(tmpLine, temp_str);
        ...
        if (tmpLine[0] != '\0')
            MXMLSetAttr(JE, (char *)MJobAttr[mjaAllocNodeList], (void *)tmpLine,
                        mdfString);
...

{
    ...
    MUSNPrintF(&BPtr, &BSpace, "%ld %d %d %d %d %d %d\n", MSched.Time,
               P->URes.Procs, P->ARes.Procs, P->UpNodes, P->IdleNodes,
               P->ActiveNodes, TotalAllocatedProcs);
    ...
    while ((MAQ[aindex] != -1) || (SuspQ[sindex] != -1)) {
        ...
        if ((UName[0] != '\0') && strcmp(J->Cred.U->Name, UName)) continue;

        RQ = J->Req[0]; /* handle only first req of job */

        if ((P->Index > 0) && (RQ->PtIndex != P->Index)) continue;

        if (J->MasterJobName != NULL) continue;

        DBG(3, fUI)
        DPrint("INFO:     %s:  adding active job[%03d] '%s' to buffer\n", FName,
               J->Index, J->Name);

        /* Format:  <JOBNAME> <UNAME> <START TIME> <QUEUE TIME> <PROCS>
        ...
        sprintf(
            Line, "%16s %8s %8ld %8ld %4d %6ld %1s %2d %3ld\n", J->Name,
            (J->Cred.U != NULL) ? J->Cred.U->Name : "-", J->StartTime,
            J->SubmitTime, MJobGetProcCount(J), Limit,
            ((J->Cred.Q != NULL) && (J->Cred.Q->Index != 0)) ? J->Cred.Q->Name
                                                             : "-",
            (J->IState != mjsNONE) ? J->IState : J->State, J->StartPriority);
        ...
        if (BSpace < 3000) {
            DBG(1, fUI)
            DPrint(
                "ALERT:    active job buffer overflow in %s.  (job %d:  buffer "
                "size: %ld)\n",
                FName, aindex, *SBufSize);
        ...
        DBG(5, fUI)
        DPrint("INFO:     line: '%s' (%d)\n", Line, (int)strlen(SBuffer));
    } /* END while (MAQ[aindex] != -1) */
    ...
    for (jindex = 0; MUIQ[jindex] != -1; jindex++) {
        ...
        sprintf(Line, "%16s %8s %8ld %8ld %4d %6ld %1s %2d %3ld\n", J->Name,
                (J->Cred.U != NULL) ? J->Cred.U->Name : "-", J->StartTime,
                J->SubmitTime, J->Request.TC * RQ->DRes.Procs, J->WCLimit,
                ((J->Cred.Q != NULL) && (J->Cred.Q->Index != 0))
                    ? J->Cred.Q->Name
                    : "-",
                tmpState, J->StartPriority);
        ...
        if (BSpace < 3000) {
            DBG(1, fUI)
            DPrint(
                "ERROR:    idle job buffer overflow in %s.  (job %d:  buffer "
                "size: %ld)\n",
                FName, jindex, *SBufSize);
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        ...
        if ((MSched.Mode == msmSim) && (J->SMinTime > MSched.Time)) continue;
        ...
        sprintf(Line, "%16s %8s %8ld %8ld %4d %6ld %1s %2d %3ld\n", J->Name,
                J->Cred.U->Name, J->StartTime, J->SubmitTime,
                J->Request.TC * J->Req[0]->DRes.Procs, J->WCLimit,
                (J->Cred.Q->Index != 0) ? J->Cred.Q->Name : "-", tmpState,
                J->StartPriority);
    ...
    DBG(2, fUI)
    DPrint("INFO:     %s buffer size: %d bytes\n", FName, (int)strlen(SBuffer));
...
int MUIShow(
    ...
    msocket_t *S, long CFlags, char *Auth)

{
    ...
    switch (S->WireProtocol) {
...

        {
             ...
             * unavailable\n"><queue type="active"><job name="X" state="X"
             * ...></job></queue></Response></Reply> */
            ...
            if ((MXMLGetChild(E, "Request", NULL, &RE) == FAILURE) ||
                (MXMLGetAttr(RE, "object", NULL, OString, sizeof(OString)) ==
                 FAILURE) ||
...
int MUIJobDiagnose(
    ...
    char *Buffer, long *BufSize, int PIndex, char *DiagOpt, int Flags)

{
    ...
    DBG(2, fUI)
    DPrint("%s(Buffer,BufSize,%s,%s,%d)\n", FName, MAList[ePartition][PIndex],
           DiagOpt, Flags);

    /* show main job list */

    DBG(4, fUI)
    DPrint("INFO:     diagnosing job table (%d job slots)\n", MSched.M[mxoJob]);
    ...
    /* FORMAT:       NAME STATE PARTI PROCS QOS WCLI RESER MPR USR GRP ACC QTIM
     * NET OPS ARC MEM DSK CLAS FEAT */

    sprintf(Buffer,
            "%-18s %8.8s %3.3s %4.4s %3s %11s %1.1s %4s %8s %8s %8s %11s %8s "
            "%6s %6s %6s %6s %6s %11s %s\n\n",
            "Name", "State", "Par", "Proc", "QOS", "WCLimit", "Reservation",
            "Min", "User", "Group", "Account", "QueuedTime", "Network", "Opsys",
            "Arch", "Mem", "Disk", "Procs", "Class", "Features");

    TotalJobCount = 0;
    ActiveJobCount = 0;

    for (jindex = 1; jindex < MSched.M[mxoJob]; jindex++) {
        if ((MJob[jindex] == NULL) || (MJob[jindex] == (mjob_t *)1)) continue;

        if (MJob[jindex]->Name[0] == '\0') continue;

        J = MJob[jindex];

        if (strcmp(DiagOpt, NONE) && strcmp(DiagOpt, J->Name)) continue;
        ...
        if (Truncated == FALSE) {
            ...
            sprintf(ProcLine, "%s%d", MComp[RQ->ProcCmp], RQ->RequiredProcs);

            sprintf(MemLine, "%s%d", MComp[RQ->MemCmp], RQ->RequiredMemory);

            sprintf(DiskLine, "%s%d", MComp[RQ->DiskCmp], RQ->RequiredDisk);
            ...
            if (ClassLine[0] == '\0') {
                ...
                for (index = 0; ClassLine[index] != '\0'; index++) {
                    if ((ClassLine[index] == ' ') &&
                        isdigit(ClassLine[index + 1]))
                        ClassLine[index] = ':';
                }
            }

            sprintf(temp_str,
                    "%-18s %8.8s %3.3s %4d %3.3s %11s %1d %4d %8.8s %8.8s "
                    "%8.8s %11s %8s %6s %6s %6s %6s %6s %11s ",
                    J->Name, MJobState[J->State],
                    MAList[ePartition][RQ->PtIndex], MJobGetProcCount(J),
                    (J->Cred.Q != NULL) ? J->Cred.Q->Name : "-", WCLimit,
                    J->R ? 1 : 0, MJobGetProcCount(J), J->Cred.U->Name,
                    J->Cred.G->Name,
                    (J->Cred.A != NULL) ? J->Cred.A->Name : "-", QueuedTime,
                    MAList[eNetwork][RQ->Network], MAList[eOpsys][RQ->Opsys],
                    MAList[eArch][RQ->Arch], MemLine, DiskLine, ProcLine,
                    ClassLine);
            strcat(Buffer, temp_str);
        ...
        DBG(5, fUI)
        DPrint("INFO:     Job[%03d]: '%s'  TotalJobCount: %3d\n", jindex,
               MJob[jindex]->Name, TotalJobCount);
        ...
        if ((J->State == mjsRunning) || (J->State == mjsStarting)) {
            if ((double)RQ->URes.Procs / 100.0 > (double)RQ->DRes.Procs) {
                if (Truncated == FALSE) {
                    sprintf(temp_str,
                            "WARNING:  job '%s' utilizes more procs than "
                            "dedicated (%.2lf > %d)\n",
                            J->Name, RQ->URes.Procs / 100.0, RQ->DRes.Procs);
                    strcat(Buffer, temp_str);
                }
            }
            ...
            if ((RQ->URes.Mem > RQ->DRes.Mem) && (RQ->DRes.Mem > 0)) {
                if (Truncated == FALSE) {
                    sprintf(temp_str,
                            "WARNING:  job '%s' utilizes more memory than "
                            "dedicated (%d > %d)\n",
                            J->Name, RQ->URes.Mem, RQ->DRes.Mem);
                    strcat(Buffer, temp_str);
                }
            }

            if ((RQ->LURes.Procs > 0) && (J->PSUtilized > 0.01) &&
                (((J->PSUtilized / RQ->LURes.Procs) > 1.1) ||
                 ((J->PSUtilized / RQ->LURes.Procs) < .9))) {
                if (Truncated == FALSE) {
                    /*
                              sprintf(temp_str,"WARNING:  job '%s'
                       unsynchronized proc usage stats (RM: %ld  Sample:
                       %ld)\n",
                                J->Name,
                                (long)RQ->LURes.Procs,
                                (long)J->PSUtilized);
            ...
            if (((J->AWallTime + J->SWallTime) - (MSched.Time - J->StartTime)) >
                MSched.RMPollInterval << 1) {
                if (Truncated == FALSE) {
                    /*
                              sprintf(temp_str,"WARNING:  job '%s' walltime
                       tracking is corrupt (W:%ld + S:%ld) != (P:%ld -
                       S:%ld)\n",
                                J->Name,
                                J->AWallTime,
                                J->SWallTime,
        ...
        if ((J->State != J->EState) && (J->EState != mjsDeferred)) {
            if (Truncated == FALSE) {
                if ((J->State != mjsRunning) || (J->EState != mjsStarting)) {
                    sprintf(temp_str,
                            "WARNING:  job '%s' state '%s' does not match "
                            "expected state '%s'\n",
        ...
        if ((J->State == mjsRunning) || (J->State == mjsStarting)) {
            ...
            if ((MSched.Time - J->StartTime) > J->WCLimit) {
                if (Truncated == FALSE) {
                    ...
                    sprintf(temp_str,
                            "WARNING:  job '%s' has exceeded wallclock limit "
                            "(%s > %s) \n",
                            J->Name, MULToTString(MSched.Time - J->StartTime),
                            WCLimit);
                    strcat(Buffer, temp_str);
                }
            } /* END if MSched.Time */
    ...
    if (!strcmp(DiagOpt, NONE)) {
        ...
        for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
            ...
            DBG(5, fUI)
            DPrint("INFO:     Job[%03d]: '%s'  link %3d (next: %p)\n", jindex,
                   J->Name, LinkJobCount, J->Next);
        } /* END for (jindex) */
        ...
        for (jindex = 0; MAQ[jindex] != -1; jindex++) {
            ...
            if (((long)strlen(Buffer) > (*BufSize - 300)) &&
                (Truncated == FALSE)) {
                DBG(2, fUI)
                DPrint("ALERT:    buffer overflow in %s()\n", FName);
            ...
            DBG(4, fUI)
            DPrint(
                "INFO:     diagnosing node consistency of running Job[%03d] "
                "--> [%02d] '%s'\n",
                jindex, MAQ[jindex], J->Name);
            ...
            for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                ...
                for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
                    ...
                    if (MUBMCheck(N->PtIndex, J->PAL) == FAILURE) {
                        if (Truncated == FALSE) {
                            sprintf(temp_str,
                                    "WARNING:  job '%s' with partition mask %s "
                                    "has node %s allocated from partition %s\n",
                                    J->Name,
                                    (J->PAL[0] == 0)
                                        ? ALL
                                        : MUListAttrs(ePartition, J->PAL[0]),
                                    N->Name, MAList[ePartition][N->PtIndex]);
                            strcat(Buffer, temp_str);
                        }
                    }

                    if ((N->State != mnsBusy) && (N->State != mnsActive) &&
                        (N->State != mnsDraining) && (N != MSched.GN) &&
                        ((MSched.Time - J->StartTime) > 300)) {
                        if (Truncated == FALSE) {
                            sprintf(temp_str,
                                    "WARNING:  active job '%s' has inactive "
                                    "node %s allocated for %s (node state: "
                                    "'%s')\n",
                                    J->Name, N->Name,
            ...
            if (NodeCount != J->NodeCount) {
                if (Truncated == FALSE) {
                    sprintf(temp_str,
                            "WARNING:  active job '%s' has inconsistent node "
                            "allocation  (nodes: %d  nodelist size: %d)\n",
                            J->Name, J->NodeCount, NodeCount);
                    strcat(Buffer, temp_str);
                }
            }
            ...
            if (pcount != MJobGetProcCount(J)) {
                if (Truncated == FALSE) {
                    sprintf(temp_str,
                            "WARNING:  active job %s has inconsistent proc "
                            "allocation  (procs: %d  nodelist procs: %d)\n",
                            J->Name, MJobGetProcCount(J), pcount);
                    strcat(Buffer, temp_str);
                }
            }
        ...
        if (TotalJobCount != LinkJobCount) {
            sprintf(temp_str,
                    "WARNING:  job table is corrupt (total jobs (%d) != linked "
                    "jobs (%d))\n",
                    TotalJobCount, LinkJobCount);
            strcat(Buffer, temp_str);
        }

        if (ActiveJobCount != RunListCount) {
            sprintf(temp_str,
                    "WARNING:  active job table is corrupt (active jobs (%d) "
                    "!= active queue size (%d))\n",
                    ActiveJobCount, RunListCount);
            strcat(Buffer, temp_str);
        }
...

{
    ...
    MUSScanF(RBuffer, "%x%s %ld", sizeof(NodeName), NodeName, &Flags);
    ...
    if (!strcmp(NodeName, "DEFAULT")) {
        ...
        if (MNodeFind(NodeName, &N) != SUCCESS) {
            ...
            DBG(3, fUI)
            DPrint("INFO:     cannot locate node '%s' in %s()\n", NodeName,
                   FName);
...
int UIJobCancel(
    ...
    char *RBuffer, /* I */
    char *SBuffer, /* O */
    int FLAGS, char *Auth, long *SBufSize)
    ...
    while (ptr != NULL) {
        ...
        if (!(FLAGS & ((1 << fAdmin1) | (1 << fAdmin2)))) {
            if (strcmp(J->Cred.U->Name, Auth) != 0) {
                DBG(2, fUI)
                DPrint(
                    "INFO:     user %s is not authorized to cancel job '%s'\n",
                    Auth, J->Name);

                MUSNPrintF(&BPtr, &BSpace,
                           "user %s is not authorized to cancel job '%s'\n",
                           Auth, J->Name);
...

{
    ...
    DBG(3, fUI)
    DPrint("%s(SBuffer,BufSize,%s,%s,%d)\n", FName, MAList[ePartition][PIndex],
           DiagOpt, Flags);
    ...
    DBG(4, fUI)
    DPrint("INFO:     diagnosing node table (%d slots)\n", MAX_MNODE);
    ...
    MUSNPrintF(&BPtr, &BSpace,
               "%-20s %8s %7s %13s %13s %13s %5s %6s %6s %3.3s %6s %3.3s %-30s "
               "%-30s %-22s\n\n",
               "Name", "State", " Procs ", "   Memory    ", "    Disk     ",
               "    Swap     ", "Speed", "Opsys", "Arch", "Partition", "Load",
               "Res", "Classes", "Network", "Features");
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (strcmp(DiagOpt, NONE) && strcmp(DiagOpt, N->Name)) continue;

        if ((PIndex != 0) && (PIndex != N->PtIndex)) continue;
        ...
        for (rindex = 0; rindex < MSched.ResDepth; rindex++) {
            if (N->R[rindex] == (mres_t *)1) continue;

            if (N->R[rindex] == NULL) break;
        ...
        MUSNPrintF(&BPtr, &BSpace,
                   (Flags & (1 << mcmVerbose))
                       ? "%-20s %8s %3d:%-3d %6d:%-6d %6d:%-6d %6d:%-6d %5.2f "
                         "%6s %6s %3s %6.2f %3d "
                       : "%-20.20s %8.8s %3d:%-3d %6d:%-6d %6d:%-6d %6d:%-6d "
                         "%5.2f %6.6s %6.6s %3.3s %6.2f %3.3d ",
                   N->Name, MAList[eNodeState][N->State],
                   MIN(N->ARes.Procs, N->CRes.Procs - N->DRes.Procs),
                   N->CRes.Procs, MIN(N->ARes.Mem, N->CRes.Mem - N->DRes.Mem),
                   N->CRes.Mem, MIN(N->ARes.Disk, N->CRes.Disk - N->DRes.Disk),
                   N->CRes.Disk, MIN(N->ARes.Swap, N->CRes.Swap - N->DRes.Swap),
                   N->CRes.Swap, N->Speed, MAList[eOpsys][N->ActiveOS],
                   MAList[eArch][N->Arch], MAList[ePartition][N->PtIndex],
                   N->Load, NodeReservationCount);
        ...
        sprintf(Line, (Flags & (1 << mcmVerbose)) ? "%-30s" : "%-30.30s",
        ...
        if (N->State == mnsUnknown) {
            continue;
        } else if ((N->State == mnsActive) || (N->State == mnsIdle) ||
                   (N->State == mnsBusy)) {
            /* check resource utilization */

            if (N->ARes.Procs < (N->CRes.Procs - N->DRes.Procs)) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' has more processors utilized "
                           "than dedicated (%d > %d)\n",
                           N->Name, N->CRes.Procs - N->ARes.Procs,
                           N->DRes.Procs);
            }
            ...
            if ((N->ARes.Mem < (N->CRes.Mem - N->DRes.Mem)) &&
                (N->DRes.Mem > 0)) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' has more memory utilized than "
                           "dedicated (%d > %d)\n",
                           N->Name, N->CRes.Mem - N->ARes.Mem, N->DRes.Mem);
            }

            if ((N->ARes.Swap <
                 (N->CRes.Swap - N->DRes.Swap - MIN_OS_SWAP_OVERHEAD)) &&
                (N->DRes.Swap > 0)) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' has more swap utilized than "
                           "dedicated (%d > %d)\n",
                           N->Name, N->CRes.Swap - N->ARes.Swap, N->DRes.Swap);
            }

            if ((N->ARes.Disk < (N->CRes.Disk - N->DRes.Disk)) &&
                (N->DRes.Disk > 0)) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' has more disk utilized than "
                           "dedicated (%d > %d)\n",
                           N->Name, N->CRes.Disk - N->ARes.Disk, N->DRes.Disk);
            }
        } /* END else if ((N->State == mnsActive) || (N->State == mnsIdle) ||
             (N->State == mnsBusy)) */
        ...
        if ((MSched.Time > N->ATime) && ((MSched.Time - N->ATime) > 600)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  node '%s' has not been updated in %s\n",
                       N->Name, MULToTString(MSched.Time - N->ATime));
        }

        /* check node memory */

        if ((N->FrameIndex != -1) && (MFrame[N->FrameIndex].Memory > 0) &&
            (N->CRes.Mem != MFrame[N->FrameIndex].Memory)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  node '%s' memory (%d MB) does not match "
                       "frame %d memory (%d MB)\n",
                       N->Name, N->CRes.Mem, N->FrameIndex,
        ...
        if ((N->CRes.Swap > 0) && (N->ARes.Swap < MIN_SWAP)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  node '%s' swap space (%d MB) is low\n",
                       N->Name, N->ARes.Swap);
        }
        ...
        if ((N->State != N->EState) && (SameStateTime > 120)) {
            if ((N->State == mnsDrained) && (N->State == mnsDraining)) {
                ...
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' state '%s' does not match "
                           "expected state '%s'.  sync deadline in %s at %s",
                           N->Name, MAList[eNodeState][N->State],
        ...
        if (N->State == mnsBusy) {
            for (jindex = 0; MAQ[jindex] != -1; jindex++) {
                ...
                for (NodeIndex = 0; J->NodeList[NodeIndex].N != NULL;
                     NodeIndex++) {
            ...
            if (JobIndex == -1) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' is busy but not assigned to an "
                           "active job\n",
                           N->Name);
        ...
        if (N->State == mnsIdle) {
            if (N->ARes.Procs != N->CRes.Procs) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  processor mismatch on idle node %s (%d "
                           "available  %d configured)\n",
                           N->Name, N->ARes.Procs, N->CRes.Procs);
            }
        }
        ...
        if ((N->CRes.Disk > 0) && (N->ARes.Disk <= 0) &&
            (N->State != mnsBusy)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  node '%s' disk space is full (%d of %d MB "
                       "available)\n",
                       N->Name, N->ARes.Disk, N->CRes.Disk);
        }
        ...
        if (strcmp(MAList[eArch][N->Arch], "RS6000") == 0) {
            if (!(N->Network & MUMAGetBM(eNetwork, "ethernet", mVerify)) &&
                (N->State != mnsDown)) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' is missing ethernet adapter "
                           "(check JM?)\n",
                           N->Name);
            }
            ...
            if (!(N->Network & MUMAGetBM(eNetwork, "hps_ip", mVerify)) &&
                (N->State != mnsDown)) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' is missing IP switch adapter "
                           "(check switch/JM?)\n",
                           N->Name);
            }

            if ((N->State != mnsBusy) && (N->State != mnsDraining) &&
                (N->State != mnsDown) && (N->State != mnsReserved) &&
                (SameStateTime > 300) &&
                !(N->Network & MUMAGetBM(eNetwork, "hps_user", mVerify)) &&
                !(N->Network & MUMAGetBM(eNetwork, "hps_us", mVerify))) {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' is not busy and is missing "
                           "user switch adapter (check switch/runaway "
                           "processes?)\n",
                           N->Name);
            }
        ...
        for (ResIndex = 0; ResIndex < MSched.ResDepth; ResIndex++) {
            if (N->R[ResIndex] == (mres_t *)1) continue;

            if (N->R[ResIndex] == NULL) break;

            if (N->R[ResIndex]->Name[0] == '\0') {
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' has empty reservation pointer  "
                           "(clearing pointer)\n",
                           N->Name);
        ...
        if ((N->State == mnsIdle) && (SameStateTime > 600) &&
            (N->Load > 0.50)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  node '%s' has been idle for %s but load is "
                       "HIGH.  load: %6.3f (check for runaway processes?)\n",
                       N->Name, MULToTString(SameStateTime), N->Load);
        }

        if (N->Load > (N->CRes.Procs * 2)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  node '%s' has excessive load (state: '%s'  "
                       "load: %6.3lf)\n",
                       N->Name, MNodeState[N->State], N->Load);
        }

        if (((N->State == mnsBusy) || (N->State == mnsActive)) &&
            (SameStateTime > 900) && ((N->Load / N->CRes.Procs) < 0.20)) {
            MUSNPrintF(&BPtr, &BSpace,
                       "WARNING:  node '%s' is active for %s but load is LOW.  "
                       "load: %6.3f (check job %s?)\n",
                       N->Name, MULToTString(SameStateTime), N->Load, JobName);
        }
        ...
        switch (N->State) {
            ...
            case mnsReserved:
                ...
                MUSNPrintF(&BPtr, &BSpace,
                           "WARNING:  node '%s' is reserved by an interactive "
                           "POE job\n",
                           N->Name);
            ...
            default:
                ...
                MUSNPrintF(
                    &BPtr, &BSpace,
                    "WARNING:  node '%s' has unexpected node state (%d)\n",
                    N->Name, N->State);
    ...
    MUSNPrintF(&BPtr, &BSpace,
               (Flags & (1 << mcmVerbose))
                   ...
                   : "%-20.20s %8.8s %3d:%-3d %6d:%-6d %6d:%-6d %6d:%-6d\n",
               "-----", "---", TotalARes.Procs, TotalCRes.Procs, TotalARes.Mem,
               TotalCRes.Mem, TotalARes.Disk, TotalCRes.Disk, TotalARes.Swap,
               TotalCRes.Swap);

    MUSNPrintF(&BPtr, &BSpace,
               "\nTotal Nodes: %d  (Active: %d  Idle: %d  Down: %d)\n",
               TotalNodeCount, ActiveNodeCount, IdleNodeCount, DownNodeCount);
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (nindex == 0) MUStrNCat(&BPtr, &BSpace, "\n\n");
...

{
    ...
    MUSScanF(RBuffer, "%d %x%s %d %x%s", &ObjectType, sizeof(PName), PName,
             &Flags, sizeof(Name), Name);
    ...
    switch (ObjectType) {
        case mxoNode:
            ...
            rc = MNodeShowRes(NULL, Name, P, Flags, mwpXML, Buffer, *BufSize);
        ...
        default:
            ...
            DBG(0, fUI)
            DPrint("ERROR:    reservation type '%d' not handled\n", ObjectType);
...
int UIResList(
    ...
    char *Auth,                                                  /* I */
    int GFlags, char *RID, int PIndex, int Flags, char *SBuffer, /* O */
    long *SBufSize)                                              /* I */

{
    ...
    DBG(2, fUI)
    DPrint("%s(Auth,GFlags,%s,%d,%d,SBuffer,SBufSize)\n", FName, RID, PIndex,
           Flags);
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        ...
        if ((PIndex > 0) && (MUBMCheck(PIndex, J->PAL) == FAILURE)) continue;
        ...
        if (R == NULL) {
            DBG(4, fUI)
            DPrint("INFO:     job '%s' has no reservation\n", J->Name);
        ...
        MUSNPrintF(&BPtr, &BSpace, "%-18s %5s %1c %11s %11s %11s %4d/%-4d %s",
                   J->Name, MResType[R->Type], MJobState[J->State][0],
                   StartTime, EndTime, Duration, R->NodeCount, R->AllocPC,
                   SMinTime);

        count++;

        DBG(5, fUI)
        DPrint("INFO:     job '%s' added to reservation list\n", J->Name);
    } /* END for (jindex) */
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if ((PIndex > 0) && (R->PtIndex != PIndex)) continue;

        if (R->Type == mrtJob) continue;
        ...
        if ((GFlags & ((1 << fAdmin1) | (1 << fAdmin2)))) {
            ...
            for (aindex = 0; R->ACL[aindex].Name[0] != '\0'; aindex++) {
                if ((R->ACL[aindex].Type == maUser) &&
                    !strcmp(R->ACL[aindex].Name, Auth)) {
                    Access = TRUE;
        ...
        MUSNPrintF(&BPtr, &BSpace, "%-18s %5s %1c %11s %11s %11s %4d/%-4d %s",
                   R->Name, MResType[R->Type], '-', StartTime, EndTime,
                   Duration, R->NodeCount, R->AllocPC, SMinTime);

        count++;

        DBG(5, fUI)
        DPrint("INFO:     reservation '%s' added to reservation list\n",
               R->Name);
...

{
    ...
    MUSScanF(RBuffer, "%d %x%s %x%s %d", &ObjectType, sizeof(ObjectID),
             ObjectID, sizeof(PName), PName, &Flags);
...

{
    ...
    while (ptr != NULL) {
        ...
        if (MResFind(RID, &R) == FAILURE) {
            DBG(3, fUI)
            DPrint("ALERT:    cannot locate reservation '%s'\n", RID);
        ...
        if (Access == FALSE) {
            DBG(2, fUI)
            DPrint(
                "INFO:     user %s is not authorized to release reservation "
                "'%s'\n",
                Auth, RID);

            sprintf(temp_str,
                    "ERROR:     user %s is not authorized to release "
                    "reservation '%s'\n",
                    Auth, RID);
            strcat(Buffer, temp_str);
        ...
        DBG(3, fUI)
        DPrint("INFO:     released %s reservation '%s'\n", MResType[rtype],
               RID);
        ...
        if (MSched.Mode != msmSim) {
            MOSSyslog(LOG_INFO, "%s reservation %s released", MResType[rtype],
                      RID);
        }
    } /* END while (ptr) */
...

{
    ...
    for (index = 0; MUIQ[index] != -1; index++) {
        ...
        if ((J->State == mjsStarting) || (J->State == mjsRunning)) continue;
        ...
        if (P->Index > 0) {
            if (MUNumListGetCount(J->StartPriority, RQ->DRes.PSlot,
                ...
                DBG(7, fSCHED)
                DPrint(
                    "INFO:     classes not available for job %s in partition "
                    "%s (%s)\n",
                    J->Name, P->Name,
                    MUCAListToString(RQ->DRes.PSlot, P->CRes.PSlot, NULL));
            ...
            DBG(4, fUI)
            DPrint("INFO:     job '%s' can run in partition %s\n", J->Name,
                   P->Name);
        }
        ...
        /* Format:  <JOBNAME> <UID> <GID> <QUEUETIME> <MINPROCS> <MEMORY>
         * <CPULIMIT> <PRIORITY> <QOS> <RES-STATE> */
        ...
        sprintf(Line, "%s %s %s %ld %d %d %ld %ld %s %c %s\n", J->Name,
                J->Cred.U->Name, (J->Cred.G != NULL) ? J->Cred.G->Name : NONE,
                J->SystemQueueTime, J->Request.TC * J->Req[0]->DRes.Procs,
                J->Request.TC * J->Req[0]->DRes.Mem, J->SpecWCLimit[0],
                J->StartPriority,
                ((J->Cred.Q != NULL) && (J->Cred.Q->Index != 0))
                    ? J->Cred.Q->Name
                    : "-",
                resState, (CListPtr[0] != '\0') ? CListPtr : NONE);
...

{
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        ...
        DBG(9, fUI)
        DPrint("INFO:     Block Reason job %s : %d, %d\n", J->Name,
               J->BlockReason, J->HoldReason);
...

{
    ...
    MUSNPrintF(&BPtr, &BSpace, "%ld %d %d %d %d %d %d\n", MSched.Time,
               PS->URes.Procs, PS->ARes.Procs, PS->UpNodes,
               PS->UpNodes - PS->ActiveNodes, PS->URes.Mem, TotalAProcs);
    ...
    for (jindex = 0; MAQ[jindex] != -1; jindex++) {
        ...
        /* Format:  <JOBNAME> <JOBSTATE> <UNAME> <GNAME> <START TIME> <QUEUE
         * TIME> <PROCS> <CPULIMIT> <QOS> <PSDEDICATED> <PSUTILIZED> <PARTITION>
         * <DMEM> <MASTERHOST> <JOBFLAGS> <PRIORITY> */

        sprintf(
            Line,
            "%s %s %s %s %ld %ld %d %ld %s %lf %lf %s %d %s %ld %ld %ld\n",
            J->Name, MJobState[(J->IState != mjsNONE) ? J->IState : J->State],
            J->Cred.U->Name, (J->Cred.G != NULL) ? J->Cred.G->Name : "-",
            J->StartTime, J->SystemQueueTime,
            (MSched.DisplayFlags & (1 << dfNodeCentric)) ? nindex
                                                         : MJobGetProcCount(J),
            Limit, (J->Cred.Q != NULL) ? J->Cred.Q->Name : "-", J->PSDedicated,
            J->PSUtilized, P->Name, RQ->TaskCount * RQ->DRes.Mem,
            ((J->NodeList != NULL) && (J->NodeList[0].N != NULL) &&
             (J->NodeList[0].N->Name[0] != '\0'))
                ? MNodeAdjustName(J->NodeList[0].N->Name, 0)
                : NONE,
            J->Flags, J->AWallTime, J->RunPriority);
...

{
    ...
    DBG(3, fUI)
    DPrint("%s(SBuffer,SBufSize,%s,%s)\n", FName, MAList[ePartition][PIndex],
           DiagOpt);

    /* check all reservations */

    DBG(4, fUI)
    DPrint("INFO:     diagnosing reservation table (%d slots)\n", MAX_MRES);
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (N->State == mnsReserved) TotalResStatePC += N->CRes.Procs;
    } /* END for (nindex) */
    ...
    for (rindex = 0; rindex < MAX_MRES; rindex++) {
        ...
        if ((PIndex > 0) && (PIndex != R->PtIndex)) continue;

        if (strcmp(DiagOpt, NONE) && strcmp(DiagOpt, R->Name)) continue;
        ...
        DBG(6, fUI)
        DPrint("INFO:     evaluating MRes[%03d] '%s'\n", rindex, R->Name);

        MResShowState(R, Flags, SBuffer, *SBufSize, mAdd);

        MResDiagnoseState(R, Flags, SBuffer, *SBufSize, mAdd);
        ...
        if ((R->Type == mrtJob) && (R->J != NULL)) {
            ...
            if (MJOBISACTIVE(J)) TotalResAPC += R->AllocPC;
        }
    } /* END for (rindex) */
    ...
    if (!strcmp(DiagOpt, NONE)) {
        ...
        if ((TotalResAPC + TotalResStatePC) != MPar[0].DRes.Procs) {
            sprintf(temp_str,
                    "WARNING:  reservation table is corrupt:  active procs "
                    "reserved does not equal active procs detected (%d != "
                    "%d)\n",
                    TotalResAPC + TotalResStatePC, MPar[0].DRes.Procs);
        }

        if (ResLast > ResEnd) {
            sprintf(temp_str,
                    "WARNING:  reservation table is corrupt:  empty slot "
                    "detected (slot: %d) before end of table\n",
                    ResEnd);
            strcat(SBuffer, temp_str);
        }
