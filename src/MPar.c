...

{
    ...
    DPrint("%s(%s,%s,PAL,%s)\n", FName, (J != NULL) ? J->Name : "NULL",
           (RPAL != NULL) ? "RPAL" : "NULL", (PDef != NULL) ? "PDef" : "NULL");
    ...
    if ((J->Cred.Q != NULL) && (J->Cred.Q->F.PALType == qalAND) &&
    ...
    if ((J->Cred.A != NULL) && (J->Cred.A->F.PALType == qalAND) &&
    ...
    if ((RPAL != NULL) && (RPAL[0] != 0)) MUBMAND(tmpPAL, RPAL, MAX_MPAR);

    if (PAL != NULL) MUBMCopy(PAL, tmpPAL, MAX_MPAR);
    ...
    if (PDef != NULL) {
        *PDef = MJobFindDefPart(J, C, tmpPAL);
        ...
        DBG(3, fSTRUCT)
        DPrint(
            "INFO:     default partition for job %s set to %s "
            "(P:%s,U:%s,G:%s,A:%s,C:%s,Q:%s)\n",
            J->Name, (*PDef)->Name, ((mpar_t *)MPar[0].F.PDef)->Name,
    ...
    if ((RPAL != NULL) && (RPAL[0] != 0)) {
        if (tmpPAL[0] == 0) {
            DBG(2, fSTRUCT)
            DPrint("WARNING:  job %s cannot access requested Partitions '%s'\n",
                   J->Name, MUListAttrs(ePartition, RPAL[0]));
...

{
    ...
    if ((J->Cred.U->F.PDef != NULL) && (J->Cred.U->F.PDef != &MPar[0]) &&
        (PAL == NULL || MUBMCheck(((mpar_t *)J->Cred.U->F.PDef)->Index, PAL))) {
        PDef = (mpar_t *)J->Cred.U->F.PDef;
    } else if ((J->Cred.G->F.PDef != NULL) && (J->Cred.G->F.PDef != &MPar[0]) &&
               (PAL == NULL ||
                MUBMCheck(((mpar_t *)J->Cred.G->F.PDef)->Index, PAL))) {
        PDef = (mpar_t *)J->Cred.G->F.PDef;
    } else if ((J->Cred.A != NULL) && (J->Cred.A->F.PDef != NULL) &&
               (J->Cred.A->F.PDef != &MPar[0]) &&
               (PAL == NULL ||
                MUBMCheck(((mpar_t *)J->Cred.A->F.PDef)->Index, PAL))) {
        PDef = (mpar_t *)J->Cred.A->F.PDef;
    } else if ((C != NULL) && (C->F.PDef != NULL) && (C->F.PDef != &MPar[0]) &&
               (PAL == NULL || MUBMCheck(((mpar_t *)C->F.PDef)->Index, PAL))) {
        PDef = (mpar_t *)C->F.PDef;
    } else if ((J->Cred.Q != NULL) && (J->Cred.Q->F.PDef != NULL) &&
               (J->Cred.Q->F.PDef != &MPar[0]) &&
               (PAL == NULL ||
                MUBMCheck(((mpar_t *)J->Cred.Q->F.PDef)->Index, PAL))) {
        PDef = (mpar_t *)J->Cred.Q->F.PDef;
    } else if ((MPar[0].F.PDef != NULL) && (MPar[0].F.PDef != &MPar[0])) {
...

{
    ...
    for (pindex = 0; pindex < MAX_MPAR; pindex++) {
        ...
        if (strcmp(P->Name, PName) != 0) continue;
...

{
    ...
    if (P->L.IP == NULL) P->L.IP = (mpu_t *)calloc(1, sizeof(mpu_t));

    if (P->L.JP == NULL) P->L.JP = (mpu_t *)calloc(1, sizeof(mpu_t));
...

{
    ...
    if (PName != NULL) memset(P, 0, sizeof(mpar_t));

    if (PName != NULL) MUStrCpy(P->Name, PName, sizeof(P->Name));
...

{
    ...
    */
    ...
    for (index = 0; index < MAX_MRESOURCETYPE; index++) {
        ...
        P->ResourceLimitViolationAction[index] =
            DEFAULT_MRESOURCELIMITVIOLATIONACTION;
    } /* END for (index) */
    ...
    if (P->Index == 0) P->F.PAL[0] = (1 << MDEF_SYSPAL);
...

{
    ...
    if (BM == NULL) return (FAILURE);

    MUBMClear(BM, MAX_MPAR);

    if (PString == NULL) return (SUCCESS);
...

{
    ...
    for (pindex = 0; pindex < MAX_MPAR; pindex++) {
        ...
        for (index = 0; index < MAX_MUSER + MAX_MHBUF; index++) {
            ...
            if (MUBMCheck(pindex, U->F.PAL)) {
                sprintf(temp_str, " %s%s", U->Name, MQALType[U->F.PALType]);
        ...
        for (index = 0; index < MAX_MGROUP + MAX_MHBUF; index++) {
            ...
            if ((G->Name[0] == '\0') || (G->Name[0] == '\1')) continue;

            if (MUBMCheck(pindex, G->F.PAL)) {
                sprintf(temp_str, " %s%s", G->Name, MQALType[G->F.PALType]);
                strcat(Line, temp_str);
            }
        } /* END for (index) */
        ...
        for (index = 0; index < MAX_MACCT + MAX_MHBUF; index++) {
            ...
            if ((A->Name[0] == '\0') || (A->Name[0] == '\1')) continue;

            if (MUBMCheck(pindex, A->F.PAL)) {
                sprintf(temp_str, " %s%s", A->Name, MQALType[A->F.PALType]);
                strcat(Line, temp_str);
            }
        } /* END for (index) */
        ...
        for (index = 0; index < MAX_MCLASS; index++) {
            ...
            if ((C->Name[0] == '\0') || (C->Name[0] == '\1')) continue;

            if (MUBMCheck(pindex, C->F.PAL)) {
                sprintf(temp_str, " %s%s", C->Name, MQALType[C->F.PALType]);
                strcat(Line, temp_str);
            }
        } /* END for (index) */
    ...
    for (rindex = mrNode; rindex <= mrDisk; rindex++) {
        ...
        for (pindex = 0; pindex < MAX_MPAR; pindex++) {
            ...
            if (strcmp(PName, NONE) && strcmp(PName, P->Name)) continue;
            ...
            sprintf(
                temp_str, "%-12s %10d %10d %6.2f%c %10d %6.2f%c %10d %6.2f%c\n",
                P->Name, CRes, URes,
                (double)((CRes > 0) ? URes * 100.0 / CRes : 0.0), '%', DRes,
...

{
    ...
    DBG(6, fUI)
    DPrint("%s(%s,Avl,Cfg,Ded,Req,%ld)\n", FName,
           (P != NULL) ? P->Name : "NULL", Time);
...
int MParProcessOConfig(
    ...
    mpar_t *P,  /* I */
    int PIndex, /* I */
    int IVal, double DVal, char *SVal, char **SArray)
    ...
    switch (PIndex) {
...

        {
            ...
            if (index != bfNONE) {
                ...
                if ((MUBoolFromString(SVal, TRUE) == FALSE) ||
                    !strcmp(SVal, "NONE"))
                    P->BFPolicy = bfNONE;
            }
        } /* END BLOCK */
        ...
        case pJobNodeMatch:
...

            {
                ...
                for (index = 0; SArray[index] != NULL; index++) {
                    for (index2 = 0; MJobNodeMatchType[index2] != NULL;
                         index2++) {
        ...
        case pJobSizePolicy:
            ...
            P->JobSizePolicy =
                MUGetIndex(SVal, MJSPolicy, FALSE, P->JobSizePolicy);

            if (MJSPolicy[P->JobSizePolicy] == NULL) {
                DBG(1, fCONFIG)
                DPrint("ALERT:    invalid %s parameter specified '%s'\n",
                       MParam[pJobSizePolicy], SVal);
            }
        ...
        case pNodeAllocationPolicy:
            ...
            P->NAllocPolicy =
                MUGetIndex(SVal, MNAllocPolicy, FALSE, P->NAllocPolicy);

            /* sync GLOBAL and DEFAULT partitions */

            if ((P->Index == 0) &&
                ((MPar[1].NAllocPolicy == mnalNONE2) ||
                 (MPar[1].NAllocPolicy == DEFAULT_MNALLOCPOLICY))) {
                MPar[1].NAllocPolicy = P->NAllocPolicy;
            } else if ((P->Index == 1) &&
                       ((MPar[0].NAllocPolicy == mnalNONE2) ||
                        (MPar[0].NAllocPolicy == DEFAULT_MNALLOCPOLICY))) {
        ...
        case mcoEnableMultiNodeJobs:
            ...
            P->EnableMultiNodeJobs =
                MUBoolFromString(SVal, P->EnableMultiNodeJobs);

            break;

        case mcoEnableMultiReqJobs:

            P->EnableMultiReqJobs =
                MUBoolFromString(SVal, P->EnableMultiReqJobs);

            break;

        case mcoEnableNegJobPriority:

            P->EnableNegJobPriority =
                MUBoolFromString(SVal, P->EnableNegJobPriority);
...

        {
            ...
            for (vindex = 0; SArray[vindex] != NULL; vindex++) {
                ...
                if ((ptr = MUStrTok(SArray[vindex], ":", &TokPtr)) == NULL) {
                    DBG(1, fCONFIG)
                    DPrint("ALERT:    invalid %s parameter specified '%s'\n",
                           MParam[mcoResourceLimitPolicy], SArray[vindex]);

                    continue;
                }

                if ((index = MUGetIndex(ptr, MResourceType, FALSE, mrNONE)) ==
                    mrNONE) {
                    DBG(1, fCONFIG)
                    DPrint("ALERT:    invalid %s parameter specified '%s'\n",
                           MParam[mcoResourceLimitPolicy], SArray[vindex]);
                ...
                if ((RLPolicy = MUGetIndex(ptr, MResourceLimitPolicyType, FALSE,
                                           mrlpNONE)) == mrlpNONE) {
                    /* invalid policy */
                ...
                if ((RLAction = MUGetIndex(ptr, MPolicyAction, FALSE,
                                           mrlaNONE)) == mrlaNONE) {
        ...
        case pTaskDistributionPolicy:
            ...
            if (tmpI != 0) {
                ...
                DBG(1, fCONFIG)
                DPrint("ALERT:    invalid %s parameter specified '%s'\n",
                       MParam[pTaskDistributionPolicy], SVal);
            }
        ...
        case pUseSystemQueueTime:
            ...
            P->UseSystemQueueTime =
                MUBoolFromString(SVal, P->UseSystemQueueTime);
        ...
        case pJobPrioAccrualPolicy:
            ...
            P->JobPrioAccrualPolicy = MUGetIndex(
                SVal, MJobPrioAccrualPolicyType, 0, P->JobPrioAccrualPolicy);
        ...
        case pBFPriorityPolicy:
            ...
            P->BFPriorityPolicy =
                MUGetIndex(SVal, MBFPriorityPolicyType, 0, P->BFPriorityPolicy);
        ...
        case pNodeLoadPolicy:
            ...
            P->NodeLoadPolicy =
                MUGetIndex(SVal, MNodeLoadPolicyType, 0, P->NodeLoadPolicy);

            break;

        case pNodeSetPolicy:

            P->NodeSetPolicy = MUGetIndex(
                SVal, (const char **)MResSetSelectionType, 0, P->NodeSetPolicy);

            break;

        case pNodeSetAttribute:

            P->NodeSetAttribute = MUGetIndex(
                SVal, (const char **)MResSetAttrType, 0, P->NodeSetAttribute);
        ...
        case pNodeSetPriorityType:
            ...
            P->NodeSetPriorityType =
                MUGetIndex(SVal, (const char **)MResSetPrioType, 0, mrspNONE);
...

{
    ...
    DBG(4, fSTRUCT)
    DPrint(
        "INFO:     P[%s]:  Total %d:%d  Up %d:%d  Idle %d:%d  Active %d:%d\n",
        SP->Name, SP->ConfigNodes, SP->CRes.Procs, SP->UpNodes, SP->URes.Procs,
        SP->IdleNodes,

        SP->ARes.Procs, SP->ActiveNodes, SP->DRes.Procs);
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        if (P == GP) {
            if (strcmp(N->Name, "GLOBAL")) {
                DBG(1, fSTRUCT)
                DPrint(
                    "ERROR:    node '%s' is not associated with any "
                    "partition\n",
                    N->Name);
            }
        ...
        if ((SP == GP) || (SP == P)) {
            DBG(5, fSTRUCT)
            DPrint("INFO:     MNode[%s] added to MPar[%s] (%d:%d)\n",

                   N->Name, MPar[N->PtIndex].Name, N->ARes.Procs,
                   N->CRes.Procs);
        }
    } /* END for (nindex) */
    ...
    DPrint(
        "INFO:     P[%s]:  Total %d:%d  Up %d:%d  Idle %d:%d  Active %d:%d\n",
        SP->Name, SP->ConfigNodes, SP->CRes.Procs, SP->UpNodes, SP->URes.Procs,
        SP->IdleNodes, SP->ARes.Procs, SP->ActiveNodes, SP->DRes.Procs);
...

{
    ...
    if ((P->RejectNegPrioJobs != DEFAULT_MREJECTNEGPRIOJOBS) ||
        ...
        strcat(Buffer,
               MUShowSArray(MParam[mcoRejectNegPrioJobs], P->Index,
    ...
    if ((P->EnableNegJobPriority != DEFAULT_MENABLENEGJOBPRIORITY) ||
        ...
        strcat(
            Buffer,
            MUShowSArray(MParam[mcoEnableNegJobPriority], P->Index,
    ...
    if ((P->EnableMultiNodeJobs != DEFAULT_MENABLEMULTINODEJOBS) ||
        ...
        strcat(
            Buffer,
            MUShowSArray(MParam[mcoEnableMultiNodeJobs], P->Index,
    ...
    if ((P->EnableMultiReqJobs != DEFAULT_MENABLEMULTIREQJOBS) ||
        ...
        strcat(Buffer, MUShowSArray(
                           MParam[mcoEnableMultiReqJobs], P->Index,
    ...
    if ((P->BFPriorityPolicy != mbfpNONE) ||
        ...
        strcat(Buffer, MUShowSArray(
                           MParam[pBFPriorityPolicy], P->Index,
    ...
    if (P->Index == 0) {
        ...
        if ((P->UntrackedProcFactor > 0.0) ||
            ...
            sprintf(temp_str, "%-30s  %.1lf\n",
                    MParam[pNodeUntrackedProcFactor], P->UntrackedProcFactor);
            strcat(Buffer, temp_str);
        }
    } /* END if (P->Index == 0) */
    ...
    if ((P->MaxJobStartTime != MAX_MTIME) ||
        ...
        strcat(Buffer, MUShowSArray(MParam[pMaxJobStartTime], P->Index,
    ...
    if ((P->MaxMetaTasks != DEFAULT_MAXMETATASKS) ||
        (VFlag || (PIndex == -1) || (PIndex == pMaxMetaTasks))) {
        strcat(Buffer,
               MUShowIArray(MParam[pMaxMetaTasks], P->Index, P->MaxMetaTasks));
    }

    /* node set policies */

    if ((P->NodeSetPolicy != mrssNONE) ||
        (VFlag || (PIndex == -1) || (PIndex == pNodeSetPolicy))) {
        strcat(Buffer,
               MUShowSArray(MParam[pNodeSetPolicy], P->Index,
                            (char *)MResSetSelectionType[P->NodeSetPolicy]));
    }

    if ((P->NodeSetAttribute != mrssNONE) ||
        (VFlag || (PIndex == -1) || (PIndex == pNodeSetAttribute))) {
        strcat(Buffer,
               MUShowSArray(MParam[pNodeSetAttribute], P->Index,
                            (char *)MResSetAttrType[P->NodeSetAttribute]));
    }

    if ((P->NodeSetList[0] != NULL) ||
        (VFlag || (PIndex == -1) || (PIndex == pNodeSetList))) {
        char tmpLine[MAX_MLINE];
        int index;
        ...
        for (index = 0; P->NodeSetList[index] != NULL; index++) {
            sprintf(temp_str, "%s ", P->NodeSetList[index]);
            strcat(tmpLine, temp_str);
        } /* END for (index) */

        strcat(Buffer, MUShowSArray(MParam[pNodeSetList], P->Index, tmpLine));
    }

    if ((P->NodeSetDelay != MDEF_NODESETDELAY) ||
        (VFlag || (PIndex == -1) || (PIndex == pNodeSetDelay))) {
        strcat(Buffer, MUShowSArray(MParam[pNodeSetDelay], P->Index,
                                    (char *)MULToTString(P->NodeSetDelay)));
    }

    if ((P->NodeSetPriorityType != mrspNONE) ||
        (VFlag || (PIndex == -1) || (PIndex == pNodeSetPriorityType))) {
        strcat(Buffer,
               MUShowSArray(MParam[pNodeSetPriorityType], P->Index,
                            (char *)MResSetPrioType[P->NodeSetPriorityType]));
    }

    if ((P->NodeSetTolerance != MDEF_NODESETTOLERANCE) ||
        (VFlag || (PIndex == -1) || (PIndex == pNodeSetTolerance))) {
        strcat(Buffer, MUShowFArray(MParam[pNodeSetTolerance], P->Index,
                                    P->NodeSetTolerance));
    }
    ...
    if (P->Index == 0) {
        ...
        if ((P->BFPolicy != bfNONE) ||
            ...
            if ((PIndex == -1) || (PIndex == pBFPolicy) || VFlag)
                strcat(Buffer, MUShowSArray(MParam[pBFPolicy], P->Index,
                                            (char *)MBFPolicy[P->BFPolicy]));

            if ((P->BFDepth != DEFAULT_BACKFILLDEPTH) ||
                (VFlag || (PIndex == -1) || (PIndex == pBFDepth)))
                strcat(Buffer,
                       MUShowIArray(MParam[pBFDepth], P->Index, P->BFDepth));

            if ((P->BFProcFactor > 0) ||
                (VFlag || (PIndex == -1) || (PIndex == pBFProcFactor)))
                strcat(Buffer, MUShowIArray(MParam[pBFProcFactor], P->Index,
                                            P->BFProcFactor));

            if ((P->BFMaxSchedules != DEFAULT_MAXBACKFILLSCHEDULES) ||
                (VFlag || (PIndex == -1) || (PIndex == pBFMaxSchedules)))
                strcat(Buffer, MUShowIArray(MParam[pBFMaxSchedules], P->Index,
                                            P->BFMaxSchedules));

            if ((PIndex == -1) || (PIndex == pBFMetric))
                strcat(Buffer, MUShowSArray(MParam[pBFMetric], P->Index,
                                            (char *)MBFMPolicy[P->BFMetric]));

            if (PIndex == -1) strcat(Buffer, "\n");
        } /* END if ((P->BFPolicy != bfNONE) || ... ) */

        if ((P->BFChunkDuration != 0) ||
            (VFlag || (PIndex == -1) || (PIndex == mcoBFChunkDuration))) {
            strcat(Buffer, MUShowSArray(MParam[mcoBFChunkDuration], P->Index,
                                        MULToTString(P->BFChunkDuration)));
        }

        if ((P->BFChunkSize != 0) ||
            (VFlag || (PIndex == -1) || (PIndex == mcoBFChunkSize))) {
            strcat(Buffer, MUShowIArray(MParam[mcoBFChunkSize], P->Index,
                                        P->BFChunkSize));
        }

        if (VFlag || (PIndex == -1) || (PIndex == pPreemptPolicy)) {
            strcat(Buffer,
                   MUShowSArray(MParam[pPreemptPolicy], P->Index,
                                (char *)MPreemptPolicy[MSched.PreemptPolicy]));
        }

        if (VFlag || (PIndex == -1) || (PIndex == mcoAdminMinSTime)) {
            strcat(Buffer, MUShowSArray(MParam[mcoAdminMinSTime], P->Index,
        ...
        if ((PIndex == mcoResourceLimitPolicy) || VFlag || (PIndex == -1)) {
            ...
            for (index = 1; MResourceType[index] != NULL; index++) {
                if (P->ResourceLimitPolicy[index] != mrlpNONE) {
                    if (P->ResourceLimitMaxViolationTime[index] > 0) {
                        sprintf(temp_str, "%s:%s:%s:%s ", MResourceType[index],
                                MResourceLimitPolicyType
                                    [P->ResourceLimitPolicy[index]],
                                MPolicyAction
                                    [P->ResourceLimitViolationAction[index]],
                                MULToTString(
                                    P->ResourceLimitMaxViolationTime[index]));
                        strcat(tmpLine, temp_str);
                    } else {
                        sprintf(temp_str, "%s:%s:%s ", MResourceType[index],
                                MResourceLimitPolicyType
                                    [P->ResourceLimitPolicy[index]],
                                MPolicyAction
                                    [P->ResourceLimitViolationAction[index]]);
            ...
            strcat(Buffer, MUShowSArray(MParam[mcoResourceLimitPolicy],
                                        P->Index, tmpLine));
        ...
        if ((PIndex == pNodeAvailPolicy) || VFlag || (PIndex == -1)) {
            ...
            for (rindex = 0; MResourceType[rindex] != NULL; rindex++) {
                ...
                sprintf(temp_str, "%s:%s", MNAvailPolicy[policyindex],
                        (rindex == 0) ? DEFAULT : MResourceType[rindex]);
                strcat(tmpLine, temp_str);
            } /* END for (rindex) */

            strcat(Buffer,
                   MUShowSArray(MParam[pNodeAvailPolicy], P->Index, tmpLine));
        ...
        if ((PIndex == pNodeAllocationPolicy) || VFlag || (PIndex == -1)) {
            strcat(Buffer,
                   MUShowSArray(MParam[pNodeAllocationPolicy], P->Index,
        ...
        if (VFlag || (PIndex == -1) || (PIndex == pTaskDistributionPolicy)) {
            strcat(Buffer, MUShowSArray(
                               MParam[pTaskDistributionPolicy], P->Index,
                               (char *)MTaskDistributionPolicy[P->DistPolicy]));
        }

        /* reservation policy */

        strcat(Buffer, MUShowSArray(MParam[pResPolicy], P->Index,
                                    (char *)MResPolicy[P->ResPolicy]));

        for (index = 0; index < MAX_MQOS; index++) {
            if (!VFlag && ((P->ResDepth[index] == DEFAULT_RESERVATIONDEPTH) ||
                           (P->ResDepth[index] == 0)) &&
                (P->ResQOSList[index][0] == (mqos_t *)MAX_MQOS)) {
                continue;
            }
            ...
            if (VFlag || (PIndex == -1) || (PIndex == pResDepth)) {
                strcat(Buffer, MUShowIArray(MParam[pResDepth], P->Index,
                                            P->ResDepth[index]));
            }
            ...
            if (VFlag || (P->ResQOSList[index][0] != (mqos_t *)MAX_MQOS)) {
                ...
                if (P->ResQOSList[index][0] == (mqos_t *)MAX_MQOS) {
                    ...
                    for (qindex = 0; P->ResQOSList[index][qindex] != NULL;
                         qindex++) {
                        if (P->ResQOSList[index][qindex] ==
                            (mqos_t *)MAX_MQOS) {
                            sprintf(temp_str, "%s ", ALL);
                            strcat(tmpLine, temp_str);
                        } else {
                            sprintf(temp_str, "%s ",
                ...
                if (VFlag || (PIndex == -1) || (PIndex == pResQOSList)) {
                    strcat(Buffer,
                           MUShowSArray(MParam[pResQOSList], index, tmpLine));
                }
            } /* END  if (P->ResQOSList[index][0] != (mqos_t *)MAX_MQOS)) */
        }     /* END for (index) */

        if ((P->ResRetryTime != 0) ||
            (VFlag || (PIndex == -1) || (PIndex == pResRetryTime))) {
            strcat(Buffer, MUShowSArray(MParam[pResRetryTime], P->Index,
                                        MULToTString(P->ResRetryTime)));
        }

        if ((P->ResTValue != 0) ||
            (VFlag || (PIndex == -1) || (PIndex == pResThresholdValue))) {
            strcat(Buffer,
                   MUShowSArray(MParam[pResThresholdType], P->Index,
                                (char *)MResThresholdType[P->ResTType]));

            strcat(Buffer, MUShowIArray(MParam[pResThresholdValue], P->Index,
                                        P->ResTValue));
        }
        ...
        if ((F->FSPolicy == fspNONE) ||
            (VFlag || (PIndex == -1) || (PIndex == pFSPolicy))) {
            sprintf(temp_str, "%-30s  %s\n", MParam[pFSInterval],
                    MULToTString(F->FSInterval));
            strcat(Buffer, temp_str);
