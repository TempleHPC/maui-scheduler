...

{
    ...
    DBG(5, fSTRUCT)
    DPrint("%s(%s,C)\n", FName, (CName != NULL) ? CName : "NULL");

    if (C != NULL) *C = NULL;

    if ((CName == NULL) || (CName[0] == '\0') || (C == NULL)) {
        return (FAILURE);
    }
...

{
    ...
    int ClAList[] = {mclaOCNode,         mclaOCDProcFactor, mclaDefReqFeature,
                     mclaMaxProcPerNode, mclaWCOverrun,     -1};

    int CAList[] = {
        mcaMaxProcPerUser, mcaMaxNodePerUser, mcaDefWCLimit,    mcaMaxWCLimit,
        mcaMaxNodePerJob,  mcaMaxProcPerJob,  mcaMaxJobPerUser, -1};
    ...
    DBG(3, fUI)
    DPrint("%s(%s,SBuffer,SBufSize,%d)\n", FName,
           (CP != NULL) ? CP->Name : "NULL", DMode);
    ...
    for (cindex = 0; cindex < MAX_MCLASS; cindex++) {
        ...
        DBG(8, fUI)
        DPrint("INFO:     checking MClass[%02d]: %s\n", cindex, C->Name);

        if (C->Name[0] == '\0') continue;

        if (!strcmp(C->Name, NONE) || !strcmp(C->Name, ALL)) continue;
        ...
        for (findex = 0; MJobFlags[findex] != NULL; findex++) {
            if (!(C->F.JobFlags & (1 << findex))) continue;
        ...
        MUStrCpy(tmpLString,
                 MCredShowAttrs(&C->L.AP, C->L.IP, NULL, NULL, NULL, &C->F, 0,
                                (1 << mcsLimits) | (1 << mcsUsage)),
                 sizeof(tmpLString));

        /*                         NAME PRIO FLAG  QDEF  QLST M PLST  FSTARG
         * Limits */

        MUSNPrintF(
            &BPtr, &BSpace, "%-14s %8ld %-12s %-12s %12s%c %-20s %5.2lf %7s\n",
            C->Name, C->F.Priority, (tmpLine[0] != '\0') ? tmpLine : NONE,
            ((mqos_t *)C->F.QDef) != NULL ? ((mqos_t *)C->F.QDef)->Name : NONE,
            (tmpQALString[0] != '\0') ? tmpQALString : NONE, QALChar,
            (C->F.PAL[0] == 0) ? NONE : MUListAttrs(ePartition, C->F.PAL[0]),
            C->F.FSTarget, (tmpLString[0] != '\0') ? tmpLString : NONE);
        ...
        for (aindex = 0; ClAList[aindex] != -1; aindex++) {
            if ((MClassAToString(C, ClAList[aindex], tmpLString, mdfString,
                                 0) == SUCCESS) &&
                (tmpLString[0] != '\0') && strcmp(tmpLString, NONE)) {
                sprintf(&tmpLine[strlen(tmpLine)], "  %s=%s",
                        MClassAttr[ClAList[aindex]], tmpLString);
            }
        } /* END for (aindex) */
        ...
        for (aindex = 0; CAList[aindex] != -1; aindex++) {
            if ((MCredAToString((void *)C, mxoClass, CAList[aindex], tmpLString,
                                mdfString) == SUCCESS) &&
                (tmpLString[0] != '\0')) {
                sprintf(&tmpLine[strlen(tmpLine)], "  %s=%s",
                        MCredAttr[CAList[aindex]], tmpLString);
            }
        } /* END for (aindex) */
...

{
    ...
    DBG(5, fCONFIG)
    DPrint("%s(%s,%s)\n", FName, (C != NULL) ? C->Name : "NULL",
           (Value != NULL) ? Value : "NULL");

    if ((C == NULL) || (Value == NULL) || (Value[0] == '\0')) {
        return (FAILURE);
    }
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MClassAttr, &aindex, NULL, TRUE, NULL,
...
int MClassConfigLShow(
    ...
    mclass_t *C, int VFlag, int PIndex, char *Buffer)

{
    int AList[] = {mclaOCNode, mclaDefReqFeature, -1};
    ...
    for (aindex = 0; AList[aindex] != -1; aindex++) {
        if ((MClassAToString(C, AList[aindex], tmpString, mfmHuman, 0) ==
             FAILURE) ||
            (tmpString[0] == '\0')) {
            continue;
        }

        sprintf(temp_str, " %s=%s", MClassAttr[AList[aindex]], tmpString);
...

{
    ...
    switch (AIndex) {
        case mclaOCNode:
            ...
            if (C->OCNodeName != NULL) strcpy(Buf, C->OCNodeName);
        ...
        case mclaWCOverrun:
            ...
            if (C->F.Overrun != 0) strcpy(Buf, MULToTString(C->F.Overrun));
...
int MClassConfigShow(
    ...
    mclass_t *CS, int VFlag, char *Buffer)

{
    ...
    for (cindex = 0; cindex < MAX_MCLASS; cindex++) {
        ...
        sprintf(tmpLine, "CLASSCFG[%s]", C->Name);
        ...
        for (aindex = 0; MClassAttr[aindex] != NULL; aindex++) {
            ...
            if (tmpVal[0] != '\0') {
                sprintf(temp_str, " %s=%s", MClassAttr[aindex], tmpVal);
...

{
    ...
    sscanf(Buf, "%s %s %ld", tmpHeader, CName, &CkTime);
    ...
    if (CS == NULL) {
        if (MClassAdd(CName, &C) != SUCCESS) {
            DBG(5, fCKPT)
            DPrint("ALERT:    cannot load CP class '%s'\n", CName);
