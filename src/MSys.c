...

{
    ...
    if ((MSched.ConfigBuffer == NULL) || (Mode & (1 << mcmForce))) {
        ...
        if ((MSched.ConfigBuffer =
                 MFULoad(FileName, 1, macmWrite, &count, &SC)) == NULL) {
            DBG(2, fCONFIG)
            DPrint(
                "WARNING:  cannot load configuration file '%s' (using internal "
                "defaults)\n",
                FileName);
    ...
    if (MUMemCCmp((char *)&GN->CRes, '\0', sizeof(GN->CRes)) == FAILURE) {
    ...
    sprintf(MSched.KeyFile, "%s/%s", MSched.HomeDir, MSCHED_KEYFILE);
...
{
    mrange_t R1[] = {{956601443, 956612113, 22, 22, 0},
    ...
    mrange_t R2[] = {{956635459, 956635459, 1, 1, 0},
...

{
    ...
    mrange_t R1[] = {{1022172521, 1022173705, 476, 119, 0},
...
{
    mrange_t R1[] = {{1016704575, 2139856000, 4, 1, 0},
                     /*
                         {1,3,1,1},
                         {5,6,1,1},
                         {6,7,2,2},
    ...
    mrange_t R2[] = {{1016704575, 2139856000, 4, 1, 0},
                     /*
                         {0,2,1,1},
                         {4,6,1,1},
                         {8,9,1,1},
...
/* configure reservation */
    ...
    MJobGetSNRange(&tmpJ, &tmpRQ, &tmpN, GRange, MAX_MRANGE, &Affinity, NULL,
                   ARange, NULL, NULL);
...

{
    ...
    if (X.XRMJobSubmit != (int (*)())0) {
        rc = (*X.XRMJobSubmit)(X.xd, SubmitString, &MRM[0], &J, JobName, Output,
                               &SC);
...

{
    ...
    const char *TName[] = {
        NONE,         "SCHED",      "XML",        "RANGEAND", "RANGECOLLAPSE",
        "RANGEMERGE", "GETSNRANGE", "FEATURESUB", "NODEPRIO", "RESPARSE",
        "SUBMIT",     "WIKINODE",   "WIKIJOB",    "RMX",      "JOBNAME",
        "JOBDIST",    NULL};
...

{
    ...
    DBG(2, fCORE)
    DPrint("%s(%s,%d,%ld,%d)\n", FName, (Message != NULL) ? Message : "NULL",
           AType, EFlags, Prio);
    ...
    sprintf(
        tmpLine,
        "%s<%s><event component=\"%s\" time=\"%s msg=\"%s\" data=\"%s\"/></%s>",
        XHeader, ReqO, CName, TString, Message, DString, ReqO);
...

{
    ...
    if (Prio > 0) {
        ...
        MSysLaunchAction(ASList, (AType != 0) ? AType : mactAdminEvent);
    }
...

{
    ...
    if (MSched.Action[AType][0] == '/') {
        ...
        sprintf(Exec, "%s%s", MSched.ToolsDir, MSched.Action[AType]);
    } else {
        sprintf(Exec, "%s/%s", MSched.ToolsDir, MSched.Action[AType]);
    }

    if (ASList[1] == NULL) {
        sprintf(Line, "\"%s %s\"", MLogGetTime(), "NODATA");

        DBG(7, fCORE)
        DPrint("INFO:     launching '%s' (AString: '%s')\n", Exec, Line);
    } else {
        DBG(7, fCORE)
        DPrint("INFO:     launching '%s' (AString: '%s', ...)\n", Exec,
               ASList[1]);
    ...
    if ((pid = fork()) == -1) {
        DBG(0, fCORE)
        DPrint("ALERT:    cannot fork for action '%s', errno: %d (%s)",
               MSched.Action[AType], errno, strerror(errno));
    ...
    if (pid == 0) {
        ...
        if (rc == -1) {
            ...
            DBG(1, fCORE)
            DPrint(
                "ALERT:    cannot exec action '%s', rc: %d, errno: %d (%s)\n",
                Exec, rc, errno, strerror(errno));
        }
    ...
    DBG(2, fCORE)
    DPrint("INFO:     action '%s' launched with message '%s'\n",
           MSched.Action[AType], ASList[1]);
...

{
    ...
    MDB(4, fCORE)
    MLog("%s(%s,%s,HostName,Port,WProtocol,SProtocol)\n", FName,
         (SName != NULL) ? SName : "NULL", (CName != NULL) ? CName : "NULL");
#endif /* !__MPROD */
    ...
    if (WProtocol != NULL) WProtocol[0] = '\0';

    if (SProtocol != NULL) SProtocol[0] = '\0';
    ...
    */
    ...
    if (MS3DoCommand(&MSched.DS, CmdString, &RspPtr, NULL, NULL, NULL) ==
        FAILURE) {
    ...
    if (MXMLFromString(&E, RspPtr, NULL, NULL) == FAILURE) {
        MDB(2, fCORE)
        MLog("ALERT:    cannot parse DS query response '%s'\n", RspPtr);
    ...
    if (MXMLGetChild(E, "error", NULL, &tE) == SUCCESS) {
        ...
        MDB(2, fCORE)
        MLog(
            "ALERT:    cannot process DS query response '%s' (FailureType: %s  "
            "Msg: '%s'\n",
            RspPtr, EType, Msg);
    ...
    if (MXMLGetChild(E, "locations", NULL, &RE) == FAILURE) {
        MDB(2, fCORE)
        MLog("ALERT:    cannot process DS query response '%s'\n", RspPtr);
    ...
    if (HostName != NULL) {
        if ((MXMLGetChild(LE, "host", NULL, &tE) == SUCCESS) &&
            (tE->Val != NULL)) {
    ...
    if (Port != NULL) {
        if ((MXMLGetChild(LE, "port", NULL, &tE) == SUCCESS) &&
            (tE->Val != NULL)) {
    ...
    if (SProtocol != NULL) {
        if ((MXMLGetChild(LE, "host", NULL, &tE) == SUCCESS) &&
            (tE->Val != NULL)) {
            /* NOTE:  directory service protocol must be translated between S3
             * and local protocols */
    ...
    MDB(2, fCORE)
    MLog("INFO:     information for service '%s' successfully queried\n",
         SName);
...

{
    ...
    if (MSched.Sync.UpdateTime <= 0) {
        ...
        sprintf(tmpLine, "%s/%s", DEFAULT_MHSYNCLOCATION, MRMType[MRM[0].Type]);
...

{
    ...
    if (Message != NULL) MXMLSetAttr(RE, "data", (void *)Message, mdfString);
    ...
    if (MXMLFromString(&E, RspPtr, NULL, NULL) == FAILURE) {
        DBG(2, fCORE)
        DPrint("ALERT:    cannot process EM response '%s'\n", RspPtr);
    ...
    DBG(4, fCORE)
    DPrint("INFO:     event '%s' successfully submitted\n", EName);
...

{
    ...
    DBG(4, fCORE)
    DPrint("%s(EM,%s,%s,%s,%s)\n", FName, (SName != NULL) ? SName : "NULL",
           (EName != NULL) ? EName : "NULL", (EData != NULL) ? EData : "NULL",
           (Dst != NULL) ? Dst : "NULL");
    ...
    if (Dst != NULL) MXMLSetAttr(RE, "respond_to", (void *)Dst, mdfString);
    ...
    if (MS3DoCommand(EM, CmdString, &RspPtr, NULL, NULL, NULL) == FAILURE) {
        DBG(2, fCORE)
        DPrint("ALERT:    cannot register for event '%s'\n", EName);
    ...
    DBG(4, fCORE)
    DPrint("INFO:     event '%s' subscription successfully registered\n",
           EName);
...

{
    ...
    MDB(4, fCORE)
    MLog("%s(%s,%s,%s,Port,WProtocol,SProtocol)\n", FName,
         (SName != NULL) ? SName : "NULL", (CName != NULL) ? CName : "NULL",
    ...
    if (MS3DoCommand(&MSched.DS, CmdString, &RspPtr, NULL, NULL, NULL) ==
        FAILURE) {
        MDB(2, fCORE)
        MLog("ALERT:    cannot un-register service '%s'\n", CName);
    ...
    if (MXMLFromString(&E, RspPtr, NULL, NULL) == FAILURE) {
        MDB(2, fCORE)
        MLog("ALERT:    cannot process DS response '%s'\n", RspPtr);
    ...
    if (MXMLGetChild(E, "error", NULL, &RE) == SUCCESS) {
        ...
        MDB(2, fCORE)
        MLog(
            "ALERT:    cannot process DS response '%s' (FailureType: %s  Msg: "
            "'%s'\n",
            RspPtr, EType, Msg);
    ...
    if (MXMLGetChild(E, "locations", NULL, &RE) == FAILURE) {
        MDB(2, fCORE)
        MLog("ALERT:    cannot process DS unregistration response '%s'\n",
             RspPtr);
    ...
    MDB(2, fCORE)
    MLog("INFO:     service '%s' successfully registered\n", SName);
...

{
  ...
  <tier>1</tier>
    ...
    */
    ...
    if (MXMLFromString(&E, RspPtr, NULL, NULL) == FAILURE) {
        MDB(2, fCORE)
        MLog("ALERT:    cannot process DS registration response '%s'\n",
             RspPtr);
    ...
    if (MXMLGetChild(E, "error", NULL, &RE) == SUCCESS) {
        ...
        MDB(2, fCORE)
        MLog(
            "ALERT:    cannot process DS registration response '%s' "
            "(FailureType: %s  Msg: '%s'\n",
            RspPtr, EType, Msg);
...

{
...

    {
        ...
        if (strcmp(MSched.Day, tmpDay) != 0) {
            ...
            DBG(2, fALL)
            DPrint("INFO:     starting new day: %s",
...

    {
        ...
        if (interval < 0) {
            DBG(1, fSCHED)
            fprintf(stderr, "ALERT:    negative interval detected (%ld)\n",
                    interval);
        ...
        if ((MSched.Time - J->StartTime) > MPar[0].MaxJobStartTime) {
            if (RM->Type == mrmtPBS) {
                if ((J->State == mjsIdle) &&
                    ...
                    DBG(2, fCORE)
                    DPrint(
                        "ALERT:    PBS job '%s' in state '%s' was started %s "
                        "ago.  assuming prolog hang and cancelling job\n",
            ...
            for (nindex = 0; nindex < MAX_MNODE_PER_JOB; nindex++) {
                ...
                if (((N->State == mnsIdle) || (N->State == mnsDown)) &&
                    ((MSched.Time - N->StateMTime) > 300)) {
                    DBG(1, fCORE)
                    DPrint(
                        "ALERT:    job '%s' has been in state '%s' for %ld "
                        "seconds.  node '%s' is in state '%s'  (job '%s' will "
                        "be cancelled)\n",
                        J->Name, MJobState[J->State],
                        MSched.Time - J->StartTime, N->Name,
                        MNodeState[N->State], J->Name);

                    sprintf(Message,
                            "JOBCORRUPTION:  job '%s' (user %s) has been in "
                            "state '%s' for %ld seconds.  node '%s' is in "
                            "state '%s'  (job '%s' will be cancelled)\n",
                            J->Name, J->Cred.U->Name, MJobState[J->State],
                            MSched.Time - J->StartTime, N->Name,
                            MNodeState[N->State], J->Name);
                    ...
                    if (N->State == mnsDown) {
                        MRMJobCancel(
                            J, "MAUI_ERROR:  job has 'DOWN' node allocated\n",
                            NULL);
...

{
    ...
    DBG(0, fALL)
    DPrint("INFO:     received signal %d.  shutting down server\n", Signo);

    if (MSysDSUnregister((char *)MS3CName[mpstSC], MRM[0].Name,
                         MSched.ServerHost, MSched.ServerPort, NULL,
                         NULL) == FAILURE) {
        DBG(1, fRM)
        DPrint("ALERT:    cannot unregister with directory service\n");
    }
...

{
    ...
    while (MCfgGetSVal(head, &ptr, MCredCfgParm[mxoSched], IndexName, NULL,
...

{
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MSchedAttr, &aindex, NULL, TRUE, NULL,
        ...
        switch (aindex) {
            case msaFBServer:
                ...
                MUURLParse(ValLine, NULL, S->FBServerHost, NULL, 0,
                           &S->FBServerPort, TRUE);
            ...
            case msaServer:
                ...
                MUURLParse(ValLine, NULL, S->ServerHost, NULL, 0,
                           &S->ServerPort, TRUE);
...

{
    ...
    /* contact primary */
    rc = MCDoCommand(MSched.FBServerHost, MSched.FBServerPort, svcShowQ,
                     CmdString, tmpBuf);

    DBG(2, fALL)
    DPrint("INFO:     connection to FBServer %s:%d %s\n", MSched.FBServerHost,
           MSched.FBServerPort, (rc == SUCCESS) ? "succeeded" : "failed");
...

{
    ...
    sprintf(temp_str,
            "Initialized: S:%s/I:%s  CCount: %d  FCount: %d  QCount: %d  "
            "JCount: %d  RCount: %d\n",
            (MSched.G.SIsInitialized == TRUE) ? "TRUE" : "FALSE",
            (MSched.G.IIsInitialized == TRUE) ? "TRUE" : "FALSE",
            MSched.G.CCount, MSched.G.FailureCount, MSched.G.QCount,
            MSched.G.JCount, MSched.G.RCount);
...

{
    ...
    DBG(0, fALL)
    DPrint("starting %s version %s (PID: %d) on %s", MSCHED_NAME,
           MSCHED_VERSION, MSched.PID, MULToDString(&MSched.Time));
    ...
    MSUInitialize(&MSched.ServerS, NULL, MSched.ServerPort,
                  MSched.ClientTimeout, (1 << TCP));

    if (MSUListen(&MSched.ServerS) == FAILURE) {
        DBG(0, fALL)
        DPrint("ERROR:    cannot open user interface socket on port %d\n",
               MSched.ServerPort);

        fprintf(stderr,
                "ERROR:    cannot open user interface socket on port %d\n",
                MSched.ServerPort);
    ...
    MSUInitialize(&MSched.ServerSH, NULL, MSched.ServerPort + 1,
                  MSched.ClientTimeout, (1 << TCP));

    if (MSUListen(&MSched.ServerSH) == FAILURE) {
        DBG(7, fALL)
        DPrint("ERROR:    cannot open extension interface socket on port %d\n",
               MSched.ServerPort);
    }

    MFSInitialize(&MPar[0].FSC);

    if (MAM[0].Type != mamtNONE) MAMActivate(&MAM[0]);
