...

{
    ...
    for (rmindex = 0; MRM[rmindex].Type != mrmtNONE; rmindex++) {
        ...
        if (R->Name[0] == '\0') {
            sprintf(R->Name, "%s.%d", MRMType[R->Type], R->Index);
        }

        if (MRMFunc[R->Type].RMInitialize == NULL) {
            DBG(7, fRM)
            DPrint(
                "ALERT:    cannot initialize RM (RM '%s' does not support "
                "function '%s')\n",
                R->Name, MRMFuncType[mrmRMInitialize]);
        ...
        if (MRMFunc[R->Type].RMInitialize(R, &SC) == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot initialize RM (RM '%s' failed in function "
                "'%s')\n",
                R->Name, MRMFuncType[mrmRMInitialize]);
    ...
    if (X.XRMInitialize != (int (*)())0) (*X.XRMInitialize)(X.xd, NULL);
...

{
    ...
    if (MSched.Mode == msmSim) {
        ...
        if ((X.XRMGetData != (int (*)())0) &&
            ...
            DBG(0, fRM)
            DPrint("WARNING:  cannot load resource manager data at time %s\n",
            ...
            if (APIFailureCount > MSched.APIFailureThreshhold) {
                sprintf(tmpLine,
                        "RMFAILURE:  API has failed %d consecutive times on %s",
...

{
    ...
    for (rmindex = 0; MRM[rmindex].Type != mrmtNONE; rmindex++) {
        ...
        if (MRMFunc[R->Type].RMEventQuery == NULL) {
            DBG(9, fRM)
            DPrint(
                "ALERT:    cannot query events on RM (RM '%s' does not support "
                "function '%s')\n",
                R->Name, MRMFuncType[mrmRMEventQuery]);
...

{
    ...
    for (rmindex = 0; MRM[rmindex].Type != mrmtNONE; rmindex++) {
        ...
        if (MRMFunc[R->Type].ClusterQuery == NULL) {
            DBG(7, fRM)
            DPrint(
                "ALERT:    cannot load cluster resources on RM (RM '%s' does "
                "not support function '%s')\n",
                R->Name, MRMFuncType[mrmClusterQuery]);
        ...
        if (rc == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot load cluster resources on RM (RM '%s' failed "
                "in function '%s')\n",
                R->Name, MRMFuncType[mrmClusterQuery]);
        ...
        DBG(1, fRM)
        DPrint("INFO:     %d %s resources detected on RM %s\n", tmpRCount,
               MRMType[R->Type], R->Name);

        TotalRCount += tmpRCount;
    } /* END for (rmindex) */

    if (RCount != NULL) *RCount = TotalRCount;
...

{
    ...
    for (rmindex = 0; MRM[rmindex].Type != mrmtNONE; rmindex++) {
        ...
        if (MRMFunc[R->Type].WorkloadQuery == NULL) {
            DBG(7, fRM)
            DPrint(
                "ALERT:    cannot load cluster workload on RM (RM '%s' does "
                "not support function '%s')\n",
                R->Name, MRMFuncType[mrmWorkloadQuery]);
        ...
        if (rc == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot load cluster workload on RM (RM '%s' failed "
                "in function '%s')\n",
                R->Name, MRMFuncType[mrmWorkloadQuery]);
        ...
        DBG(1, fRM)
        DPrint("INFO:     %d %s jobs detected on RM %s\n", tmpWCount,
               MRMType[R->Type], R->Name);

        TotalWCount += tmpWCount;
    } /* END for (rmindex) */

    if (WCount != NULL) *WCount = TotalWCount;
...

{
    ...
    switch (MSched.Mode) {
        ...
        case msmTest:
            ...
            DBG(3, fRM)
            DPrint("INFO:     cannot start job %s (%s)\n", J->Name, MPtr);
    ...
    for (rmindex = 0; rmindex < rmcount; rmindex++) {
        ...
        if (MRMFunc[R->Type].JobStart == NULL) {
            sprintf(MPtr,
                    "cannot start job - RM '%s' does not support function '%s'",
                    R->Name, MRMFuncType[mrmJobStart]);

            DBG(3, fRM)
            DPrint("ALERT:    cannot start job %s (%s)\n", J->Name, MPtr);

            if (SC != NULL) *SC = mscSysFailure;
        ...
        if (rc == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot start job %s (RM '%s' failed in function "
                "'%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobStart]);
...

{
    ...
    switch (MSched.Mode) {
        ...
        case msmTest:
            ...
            DBG(3, fRM)
            DPrint("INFO:     cannot cancel job '%s' (test mode)\n", J->Name);
    ...
    for (rmindex = 0; rmindex < rmcount; rmindex++) {
        ...
        if (MSched.Iteration == R->FailIteration) {
            DBG(3, fRM)
            DPrint("INFO:     cannot cancel job '%s' (fail iteration)\n",
                   J->Name);
        ...
        if (MRMFunc[R->Type].JobCancel == NULL) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot cancel job %s (RM '%s' does not support "
                "function '%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobCancel]);
        ...
        if (rc == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot cancel job %s (RM '%s' failed in function "
                "'%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobCancel]);
...
int MRMJobGetProximateMNL(
    ...
    mjob_t *J, /* I:  req */
    mrm_t *R, mnodelist_t SMNL, mnodelist_t DMNL, long StartTime,
    int DoExactMatch, char *Message, /* I:  opt */
    int *SC)                         /* IO: opt */

{
    ...
    DBG(1, fRM)
    DPrint("%s(R,%s,SMNL,DMNL,%ld,%d,%s,SC)\n", FName,
           (J != NULL) ? J->Name : "NULL", StartTime, DoExactMatch,
           (Message != NULL) ? Message : "NULL");
    ...
    if (MRMFunc[R->Type].JobGetProximateTasks == NULL) {
        DBG(3, fRM)
        DPrint(
            "ALERT:    cannot get proximate tasks for job %s (RM '%s' does not "
            "support function '%s')\n",
            J->Name, R->Name, MRMFuncType[mrmJobGetProximateTasks]);
    ...
    if (rc == FAILURE) {
        DBG(3, fRM)
        DPrint(
            "ALERT:    cannot get proximate tasks for job %s (RM '%s' failed "
            "in function '%s')\n",
            J->Name, R->Name, MRMFuncType[mrmJobGetProximateTasks]);
...

{
    ...
    switch (MSched.Mode) {
        ...
        case msmTest:
            ...
            DBG(3, fRM)
            DPrint("INFO:     cannot modify job '%s' (test mode)\n", J->Name);
    ...
    for (rmindex = 0; rmindex < rmcount; rmindex++) {
        ...
        if (MSched.Iteration == R->FailIteration) {
            DBG(3, fRM)
            DPrint("INFO:     cannot modify job '%s' (fail iteration)\n",
                   J->Name);
        ...
        if (MRMFunc[R->Type].JobModify == NULL) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot modify job %s (RM '%s' does not support "
                "function '%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobModify]);

            return (FAILURE);
        }

        if (MRMFunc[R->Type].JobModify(J, R, Attr, SubAttr, Value, NULL, SC) ==
            FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot modify job %s (RM '%s' failed in function "
                "'%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobModify]);
...

{
    ...
    switch (MSched.Mode) {
        ...
        case msmTest:
            ...
            DBG(3, fRM)
            DPrint("INFO:     cannot migrate job '%s' (test mode)\n", J->Name);
    ...
    for (rmindex = 0; rmindex < rmcount; rmindex++) {
        ...
        if (MRMFunc[R->Type].JobMigrate == NULL) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot migrate job %s (RM '%s' does not support "
                "function '%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobMigrate]);
        ...
        if (MRMFunc[R->Type].JobMigrate(J, R, NL, NULL, SC) == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot migrate job %s (RM '%s' failed in function "
                "'%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobMigrate]);
...

{
    ...
    switch (MSched.Mode) {
        ...
        case msmTest:
            ...
            DBG(3, fRM)
            DPrint("INFO:     cannot requeue job '%s' (test mode)\n", J->Name);
    ...
    for (rmindex = 0; rmindex < rmcount; rmindex++) {
        ...
        if (MRMFunc[R->Type].JobRequeue == NULL) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot requeue job %s (RM '%s' does not support "
                "function '%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobRequeue]);
        ...
        if (MRMFunc[R->Type].JobRequeue(J, R, JPeer, NULL, SC) == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot requeue job %s (RM '%s' failed in function "
                "'%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobRequeue]);
...
int MRMJobSubmit(
    ...
    char *JobDesc, mrm_t *R, mjob_t **JPtr, int *SC)

{
    ...
    while (JobString != NULL) {
        ...
        MUSScanF(JobString, "%ld %ld %x%s %x%s %x%s", &ProcCount, &Walltime,
                 sizeof(QueueName), QueueName, sizeof(ExecString), ExecString,
                 sizeof(JobName), JobName);
        ...
        */
        ...
        if (MReqCreate(J, NULL, &RQ, FALSE) == FAILURE) {
            DBG(1, fPBS)
            DPrint("ALERT:    cannot add requirements to job '%s'\n", J->Name);
        ...
        if (MJobSetCreds(J, "DEFAULT", "DEFAULT", "DEFAULT") == FAILURE) {
            DBG(1, fPBS)
            DPrint(
                "ERROR:    cannot authenticate job '%s' (U: %s  G: %s  A: "
                "'%s')\n",
                J->Name, "DEFAULT", "DEFAULT", "DEFAULT");
        ...
        if (MClassFind(QueueName, &C) == FAILURE) {
            DBG(1, fPBS)
            DPrint(
                "ERROR:    cannot locate requested class '%s' for job '%s'\n",
                QueueName, J->Name);
...

{
    ...
    for (rmindex = 0; rmindex < rmcount; rmindex++) {
        ...
        if (MRMFunc[R->Type].JobSuspend == NULL) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot suspend job %s (RM '%s' does not support "
                "function '%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobSuspend]);
        ...
        if (MRMFunc[R->Type].JobSuspend(J, R, Msg, SC) == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot suspend job %s (RM '%s' failed in function "
                "'%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobSuspend]);
...

{
    ...
    for (rmindex = 0; rmindex < rmcount; rmindex++) {
        ...
        if (MRMFunc[R->Type].JobCheckpoint == NULL) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot checkpoint job %s (RM '%s' does not support "
                "function '%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobCheckpoint]);

            return (FAILURE);
        }

        if (MRMFunc[R->Type].JobCheckpoint(J, R, TerminateJob, Msg, SC) ==
            FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot checkpoint job %s (RM '%s' failed in "
                "function '%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobCheckpoint]);
...

{
    ...
    for (rmindex = 0; rmindex < rmcount; rmindex++) {
        ...
        if (MRMFunc[R->Type].JobResume == NULL) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot resume job %s (RM '%s' does not support "
                "function '%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobResume]);
        ...
        if (MRMFunc[R->Type].JobResume(J, R, Msg, SC) == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot resume job %s (RM '%s' failed in function "
                "'%s')\n",
                J->Name, R->Name, MRMFuncType[mrmJobResume]);
    ...
    for (nindex = 0; J->NodeList[nindex].N != NULL; nindex++) {
        ...
        if (RQ->SDRes != NULL)
            MCResRemove(&N->ARes, &N->CRes, RQ->SDRes, J->NodeList[nindex].TC,
                        TRUE);
        else
            MCResRemove(&N->ARes, &N->CRes, &RQ->DRes, J->NodeList[nindex].TC,
                        TRUE);

        if (RQ->SDRes != NULL)
            MCResAdd(&N->DRes, &N->CRes, RQ->SDRes, J->NodeList[nindex].TC,
                     TRUE);
        else
            MCResAdd(&N->DRes, &N->CRes, &RQ->DRes, J->NodeList[nindex].TC,
                     TRUE);
    } /* END for (nindex) */

    DBG(2, fSIM)
    DPrint("INFO:     job %s successfully resumed on %d procs\n", J->Name,
           J->Request.TC);
...

{
    ...
    if ((RMName == NULL) || (RMName[0] == '\0')) {
        ...
        while (MCfgGetSVal(MSched.ConfigBuffer, &ptr, MRMCfgParam, IndexName,
                           NULL, Value, sizeof(Value), 0, NULL) != FAILURE) {
        ...
        while (MCfgGetSVal(MSched.ConfigBuffer, &ptr, MRMCfgParam, RMName, NULL,
                           Value, sizeof(Value), 0, NULL) == SUCCESS) {
            if ((R == NULL) && (MRMFind(RMName, &R) == FAILURE)) {
...

{
    ...
    for (rmindex = 0; rmindex < MAX_MRM; rmindex++) {
        ...
        if (R == NULL) {
            if (MRMCreate(RMName, &MRM[rmindex]) == FAILURE) {
                DBG(1, fALL)
                DPrint("ERROR:    cannot alloc memory for RM '%s'\n", RMName);
        ...
        if (MSched.Mode != msmSim) MCPRestore(mcpRM, RMName, (void *)R);
    ...
    DBG(1, fSTRUCT)
    DPrint("ALERT:    RM table overflow.  cannot add %s\n", RMName);
...

{
    ...
    for (rmindex = 0; rmindex < MAX_MRM; rmindex++) {
        ...
        if (strcmp(R->Name, RMName) != 0) continue;
...

{
    ...
    switch (AIndex) {
        case mrmaAuthType:
            ...
            if (R->AuthType != 0) strcpy(SVal, MRMAuthType[R->AuthType]);
        ...
        case mrmaHost:
            ...
            if (R->P[0].HostName != NULL) strcpy(SVal, R->P[0].HostName);
        ...
        case mrmaName:
            ...
            if (R->Name[0] != '\0') strcpy(SVal, R->Name);
        ...
        case mrmaNMPort:
            ...
            if (R->NMPort != 0) {
                /* NOTE:  NMPort affect connection to BOTH global node monitor
                 * and local node manager */

                sprintf(SVal, "%d", R->NMPort);
            }
        ...
        case mrmaNMServer:
            ...
            if (R->P[1].HostName != NULL) strcpy(SVal, R->P[1].HostName);
        ...
        case mrmaSuspendSig:
            ...
            if (R->SuspendSig[0] != '\0') strcpy(SVal, R->SuspendSig);
        ...
        case mrmaVersion:
            ...
            if (R->APIVersion[0] != '\0') strcpy(SVal, R->APIVersion);
...

{
    ...
    if ((R == NULL) || (Value == NULL) || (Value[0] == '\0')) {
        return (FAILURE);
    }
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MRMAttr, &aindex, NULL, TRUE, NULL,
        ...
        switch (aindex) {
            ...
            case mrmaConfigFile:
                ...
                MUStrCpy(R->U.LL.ConfigFile, ValLine,
                         sizeof(R->U.LL.ConfigFile));
            ...
            case mrmaLocalDiskFS:
                ...
                MUStrCpy(R->U.PBS.LocalDiskFS, ValLine,
                         sizeof(R->U.PBS.LocalDiskFS));
            ...
            case mrmaSocketProtocol:
                ...
                R->P[0].SocketProtocol =
                    MUGetIndex(ValLine, MSockProtocol, FALSE, 0);
            ...
            case mrmaWireProtocol:
                ...
                R->P[0].WireProtocol =
                    MUGetIndex(ValLine, MWireProtocol, FALSE, 0);
            ...
            default:
                ...
                DBG(4, fRM)
                DPrint("WARNING:  RM attribute '%s' not handled\n",
                       MRMAttr[aindex]);
...

{
    ...
    for (rmindex = 0; MRM[rmindex].Type != mrmtNONE; rmindex++) {
        ...
        MUSNPrintF(&BPtr, &BSpace, "RM[%s]  type: '%s'  state: '%s'\n", R->Name,
                   MRMType[R->Type], MRMState[R->State]);

        /* NOTE:  display optional RM version, failures, and fault tolerance
         * config */
        ...
        if (R->RespTotalCount[0] > 0) {
            MUSNPrintF(
                &BPtr, &BSpace,
                "  RM Performance:  Avg Time: %.2lfs  Max Time:  %.2lfs  (%d "
                "samples)\n",
                (double)R->RespTotalTime[0] / R->RespTotalCount[0] / 1000,
                (double)R->RespMaxTime[0] / 1000, R->RespTotalCount[0]);
        }
        ...
        for (index = 0; index < MAX_MRMFAILURE; index++) {
            ...
            if (R->FailTime[findex] <= 0) continue;
...

{
    ...
    const int AList[] = {mrmaAuthType,    mrmaConfigFile,   mrmaCSAlgo,
                         mrmaCSKey,       mrmaEPort,        mrmaHost,
                         mrmaLocalDiskFS, mrmaMinETime,     mrmaNMPort,
                         mrmaNMServer,    mrmaPort,         mrmaSocketProtocol,
                         mrmaSuspendSig,  mrmaTimeout,      mrmaType,
                         mrmaVersion,     mrmaWireProtocol, -1};

    const char *FName = "MRMConfigShow";

    DBG(6, fSTRUCT)
    DPrint("%s(%s,%d,%s,%d)\n", FName, (RP != NULL) ? "RP" : "NULL", Mode,
           (Buffer != NULL) ? "Buffer" : "NULL", BufSize);
    ...
    for (rmindex = 0; rmindex < MAX_MRM; rmindex++) {
        ...
        if (R->Type == mrmtNONE) continue;

        sprintf(tmpLine, "RMCFG[%s]", R->Name);
        ...
        for (aindex = 0; AList[aindex] != -1; aindex++) {
            ...
            if (tmpVal[0] != '\0') {
                sprintf(temp_str, " %s=%s", MRMAttr[AList[aindex]], tmpVal);
...

{
    ...
    for (rmindex = 0; rmindex < MAX_MRM; rmindex++) {
        ...
        if (!VFlag && (PIndex != -1) && (PIndex != pRMType)) continue;

        if (!memcmp(&ZR, R, sizeof(ZR))) continue;

        if ((R->Type == mrmtNONE) && !VFlag) continue;

        if (R->Type == mrmtLL) {
            if ((R->U.LL.ConfigFile[0] != '\0') ||
                (VFlag || (PIndex == -1) || (PIndex == pRMConfigFile)))
                strcat(Buffer, MUShowSArray(MParam[pRMConfigFile], rmindex,
                                            R->U.LL.ConfigFile));
        }

        if ((VFlag || (PIndex == -1) || (PIndex == pRMAuthType)))
            strcat(Buffer, MUShowSArray(MParam[pRMAuthType], rmindex,
                                        (char *)MRMAuthType[R->AuthType]));
...

{
    ...
    for (rmindex = 0; MRM[rmindex].Type != mrmtNONE; rmindex++) {
        ...
        if (MRMFunc[R->Type].CycleFinalize == NULL) {
            DBG(7, fRM)
            DPrint(
                "INFO:     cannot finalize RM cycle (RM '%s' does not support "
                "function '%s')\n",
                R->Name, MRMFuncType[mrmCycleFinalize]);
        ...
        if (MRMFunc[R->Type].CycleFinalize(R, &SC) == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot finalize RM cycle (RM '%s' failed in "
                "function '%s')\n",
                R->Name, MRMFuncType[mrmCycleFinalize]);
...

{
    ...
    switch (AIndex) {
        case mrmaCSAlgo:
            ...
            R->P[0].CSAlgo =
                MUGetIndex((char *)Value, MCSAlgoType, FALSE, R->P[0].CSAlgo);
            R->P[1].CSAlgo =
                MUGetIndex((char *)Value, MCSAlgoType, FALSE, R->P[1].CSAlgo);
        ...
        case mrmaHost:
            ...
            if (Value != NULL) strcpy(R->P[0].HostName, (char *)Value);
        ...
        case mrmaSuspendSig:
            ...
            if (Format == mdfString) {
                ...
                sprintf(R->SuspendSig, "%d", *(int *)Value);
            }
...

{
    ...
    DPrint("%s(%s,%s,%s)\n", FName, (N != NULL) ? N->Name : "NULL",
           MAList[eNodeState][NState], (R != NULL) ? R->Name : "NULL");
    ...
    if (MSched.DefaultN.CRes.Disk > 0) N->CRes.Disk = MSched.DefaultN.CRes.Disk;
...

{
    ...
    if (strcmp(N->Name, tmpName) != 0) MUStrDup(&N->FullName, tmpName);
    ...
    if (N->ARes.Mem == -1) N->ARes.Mem = N->CRes.Mem;

    N->CRes.Swap = MAX(N->ARes.Swap, N->CRes.Swap);

    if (N->ARes.Swap == -1) N->ARes.Swap = N->CRes.Swap;

    N->CRes.Disk = MAX(N->ARes.Disk, N->CRes.Disk);

    if (N->ARes.Disk == -1) N->ARes.Disk = N->CRes.Disk;

    if (N->URes.Procs == -1) N->URes.Procs = (int)N->Load;
    ...
    if (N->CRes.Procs == -1) N->CRes.Procs = 1;

    if ((MPar[0].NAvailPolicy[0] == mrapUtilized) ||
        (MPar[0].NAvailPolicy[0] == mrapCombined)) {
        /* use utilized resource info */

        if ((MSched.MaxOSCPULoad > 0.0) && (N->DRes.Procs == 0) &&
            (N->ARes.Procs < N->CRes.Procs)) {
            N->ARes.Procs = MAX(
                0, N->ARes.Procs - (int)(MSched.MaxOSCPULoad * N->CRes.Procs));
        }
    ...
    if (N->ProcSpeed > 0)
        MSched.ReferenceProcSpeed =
            MIN(MSched.ReferenceProcSpeed, N->ProcSpeed);

    if (N->Speed <= 0.0) N->Speed = 1.0;
    ...
    if ((N->MaxProcPerClass != NULL) ||
        ...
        for (cindex = 0; cindex < MAX_MCLASS; cindex++) {
            if (N->CRes.PSlot[cindex].count <= 0) continue;

            if ((N->MaxProcPerClass != NULL) &&
                (N->MaxProcPerClass[cindex] > 0)) {
            ...
            N->CRes.PSlot[cindex].count =
                MIN(N->CRes.PSlot[cindex].count, tmpI);
            N->CRes.PSlot[cindex].count =
                MIN(N->CRes.PSlot[cindex].count, tmpI);
        } /* END for (cindex) */
...

{
    ...
    if ((N->State != mnsDown) && (N->State != mnsNone) &&
        (N->State != mnsFlush) && (N->State != mnsDraining)) {
    ...
    if (((N->EState == mnsActive) || (N->EState == mnsBusy)) &&
        ...
        if (MNodeCheckAllocation(N) != SUCCESS) {
            DBG(4, fRM)
            DPrint(
                "ALERT:    node '%s' expected Busy but is Idle and not "
                "allocated\n",
                N->Name);
            ...
            DBG(4, fRM)
            DPrint(
                "ALERT:    node '%s' expected Active but is Idle and "
                "allocated\n",
                N->Name);
        }
    }
    ...
    if (N->State != OldState) {
        DBG(2, fRM)
        DPrint("INFO:     node '%s' changed states from %s to %s\n", N->Name,
               MAList[eNodeState][OldState], MAList[eNodeState][N->State]);
        ...
        if ((N->State == mnsDown) || (N->State == mnsDraining) ||
            (N->State == mnsDrained) || (N->State == mnsFlush)) {
            N->EState = N->State;
            N->SyncDeadLine = MAX_MTIME;
        }
        ...
        if ((OldState == mnsDown) || (OldState == mnsDraining) ||
            (OldState == mnsDrained) || (OldState == mnsFlush) ||
        ...
        if ((OldState == mnsIdle) && (N->State == mnsBusy)) {
            if (MSched.Mode != msmTest) {
                DBG(1, fRM)
                DPrint(
                    "ALERT:    unexpected node transition on node '%s'  Idle "
                    "-> Busy\n",
                    N->Name);
            }
...

{
    ...
    if (N->CRes.Disk == -1) N->CRes.Disk = 1;
    ...
    */
    ...
    /* unspecifed state, DRes.Procs, and ARes.Procs handled in
     * MNodeUpdateState() */

    if ((N->State != N->EState) && (MSched.Time > N->SyncDeadLine)) {
        DBG(3, fRM)
        DPrint(
            "INFO:     synchronizing node '%s' to state '%s' (expected state: "
            "'%s')\n",
...

{
    ...
    DBG(5, fRM)
    DPrint("%s(J,%s,%d)\n", FName, (JobName != NULL) ? JobName : "NULL",
           RMIndex);
    ...
    if (JobName != NULL) MUStrCpy(J->Name, JobName, sizeof(J->Name));
...

{
    ...
    DBG(5, fRM)
    DPrint("%s(%s,TaskList,%s)\n", FName, (J != NULL) ? J->Name : "NULL",
           (R != NULL) ? R->Name : "NULL");

    if ((J == NULL) || (R == NULL)) {
        DBG(1, fRM)
        DPrint("ERROR:    invalid job/RM pointer passed to %s()\n", FName);
    ...
    if (J->Cred.C != NULL) {
        if (J->Cred.C->NonExeType == TRUE) MJobSetState(J, mjsNotQueued);
    ...
    if ((J->SpecWCLimit[1] == 0) || (J->SpecWCLimit[1] == -1)) {
        DBG(4, fRM)
        DPrint("WARNING:  wallclock limit not specified for job %s\n", J->Name);

        if ((J->Cred.Q != NULL) && (J->Cred.Q->L.JDef != NULL) &&
            (((mjob_t *)J->Cred.Q->L.JDef)->SpecWCLimit[0] > 0)) {
            J->SpecWCLimit[1] = ((mjob_t *)J->Cred.Q->L.JDef)->SpecWCLimit[0];
        } else if ((J->Cred.C != NULL) && (J->Cred.C->L.JDef != NULL) &&
        ...
        DBG(4, fRM)
        DPrint("INFO:     wallclock limit set to %s on job %s\n",
               MULToTString(J->SpecWCLimit[1]), J->Name);
    }
    ...
    if ((MPar[0].UseMachineSpeed == FALSE) ||
        ...
        if (tmpD > 0.0) J->WCLimit = (long)((double)J->SpecWCLimit[0] / tmpD);
    }
    ...
    if (TaskList != NULL) {
        ...
        if ((J->NodeList == NULL) &&
            ((J->NodeList = (mnalloc_t *)calloc(
                  1, sizeof(mnalloc_t) * (MAX_MNODE_PER_JOB + 1))) == NULL)) {
            DBG(1, fRM)
            DPrint(
                "ALERT:    cannot allocate memory for job %s nodelist, errno: "
                "%d (%s)\n",
                J->Name, errno, strerror(errno));
        ...
        if (J->Req[0] != NULL) {
            if (tindex == 0) {
                for (tindex = 0; tindex < MAX_MNODE_PER_JOB; tindex++) {
                    if (J->NodeList[tindex].N == NULL) break;
                }
            }

            memcpy(J->Req[0]->NodeList, J->NodeList,
    ...
    if (MJobCheckClassJLimits(J, J->Cred.C, FALSE, tmpLine, sizeof(tmpLine)) ==
        FAILURE) {
        char tmpMsg[MAX_MLINE];

        /* job does not meet class constraints */

        sprintf(tmpMsg, "job violates class configuration '%s'", tmpLine);

        MJobSetHold(J, (1 << mhBatch), MSched.DeferTime, mhrPolicyViolation,
                    tmpLine);
    ...
    if (MAM[0].FallbackAccount[0] != '\0') {
        if (MAMAllocJReserve(&MAM[0], J, TRUE, &Reason, ErrMsg) == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "WARNING:  cannot reserve allocation for job '%s', reason: "
                "%s\n",
            ...
            if (Reason == mhrNoFunds || Reason == mhrAMFailure) {
                ...
                if (MAM[0].FallbackAccount[0] == '+') {
                    snprintf(tmpFBAccount, sizeof(tmpFBAccount), "%s%s",
                             J->Cred.A->Name, MAM[0].FallbackAccount + 1);
                } else {
                ...
                DBG(3, fRM)
                DPrint(
                    "WARNING:  insufficient allocation in account '%s' to run "
                    "job '%s' (attempting fallback account '%s')\n",
                    J->Cred.A->Name, J->Name, tmpFBAccount);
                ...
                if (MAcctFind(tmpFBAccount, &A) == SUCCESS) {
                    ...
                    DBG(2, fRM)
                    DPrint("ALERT:    cannot locate fallback account '%s'\n",
                           tmpFBAccount);
                }
            }
        } else {
/* adequate allocations available on primary account, release 'test' bank
 * reservation */

#if !defined(__MQBANK29)
            if (MAMAllocResCancel(J->Cred.A->Name, J->Name, "allocation test",
                                  NULL, &Reason) == FAILURE) {
                DBG(1, fRM)
                DPrint(
    ...
    if ((MQOSGetAccess(J, J->QReq, NULL, &QDef) == FAILURE) ||
        ...
        if ((J->QReq != NULL) && (AccountFallback == FALSE)) {
            DBG(2, fRM)
            DPrint(
                "ALERT:    job '%s' cannot use requested QOS %s "
                "(deferring/setting QOS to default %s)\n",
                J->Name, J->QReq->Name, QDef->Name);

            if (MSched.Mode != msmSim) {
                /* job cannot run in simulation mode w/o admin intervention */

                MJobSetHold(J, (1 << mhDefer), MSched.DeferTime, mhrQOSAccess,
    ...
    if (MJobGetPAL(J, J->SpecPAL, J->PAL, NULL) == FAILURE) {
        ...
        DBG(2, fRM)
        DPrint("ALERT:    cannot utilize requested PAL '%x'\n", J->SpecPAL[0]);
    }
    ...
    if (J->MasterJobName != NULL) {
        ...
        if (MJobFind(J->MasterJobName, &MJ, 0) == FAILURE) {
            DBG(3, fRM)
            DPrint(
                "ALERT:    cannot locate master job '%s' for subjob '%s' "
                "(cancelling subjob)\n",
            ...
            for (rqindex = 0; MJ->Req[rqindex] != NULL; rqindex++) {
                if (MJ->Req[rqindex]->RMIndex == R->Index) {
                    ...
                    if (MJ->IState == mjsStaging) {
                        ...
                        DBG(3, fRM)
                        DPrint(
                            "INFO:     located job '%s' subjob[%d] '%s' (RM: "
                            "%s)\n",
                            MJ->Name, rqindex, J->Name, R->Name);
                    } else {
                        DBG(3, fRM)
                        DPrint(
                            "ALERT:    master job '%s' in state '%s' "
                            "subjob[%d] '%s' (RM: %s) loaded\n",
                            MJ->Name, MAList[eJobState][MJ->State], rqindex,
                            J->Name, R->Name);
                    }
                } /* END if (MJ->Req[index].RMIndex == R->Index) */

                if (MJ->Req[rqindex]->State == mjsStaged) sjcount++;
            } /* END  for (rqindex) */
            ...
            if (sjcount == rqindex) {
                if (MJ->IState == mjsStaging) {
                    DBG(1, fRM)
                    DPrint(
                        "INFO:     all subjobs located.  starting masterjob "
                        "'%s'\n",
                        MJ->Name);
    ...
    else  /* (J->MasterJobName != NULL) */
    {
        if ((J->State == mjsIdle) && (J->EState != mjsDeferred)) {
            ...
            DBG(4, fRM)
            DPrint("INFO:     allocated job '%s' detected\n", J->Name);
            ...
            if (J->TaskCount == 0) {
                DBG(2, fRM)
                DPrint(
                    "WARNING:  job '%s' loaded in allocated state with no "
                    "tasks allocated\n",
                    J->Name);

                DBG(2, fRM)
                DPrint(
                    "INFO:     adjusting allocated procs to %d for job '%s'\n",
                    MJobGetProcCount(J), J->Name);
            ...
            if (J->AWallTime <= 0)
                ...
                J->AWallTime = MIN(J->AWallTime,
                                   MSched.Time - J->StartTime - J->SWallTime);
            ...
            for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                ...
                for (nindex = 0; RQ->NodeList[nindex].N != NULL; nindex++) {
                    if (RQ->NodeList[nindex].TC == 0) break;
            ...
            if (J->DispatchTime == 0) {
                DBG(0, fRM)
                DPrint(
                    "ERROR:    job '%s' loaded in alloc state '%s' with NULL "
                    "dispatch time\n",
            ...
            if ((RQ->NodeList[0].N == NULL) || (RQ->NodeList[0].TC == 0))
                memcpy(RQ->NodeList, J->NodeList,
    ...
    /* FORMAT:                         N           TR  UN GN    WC   JS   UP
     * QT NET  OS  AR  MC MEM  DC DSK */

    DBG(1, fRM)
    DPrint(
        "INFO:     job '%s' loaded: %3d %8s %8s %6ld %10s %3ld %10ld %8s %6s "
        "%6s %2s %6d %2s %6d %s %ld\n",
        J->Name, J->Request.TC, J->Cred.U->Name, J->Cred.G->Name, J->WCLimit,
        MJobState[J->State], J->UPriority, J->SubmitTime,
        ...
        MUMAList(eFeature, RQ->ReqFBM, sizeof(RQ->ReqFBM)), J->ATime);

    DBG(6, fRM)
    DPrint("INFO:     job '%s' size: %d + %d\n", J->Name, J->ImageSize,
           J->ExecSize);

    if (MSched.Mode == msmSim) {
        /* NYI */
    } else if (MSched.Mode == msmNormal) {
        MOSSyslog(LOG_INFO, "new job %s loaded", J->Name);
    }
...

{
    ...
    DBG(5, fRM)
    DPrint("%s(%s,TaskList,%s,%s)\n", FName, (J != NULL) ? J->Name : "NULL",
           MAList[eJobState][OldState], (R != NULL) ? R->Name : "NULL");
    ...
    switch (OldState) {
        ...
        case mjsRunning:
            ...
            if (J->AWallTime <= 0) {
                J->AWallTime =
                    MAX(0, (long)(MSched.Time - J->StartTime - J->SWallTime));
        ...
        case mjsSystemHold:
            ...
            if ((J->State == mjsIdle) || (J->State == mjsNotQueued) ||
                (J->State == mjsHold) || (J->State == mjsSystemHold)) {
                ...
                DBG(1, fRM)
                DPrint("ALERT:   job '%s' was cancelled externally\n", J->Name);
    ...
    if (J->SpecWCLimit[1] == 0) {
        ...
        DBG(4, fRM)
        DPrint("INFO:     wallclock limit set to unlimited to job %s\n",
               J->Name);
    ...
    if (MPar[0].L.JP->HLimit[mptMaxWC][0] > 0)
      J->SpecWCLimit[1] =
    MIN(J->SpecWCLimit[1],MPar[0].L.JP->HLimit[mptMaxWC][0]);
    */

    /* check class constraints */

    if (MJobCheckClassJLimits(J, J->Cred.C, FALSE, tmpLine, MAX_MLINE) ==
        FAILURE) {
        char tmpMsg[MAX_MLINE];
    ...
    if (MJOBISACTIVE(J)) {
        ...
        if ((OldState == mjsHold) || (OldState == mjsIdle) ||
            (OldState == mjsNotRun) || (J->NodeList[0].N == NULL)) {
            /* build new job list */
                ...
                for (tindex = 0; TaskList[tindex] != -1; tindex++) {
                    if (J->TaskMap[tindex] != TaskList[tindex]) {
                        if (JobHasMap == TRUE) {
                            DBG(1, fRM)
                            DPrint(
                                "ALERT:    task %d changed from '%s' to '%s' "
                                "for active job '%s'\n",
                                tindex, MNode[J->TaskMap[tindex]]->Name,
                                MNode[TaskList[tindex]]->Name, J->Name);
                        }
        ...
        if (TaskList != NULL) {
            ...
            for (tindex = 0; TaskList[tindex] != -1; tindex++) {
                ...
                if (tindex > MAX_MTASK_PER_JOB) {
                    DBG(0, fRM)
                    DPrint(
                        "WARNING:  job size exceeds internal maximum job size "
                        "(%d)\n",
                        MAX_MTASK_PER_JOB);
                ...
                if (BuildTaskMap == TRUE) {
                    ...
                    for (jnindex = 0; J->NodeList[jnindex].N != NULL;
                         jnindex++) {
                ...
                DBG(6, fRM)
                DPrint("INFO:     task %d assigned to job '%s'\n", tindex,
                       J->Name);

                if (N->State == mnsIdle) {
                    DBG(3, fRM)
                    DPrint(
                        "ALERT:    RM state corruption.  job '%s' has idle "
                        "node '%s' allocated (node forced to active state)\n",
                        J->Name, N->Name);

                    if (Message[0] == '\0') {
                        sprintf(Message,
                                "JOBCORRUPTION:  job '%s' has the following "
                                "idle node(s) allocated:\n",
                                J->Name);
                    }

                    if (strlen(Message) + 100 < sizeof(Message)) {
                        sprintf(temp_str, " '%s' ", N->Name);
            ...
            if (BuildTaskMap == TRUE) J->TaskMap[tindex] = -1;
            ...
            for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                ...
                for (nindex = SNIndex; J->NodeList[nindex].TC > 0; nindex++) {
                    if (J->NodeList[nindex].N == NULL) break;
        ...
        if (J->StartTime == 0) {
            DBG(0, fRM)
            DPrint(
                "ERROR:    job '%s' updated in active state '%s' with NULL "
                "start time\n",
                J->Name, MJobState[J->State]);
        ...
        if (J->TaskCount <= 0) {
            DBG(2, fRM)
            DPrint(
                "WARNING:  job '%s' loaded in active state with no tasks "
                "allocated\n",
                J->Name);

            DBG(2, fRM)
            DPrint("INFO:     adjusting allocated tasks for job '%s'\n",
                   J->Name);
    ...
    if (MJOBISCOMPLETE(J)) {
        if (J->StartTime == 0) {
            DBG(0, fRM)
            DPrint(
                "ERROR:    Job[%03d] '%s' updated to completed state '%s' with "
                "NULL start time\n",
                J->Index, J->Name, MJobState[J->State]);
        ...
        if (J->CompletionTime <= J->StartTime) {
            J->CompletionTime = MIN(MSched.Time, J->StartTime + J->SWallTime +
                                                     MAX(J->AWallTime, 1));
        }

        if (J->TaskCount <= 0) J->TaskCount = J->Request.TC;
    } /* END if (MJOBISCOMPLETE(J)) */

    /* handle rejected jobs */

    if (((J->EState == mjsRunning) || (J->EState == mjsStarting)) &&
        ((J->State == mjsIdle) || (J->State == mjsHold))) {
        if (J->Cred.A != NULL) {
            if (MAMAllocResCancel(J->Cred.A->Name, J->Name, "job rejected",
                                  NULL,
                                  (enum MHoldReasonEnum *)&SC) == FAILURE) {
                DBG(1, fRM)
                DPrint(
                    "ERROR:    cannot cancel account reservation for job "
                    "'%s'\n",
                    J->Name);
            }
        }
        ...
        DBG(2, fRM)
        DPrint(
            "ALERT:    job '%s' was in state '%s' but is now in state '%s' "
            "(job was rejected)\n",
            J->Name, MJobState[J->EState], MJobState[J->State]);

        DBG(2, fRM)
        DPrint("ALERT:    job '%s' is being deferred for %ld seconds\n",
               J->Name, DeferTime);

        DBG(4, fRM) {
            DBG(4, fRM) DPrint("INFO:     job '%s' nodelist: ", J->Name);

            for (index = 0; index < MAX_MNODE_PER_JOB; index++) {
                if (J->NodeList[index].N == NULL) break;

                fprintf(mlog.logfp, "[%16s]", J->NodeList[index].N->Name);
            } /* END for (index) */

            fprintf(mlog.logfp, "\n");
        }

        MJobSetHold(J, (1 << mhDefer), MSched.DeferTime, mhrRMReject,
                    "job rejected by RM");
    ...
    if (MJOBISACTIVE(J)) {
        ...
        if ((OldState == mjsHold) || (OldState == mjsIdle) ||
            ...
            if ((Message[0] != '\0') && (MSched.Time - J->StartTime > 150)) {
    ...
    if (((OldState == mjsHold) || (OldState == mjsIdle) ||
         (OldState == mjsNotRun)) &&
        ((J->State == mjsRemoved) || (J->State == mjsCompleted) ||
         (J->State == mjsVacated))) {
        DBG(4, fRM)
        DPrint("INFO:     job '%s' state transition (%s --> %s)\n", J->Name,
               MJobState[OldState], MJobState[J->State]);

        /* if job starts and completes while scheduler is sleeping (cannot
         * happen with x scheduling) */

        if (J->EState != mjsRunning) {
            DBG(0, fRM)
            DPrint(
                "WARNING:  scheduler cannot handle completion in 1 iteration "
                "on job '%s'\n",
                J->Name);

            /* set job to active so it can be properly removed */

            if (MAMAllocJReserve(&MAM[0], J, FALSE, (enum MHoldReasonEnum *)&SC,
                                 Message) == FAILURE) {
                DBG(3, fRM)
                DPrint("WARNING:  cannot create AM reservation for job '%s'\n",
                       J->Name);
            }
            ...
            if (J->DispatchTime == 0) {
                if (J->State == mjsCompleted) {
                    DBG(0, fRM)
                    DPrint(
                        "ERROR:    Job[%03d] '%s' updated to completed state "
                        "'%s' with NULL dispatch time\n",
                        J->Index, J->Name, MJobState[J->State]);
                }
    ...
    if (J->State != J->EState) {
        if (MSched.Time > J->SyncDeadLine) {
            ...
            if (J->EState == mjsDeferred) {
                DBG(2, fRM)
                DPrint("INFO:     restoring job '%s' from deferred state\n",
                       J->Name);
    ...
    if ((J->NodeList[0].N == NULL) && (RQ->NodeList[0].N != NULL)) {
        memcpy(J->NodeList, RQ->NodeList,
               sizeof(mnalloc_t) * MAX_MNODE_PER_JOB);
    }
...
int MRMProcessOConfig(
    ...
    mrm_t *R, /* I (modified) */
    int PIndex, int IVal, double DVal, char *SVal, char **SArray)

{
    ...
    switch (PIndex) {
        ...
        case pRMTimeout:
            ...
            if (R->P[0].Timeout > 1000) R->P[0].Timeout = 1000;
...

{
        ...
        if ((N->CRes.PSlot[cindex].count > 0) && (C->IsDisabled == FALSE)) {
            ...
            if (C->MaxProcPerNode > 0)
                N->ARes.PSlot[cindex].count =
                    MIN(N->ARes.PSlot[cindex].count, C->MaxProcPerNode);

            if (N->AP.HLimit[mptMaxProc][0] > 0)
                N->ARes.PSlot[cindex].count = MIN(N->ARes.PSlot[cindex].count,
    ...
    N->ARes.PSlot[0].count =
        MIN(N->CRes.PSlot[0].count, N->ARes.PSlot[0].count);
    ...
    if (N->AP.HLimit[mptMaxProc][0] > 0)
        N->ARes.PSlot[0].count =
            MIN(N->ARes.PSlot[0].count, N->AP.HLimit[mptMaxProc][0]);
