...

{
    ...
    for (index = 0; index < MAX_FILECACHE; index++) {
        if (MFileCache[index].Buffer != NULL) free(MFileCache[index].Buffer);
    } /* END for (index) */
...

{
    ...
    if ((count = (int)fread(ptr, ReadCount, BlockSize, dfp)) < 0) {
        MOSSyslog(LOG_ERR, "cannot read file '%s', errno: %d (%s)", FileName,
...

{
    ...
    if (stat(IndexFileName, &sbuf) == -1) {
        ...
        if ((ifp = fopen(IndexFileName, "w+")) == NULL) {
            DBG(2, fCORE)
            DPrint("WARNING:  cannot open index file '%s', errno: %d (%s)\n",
                   IndexFileName, errno, strerror(errno));
        ...
        sprintf(rindex.Key, "%08d", RecordCount);

        if (fwrite(&rindex, sizeof(rindex_t), 1, ifp) < 1) {
            DBG(2, fCORE)
            DPrint(
                "WARNING:  cannot write to index file '%s', errno: %d (%s)\n",
                IndexFileName, errno, strerror(errno));
        ...
        if ((Index = (rindex_t *)MFULoad(IndexFileName, sizeof(rindex_t),
                                         macmRead, &count, NULL)) == NULL) {
            DBG(2, fCORE)
            DPrint("WARNING:  cannot load file '%s', errno: %d (%s)\n",
                   IndexFileName, errno, strerror(errno));
    ...
    if (stat(DataFileName, &sbuf) == -1) {
        if (errno != ENOENT) {
            DBG(2, fCORE)
            DPrint("INFO:     cannot stat file '%s', errno: %d (%s)\n",
                   DataFileName, errno, strerror(errno));
...
int MFUGetRecord(
    ...
    char *FileName, char *Key, char **Buffer, int *RecordCount, int *SC)

{
    ...
    DBG(3, fCORE)
    DPrint("%s(%s,%s,Buffer,RecordCount,SC)\n", FName, FileName, Key);
    ...
    if ((Index = (rindex_t *)MFULoad(IndexFileName, sizeof(rindex_t), macmRead,
                                     &icount, SC)) == FAILURE) {
        DBG(2, fCORE)
        DPrint("ALERT:    cannot load '%s' index file\n", IndexFileName);
    ...
    if ((dptr = MFULoad(DataFileName, RecSize, macmRead, &dcount, SC)) ==
        FAILURE) {
        DBG(2, fCORE)
        DPrint("ALERT:    cannot load '%s' index file\n", DataFileName);
    ...
    for (index = 1; index < icount; index++) {
        if (Index[index].Key[0] == '\0') continue;
        ...
        if (Key == NULL) {
            ...
            if ((*Buffer = (char *)calloc(RecSize, 1)) == NULL) {
                MOSSyslog(
                    LOG_ERR,
                    "cannot calloc buffer for record '%s', errno: %d (%s)", Key,
                    errno, strerror(errno));

                DBG(2, fCORE)
                DPrint(
                    "ERROR:    cannot calloc %d buffer for record '%s', errno: "
                    "%d (%s)\n",
                    RecSize, Key, errno, strerror(errno));
    ...
    if ((*Buffer = (char *)calloc(rcount, RecSize)) == NULL) {
        MOSSyslog(LOG_ERR,
                  "cannot calloc buffer for all records, errno: %d (%s)", errno,
                  strerror(errno));

        DBG(2, fCORE)
        DPrint(
            "ERROR:    cannot calloc %d byte buffer for all records, errno: %d "
            "(%s)\n",
            rcount * RecSize, errno, strerror(errno));
...
int MFUPutRecord(
    ...
    char *FileName, char *Key, char *Buffer, int *SC)

{
    ...
    DBG(5, fCORE)
    DPrint("INFO:     index file '%s' loaded with %d records\n", IndexFileName,
           count - 1);
    ...
    if ((dfp = fopen(DataFileName, "rw+")) == NULL) {
        DBG(1, fCORE)
        DPrint("ALERT:    cannot open file '%s', errno: %d (%s)\n",
               DataFileName, errno, strerror(errno));
    ...
    if (fseek(dfp, Offset, SEEK_SET) == -1) {
        DBG(1, fCORE)
        DPrint(
            "ALERT:    cannot seek to offset %d in file '%s', errno: %d (%s)\n",
            Offset, DataFileName, errno, strerror(errno));
    ...
    if (fwrite(Buffer, RecSize, 1, dfp) < 1) {
        DBG(1, fCORE)
        DPrint("WARNING:  cannot write to file '%s', errno: %d (%s)\n",
               DataFileName, errno, strerror(errno));
    ...
    sprintf(HeadIndex.Key, "%08d", Counter);
    ...
    if (fseek(ifp, 0, SEEK_SET) == -1) {
        ...
        DPrint(
            "ALERT:    cannot seek to offset %d in file '%s', errno: %d (%s)\n",
            Offset, IndexFileName, errno, strerror(errno));
    ...
    if (fseek(ifp, bestindex * sizeof(rindex_t), SEEK_SET) == -1) {
        ...
        DPrint(
            "ALERT:    cannot seek to offset %d in file '%s', errno: %d (%s)\n",
            (int)(bestindex * sizeof(rindex_t)), IndexFileName, errno,
            strerror(errno));
...

{
    ...
    for (index = 1; index < count; index++) {
        if (!strcmp(Key, Index[index].Key)) break;
    }

    if (index == count) {
        DBG(2, fCORE)
        DPrint("ALERT:    cannot locate record '%s' in file '%s'\n", Key,
               FileName);
    ...
    if (fseek(ifp, index * sizeof(rindex_t), SEEK_SET) == -1) {
        ...
        DPrint(
            "ALERT:    cannot seek to offset %d in file '%s', errno: %d (%s)\n",
            (int)(index * sizeof(rindex_t)), IndexFileName, errno,
            strerror(errno));
...
int MFUGetCurrentIndex(
    ...
    char *FileName, int *Index, int Mode)

{
    ...
    for (index = 0; index < MAX_FILECACHE; index++) {
        if (!strcmp(FileName, MIndexState[index].FileName)) {
            if (Mode == 1) MIndexState[index].Index++;
...

{
    if (rename(SrcName, DstName) == -1) {
        DBG(0, fCORE)
        DPrint("ERROR:    cannot rename file '%s' to '%s' errno: %d (%s)\n",
               SrcName, DstName, errno, strerror(errno));
