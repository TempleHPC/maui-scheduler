...
int UIFormatShowAllJobs(
    ...
    char *SrcBuffer, char *DstBuffer, int FormatMode)

{
    ...
    while ((ptr = MUStrTok(NULL, "\n", &TokPtr)) != NULL) {
        ...
        sscanf(ptr, "%s %s %ld %ld %d %ld %s %d %d", name, UserName, &stime,
               &qtime, &procs, &cpulimit, tmpQOS, &state, &priority);

        /* display job */

        sprintf(temp_str, "%18s %8s %10s %5d %11s  %19s", name, UserName,
                MJobState[state], procs, MULToTString(cpulimit - (Now - stime)),
                MULToDString(&stime));
        strcat(DstBuffer, temp_str);
    } /* END while (ptr) */
    ...
    while ((ptr = MUStrTok(NULL, "\n", &TokPtr)) != NULL) {
        ...
        fprintf(stdout, "%18s %8s %10s %5d %11s  %19s", name, UserName,
                MJobState[state], procs, MULToTString(cpulimit),
                MULToDString(&qtime));
    }
    ...
    sprintf(temp_str,
            "\nTotal Jobs: %d   Active Jobs: %d   Idle Jobs: %d   Non-Queued "
            "Jobs: %d\n",
...

{
    ...
    while ((ptr = MUStrTok(NULL, "\n", &TokPtr)) != NULL) {
        ...
        if (acount == 0) {
            ...
            sprintf(temp_str,
                    "<TR><TD><B>%s</B></TD><TD><B>%s</B></TD><TD><B>%s</B></"
                    "TD><TD><B>%s</B></TD><TD><B>%s</B></TD><TD><B>%s</B></"
                    "TD></TR>",
                    "Active Jobs", "User Name", "Job State", "Processors",
        ...
        sprintf(temp_str,
                "<TR><TD>%s</TD><TD>%s</TD><TD>%s</TD><TD>%d</TD><TD>%s</"
                "TD><TD>%s</TD></TR>",
                name, UserName, MJobState[state], procs,
                MULToTString(cpulimit - (Now - stime)), MULToDString(&stime));
        strcat(DstBuffer, temp_str);
    } /* END while (ptr) */
    ...
    if (acount > 0) {
        ...
        sprintf(temp_str, "%d Active Job%c<br>", acount,
                (acount == 1) ? ' ' : 's');
        strcat(DstBuffer, temp_str);

        sprintf(temp_str, "%d of %d Processors Active (%.2f%c)<br>", BusyProcs,
                UpProcs,
                (UpProcs > 0) ? (double)BusyProcs / UpProcs * 100.0 : 0.0, '%');
        strcat(DstBuffer, temp_str);

        if (UpProcs != UpNodes) {
            sprintf(temp_str, "%d of %d Nodes Active (%.2f%c)<br>", BusyNodes,
                    UpNodes,
                    (UpNodes > 0) ? (double)BusyNodes / UpNodes * 100.0 : 0.0,
                    '%');
            strcat(DstBuffer, temp_str);
    ...
    while ((ptr = MUStrTok(NULL, "\n", &TokPtr)) != NULL) {
        ...
        if (icount == 0) {
            ...
            sprintf(temp_str,
                    ...
                    "TD><TD><B>%s</B></TD><TD><B>%s</B></TD><TD><B>%s</B></"
                    "TD></TR>",
                    "Idle Jobs", "User Name", "Job State", "Processors",
                    "WallClock Limit", "Submission Time");
            strcat(DstBuffer, temp_str);
        }
        ...
        sprintf(temp_str,
                "<TR><TD>%s</TD><TD>%s</TD><TD>%s</TD><TD>%d</TD><TD>%s</"
                "TD><TD>%s</TD></TR>",
                name, UserName, MJobState[state], procs, MULToTString(cpulimit),
                MULToDString(&qtime));
        strcat(DstBuffer, temp_str);
    } /* END while (ptr) */
    ...
    if (icount > 0) {
        ...
        sprintf(temp_str, "%d Idle Job%c<p>", icount,
                (icount == 1) ? ' ' : 's');
        strcat(DstBuffer, temp_str);
    }
    ...
    while ((ptr = MUStrTok(NULL, "\n", &TokPtr)) != NULL) {
        ...
        if (ncount == 0) {
            ...
            sprintf(temp_str,
                    ...
                    "TD><TD><B>%s</B></TD><TD><B>%s</B></TD><TD><B>%s</B></"
                    "TD><TD><B>%s</B></TD></TR>",
                    "Ineligible Jobs", "User Name", "Job State", "Processors",
                    "WallClock Limit", "Submission Time", "Reason");
            strcat(DstBuffer, temp_str);
        }
    ...
    sprintf(temp_str,
            "<p>Total Jobs: %d &nbsp; Active Jobs: %d &nbsp; Idle Jobs: %d "
            "&nbsp; Ineligible Jobs: %d<p>",
            count, acount, icount, ncount);
    strcat(DstBuffer, temp_str);

    while ((ptr = MUStrTok(NULL, "\n", &TokPtr)) != NULL) {
        sprintf(temp_str, "<p>%s<p>", ptr);
        strcat(DstBuffer, temp_str);
    }
