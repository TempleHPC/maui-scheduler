...
int MFSCheckCap(
    ...
    mfs_t *FSPtr, mjob_t *J, mpar_t *P, int *OIPtr) /* O (optional) */
        ...
        for (oindex = 0; OList[oindex] != -1; oindex++) {
            ...
            switch (OList[oindex]) {
                case mxoUser:
                    ...
                    if (J->Cred.U != NULL) F = &J->Cred.U->F;

                    break;

                case mxoGroup:

                    if (J->Cred.G != NULL) F = &J->Cred.G->F;

                    break;

                case mxoAcct:

                    if (J->Cred.A != NULL) F = &J->Cred.A->F;

                    break;

                case mxoClass:

                    if (J->Cred.C != NULL) F = &J->Cred.C->F;

                    break;

                case mxoQOS:

                    if (J->Cred.Q != NULL) F = &J->Cred.Q->F;
...

{
    ...
    if (NewFSInterval != MSched.CurrentFSInterval) {
        ...
        DBG(1, fFS)
        DPrint("INFO:     FS interval rolled to interval %ld\n",
...

{
    ...
    while ((ptr = MUStrTok(head, "\n", &TokPtr)) != NULL) {
        ...
        if ((tmp = strchr(ptr, '#')) != NULL) *tmp = '\0';

        DBG(6, fFS) DPrint("INFO:     parsing FS data line '%s'\n", ptr);

        /* load value */

        rc = sscanf(ptr, "%32s %32s %lf", Type, Name, &Value);
        ...
        if (!strcmp(Type, "TOTAL")) {
            oindex = mxoSched;
        } else if ((oindex = MUGetIndex(Type, MXO, FALSE, mxoNONE)) ==
                   mxoNONE) {
            continue;
        }
        ...
        DBG(6, fFS)
        DPrint("INFO:     %s '%s' FSUsage[%d]: %lf\n", MXO[oindex],
               (Name[0] != '\0') ? Name : "NONE", FSSlot, Value);
...
int MFSUpdateData(
    ...
    mfsc_t *FC, int FSInterval, /* IN:  FS data interval */
    int FA)                     /* IN:  FS action */

{
    ...
    const int OList[] = {mxoUser,  mxoGroup, mxoAcct, mxoQOS,
                         mxoClass, mxoSched, -1};
    ...
    if (FA & ((1 << mfsactWrite) | (1 << mfsactRotate))) {
        ...
        if (MStat.StatDir[strlen(MStat.StatDir) - 1] == '/') {
            sprintf(FSFile, "%sFS.%d", MStat.StatDir, FSInterval);
        } else {
            sprintf(FSFile, "%s/FS.%d", MStat.StatDir, FSInterval);
        }

        if ((fsfp = fopen(FSFile, "w+")) == NULL) {
            DBG(0, fFS)
            DPrint("WARNING:  cannot open FS data file '%s', errno: %d (%s)\n",
                   FSFile, errno, strerror(errno));
        ...
        fprintf(fsfp, "# FS Data File (Duration: %6ld seconds)  Starting: %s\n",
                FC->FSInterval, MULToDString(&MSched.Time));
    ...
    for (oindex = 0; OList[oindex] != -1; oindex++) {
        ...
        DBG(4, fFS)
        DPrint("INFO:     updating %s fairshare\n", MXO[OList[oindex]]);

        while ((O = MOGetNextObject(&OP, OList[oindex], OS, OE, &NameP)) !=
               NULL) {
            if (MOGetComponent(O, OList[oindex], (void *)&F, mxoFS) == FAILURE)
                continue;

            DBG(7, fFS)
            DPrint("INFO:     updating %s %s\n", MXO[OList[oindex]],
                   (NameP != NULL) ? NameP : "NONE");

            if ((FA & ((1 << mfsactWrite) | (1 << mfsactRotate))) &&
                (F->FSUsage[0] != 0.0)) {
                fprintf(fsfp, "%-10s %15s %12.3f\n", MXO[OList[oindex]],
                        (NameP != NULL) ? NameP : "NONE", F->FSUsage[0]);
            }

            if (FA & (1 << mfsactRotate)) {
                for (fsindex = 1; fsindex < MAX_MFSDEPTH; fsindex++)
                    F->FSUsage[MAX_MFSDEPTH - fsindex] =
                        F->FSUsage[MAX_MFSDEPTH - fsindex - 1];
...

{
    ...
    for (interval = 0; interval <= MAX_MFSDEPTH; interval++) {
        if (interval > FC->FSDepth) break;

        if (MStat.StatDir[strlen(MStat.StatDir) - 1] == '/') {
            sprintf(FileName, "%sFS.%ld", MStat.StatDir,
                    StartFSInterval + interval * FC->FSInterval);
        } else {
            sprintf(FileName, "%s/FS.%ld", MStat.StatDir,
                    StartFSInterval + interval * FC->FSInterval);
        }

        if (MFSLoadDataFile(FileName, FC->FSDepth - interval) == FAILURE) {
            DBG(3, fFS)
            DPrint("WARNING:  cannot load FS file '%s' for slot %d\n", FileName,
                   FC->FSDepth - interval);
        }
    } /* END for (interval) */
...
double MFSCalcFactor(
    ...
    mfsc_t *F, double FSUsage[MAX_MFSDEPTH]) /* IN:  CPU usage history */

{
    ...
    for (cindex = 1; cindex < F->FSDepth; cindex++) {
        ...
        DBG(7, fFS)
        DPrint("INFO:  FSUsage[%d]  %0.2lf\n", cindex, FSUsage[cindex]);
    } /* END for (cindex) */
...

{
    ...
    switch (PIndex) {
        ...
        case pFSDepth:
            ...
            DBG(1, fCONFIG)
            DPrint("INFO:     %s set to %d\n", MParam[PIndex], F->FSDepth);
        ...
        case pFSPolicy:
            ...
            if ((ptr = strchr(SVal, '%')) != NULL) {
                ...
                /* patch submitted by Ake Sandgren--assumes that % is always at
                 * the end of SVal */
            ...
            if (MUBoolFromString(SVal, FALSE) == TRUE) {
                ...
                F->FSPolicy =
                    MUGetIndex(SVal, MFSPolicyType, FALSE, F->FSPolicy);
            }
...

{
    ...
    /** DsT Begin check if given option is supported, else just skip and show
     * all  **/
    ...
    MUSNPrintF(
        &BPtr, &BSpace,
        "Depth: %d intervals   Interval Length: %s   Decay Rate: %.2f\n\n",
        GP->FSC.FSDepth, MULToTString(GP->FSC.FSInterval), GP->FSC.FSDecay);

    MUSNPrintF(&BPtr, &BSpace, "FS Policy: %s\n",
               MFSPolicyType[GP->FSC.FSPolicy]);

    MUSNPrintF(&BPtr, &BSpace,
               "System FS Settings:  Target Usage: %s   Flags: %ld\n\n",
               MFSTargetToString(GP->F.FSTarget, GP->F.FSMode), GP->F.JobFlags);

    /* display header */

    MUSNPrintF(&BPtr, &BSpace, "%-14s %7s %7s", "FSInterval", "  %   ",
               "Target");

    for (fsindex = 0; fsindex < GP->FSC.FSDepth; fsindex++) {
        if (GP->F.FSUsage[fsindex] > 0.0) {
            MUSNPrintF(&BPtr, &BSpace, " %7d", fsindex);
        }
    } /* END for (fsindex) */
    ...
    MUSNPrintF(&BPtr, &BSpace, "%-14s %7s %7s", "FSWeight", "-------",
               "-------");

    for (fsindex = 0; fsindex < GP->FSC.FSDepth; fsindex++) {
        if (GP->F.FSUsage[fsindex] > 0.0) {
            MUSNPrintF(&BPtr, &BSpace, " %7.4f", pow(GP->FSC.FSDecay, fsindex));
        }
    } /* END for (fsindex) */
    ...
    MUSNPrintF(&BPtr, &BSpace, "%-14s %7.2f %7s", "TotalUsage", 100.0,
               "-------");
    ...
    DBG(6, fUI)
    DPrint("INFO:     Total FSFactor: %8.2f  FSUsage[0]: %8.2f\n",
           GP->F.FSFactor, GP->F.FSUsage[0]);
        ...
        while ((O = MOGetNextObject(&OP, OList[oindex], OS, OE, &NameP)) !=
            ...
            switch (OList[oindex]) {
                case mxoUser:
                    ...
                    /** DsT if limit is true but Opts does not compare to users,
                     * skip **/
                    if (LIMITOUTPUT == TRUE && strcmp(Opts, "user") != 0)
                        continue;

                    if (MSched.DefaultU != NULL) DF = &MSched.DefaultU->F;

                    break;

                case mxoGroup:

                    /** DsT if limit is true but Opts does not compare to group,
                     * skip **/
                    if (LIMITOUTPUT == TRUE && strcmp(Opts, "group") != 0)
                        continue;

                    if (MSched.DefaultG != NULL) DF = &MSched.DefaultG->F;

                    break;

                case mxoAcct:

                    /** DsT if limit is true but Opts does not compare to acct,
                     * skip **/
                    if (LIMITOUTPUT == TRUE && strcmp(Opts, "acct") != 0)
                        continue;

                    if (MSched.DefaultA != NULL) DF = &MSched.DefaultA->F;

                    break;

                case mxoClass:

                    /** DsT if limit is true but Opts does not compare to class,
                     * skip **/
                    if (LIMITOUTPUT == TRUE && strcmp(Opts, "class") != 0)
                        continue;

                    if (MSched.DefaultC != NULL) DF = &MSched.DefaultC->F;

                    break;

                case mxoQOS:

                    /** DsT if limit is true but Opts does not compare to qos,
                     * skip **/
                    if (LIMITOUTPUT == TRUE && strcmp(Opts, "qos") != 0)
                        continue;

                    if (MSched.DefaultQ != NULL) DF = &MSched.DefaultQ->F;
            ...
            if ((NameP == NULL) || (NameP[0] == '\0') || !strcmp(NameP, ALL)) {
            ...
            if ((FSTarget == 0.0) && (F->FSFactor == 0.0) &&
                (F->FSUsage[0] == 0.0) && !(Mode & (1 << mcmVerbose))) {
            ...
            MUSNPrintF(&BPtr, &BSpace, "%-14s %7.2f %7s", tmpName, FSPercent,
                       tmpString);

            DBG(6, fUI)
            DPrint(
                "INFO:     %s '%s'  FSFactor: %8.2f  FSUsage[0]: %8.2f  "
                "FSPercent: %8.2f\n",
                MXO[OList[oindex]], NameP, F->FSFactor, F->FSUsage[0],
                FSPercent);
