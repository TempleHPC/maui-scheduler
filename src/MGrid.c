...

{
    ...
    MajorSC = globus_gss_assist_init_sec_context(
        &MinorSC, MLocalCredential, &context, RemoteSubject, GSS_C_DELEG_FLAG,
        &flags, &ts, __MGSSGetToken, &S->sd, __MGSSSendToken, &S->sd);
    ...
    MDB(3, fGRID)
    MLog(
        "WARNING:    grid credential services were invoked, but are not "
        "currently enabled");
    return (FAILURE);
#endif /* __MGSS */
} /* END MGSSPostCred() */
...

{
    ...
    *C = GSS_C_NO_CREDENTIAL;
    ...
    MajorSC = globus_gss_assist_accept_sec_context(
        &MinorSC, &Context, MLocalCredential, NULL, &flags, NULL, &ts,
        (gss_cred_id_t *)C, __MGSSGetToken, (void *)&S->sd, __MGSSSendToken,
        (void *)&S->sd);
...

{
    ...
    if (MSUSendPacket(fd, (char *)buffer, 4, MCONST_TOKENTIMEOUT, NULL) ==
            FAILURE ||
        MSUSendPacket(fd, (char *)Tok, Len, MCONST_TOKENTIMEOUT, NULL) ==
            FAILURE) {
...

{
    ...
    if (MSURecvPacket(fd, &ptr, 4, NULL, MCONST_TOKENTIMEOUT, NULL) ==
        FAILURE) {
    ...
    if (MSURecvPacket(fd, (char **)&tok, len, NULL, MCONST_TOKENTIMEOUT,
                      NULL) == FAILURE) {
        MDB(0, fSOCK) MLog("ALERT:    cannot get gss packet\n");
...

{
    ...
    MDB(3, fGRID)
    MLog("%s(C,%s,%ld,%.32s,%s)\n", FName,
         (CmdString != NULL) ? CmdString : "NULL", Timeout,
    ...
    rc = globus_gram_client_callback_allow(__MGlobusCallBackHandler, &monitor,
                                           &contact);
    ...
    rc = globus_gram_client_job_request(CmdString, JobString, mask, contact,
                                        &job);
    ...
    MDB(3, fGRID)
    MLog(
        "WARNING:    Globus services were invoked, but are not currently "
        "enabled");
    return (FAILURE);
#endif /* __MGLOBUS */
} /* END MGlobusExec() */
...

{
    ...
    MLog("%s(%s,%s,%s,%d)\n", FName, (J != NULL) ? J->Name : "NULL",
         (R != NULL) ? R->Name : "NULL", (Buf != NULL) ? "Buf" : "NULL",
         BufSize);
    ...
    for (index = 0; RSLAttr[index].OType != mxoNONE; index++) {
        switch (RSLAttr[index].OType) {
            case mxoJob:
                ...
                rc = MJobAToString(J, (enum MJobAttrEnum)RSLAttr[index].AIndex,
                                   tmpLine,
            ...
            case mxoReq:
                ...
                rc = MReqAToString(
                    J, RQ, (enum MReqAttrEnum)RSLAttr[index].AIndex, tmpLine,
                    (enum MFormatModeEnum)RSLAttr[index].Format);
        ...
        if ((rc == FAILURE) || (tmpLine[0] == '\0')) {
            if (RSLAttr[index].IsRequired == TRUE) {
                MDB(3, fGRID)
                MLog(
                    "ALERT:    job '%s' is missing attribute '%s' for "
                    "conversion to RSL\n",
                    J->Name, RSLAttr[index].RSLToken);
        ...
        MUSNPrintF(&BPtr, &BSpace, "(%s=\"%s%s\")",
                   (IData->IsExec == TRUE) ? "executable" : "stdin", PreString,
                   tmpLine);
    } /* END for (index) */
    ...
    while (IData != NULL) {
        ...
        if (StageFile == TRUE) {
            ...
            /* NOTE:  currently implemented in client until credential
             * delegation is enabled */

            /* build stage string */

            /* SDataGetDefProtocol(RQ->IData,SJ->SubmitR,NULL,&Protocol);
             * (re-enable NYI) */

            DS = (J->SRM->DS != NULL) ? J->SRM->DS[Protocol] : NULL;

            if (DS != NULL) {
                sprintf(PreString, "%s://%s", MDSProtocol[DS->Protocol],
                        (DS->HostName != NULL) ? DS->HostName : "localhost");

                if (DS->Port > 0) {
                    sprintf(temp_str, ":%d", DS->Port);
                    strcat(PreString, temp_str);
                }
            }

            if (IData->IsExec == TRUE) ExecSpecified = TRUE;

            MUSNPrintF(&BPtr, &BSpace, "(%s=\"%s%s\")", RSLAttr[index].RSLToken,
                       PreString, tmpLine);
...

{
    ...
    MDB(3, fGRID)
    MLog("%s(J,%s,%s,%d,%s,%s,JobName,EMsg)\n", FName,
         (R != NULL) ? R->Name : "NULL",
         (JobString != NULL) ? JobString : "NULL", Format, Destination,
    ...
    if (J == NULL) {
        ...
        if ((J->Cred.U == NULL) || (J->Cred.U->GCred == NULL) ||
            (J->Cred.U->GCred->C == NULL)) {
            MDB(1, fGRID)
            MLog("ALERT:    credential not initialized in %s\n", FName);

            if (EMsg != NULL) strcpy(EMsg, "no credential");
        ...
        if ((J->Cred.U->GCred->ExpireTime > 0) &&
            ...
            if (EMsg != NULL) strcpy(EMsg, "expired credential");
    ...
    MDB(3, fGRID)
    MLog("INFO:     launching job on resource '%s' with command '%s'\n",
         Destination, JobString);
    ...
    if ((JobString == NULL) || (JobString[0] == '\0')) {
        ...
        if ((J->RMSubmitString != NULL) && (J->RMSubmitType != mrmtNONE)) {
        ...
        } else if (MGlobusJobToRSL(J, R, tmpBuffer, sizeof(tmpBuffer)) ==
                   SUCCESS) {
            jptr = tmpBuffer;
            ...
            MDB(2, fGRID)
            MLog("ALERT:    cannot translate job '%s' for staging\n", J->Name);

            if (EMsg != NULL) strcpy(EMsg, "cannot translate job to RSL");
    ...
    switch (Format) {
        case mrmtPBS:
            ...
            sprintf(CmdString, "%s/jobmanager-pbsnative", Destination);

            break;

        case mrmtLL:

            sprintf(CmdString, "%s/jobmanager-llnative", Destination);

            break;

        case mrmtSGE:

            sprintf(CmdString, "%s/jobmanager-sgenative", Destination);

            break;

        case mrmtLSF:

            sprintf(CmdString, "%s/jobmanager-lsfnative", Destination);
        ...
        default:
            ...
            sprintf(CmdString, "%s", Destination);

            break;
    } /* END switch (Format) */

    rc = MGlobusExec(C, CmdString, R->P[0].Timeout, JobString, EMsg);

    MDB(3, fGRID)
    MLog("INFO:     MGlobusExec %s\n",
...
int __MGSSGetError(
    ...
    long MajorSC, long MinorSC, int Level)

{
    ...
    globus_gss_assist_display_status_str(&msg, NULL, (OM_uint32)MajorSC,
                                         (OM_uint32)MinorSC, 0);
...
static void __MGlobusCallBackHandler(
    ...
    void *Data, char *Job, int Status, int Error)
...

{
    ...
    MDB(1, fGRID)
    MLog("%s(%s)\n", FName, (CertFile != NULL) ? CertFile : "NULL");
#endif /* __MPROD */
  ...
  if (s->DefDS != NULL)
    ...
    if (CertFile != NULL) {
        if (setenv(DEFAULT_MCREDVAR, CertFile, 1) == -1) {
            MDB(0, fGRID)
            MLog("ALERT:    setenv(%s,%s) failed, errno: %d (%s)\n",
                 DEFAULT_MCREDVAR, CertFile, errno, strerror(errno));
    ...
    MajorSC =
        globus_gss_assist_acquire_cred(&MinorSC, GSS_C_BOTH, &MLocalCredential);
    ...
    if ((SGICredExpirationTime(s->C, &s->CETime) == FAILURE) ||
        ...
        if (EMsg != NULL) strcpy(EMsg, "globus credential is expired");
    ...
    MDB(3, fGRID)
    MLog(
        "WARNING:    Globus credential services were invoked, but are not "
        "currently enabled");
    return (FAILURE);
...

{
    ...
    MDB(1, fGRID)
    MLog("%s(SrcURLs,DstURLs,%s,%s,%s)\n", FName,
         (Username != NULL) ? Username : "NULL",
         (StatusFile != NULL) ? StatusFile : "NULL",
         (URLFile != NULL) ? URLFile : "NULL");
#endif /* !__MPROD */

    if ((SrcURLs == NULL) || (DstURLs == NULL) || (Username == NULL) ||
        (StatusFile == NULL) || (URLFile == NULL) || (copyPID == NULL)) {
        return (FAILURE);
    }

    if ((SrcURLs[0] == NULL) || (DstURLs[0] == NULL)) {
        return (FAILURE);
    }
    ...
    *copyPID = -1;
    ...
    if ((urlFileDesc = fopen(URLFile, "w")) == NULL) {
        MDB(1, fGRID)
        MLog("ERROR:    cannot open globus-url file '%s' (errno %d: '%')\n",
             URLFile, errno, strerror(errno));

        return (FAILURE);
    }

    fprintf(urlFileDesc,
            "# Moab globus-url-copy multi-file data staging list\n# Auto "
            "generated.\n\n");

    /* TODO: Change MMAX_NAME to a const more meaningful to data staging */
    for (urlIndex = 0; urlIndex < MMAX_NAME; urlIndex++) {
        if (SrcURLs[urlIndex] == NULL) break;

        if (DstURLs[urlIndex] == NULL) {
            MDB(1, fGRID)
            MLog(
                "ERROR:    src globus-url '%s' without corresponding dst url\n",
                SrcURLs[urlIndex]);
    ...
    *copyPID = fork();
    ...
    if (copyUserInfo == NULL) {
        MDB(1, fGRID)
        MLog(
            "ERROR:    globus-url-copy failed - could not locate UID for user "
            "'%s'\n",
            Username);
    ...
    if (setuid(uid) < 0) {
        MDB(1, fGRID)
        MLog(
            "ERROR:    globus-url-copy failed - could not become user '%s'! "
            "Ensure Moab is running as root.\n",
            Username);
    ...
    if (execv(globus - url - copy, copyArgs) < 0) {
        ...
        MDB(1, fGRID)
        MLog(
            "ERROR:    globus-url-copy failed - could not launch staging "
            "process (errno: %d: '%s')\n",
    ...
    *IsDone = FALSE;
    ...
    if (rc == 0) {
        *IsDone = FALSE;
        ...
        MDB(1, fGRID)
        MLog("WARNING:  cannot check copy PID %d - doesn't exist!\n", copyPID);
    ...
    *IsDone = TRUE;
    ...
    if (WIFEXITED(childStatus)) {
        if (WEXITSTATUS(childStatus) == 0) {
            MDB(1, fGRID)
            MLog("WARNING:  globus-url-copy process %d returned with code %d\n",
                 copyPID, WEXITSTATUS(childStatus));

            if (EMsg != NULL) {
                sprintf(EMsg,
                        "globus-url-copy process %d returned with code %d\n",
                        copyPID, WEXITSTATUS(childStatus));
            }
            ...
            MDB(1, fGRID)
            MLog(
                "WARNING:  globus-url-copy process %d did not exit normally: "
                "code %d\n",
                copyPID, WEXITSTATUS(childStatus));

            if (EMsg != NULL) {
                sprintf(EMsg,
                        "globus-url-copy process %d did not exit normally: "
                        "code %d\n",
                        copyPID, WEXITSTATUS(childStatus));
            }
    ...
    MDB(1, fGRID)
    MLog("WARNING:  globus-url-copy process %d was terminated by signal %d\n",
         copyPID, WTERMSIG(childStatus));

    if (EMsg != NULL) {
        sprintf(EMsg,
                "globus-url-copy process %d was terminated by signal %d\n",
                copyPID, WTERMSIG(childStatus));
    }
