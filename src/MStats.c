...

{
    ...
    if (P->TimeStepSize > 0)
        P->MaxTime = (unsigned long)(P->MinTime *
                                     pow(P->TimeStepSize, P->TimeStepCount));

    if (P->NodeStepSize > 0)
        P->MaxNode = (unsigned long)(P->MinNode *
                                     pow(P->NodeStepSize, P->NodeStepCount));
    ...
    DBG(4, fSTAT)
    DPrint("INFO:     time min value: %4ld  distance: %4d  step: %6.2f\n",
           P->MinTime, distance, gstep);
    ...
    for (index = 1; index <= P->TimeStepCount; index++) {
        ...
        DBG(5, fSTAT)
        DPrint("INFO:     TimeStep[%02d]: %8ld\n", index, P->TimeStep[index]);
    }

    P->TimeStep[P->TimeStepCount + 1] = 999999999;

    DBG(4, fSTAT)
    DPrint("INFO:     time steps eliminated: %3d\n", ScaleModFactor);
    ...
    DBG(4, fSTAT)
    DPrint("INFO:     node min value: %3ld  distance: %3d  step: %4.2f\n",
           P->MinNode, distance, gstep);
    ...
    for (index = 1; index <= P->NodeStepCount; index++) {
        ...
        if ((index != 0) && (P->NodeStep[index - ScaleModFactor] ==
                             P->NodeStep[index - ScaleModFactor - 1])) {
            ScaleModFactor++;
        }

        DBG(5, fSTAT)
        DPrint("INFO:     NodeStep[%02d]: %4ld\n", index, P->NodeStep[index]);
    }

    P->NodeStep[P->NodeStepCount + 1] = 999999999;

    DBG(4, fSTAT)
    DPrint("INFO:     node steps eliminated: %d\n", ScaleModFactor);
    ...
    for (index = 0; index < P->AccuracyScale; index++) {
        ...
        DBG(5, fSTAT)
        DPrint("INFO:     AccuracyStep[%02d]: %4ld\n", index,
...

{
    ...
    for (nindex = 0; nindex < MAX_MNODE; nindex++) {
        ...
        for (cindex = 0; cindex < MAX_MRCLASS; cindex++) {
            ...
            if ((N->CRes.Mem < RClass[cindex].Memory) &&
                ...
                for (cindex2 = cindex + 1; cindex2 < MAX_MRCLASS - 1;
                     cindex2++) {
                    if (RClass[cindex2].Memory == 0) break;
                }

                /* extend class list */

                for (; cindex2 > cindex; cindex2--) {
                    memcpy(&RClass[cindex2], &RClass[cindex2 - 1],
                           sizeof(RClass[cindex2]));
                } /* END for (cindex2) */
    ...
    for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = J->Next) {
        ...
        for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
            ...
            DBG(4, fSTAT)
            DPrint("INFO:     evaluating requirements of %s:%d\n", J->Name,
                   rqindex);
    ...
    DBG(3, fSTAT) {
        ...
        DBG(3, fSTAT)
        DPrint("INFO:     %8s %5s %9s %7s\n", "Class", "Nodes", "NodeHours",
               "Average");
        ...
        for (cindex = 0; cindex < MAX_MRCLASS; cindex++) {
            ...
            DBG(3, fSTAT)
            DPrint("INFO:     %8d %5d %9ld %7lu\n", RClass[cindex].Memory,
                   RClass[cindex].Nodes, RClass[cindex].Workload, Average);
        }
    }
    ...
    for (cindex = MAX_MRCLASS - 2; cindex >= 0; cindex--) {
        DBG(5, fSTAT)
        DPrint("INFO:     evaluating class %d\n", RClass[cindex].Memory);
        ...
        for (cindex2 = cindex + 1; cindex2 < MAX_MRCLASS; cindex2++) {
            if (RClass[cindex2].Nodes > 0) {
                if (AvgWorkLoad >=
                    ...
                    DBG(6, fSTAT)
                    DPrint(
                        "INFO:     class %d (avg: %ld) adding workload (%d "
                        "nodes, %ld PS) to class %d (avg: %lu)\n",
                        RClass[cindex2].Memory,
                        RClass[cindex2].Workload / RClass[cindex2].Nodes,
                        RClass[cindex2].Nodes, RClass[cindex2].Workload,
                        RClass[cindex].Memory, AvgWorkLoad);
        ...
        DBG(6, fSTAT)
        DPrint("INFO:     total node pool: %lu  workload: %lu  avg: %lu\n",
               Nodes, Workload, Workload / Nodes);
        ...
        if ((Nodes > 0) && (Workload > 0)) {
            for (cindex2 = cindex; cindex2 < MAX_MRCLASS; cindex2++) {
                DBG(5, fSTAT)
                DPrint(
                    "INFO:     redistributing workload from class %d to class "
                    "%d\n",
                    RClass[cindex].Memory, RClass[cindex2].Memory);

                if (RClass[cindex2].Nodes > 0) {
                    if (AvgWorkLoad >=
                        RClass[cindex2].Workload / RClass[cindex2].Nodes) {
                        RClass[cindex2].Workload =
                            (unsigned long)((double)Workload *
                                            RClass[cindex2].Nodes / Nodes);

                        DBG(5, fSTAT)
                        DPrint(
                            "INFO:     new avg workload for class %d: %lu\n",
                            RClass[cindex2].Memory,
                            RClass[cindex2].Workload / RClass[cindex2].Nodes);
                    }
                }
        ...
        switch (MSim.CommunicationType) {
            case comRoundRobin:
                ...
                if (J->NodeList[nindex + 1].N == NULL)
                    cost +=
                        MStatGetCom(J->NodeList[nindex].N, J->NodeList[0].N);
                else
                    cost += MStatGetCom(J->NodeList[nindex].N,
                                        J->NodeList[nindex + 1].N);
...

{
    ...
    int OList[] = {mxoUser,  mxoGroup, mxoAcct, mxoQOS,
                   mxoClass, mxoPar,   mxoNode, -1};
    ...
    for (oindex = 0; OList[oindex] != -1; oindex++) {
        if ((OType > 0) && !(OType & (1 << OList[oindex]))) continue;

        if ((OType == 0) && (OList[oindex] == mxoNode)) continue;
        ...
        for (cindex = 0; cindex < MaxO; cindex++) {
            ...
            DBG(8, fSTAT)
            DPrint("INFO:     clearing usage stats for %s %s\n",
                   MXO[OList[oindex]], NPtr);

            if (PTypeBM & (1 << mlActive)) {
                if (AP != NULL) memset(AP->Usage, 0, sizeof(AP->Usage));
                ...
                if (APG != NULL) {
                    ...
                    for (cindex = 0; cindex < MAX_MGROUP + MAX_MHBUF;
                         cindex++) {
...

{
    ...
    */
    ...
    if ((J->Req[0] != NULL) && (J->Req[0]->NAccessPolicy == mnacSingleJob) &&
    ...
    if (TotalProcs == 0) {
        DBG(3, fSTAT)
        DPrint(
            "INFO:     no tasks associated with job '%s' (no statistics "
            "available)\n",
            J->Name);
    ...
    interval = MIN((double)MSched.Interval / 100.0,
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        if ((J->StartTime != MSched.Time) && (J->CTime != MSched.Time)) {
            ...
            for (nindex = 0; nindex < MAX_MNODE_PER_JOB; nindex++) {
                ...
                /*
                 *  HvB: There is a difference between shared nodes and
                 * NodeAllocMaxPS (exclusive)
                 *    exclusive : nodes * interval
                 *     shared   : total cores * interval
                 *
                 *  Load and cpu time is calculated for all cores in a node. So
                 * we have to normalize
                 *  for psutilized. if singlejob per node is set
                */
                if ((J->Req[0]->NAccessPolicy == mnacSingleJob) &&
                    (MSched.NodeAllocMaxPS == TRUE)) {
                    psutilized_cpu += interval * RQ->URes.Procs /
                                      (100.0 * TotalProcs * N->CRes.Procs);
                    psutilized_load +=
                        interval * (double)N->Load / N->CRes.Procs;
                } else
                /* Old calculation */
                {
                    if (RQ->URes.Procs > 0) {
                        ...
                        /* cannot properly determine efficiency with available
                         * information */
            ...
            /* HvB
             * Some Jobs do not update the cputime properly , but the load is
             * high. Then use this factor
             * for jobs that uses a single node exclusive
            */
            if ((J->Req[0]->NAccessPolicy == mnacSingleJob) &&
                (MSched.NodeAllocMaxPS == TRUE)) {
                DBG(8, fSTAT)
                DPrint("INFO:     psutilized_cpu = %f, psutilized_load = %f\n",
            ...
            if (psdedicated == 0.0) {
                DBG(1, fSTAT)
                DPrint(
                    "ALERT:    job %s active for %s has no dedicated time on "
                    "%d nodes\n",
                    J->Name, MULToTString(MSched.Time - J->StartTime), nindex);
            } else {
                DBG(6, fSTAT)
                DPrint(
                    "INFO:     job '%18s'  nodes: %3d  PSDedicated: %lf  "
                    "PSUtilized: %lf  (efficiency: %5.2f)\n",
                    J->Name, nindex, psdedicated, psutilized,
        ...
        for (timeindex = 0;
             MIN(J->WCLimit, MStat.P.MaxTime) > MStat.P.TimeStep[timeindex];
             timeindex++)
            ;
        for (procindex = 0; MIN(RQ->TaskCount * RQ->DRes.Procs,
                                MStat.P.MaxNode) > MStat.P.NodeStep[procindex];
             procindex++)
            ;
        ...
        DBG(7, fSTAT)
        DPrint("INFO:     updating statistics for Grid[time: %d][proc: %d]\n",
               timeindex, procindex);
        ...
        if ((J != NULL) && (J->Cred.C != NULL))
            DBG(1, fSTAT)
            DPrint("INFO: Average nodespeed for Job %s is  %f, %f, %i  \n",
        ...
        if (U != NULL) {
            DBG(7, fSTAT)
            DPrint("INFO:     updating statistics for user %s (UID: %ld) \n",
                   U->Name, U->OID);
            ...
            DBG(3, fSTAT)
            DPrint("ALERT:    cannot locate user record for job '%s'\n",
                   J->Name);
        }
        ...
        if (G != NULL) {
            DBG(7, fSTAT)
            DPrint("INFO:     updating statistics for group %s (GID %ld)\n",
                   G->Name, G->OID);
            ...
            DBG(3, fSTAT)
            DPrint("ALERT:    cannot locate group record for job '%s'\n",
                   J->Name);
        }
        ...
        if (A != NULL) {
            DBG(7, fSTAT)
            DPrint("INFO:     updating statistics for account %s\n", A->Name);
            ...
            DBG(7, fSTAT)
            DPrint("INFO:     job '%s' has no account assigned\n", J->Name);
        }
        ...
        if (J->Cred.Q != NULL) {
            ...
            DBG(7, fSTAT)
            DPrint("INFO:     updating statistics for QOS %s\n", Q->Name);
            ...
            DBG(7, fSTAT)
            DPrint("INFO:     job '%s' has no QOS assigned\n", J->Name);
        }
        ...
        if (C != NULL) {
            DBG(7, fSTAT)
            DPrint("INFO:     updating statistics for class %s\n", C->Name);
            ...
            DBG(7, fSTAT)
            DPrint("INFO:     job '%s' has no class assigned\n", J->Name);
        }
...

{
    ...
    if (!strstr(MSched.HomeDir, DEFAULT_MSTATDIR)) {
        sprintf(MStat.StatDir, "%s%s", MSched.HomeDir, DEFAULT_MSTATDIR);
    } else {
...

{
    ...
    if ((MSched.Mode != msmSim) && (MSched.Mode != msmTest)) {
        ...
        sprintf(temp_str, "_%02d", atoi(ptr));
        strcat(tmpfile, temp_str);
    ...
    sprintf(StatFile, "%s%s", MStat.StatDir, tmpfile);
    ...
    if ((rc = umask(027)) != 027) {
        DBG(1, fSTAT)
        DPrint("ERROR:    cannot set umask before opening statfile '%s'(%o)\n",
               StatFile, rc);
    }

    if ((MSched.statfp = fopen(StatFile, "a+")) == NULL) {
        DBG(0, fSTAT)
        DPrint("WARNING:  cannot open statfile '%s', errno: %d (%s)\n",
               StatFile, errno, strerror(errno));
...

{
    ...
    switch (SIndex) {
        ...
        case stJobEfficiency:
            ...
            strcat(Buffer,
                   "Average Job Efficiency (percent CPU utilization)\n\n");
        ...
        case stQOSDelivered:
            ...
            strcat(Buffer,
                   "Quality of Service Delivered (percent of jobs meeting "
                   "QOS)\n\n");
        ...
        default:
            ...
            DBG(1, fSTAT)
            DPrint("ERROR:    unexpected statistics type, %d, in %s()\n",
                   SIndex, FName);
    ...
    for (procindex = 0; procindex <= MStat.P.NodeStepCount; procindex++) {
        if (Mode & 8) {
            sprintf(temp_str, "  %4d   ", MStat.P.NodeStep[procindex]);
            strcat(Buffer, temp_str);
        } else {
            sprintf(temp_str, "[ %4d  ]", MStat.P.NodeStep[procindex]);
            strcat(Buffer, temp_str);
    ...
    switch (SIndex) {
        ...
        case stWCAccuracy:
            ...
            sprintf(temp_str, "%-26s %8.3f\n", "Overall WallClock Accuracy:",
                    (T->PSRequest > 0.0)
                        ? (double)T->PSRun / T->PSRequest * 100.0
                        : 0.0);
            strcat(Buffer, temp_str);
        ...
        case stBFCount:
            ...
            sprintf(temp_str, "%-26s %8.4f (%d / %d)\n",
                    ...
                    (double)T->BFCount / T->Count * 100.0, T->BFCount,
                    T->Count);
            strcat(Buffer, temp_str);
        ...
        case stBFPSRun:
            ...
            sprintf(temp_str, "%-26s %8.4f (%6.2f / %6.2f)\n",
                    ...
                    T->BFPSRun / T->PSRun * 100.0, T->BFPSRun, T->PSRun);
            strcat(Buffer, temp_str);
        ...
        case stJobEfficiency:
            ...
            sprintf(temp_str, "%-26s %8.4f (%6.2f / %6.2f)\n",
                    ...
                    (T->PSDedicated > 0.0)
                        ? T->PSUtilized / T->PSDedicated * 100.0
                        : 0.0,
                    T->PSUtilized / 3600.0, T->PSDedicated / 3600.0);
            strcat(Buffer, temp_str);
...
char *MStatGetGrid(
    ...
    int SIndex, must_t *G, must_t *R, must_t *C, must_t *T, int Mode)

{
    ...
    switch (SIndex) {
        case stAvgXFactor:
            ...
            sprintf(Line, doubleline, G->XFactor / G->Count, G->Count);

            break;

        case stMaxXFactor:

            sprintf(Line, doubleline, G->MaxXFactor, G->Count);

            break;

        case stAvgQTime:

            sprintf(Line, doubleline, (double)(G->TotalQTS) / G->Count / 3600.0,
                    G->Count);
        ...
        case stAvgBypass:
            ...
            sprintf(Line, doubleline, (double)G->Bypass / G->Count, G->Count);

            break;

        case stMaxBypass:

            sprintf(Line, intline, G->MaxBypass, G->Count);

            break;

        case stJobCount:

            sprintf(Line, intonly, G->Count);

            break;

        case stPSRequest:

            sprintf(Line, doubleline, G->PSRequest / T->PSRequest * 100.0,
                    G->Count);
        ...
        case stPSRun:
            ...
            sprintf(Line, doubleline, G->PSRun / T->PSRun * 100.0, G->Count);

            break;

        case stWCAccuracy:

            sprintf(Line, doubleline, G->PSRun / G->PSRequest * 100, G->Count);

            break;

        case stBFCount:

            sprintf(Line, doubleline, (double)G->BFCount / G->Count * 100.0,
                    G->Count);
        ...
        case stBFPSRun:
            ...
            sprintf(Line, doubleline, G->BFPSRun / G->PSRun * 100.0, G->Count);

            break;

        case stJobEfficiency:

            sprintf(Line, doubleline, G->PSUtilized / G->PSDedicated * 100.0,
                    G->Count);
        ...
        case stQOSDelivered:
            ...
            sprintf(Line, doubleline, (double)G->QOSMet / G->Count * 100.0,
                    G->Count);
        ...
        default:
            ...
            DBG(3, fSTAT)
            DPrint("ALERT:    stat type %d not handled\n", SIndex);
...

{
    ...
    DBG(5, fSTAT)
    DPrint("%s(%s,%x,%x)\n", FName, (J != NULL) ? J->Name : "NULL", RunMode,
           ProfileMode);
    ...
    if (((J->SubmitTime == 0) && (MSched.Time > 100000)) ||
        (J->StartTime == 0) || (J->CompletionTime == 0)) {
        DBG(2, fSTAT)
        DPrint("ALERT:    job %s did not run. (Q: %ld S: %ld: C: %ld)\n",
               J->Name, J->SubmitTime, J->StartTime, J->CompletionTime);
    ...
    run = (J->CompletionTime > J->StartTime) ? J->CompletionTime - J->StartTime
                                             : 0;
    ...
    if (J->StartTime < J->SystemQueueTime) {
        DBG(0, fSTAT)
        DPrint(
            "ALERT:    job '%18s' has invalid system queue time (SQ: %ld > ST: "
            "%ld)\n",
            J->Name, J->SystemQueueTime, J->StartTime);
    }

    psremaining = TotalProcs * (J->StartTime + J->WCLimit - MSched.Time);

    if (psremaining < 0) psremaining = 0;
    ...
    DBG(3, fSTAT)
    DPrint(
        "INFO:     job '%18s' completed.  QueueTime: %6d  RunTime: %6d  "
        "Accuracy: %5.2f  XFactor: %5.2f\n",
        J->Name, queuetime, run, accuracy * 100, xfactor);

    DBG(4, fSTAT)
    DPrint("INFO:     start: %8d  complete: %8d  SystemQueueTime: %8d\n",
           J->StartTime, J->CompletionTime, J->SystemQueueTime);

    DBG(3, fSTAT)
    DPrint("INFO:     overall statistics.  Accuracy: %5.2f  XFactor: %5.2f\n",
    ...
    for (timeindex = 0; timeindex < MAX_MGRIDTIMES; timeindex++) {
        if (rval <= MStat.P.TimeStep[timeindex]) break;
    }
    ...
    if (RQ->PtIndex != 0) S[stattotal++] = &MPar[RQ->PtIndex].S;

    /* locate/adjust user stats */

    if (J->Cred.U != NULL) {
        DBG(7, fSTAT)
        DPrint("INFO:     updating statistics for UID %ld (user: %s)\n",
               J->Cred.U->OID, J->Cred.U->Name);
    ...
    if (J->Cred.G != NULL) {
        DBG(7, fSTAT)
        DPrint("INFO:     updating statistics for GID %ld (group: %s)\n",
               J->Cred.G->OID, J->Cred.G->Name);
    ...
    DBG(2, fSTAT)
    DPrint("INFO:     job '%s' completed  X: %lf  T: %ld  PS: %d  A: %lf\n",
           J->Name, xfactor, run, psrun, accuracy);
    ...
    if (MSched.Mode == msmSim) {
        ...
        DBG(4, fSTAT)
        DPrint(
            "INFO:     comcost:  job '%18s' procs: %3d  comcost: %7.3f  time: "
            "%5ld  com: %7.3f  proc: %7.3f\n",
            J->Name, TotalProcs, cost, run, (cost * run * MSim.ComRate / 3600),
            (double)psrun / 3600);
...

{
    ...
    DBG(5, fSTAT)
    DPrint("%s(%s,%x)\n", FName, (J != NULL) ? J->Name : "NULL", ProfileMode);
    ...
    for (timeindex = 0; rval > MStat.P.TimeStep[timeindex]; timeindex++)
        ;
    for (procindex = 0;
         MIN(TotalProcs, MStat.P.MaxNode) > MStat.P.NodeStep[procindex];
         procindex++)
        ;
    ...
    DBG(7, fSTAT)
    DPrint("INFO:     updating statistics (user %s  group %s  account %s)\n",
           J->Cred.U->Name, J->Cred.G->Name,
...

{
    ...
    DBG(7, fSTAT)
    DPrint(
        "INFO:     updating submit statistics for Grid[time: %d][proc: %d]\n",
        timeindex, procindex);
    ...
    if (J->Cred.A != NULL) S[stattotal++] = &J->Cred.A->Stat;
...
{
    const int CPAList[] = {msysaStatInitTime, msysaVersion, msysaSyncTime, -1};

    const int AList[] = {msysaPresentTime, msysaStatInitTime, msysaVersion,
                         msysaSyncTime, -1};
    ...
    MSysToXML(S, &E, (IsCP == TRUE) ? (int *)CPAList : (int *)AList,
              (IsCP == TRUE) ? (int *)CPDList : NULL, 0);
...

{
    ...
    int DAList[] = {msysaSyncTime, msysaVersion, -1};

    int DCList[] = {mxoStats, mxoLimits, mxoFS, -1};
...

{
    ...
    switch (AIndex) {
        case msysaPresentTime:
            ...
            sprintf(Buf, "%ld", MSched.Time);

            break;

        case msysaStatInitTime:

            sprintf(Buf, "%ld", MStat.InitTime);

            break;

        case msysaSyncTime:

            sprintf(Buf, "%ld", S->Sync.UpdateTime);
...
{
    int DAList[] = {mlaAJobs, mlaAProcs, mlaAPS, -1};
    ...
    for (aindex = 0; AList[aindex] != -1; aindex++) {
        ...
        MXMLSetAttr(*EP, (char *)MLimitAttr[AList[aindex]], tmpString,
                    mdfString);
    } /* END for (aindex) */
...
int MLimitAToString(
    ...
    mcredl_t *L, int AIndex, char *Buf, int Format)

{
    ...
    switch (AIndex) {
        case mlaAJobs:
            ...
            sprintf(Buf, "%d", L->AP.Usage[mptMaxJob][0]);

            break;

        case mlaAProcs:

            sprintf(Buf, "%d", L->AP.Usage[mptMaxProc][0]);

            break;

        case mlaAPS:

            sprintf(Buf, "%d", L->AP.Usage[mptMaxPS][0]);
...
{
    int DAList[] = {mstaTJobCount,  mstaTNJobCount,
    ...
    for (aindex = 0; AList[aindex] != -1; aindex++) {
        ...
        MXMLSetAttr(*EP, (char *)MStatAttr[AList[aindex]], tmpString,
                    mdfString);
    } /* END for (aindex) */
...

{
    ...
    switch (AIndex) {
        ...
        case mstaTNJobCount:
            ...
            sprintf(Buf, "%d", S->NCount);

            break;

        case mstaTQueueTime:

            sprintf(Buf, "%ld", S->TotalQTS);

            break;

        case mstaMQueueTime:

            sprintf(Buf, "%ld", S->MaxQTS);

            break;

        case mstaTReqWTime:

            sprintf(Buf, "%.3lf", S->TotalRequestTime);

            break;

        case mstaTExeWTime:

            sprintf(Buf, "%.3lf", S->TotalRunTime);

            break;

        case mstaTMSAvl:

            sprintf(Buf, "%.3lf", S->MSAvail);

            break;

        case mstaTMSDed:

            sprintf(Buf, "%.3lf", S->MSDedicated);

            break;

        case mstaTPSReq:

            sprintf(Buf, "%.3lf", S->PSRequest);

            break;

        case mstaTPSExe:

            sprintf(Buf, "%.3lf", S->PSRun);

            break;

        case mstaTPSDed:

            sprintf(Buf, "%.3lf", S->PSDedicated);

            break;

        case mstaTPSUtl:

            sprintf(Buf, "%.3lf", S->PSUtilized);

            break;

        case mstaTJobAcc:

            sprintf(Buf, "%.3lf", S->JobAcc);

            break;

        case mstaTNJobAcc:

            sprintf(Buf, "%.3lf", S->NJobAcc);

            break;

        case mstaTXF:

            sprintf(Buf, "%.3lf", S->XFactor);

            break;

        case mstaTNXF:

            sprintf(Buf, "%.3lf", S->NXFactor);

            break;

        case mstaMXF:

            sprintf(Buf, "%.3lf", S->MaxXFactor);

            break;

        case mstaTBypass:

            sprintf(Buf, "%d", S->Bypass);

            break;

        case mstaMBypass:

            sprintf(Buf, "%d", S->MaxBypass);

            break;

        case mstaTQOSAchieved:

            sprintf(Buf, "%d", S->QOSMet);
        ...
        case mstaGCEJobs: /* current eligible jobs */
            ...
            sprintf(Buf, "%d", MStat.EligibleJobs);

            break;

        case mstaGCIJobs: /* current idle jobs */

            sprintf(Buf, "%d", MStat.IdleJobs);

            break;

        case mstaGCAJobs: /* current active jobs */

            sprintf(Buf, "%d", MStat.ActiveJobs);

            break;

        case mstaGPHAvl:

            sprintf(Buf, "%lf", MStat.TotalPHAvailable);

            break;

        case mstaGPHUtl:

            sprintf(Buf, "%lf", MStat.TotalPHBusy);

            break;

        case mstaGPHSuc:

            sprintf(Buf, "%lf", MStat.SuccessfulPH);

            break;

        case mstaGMinEff:

            sprintf(Buf, "%lf", MStat.MinEff);

            break;

        case mstaGMinEffIteration:

            sprintf(Buf, "%d", MStat.MinEffIteration);

            break;

        case mstaTPHPreempt:

            sprintf(Buf, "%lf", MStat.TotalPHPreempted);

            break;

        case mstaTJPreempt:

            sprintf(Buf, "%d", MStat.TotalPreemptJobs);

            break;

        case mstaTJEval:

            sprintf(Buf, "%d", MStat.JobsEvaluated);

            break;

        case mstaTPHQueued:

            sprintf(Buf, "%lu", MStat.AvgQueuePH);

            break;

        case mstaSchedDuration:

            sprintf(Buf, "%ld", MStat.SchedRunTime);

            break;

        case mstaSchedCount:

            sprintf(Buf, "%d", MSched.Iteration);
