 ...
 *   int
 * MPolicyAdjustUsage(ResUsage,J,R,LimitType,PU,OIndex,Count,ViolationDetected)
 * *
 *                                                          */
...
static int MQueueCheckSingleJob(
    ...
    mjob_t *J, int *Reason, mpar_t *P, mpar_t *GP, int PLevel, int MaxNC,
    int MaxPC, time_t MaxWCLimit, int OrigPIndex, mbool_t UpdateStats);
...

{
    ...
    DBG(3, fSCHED)
    DPrint("%s(SrcQ,DstQ,%s,%d,%d,%ld,%s,FReason,%s)\n", FName,
           MPolicyMode[PLevel], MaxNC, MaxPC, MaxWCLimit,
    ...
    if ((SrcQ == NULL) || (DstQ == NULL)) {
        DBG(1, fSCHED)
        DPrint("ERROR:    invalid arguments passed to %s()\n", FName);
    ...
    if (SrcQ[0] == -1) {
        DBG(4, fSCHED)
        DPrint("INFO:     idle job queue is empty on iteration %d\n",
               MSched.Iteration);
    ...
    for (jindex = 0; SrcQ[jindex] != -1; jindex++) {
        ...
        if (J != NULL) {
            DBG(7, fSCHED)
            DPrint("INFO:     checking job[%d] '%s'\n", jindex, J->Name);
        } else {
            DBG(7, fSCHED)
            DPrint("INFO:     queue job[%d] has been removed\n", jindex);
        ...
        if (OnlyDefPart == TRUE && MJobFindDefPart(J, NULL, NULL) != P) {
            DBG(7, fSCHED)
            DPrint(
                "INFO:     skipping job[%d] '%s', only default partition check "
                "requested (and current partition is %s)\n",
                jindex, J->Name, P->Name);

            continue;
        }

        if (MQueueCheckSingleJob(J, Reason, P, GP, PLevel, MaxNC, MaxPC,
                                 MaxWCLimit, OrigPIndex,
                                 UpdateStats) == FAILURE)
            continue;
        ...
        J->EffQueueDuration = (MSched.Time > J->SystemQueueTime)
                                  ? MSched.Time - J->SystemQueueTime
                                  : 0;

        /* add job to destination queue */

        DBG(5, fSCHED)
        DPrint("INFO:     job '%s' added to queue at slot %d\n", J->Name,
               sindex);
    ...
    DBG(1, fSCHED) {
        DBG(1, fSCHED)
        DPrint("INFO:     total jobs selected in partition %s: %d/%-d ",
               MAList[ePartition][PIndex], sindex, jindex);
 ...
 */
static int MQueueCheckSingleJob(mjob_t *J, int *Reason, mpar_t *P, mpar_t *GP,
                                int PLevel, int MaxNC, int MaxPC,
                                time_t MaxWCLimit, int OrigPIndex,
                                mbool_t UpdateStats)

{
    ...
    if (UpdateStats == TRUE) {
        ...
        if (J->State == mjsIdle) MStat.IdleJobs++;
    }
    ...
    if (OrigPIndex != -1) {
        if ((P->Index == 0) && !(J->Flags & (1 << mjfSpan))) {
            ...
            DBG(3, fSCHED)
            DPrint("INFO:     job %s not considered for spanning\n", J->Name);
            ...
            DBG(3, fSCHED)
            DPrint(
                "INFO:     spanning job %s not considered for partition "
                "scheduling\n",
                J->Name);
        ...
        if ((P->Index > 0) && (MUBMCheck(P->Index, J->PAL) == FAILURE)) {
            DBG(7, fSCHED)
            DPrint(
                "INFO:     job %s not considered for partition %s (allowed "
                "%s)\n",
    ...
    if ((J->State != mjsIdle) && (J->State != mjsSuspended)) {
        DBG(6, fSCHED)
        DPrint("INFO:     job %s rejected (job in non-idle state '%s')\n",
               J->Name, MJobState[J->State]);

        Reason[marState]++;

        if ((MaxNC == MAX_MNODE) && (MaxWCLimit == MAX_MTIME) &&
            (J->R != NULL)) {
    ...
    if ((J->EState != mjsIdle) && (J->EState != mjsSuspended)) {
        DBG(6, fSCHED)
        DPrint(
            "INFO:     job %s rejected (job in non-idle expected state: "
            "'%s')\n",
    ...
    if (PReq > P->CRes.Procs) {
        DBG(6, fSCHED)
        DPrint(
            "INFO:     job %s rejected in partition %s (exceeds configured "
            "procs: %d > %d)\n",
            J->Name, P->Name, PReq, P->CRes.Procs);
    ...
    if (MJobCheckLimits(J, PLevel, P, (1 << mlSystem), tmpLine) == FAILURE) {
        DBG(6, fSCHED)
        DPrint("INFO:     job %s rejected, partition %s (%s)\n", J->Name,
               P->Name, tmpLine);
        ...
        if (P->Index <= 0) {
            ...
            MJobSetHold(J, (1 << mhDefer), MSched.DeferTime, mhrSystemLimits,
    ...
    if (PReq > MaxPC) {
        DBG(6, fSCHED)
        DPrint(
            "INFO:     job %s rejected in partition %s (exceeds window size: "
            "%d > %d)\n",
            J->Name, P->Name, PReq, MaxPC);
    ...
    if (J->SpecWCLimit[0] > MaxWCLimit) {
        DBG(6, fSCHED)
        DPrint(
            "INFO:     job %s rejected in partition %s (exceeds window time: "
            "%ld > %ld)\n",
            J->Name, P->Name, J->SpecWCLimit[0], MaxWCLimit);
    ...
    if (P->Index > 0) {
        if (MUNumListGetCount(J->StartPriority, RQ->DRes.PSlot, P->CRes.PSlot,
            ...
            DBG(6, fSCHED)
            DPrint(
                "INFO:     job %s rejected, partition %s (classes not "
                "supported '%s')\n",
                J->Name, P->Name,
    ...
    if (MJobCheckDependency(J, &DType, DValue) == FAILURE) {
        DBG(6, fSCHED)
        DPrint("INFO:     job %s rejected (dependent on job '%s' %s)\n",
               J->Name, DValue, MJobDependType[DType]);
    ...
    if (MJobCheckPolicies(J, PLevel, (1 << mlActive),
                          P, /* NOTE:  may set to &MPar[0] */
                          &PReason, NULL, MAX_MTIME) == FAILURE) {
        DBG(6, fSCHED)
        DPrint(
            "INFO:     job %s rejected, partition %s (policy failure: '%s')\n",
            J->Name, P->Name, MPolicyRejection[PReason]);
    ...
    if (J->Cred.A != NULL) J->Cred.A->MTime = MSched.Time;
    ...
    if (MPar[0].FSC.FSPolicy != fspNONE) {
        ...
        if (MFSCheckCap(NULL, J, P, &OIndex) == FAILURE) {
            DBG(5, fSCHED)
            DPrint("INFO:     job '%s' exceeds %s FS cap\n", J->Name,
    ...
    if (MLocalCheckFairnessPolicy(J, MSched.Time, NULL) == FAILURE) {
        DBG(6, fSCHED)
        DPrint(
            "INFO:     job %s rejected, partition %s (violates local fairness "
            "policy)\n",
            J->Name, P->Name);
...
int MQueueSelectAllJobs(
    ...
    mpar_t *P, int *JIList, /* O:  idle job list */
...

{
    ...
    DBG(3, fSCHED)
    DPrint("%s(%s,%s,%s,%s,DP,Msg)\n", FName, (Q != NULL) ? "Q" : "NULL",
           MPolicyMode[PLevel], (P != NULL) ? P->Name : "NULL",
    ...
    if ((P == NULL) || (Q == NULL) ||
        ((DoPrioritize == TRUE) &&
         ((Q[0] == NULL) || (Q[0]->Next == NULL) || (Q[0]->Next == Q[0])))) {
        if (Msg != NULL) strcpy(Msg, "queue is corrupt");
    ...
    for (jindex = 0; tmpJIList[jindex] != -1; jindex++) {
        ...
        if ((J->State != mjsIdle) && (J->State != mjsSuspended)) {
            if (Msg != NULL) {
                sprintf(Msg, "non-idle state '%s'", MJobState[J->State]);
            }
            ...
            if ((J->State != mjsStarting) && (J->State != mjsRunning)) {
                ...
                if (UpdateStats == TRUE) {
                    if (MJobCheckLimits(J, ptSOFT, P, (1 << mlActive),
                                        Message) == FAILURE) {
                    ...
                    MPolicyAdjustUsage(NULL, J, NULL, mlActive, &MPar[0].L.AP,
                                       -1, 1, NULL);
                    MPolicyAdjustUsage(NULL, J, NULL, mlActive, NULL, -1, 1,
                                       NULL);
                }
        ...
        if (J->Hold != 0) {
            ...
            DBG(6, fSCHED)
            DPrint("INFO:     job %s rejected (hold on job)\n", J->Name);
        ...
        if ((J->Cred.C != NULL) && (J->Cred.C->IsDisabled == TRUE)) {
            ...
            DBG(6, fSCHED)
            DPrint("INFO:     job %s rejected (class not enabled)\n", J->Name);
        ...
        if ((MPar[0].RejectNegPrioJobs == TRUE) && (J->StartPriority < 0)) {
            ...
            DBG(6, fSCHED)
            DPrint("INFO:     job %s rejected (negative priority)\n", J->Name);
        ...
        if (J->SMinTime > MSched.Time) {
            ...
            DBG(6, fSCHED)
            DPrint(
                "INFO:     job %s rejected (release time violation: %ld > "
                "%ld)\n",
                J->Name, J->SMinTime, MSched.Time);
        ...
        if (J->MasterJobName != NULL) {
            if (Msg != NULL) {
                sprintf(Msg, "job is subjob to '%s'", J->MasterJobName);
            }

            DBG(6, fSCHED)
            DPrint("INFO:     job %s rejected (job is subjob to '%s')\n",
                   J->Name, J->MasterJobName);
        ...
        if (MJobGetProcCount(J) > MPar[0].URes.Procs) {
            if (Msg != NULL) {
                sprintf(Msg, "inadequate procs in system: %d < %d",
                        MJobGetProcCount(J), MPar[0].URes.Procs);
            }

            DBG(6, fSCHED)
            DPrint(
                "INFO:     job %s rejected, (exceeds available procs in "
                "system: %d > %d)\n",
                J->Name, MJobGetProcCount(J), MPar[0].URes.Procs);
        ...
        if ((J->R == NULL) &&
            ...
            if (UpdateStats == TRUE) J->BlockReason = mjneIdlePolicy;
        ...
        DBG(7, fSCHED)
        DPrint("INFO:     job[%02d] '%s' added to master list\n", JIndex,
               J->Name);
    ...
    DBG(1, fSCHED) {
        ...
        DBG(1, fSCHED)
        DPrint("INFO:     total jobs selected (ALL): %d/%-d ", JIndex,
               JobCount);

        for (findex = 0; findex < MAX_MREJREASON; findex++) {
            if (Reason[findex] != 0) {
                fprintf(mlog.logfp, "[%s: %d]", MAllocRejType[findex],
                        Reason[findex]);
            }
        } /* END for (findex) */
    ...
    DBG(4, fSCHED) {
        ...
        for (jindex = 0; JIList[jindex] != -1; jindex++) {
            fprintf(mlog.logfp, "[%03d: %3d]", jindex, JIList[jindex]);
        } /* END for (jindex) */
...
int MJobCheckPolicies(
    ...
    int PType, mpar_t *P, /* IN:  node partition              */
...

{
    ...
    DBG(5, fSCHED)
    DPrint("%s(%s,%s,%d,%s,RIndex,%s,%ld)\n", FName, J->Name,
           MPolicyMode[PLevel], PType, (P != NULL) ? P->Name : "NULL",
           (Buffer == NULL) ? "NULL" : "Buffer", StartTime);

    if ((J == NULL) || (P == NULL)) return (FAILURE);

    if (MJobCheckLimits(J, PLevel, P,
                        (PType != 0) ? PType : (1 << mlActive) | (1 << mlIdle) |
                                                   (1 << mlSystem),
                        Buffer) == FAILURE) {
        /* job limit rejection */

        if (RIndex != NULL) *RIndex = prMaxJobPerUserPolicy;
    ...
    if (Buffer != NULL) {
        ...
        DBG(4, fSCHED) DPrint("INFO:     %s", Buffer);
    }
...
int MPolicyCheckLimit(
    ...
    int PlIndex,    /* I */
    int PLevel,     /* I */
    int PtIndex, mpu_t *ObjectP, mpu_t *DefaultP, mpu_t *QOSP, int *LimitP)

{
    ...
    if ((QOSP != NULL) && (QOSP->HLimit[PlIndex][PtIndex] != 0)) {
        ...
        Limit = (PLevel == ptSOFT) ? QOSP->SLimit[PlIndex][PtIndex]
                                   : QOSP->HLimit[PlIndex][PtIndex];
    } else if ((ObjectP != NULL) && (ObjectP->HLimit[PlIndex][PtIndex] != 0)) {
        /* use object */

        Limit = (PLevel == ptSOFT) ? ObjectP->SLimit[PlIndex][PtIndex]
                                   : ObjectP->HLimit[PlIndex][PtIndex];
    } else if ((DefaultP != NULL) &&
               (DefaultP->HLimit[PlIndex][PtIndex] != 0)) {
        /* use default object */

        Limit = (PLevel == ptSOFT) ? DefaultP->SLimit[PlIndex][PtIndex]
    ...
    if (ObjectP != NULL) Usage += ObjectP->Usage[PlIndex][PtIndex];

    if (LimitP != NULL) *LimitP = Limit;
...
int MPolicyCheckJob(
    ...
    mjob_t *J, long StartTime, int *RIndex, char *Message)
...

{
    ...
    DPrint("%s(%s,%s,%s,Time)\n", FName, (J != NULL) ? J->Name : "NULL",
           (P != NULL) ? P->Name : "NULL", MPolicyMode[PLevel]);
    ...
    *Time = MAX_MTIME;
    ...
    for (oindex = 0; OList[oindex] != -1; oindex++) {
        ...
        switch (OList[oindex]) {
            case mxoUser:
                ...
                if (J->Cred.U != NULL) OP = &J->Cred.U->L.AP;

                if (MSched.DefaultU != NULL) DP = &MSched.DefaultU->L.AP;

                break;

            case mxoGroup:

                if (J->Cred.G != NULL) OP = &J->Cred.G->L.AP;

                if (MSched.DefaultG != NULL) DP = &MSched.DefaultG->L.AP;

                break;

            case mxoAcct:

                if (J->Cred.A != NULL) OP = &J->Cred.A->L.AP;

                if (MSched.DefaultA != NULL) DP = &MSched.DefaultA->L.AP;

                break;

            case mxoQOS:

                if (J->Cred.Q != NULL) OP = &J->Cred.Q->L.AP;

                if (MSched.DefaultQ != NULL) DP = &MSched.DefaultQ->L.AP;

                break;

            case mxoClass:

                if (J->Cred.C != NULL) OP = &J->Cred.C->L.AP;

                if (MSched.DefaultC != NULL) DP = &MSched.DefaultC->L.AP;
        ...
        for (pindex = 0; PList[pindex] != -1; pindex++) {
            MPolicyCheckLimit(
                PConsumed[PList[pindex]], PList[pindex], PLevel, 0, OP, DP, QP,
    ...
    if (J->Cred.U != NULL) strcpy(JobAttrs[mxoUser], J->Cred.U->Name);

    if (J->Cred.G != NULL) strcpy(JobAttrs[mxoGroup], J->Cred.G->Name);

    if (J->Cred.A != NULL) strcpy(JobAttrs[mxoAcct], J->Cred.A->Name);

    if (J->Cred.Q != NULL) strcpy(JobAttrs[mxoQOS], J->Cred.Q->Name);

    if (J->Cred.C != NULL) strcpy(JobAttrs[mxoClass], J->Cred.C->Name);
    ...
    for (rindex = 0; rindex < (MAX_MRES << 2); rindex++) {
        if (MRE[rindex].Type == mreNONE) break;

        if (MRE[rindex].Index < 0) continue;
        ...
        if ((AvailStart > 0) &&
            ...
            *Time = AvailStart;
            ...
            DBG(4, fSCHED)
            DPrint(
                "INFO:     adequate policy slot located at time %s for job "
                "%s\n",
                MULToTString(AvailStart - MSched.Time), J->Name);
        ...
        for (aindex = 0; aindex < MAX_MACL; aindex++) {
            if (R->CL[aindex].Name[0] == '\0') break;

            if (strcmp(R->CL[aindex].Name, JobAttrs[(int)R->CL[aindex].Type]) !=
                0)
                continue;

            if (!memcmp(PAvailable[(int)R->CL[aindex].Type].HLimit, ZList,
                        sizeof(ZList))) {
            ...
            if (MRE[rindex].Type == mreStart) {
                ...
                /* NOTE:  decrease PS usage for all jobs not completing at time
                 * T by time dT */
                ...
                MPolicyAdjustUsage(NULL, NULL, R, mlActive, PAvailable,
                                   R->CL[aindex].Type, -1,
    ...
    if (AvailStart > 0) {
        *Time = AvailStart;
        ...
        DBG(4, fSCHED)
        DPrint("INFO:     policy start time found for job %s in %s\n", J->Name,
    ...
    DBG(4, fSCHED)
    DPrint("INFO:     no policy start time found for job %s\n", J->Name);
...

{
    ...
    DBG(4, fSCHED)
    DPrint("%s(%s,%s,%s,%s,%s,%s,%d,%s)\n", FName,
           (ResUsage != NULL) ? "ResUsage" : "NULL",
           (J != NULL) ? J->Name : "NULL", (R != NULL) ? R->Name : "NULL",
           MTLimitType[LimitType], (PU != NULL) ? "PU" : "NULL",
           (OIndex >= 0) ? MXO[OIndex] : ALL, Count,
    ...
    if (ResUsage != NULL) {
        ...
        if (PU == NULL) PIndex = J->Req[0]->PtIndex;
    } else if (R != NULL) {
        memset(PConsumed, 0, sizeof(PConsumed));
    ...
    if (PU == NULL) {
        switch (LimitType) {
            case mlIdle:
                ...
                if (J->Cred.U != NULL) tPU[mxoNONE][mxoUser] = J->Cred.U->L.IP;

                if (J->Cred.G != NULL) tPU[mxoNONE][mxoGroup] = J->Cred.G->L.IP;

                if (J->Cred.A != NULL) tPU[mxoNONE][mxoAcct] = J->Cred.A->L.IP;

                if (J->Cred.Q != NULL) tPU[mxoNONE][mxoQOS] = J->Cred.Q->L.IP;

                if (J->Cred.C != NULL) tPU[mxoNONE][mxoClass] = J->Cred.C->L.IP;
            ...
            default:
                ...
                if (J->Cred.U != NULL) {
                    if ((J->Cred.U->L.APC != NULL) && (J->Cred.C != NULL))
                        tPU[mxoClass][mxoUser] =
                            &J->Cred.U->L.APC[J->Cred.C->Index];

                    if ((J->Cred.U->L.APQ != NULL) && (J->Cred.Q != NULL))
                        tPU[mxoQOS][mxoUser] =
                            &J->Cred.U->L.APQ[J->Cred.Q->Index];

                    if ((J->Cred.U->L.APG != NULL) && (J->Cred.G != NULL))
                        tPU[mxoGroup][mxoUser] =
                ...
                if (J->Cred.A != NULL) tPU[mxoNONE][mxoAcct] = &J->Cred.A->L.AP;
    ...
    for (oindex1 = 0; oindex1 < mxoLAST; oindex1++) {
        for (oindex2 = 1; oindex2 < mxoLAST; oindex2++) {
            ...
            for (pindex = 0; pindex < MAX_MPOLICY; pindex++) {
                tPU[oindex1][oindex2]->Usage[pindex][0] +=
                    Count * PConsumed[pindex];
            }

            if (PIndex != 0) {
                for (pindex = 0; pindex < MAX_MPOLICY; pindex++) {
                    tPU[oindex1][oindex2]->Usage[pindex][PIndex] +=
                        Count * PConsumed[pindex];
                }
            }
