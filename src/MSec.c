...
#if !defined(SHA1_LITTLE_ENDIAN) && !defined(SHA1_BIG_ENDIAN)
#if defined(__AIX43) || defined(__AIX51) || defined(__HPUX) || \
    defined(__IRIX) || defined(__SOLARIS)
...
int MSecHMACGetDigest(unsigned char *, int, unsigned char *, int, char *, int,
...

{
    ...
    MDB(5, fSTRUCT)
    MLog("%s(Buf,%d,Checksum,%s,CSKey)\n", FName, BufSize, MCSAlgoType[Algo]);
#endif /* !__MPROD */
    ...
    switch (Algo) {
        ...
        {
            MSecHMACGetDigest((unsigned char *)CSKeyString, strlen(CSKeyString),
        ...
        {
            MSecMD5GetDigest(CSKeyString, strlen(CSKeyString), Buf, strlen(Buf),
...

        {
            ...
            for (index = 0; index < BufSize; index++) {
                crc =
                    (unsigned int)__MSecDoCRC((unsigned short)crc, Buf[index]);
            }
...

{
    ...
    switch (Algo) {
...

        {
            ...
            MSecMD5GetDigest(CSKeyString, strlen(CSKeyString), ptr,
...

        {
            ...
            for (index = 0; index < BufSize1; index++) {
                crc =
                    (unsigned int)__MSecDoCRC((unsigned short)crc, Buf1[index]);
            } /* END for (index) */

            for (index = 0; index < BufSize2; index++) {
                crc =
                    (unsigned int)__MSecDoCRC((unsigned short)crc, Buf2[index]);
            } /* END for (index) */
...

{
    ...
    MLog("%s(%s,%d,%s,%d,CSString,%d,DigestString,%s,%s)\n", FName,
         ...
         MaxCSStringLen, MBool[TwoPass], MBool[Use64BitEncoding]);
#endif /* !__MPROD */
    ...
    if (TwoPass == TRUE) {
        ...
        if (DigestString != NULL) {
            ...
            if (Use64BitEncoding == TRUE) {
                MSecBufTo64BitEncoding((char *)osha2, SHA_DIGESTSIZE,
                                       DigestString);
            } else {
                MSecBufToHexEncoding((char *)osha2, SHA_DIGESTSIZE,
                                     DigestString);
            }
    ...
    __MSecHMACTruncate((char *)osha, tmpBuf,
                       MIN(SHA_DIGESTSIZE, MaxCSStringLen));

    if (Use64BitEncoding == TRUE) {
        MSecBufTo64BitEncoding(tmpBuf, MIN(SHA_DIGESTSIZE, MaxCSStringLen),
                               CSString);
    } else {
        MSecBufToHexEncoding(tmpBuf, MIN(SHA_DIGESTSIZE, MaxCSStringLen),
                             CSString);
    }
...

{
    ...
    rc = MSecGetChecksum(Msg, strlen(Msg), CSBuf, Digest, AIndex, CSKey);
...

{
    ...
    for (; (X < SrcSize) && (Y <= SrcSize);) {
        ...
        if (Size >= 16) {
            ...
            X += Size;
            Command = (Command << 1) + 1;
        } else if (MSecCompressionGetMatch(SrcString, X, SrcSize, Hash, &Size,
                                           (int *)&Pos)) {
            Key = ((X - Pos) << 4) + (Size - 3);
            Dest[Y++] = (Key >> 8) & 0x00ff;
            Dest[Y++] = Key & 0x00ff;
