...
typedef struct { msocket_t S; } s3handle_t;
...
int MS3JobSubmit(char *, mrm_t *, mjob_t **, long, char *, char *,
...
int __MS3JobSetAttr(mjob_t *, mreq_t *, int, char *, char *, mbool_t, mbool_t,
                    void *);
...
/* NOTE:  'NodeList' maps to mjaAllocNodeList in 'Utilized' and mjaHostList in
 * 'Requested' */

/* attribute           object    0.2 attr                  3.0 attr        4.0
 * attr  */
...
extern "C" const mobjattr_t MS3JobTable[] = {
    ...
    {mjaCompletionTime,
     mxoJob,
     {NULL, "CompletionTime", NULL, NULL}}, /* change to 'EndTime' */
    ...
    {mjaRMXString,
     mxoJob,
     {"opaqueFlags", "OpaqueFlags", NULL,
      NULL}}, /* replace w/Extension (attrs-> Type={scheduler,grid,am,???}
                 Name=<attr>  5.1.10 of S3 Job Description */
    ...
    {mjaSubmitTime,
     mxoJob,
     {NULL, "SubmitTime", NULL, NULL}}, /* change to SubmissionTime */
...
const mobjattr_t MS3JobTable[] = {
    ...
    {mrqaReqNodeFeature,
     ...
     {NULL, "Feature", NULL,
      NULL}}, /* should be
                 <NodeProperties><Feature></Feature><Name></Name><ConsumableResources/></NodeProperties>,
                 see 5.1.9 NOTE:  consumable resources are attributes of node,
                 ie, minswappernode, not to be allocated to job */
    ...
    MS3NodeTable[] =
        {
            {mnaFeatures,
             ...
             {"features", "Features", NULL,
              "Features"}}, /* should be NodeProperties, see 5.1.9, may have
                               child elements of Name, Feature,
                               ConsumableResources */
            {mnaNodeID, mxoNode, {"nodeId", "NodeId", NULL, "NodeId"}},
            {mnaNodeState,
             mxoNode,
             {"nodeState", "NodeState", NULL, "NodeState"}},
            {mnaRCMem,
             mxoNode,
             {"memTot", "ConfiguredMemory", NULL, "Configured/Memory"}},
            {mnaRAMem,
             mxoNode,
             {"memFree", "AvailableMemory", NULL, "Available/Memory"}},
            {mnaRCSwap,
             mxoNode,
             {"swapTot", "ConfiguredSwap", NULL, "Configured/Swap"}},
            {mnaRASwap,
             mxoNode,
             {"swapFree", "AvailableSwap", NULL, "Available/Swap"}},
            {mnaRCDisk,
             mxoNode,
             {"diskTot", "ConfiguredDisk", NULL, "Configured/Disk"}},
            {mnaRADisk,
             mxoNode,
             {"diskFree", "AvailableDisk", NULL, "Available/Disk"}},
            {mnaRCProc,
             mxoNode,
             {"numProcessors", "ConfiguredProcessors", NULL,
              "Configured/Processors"}},
            {mnaLoad,
             mxoNode,
             {"utlProcessors", "UtilizedProcessors", NULL,
              "Utilized/Processors"}},

            /* { mna???,         mxoNode,  { "nodeType",     "NodeType", NULL,
               NULL } }, */
            /* { mna???,         mxoNode,  { "maxtask",
               "VirtualProcessors", NULL, NULL } }, */

            /* NOTE:  percentCPUUsed is available, node load is not */

            {mnaRAProc,
             mxoNode,
             {"AVLProcCount", "AvailableProcessors", NULL,
...
const char *MS3RangeAttr[] = {NONE,  "CompletionTime", "Cost",      "Nodes",
                              "QOI", "ScalingFactor",  "StartTime", "Tasks",
                              NULL};

/* sync w/ enum XXX */

const char *MS3RsvAttr[] = {NONE,        "EndTime", "Name",
...

{
    ...
    for (vindex = 0; vindex < MMAX_S3VERS; vindex++) {
        ...
        for (aindex = 0; MS3OTable[aindex].AIndex != mxoNONE; aindex++) {
            MS3Object[MS3OTable[aindex].AIndex] =
...

{
    ...
    if (PE == NULL) {
        ...
        MXMLSetAttr(PE, MSAN[msanAction], (void *)MS3Action[msssaQuery],
                    mdfString);
    ...
    rc = MS3DoCommand(&R->P[0], tmpLine, NULL, NULL, NULL, NULL);
...

{
    ...
    if (!MISSET(R->Flags, mrmfLocalQueue)) {
        ...
        if (MSysDSRegister((char *)MS3CName[mpstSC], MRM[0].Name,
                           MSched.ServerHost, MSched.ServerPort, NULL,
                           (char *)MS3SockProtocol[mspSingleUseTCP]) ==
            FAILURE) {
            MDB(1, fRM)
            MLog("ALERT:    cannot register with directory service\n");
        }

        if (R->UseDirectoryService == TRUE) {
            /* obtain interface information */

            if (MSysDSQuery((char *)MS3CName[mpstQM], MRM[0].Name, HostName,
                            &Port, WireProtocol, SocketProtocol) == SUCCESS) {
                MDB(1, fRM)
                MLog("INFO:     obtained QM info (H: %s  P: %d  S: %s)\n",
                     HostName, Port, SocketProtocol);
            ...
            if (MSysDSQuery((char *)MS3CName[mpstNM], MRM[0].Name, HostName,
                            &Port, WireProtocol, SocketProtocol) == SUCCESS) {
        ...
        if (MSysDSQuery((char *)MS3CName[mpstEM], MRM[0].Name, HostName, &Port,
                        WireProtocol, SocketProtocol) == SUCCESS) {
            /* update active EM interface */

            if (HostName[0] != '\0') MUStrDup(&MSched.EM.HostName, HostName);

            if (Port != -1) MSched.EM.Port = Port;
        }

        MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC], "schedstart",
                     MSched.Name);

        /* NOTE:  currently ignore EM responses */

        MSysEMRegister(&MSched.EM, (char *)MS3CName[mpstQM],
                       (char *)"job-submitted", NULL, (char *)"NONE");

        MSysEMRegister(&MSched.EM, (char *)MS3CName[mpstQM],
                       (char *)"job-finished", NULL, (char *)"NONE");
...

{
    ...
    while (EventsOutstanding == TRUE) {
        ...
        if (select(FD_SETSIZE, &fdset, NULL, NULL, &timeout) == -1) {
            MDB(1, fS3)
            MLog("ALERT:    select failed checking S3 sched socket\n");
        ...
        if ((rpp_fd != -1) && !FD_ISSET(rpp_fd, &fdset)) {
            ...
            MDB(9, fS3)
            MLog("ALERT:    no S3 RPP sched socket connections ready\n");
            ...
            MDB(9, fS3)
            MLog("INFO:     no S3 sched socket connections ready\n");
        ...
        if (MSUAcceptClient(&tmpS, &tmpC, NULL, (1 << msftTCP)) == FAILURE) {
            ...
            MDB(1, fS3)
            MLog("ALERT:    cannot accept client on S3 sched socket\n");
        ...
        if (MSURecvPacket(tmpS.sd, (char **)&Cptr, sizeof(int), NULL,
                          MDEF_CLIENTTIMEOUT, NULL) == FAILURE) {
...

{
    ...
    if (QueueReqString == NULL) {
        ...
        if ((R->Version == msssV3_0) || (R->Version == msssV4_0)) {
            ...
            MXMLSetAttr(E, MSAN[msanAction],
                        (void *)MS3JAName[R->Version][mrmJobQuery], mdfString);
        ...
        MS3DoCommand(&R->P[0], QueueReqString, NULL, &RDE, NULL, MQueueErrMsg);
    ...
    if (JobReqString == NULL) {
        ...
        if (MISSET(R->Flags, mrmfLocalQueue)) {
            ...
            const enum MJobAttrEnum JAttrList[] = {
                mjaJobName,        mjaJobID,       mjaUser,
                mjaGroup,          mjaAccount,     mjaState,
                mjaAWDuration,     mjaReqSMinTime, mjaRMXString,
                mjaAllocNodeList,  mjaSubmitTime,  mjaStartTime,
                mjaCompletionTime, mjaUserPrio,    mjaNONE};

            const enum MReqAttrEnum RAttrList[] = {mrqaReqClass, mrqaNCReqMin,
                                                   mrqaTCReqMin, mrqaNONE};

            MXMLCreateE(&E, MSON[msonRequest]);

            MXMLSetAttr(E, MSAN[msanAction], (void *)MS3Action[msssaQuery],
                        mdfString);
    ...
    MS3DoCommand(&R->P[0], JobReqString, NULL, &RDE, NULL, MWorkloadErrMsg);
    ...
    if (ResReqString == NULL) {
        ...
        if ((R->Version == msssV3_0) || (R->Version == msssV4_0)) {
            ...
            enum MNodeAttrEnum NAttrList[] = {
                mnaNodeID, mnaNodeState, mnaRCProc, mnaRCMem,
                mnaRAMem,  mnaRCSwap,    mnaRASwap, mnaNONE};
            ...
            for (aindex = 0; NAttrList[aindex] != mnaNONE; aindex++) {
                MS3AddGet(E, MS3NodeAttr[MCONST_S3XPATH][NAttrList[aindex]],
                          NULL);
            } /* END for (aindex) */
        }     /* END if ((R->Version == msssV3_0) || ...) */
    ...
    rc = MS3DoCommand(P, ResReqString, NULL, &RDE, NULL, MResourceErrMsg);
    ...
    MDB(1, fS3)
    MLog("INFO:     S3 data updated for iteration %d (rc: %d)\n",
         S->UpdateIteration, rc);
...

{
    ...
    if (S->QueueBuffer == NULL) {
        if (EMsg != NULL) strcpy(EMsg, MQueueErrMsg);
    ...
    while (MXMLGetChildCI(DE, MS3ObjName[R->Version][mxoQueue], &CTok, &QE) ==
           SUCCESS) {
        QID[0] = '\0';
        ...
        if (QCount != NULL) (*QCount)++;
    } /* END while(MXMLGetChildCI(DE,MS3ObjName[R->Version][mxoQueue],&CTok,&QE)
         == SUCCESS) */
...

{
    ...
    if (MISSET(R->Flags, mrmfLocalQueue)) {
        ...
        for (jindex = 0; jindex < MMAX_JOB; jindex++) {
            if (JL[jindex] == NULL) break;

            if (JL[jindex] == (char *)1) continue;

            if (MJobFind(JL[jindex], &J, mjsmExtended) == FAILURE) {
                /* NOTE:  If a job has not been found then it is a local queue
                 * job *
                 *        and must be created here. Job information must come
                 * from *
                 *        the checkpoint file, if no information is found,
                 * destroy job */
...
#endif /* __M32COMPAT */
                {
                    /* Invalid job -- insufficient data recorded in checkpoint
                     * file to instantiate job */

                    MDB(2, fS3)
                    MLog(
                        "ALERT:    invalid job %s in internal queue (removing "
                        "job)\n",
                        J->Name);
...
/* add failure message */
                ...
                if (J->Request.TC <= 1) {
                    if (J->Request.NC == 0) {
                        MDB(2, fS3)
                        MLog(
                            "ALERT:    no job task info located for job '%s' "
                            "(assigning taskcount to 1)\n",
                            J->Name);
                    }
                ...
                switch (J->State) {
                    ...
                    case mjsHold:
                        ...
                        if (MS3JobUpdate(NULL, &J, TaskList, R) == FAILURE) {
                            MDB(1, fS3)
                            MLog("ALERT:    cannot update S3 job '%s'\n",
                                 J->Name);
                    ...
                    case mjsVacated:
                        ...
                        /* if job never ran, remove record.  job cancelled
                         * externally */

                        if (MJOBISACTIVE(J) == FALSE) {
                            MDB(1, fS3)
                            MLog(
                                "INFO:     job '%s' was cancelled externally\n",
                                J->Name);
                        ...
                        MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC],
                                     "job-terminated", J->Name);

#ifndef __M32COMPAT
                        if (MISSET(J->SRM->Flags, mrmfFullCP)) {
                            char FileName[MMAX_PATH_LEN];

                            sprintf(FileName, "%s/%s.cp", MSched.SpoolDir,
                                    J->Name);
                        ...
                        switch (Status) {
                            ...
                            case mjsVacated:
                                ...
                                if (MSched.Time < (J->StartTime + J->WCLimit)) {
                                    ...
                                    sprintf(Message,
                                            "JOBWCVIOLATION:  job '%s' "
                                            "exceeded WC limit %s\n",
                                            J->Name, MULToTString(J->WCLimit));

                                    MSysRegEvent(Message, mactNONE, 0, 1);

                                    MDB(3, fS3)
                                    MLog(
                                        "INFO:     job '%s' exceeded wallclock "
                                        "limit %s\n",
                                        J->Name, MULToTString(J->WCLimit));
                            ...
                            default:
                                ...
                                MDB(1, fS3)
                                MLog(
                                    "WARNING:  unexpected job state (%d) "
                                    "detected for job '%s'\n",
                                    Status, J->Name);
    ...
    if (MISSET(R->Flags, mrmfLocalQueue)) {
        ...
        while (MXMLGetChildCI(DE, MS3ObjName[R->Version][mxoJob], &JCTok,
                              &JE) == SUCCESS) {
            ...
            while (NextSE != NULL) {
                ...
                if (__MS3JobStepGetState(StepE, R, RMJID, &Status) == FAILURE) {
                    ...
                    MDB(1, fS3)
                    MLog(
                        "ALERT:    cannot determine jobid/state in %s "
                        "(ignoring job)\n",
                        FName);
                ...
                switch (Status) {
                    ...
                    case mjsHold:
                        ...
                        if (MJobFind(SJobID, &J, mjsmBasic) == SUCCESS) {
                            ...
                            if (MS3JobUpdate(StepE, &J, TaskList, R) ==
                                FAILURE) {
                                MDB(1, fS3)
                                MLog("ALERT:    cannot update S3 job '%s'\n",
                                     SJobID);
                            ...
                            if (MJobCreate(SJobID, TRUE, &J) == FAILURE) {
                                MDB(1, fS3)
                                MLog(
                                    "ERROR:    job buffer is full  (ignoring "
                                    "job '%s')\n",
                                    SJobID);
...
/* if new job, load data */
                            ...
                            MJobSetAttr(J, mjaSRMJID, (void **)RMJID, mdfString,
                                        mSet);
                            MJobSetAttr(J, mjaDRMJID, (void **)RMJID, mdfString,
                                        mSet);

                            if (MS3JobLoad(SJobID, StepE, &J, TaskList, R) ==
                                FAILURE) {
                                MDB(1, fS3)
                                MLog("ALERT:    cannot load S3 job '%s'\n",
                                     SJobID);
                            ...
                            /* NOTE:  all MJobProcessExtensionString() calls
                               should be moved
                                      to MRMJobPostLoad() */

                            if (J->RMXString != NULL) {
#ifdef __M32COMPAT
                                if (MJobProcessExtensionString(
                                        J, J->RMXString) == FAILURE)
#else  /* __M32COMPAT */
                                if (MJobProcessExtensionString(
                                        J, J->RMXString, mxaNONE) == FAILURE)
#endif /* __M32COMPAT */
                                {
                                    MDB(1, fS3)
                                    MLog(
                                        "ALERT:    cannot process extension "
                                        "string for job '%s'\n",
                                        SJobID);
                    ...
                    case mjsVacated:
                        ...
                        if (MJobFind(SJobID, &J, mjsmBasic) == SUCCESS) {
                            ...
                            MDB(4, fS3)
                            MLog("INFO:     ignoring job '%s'  (state: %s)\n",
                                 SJobID, MJobState[Status]);
                        }
                    ...
                    default:
                        ...
                        MDB(1, fS3)
                        MLog(
                            "WARNING:  job '%s' detected with unexpected state "
                            "'%d'\n",
                            SJobID, Status);
            ...
            MDB(1, fS3) MLog("INFO:     all steps loaded for job\n");
             ...
             (MXMLGetChildCI(DE,MS3ObjName[R->Version][mxoJob],&JCTok,&JE) ==
             SUCCESS) */
    ...
    if (R->FailIteration != MSched.Iteration) {
        ...
        for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = JNext) {
            JNext = J->Next; /* store next job pointer in case current job is
                                removed */
            ...
            if ((J->ATime > 0) &&
                ...
                if (MJOBISACTIVE(J) == TRUE) {
                    MDB(1, fPBS)
                    MLog(
                        "INFO:     active S3 job %s has been removed from the "
                    ...
                    MDB(1, fPBS)
                    MLog(
                        "INFO:     non-active S3 job %s has been removed from "
                        "the queue.  assuming job was cancelled\n",
                        J->Name);
...

{
    ...
    if (Status != NULL) *Status = mclsNONE;
    ...
    if ((StepID != NULL) && (StepID[0] == '\0')) {
        switch (R->Version) {
...

            {
                ...
                if (MXMLGetChildCI(StepE, MS3JobAttr[R->Version][mjaJobID],
                                   NULL, &StepIDE) == SUCCESS) {
    ...
    if (Status != NULL) {
        switch (R->Version) {
...

            {
                ...
                if (MXMLGetChildCI(StepE, MS3JobAttr[R->Version][mjaState],
                                   NULL, &StateE) == FAILURE) {
                    return (FAILURE);
                }
                ...
                *Status = (enum MJobStateEnum)MUGetIndex(StateE->Val,
                                                         MS3JobState, FALSE, 0);
            } /* END BLOCK */
...

            {
                ...
                if (MXMLGetChildCI(StepE, MS3JobAttr[R->Version][mjaState],
                    ...
                    if (MXMLGetChildCI(StepE,
                                       MS3JobAttr[R->Version][mjaAWDuration],
                                       NULL, &C2) == SUCCESS) {
                        if (!strcmp(C2->Val, "0"))
                            *Status = mjsIdle;
                        else
                    ...
                    if ((*Status = (enum MJobStateEnum)MUGetIndex(
                             XE->Val, MJobState, FALSE, 0)) == 0) {
                        *Status = (enum MJobStateEnum)MUGetIndex(
                            XE->Val, MS3JobState, FALSE, 0);
                    }
...

{
    ...
    if ((R == NULL) || (R->S == NULL)) {
        ...
        if (EMsg != NULL) strcpy(EMsg, "invalid parameters");
    ...
    if (S->ResourceBuffer == NULL) {
        ...
        if (EMsg != NULL) strcpy(EMsg, MResourceErrMsg);
    ...
    if ((R->Version == msssV3_0) || (R->Version == msssV4_0)) {
        ...
        if (DE == NULL) {
            MDB(1, fS3)
            MLog("ALERT:    invalid XML element passed to %s\n", FName);
        ...
        if (((ptr = strstr(S->ResourceBuffer, "<Body")) == NULL) ||
            ...
            MDB(1, fS3)
            MLog("ALERT:    cannot parse XML passed to %s\n", FName);
        ...
        if (MXMLGetChildCI(E, MSON[msonResponse], NULL, &RE) == FAILURE) {
            ...
            MDB(1, fS3)
            MLog("ALERT:    cannot locate '%s' element in %s\n",
                 MSON[msonResponse], FName);
        ...
        MDB(1, fS3)
        MLog("ALERT:    unexpected S3 version '%d' in %s\n", R->Version, FName);
    ...
    while (MXMLGetChildCI(DE, MS3ObjName[R->Version][mxoNode], &CTok, &NE) ==
           SUCCESS) {
        NodeName[0] = '\0';
        ...
        if (MNodeFind(NodeName, &N) == SUCCESS) {
            ...
            MDB(1, fS3)
            MLog("ERROR:    node buffer is full  (ignoring node '%s')\n",
                 NodeName);
        }
    } /* END for (cindex) */
...

{
    ...
    MDB(1, fS3)
    MLog("%s(%s,%s,%s,%s)\n", FName, (NP != NULL) ? "NP" : "NULL",
         (R != NULL) ? R->Name : "NULL", (NodeID != NULL) ? "NodeID" : "NULL",
    ...
    if ((NodeID != NULL) && (NodeID[0] == '\0')) {
        ...
        if (MXMLGetChildCI(E, MS3NodeAttr[R->Version][mnaNodeID], NULL, &C) ==
            FAILURE) {
            return (FAILURE);
        }
    ...
    if (Status != NULL) {
        ...
        *Status = mnsNone;
        ...
        if (MXMLGetChildCI(E, MS3NodeAttr[R->Version][mnaNodeState], NULL,
                           &C) == FAILURE) {
            ...
            *Status = mnsUnknown;
            ...
            const enum MNodeStateEnum S3NSEq[] = {mnsNone, mnsUnknown, mnsDown,
                                                  mnsNONE};
            ...
            *Status = S3NSEq[index];
            ...
            const char *S3NState[] = {NONE,   "active", "busy",
                                      "down", "idle",   NULL};
            const enum MNodeStateEnum S3NSEq[] = {mnsNone, mnsActive, mnsBusy,
                                                  mnsDown, mnsIdle,   mnsNONE};
        ...
        MDB(1, fS3)
        MLog("INFO:     S3 node %s set to state %s (%s)\n",
             (NodeID != NULL) ? NodeID : NONE, MNodeState[*Status],
             (C != NULL) ? C->Val : "NULL");
    } /* END if (Status != NULL) */
...

{
    ...
    if (J->NodeList[0].N == NULL) {
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be started: (empty hostlist)\n",
             J->Name);
    ...
    switch (R->Version) {
        ...
        case msssV4_0:
            ...
            MXMLCreateE(&E, MSON[msonRequest]);
            MXMLSetAttr(E, MSAN[msanAction],
                        (void *)MS3JAName[R->Version][mrmJobStart], mdfString);
            ...
            for (nindex = 0; J->NodeList[nindex].N != NULL; nindex++) {
                if (J->NodeList[nindex].N->RM != R) continue;

                for (tindex = 0; tindex < MAX(1, J->NodeList[nindex].TC);
                     tindex++) {
                    N = J->NodeList[nindex].N;
    ...
    if (MS3DoCommand(&R->P[0], CmdString, &Response, NULL, NULL, Msg) ==
        FAILURE) {
        ...
        if (SC != NULL) *SC = mscRemoteFailure;
    ...
    if (MS3CheckStatus(RE, &tmpSC, tmpMsg) == FAILURE) {
        ...
        snprintf(tmpLine, MMAX_LINE,
                 "cannot start job - RM failure, rc: %d, msg: '%s'", tmpSC,
                 tmpMsg);

        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be started: RM failure (%s)\n", J->Name,
             tmpMsg);

        if (Msg != NULL) strcpy(Msg, tmpLine);
    ...
    MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC], (char *)"jobstart",
                 (char *)J->Name);
...

{
    ...
    MDB(1, fS3)
    MLog("%s(%s,%s,%s,EMsg,SC)\n", FName, (J != NULL) ? J->Name : "NULL",
         (R != NULL) ? R->Name : "NULL", (Message != NULL) ? Message : "NULL");
#endif /* !__MPROD */
    ...
    if ((J == NULL) || (R == NULL)) {
        if (EMsg != NULL) strcpy(EMsg, "internal error - bad parameters");
    ...
    switch (R->Version) {
        ...
        default:
            ...
            MXMLCreateE(&E, MSON[msonRequest]);
            MXMLSetAttr(E, MSAN[msanAction],
                        (void *)MS3JAName[R->Version][mrmJobCancel], mdfString);
    ...
    if (MS3DoCommand(&R->P[0], CmdString, &Response, NULL, NULL, EMsg) ==
        FAILURE) {
        /* job could not be cancelled */

        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be cancelled (command failed)\n",
             J->Name);
    ...
    if (((ptr = strstr(Response, "<Body")) == NULL) ||
        ...
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be cancelled (invalid response)\n",
             J->Name);

        MUFree(&Response);

        if (R->FailIteration == -1) R->FailIteration = MSched.Iteration;

        if (EMsg != NULL) strcpy(EMsg, "cannot parse response");
    ...
    if (MXMLGetChildCI(E, MSON[msonResponse], NULL, &RE) == FAILURE) {
        ...
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be cancelled (cannot parse response)\n",
             J->Name);
    ...
    if (MS3CheckStatus(RE, &tmpSC, tmpMsg) == FAILURE) {
        ...
        snprintf(tmpLine, MMAX_LINE,
                 "cannot cancel job - RM failure, rc: %d, msg: '%s'", tmpSC,
                 tmpMsg);

        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be cancelled: RM failure (%s)\n",
             J->Name, tmpMsg);

        if (EMsg != NULL) strcpy(EMsg, tmpLine);
    ...
    MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC], (char *)"jobcancel",
                 (char *)J->Name);
...

{
    ...
    MLog("%s(%s,%s,%s,%s)\n", FName, (N != NULL) ? N->Name : "NULL",
         (NP != NULL) ? "NP" : "NULL", MNodeState[Status],
         (R != NULL) ? R->Name : "NULL");
#endif /* !__MPROD */
    ...
    if (N->ARes.Disk > 0) {
        if (N->CRes.Disk <= 0) N->CRes.Disk = N->ARes.Disk;
    } else {
        N->ARes.Disk = 1;
        N->CRes.Disk = 1;
    }
    ...
    if (N->CRes.Mem > 0) {
        if (N->ARes.Mem <= 0) N->ARes.Mem = N->CRes.Mem;
    } else {
        N->ARes.Mem = 1;
        N->CRes.Mem = 1;
    }
    ...
    if (N->CRes.Swap > 0) {
        if (N->ARes.Swap <= 0) N->ARes.Swap = N->CRes.Swap;
    } else {
    ...
    for (cindex = 0; cindex < MMAX_CLASS; cindex++) {
        ...
        if ((C == NULL) || (C->Name[0] == '\0')) break;

        if (!strcmp(C->Name, "DEFAULT") || !strcmp(C->Name, "ALL")) continue;
        ...
        if (C->NodeList != NULL) {
            ...
            for (nindex = 0; nindex < MMAX_NODE; nindex++) {
                ...
                if (NL[nindex].N == N) break;
            } /* END for (nindex) */

            if ((nindex >= MMAX_NODE) || (NL[nindex].N == NULL)) continue;
    ...
    MDB(6, fS3) {
...
#ifdef __M32COMPAT
        MUNumListToString(N->ARes.PSlot, N->CRes.PSlot, NULL, tmpLine,
                          sizeof(tmpLine));
#else  /* __M32COMPAT */
        MUNumListToString(N->ARes.PSlot, N->CRes.PSlot, NULL, tmpLine,
                          sizeof(tmpLine), 0);
        ...
        MLog(
            "INFO:     MNode[%03d] '%18s' %9s VM: %8d Mem: %5d Dk: %5d Cl: %6s "
            "%s\n",
            N->Index, N->Name, MAList[meNodeState][N->State], N->CRes.Swap,
            N->CRes.Mem, N->CRes.Disk, tmpLine,
            MUMAToString(meNFeature, ',', N->FBM, sizeof(N->FBM)));

        MLog("INFO:     MNode[%03d] '%18s' C/A/D procs:  %d/%d/%d\n", N->Index,
             N->Name, N->CRes.Procs, N->ARes.Procs, N->DRes.Procs);
    }
...

{
    ...
    if (MReqCreate(J, NULL, &RQ, FALSE) == FAILURE) {
        MDB(1, fS3)
        MLog("ALERT:    cannot add requirements to job '%s'\n", J->Name);
    ...
    */
    ...
    /* NOTE:  if no taskgroups are defined, job level 'requested' attributes are
              assigned to req 0.  if one or more taskgroups are defined, job
       level
    ...
    for (cindex = 0; cindex < E->CCount; cindex++) {
        ...
        __MS3JobSetAttr(J, NULL, R->Version, CE->Name, CE->Val, MBNOTSET,
                        MBNOTSET, CE);
    } /* END for (cindex) */

    /* pass 3 - process all taskgroup level attributes and assign to specific
     * reqs */
    ...
    for (cindex = 0; cindex < E->CCount; cindex++) {
        ...
        __MS3JobSetAttr(J, J->Req[rqindex], R->Version, CE->Name, CE->Val,
                        MBNOTSET, MBNOTSET, CE);
    ...
    if (J->Request.TC == 0) {
        if (J->Request.NC == 0) {
            MDB(2, fS3)
            MLog(
                "ALERT:    no job task info located for job '%s' (assigning "
                "taskcount to 1)\n",
                J->Name);
        }
    ...
    if (J->Cred.U == NULL) {
        MDB(1, fS3)
        MLog("ERROR:    cannot authenticate job '%s' (User: %s  Group: %s)\n",
             J->Name, (J->Cred.U != NULL) ? J->Cred.U->Name : "NULL",
...

{
    ...
    if (J->State != OldState) {
        MDB(1, fS3)
        MLog("INFO:     job '%s' changed states from %s to %s\n", J->Name,
             MJobState[OldState], MJobState[J->State]);
    ...
    if (!MISSET(J->Flags, mjfNoRMStart)) {
        ...
        /* pass 3 - process all taskgroup level attributes and assign to
         * specific reqs */
    ...
    if ((J->State == mjsCompleted) || (J->State == mjsRemoved) ||
        ...
        if (J->AWallTime > 0)
            J->CompletionTime =
                MIN(J->CompletionTime, J->StartTime + J->AWallTime);
    }
...

{
    ...
    if ((aindex = MUGetIndex(AName, (const char **)MS3NodeAttr[R->Version],
                             FALSE, 0)) != 0) {
        if (AVal == NULL) {
            return (SUCCESS);
        }
        ...
        switch (aindex) {
            ...
            case mnaRAMem:
                ...
                if (AData != NULL) {
                    ...
                    if (MXMLGetAttr((mxml_t *)AData, "units", NULL, UData,
                                    sizeof(UData)) == SUCCESS) {
                        sprintf(tmpData, "%s%s", AVal, UData);
                ...
                if (tmpL == -1) tmpL = MURSpecToL(AVal, mvmMega, mvmMega);

            case mnaNodeID:
            case mnaLoad:

                if (strtol(AVal, NULL, 0) > 0)
                    MNodeSetAttr(N, (enum MNodeAttrEnum)aindex, (void **)AVal,
                                 mdfString, mSet);
            ...
            case mnaNodeState:
                ...
                if (!strcmp(R->APIVersion, MS3VName[msssV3_0]) ||
                    ...
                    MNodeSetAttr(N, mnaNodeState, (void **)&SIndex, mdfInt,
                                 mSet);
                } else {
                    const char *S3NState[] = {NONE,   "active", "busy",
                                              "down", "idle",   NULL};
                    const int S3NSEq[] = {mnsNone, mnsActive, mnsBusy,
                                          mnsDown, mnsIdle,   -1};
    ...
    }     /* END if (MUGetIndex(AName,(const char
             **)MS3NodeAttr[R->Version],FALSE,0) != 0) */
    else {
        int rindex;
        ...
        while (MXMLGetChildCI(AE, NULL, &CTok, &CE) == SUCCESS) {
            ...
            if ((rindex = MUGetIndex(CE->Name,
                                     (const char **)MS3NodeRAttr[R->Version],
                                     FALSE, 0)) == 0) {
                return (FAILURE);
            }
...

{
    ...
    if ((aindex = MUGetIndex(AName, (const char **)MS3QueueAttr, FALSE, 0)) !=
        0) {
        switch (aindex) {
            case mclaHostList:

            {
                ...
                if (C->NodeList == NULL) {
                    if ((C->NodeList = (void *)calloc(
                             1, sizeof(mnalloc_t) * MMAX_NODE)) == NULL) {
                        break;
                    }
                }
...
/* NOTE:  __MS3JobSetAttr() will process multiple taskgroups and create
   corresponding
          job reqs */
...

{
    ...
    if ((aindex = MUGetIndex(AName, (const char **)MS3JobAttr[VIndex], FALSE,
                             0)) != 0) {
        if (AVal == NULL) {
        ...
        switch (aindex) {
            ...
            case mjaUserPrio:
                ...
                MJobSetAttr(J, (enum MJobAttrEnum)aindex, (void **)AVal,
                            mdfString, mSet);
...

            {
                ...
                while (MXMLGetChildCI((mxml_t *)E, "Node", &CTok, &NE) ==
                       SUCCESS) {
                    if (tmpBuf[0] != '\0') MUSNCat(&BPtr, &BSpace, ",");
                ...
                if (tmpBuf[0] != '\0') {
                    if (IsUtilized == TRUE)
                        MJobSetAttr(J, mjaAllocNodeList, (void **)tmpBuf,
                                    mdfString, mSet);
                    else
                        MJobSetAttr(J, mjaHostList, (void **)tmpBuf, mdfString,
                                    mSet);
                }
            } /* END BLOCK */
            ...
            case mjaState:
                ...
                switch (VIndex) {
                    ...
                    case msssV4_0:
                        ...
                        J->State = (enum MJobStateEnum)MUGetIndex(
                            (char *)AVal, MS3JobState, FALSE, 0);
                    ...
                    default:
                        ...
                        if ((J->State = (enum MJobStateEnum)MUGetIndex(
                                 (char *)AVal, MJobState, FALSE, 0)) == 0) {
                            J->State = (enum MJobStateEnum)MUGetIndex(
                                (char *)AVal, MS3JobState, FALSE, 0);
                        }
    ...
    } else if ((aindex = MUGetIndex(AName, (const char **)MS3ReqAttr[VIndex],
                                    FALSE, 0)) != 0) {
        switch (aindex) {
            case mrqaNCReqMin:
            case mrqaTCReqMin:
            ...
            case mrqaReqNodeFeature:
                ...
                for (rqindex = 0; rqindex < MMAX_REQ_PER_JOB; rqindex++) {
                    if (J->Req[rqindex] == NULL) break;

                    if ((RQ != NULL) && (RQ != J->Req[rqindex])) continue;

                    MReqSetAttr(J, J->Req[rqindex], (enum MReqAttrEnum)aindex,
                                (void **)AVal, mdfString, mSet);
                } /* END for (rqindex) */
        ...
        while (MXMLGetChildCI(AE, NULL, &CTok, &CE) == SUCCESS) {
            ...
            if (MXMLGetAttr(CE, "units", NULL, UData, sizeof(UData)) ==
                SUCCESS) {
            ...
            __MS3JobSetAttr(J, RQ, (J->SRM != NULL) ? J->SRM->Version : 0,
                            CE->Name, CE->Val, IsUtilized, IsConsumable,
                            (void *)CE);
...

{
    ...
    if ((P == NULL) || (P->Type <= mpstNONE)) {
        MDB(2, fS3) MLog("ALERT:    invalid peer type in %s\n", FName);

        return (FAILURE);
    }

    if ((OBuf != NULL) && (*OBuf != NULL)) MUFree(OBuf);
    ...
    switch (P->Type) {
        ...
        default:
            ...
            if (EMsg != NULL) strcpy(EMsg, "invaild service requested");
    ...
    if ((P->Type != mpstEM) && (P->Type != mpstWWW)) {
        ...
        sprintf(tmpLine, "%s,%s;", MS3CName[mpstSC], MS3CName[P->Type]);

        MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC], "comcom", tmpLine);
    }
    ...
    if (S->sd <= 0) {
        ...
        MSUInitialize(S, ServerHost, ServerPort, 0, (1 << msftTCP));
        ...
        if (CSKey != NULL) strcpy(S->CSKey, CSKey);

        if (CSAlgo != mcsaNONE) S->CSAlgo = CSAlgo;

        if (MSUConnect(S, TRUE, NULL) == FAILURE) {
            char tmpMsg[MMAX_LINE];

            MDB(0, fS3)
            MLog("ERROR:    cannot connect to %s server '%s':%d\n",
                 MS3CName[P->Type], ServerHost, ServerPort);

            snprintf(tmpMsg, sizeof(tmpMsg),
                     "RMFAILURE:  cannot connect to %s server %s:%d (command: "
                     "'%s')\n",
                     MS3CName[P->Type], ServerHost, ServerPort,
            ...
            if (EMsg != NULL) strcpy(EMsg, tmpMsg);
    ...
    if (CmdString != NULL) {
        MS3EncapsulateMessage(CmdString, P->Version, TRUE, &S->SBuffer,
                              &S->SBufSize);
    }
    ...
    if (MSUSendData(S, Timeout * 1000000, FALSE, FALSE) == FAILURE) {
        ...
        MDB(0, fS3)
        MLog("ERROR:    cannot send data to %s server '%s':%d\n",
             MS3CName[P->Type], ServerHost, ServerPort);

        snprintf(tmpMsg, sizeof(tmpMsg),
                 "FAILURE:  cannot send data to %s server %s:%d (cmd: '%s')\n",
                 MS3CName[P->Type], ServerHost, ServerPort,
                 (CmdString != NULL) ? CmdString : "<XML>");
    ...
    if (MSURecvData(S, Timeout * 1000000, FALSE, &tSC, EMsg) == FAILURE) {
        ...
        MDB(0, fS3)
        MLog("ERROR:    cannot receive response from %s server '%s':%d\n",
             MS3CName[P->Type], ServerHost, ServerPort);

        snprintf(tmpMsg, sizeof(tmpMsg),
                 "FAILURE:  cannot receive response from %s server %s:%d (cmd: "
                 "'%s')\n",
                 MS3CName[P->Type], ServerHost, ServerPort,
                 (CmdString != NULL) ? CmdString : "<XML>");
        ...
        if (EMsg != NULL) {
            if (EMsg[0] == '\0') strcpy(EMsg, tmpMsg);
        }

        if (SC != NULL) *SC = (int)tSC;
    ...
    if ((P->Type != mpstEM) && (P->Type != mpstWWW)) {
        ...
        sprintf(tmpLine, "%s,%s;", MS3CName[P->Type], MS3CName[mpstSC]);
    ...
    if (OBuf != NULL) *OBuf = strdup(S->RBuffer);
...

{
    ...
    if (*tail == ':') /* time resource -> convert to seconds */
    {
        ...
        if (*tail2 == ':') val += strtol(tail2 + 1, &tail, 10); /* seconds */
...
int MS3QueueGetInfo(
    ...
    mnode_t *N, char CClass[][MMAX_NAME], char AClass[][MMAX_NAME])

{
    ...
    for (cindex = 0; CClass[cindex][0] != '\0'; cindex++) {
        MDB(8, fS3)
        MLog("INFO:     configured class '%s' specified for node %s\n",
        ...
        if ((CIndex = MUMAGetIndex(meClass, CClass[cindex], mAdd)) != FAILURE) {
            ...
            MDB(1, fS3)
            MLog("ALERT:    cannot add configured class '%s' to node %s\n",
                 CClass[cindex], N->Name);
        }
    } /* END for (cindex) */
    ...
    for (cindex = 0; AClass[cindex][0] != '\0'; cindex++) {
        MDB(8, fS3)
        MLog("INFO:     available class '%s' specified for node %s\n",
        ...
        if ((CIndex = MUMAGetIndex(meClass, AClass[cindex], mAdd)) != FAILURE) {
            if (N->CRes.PSlot[CIndex].count > N->ARes.PSlot[CIndex].count) {
                ...
                MDB(1, fS3)
                MLog("ALERT:    class '%s' available but not configured\n",
                     AClass[cindex]);
            }
        } else {
            MDB(1, fS3)
            MLog("ALERT:    cannot add available class '%s' to node %s\n",
                 AClass[cindex], N->Name);
        }
    } /* END for (cindex) */
    ...
    if (((N->State == mnsIdle) &&
         (MUNumListGetCount(MMAX_PRIO_VAL, N->CRes.PSlot, N->ARes.PSlot, 0,
                            NULL) == FAILURE)) ||
        (MUNumListGetCount(MMAX_PRIO_VAL, N->ARes.PSlot, N->CRes.PSlot, 0,
                           NULL) == FAILURE)) {
        char tmpLine[MMAX_LINE];
...
/* S3 corruption */
        ...
        MDB(1, fS3)
        MLog("ALERT:    %s node %s has class mismatch.  classes: %s\n",
             MAList[meNodeState][N->State], N->Name, tmpLine);

        if (MSched.Mode == msmNormal) {
            MOSSyslog(LOG_NOTICE,
                      "node %s in state %s has class mismatch.  classes: %s\n",
                      N->Name, MAList[meNodeState][N->State], tmpLine);
        }
    }
...

{
    ...
    switch (R->Version) {
        ...
        case msssV4_0:
            ...
            MXMLCreateE(&E, MSON[msonRequest]);
            MXMLSetAttr(E, MSAN[msanAction],
                        (void *)MS3JAName[R->Version][mrmJobRequeue],
                        mdfString);
            MS3SetObject(E, MS3ObjName[R->Version][mxoJob], NULL);
    ...
    if (MS3DoCommand(&R->P[0], CmdString, &Response, NULL, NULL, Msg) ==
        ...
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be requeued (command failed)\n",
             J->Name);
    ...
    if (((ptr = strstr(Response, "<Body")) == NULL) ||
        ...
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be requeued (invalid response)\n",
             J->Name);
    ...
    if (MXMLGetChildCI(E, MSON[msonResponse], NULL, &RE) == FAILURE) {
        ...
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be requeued (cannot parse response)\n",
             J->Name);
    ...
    if (MS3CheckStatus(RE, &tSC, EMsg) == FAILURE) {
        ...
        MDB(0, fS3)
        MLog(
            "ERROR:    job '%s' cannot be requeued (cannot parse command "
            "status)\n",
            J->Name);
    ...
    MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC], (char *)"jobrequeue",
                 (char *)J->Name);
...

{
    ...
    MDB(1, fS3)
    MLog("%s(%s,%s,%s,SC)\n", FName, (J != NULL) ? J->Name : "NULL",
         (R != NULL) ? R->Name : "NULL", (EMsg != NULL) ? EMsg : "NULL");
#endif /* !__MPROD */
    ...
    if (MJOBISACTIVE(J) == FALSE) {
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be suspended: (invalid state)\n",
             J->Name);
    ...
    switch (R->Version) {
        ...
        default:
            ...
            MXMLCreateE(&E, MSON[msonRequest]);
            MXMLSetAttr(E, MSAN[msanAction],
                        (void *)MS3JAName[R->Version][mrmJobSuspend],
                        mdfString);
    ...
    if (strcmp(CE->Val, "true")) {
        ...
        snprintf(Message, MMAX_LINE,
                 "cannot suspend job - RM failure, rc: %d, msg: '%s'", tmpSC,
                 tmpMsg);

        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be suspended: RM failure (%s)\n",
             J->Name, Message);
    ...
    MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC], (char *)"jobsuspend",
                 (char *)J->Name);
...

{
    ...
    if (MJOBISSUSPEND(J) == FALSE) {
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be resumed: (invalid state)\n",
             J->Name);
    ...
    switch (R->Version) {
        ...
        default:
            ...
            MXMLCreateE(&E, MSON[msonRequest]);
            MXMLSetAttr(E, MSAN[msanAction],
                        (void *)MS3JAName[R->Version][mrmJobResume], mdfString);
    ...
    if (MS3CheckStatus(RE, &tmpSC, tmpMsg) == FAILURE) {
        ...
        MDB(0, fS3)
        MLog(
            "ERROR:    job '%s' cannot be checkpointed (status failure - %s)\n",
            J->Name, (EMsg != NULL) ? EMsg : "N/A");
        ...
        snprintf(Message, MMAX_LINE,
                 "cannot resume job - RM failure, rc: %d, msg: '%s'", tmpSC,
                 tmpMsg);

        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be resumed: RM failure (%s)\n", J->Name,
             Message);
    ...
    MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC], (char *)"jobresume",
                 (char *)J->Name);
...

{
    ...
    switch (R->Version) {
        ...
        default:
            ...
            MXMLCreateE(&E, MSON[msonRequest]);
            MXMLSetAttr(E, MSAN[msanAction],
                        (void *)MS3JAName[R->Version][mrmJobCheckpoint],
                        mdfString);
    ...
    if (MS3DoCommand(&R->P[0], CmdString, &Response, NULL, NULL, EMsg) ==
        ...
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be checkpointed (command failed)\n",
             J->Name);
    ...
    if (((ptr = strstr(Response, "<Body")) == NULL) ||
        ...
        MDB(0, fS3)
        MLog("ERROR:    job '%s' cannot be checkpointed (invalid response)\n",
             J->Name);
    ...
    if (MXMLGetChildCI(E, MSON[msonResponse], NULL, &RE) == FAILURE) {
        ...
        MDB(0, fS3)
        MLog(
            "ERROR:    job '%s' cannot be checkpointed (cannot parse "
            "response)\n",
            J->Name);
    ...
    MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstSC], (char *)"jobcheckpoint",
                 (char *)J->Name);
...
int __MS3GetTaskList(
    ...
    char TaskString[], short TaskList[], int *TaskCount, int *TPN)

{
    ...
    while (ptr != NULL) {
        ...
        if ((tptr = strchr(tmpHostName, '/')) != NULL) *tptr = '\0';
        ...
        if ((tmpL != 0) && (*tail == '\0')) {
            ...
            if (MNodeFind(tmpHostName, &N) == FAILURE) {
                MDB(1, fS3)
                MLog("ALERT:    cannot locate host '%s' for job hostlist\n",
                     tmpHostName);
            } else {
    ...
    if (TaskCount != NULL) *TaskCount = tindex;
...

{
    ...
    MJobGetName(J, NULL, (J->SRM != NULL) ? J->SRM : &MRM[0], tmpJName,
                sizeof(tmpJName), mjnRMName);
    ...
    if (rc != 0) {
        ...
        MDB(0, fS3)
        MLog(
            "ERROR:    cannot set job '%s' attr '%s:%s' to '%s' (rc: %d "
            "'%s')\n",
            J->Name, Name, Resource, Value, rc, ErrMsg);
...

{
    ...
    if ((DataString == NULL) || (SBuf == NULL) || (SBufSize == NULL)) {
        return (FAILURE);
    }
...

{
    ...
    MDB(1, fS3)
    MLog("%s(%s,%s,%s,%ld,JobID,%s,SC)\n", FName,
         (Data != NULL) ? "Data" : "NULL", (R != NULL) ? R->Name : "NULL",
         (JP != NULL) ? "JP" : "NULL", FlagBM,
         (EMsg != NULL) ? "EMsg" : "NULL");
#endif /* !__MPROD */
    ...
    if (!MISSET(R->Flags, mrmfLocalQueue)) {
        ...
        if (EMsg != NULL) strcpy(EMsg, "invalid resource manager");
    ...
    if (!MISSET(FlagBM, mjsufTemplateJob)) {
        if (MXMLFromString(&JE, Data, FALSE, 0) == FAILURE) {
            ...
            if (EMsg != NULL) strcpy(EMsg, "cannot parse job");
    ...
    if (!MISSET(FlagBM, mjsufTemplateJob)) {
        if (MSched.Name[0] != '\0') {
            snprintf(SJobID, sizeof(SJobID), "%s.%d", MSched.Name,
                     R->JobCounter++);
        } else {
            snprintf(SJobID, sizeof(SJobID), "Moab.%d", R->JobCounter++);
        }
    } else {
    ...
    if (MJobCreate(SJobID, TRUE, &J) == FAILURE) {
        MDB(1, fS3)
        MLog("ERROR:    job buffer is full  (ignoring job '%s')\n", SJobID);

        if (EMsg != NULL) strcpy(EMsg, "job buffer is full");
    ...
    if (!MISSET(FlagBM, mjsufTemplateJob)) {
        if (MS3JobLoad(SJobID, JE, &J, TaskList, R) == FAILURE) {
            ...
            if (EMsg != NULL) strcpy(EMsg, "cannot valid job");
        ...
        if (MReqCreate(J, J->Req[0], NULL, FALSE) == FAILURE) {
            ...
            if (EMsg != NULL) strcpy(EMsg, "cannot add job requirements");
    ...
    if ((J->RMXString != NULL) &&
        ...
        if (MTransFind(TID, &TSpec1, &TSpec2, &TSpec3, &TSpec4, &TSpec5,
                       &TSpec6, &TSpec7, &TSpec8, &TSpec9) == SUCCESS) {
            if (TSpec1 != NULL) {
                int index;
            ...
            if (TSpec2 != NULL) {
                MJobSetAttr(J, mjaReqAWDuration, (void **)TSpec2, mdfString,
                            mSet);
            }

            if (TSpec3 != NULL) {
                MJobSetAttr(J, mjaReqSMinTime, (void **)TSpec3, mdfString,
                            mSet);
            }
        } /* END if (MTransFind()) */
    }     /* END if ((J->RMXString != NULL) && ... */

    if (JE != NULL) MXMLDestroyE(&JE);

    if (MRMJobPostLoad(J, TaskList, R, EMsg) == FAILURE) {
        MDB(1, fS3)
        MLog("INFO:     job '%s' is invalid/cannot be processed\n", J->Name);
...

{
    ...
    if (WE->Val != NULL) MUStrCpy(Value, WE->Val, VSize);
...

{
    ...
    if (SE->Val != NULL) MUStrCpy(Value, SE->Val, VSize);
...

{
    ...
    if (GE->Val != NULL) MUStrCpy(Name, GE->Val, NSize);
...

{
...

    {
        ...
        if (MXMLGetChildCI(E, (char *)MSON[msonObject], NULL, &OE) == SUCCESS) {
            if (OE->Val != NULL) {
                if (OName != NULL) MUStrCpy(OName, OE->Val, MMAX_NAME);
...

{
    ...
    if (R->S == NULL) R->S = (void *)calloc(1, sizeof(s3rm_t));
...

{
    ...
    if (JL[jindex] == (char *)1) JL[jindex] = NULL;
...

{
    ...
    */
    ...
    if ((CE->Val == NULL) || strcmp(CE->Val, "true")) {
        ...
        if ((EMsg != NULL) &&
            (MXMLGetChild(E, "Message", NULL, &CE) == SUCCESS)) {
            strcpy(EMsg, CE->Val);
        }
...

{
    ...
    if (Msg != NULL) {
        ...
        MXMLSetVal(CE, (void *)Msg, mdfString);
