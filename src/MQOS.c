...

{
    ...
    for (qindex = 0; qindex < MAX_MQOS; qindex++) {
        if (MQOS[qindex].Name[0] == '\0') {
            ...
            if (Q != NULL) *Q = &MQOS[qindex];

            break;
        }

        if (strcmp(MQOS[qindex].Name, QName) != 0) continue;
...

{
    ...
    if ((QName == NULL) || (QName[0] == '\0')) {
        ...
        while (MCfgGetSVal(MSched.ConfigBuffer, &ptr, MPARM_QOSCFG, IndexName,
        ...
        if (MCfgGetSVal(MSched.ConfigBuffer, NULL, MPARM_QOSCFG, QName, NULL,
...

{
    ...
    if ((Q == NULL) || (QName == NULL) || (QName[0] == '\0')) {
        return (FAILURE);
    }
    ...
    if (Q->L.OAP == NULL) Q->L.OAP = (mpu_t *)calloc(1, sizeof(mpu_t));

    if (Q->L.OIP == NULL) Q->L.OIP = (mpu_t *)calloc(1, sizeof(mpu_t));

    if (Q->L.OJP == NULL) Q->L.OJP = (mpu_t *)calloc(1, sizeof(mpu_t));
...

{
    ...
    DBG(5, fCONFIG)
    DPrint("%s(%s,%s)\n", FName, (Q != NULL) ? Q->Name : "NULL",
           (Value != NULL) ? Value : "NULL");

    if ((Q == NULL) || (Value == NULL) || (Value[0] == '\0')) {
        return (FAILURE);
    }
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MQOSAttr, &aindex, NULL, TRUE, NULL,
...

{
    ...
    if (QAL != NULL) MUBMCopy(QAL, tmpQAL, MAX_MQOS);

    /* determine allowed QOS default (precedence: U,G,A,C,S,0) */

    if (QDef != NULL) {
        if ((J->Cred.U->F.QDef != NULL) && (J->Cred.U->F.QDef != &MQOS[0]) &&
            ...
            *QDef = (mqos_t *)J->Cred.U->F.QDef;
                   ...
                   MUBMCheck(((mqos_t *)MSched.DefaultU->F.QDef)->Index,
                             tmpQAL)) {
            ...
            *QDef = (mqos_t *)J->Cred.G->F.QDef;
                   ...
                   MUBMCheck(((mqos_t *)MSched.DefaultG->F.QDef)->Index,
                             tmpQAL)) {
            *QDef = (mqos_t *)MSched.DefaultG->F.QDef;
        } else if ((J->Cred.A != NULL) && (J->Cred.A->F.QDef != NULL) &&
            ...
            *QDef = (mqos_t *)J->Cred.A->F.QDef;
                   ...
                   MUBMCheck(((mqos_t *)MSched.DefaultA->F.QDef)->Index,
                             tmpQAL)) {
            *QDef = (mqos_t *)MSched.DefaultA->F.QDef;
        } else if ((C != NULL) && (C->F.QDef != NULL) &&
                   (C->F.QDef != &MQOS[0]) &&
            ...
            *QDef = (mqos_t *)C->F.QDef;
                   ...
                   MUBMCheck(((mqos_t *)MSched.DefaultC->F.QDef)->Index,
                             tmpQAL)) {
            *QDef = (mqos_t *)MSched.DefaultC->F.QDef;
        } else if ((MPar[0].F.QDef != NULL) && (MPar[0].F.QDef != &MQOS[0])) {
...

        {
            ...
            if ((C != NULL) && (C->F.QDef != NULL)) CQD = (mqos_t *)C->F.QDef;

            DBG(3, fSTRUCT)
            DPrint(
                "INFO:     default QOS for job %s set to %s(%d) "
                "(P:%s,U:%s,G:%s,A:%s,C:%s)\n",
                J->Name, (*QDef)->Name, (*QDef)->Index,
                (MPar[0].F.QDef != NULL) ? ((mqos_t *)MPar[0].F.QDef)->Name
                                         : NONE,
                (J->Cred.U->F.QDef != NULL)
                    ? ((mqos_t *)J->Cred.U->F.QDef)->Name
                    : NONE,
                (J->Cred.G->F.QDef != NULL)
    ...
    if (QReq != NULL) {
        if (!MUBMCheck(QReq->Index, tmpQAL)) {
            DBG(2, fSTRUCT)
            DPrint("WARNING:  job %s cannot access requested QOS[%d] %s\n",
                   J->Name, QReq->Index, QReq->Name);
...
int MQOSListBMFromString(
    ...
    char *QOSString, int *BM, int Mode)
...

{
    ...
    for (bindex = 0; bindex < MAX_MQOS; bindex++) {
        ...
        if (Q->Name[0] != '\0') {
            sprintf(temp_str, "%s", Q->Name);
            strcat(tmpLine, temp_str);
        } else {
            sprintf(temp_str, "%d", Q->Index);
            strcat(tmpLine, temp_str);
        }
    } /* END for (bindex) */
...

{
    ...
    MUSNPrintF(&BPtr, &BSpace,
               "System QOS Settings:  QList: %s (Def: %s)  Flags: %ld\n\n",
               (QList[0] != '\0') ? QList : NONE,
               ((mqos_t *)MPar[0].F.QDef)->Name, MPar[0].F.JobFlags);

    MUSNPrintF(&BPtr, &BSpace, "%-20s%c %8s %8s %8s %8s %8s %10s %10s %s\n\n",
               "Name", '*', "Priority", "QTWeight", "QTTarget", "XFWeight",
               "XFTarget", "QFlags", "JobFlags", "Limits");

    for (qindex = 0; qindex < MAX_MQOS; qindex++) {
        Q = &MQOS[qindex];

        if ((Q->Name[0] == '\0') && (Q->Flags == 0) && (Q->F.Priority == 0) &&
            (Q->QTWeight == 0) && (Q->XFWeight == 0) && (Q->XFTarget == 0.0) &&
            (Q->QTTarget == 0)) {
            continue;
        }
        ...
        lptr = MCredShowAttrs(&Q->L.AP, Q->L.IP, Q->L.OAP, Q->L.OIP, Q->L.OJP,
                              &Q->F, 0, (1 << mcsLimits));

        MUBMToString(Q->F.JobFlags, MJobFlags, ':', JFlags, NONE);

        MUSNPrintF(&BPtr, &BSpace,
                   "%-20s%c %8ld %8ld %8ld %8ld %8.2lf %10s %10s %s\n",
                   (Q->Name[0] != '\0') ? Q->Name : NONE, QALChar,
                   Q->F.Priority, Q->QTWeight, Q->QTTarget, Q->XFWeight,
                   Q->XFTarget, (Line[0] != '\0') ? Line : NONE, JFlags,
        ...
        for (index = 0; index < MAX_MUSER + MAX_MHBUF; index++) {
            ...
            if (MUBMCheck(qindex, U->F.QAL)) {
                sprintf(temp_str, " %s%s", U->Name, MQALType[U->F.QALType]);
        ...
        for (index = 0; index < MAX_MGROUP + MAX_MHBUF; index++) {
            ...
            if (MUBMCheck(qindex, G->F.QAL)) {
                sprintf(temp_str, " %s%s", G->Name, MQALType[G->F.QALType]);
                strcat(Line, temp_str);
            }
        } /* END for (index) */
        ...
        for (index = 0; index < MAX_MACCT + MAX_MHBUF; index++) {
            ...
            if (MUBMCheck(qindex, A->F.QAL)) {
                sprintf(temp_str, " %s%s", A->Name, MQALType[A->F.QALType]);
                strcat(Line, temp_str);
            }
        } /* END for (index) */
        ...
        for (index = 0; index < MAX_MCLASS; index++) {
            ...
            if (MUBMCheck(qindex, C->F.QAL)) {
                sprintf(temp_str, " %s%s", C->Name, MQALType[C->F.QALType]);
                strcat(Line, temp_str);
            }
        } /* END for (index) */
...

{
    ...
    if ((Q->F.Priority != 0) ||
        (VFlag || (PIndex == -1) || (PIndex == pQOSPriority)))
        strcat(Buffer,
               MUShowIArray(MParam[pQOSPriority], Q->Index, Q->F.Priority));

    if ((Q->QTWeight != 0) ||
        (VFlag || (PIndex == -1) || (PIndex == pQOSQTWeight)))
        strcat(Buffer,
               MUShowIArray(MParam[pQOSQTWeight], Q->Index, Q->QTWeight));

    if ((Q->XFWeight != 0) ||
        (VFlag || (PIndex == -1) || (PIndex == pQOSXFWeight)))
        strcat(Buffer,
               MUShowIArray(MParam[pQOSXFWeight], Q->Index, Q->XFWeight));

    if ((Q->XFTarget != 0.0) ||
        (VFlag || (PIndex == -1) || (PIndex == pQOSTargetXF)))
        strcat(Buffer,
               MUShowFArray(MParam[pQOSTargetXF], Q->Index, Q->XFTarget));

    if ((Q->QTTarget != 0) ||
        (VFlag || (PIndex == -1) || (PIndex == pQOSTargetQT)))
        strcat(Buffer, MUShowSArray(MParam[pQOSTargetQT], Q->Index,
                                    MULToTString(Q->QTTarget)));
...

{
        ...
        if (strstr(Buf, MQOSFlags[vindex]) != NULL) {
            switch (vindex) {
...

                {
                    ...
                    if (((ptr = MUStrTok(tmpLine, ":", &TokPtr)) != NULL) &&
                        ...
                        for (rindex = 0; rindex < 12; rindex++) {
                            MUStrCpy(Q->ResName[rindex], ptr,
                                     sizeof(Q->ResName[rindex]));
                ...
                default:
                    ...
                    DBG(2, fCONFIG)
                    DPrint("ALERT:    unexpected QOS flag value detected: %d\n",
                           vindex);
    ...
    DBG(2, fCONFIG)
    DPrint("INFO:     QOS '%s' flags set to %lu\n", Q->Name, Q->Flags);
...

{
    ...
    if ((Q->Flags & QFALL) == QFALL) {
        ...
        if ((Q->Flags & QFUSER) == QFUSER) {
            if (Line[0] != '\0') strcat(Line, SepString);
...

    {
        ...
        int FList[] = {mqfpreemptor,  mqfpreemptee,   mqfdedicated,
                       mqfreserved,   mqfRunNow,      mqfnobf,
                       mqfPreemptSPV, mqfignHostList, -1};
    ...
    if (Q->Flags & (1 << mqfusereservation)) {
        ...
        if (Q->ResName[0][0] != '\0') {
            ...
            for (rindex = 0; rindex < 12; rindex++) {
                ...
                sprintf(temp_str, ":%s", Q->ResName[rindex]);
                strcat(Line, temp_str);
            } /* END for (rindex) */
        }
    }
...

{
    ...
    for (aindex = 0; AList[aindex] != -1; aindex++) {
        ...
        sprintf(temp_str, " %s=%s", MQOSAttr[AList[aindex]], tmpString);
