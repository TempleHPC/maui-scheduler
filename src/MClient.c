...

{
    ...
    while (MCfgGetSVal(MSched.PvtConfigBuffer, &ptr, MParam[pClientCfg], NULL,
                       NULL, tmpLine, sizeof(tmpLine), 0, NULL) == SUCCESS) {
...

{
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MClientAttr, &aindex, AttrArray, TRUE,
...

{
    ...
    sprintf(S.SBuffer, "%s%s %s%s %s%s\n", MCKeyword[mckCommand],
            MService[CIndex], MCKeyword[mckAuth], MUUIDToName(MOSGetEUID()),
            MCKeyword[mckArgs], CmdString);
