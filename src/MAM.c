...
const char *MAMCommands[] = {NONE, "make_withdrawal", NULL};

enum MAMCmdTypes { mamcNone = 0, mamcJobDebitAlloc };
...

{
    ...
    for (aindex = 0; aindex < MMAX_AM; aindex++) {
        ...
        if (A->Type == mamtNONE) continue;

        if ((AS != NULL) && (A != AS)) continue;
        ...
        switch (A->Type) {
...

            {
                ...
                rc = MAMQBDoCommand(A, 0, tmpSBuffer, NULL, &tmpSC, tmpRBuffer);
        ...
        if (MSysDSRegister(
                (char *)MS3CName[mpstAM], MRM[0].Name, A->Host, A->Port, NULL,
                (char *)MS3SockProtocol[A->SocketProtocol]) == FAILURE) {
            MDB(1, fRM)
            MLog("ALERT:    cannot register AM with directory service\n");
        }
        ...
        if (A->UseDirectoryService == TRUE) {
            ...
            if (MSysDSQuery((char *)MS3CName[mpstSC], MRM[0].Name, HostName,
...

{
    ...
    MDB(3, fAM)
    MLog("%s(A,%s,SC,ErrMsg)\n", FName, (J != NULL) ? J->Name : "NULL");

    if (SC != NULL) *SC = mhrNONE;
    ...
    if (((J->Cred.A == NULL) || (!strcmp(J->Cred.A->Name, NONE))) &&
        (MSched.DefaultAccountName[0] == '\0') && (A->Type != mamtQBANK) &&
        (A->Type != mamtGOLD) && (A->Type != mamtFILE)) {
        MDB(2, fAM)
        MLog("ALERT:    no account specified for job %s\n", J->Name);
    ...
    if ((J->Cred.Q != NULL) && (J->Cred.Q->Flags & (1 << mqfdedicated)) &&
        (J->NodeList != NULL)) {
        int nindex;
        ...
        case mamcpDebitSuccessfulCPU:
            ...
            if ((J->PSUtilized > 0.01)) {
                if (J->Req[0]->LURes.Procs > 0)
                    ProcConsumptionRate = (double)J->Req[0]->LURes.Procs /
                                          100.0 / Duration / ProcCount;
                else
                    ProcConsumptionRate =
                        (double)J->PSUtilized / Duration / ProcCount;
            } else {
                ProcConsumptionRate = 1.0;
            }
    ...
    if (MUNLGetMinAVal(J->NodeList, mnaNodeType, NULL, (void **)NodeType) ==
        FAILURE) {
    ...
    if ((J->Cred.Q != NULL) && strcmp(J->Cred.Q->Name, DEFAULT)) {
        sprintf(QOSString, "QOS=%s ", J->Cred.Q->Name);
    } else {
        QOSString[0] = '\0';
    }
    ...
    switch (A->Type) {
        case mamtFILE:
            ...
            fprintf(A->FP,
                    "%-15s TYPE=job MACHINE=%s ACCOUNT=%s SUBACCOUNT=%s "
                    "RESOURCE=%d PROCCRATE=%.2lf RESOURCETYPE=%s DURATION=%ld "
                    "%sREQUESTID=%s NODES=%d\n",
                    "WITHDRAWAL", R->Name, AccountName, J->Cred.U->Name,
                    ProcCount, ProcConsumptionRate, NodeType, Duration,
                    QOSString, J->Name, J->NodeCount);
        ...
        case mamtQBANK:
            ...
            /* FORMAT:  QOSFACTOR=<FACTORSPEC> NODEFACTOR=<FACTORSPEC>
             * TIMEFACTOR=<FACTORSPEC> */
            ...
            sprintf(AMISBuffer,
                    "COMMAND=make_withdrawal AUTH=%s MACHINE=%s%s%s USER=%s "
                    "WCTIME=%ld PROCCOUNT=%d PROCCRATE=%.2lf %sCLASS=%s "
                    "NODETYPE=%s JOBID=%s JOBTYPE=job NODES=%d",
                    MSched.Admin1User[0], R->Name,
                    (AccountName[0] != '\0') ? " ACCOUNT=" : "", AccountName,
                    J->Cred.U->Name, Duration, ProcCount, ProcConsumptionRate,
                    QOSString, (J->Cred.C != NULL) ? J->Cred.C->Name : DEFAULT,
                    NodeType, J->Name, J->NodeCount);

            MAMQBDoCommand(A, 0, AMISBuffer, NULL, &tmpSC, AMIRBuffer);
...

        {
            ...
            MXMLSetAttr(RE, (char *)MSAN[msanAction], (void *)"Charge",
                        mdfString);
            ...
            if (J->CompletionTime > 0) {
                ...
                MXMLCreateE(
                    &AE,
                    "EndTime"); /* change to (char
                                   *)MS3JobAttr[A->Version][mjaCompletion]); */
...

            {
                ...
                if (MAMGoldDoCommand(RE, &A->P, &RIndex, ErrMsg) == FAILURE) {
                    MDB(2, fAM)
                    MLog("ALERT:    cannot debit allocation for job\n");

                    if (SC != NULL) *SC = RIndex;
            ...
            MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstAM], "joballoccharge",
                         J->Name);
...

{
    ...
    MDB(3, fAM)
    MLog("%s(%s,RIndex,SC,ErrMsg)\n", FName, (R != NULL) ? R->Name : "NULL");

    if (RIndex != NULL) *RIndex = mhrNONE;
    ...
    if ((R->A == NULL) || !strcmp(R->A->Name, NONE)) {
        MDB(2, fAM)
        MLog("ALERT:    no account specified for res %s\n", R->Name);

        if (RIndex != NULL) *RIndex = mhrNoFunds;
    ...
    if ((ptr = strchr(R->A->Name, '@')) != NULL) {
        MUStrCpy(SAName, R->A->Name,
                 MIN((long)sizeof(SAName), (ptr - R->A->Name + 1)));
    ...
    switch (AM->Type) {
        case mamtFILE:
            ...
            fprintf(AM->FP,
                    "%-15s TYPE=res MACHINE=DEFAULT ACCOUNT=%s SUBACCOUNT=%s "
                    "RESOURCE=%d RESOURCETYPE=%s PROCCRATE=%.2lf DURATION=%ld "
                    "QOS=%s REQUESTID=%s NODES=%d\n",
                    "WITHDRAWAL", AName, SAName, R->AllocPC, NodeType, PCRate,
                    WCTime, DEFAULT, R->Name, R->NodeCount);
        ...
        case mamtQBANK:
            ...
            sprintf(AMISBuffer,
                    "COMMAND=make_withdrawal AUTH=%s MACHINE=%s ACCOUNT=%s "
                    "USER=%s WCTIME=%ld PROCCRATE=%.2lf PROCCOUNT=%d QOS=%s "
                    "CLASS=%s NODETYPE=%s JOBID=%s JOBTYPE=res NODES=%d",
                    MSched.Admin1User[0], MRM[0].Name, AName, SAName, WCTime,
                    PCRate, R->AllocPC, DEFAULT, "DEFAULT", NodeType, R->Name,
                    R->NodeCount);

            rc = MAMQBDoCommand(AM, 0, AMISBuffer, NULL, &StatusCode,
                                AMIRBuffer);

            if (rc == FAILURE) {
                MDB(1, fAM)
                MLog(
                    "ALERT:    cannot debit account %s for reservation '%s' "
                    "(AM failure)\n",
                    R->A->Name, R->Name);

                if (RIndex != NULL) *RIndex = mhrAMFailure;
...

        {
            ...
            MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstAM], "rsvalloccharge",
                         R->Name);
...

{
    ...
    if (((J->Cred.A == NULL) || (!strcmp(J->Cred.A->Name, NONE))) &&
        (AM->Type != mamtQBANK) && (AM->Type != mamtGOLD) &&
        (AM->Type != mamtFILE)) {
        MDB(2, fAM)
        MLog("ALERT:    no account specified for job %s\n", J->Name);
    ...
    if ((J->Cred.A != NULL) && strcmp(J->Cred.A->Name, NONE)) {
        ...
        if (AM->AppendMachineName == TRUE) {
            strcat(AccountName, "@");
            strcat(AccountName,
                   MRM[0].Name); /* FIXME:  assumes single RM per scheduler */
        }
    } else {
        AccountName[0] = '\0';
    }
    ...
    switch (AM->Type) {
        case mamtFILE:
            ...
            fprintf(AM->FP,
                    "%-15s TYPE=job ACCOUNT=%s SUBACCOUNT=%s RESOURCE=%d "
                    "RESOURCETYPE=%s DURATION=%ld %sREQUESTID=%s NODES=%d\n",
                    "RESERVE", AccountName, J->Cred.U->Name, ProcCount,
                    NodeType, WCLimit, QOSString, J->Name, J->NodeCount);
        ...
        case mamtQBANK:
            ...
            sprintf(AMISBuffer,
                    "COMMAND=make_reservation AUTH=%s MACHINE=%s%s%s USER=%s "
                    "WCLIMIT=%ld PROCCOUNT=%d %sCLASS=%s NODETYPE=%s TYPE=%s "
                    "JOBID=%s JOBTYPE=%s NODES=%d",
                    MSched.Admin1User[0], RM->Name,
                    (AccountName[0] != '\0') ? " ACCOUNT=" : "", AccountName,
                    J->Cred.U->Name, WCLimit, ProcCount, QOSString,
                    (J->Cred.C != NULL) ? J->Cred.C->Name : DEFAULT, NodeType,
                    AM->ClientName, J->Name,
                    (TestAlloc == TRUE) ? "tempjob" : "job", J->NodeCount);

            rc = MAMQBDoCommand(AM, 0, AMISBuffer, NULL, &tmpSC, AMIRBuffer);

            if (rc == FAILURE) {
                MDB(1, fAM)
                MLog(
                    "ALERT:    cannot create AM reservation for job '%s' (AM "
                    "failure)\n",
                    J->Name);

                if (RIndex != NULL) *RIndex = mhrAMFailure;

                if (ErrMsg != NULL) strcpy(ErrMsg, "AM transaction failed");
            ...
            if (tmpSC == FAILURE) {
                MDB(1, fAM)
                MLog(
                    "ALERT:    cannot create AM reservation for job '%s' "
                    "(request refused)\n",
                    J->Name);

                if (RIndex != NULL) *RIndex = mhrNoFunds;

                if (ErrMsg != NULL) strcpy(ErrMsg, AMIRBuffer);
...

        {
            ...
            */
            ...
            if (WCLimit > 0) {
                AE = NULL;
                MXMLCreateE(&AE,
                            (char *)MS3JobAttr[AM->Version][mjaAWDuration]);
            ...
            if (MAMGoldDoCommand(RE, &AM->P, RIndex, ErrMsg) == FAILURE) {
                MDB(2, fAM)
                MLog("ALERT:    cannot reserve allocation for job\n");
            ...
            MSysEMSubmit(&MSched.EM, (char *)MS3CName[mpstAM],
                         "joballocreserve", J->Name);
...

{
    ...
    if ((AMName == NULL) || (AMName[0] == '\0')) {
        ...
        while (MCfgGetSVal(head, &ptr, MAMCfgParam, IndexName, NULL, Value,
        ...
        while (MCfgGetSVal(head, &ptr, MAMCfgParam, AMName, NULL, Value,
                           sizeof(Value), 0, NULL) == SUCCESS) {
            if ((A == NULL) && (MAMFind(AMName, &A) == FAILURE)) {
...

{
    ...
    for (amindex = 0; amindex < MMAX_AM; amindex++) {
        ...
        if (A == NULL) {
            ...
            if (MAMCreate(AMName, &A) == FAILURE) {
                MDB(1, fALL)
                MLog("ERROR:    cannot alloc memory for AM '%s'\n", AMName);
        ...
        if (MSched.Mode != msmSim) MCPRestore(mcpAM, AMName, (void *)A);
    ...
    MDB(1, fSTRUCT)
    MLog("ALERT:    AM table overflow.  cannot add %s\n", AMName);
...

{
    ...
    for (amindex = 0; amindex < MMAX_AM; amindex++) {
        ...
        if (strcmp(A->Name, AMName) != 0) continue;
...

{
    ...
    switch (A->Type) {
        ...
        case mamtGOLD:
            ...
            if (A->WireProtocol == mwpNONE) A->WireProtocol = mwpXML;

            if (A->SocketProtocol == mspNONE) A->SocketProtocol = mspHalfSocket;

            if (A->Version == 0) A->Version = msssV3_0;

            break;

        case mamtQBANK:

            if (A->WireProtocol == mwpNONE) A->WireProtocol = mwpAVP;

            if (A->SocketProtocol == mspNONE)
                A->SocketProtocol = mspSingleUseTCP;

            if (A->CSAlgo == mcsaNONE) A->CSAlgo = mcsaHMAC;
...

{
    ...
    if ((A == NULL) || (Value == NULL) || (Value[0] == '\0')) {
        return (FAILURE);
    }
    ...
    while (ptr != NULL) {
        ...
        if (MUGetPair(ptr, (const char **)MAMAttr, &aindex, NULL, TRUE, NULL,
                      ValLine, MMAX_NAME) == FAILURE) {
        ...
        switch (aindex) {
            ...
            case mamaChargePolicy:
                ...
                A->ChargePolicy =
                    MUGetIndex(ValLine, (const char **)MAMChargePolicy, FALSE,
                               MDEF_AMCHARGEPOLICY);
            ...
            case mamaJFAction:
                ...
                A->JFAction =
                    MUGetIndex(ValLine, MJFActionType, TRUE, mamjfaNONE);

                break;

            case mamaFallbackAccount:

                MUStrCpy(A->FallbackAccount, ValLine,
                         sizeof(A->FallbackAccount));
...

            {
                ...
                if (MUURLParse(ValLine, tmpProtocol, tmpHost, NULL, 0, &A->Port,
                               FALSE) == FAILURE) {
                    /* cannot parse string */
            ...
            case mamaTimeout:
                ...
                if (A->Timeout > 1000) A->Timeout /= 1000;
            ...
            default:
                ...
                MDB(4, fAM)
                MLog("WARNING:  AM attribute '%s' not handled\n",
                     MAMAttr[aindex]);
...

{
    ...
    for (amindex = 0; MAM[amindex].Type != mamtNONE; amindex++) {
        ...
        MUSNPrintF(&BPtr, &BSpace, "AM[%s]  type: '%s'  state: '%s'\n", A->Name,
                   MAMType[A->Type], MAMState[A->State]);
        ...
        if (A->RespTotalCount[0] > 0) {
            MUSNPrintF(
                &BPtr, &BSpace,
                "  AM Performance:  Avg Time: %.2lfs  Max Time:  %.2lfs  (%d "
                "samples)\n",
                (double)A->RespTotalTime[0] / A->RespTotalCount[0] / 1000,
                (double)A->RespMaxTime[0] / 1000, A->RespTotalCount[0]);
        }
        ...
        for (index = 0; index < MMAX_RMFAILURE; index++) {
            ...
            if (A->FailTime[findex] <= 0) continue;
...

{
    ...
    for (amindex = 0; MAM[amindex].Type != mamtNONE; amindex++) {
        ...
        sprintf(tmpLine, "AMCFG[%s]", A->Name);
        ...
        for (aindex = 0; MAMAttr[aindex] != NULL; aindex++) {
            ...
            switch (aindex) {
                case mamaAppendMachineName:
                    ...
                    if (A->AppendMachineName == TRUE) strcpy(tmpVal, "TRUE");
                ...
                case mamaHost:
                    ...
                    if (A->Host[0] != '\0') strcpy(tmpVal, A->Host);
                ...
                case mamaPort:
                    ...
                    if (A->Port != 0) sprintf(tmpVal, "%d", A->Port);
                ...
                case mamaType:
                    ...
                    if (A->Type != mamtNONE) strcpy(tmpVal, MAMType[A->Type]);
            ...
            if (tmpVal[0] != '\0') {
                sprintf(&tmpLine[strlen(tmpLine)], " %s=%s", MAMAttr[aindex],
                        tmpVal);
            }
        } /* END for (aindex) */
...

{
        ...
        case mamtFILE:
            ...
            if ((FileName[0] != '/') && (FileName[0] != '~')) {
                if (MSched.HomeDir[strlen(MSched.HomeDir) - 1] == '/') {
                    sprintf(PathName, "%s%s", MSched.HomeDir, FileName);
                } else {
                    sprintf(PathName, "%s/%s", MSched.HomeDir, FileName);
                }
            } else {
            ...
            if ((A->FP = fopen(PathName, "a+")) == NULL) {
                MDB(0, fAM)
                MLog("WARNING:  cannot open AM file '%s', errno: %d (%s)\n",
            ...
            MDB(3, fAM)
            MLog("INFO:     AM type '%s' initialized (file '%s' opened)\n",
                 MAMType[mamtFILE], PathName);
    ...
    if (A->FallbackAccount[0] != '\0') {
        ...
        if (MAcctAdd(A->FallbackAccount, NULL) == FAILURE) {
            MDB(1, fAM)
            MLog("ALERT:    cannot add fallback account '%s'\n",
                 A->FallbackAccount);
...

{
    ...
    if (CacheLoaded == FALSE) {
        ...
        switch (A->Type) {
...

            {
                ...
                sprintf(
                    Line,
                    "COMMAND=get_users SHOW=username,defaultaccount AUTH=%s",
                    MSched.Admin1User[0]);

                if ((MAMQBDoCommand(A, 0, Line, NULL, &SC, Response) ==
                     SUCCESS) &&
                    (SC == SUCCESS)) {
                    /* FORMAT:  {<UNAME>|<ANAME>\n}... */
                    ...
                    while (ptr != NULL) {
                        ...
                        MUStrCpy(DACache[uindex].UName, ptr,
                                 sizeof(DACache[uindex].UName));

                        if ((ptr = MUStrTok(NULL, FS, &TokPtr2)) != NULL) {
                            MUStrCpy(DACache[uindex].AName, ptr,
                                     sizeof(DACache[uindex].AName));
                    ...
                    MDB(1, fAM)
                    MLog(
                        "ALERT:    cannot obtain default account list (query "
                        "failed)\n");
                }
            } /* END BLOCK */
...

            {
                ...
                MXMLSetAttr(RE, (char *)MSAN[msanAction], (void *)"Query",
                            mdfString);
                ...
                MDB(3, fAM)
                MLog("INFO:     account query response '%s'\n",
                ...
                /* FORMAT:
                 * <Data><User><Name>X</Name><DefaultProject>Y</DefaultProject></User></Data>
                 */
                ...
                while (MXMLGetChild(DE, "User", &CTok, &UE) == SUCCESS) {
                    ...
                    if ((NE->Val == NULL) || (NE->Val[0] == '\0')) continue;
                 ...
                 || !strcmp(NE->Val,"$SPECIFIED") */) {
                    ...
                    if (MXMLGetChild(UE, "DefaultProject", NULL, &AE) ==
                        FAILURE)
                        continue;

                    if ((AE->Val == NULL) || (AE->Val[0] == '\0')) continue;

                    /* populate cache */

                    MUStrCpy(DACache[uindex].UName, NE->Val,
                             sizeof(DACache[uindex].UName));

                    MUStrCpy(DACache[uindex].AName, AE->Val,
                             sizeof(DACache[uindex].AName));
    ...
    for (uindex = 0; uindex < MMAX_USER; uindex++) {
        if (DACache[uindex].UName[0] == '\0') break;

        if (strcmp(DACache[uindex].UName, UName)) continue;

        /* match located */

        if (AName != NULL) strcpy(AName, DACache[uindex].AName);

        MDB(3, fAM)
        MLog("INFO:     default account '%s' located for user '%s'\n", AName,
             UName);
    ...
    switch (A->Type) {
        ...
        case mamtQBANK:
            ...
            sprintf(Line,
                    "COMMAND=get_users USER=%s SHOW=defaultaccount AUTH=%s",
                    MSched.Admin1User[0], UName);

            rc = MAMQBDoCommand(A, 0, Line, NULL, &SC, Response);

            if (rc == FAILURE) {
                MDB(1, fAM)
                MLog(
                    "ALERT:    cannot determine default account for user '%s' "
                    "(AM failure)\n",
                    UName);
            ...
            if (AName != NULL) MUStrCpy(AName, Response, MMAX_NAME);
...

{
    ...
    MDB(3, fAM)
    MLog("%s(%s,%d,%s,E,SC,Response)\n", FName, (A != NULL) ? A->Name : "NULL",
         CmdIndex, Transaction);

    if (E != NULL) *E = NULL;

    if ((A == NULL) || ((A->Type != mamtQBANK) && (A->Type != mamtGOLD))) {
        return (FAILURE);
    }
    ...
    if (A->SocketProtocol != 0) S.SocketProtocol = A->SocketProtocol;

    if (A->WireProtocol != 0) S.WireProtocol = A->WireProtocol;
    ...
    switch (A->WireProtocol) {
        ...
        default:
            ...
            sprintf(tmpHeader, "COMMAND=%s AUTH=%s ", MSched.Admin1User[0],
                    MAMCommands[CmdIndex]);

            MSecGetChecksum(
                Transaction, strlen(Transaction), Checksum, NULL,
    ...
    switch (A->SocketProtocol) {
        ...
        case mspHalfSocket:
            ...
            if (MSUConnect(&S, FALSE, NULL) == FAILURE) {
                MDB(0, fAM)
                MLog("ERROR:    cannot connect to %s AM server '%s':%d\n",
                     MAMType[A->Type], A->Host, A->Port);

                sprintf(Message,
                        "AMFAILURE:  cannot connect to %s AM server %s:%d "
                        "(transaction: '%s')\n",
                        MAMType[A->Type], A->Host, A->Port, Transaction);
            ...
            if (MSUSendData(&S, A->Timeout * 1000000, FALSE, FALSE) ==
                FAILURE) {
                MDB(0, fAM)
                MLog("ERROR:    cannot send data to %s AM server '%s':%d\n",
                     MAMType[A->Type], A->Host, A->Port);

                sprintf(Message,
                        "AMFAILURE:  cannot send data to %s AM server %s:%d "
                        "(transaction: '%s')\n",
                        MAMType[A->Type], A->Host, A->Port, Transaction);
            ...
            if (MSURecvData(&S, A->Timeout * 1000000, FALSE, NULL, NULL) ==
                FAILURE) {
                MDB(0, fAM)
                MLog(
                    "ERROR:    cannot receive data from %s AM server '%s':%d\n",
                    MAMType[A->Type], A->Host, A->Port);

                sprintf(Message,
                        "AMFAILURE:  cannot receive data from %s AM server "
                        "%s:%d (transaction: '%s')\n",
                        MAMType[A->Type], A->Host, A->Port, Transaction);
            ...
            MDB(4, fAM)
            MLog("INFO:     received message '%s' from server\n", S.RBuffer);
            ...
            if ((ptr = strstr(S.RBuffer, "STATUSCODE=")) != NULL) {
                ...
                *SC = (int)strtol(ptr, NULL, 0);
                ...
                if (*SC == 0) {
                    MDB(2, fAM)
                    MLog(
                        "ALERT:    AM transaction failed.  transaction: '%s'  "
                        "response: '%s'\n",
                        Transaction, Response);

                    sprintf(Message,
                            "AMFAILURE:  transaction failed (transaction: '%s' "
                            ": response: '%s')\n",
                            Transaction, Response);
                ...
                MDB(2, fAM)
                MLog(
                    "ALERT:    cannot determine STATUSCODE for transaction "
                    "'%s'\n",
                    Transaction);

                sprintf(Message,
                        "AMFAILURE:  cannot parse reply from AM server %s:%d "
                        "(transaction: '%s')\n",
                        A->Host, A->Port, Transaction);
...
int MAMProcessOConfig(
    ...
    mam_t *AM, int PIndex, int IVal, double DVal, char *SVal, char **SArray)

{
    ...
    switch (PIndex) {
        ...
        case pAMChargePolicy:
            ...
            AM->ChargePolicy = MUGetIndex(SVal, (const char **)MAMChargePolicy,
                                          0, DEFAULT_MAMCHARGEPOLICY);
        ...
        case pAMProtocol:
            ...
            AM->WireProtocol =
                MUGetIndex(SVal, MAMProtocol, 0, DEFAULT_MAMWIREPROTOCOL);
        ...
        case pAMTimeout:
            ...
            if (AM->Timeout > 1000) AM->Timeout /= 1000;
...

{
    ...
    MDB(7, fSCHED)
    MLog("%s(%s,%d,Value,%d)\n", FName, (A != NULL) ? A->Name : "NULL", AIndex,
         Mode);
    ...
    switch (AIndex) {
        case mamaCSAlgo:
            ...
            A->CSAlgo =
                MUGetIndex((char *)Value, MCSAlgoType, FALSE, A->CSAlgo);
...

{
    ...
    switch (A->Type) {
        ...
        case mamtQBANK:
            ...
            sprintf(
                Line,
                "COMMAND=sync_reservations AUTH=%s MACHINE=%s TYPE=%s LIST=%s",
                MSched.Admin1User[0], R->Name, MSCHED_SNAME, JobList);

            rc = MAMQBDoCommand(A, 0, Line, NULL, &StatusCode, Response);

            if ((rc == FAILURE) || (StatusCode == FAILURE)) {
                MDB(1, fAM)
                MLog("ALERT:    cannot sync reservations with AM\n");
            ...
            MDB(3, fAM)
            MLog("INFO:     the following jobs require reservations: '%s'\n",
                 Response);
            ...
            while (ptr != NULL) {
                if (MJobFind(ptr, &J, 0) == SUCCESS) {
                    ...
                    MDB(1, fAM)
                    MLog(
                        "ALERT:    cannot locate job '%s' for sync'ing AM "
                        "alloc\n",
                        ptr);
                }
...

{
    ...
    if ((CAName[0] == '\0') || !strcmp(CAName, NONE)) {
        MDB(2, fAM)
        MLog("ALERT:    no account specified for request %s\n", ReqID);
    ...
    if ((MAM[0].Version > 290) && (Msg != NULL) && (Msg[0] != '\0')) {
        sprintf(tmpMsg, " REASON=\"%s\"", Msg);
    } else {
        tmpMsg[0] = '\0';
    }
    ...
    switch (MAM[0].Type) {
        ...
        case mamtQBANK:
            ...
            sprintf(MAMSBuf,
                    "COMMAND=remove_reservation AUTH=%s ACCOUNT=%s JOBID=%s%s",
                    MSched.Admin1User[0], AName, ReqID, tmpMsg);

            rc = MAMQBDoCommand(&MAM[0], 0, MAMSBuf, NULL, &SC, MAMRBuf);

            if (rc == FAILURE) {
                MDB(1, fAM)
                MLog(
                    "ALERT:    cannot cancel allocation reservation for "
                    "request '%s' (AM failure)\n",
                    ReqID);
...
int MAMAllocRReserve(
    ...
    mam_t *A, /* I */
    char *ResName, long StartTime, char *CAName, int ProcCount, int NodeCount,
    long Duration, char *QOSName, char *NodeType, /* I */
    enum MHoldReasonEnum *RIndex)                 /* O */

{
    ...
    MDB(3, fAM)
    MLog("%s(%s,%ld,%s,%d,%d,%ld,%s,%s,RIndex)\n", FName, ResName, StartTime,
         CAName, ProcCount, NodeCount, Duration, QOSName, NodeType);
    ...
    if ((CAName == NULL) || !strcmp(CAName, NONE)) {
        MDB(2, fAM)
        MLog("ALERT:    no account specified for reservation %s\n", ResName);
    ...
    if (A->AppendMachineName == TRUE) {
        ...
        MUStrCat(AName, MRM[0].Name,
                 sizeof(AName)); /* FIXME:  assumes single RM per scheduler */
    }
    ...
    switch (A->Type) {
        case mamtFILE:
            ...
            fprintf(A->FP,
                    "%-15s TYPE=res ACCOUNT=%s SUBACCOUNT=%s RESOURCE=%d "
                    "RESOURCETYPE=%s DURATION=%ld QOS=%s REQUESTID=%s "
                    "NODES=%d\n",
                    "RESERVE", AName, SAName, ProcCount, NodeType, Duration,
                    DEFAULT, ResName, NodeCount);
        ...
        case mamtQBANK:
            ...
            sprintf(Line,
                    "COMMAND=make_reservation AUTH=%s MACHINE=%s ACCOUNT=%s "
                    "USER=%s WCLIMIT=%ld PROCCOUNT=%d QOS=%s CLASS=%s "
                    "NODETYPE=%s TYPE=%s JOBID=%s NODES=%d",
                    MSched.Admin1User[0], MRM[0].Name, AName, SAName, Duration,
                    ProcCount, QOSName, DEFAULT, NodeType, MSCHED_SNAME,
                    ResName, NodeCount);
            ...
            if (rc == FAILURE) {
                MDB(1, fAM)
                MLog(
                    "ALERT:    cannot create AM reservation for reservation "
                    "'%s' (AM failure)\n",
                    ResName);
...

{
    ...
    if (MSysDSUnregister((char *)MS3CName[mpstAM], MRM[0].Name,
                         MSched.ServerHost, MSched.ServerPort, NULL,
                         NULL) == FAILURE) {
        MDB(1, fRM)
        MLog("ALERT:    cannot unregister with directory service\n");
    }
