...
int MPBSNodeUpdate(mnode_t *, struct batch_status *, enum MNodeStateEnum,
                   mrm_t *);
int __MPBSJobGetState(struct batch_status *, mrm_t *, char *,
                      enum MJobStateEnum *, mbool_t *);
...

{
    ...
    if (InitialAttempt == TRUE) {
        ...
        if (R->P[0].Timeout != 0) {
            ...
            sprintf(tmpLine, "%ld", R->P[0].Timeout);
    ...
    if ((sd = pbs_connect(PBSServer)) < 0) {
        DBG(1, fPBS)
        DPrint(
            "ERROR:    cannot connect to PBS server '%s'  rc: %d (errno: %d)\n",
            PBSServer, sd, pbs_errno);
    ...
    if (R->U.PBS.SubmitExe[0] == '\0') {
        strcpy(R->U.PBS.SubmitExe, DEFAULT_PBSQSUBPATHNAME);
    }
    ...
    if (R->EPort > 0) {
        ...
        PBSEPort = get_svrport(PBS_SCHEDULER_SERVICE_NAME, "tcp",
                               PBS_SCHEDULER_SERVICE_PORT);

        R->EPort = PBSEPort;
    }

    MSUInitialize(&tmpS, NULL, PBSEPort, MSched.ClientTimeout, (1 << TCP));

    if (MSUListen(&tmpS) == FAILURE) {
        DBG(1, fPBS)
        DPrint("WARNING:  cannot connect to PBS scheduler port %d\n", PBSEPort);
    ...
    if (X.XPBSInitialize != (int (*)())0) (*X.XPBSInitialize)(X.xd, R);

    DBG(1, fPBS)
    DPrint("INFO:     connected to PBS server %s:%d on sd %d\n",
           (R->P[0].HostName != NULL) ? R->P[0].HostName : "", R->P[0].Port,
           R->U.PBS.ServerSD);
...

{
    ...
    while (EventsOutstanding == TRUE) {
        ...
        if (select(FD_SETSIZE, &fdset, NULL, NULL, &timeout) == -1) {
            DBG(1, fPBS)
            DPrint("ALERT:    select failed checking PBS sched socket\n");
        ...
        if ((rpp_fd != -1) && !FD_ISSET(rpp_fd, &fdset)) {
            ...
            DBG(9, fPBS)
            DPrint("ALERT:    no PBS RPP sched socket connections ready\n");
            ...
            DBG(9, fPBS)
            DPrint("INFO:     no PBS sched socket connections ready\n");
        ...
        if (MSUAcceptClient(&S, &C, NULL, (1 << TCP)) == FAILURE) {
            ...
            DBG(1, fPBS)
            DPrint("ALERT:    cannot accept client on PBS sched socket\n");
        ...
        if (MSURecvPacket(C.sd, (char **)&Iptr, sizeof(PBSCmd), NULL, 100000,
                          NULL) == FAILURE) {
...

{
    ...
    if ((MSched.Time > (R->U.PBS.ServerSDTimeStamp + 3000)) ||
        ...
        if (R->U.PBS.ServerSD <= 0) {
            ...
            DBG(1, fPBS)
            DPrint("ALERT:    cannot re-initialize PBS interface\n");
    ...
    if (JCount != NULL) *JCount = 0;

    if ((MSim.RMFailureTime >= MSched.Time) ||
        (jobs = pbs_statjob(R->U.PBS.ServerSD, NULL, NULL,
                            "exec_queue_only")) == NULL) {
        if (MSim.RMFailureTime < MSched.Time)
        ...
        if (pbs_errno == 0) {
            ...
            DBG(0, fPBS)
            DPrint("ALERT:    queue is empty or cannot get PBS job info: %s\n",
 ...
 */
            ...
            if (__MPBSJobChkExecutable(cur_job) == FAILURE) {
                DBG(3, fPBS)
                DPrint("INFO:     ignoring job %s in routing queue\n",
                       cur_job->name);
            ...
            if (JCount != NULL) (*JCount)++;

            RMJID[0] = '\0';

            if (__MPBSJobGetState(cur_job, R, RMJID, &Status, &IsExiting) ==
                FAILURE)
                break;
            ...
            switch (Status) {
                ...
                case mjsHold:
                    ...
                    if (MJobFind(SJID, &J, 0) == SUCCESS) {
                        if (IsExiting == TRUE) J->Flags |= (1 << mjfIsExiting);
                        ...
                        MJobSetAttr(J, mjaRMJID, (void **)RMJID, mdfString,
                                    mSet);

                        if (MPBSJobLoad(SJID, cur_job, J, TaskList, R->Index) ==
                            FAILURE) {
                            DBG(1, fPBS)
                            DPrint("ALERT:    cannot load PBS job '%s'\n",
                                   SJID);
                        ...
                        DBG(1, fPBS)
                        DPrint(
                            "ERROR:    job buffer is full  (ignoring job "
                            "'%s')\n",
                            RMJID);
                    }
                ...
                case mjsVacated:
                    ...
                    if (MJobFind(SJID, &J, 0) == SUCCESS) {
                        ...
                        DBG(4, fPBS)
                        DPrint("INFO:     ignoring job '%s'  (state: %s)\n",
                               RMJID, MJobState[Status]);
                    }
        ...
        for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]); J = JNext) {
            ...
            if ((J->ATime > 0) &&
                ...
                if ((J->State == mjsStarting) || (J->State == mjsRunning)) {
                    DBG(1, fPBS)
                    DPrint(
                        "INFO:     active PBS job %s has been removed from the "
                    ...
                    DBG(1, fPBS)
                    DPrint(
                        "INFO:     non-active PBS job %s has been removed from "
...

{
    ...
    *Status = mjsNONE;
    ...
    if (IsExiting != NULL) *IsExiting = FALSE;
        ...
        if (!strcmp(AP->name, ATTR_state)) {
            switch (AP->value[0]) {
                ...
                case 'E': /* differences between 'exiting' and 'completed?' */
                    ...
                    if (IsExiting != NULL) *IsExiting = TRUE;
                ...
                default:
                    ...
                    DBG(1, fPBS)
                    DPrint(
                        "ALERT:    unexpected job state '%s' detected for job "
                        "'%s'\n",
                        AP->value, PJob->name);
...

{
    ...
    if ((MSim.RMFailureTime >= MSched.Time) ||
        ...
        DBG(0, fPBS)
        DPrint("ERROR:    cannot get server info: %s\n",
    ...
    for (SP = Server; SP != NULL; SP = SP->next) {
        for (AP = SP->attribs; AP != NULL; AP = AP->next) {
            DBG(6, fPBS)
            DPrint(
                "INFO:     PBS system attribute '%s'  value: '%s'  (r: %s)\n",
                AP->name, (AP->value != NULL) ? AP->value : "NULL",
                (AP->resource != NULL) ? AP->resource : "NULL");

            if (!strcmp(AP->name, "resources_max")) {
                if (!strcmp(AP->resource, "ncpus")) {
                    MSched.DefaultN.CRes.Procs =
                        (int)strtol(AP->value, NULL, 10);
                ...
                if (MSched.DefaultC != NULL) {
                    MUStrCpy(MSched.DefaultC->Name, AP->value,
                ...
                /* FORMAT:  <PRODNAME>_X.Y.Z.<BUILDNUMBER> where <PRODNAME> is {
                 * PBSPro | TORQUE } */
                ...
                if (R->Version <= 0) {
                    ...
                    if (ptr != NULL) Version += (int)strtol(ptr, NULL, 10) * 10;

                    ptr = MUStrTok(NULL, ".", &TokPtr);

                    if (ptr != NULL) Version += (int)strtol(ptr, NULL, 10);
...

{
    ...
    if ((MSim.RMFailureTime >= MSched.Time) ||
        ...
        DBG(0, fPBS)
        DPrint("ERROR:    cannot get node info: %s\n",
               (ErrMsg != NULL) ? ErrMsg : "NULL");
    ...
    for (cur_node = nodes; cur_node != NULL; cur_node = cur_node->next) {
        ...
        if (__MPBSGetNodeState(Name, &NewState, cur_node) == FAILURE) {
            DBG(2, fPBS)
            DPrint("ALERT:    cannot get PBS node state for node %s\n",
                   cur_node->name);
        ...
        if (MNodeFind(Name, &N) == SUCCESS) {
            ...
            DBG(1, fPBS)
            DPrint("ERROR:    node buffer is full  (ignoring node '%s')\n",
                   Name);
        }
    } /* END for (cur_node) */
...
int MPBSQueueQuery(
    ...
    mrm_t *R, int *QCount, char *EMsg, int *SC)

{
    int rc;

    rc = MPBSLoadQueueInfo(R, NULL, (MSched.Iteration <= 1) ? TRUE : FALSE, SC);
...

{
    ...
    if (UpdateIteration != MSched.Iteration) {
        ...
        if ((MSim.RMFailureTime >= MSched.Time) ||
            ...
            DBG(0, fPBS)
            DPrint("ERROR:    cannot get queue info: %s\n",
                   (ErrMsg != NULL) ? ErrMsg : "NULL");
    ...
    for (QP = QL; QP != NULL; QP = QP->next) {
        ...
        if (MClassAdd(QP->name, &C) == FAILURE) continue;
        ...
        for (AP = QP->attribs; AP != NULL; AP = AP->next) {
            /* load limited set of queue attributes (priority, state, limits,
             * ACLs) */
            ...
            if (!strcmp(AP->name, ATTR_p)) {
                ...
                if (C->F.IsLocalPriority == FALSE) {
                    ...
                    DBG(3, fPBS)
                    DPrint("INFO:     queue '%s' priority set to %ld\n",
                           QP->name, C->F.Priority);
                }
            ...
            if (!strcmp(AP->name, ATTR_start)) {
                ...
                DBG(3, fPBS)
                DPrint("INFO:     queue '%s' started state set to %s\n",
                       QP->name, AP->value);
            ...
            if (!strcmp(AP->name, "max_running")) {
                ...
                MCredSetAttr((void *)C, mxoClass, mcaMaxJob, (void **)AP->value,
                             mdfString, mSet);

                DBG(3, fPBS)
                DPrint("INFO:     queue '%s' maxrunning set to %s\n", QP->name,
                       AP->value);
            ...
            if (!strcmp(AP->name, "max_user_run")) {
                ...
                MCredSetAttr((void *)C, mxoClass, mcaMaxJobPerUser,
                             (void **)AP->value, mdfString, 0);

                DBG(3, fPBS)
                DPrint("INFO:     queue '%s' maxuserrun set to %s\n", QP->name,
                       AP->value);
            ...
            if (!strcmp(AP->name, ATTR_aclhten)) {
                        ...
                        if (MNodeFind(ptr, &N) == FAILURE) {
                            ...
                            if (N->CRes.PSlot[C->Index].count == 0) {
                                ...
                                if ((N->MaxProcPerClass != NULL) &&
                                    ...
                                    CCount = MIN(CCount,
                                                 N->MaxProcPerClass[C->Index]);
                                } else if ((MSched.DefaultN.MaxProcPerClass !=
                                            NULL) &&
                                           (MSched.DefaultN
                                                .MaxProcPerClass[C->Index] >
                                            0)) {
                                    CCount = MIN(
                                        CCount, MSched.DefaultN
                                                    .MaxProcPerClass[C->Index]);
                                }
                ...
                if (!strcmp(AP->resource, "neednodes") ||
                    ...
                    if (*ptr != '\0') {
                        ...
                        MClassSetAttr(C, mclaDefReqFeature, (void **)AP->value,
                                      mdfString, mSet);
                    } /* END if (*ptr != '\0') */
                }
                ...
                if (!strcmp(AP->resource, "nodect")) {
                    ...
                    MCredSetAttr((void *)C, mxoClass, mcaMaxNodePerJob,
                                 (void **)AP->value, mdfString, mSet);
                } /* END if (!strcmp(AP->resource,"nodect")) */
                else if (!strcmp(AP->resource, "ncpus")) {
                    /* NOTE:  PBS 'ncpus' constrains job procs */

                    MCredSetAttr((void *)C, mxoClass, mcaMaxProcPerJob,
                                 (void **)AP->value, mdfString, mSet);
        ...
        if ((HostListDetected == TRUE) && (ACLHostEnabled == FALSE)) {
            DBG(3, fPBS)
            DPrint("INFO:     class to node mapping enabled for queue '%s'\n",
                   QP->name);
        ...
        if (IsGlobal == TRUE) {
            DBG(3, fPBS)
            DPrint(
                "INFO:     class to node not mapping enabled for queue '%s' "
            ...
            for (nindex = 0; nindex < MAX_MNODE; nindex++) {
                ...
                if ((SpecN != NULL) && (SpecN != N)) continue;

                if (N->PrivateQueue == TRUE) continue;
                ...
                if (N->CRes.PSlot[C->Index].count == 0) {
                    ...
                    if ((N->MaxProcPerClass != NULL) &&
                        ...
                        CCount = MIN(CCount, N->MaxProcPerClass[C->Index]);
                               ...
                               (MSched.DefaultN.MaxProcPerClass[C->Index] >
                                0)) {
                        CCount = MIN(CCount,
                                     MSched.DefaultN.MaxProcPerClass[C->Index]);
                    }
...
int __MPBSGetNodeState(
    ...
    char *Name, enum MNodeStateEnum *State, struct batch_status *PNode)
    ...
    *State = mnsNone;
    ...
    for (AP = PNode->attribs; AP != NULL; AP = AP->next) {
        if (!strcmp(AP->name, ATTR_NODE_state)) {
            if (strstr(AP->value, ND_down) ||
                strstr(AP->value, ND_state_unknown)) {
                *State = mnsDown;
                ...
                *State = mnsActive;
                ...
                DBG(1, fPBS)
                DPrint(
                    "ALERT:    cannot map PBS state '%s' to scheduler state on "
                    "node '%s'\n",
                    AP->value, Name);
            }
    ...
    if (AP == NULL) {
        DBG(1, fPBS)
        DPrint("ALERT:    PBS node state not specified on node '%s'\n", Name);
    } else {
        DBG(1, fPBS)
        DPrint("INFO:     PBS node %s set to state %s (%s)\n", Name,
               MNodeState[*State], AP->value);
    }
...

{
    ...
    if (HostList[0] == '\0') {
        DBG(0, fPBS)
        DPrint("ERROR:    job '%s' cannot be started: (empty hostlist)\n",
               J->Name);

        if (Msg != NULL) strcpy(Msg, "job cannot be started - empty hostlist");
    ...
    if (rc != 0) {
        ...
        sprintf(Message, "cannot start job - RM failure, rc: %d, msg: '%s'", rc,
                (ErrMsg != NULL) ? ErrMsg : "");
        ...
        DBG(0, fPBS)
        DPrint(
            "ERROR:    job '%s' cannot be started: (rc: %d  errmsg: '%s'  "
            "hostlist: '%s')\n",
            J->Name, rc, ErrMsg, HostList);
...

{
    ...
    rc = pbs_sigjob(R->U.PBS.ServerSD, tmpJobName,
    ...
    if (rc != 0) {
        ...
        DBG(0, fPBS)
        DPrint("ERROR:    job '%s' cannot be suspended: %s\n", J->Name, ErrMsg);

        if (Msg != NULL) {
            sprintf(Msg, "RM Failure %d: '%s'", rc, ErrMsg);
        }
...

{
    ...
    rc = pbs_sigjob(R->U.PBS.ServerSD, tmpJobName, "resume", NULL);

    if (rc != 0) {
        ErrMsg = pbs_geterrmsg(R->U.PBS.ServerSD);

        DBG(0, fPBS)
        DPrint("ERROR:    job '%s' cannot be resumed: %s\n", J->Name, ErrMsg);
...

{
    ...
    if (rc != 0) {
        ...
        DBG(0, fPBS)
        DPrint("ERROR:    job '%s' cannot be cancelled: %s\n", J->Name, ErrMsg);
...

{
    ...
    DPrint("%s(%s,%s,%s,%s)\n", FName, (N != NULL) ? N->Name : "NULL",
           (PNode != NULL) ? PNode->name : "NULL", MNodeState[Status],
           (R != NULL) ? R->Name : "NULL");
    ...
    if (R->U.PBS.SSSIsEnabled == TRUE) {
        ...
        for (AP = PNode->attribs; AP != NULL; AP = AP->next) {
            ...
            if (strcmp(AP->name, "status")) continue;
        ...
        /* get CRes.Procs, CRes.Mem, ARes.Disk, Load, Arch, CRes.Swap, and
         * ARes.Swap from MOM */
    ...
    if (N->ActiveOS == 0) N->ActiveOS = MUMAGetIndex(eOpsys, "DEFAULT", mAdd);

    if (N->Network == 0) N->Network = MUMAGetBM(eNetwork, "DEFAULT", mAdd);
    ...
    for (AP = PNode->attribs; AP != NULL; AP = AP->next) {
        if (!strcmp(AP->name, ATTR_NODE_jobs)) {
            ...
            DBG(3, fPBS)
            DPrint("INFO:     node %s has joblist '%s'\n", N->Name, tmpBuffer);
            ...
            while (ptr != NULL) {
                ...
                if (MJobFind(JobID, &J, 0) == SUCCESS) {
                    ...
                    if (J->NodeList != NULL) {
                        for (nindex = 0; nindex < MAX_MNODE_PER_JOB; nindex++) {
                            if (J->NodeList[nindex].N == N) break;
                    ...
                    DBG(2, fPBS)
                    DPrint(
                        "INFO:     cannot locate PBS job '%s' (running on node "
                        "%s)\n",
                        JobID, N->Name);
                ...
                if ((N->State == mnsIdle) || (N->State == mnsActive)) {
                    ...
                    if (N->State != OldState) {
                        DBG(3, fPBS)
                        DPrint(
                            "INFO:     adjusted node %s from state %s to %s "
                            "(DRes.Procs: %d  CRes.Procs: %d)\n",
                            N->Name, MNodeState[OldState], MNodeState[N->State],
                            N->DRes.Procs, N->CRes.Procs);
            ...
            if (!strcmp(AP->value, ND_cluster)) {
                /* NOTE:  FIXME:  cluster vs time_shared must be handled, but
                 * not this way */
                /*
            ...
            if (MSched.NodeCPUOverCommitFactor > 0.0)
                N->CRes.Procs =
                    (int)(N->CRes.Procs * MSched.NodeCPUOverCommitFactor);
    ...
    for (cindex = 0; cindex < MAX_MCLASS; cindex++) {
        if ((N->State != mnsIdle) && (N->State != mnsActive)) {
            ...
            N->ARes.PSlot[cindex].count =
                MIN(N->ARes.PSlot[cindex].count, N->ARes.Procs);
        }
    } /* END for (cindex) */
    ...
    if (N->State == mnsNone) {
        DBG(3, fPBS)
        DPrint("WARNING:  node '%s' is unusable in state 'NONE'\n", N->Name);
    }

    if ((N->ARes.Disk < 0) && (N->State == mnsIdle)) {
        DBG(2, fPBS)
        DPrint(
            "WARNING:  idle node %s is unusable  (inadequate disk space in "
            "/var)\n",
            N->Name);
    }
    ...
    DPrint(
        ...
        "%s\n",
        N->Index, N->Name, MAList[eNodeState][N->State], N->CRes.Swap,
        N->CRes.Mem, N->CRes.Disk,
        MUCAListToString(N->ARes.PSlot, N->CRes.PSlot, NULL),
        MUMAList(eFeature, N->FBM, sizeof(N->FBM)));

    DBG(6, fPBS)
    DPrint("INFO:     MNode[%03d] '%18s' C/A/D procs:  %d/%d/%d\n", N->Index,
...

{
    ...
    char *PBSResList[] = {"ncpus",   /* number of CPUS */
                          "arch",    /* the architecture of the machine */
                          "physmem", /* the amount of physical memory */
                          "loadave", /* the current load average */
                          "availmem", "totmem", NULL};

    char *PBS5ResList[] = {"loadave", /* the current load average */
                           NULL};
    ...
    if (MSched.Iteration > 0) {
        ...
        if ((N->IterationCount < MSched.NodePollFrequency) &&
            (N->CTime < MSched.Time) && (N->StateMTime < MSched.Time)) {
            DBG(6, fPBS)
            DPrint(
                "INFO:     MOM on host '%s' not updated this iteration (%d < "
                "%d)\n",
                N->Name, N->IterationCount, MSched.NodePollFrequency);
    ...
    if ((sd = openrm(N->Name, N->RM->NMPort)) < 0) {
        DBG(2, fPBS)
        DPrint("ALERT:    cannot connect to MOM on node '%s', rc: %d\n",
               N->Name, sd);
    ...
    for (rindex = 0; RL[rindex] != NULL; rindex++) {
        if (addreq(sd, RL[rindex]) != 0) {
            DBG(2, fPBS)
            DPrint(
                "ALERT:    cannot add req '%s' to MOM on node '%s' (errno: "
                "%d:%d)\n",
                RL[rindex], N->Name, errno, pbs_errno);
    ...
    if (N->RM->U.PBS.LocalDiskFS[0] != '\0') {
        sprintf(DiskLine, "size[fs=%s]", N->RM->U.PBS.LocalDiskFS);

        if (addreq(sd, DiskLine) != 0) {
            DBG(2, fPBS)
            DPrint(
                "ALERT:    cannot add req to MOM on node '%s' (errno: %d:%d)\n",
                N->Name, errno, pbs_errno);
    ...
    for (rindex = 0; rindex < RCount; rindex++) {
        if ((Value = (char *)getreq(sd)) == NULL) {
            DBG(2, fPBS)
            DPrint(
                "ALERT:    cannot get req from MOM on node '%s' (errno: "
                "%d:%d)\n",
                N->Name, errno, pbs_errno);
    ...
    if (TotMem > 0) N->CRes.Swap = TotMem;
...

{
    ...
    DBG(2, fPBS)
    DPrint("%s(%s,%s,%s,%s)\n", FName, (N != NULL) ? N->Name : "NULL",
           (PNode != NULL) ? PNode->name : "NULL", MNodeState[NState],
           (R != NULL) ? R->Name : "NULL");
    ...
    if ((R->U.PBS.SSSIsEnabled != TRUE) && (N->Load <= 0.0)) {
        if ((NState != mnsDown) && (NState != mnsDrained)) {
            if (MPBSQueryMOM(N, R, NULL, NULL) == FAILURE) {
                MUStrDup(&N->Message,
                         "unable to obtain node information from MOM");
    ...
    for (AP = PNode->attribs; AP != NULL; AP = AP->next) {
        if (!strcmp(AP->name, ATTR_NODE_jobs)) {
            ...
            /* NOTE: jobs listed in
             * '<JOBID>[/<TASKID>][,<WS><JOBID>[/<TASKID>]]...' format */
            ...
            while (ptr != NULL) {
                ...
                if (MJobFind(JobID, &J, 0) == SUCCESS) {
                    ...
                    N->DRes.Procs =
                        MIN(N->DRes.Procs + tmpProcs, N->CRes.Procs);

                    DBG(3, fPBS)
                    DPrint(
                        "INFO:     job %s adds %d processors per task to node "
                        "%s (%d)\n",
                        J->Name, J->Req[0]->DRes.Procs, N->Name, N->DRes.Procs);
                    ...
                    DBG(2, fPBS)
                    DPrint(
                        "ALERT:    cannot locate PBS job '%s' (running on node "
                        "%s)\n",
                        JobID, N->Name);
                ...
                if ((N->State == mnsIdle) || (N->State == mnsActive)) {
                    ...
                    if (N->State != OldState) {
                        ...
                        N->EState = N->State;
                    } /* END if ((N->State == mnsIdle) || (N->State ==
                         mnsActive)) */
                }     /* END while (ptr != NULL) */
    ...
    if ((N->State != N->EState) && (MSched.Time > N->SyncDeadLine)) {
        DBG(3, fPBS)
        DPrint(
            "INFO:     synchronizing node '%s' to expected state '%s' "
            "(expected state was '%s')\n",
...

{
    ...
    DPrint("%s(%s,%s,J,TaskList,%d)\n", FName, JobName,
           (PJob != NULL) ? PJob->name : "NULL", RMIndex);
    ...
    if (__MPBSJobGetState(PJob, &MRM[RMIndex], NULL, &J->State, &IsExiting) ==
        FAILURE) {
        DBG(1, fPBS)
        DPrint("ALERT:    cannot get job state info for job '%s'\n", J->Name);
    ...
    for (AP = PJob->attribs; AP != NULL; AP = AP->next) {
        DBG(6, fPBS)
        DPrint("INFO:     PBS attribute '%s'  value: '%s'  (r: %s)\n", AP->name,
    ...
    if ((J->Cred.U == NULL) || (J->Cred.G == NULL)) {
        DBG(1, fPBS)
        DPrint("ERROR:    cannot authenticate job '%s' (User: %s  Group: %s)\n",
    ...
    if (MJobSetCreds(J, J->Cred.U->Name, J->Cred.G->Name, AName) == FAILURE) {
        DBG(1, fPBS)
        DPrint(
            "ERROR:    cannot authenticate job '%s' (U: %s  G: %s  A: '%s')\n",
            J->Name, J->Cred.U->Name, J->Cred.G->Name, AName);
    ...
    if (MJOBISALLOC(J) && (J->StartTime == 0)) {
        ...
        if (TA.WallTime > 0) J->StartTime -= TA.WallTime;

        /* job start must be later than eligible time */

        if (TA.ETime > 0) J->StartTime = MAX(J->StartTime, TA.ETime);

        /* job start must be earlier than most recent modify time */

        if (TA.MTime > 0) J->StartTime = MIN(J->StartTime, TA.MTime);
...

{
    ...
    if (J->State != OldState) {
        DBG(1, fPBS)
        DPrint("INFO:     job '%s' changed states from %s to %s\n", J->Name,
    ...
    for (AP = PJob->attribs; AP != NULL; AP = AP->next) {
        if (!strcmp(AP->name, ATTR_qtime)) {
            ...
            if (!strcmp(AP->resource, "walltime")) {
                ...
                DBG(4, fPBS)
                DPrint("INFO:     walltime for job %s in state %s: %ld (%s)\n",
                       J->Name, MJobState[J->State], WallTime, AP->value);
            ...
            if ((tail != NULL) && (!isspace(*tail))) tmpL = 0;
            ...
            if (!strcmp(AP->resource, "walltime")) {
                ...
                if ((RIndex = MUMAGetIndex(eGRes, AP->value, mAdd)) == 0) {
                    ...
                    DBG(1, fPBS)
                    DPrint(
                        "ALERT:    cannot add support for GRes software '%s'\n",
                        AP->value);
                ...
                for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
                    if (J->Req[rqindex]->DRes.GRes[RIndex].count > 0) break;
                } /* END for (rqindex) */
                ...
                if (MReqCreate(J, NULL, &tmpRQ, FALSE) == FAILURE) {
                    DBG(1, fPBS)
                    DPrint(
                        "ALERT:    cannot add req to job %s for GRes software "
                        "'%s'\n",
                        J->Name, AP->value);
                ...
                /* NOTE:  PBS currently supports only one license request per
                 * job */
                ...
                /* MReqSetAttr(J,RQ,mrqaReqNodeFeature,(void
                 * **)AP->value,mdfString,mAdd); */
            } else {
                /* host     (master node)   not yet supported for PBS */
            }
        } else if (!strcmp(AP->name, ATTR_queue)) {
            MReqSetAttr(J, RQ, mrqaReqClass, (void **)AP->value, mdfString,
                        mSet);
        }
    } /* END for (AP) */
    ...
    if (MJOBISALLOC(J) && (J->StartTime <= 0)) {
        ...
        if (WallTime > 0) J->StartTime -= WallTime;

        if (ETime > 0) J->StartTime = MAX(J->StartTime, ETime);

        if (MTime > 0) J->StartTime = MIN(J->StartTime, MTime);
    ...
    DBG(4, fPBS)
    DPrint(
        "INFO:     job %s starttime: %ld (%s)  presenttime: %ld  wclimit: %ld  "
        "mtime: %ld  etime: %ld  walltime: %ld  state: %s\n",
        J->Name, J->StartTime, MULToTString(MSched.Time - J->StartTime),
        MSched.Time, J->SpecWCLimit[0], MTime, ETime, WallTime,
        MJobState[J->State]);
    ...
    if ((NodeCount > 1) && (NCPUs > 1)) RQ->DRes.Procs = 1;

/* NOTE:  on some systems, PBS is not aware of cputime consumption on non-master
 * nodes */
/*        on these systems, CPUTime should be scaled 'assuming' master node
 * usage is   */
/*        identical to usage on all other nodes */
    ...
    if (MJOBISACTIVE(J) || MJOBISCOMPLETE(J)) {
        ...
        if ((CPUPercent < 5) && (CPUTime > 0.1) && (WallTime > 0)) {
            /* PBS does not update 'cpupercent' (RQ->URes.Procs) quickly on some
             * systems */
...
int MPBSGetClassInfo(
    ...
    mnode_t *N, char CClass[][MAX_MNAME], char AClass[][MAX_MNAME])
...

{
    ...
    if (rc != 0) {
        ...
        DBG(0, fPBS)
        DPrint("ERROR:    PBS job '%s' cannot be requeued (rc: %d  '%s')\n",
               J->Name, rc, ErrMsg);
    ...
    if (MPBSJobModify(J, R, ATTR_l,
                      (char *)(R->Version >= 710 ? "select" : "neednodes"),
                      J->NeedNodes, NULL, NULL) == FAILURE) {
        DBG(7, fPBS)
        DPrint("WARNING:  cannot reset hostlist for job '%s')\n", J->Name);
    } else {
        DBG(7, fPBS)
        DPrint("INFO:     hostlist for job '%s' set to '%s'\n", J->Name,
...

{
    ...
    if (rc != 0) {
        ...
        DBG(0, fPBS)
        DPrint("ERROR:    PBS job '%s' cannot be checkpointed (rc: %d  '%s')\n",
               J->Name, rc, ErrMsg);
    ...
    if (rc != 0) {
        ...
        DBG(0, fPBS)
        DPrint(
            "ERROR:    PBS job '%s' cannot be released from hold (rc: %d  "
            "'%s')\n",
            J->Name, rc, ErrMsg);
...

{
    ...
    DPrint("%s(%s,%s,%s,%d)\n", FName, (J != NULL) ? J->Name : "NULL",
           ...
           (TaskList != NULL) ? "TaskList" : "NULL", IsExecList);
    ...
    if ((MJOBISACTIVE(J) == FALSE) && (IsExecList == FALSE)) {
        MReqSetAttr(J, RQ, mrqaReqNodeFeature, (void **)NULL, mdfString, mSet);
    }
    ...
    while (ptr != NULL) {
        ...
        if ((tail = strchr(tmpHostName, '/')) != NULL) *tail = '\0';
        ...
        while (ptr2 != NULL) {
            if (strncmp(ptr2, "ppn=", strlen("ppn=")) == 0) {
                ...
                MReqSetAttr(J, RQ, mrqaReqNodeFeature, (void **)ptr2, mdfString,
                            mAdd);
            }
        ...
        if ((tmpTaskCount != 0) && (*tail == '\0')) {
            ...
            if (PPNSet == TRUE) RQ->TasksPerNode = ppn;
            ...
            if (MNodeFind(tmpHostName, &N) == FAILURE) {
                DBG(1, fPBS)
                DPrint("ALERT:    cannot locate host '%s' for job hostlist\n",
                       tmpHostName);
    ...
    if ((tindex > 0) && (HLSet == TRUE) && (IsExecList == FALSE)) {
        ...
        if ((J->ReqHList == NULL) ||
            ...
            DBG(7, fPBS)
            DPrint("ALERT:    cannot allocate hostlist for job %s\n", J->Name);
        ...
        DBG(7, fPBS)
        DPrint("INFO:     required hostlist set for job %s (%s)\n", J->Name,
               J->ReqHList[0].N->Name);
    }
    ...
    if (rqindex > 0) {
        DBG(7, fPBS)
        DPrint("INFO:     %d task(s) located for job req\n",
               J->Req[0]->TaskCount);
    }
...

{
    ...
    __MPBSNLToTaskString(NL, R, tmpHList, sizeof(tmpHList));

    /* modify job */

    if (MPBSJobModify(J, R, "Resource_List",
                      (char *)(R->Version >= 710 ? "select" : "neednodes"),
                      tmpHList, EMsg, SC) == FAILURE) {
        /* clean up */
...

{
    ...
    if (rc != 0) {
        ...
        DBG(0, fPBS)
        DPrint(
            "WARNING:  cannot set job '%s' attr '%s:%s' to '%s' (rc: %d "
            "'%s')\n",
            tmpJName, Name, RPtr, Value, rc, ErrMsg);
...

{
    ...
    if (rc != 0) {
        ...
        DBG(0, fPBS)
        DPrint(
            "ERROR:    cannot stage data for job '%s' location '%s' (rc: %d "
            "'%s')\n",
            J->Name, Location, rc, ErrMsg);
...

{
    ...
    DBG(6, fPBS)
    DPrint("INFO:     PBS node attribute '%s'  value: '%s'  (r: %s)\n",
           AP->name, (AP->value != NULL) ? AP->value : "NULL",
           (AP->resource != NULL) ? AP->resource : "NULL");
 ...
 * @return a count of the numbers specified by the range string. Always at least
 * 1
 * because the value returned is only used with a valid node name, and we are
 * always
...
int MJobParsePBSExecHost(
    ...
    mjob_t *J, char *TaskString, short *TaskList, char *EMsg)

{
    ...
    if ((J == NULL) || (J->Req[0] == NULL) || (TaskString == NULL) ||
        (TaskList == NULL)) {
    ...
    /* NOTE:  ppn is 'processes per node' -> number of threads launched per
     * node/virtual node allocated */
    ...
    while (ptr != NULL) {
        ...
        /* Format: exec_host =
         * <node_id>/<proccessor>[+<node_id>/<proccessor>...] */
        ...
        /* As of Kilby, format is
           <node_id>/<processor_range>[+<node_id>/<processor_range[...]]
           processor range is defined in comments for
           MCountIntegersInRangeString */
        ...
        if (NoLoad == FALSE) {
            ...
            /* FIXME: for RequestIsProcCentric == FALSE same hosts belong in the
             * same task */
    ...
    if ((RQ->TaskCount == 0) && (OldTC != 0)) RQ->TaskCount = OldTC;
...

{
    ...
    if (A != NULL) {
        ...
        if ((AP->name == NULL) || (AP->value == NULL)) return (FAILURE);
    } /* END else (A != NULL) */
    ...
    if (!strcmp(AP->name, ATTR_qtime)) {
        ...
        DBG(6, fPBS)
        DPrint("INFO:     processing job %s exechost list '%s'\n", J->Name,
               AP->value);
        ...
        if (ptr != NULL) J->SIData->DstFileName = strdup(ptr);
        ...
        if (ptr != NULL) J->SOData->DstHostName = strdup(ptr);
        ...
        if (MJobProcessExtensionString(J, J->RMXString) == FAILURE) {
            DBG(1, fPBS)
            DPrint(
                "ALERT:    cannot process extension line '%s' for job %s "
                "(cancelling job)\n",
                (J->RMXString != NULL) ? J->RMXString : "NULL", J->Name);
    ...
    } else if (!strcmp(AP->name, ATTR_euser) || !strcmp(AP->name, ATTR_owner)) {
        /* get user name */
        ...
        if ((ptr = MUStrTok(AP->value, "@ \t\n", &TokPtr)) == NULL) {
            DBG(1, fPBS)
            DPrint("ERROR:    cannot add user for job %s (invalid name)\n",
                   J->Name);
        ...
        if ((J->Cred.U != NULL) && (!strcmp(AP->name, ATTR_euser))) {
            if (strcmp(J->Cred.U->Name, ptr)) {
                ...
                MJobSetHold(J, (1 << mhBatch), 0, mhrCredAccess,
                            "job not authorized to use proxy credentials");
            }
        }
    } else if (!strcmp(AP->name, ATTR_egroup)) {
        /* HvB
         * Only update job credentials when credentials are not set by
         * MGroupSecondary
        */
        if (J->Cred.G == NULL) {
            if (MGroupAdd(AP->value, &J->Cred.G) == FAILURE) {
                DBG(1, fPBS)
                DPrint("ERROR:    cannot add group for job %s (Name: %s)\n",
                       J->Name, AP->value);
    ...
    /* HvB
     * set group credential to the one specified via qsub paramter -W
     * group_list=<value>
    */
    {
        if (MPar[0].IgnPbsGroupList == FALSE) {
            ...
            DBG(3, fPBS)
            DPrint(
                "INFO:    ignoring PBS group_list(%s) attribute as requested\n",
                AP->value);
        }
    } else if (!strcmp(AP->name, ATTR_A)) {
        /* get account name */
        ...
        if (MAcctAdd(AP->value, &J->Cred.A) == FAILURE) {
            DBG(1, fPBS)
            DPrint("ERROR:    cannot add account for job %s (Name: %s)\n",
                   J->Name, AP->value);
        }
    } else if (!strcmp(AP->name, ATTR_used)) {
        if (!strcmp(AP->resource, "walltime")) {
        ...
        if (!strcmp(AP->resource, "neednodes") ||
            ...
            DBG(1, fPBS)
            DPrint("INFO:     processing node request line '%s'\n", AP->value);
            ...
            if ((J->Req[1] != NULL) && (MPar[0].EnableMultiReqJobs == FALSE)) {
                ...
                DBG(1, fPBS)
                DPrint(
                    "ALERT:    multi-req PBS job submitted (multi-req not "
                    "allowed)\n");

                MJobSetHold(J, (1 << mhBatch), 0, mhrPolicyViolation,
            ...
            if ((RIndex = MUMAGetIndex(eGRes, AP->value, mAdd)) == 0) {
                ...
                DBG(1, fPBS)
                DPrint("ALERT:    cannot add support for GRes software '%s'\n",
                       AP->value);
    ...
    if (TA != NULL) {
        if (R->U.PBS.PBS5IsEnabled == TRUE) {
            if (TA->NodesRequested > 1) {
                ...
                if (RQ->NodeCount == 1) RQ->NodeCount = 0;
    ...
    for (rqindex = 0; J->Req[rqindex] != NULL; rqindex++) {
        ...
        if (TA != NULL) {
            if ((TA->JobMemLimit > 0) && (RQ->DRes.Procs > 0))
                RQ->DRes.Mem =
                    MAX(RQ->DRes.Mem, TA->JobMemLimit / RQ->TaskCount);

            if ((TA->JobSwapLimit > 0) && (RQ->DRes.Procs > 0))
                RQ->DRes.Swap =
                    MAX(RQ->DRes.Swap, TA->JobSwapLimit / RQ->TaskCount);
        } /* END if (TA != NULL) */

        if (rqindex > 0) {
            if (RQ->DRes.Procs != 0) {
                /* NOTE:  currently only support one task type for proc based
                 * reqs */
        ...
        if ((J->ReqHList == NULL) &&
            (MPar[0].JobNodeMatch & (1 << nmExactNodeMatch))) {
        ...
        if ((RQ->TaskCount > 1) && (RQ->TasksPerNode > 1) &&
            (RQ->NodeCount <= 1)) {
            RQ->NodeCount = MAX(1, RQ->TaskCount / RQ->TasksPerNode);
        }

        if ((RQ->NodeCount > 1) && (RQ->DRes.Procs > 1)) RQ->DRes.Procs = 1;
...
/* obtain 'per task' statistics */
...
#ifdef __LINUX
            if (TA != NULL) TA->UtlJobCPUTime *= RQ->TaskCount;
#endif /* __LINUX */

            if (RQ->TaskCount > 0) {
                if (RQ->URes.Procs > 0) RQ->URes.Procs /= RQ->TaskCount;

                if ((TA != NULL) && (TA->UtlJobCPUTime > 0) &&
                    (RQ->RMWTime > 0)) {
                    RQ->LURes.Procs =
                        (int)(TA->UtlJobCPUTime * 100.0 / RQ->TaskCount);
                } else {
                    RQ->LURes.Procs = 100;
                }
            }
            ...
            if ((RQ->URes.Procs < 5) && (RQ->LURes.Procs > 0.1) &&
                (RQ->RMWTime > 0)) {
                RQ->URes.Procs =
                    MAX(RQ->URes.Procs, RQ->LURes.Procs / RQ->RMWTime);
            }
        }
    } /* END for (rqindex) */
...

{
    ...
    rc = (*X.XRMJobSubmit)(X.xd, SubmitString, R, J, JobName, Msg, SC);
...

{
    ...
    while (ptr != NULL) {
        ...
        if ((Name = MUStrTok(optr, "=", &TokPtr2)) == NULL) continue;

        if ((Value = MUStrTok(NULL, "=", &TokPtr2)) == NULL) continue;

        if ((Value[0] == '\0') || (Value[0] == '?')) continue;

        /* NOTE:  also available:  uname, sessions, nsessions, nusers, size,
         * idletime, resi */
        ...
        if (!strcmp(Name, "arch")) {
            ...
            if (*tail == '\0') N->Load = tmpD;
        } /* END else if (!strcmp(Name,"loadave")) */
        else if (!strcmp(Name, "message")) {
            if (!strncmp(Value, "ERROR:", strlen("ERROR:"))) {
                DBG(7, fPBS)
                DPrint(
                    "INFO:     node '%s' marked down - reports internal error "
                    "'%s'\n",
                    N->Name, Value + strlen("ERROR:"));
...

{
    ...
    for (tindex = 0; NL[tindex].N != NULL; tindex++) {
        ...
        DBG(7, fPBS)
        DPrint("INFO:     checking node '%s' for tasklist\n", N->Name);

        if (N->RM != R) continue;

        if (TSBuf[0] != '\0') MUStrCat(TSBuf, "+", BufSize);
        ...
        if (NL[tindex].TC == 1) {
            ...
            snprintf(tmpTaskList, sizeof(tmpTaskList), "%s:ppn=%d", tmpHostName,
                     NL[tindex].TC);
...

{
    ...
    for (AP = PJob->attribs; AP != NULL; AP = AP->next) {
        if (!strcmp(AP->name, ATTR_queue)) break;
    }
    ...
    if (C == NULL) {
        return (FAILURE); /* must be a brand new queue, we'll find it next
                             iteration */
    }
...
int MPBSJobSetAttr(
    ...
    mjob_t *J, void *A, char *ValLine, tpbsa_t *TA, short *TaskList, int Mode)
