...

{
    ...
    DBG(6, fSCHED)
    DPrint("%s(%s,%d,%s,%s)\n", FName, (J != NULL) ? J->Name : "[NONE]", PIndex,
    ...
    if (X.XJobGetStartPriority != (int (*)())0) {
        return (*X.XJobGetStartPriority)(X.xd, J, Priority,
                                         (BPtr != NULL) ? *BPtr : NULL);
    }
    ...
    if ((J != NULL) && (J->Cred.Q != NULL)) {
        ...
        MinWCLimit =
            (F->XFMinWCLimit != -1) ? F->XFMinWCLimit : GF->XFMinWCLimit;
    } else {
        MinWCLimit = 0;
    }

    if ((J != NULL) && (J->Cred.G != NULL)) {
        if (J->Cred.G->ClassWeight > 0) SWeight[mpsCC] = J->Cred.G->ClassWeight;
    }
    ...
    if (Mode == 1) {
        ...
        if (CWeight[mpcCred] != 0) {
            ...
            for (index = mpsCU; index <= mpsCC; index++) {
                ...
                switch (index) {
                    case mpsCU:
                        strcat(tmpHeader, " User");
                        break;
                    case mpsCG:
                        strcat(tmpHeader, "Group");
                        break;
                    case mpsCA:
                        strcat(tmpHeader, "Accnt");
                        break;
                    case mpsCC:
                        strcat(tmpHeader, "Class");
                        break;
                    case mpsCQ:
                        strcat(tmpHeader, "  QOS");
                        break;
                }

                sprintf(temp_str, "%5ld", (long)SWeight[index]);
                strcat(tmpCWLine, temp_str);
            } /* END for (index) */

            if (tmpHeader[0] != '\0') {
                sprintf(CHeader[mpcCred], "  Cred(%s)", tmpHeader);

                sprintf(CWLine[mpcCred], " %5ld(%s)", CWeight[mpcCred],
                        tmpCWLine);
            }
        ...
        if (CWeight[mpcFS] != 0) {
            ...
            for (index = mpsFU; index <= mpsFC; index++) {
                ...
                sprintf(temp_str, "%5ld", SWeight[index]);
                strcat(tmpCWLine, temp_str);
            } /* END for (index) */

            if (tmpHeader[0] != '\0') {
                sprintf(CHeader[mpcFS], "    FS(%s)", tmpHeader);

                sprintf(CWLine[mpcFS], " %5ld(%s)", CWeight[mpcFS], tmpCWLine);
            }
        ...
        if (CWeight[mpcServ] != 0) {
            ...
            for (index = mpsSQT; index <= mpsSBP; index++) {
                ...
                switch (index) {
                    case mpsSQT:
                        strcat(tmpHeader, "QTime");
                        break;
                    case mpsSXF:
                        strcat(tmpHeader, "XFctr");
                        break;
                    case mpsSSPV:
                        strcat(tmpHeader, "SPVio");
                        break;
                    case mpsSBP:
                        strcat(tmpHeader, "Bypas");
                        break;
                }
            ...
            if (tmpHeader[0] != '\0') {
                sprintf(CHeader[mpcServ], "  Serv(%s)", tmpHeader);

                sprintf(CWLine[mpcServ], " %5ld(%s)", CWeight[mpcServ],
                        tmpCWLine);
            }
        ...
        if (CWeight[mpcTarg] != 0) {
            ...
            if (tmpHeader[0] != '\0') {
                sprintf(CHeader[mpcTarg], "  Targ(%s)", tmpHeader);

                sprintf(CWLine[mpcTarg], " %5ld(%s)", CWeight[mpcTarg],
                        tmpCWLine);
            }
        ...
        if (CWeight[mpcRes] != 0) {
            ...
            for (index = mpsRNode; index <= mpsRWallTime; index++) {
                ...
                switch (index) {
                    case mpsRNode:
                        strcat(tmpHeader, " Node");
                        break;
                    case mpsRProc:
                        strcat(tmpHeader, " Proc");
                        break;
                    case mpsRMem:
                        strcat(tmpHeader, "  Mem");
                        break;
                    case mpsRSwap:
                        strcat(tmpHeader, " Swap");
                        break;
                    case mpsRDisk:
                        strcat(tmpHeader, " Disk");
                        break;
                    case mpsRPS:
                        strcat(tmpHeader, "   PS");
                        break;
                    case mpsRPE:
                        ...
                        break;
                    case mpsRUProc:
                        strcat(tmpHeader, "UProc");
                        break;
                    case mpsRUJob:
                        strcat(tmpHeader, " UJob");
                        break;
                    case mpsRWallTime:
                        strcat(tmpHeader, "WTime");
                        break;
                }
            ...
            if (tmpHeader[0] != '\0') {
                sprintf(CHeader[mpcRes], "   Res(%s)", tmpHeader);

                sprintf(CWLine[mpcRes], " %5ld(%s)", CWeight[mpcRes],
                        tmpCWLine);
            }
        ...
        if (CWeight[mpcUsage] != 0) {
            ...
            for (index = mpsUCons; index <= mpsUPerC; index++) {
                ...
                switch (index) {
                    case mpsUCons:
                        strcat(tmpHeader, "Cons ");
                        break;
                    case mpsURem:
                        strcat(tmpHeader, "Rem  ");
                        break;
                    case mpsUPerC:
                        strcat(tmpHeader, "PerC ");
                        break;
                }
            ...
            if (tmpHeader[0] != '\0') {
                sprintf(CHeader[mpcUsage], "   Res(%s)", tmpHeader);

                sprintf(CWLine[mpcUsage], " %5ld(%s)", CWeight[mpcUsage],
                        tmpCWLine);
            }
        } /* END if (CWeight[mpcUsage] != 0) */

        MUSNPrintF(BPtr, BSpace, "%-20s %10s%c %*s%*s%*s%*s%*s%*s%*s\n", "Job",
                   "PRIORITY", '*', (int)strlen(CHeader[mpcCred]),
                   CHeader[mpcCred], (int)strlen(CHeader[mpcFS]),
                   CHeader[mpcFS], (int)strlen(CHeader[mpcAttr]),
                   CHeader[mpcAttr], (int)strlen(CHeader[mpcServ]),
                   CHeader[mpcServ], (int)strlen(CHeader[mpcTarg]),
                   CHeader[mpcTarg], (int)strlen(CHeader[mpcRes]),
                   CHeader[mpcRes], (int)strlen(CHeader[mpcUsage]),
                   CHeader[mpcUsage]);

        MUSNPrintF(BPtr, BSpace, "%20s %10s%c %*s%*s%*s%*s%*s%*s%*s\n",
                   "Weights", "--------", ' ', (int)strlen(CWLine[mpcCred]),
                   CWLine[mpcCred], (int)strlen(CWLine[mpcFS]), CWLine[mpcFS],
                   (int)strlen(CWLine[mpcAttr]), CWLine[mpcAttr],
                   (int)strlen(CWLine[mpcServ]), CWLine[mpcServ],
                   (int)strlen(CWLine[mpcTarg]), CWLine[mpcTarg],
                   (int)strlen(CWLine[mpcRes]), CWLine[mpcRes],
                   (int)strlen(CWLine[mpcUsage]), CWLine[mpcUsage]);
    ...
    if (Mode == 2) {
        ...
        for (cindex = mpcServ; cindex <= mpcUsage; cindex++) {
            ...
            if (CWeight[cindex] != 0) {
                ...
                for (sindex = sindex1; sindex <= sindex2; sindex++) {
                    ...
                    sprintf(tmpS, "%3.1lf",
                            (TotalPriority != 0.0)
                                ? ABS(((double)TotalSFactor[sindex] *
                                       SWeight[sindex] * CWeight[cindex] *
                                       100.0 / TotalPriority))
                                : 0.0);

                    sprintf(temp_str, "%5.5s", tmpS);
                ...
                if (tmpLine[0] != '\0') {
                    sprintf(tmpS, "%3.1lf",
                            (TotalPriority != 0.0)
                                ? ABS(((double)TotalCFactor[cindex] *
                                       CWeight[cindex] * 100.0 / TotalPriority))
                                : 0.0);

                    sprintf(CFooter[cindex], " %5.5s(%s)", tmpS, tmpLine);
                }
            }
        } /* END for (cindex) */
        ...
        MUSNPrintF(BPtr, BSpace, "%-20s %10s%c %*s%*s%*s%*s%*s%*s%*s\n",
                   ...
                   (int)strlen(CFooter[mpcCred]), CFooter[mpcCred],
                   (int)strlen(CFooter[mpcFS]), CFooter[mpcFS],
                   (int)strlen(CFooter[mpcAttr]), CFooter[mpcAttr],
                   (int)strlen(CFooter[mpcServ]), CFooter[mpcServ],
                   (int)strlen(CFooter[mpcTarg]), CFooter[mpcTarg],
                   (int)strlen(CFooter[mpcRes]), CFooter[mpcRes],
                   (int)strlen(CFooter[mpcUsage]), CFooter[mpcUsage]);
    ...
    if (J->SpecWCLimit[0] == 0) {
        DBG(0, fSCHED)
        DPrint("ERROR:    job '%s' has NULL WCLimit field\n", J->Name);
    }
    ...
    XFactor = (double)(((unsigned long)(MSched.Time - EffQTime) +
                        J->SpecWCLimit[0])) /
    ...
    if (J->Cred.Q != NULL) SFactor[mpsCQ] = J->Cred.Q->F.Priority;
    ...
    for (index = mpsCU; index <= mpsCC; index++) {
        SFactor[index] = (SCap[index] > 0) ? MIN(SFactor[index], SCap[index])
                                           : SFactor[index];
    ...
    if ((GF->FSPolicy != fspNONE) &&
        ...
        if (J->Cred.U != NULL) {
            if (J->Cred.U->F.FSTarget > 0.0) {
                ...
                FSMode = J->Cred.U->F.FSMode;
                       ...
                       (MSched.DefaultU->F.FSTarget > 0.0)) {
            ...
            if (FSTargetUsage > 0.0) {
                ...
                switch (FSMode) {
                    case mfstCeiling:
                        SFactor[mpsFU] = MIN(SFactor[mpsFU], 0.0);
                        break;
                    case mfstFloor:
                        SFactor[mpsFU] = MAX(SFactor[mpsFU], 0.0);
                        break;
                    case mfstCapAbs:
                        SFactor[mpsFU] = 0.0;
                        break;
        ...
        if (J->Cred.G != NULL) {
            if (J->Cred.G->F.FSTarget > 0.0) {
                ...
                FSMode = J->Cred.G->F.FSMode;
                       ...
                       (MSched.DefaultG->F.FSTarget > 0.0)) {
            ...
            if (FSTargetUsage > 0.0) {
                ...
                switch (FSMode) {
                    case mfstCeiling:
                        SFactor[mpsFG] = MIN(SFactor[mpsFG], 0.0);
                        break;
                    case mfstFloor:
                        SFactor[mpsFG] = MAX(SFactor[mpsFG], 0.0);
                        break;
                    case mfstCapAbs:
                        SFactor[mpsFG] = 0.0;
                        break;
        ...
        if ((J->Cred.A != NULL) && (J->Cred.A->F.FSTarget > 0.0)) {
            ...
            FSMode = J->Cred.A->F.FSMode;
                   ...
                   (MSched.DefaultA->F.FSTarget > 0.0)) {
        ...
        if (FSTargetUsage > 0.0) {
            ...
            switch (FSMode) {
                case mfstCeiling:
                    SFactor[mpsFA] = MIN(SFactor[mpsFA], 0.0);
                    break;
                case mfstFloor:
                    SFactor[mpsFA] = MAX(SFactor[mpsFA], 0.0);
                    break;
                case mfstCapAbs:
                    SFactor[mpsFA] = 0.0;
                    break;
        ...
        if ((J->Cred.C != NULL) && (J->Cred.C->F.FSTarget > 0.0)) {
            ...
            FSMode = J->Cred.C->F.FSMode;
                   ...
                   (MSched.DefaultC->F.FSTarget > 0.0)) {
        ...
        if (FSTargetUsage > 0.0) {
            ...
            switch (FSMode) {
                case mfstCeiling:
                    SFactor[mpsFC] = MIN(SFactor[mpsFC], 0.0);
                    break;
                case mfstFloor:
                    SFactor[mpsFC] = MAX(SFactor[mpsFC], 0.0);
                    break;
                case mfstCapAbs:
                    SFactor[mpsFC] = 0.0;
                    break;
        ...
        if ((J->Cred.Q != NULL) && (J->Cred.Q->F.FSTarget > 0.0)) {
            ...
            FSMode = J->Cred.Q->F.FSMode;
                   ...
                   (MSched.DefaultQ->F.FSTarget > 0.0)) {
        ...
        if (FSTargetUsage > 0.0) {
            ...
            switch (FSMode) {
                case mfstCeiling:
                    SFactor[mpsFQ] = MIN(SFactor[mpsFQ], 0.0);
                    break;
                case mfstFloor:
                    SFactor[mpsFQ] = MAX(SFactor[mpsFQ], 0.0);
                    break;
                case mfstCapAbs:
                    SFactor[mpsFQ] = 0.0;
                    break;
    ...
    if (J->Cred.Q != NULL) {
        if (J->Cred.Q->QTTarget > 0) {
            /* give exponentially increasing priority as we approach target
             * qtime */
            /* Equation:  (QTTarget - QTCurrent)^(-2) */

            SFactor[mpsTQT] =
                (double)pow((MAX(.0001, J->Cred.Q->QTTarget - EffQTime)), -2.0);
        }
        ...
        if (J->Cred.Q->XFTarget > 0.0) {
            ...
            SFactor[mpsTXF] =
                (double)pow((MAX(.0001, J->Cred.Q->XFTarget - XFactor)), -2.0);
        }
    }

    SFactor[mpsTQT] = (SCap[mpsTQT] > 0) ? MIN(SFactor[mpsTQT], SCap[mpsTQT])
                                         : SFactor[mpsTQT];
    SFactor[mpsTXF] = (SCap[mpsTXF] > 0) ? MIN(SFactor[mpsTXF], SCap[mpsTXF])
                                         : SFactor[mpsTXF];

    CFactor[mpcTarg] =
        SWeight[mpsTQT] * SFactor[mpsTQT] + SWeight[mpsTXF] * SFactor[mpsTXF];
    ...
    */
    ...
    for (index = mpcServ; index <= mpcUsage; index++) {
        CFactor[index] = (CCap[index] > 0) ? MIN(CFactor[index], CCap[index])
                                           : CFactor[index];
    ...
    if ((BPtr != NULL) && (*BPtr != NULL)) {
        ...
        for (index = 1; index < MAX_MPRIOCOMPONENT; index++) {
            if (APrio != 0.0)
                CP[index] = ABS((double)CWeight[index] * CFactor[index]) /
                            APrio * 100.0;
            else
                CP[index] = 0.0;
        ...
        for (index = 1; index < MAX_MPRIOSUBCOMPONENT; index++) {
            SP[index] = (APrio != 0.0)
                            ? ABS((double)SWeight[index] * SFactor[index]) /
                                  APrio * 100.0
                            : 0.0;
        ...
        if (CWeight[mpcCred] != 0) {
            ...
            if (tmpLine[0] != '\0') {
                sprintf(tmpS, "%3.1lf", CP[mpcCred]);

                sprintf(CLine[mpcCred], " %5.5s(%s)", tmpS, tmpLine);
            }
        } /* END if (CWeight[mpcCred] != 0) */
        ...
        if (CWeight[mpcFS] != 0) {
            ...
            if (tmpLine[0] != '\0') {
                sprintf(tmpS, "%3.1lf", CP[mpcFS]);

                sprintf(CLine[mpcFS], " %5.5s(%s)", tmpS, tmpLine);
            }
        } /* END if (CWeight[mpcFS] != 0) */
        ...
        if (CWeight[mpcAttr] != 0) {
            ...
            for (index = mpsAAttr; index <= mpsAState; index++) {
                ...
                tmpD =
                    (double)CWeight[mpcAttr] * SWeight[index] * SFactor[index];
            ...
            if (tmpLine[0] != '\0') {
                sprintf(tmpS, "%3.1lf", CP[mpcAttr]);

                sprintf(CLine[mpcAttr], " %5.5s(%s)", tmpS, tmpLine);
            }
        } /* END if (CWeight[mpcCred] != 0) */
        ...
        if (CWeight[mpcServ] != 0) {
            ...
            for (index = mpsSQT; index <= mpsSBP; index++) {
                ...
                tmpD =
                    (double)CWeight[mpcServ] * SWeight[index] * SFactor[index];
            ...
            if (tmpLine[0] != '\0') {
                sprintf(tmpS, "%3.1lf", CP[mpcServ]);

                sprintf(CLine[mpcServ], " %5.5s(%s)", tmpS, tmpLine);
            }
        } /* END if (CWeight[mpcServ] != 0) */
        ...
        if (CWeight[mpcTarg] != 0) {
            ...
            for (index = mpsTQT; index <= mpsTXF; index++) {
                ...
                tmpD =
                    (double)CWeight[mpcTarg] * SWeight[index] * SFactor[index];

                sprintf(tmpS, "%3.1lf", CP[mpcTarg]);
            ...
            if (tmpLine[0] != '\0') {
                ...
                sprintf(CLine[mpcTarg], " %5.5s(%s)", tmpS, tmpLine);
            }
        } /* END if (CWeight[mpcTarg] != 0) */
        ...
        if (CWeight[mpcRes] != 0) {
            ...
            for (index = mpsRNode; index <= mpsRWallTime; index++) {
                ...
                tmpD =
                    (double)CWeight[mpcRes] * SWeight[index] * SFactor[index];
            ...
            if (tmpLine[0] != '\0') {
                sprintf(tmpS, "%3.1lf", CP[mpcRes]);

                sprintf(CLine[mpcRes], " %5.5s(%s)", tmpS, tmpLine);
            }
        } /* END if (CWeight[mpcRes] != 0) */
        ...
        if (CWeight[mpcUsage] != 0) {
            ...
            for (index = mpsUCons; index <= mpsUPerC; index++) {
                ...
                tmpD =
                    (double)CWeight[mpcUsage] * SWeight[index] * SFactor[index];
            ...
            if (tmpLine[0] != '\0') {
                sprintf(tmpS, "%3.1lf", CP[mpcUsage]);

                sprintf(CLine[mpcUsage], " %5.5s(%s)", tmpS, tmpLine);
            }
        } /* END if (CWeight[mpcUsage] != 0) */

        MUSNPrintF(BPtr, BSpace, "%-20s %10.0lf%c %*s%*s%*s%*s%*s%*s%*s\n",
                   J->Name, Prio, (J->SystemPrio > 0) ? '*' : ' ',
                   (int)strlen(CLine[mpcCred]), CLine[mpcCred],
                   (int)strlen(CLine[mpcFS]), CLine[mpcFS],
                   (int)strlen(CLine[mpcAttr]), CLine[mpcAttr],
                   (int)strlen(CLine[mpcServ]), CLine[mpcServ],
                   (int)strlen(CLine[mpcTarg]), CLine[mpcTarg],
                   (int)strlen(CLine[mpcRes]), CLine[mpcRes],
                   (int)strlen(CLine[mpcUsage]), CLine[mpcUsage]);
    } /* END if (BPtr != NULL) */

    /* clip prio at min value */

    if ((GP->RejectNegPrioJobs == FALSE) &&
        (GP->EnableNegJobPriority == FALSE)) {
    ...
    DBG(3, fSCHED)
    DPrint("INFO:     job '%s' Priority: %8.0lf\n", J->Name, Prio);

    DBG(3, fSCHED)
    DPrint(
        "INFO:     Cred: %6.0lf(%04.1lf)  FS: %6.0lf(%04.1lf)  Attr: "
        "%6.0lf(%04.1lf)  Serv: %6.0lf(%04.1lf)  Targ: %6.0lf(%04.1lf)  Res: "
        "%6.0lf(%04.1lf)  Us: %6.0lf(%04.1lf)\n",
        (double)CWeight[mpcCred] * CFactor[mpcCred], CP[mpcCred],
        (double)CWeight[mpcFS] * CFactor[mpcFS], CP[mpcFS],
        (double)CWeight[mpcAttr] * CFactor[mpcAttr], CP[mpcAttr],
        (double)CWeight[mpcServ] * CFactor[mpcServ], CP[mpcServ],
        (double)CWeight[mpcTarg] * CFactor[mpcTarg], CP[mpcTarg],
        (double)CWeight[mpcRes] * CFactor[mpcRes], CP[mpcRes],
        (double)CWeight[mpcUsage] * CFactor[mpcUsage], CP[mpcUsage]);

    /* cap job start priority */

    if (Prio > (double)MAX_PRIO_VAL) Prio = (double)MAX_PRIO_VAL;
    ...
    DBG(5, fSCHED)
    DPrint("INFO:     job '%s'  priority: %8.2lf\n", J->Name, Prio);

    if (Priority != NULL) *Priority = Prio;
...

{
    ...
    for (index = 1; index < MAX_MPRIOCOMPONENT; index++) {
        if (F->PCW[index] != -1)
            strcat(Buffer, MUShowLArray(MParam[pServWeight + index - 1], PIndex,
                                        F->PCW[index]));
    } /* END for (index) */

    for (index = 1; index < MAX_MPRIOSUBCOMPONENT; index++) {
        if (F->PSW[index] != -1)
            strcat(Buffer, MUShowLArray(MParam[pSQTWeight + index - 1], PIndex,
                                        F->PSW[index]));
    } /* END for (index) */

    if (F->XFMinWCLimit != -1)
        strcat(Buffer, MUShowSArray(MParam[pXFMinWCLimit], PIndex,
                                    MULToTString(F->XFMinWCLimit)));

    for (index = 1; index < MAX_MPRIOCOMPONENT; index++) {
        if (F->PCC[index] > 0)
            strcat(Buffer, MUShowLArray(MParam[pServCap + index - 1], PIndex,
                                        F->PCC[index]));
    } /* END for (index) */

    for (index = 1; index < MAX_MPRIOSUBCOMPONENT; index++) {
        if (F->PSC[index] > 0)
            strcat(Buffer, MUShowLArray(MParam[pSQTCap + index - 1], PIndex,
                                        F->PSC[index]));
    } /* END for (index) */
