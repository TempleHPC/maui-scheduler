...

{
    ...
    if ((S->SyslogActive == FALSE) && (S->UseSyslog == TRUE)) {
        openlog(MSCHED_SNAME, LOG_PID | LOG_NDELAY,
                (S->SyslogFacility > 0) ? S->SyslogFacility : LOG_DAEMON);

        S->SyslogActive = TRUE;

        if (S->Mode == msmNormal) {
            MOSSyslog(LOG_INFO, "%s initialized", MSCHED_SNAME);
        }
    }
...

{
    ...
    if ((MGUSyslogActive == NULL) || (*MGUSyslogActive == FALSE)) {
        return (SUCCESS);
    }
...

{
    ...
    if (MFUGetAttributes(tmpFileName, NULL, NULL, NULL, &UID, &IsPrivate,
                         NULL) == SUCCESS) {
        if (IsPrivate == TRUE) {
            if (IsServer == TRUE) {
                if ((S != NULL) && (UID == S->UID)) {
...

{
    ...
    if (S != NULL) {
        if (S->X != NULL) {
            ...
            if ((X->XGetTime != (int (*)())0) &&
                ((S->Mode != msmSim) ||
                 ((S->TimePolicy != mtpReal) && (S->TimePolicy != mtpNONE)))) {
...

{
    ...
    const char *MTrueString[] = {"1", "true", "True", "TRUE",
                                 "on"
                                 "On",
                                 "ON", "yes", "Yes", "YES", NULL};

    const char *MFalseString[] = {"0",   "false", "False", "FALSE",
                                  "off", "Off",   "OFF",   "no",
                                  "No",  "NO",    NULL};
...

{
    ...
    if (CPrio != NULL) *CPrio = 0;

    if ((J == NULL) || (CPrio == NULL)) {
        return (FAILURE);
    }

    if (J->Cred.C != NULL) *CPrio = J->Cred.C->F.Priority;
...

{
    ...
    DBG(7, fSTRUCT)
    DPrint("INFO:     node %s state set to %s\n", N->Name, MNodeState[*State]);
...

{
    ...
    for (index = 0; MComp[index] != '\0'; index++) {
        ...
        if (strncmp(Line, MComp[index], Len) != 0) continue;
...

{
    ...
    DBG(9, fSTRUCT)
    DPrint("%s(%s,%s,%d)\n", FName, MAttrType[Attr],
           (Value != NULL) ? Value : "NULL", Mode);
    ...
    for (index = 1; index < MAX_MATTR; index++) {
        ...
        if (MAList[Attr][index][0] == '\0') break;
    } /* END for (index) */
    ...
    DBG(5, fSTRUCT)
    DPrint("INFO:     adding MAList[%s][%d]: '%s'\n", MAttrType[Attr], index,
           Value);
...
int MAttrSubset(
    ...
    int *AvlMap, int *ReqMap, int MapSize, int Mode)

{
    ...
    if (Mode == tlAND) {
        ...
        for (index = 0; index<MapSize>> 2; index++) {
            if (ReqMap[index] == 0) continue;

            ReqFound = TRUE;

            if ((AvlMap[index] & ReqMap[index]) != 0) return (SUCCESS);
        } /* END for (index) */
...

{
    ...
    DBG(9, fSTRUCT)
    DPrint("%s(%s,%s,%s,%d)\n", FName, MAttrType[AttrIndex],
           (AttrValue != NULL) ? AttrValue : "NULL",
           (SearchMode != mVerify) ? "ADD" : "CHECK", MapSize);
    ...
    for (index = 0; index < MapSize << 3; index++) {
        if (MAList[AttrIndex][index][0] == '\0') break;
    ...
    if (AttrMap != NULL) {
        ...
        DBG(5, fSTRUCT)
        DPrint("INFO:     added MAList[%s][%d]: '%s'\n", MAttrType[AttrIndex],
               index, AttrValue);
    }
...

{
    ...
    DBG(9, fSTRUCT)
    DPrint("%s(%s,%s,%s)\n", FName, MAttrType[AIndex], Value,
           (Mode == mVerify) ? "CHECK" : "ADD");

    for (index = 1; index < MAX_MATTR; index++) {
        if (!strcmp(MAList[AIndex][index], Value)) return (index);

        if (MAList[AIndex][index][0] == '\0') break;
    }
...

{
    ...
    DBG(6, fCONFIG)
    DPrint("%s(%s,'%s',%d)\n", FName, MAttrType[AIndex],
           (AttrLine != NULL) ? AttrLine : "NULL", Mode);
    ...
    DBG(5, fCONFIG)
    DPrint("INFO:     %s attributes '%s' set\n", MAttrType[AIndex],
...
int MUMAMAttrFromLine(
    ...
    int AttrIndex, char *AttrLine, int Mode, int *AttrMap, int MapSize)

{
    ...
    DBG(6, fCONFIG)
    DPrint("%s(%s,'%s',%d)\n", FName, MAttrType[AttrIndex], AttrLine, Mode);
...

{
    ...
    for (i = 1; i < (M64.INTBITS - 1); i++) {
        if ((Value & (1 << i)) && (MAList[Attr][i][0] != '\0')) {
            sprintf(temp_str, "[%s]", MAList[Attr][i]);
...
char *MAttrFind(
    ...
    char *SearchString, /* I */
    int AttrIndex, int *ValueMap, int MapSize, char **Head)
        ...
        for (index = 0; index < M64.INTBITS; index++) {
            if ((ValueMap[findex] & (1 << index)) &&
                ...
                if (tail != '$') {
                    ...
                    if (strncmp(MAList[AttrIndex][index], SearchString,
                                Len - 1) &&
...

{
    ...
    for (cindex = 1; cindex < MAX_MCLASS; cindex++) {
        if (MAList[eClass][cindex][0] == '\0') break;

        if (CClass != NULL) {
            if ((CClass[cindex].count > 0) || (AClass[cindex].count > 0)) {
                sprintf(temp_str, "[%s %d:%d]", MAList[eClass][cindex],
                        AClass[cindex].count, CClass[cindex].count);
            }
            strcat(ptr, temp_str);
        } else {
...
int MUParseComp(
    ...
    char *CString, char *AName, int *Cmp, char *ValLine)

{
    ...
    if ((*ptr == 'e') || (*ptr == 'n')) {
        ...
        if (Cmp != NULL) *Cmp = MUCmpFromString(tmpLine, NULL);
    }

    if (Cmp != NULL) {
        if (*Cmp == mcmpNE2) *Cmp = mcmpNE;
    }
...

{
    ...
    for (ptr = Head; *ptr != '\0'; ptr++) {
        ...
        if ((*ptr == '\"') && (EscapeMode == 0)) QuoteMode = 1 - QuoteMode;
...
int MUNLFromTL(
    ...
    mnalloc_t *NL, short *TL, int *NCount)
...

{
    ...
    const char *ResName[] = {"PROCS", "MEM", "SWAP", "DISK", NULL};
    ...
    for (index = 0; ResName[index] != NULL; index++) {
        ...
        if (Val > 0) {
            ...
            if (DisplayMode == 1) {
                ...
                sprintf(temp_str, "%s: %0.2lf", N, (double)tmpI / 100.0);
                strcat(Line, temp_str);
            } else if (DisplayMode == 2) {
                /* machine readable */

                sprintf(temp_str, "%s=%d", N, tmpI);
                strcat(Line, temp_str);
            } else {
                /* human readable - basic */

                if (index > 0) {
                    sprintf(temp_str, "%s: %s", N,
    ...
    for (index = 1; index < MAX_MGRES; index++) {
        if (R->GRes[index].count == 0) continue;

        if (Line[0] != '\0') MUStrCat(Line, "  ", MAX_MLINE);

        sprintf(temp_str, "%s: %d", MAList[eGRes][index], R->GRes[index].count);
        strcat(Line, temp_str);
    } /* END for (index) */
...
int MUREToList(
    ...
    char *Pattern, int ObjType, int PIndex, short *List, int *Count,
...

{
    ...
    DBG(3, fUI)
    DPrint("%s(%s,%s,%d,List,Count,Buffer)\n", FName, Pattern, MHRObj[ObjType],
           PIndex);
    ...
    if ((Buffer != NULL) && (Buffer[0] != '\0')) {
        ...
        DBG(4, fSTRUCT)
        DPrint("INFO:     checking for regex match with '%s'\n", Match);
    } else {
        Match[0] = '\0';
    }
    ...
    if (ObjType == mxoNode) {
        for (rxindex = 0; rxindex < MAX_RXCACHE; rxindex++) {
            if (rxcache[rxindex].Pattern[0] == '\0') break;

            if (!strcmp(rxcache[rxindex].Pattern, Pattern)) break;
        } /* END for (rxindex) */
    } else {
        rxindex = MAX_RXCACHE;
    }
    ...
    if (rxcache[rxindex].Pattern[0] == '\0') {
        ...
        if (ptr != NULL) {
            ...
            DBG(1, fUI)
            DPrint("ALERT:    cannot compile regular expression '%s'\n",
                   Pattern);

            if (Buffer != NULL) {
                sprintf(Buffer,
                        "ERROR:    cannot interpret regular expression '%s'\n",
                        Pattern);
            }
        ...
        rc = regcomp(re, Pattern,
                     REG_EXTENDED | REG_ICASE | REG_NEWLINE | REG_NOSUB);

        if (rc != 0) {
            DBG(1, fUI)
            DPrint("ALERT:    cannot compile regular expression '%s', rc: %d\n",
                   Pattern, rc);
    ...
    switch (ObjType) {
        case mxoNode:
            ...
            for (index = 0; index < MAX_MNODE; index++) {
                ...
                if ((PIndex > 0) && (N->PtIndex != PIndex)) continue;
                ...
                while ((ptr = strstr(ptr, N->Name)) != NULL) {
                    ...
                    if (((ptr > Pattern) && *(ptr - 1) == '+') &&
                        ...
                        DBG(3, fUI)
                        DPrint(
                            "INFO:     MNode[%03d] '%s' added to regex list\n",
                            LIndex, N->Name);
                ...
                if (!strcmp(Pattern, "ALL") ||
                    ...
                    ((N->FullName != NULL) &&
                     (regex(re, N->FullName, NULL) != NULL)))
#else  /* LIBGEN */
                if (!strcmp(Pattern, "ALL") ||
                    !regexec(re, N->Name, 0, NULL, 0) ||
                    ((N->FullName != NULL) &&
                     !regexec(re, N->FullName, 0, NULL, 0)))
#endif /* LIBGEN */
                {
                    if (Match[0] == '\0') {
                        List[LIndex++] = index;
...
/* DO NOTHING */
            ...
            if ((LIndex == 0) && (Match[0] == '\0')) {
                DBG(3, fUI)
                DPrint("INFO:     no matches found for node expression\n");

                if (Buffer != NULL) {
                    sprintf(temp_str,
                            "ERROR:    no matches found for node expression\n");
                    strcat(Buffer, temp_str);
                }
        ...
        case mxoJob:
            ...
            for (J = MJob[0]->Next; (J != NULL) && (J != MJob[0]);
                 J = J->Next) {
                DBG(6, fUI)
                DPrint("INFO:     comparing job '%s' against regex\n", J->Name);

                if (!strcmp(Pattern, "ALL") || !strcmp(Pattern, "^(ALL)$") ||
#ifdef LIBGEN
...
#endif /* LIBGEN */
                {
                    if (Match[0] == '\0') {
                        DBG(3, fUI)
                        DPrint("INFO:     job '%s' added to regex list\n",
                               J->Name);

                        DBG(6, fUI)
                        DPrint("INFO:     List[%02d]: '%s'\n", LIndex, J->Name);
                        ...
                        if (Buffer != NULL) strcpy(Buffer, J->Name);
...
/* DO NOTHING */
            ...
            if ((LIndex == 0) && (Match[0] == '\0')) {
                DBG(3, fUI)
                DPrint("INFO:     no matches found for job expression\n");

                if (Buffer != NULL) {
                    sprintf(temp_str,
                            "ERROR:    no matches found for job expression\n");
                    strcat(Buffer, temp_str);
                }
...
#ifdef LIBGEN
                if (!strcmp(Pattern, "ALL") ||
                    (regex(re, R->Name, NULL) != NULL))
#else  /* LIBGEN */
                if (!strcmp(Pattern, "ALL") ||
                    !regexec(re, R->Name, 0, NULL, 0))
#endif /* LIBGEN */
                {
                    if (Match[0] == '\0') {
                        List[LIndex++] = index;
                        ...
                        DBG(3, fUI)
                        DPrint("INFO:     res '%s' added to regex list\n",
                               R->Name);
                        ...
                        if (Buffer != NULL) strcpy(Buffer, R->Name);
...
/* DO NOTHING */
        ...
        default:
            ...
            DBG(0, fUI)
            DPrint("ERROR:    unexpected object type '%d' passed to %s()\n",
                   ObjType, FName);
...

{
    ...
    if (*ptr == ':') /* time resource -> convert to seconds */
    {
        ...
        if (*ptr2 == ':') val += strtol(ptr2 + 1, &ptr, 10); /* seconds */
    ...
    if ((ptr[0] != '\0') &&
        ((ptr[1] == ModC1[mvmWord]) || (ptr[1] == ModC2[mvmWord]))) {
        /* adjust for word */
...

{
    ...
    if ((String[4] == '0') && (String[3] == ' ')) String[3] = '0';
    ...
    for (index = 3; index >= 0; index--) {
        if (String[index] == ' ') return (&String[index + 1]);
    }
    ...
    for (aindex = AStart; ArgV[aindex] != NULL; aindex++) {
        ...
        if (*(ptr + 1) == ':') {
            ...
            if (ArgV[aindex][2] != '\0') {
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     arg '%s' found for flag '%c'\n",
                       &ArgV[aindex][2], (char)flag);

                strncpy(ArgVal, &ArgV[aindex][2], sizeof(ArgVal));
                ArgVal[sizeof(ArgVal) - 1] = '\0';

                *OptArg = ArgVal;
            } else if ((aindex < *ArgC) && (ArgV[aindex + 1] != NULL) &&
                       (ArgV[aindex + 1][0] != '-')) {
                /* if flag value contained in next arg */

                DBG(3, fCONFIG)
                DPrint("INFO:     arg '%s' found for flag '%c'\n",
                       ArgV[aindex + 1], (char)flag);
                ...
                *OptArg = ArgVal;
                ...
                DBG(3, fCONFIG)
                DPrint("INFO:     expected arg not supplied for flag '%c'\n",
                       flag);
...

{
    ...
    for (index = 0; EnvVal[index][0][0] != '\0'; index++) {
        if (!strcmp(Var, EnvVal[index][0])) {
            sprintf(EnvVal[index][1], "%s=%s", Var, Value);
...
int MUGetTokens(
    ...
    char **Line, short *TokList, char *TypeList, char **ValList)
...

{
    ...
    if (D->Size <= D->Count) {
        if ((D->Data = (char *)realloc(
                 D->Data, ((D->Size + DSTAT_STEPSIZE) * D->DSize))) == NULL) {
...
#else  /* __LINUX */
    Offset = Time->tm_hour * 3600 + Time->tm_min * 60 + Time->tm_sec;
#endif /* __LINUX */
...

{
    ...
    for (i = 1; i < M64.INTBITS; i++) {
        if ((BM & (1 << i)) && (AList[i] != NULL) && (AList[i][0] != '\0')) {
            if (Delim == '[') {
                sprintf(temp_str, "[%s]", AList[i]);
                strcat(ptr, temp_str);
            } else {
                if (ptr[0] != '\0') strcat(ptr, DS);
    ...
    if ((ptr[0] == '\0') && (EString != NULL)) strcpy(ptr, EString);
...
int MUBMFromString(
    ...
    char *Line, const char *StringArray[], unsigned long *BM)
...
char *MUCResRatioToString(
    ...
    mcres_t *DRes, mcres_t *BRes, mcres_t *CRes, int RC)

{
    ...
    for (index = 0; ResName[index] != NULL; index++) {
        ...
        if (BVal == -1) BVal = TotalRes;

        if (TotalRes > 0) {
            if (Line[0] != '\0') MUStrCat(Line, "  ", sizeof(Line));

            sprintf(temp_str, "%s: %d/%d (%.2f%c)", N, BVal, TotalRes,
...
char *MUShowIArray(
    ...
    const char *Parm, int PIndex, int Val)
...
char *MUShowLArray(
    ...
    const char *Parm, int PIndex, long Val)
...
char *MUShowSArray(
    ...
    const char *Parm, int PIndex, char *Val)
...
char *MUShowFArray(
    ...
    const char *Parm, int PIndex, double Val)
...
int MUCResGetMin(
    ...
    mcres_t *R, mcres_t *Res1, mcres_t *Res2)

{
    ...
    for (index = 1; index < MAX_MCLASS; index++) {
        ...
        R->PSlot[index].count =
            MIN(Res1->PSlot[index].count, Res2->PSlot[index].count);
    ...
    for (index = 1; index < MAX_MCLASS; index++) {
        ...
        R->GRes[index].count =
            MIN(Res1->GRes[index].count, Res2->GRes[index].count);
...

{
    ...
    for (index = 1; index < MAX_MCLASS; index++) {
        ...
        tmp1 =
            (Res1->PSlot[index].count < 32000) ? Res1->PSlot[index].count : 0;
        tmp2 =
            (Res2->PSlot[index].count < 32000) ? Res2->PSlot[index].count : 0;
    ...
    for (index = 1; index < MAX_MGRES; index++) {
        ...
        R->GRes[index].count =
            MAX(Res1->GRes[index].count, Res2->GRes[index].count);
        R->GRes[0].count += R->GRes[index].count;
    } /* END for (index) */
...
{
    if ((R->Procs < 0) || (R->Mem < 0) || (R->Disk < 0) || (R->Swap < 0)) {
        return (SUCCESS);
    }

    if ((R->PSlot[0].count < 0) || (R->GRes[0].count < 0)) {
        return (SUCCESS);
    }
...
{
    fprintf(stderr,
            "This software utilizes the Moab Scheduling Library, version %s\n",
            MOAB_VERSION);
...

{
    ...
    for (sindex = 0; sindex < slen; sindex++) {
        ...
        if (Pattern[pindex] == '\0') return (&String[sindex]);
    } /* END for (index) */
...
int MUSNCat(char **BPtr, int *BSpace, char *Src) {
    return (MUStrNCat(BPtr, BSpace, Src));
}

int MUStrNCat(

    char **BPtr, int *BSpace, char *Src)
...

{
    ...
    for (index = 0; TM[index] != -1; index++) {
        if (MNode[TM[index]]->RM == R) {
            ...
            DBG(6, fLL)
            DPrint("INFO:     TM[%02d] %s\n", tindex, HostList[tindex]);
...

{
    ...
    if (Msg != NULL) MXMLSetAttr(C, "message", (void **)Msg, mdfString);
...

{
    ...
    if ((Protocol != NULL) && (Protocol[0] != '\0')) {
        MUSNPrintF(&BPtr, &BSpace, "%s://%s", Protocol, HostName);
    } else {
        MUSNPrintF(&BPtr, &BSpace, "%s", HostName);
    }

    if (Port > 0) {
        MUSNPrintF(&BPtr, &BSpace, ":%d", Port);
    }

    if ((Directory != NULL) && (Directory[0] != '\0')) {
        MUSNPrintF(&BPtr, &BSpace, "%s%s", (Directory[0] != '/') ? "/" : "",
                   Directory);
    }
    ...
    for (findex = 0; findex < (MapSize >> M64.INTSHIFT); findex++) {
        for (index = 0; index < M64.INTBITS; index++) {
            if ((ValueMap[findex] & (1 << index)) &&
                ...
                if (Delim != '\0') {
                    if (Line[0] != '\0') {
                        sprintf(temp_str, "%c%s", Delim, ptr);
                        strcat(Line, temp_str);
                    } else {
                        strcpy(Line, ptr);
                    }
                } else {
                    sprintf(temp_str, "[%s]", ptr);
                    strcat(Line, temp_str);
                }
            }
        } /* END for (index) */
...

{
    ...
    for (cindex = 1; cindex < MMAX_CLASS; cindex++) {
        if (MAList[meClass][cindex][0] == '\0') break;

        if (CNL != NULL) {
            if ((CNL[cindex].count > 0) || (ANL[cindex].count > 0)) {
                /* NOTE:  DString is ignored */

                MUSNPrintF(&BPtr, &BSpace, "[%s %d:%d]",
                           MAList[meClass][cindex], ANL[cindex].count,
                           CNL[cindex].count);
            }
        } else {
            if (ANL[cindex].count > 0) {
                if (DString != NULL) {
                    ...
                    if (Head[0] != '\0') MUStrNCat(&BPtr, &BSpace, DString);
